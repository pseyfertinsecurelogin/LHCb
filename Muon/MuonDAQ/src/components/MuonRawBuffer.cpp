...
//=============================================================================
MuonRawBuffer::MuonRawBuffer( const std::string& type, const std::string& name, const IInterface* parent )
...
void MuonRawBuffer::clearData() {
  ...
  for ( unsigned int i = 0; i < MuonDAQHelper_maxODENumber; i++ ) { m_ODEData[i].resetODEData(); }
}

StatusCode MuonRawBuffer::finalize() {

  auto any_error = []( const auto& container ) {
    return std::any_of( begin( container ), end( container ), []( const auto& i ) { return i != 0; } );
  };
  bool print_ZS = any_error( m_counter_invalid_hit ) || any_error( m_pad_checkSize ) || any_error( m_hit_checkSize );

  bool print_NZS = any_error( m_NZScounter_invalid_hit );

  bool print_errors = any_error( m_tell1_header_error ) || any_error( m_tell1_header_ORODE_error ) ||
                      any_error( m_tell1_header_SYNCH_data_error ) || any_error( m_tell1_header_SYNCH_BC_error ) ||
                      any_error( m_tell1_header_SYNCH_Evt_error );

  if ( print_ZS || print_NZS || print_errors ) { info() << " Summary of Muon banks decoding errors " << endmsg; }
  ...
  if ( print_ZS ) {
    ...
    for ( unsigned int i = 0; i < MuonDAQHelper_maxTell1Number; i++ ) {
      if ( m_counter_invalid_hit[i] > 0 ) {
        ...
        info() << " in Tell1 " << i << " invalid hit address " << m_counter_invalid_hit[i] << " processed banks # "
               << m_processed_bank[i] << endmsg;
      }
      if ( m_pad_checkSize[i] > 0 ) {
        info() << " in Tell1 pad decoding " << i << " invalid bank size " << m_pad_checkSize[i] << " processed banks # "
               << m_processed_bank[i] << endmsg;
      }
      if ( m_hit_checkSize[i] > 0 ) {
        info() << " in Tell1 hit decoding " << i << " invalid bank size " << m_hit_checkSize[i] << " processed banks # "
               << m_processed_bank[i] << endmsg;
      }
    }
  }
  ...
  if ( print_errors ) {
    for ( unsigned int i = 0; i < MuonDAQHelper_maxTell1Number; i++ ) {
      ...
      if ( m_tell1_header_SYNCH_BC_error[i] > 0 ) {
        info() << " in Tell1 " << i << " SYNCH BC consistency  error " << m_tell1_header_SYNCH_BC_error[i] << endmsg;
      }
      if ( m_tell1_header_SYNCH_Evt_error[i] > 0 ) {

        info() << " in Tell1 " << i << " SYNCH L0Evt consistency  error " << m_tell1_header_SYNCH_BC_error[i] << endmsg;
      }
    }
  }
  ...
  if ( print_NZS ) {
    ...
    for ( unsigned int i = 0; i < MuonDAQHelper_maxTell1Number; i++ ) {
      info() << " in Tell1 " << i << " invalid hit address " << m_NZScounter_invalid_hit[i] << " processed banks # "
...
StatusCode MuonRawBuffer::getTile( std::vector<LHCb::MuonTileID>& storage ) {
  ...
  return raw ? getTile( raw, storage, rootInTES() ) : Error( "Failed to find raw data" );
}

StatusCode MuonRawBuffer::getTileAndTDC( std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& storage ) {
  LHCb::RawEvent* raw = findFirstRawEvent();
  return raw ? getTileAndTDC( raw, storage, rootInTES() ) : Error( "Failed to find raw data" );
}
...
StatusCode MuonRawBuffer::decodeTileAndTDCDC06( const RawBank* rawdata ) {
  ...
  for ( itODE = 0; itODE < nODE; itODE++ ) {
    ...
    unsigned int odenumber = ( m_muonDet->getDAQInfo() )->getODENumberInTell1( ( rawdata )->sourceID(), itODE );
    for ( channels = 0; channels < firedInODE[itODE]; channels++ ) {
      unsigned int address = *it;
      MuonTileID   tile    = ( m_muonDet->getDAQInfo() )->getADDInODENoHole( odenumber - 1, address );
...
StatusCode MuonRawBuffer::decodePadsV1( const LHCb::RawBank* r ) {
  ...
  if ( (unsigned int)r->sourceID() >= m_M1Tell1 ) {
    ...
    std::transform( first, first + npads, std::back_inserter( m_padStorage[tell] ),
                    [&]( short addr ) -> MuonTileID { return di->getPadInTell1V1( tell, addr ); } );
  }
  return StatusCode::SUCCESS;
}
...
StatusCode MuonRawBuffer::decodePadsDC06( const LHCb::RawBank* r ) {
  ...
  if ( tell >= m_M1Tell1 ) {
    ...
    unsigned int npads = *first++;
    std::transform( first, first + npads, std::back_inserter( m_padStorage[tell] ),
                    [&]( const short s ) -> MuonTileID { return di->getPadInTell1DC06( tell, s ); } );
  }
  return StatusCode::SUCCESS;
}
...
StatusCode MuonRawBuffer::getTile( const LHCb::RawBank* r, std::vector<LHCb::MuonTileID>& tile ) {
  ...
  std::transform( m_storage[tell1Number].begin(), m_storage[tell1Number].end(), std::back_inserter( tile ),
                  []( const std::pair<LHCb::MuonTileID, unsigned int>& p ) { return p.first; } );
  return StatusCode::SUCCESS;
}

StatusCode MuonRawBuffer::getTile( const LHCb::RawEvent* raw, std::vector<LHCb::MuonTileID>& storage,
  ...
  if ( !sc.isFailure() ) {
    ...
    for ( const auto& bank : b ) {
      ...
      std::transform( m_storage[tell1Number].begin(), m_storage[tell1Number].end(), std::back_inserter( storage ),
                      []( const std::pair<LHCb::MuonTileID, unsigned int>& p ) { return p.first; } );
    }
  }
  putStatusOnTES();
...
StatusCode MuonRawBuffer::getTileAndTDC( const LHCb::RawEvent*                                   raw,
  ...
  if ( !sc.isFailure() ) {
    ...
    for ( const auto& r : mb ) {
      unsigned int tell1Number = r->sourceID();
      std::copy( m_storage[tell1Number].begin(), m_storage[tell1Number].end(), std::back_inserter( storage ) );
    }
  }
...
StatusCode MuonRawBuffer::getTileAndTDC( const LHCb::RawBank*                                    r,
                                         std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& tile )

{
  ...
  // copy in output container
  tile.reserve( m_storage[tell1Number].size() );
  std::copy( m_storage[tell1Number].begin(), m_storage[tell1Number].end(), std::back_inserter( tile ) );
  return StatusCode::SUCCESS;
}

StatusCode MuonRawBuffer::getPads( const LHCb::RawEvent* raw, std::vector<LHCb::MuonTileID>& pads,
                                   const std::string& offset ) {
  ...
  if ( !sc.isFailure() ) {
    ...
    for ( const auto& rb : b ) {
      ...
      std::copy( m_padStorage[tell1Number].begin(), m_padStorage[tell1Number].end(), std::back_inserter( pads ) );
    }
  }
...
StatusCode MuonRawBuffer::getPads( const LHCb::RawBank* r, std::vector<LHCb::MuonTileID>& pads ) {
  ...
  unsigned int tell = r->sourceID();
  std::copy( m_padStorage[tell].begin(), m_padStorage[tell].end(), std::back_inserter( pads ) );
  return StatusCode::SUCCESS;
}
...
StatusCode MuonRawBuffer::decodeNZSupp( int tell1Number ) {
  ...
  auto i = std::find_if( b.begin(), b.end(), [&]( const RawBank* rb ) { return rb->sourceID() == tell1Number; } );
...
StatusCode MuonRawBuffer::decodeNZSupp( const LHCb::RawBank* r ) {
  ...
  std::array<std::array<unsigned int, 35>, 24> data;
...
StatusCode MuonRawBuffer::getNZSupp( const LHCb::RawBank*                                    r,
    ...
    if ( ODE_num > 0 ) {
      ...
      for ( unsigned int iData = 1; iData < c_NLink + 1; iData++ ) {
        for ( int iTDC = 0; iTDC < 8; iTDC++ ) {
          unsigned int TDC_value = ( m_ODEData[c_NLink * tell1Number + iLink] ).getTDC( iData, iTDC );
          if ( TDC_value > 0 ) {
            MuonTileID tile = ( m_muonDet->getDAQInfo() )->getADDInODE( ODE_num - 1, ch );
            if ( tile.isValid() ) {
...
StatusCode MuonRawBuffer::getNZSupp( std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& tileAndTDC ) {
  LHCb::RawEvent* raw = findFirstRawEvent();
  return raw ? getNZSupp( raw, tileAndTDC, rootInTES() ) : Error( "Failed to find raw data" );
}
...
StatusCode MuonRawBuffer::getNZSupp( const LHCb::RawEvent*                                   raw,
  ...
  for ( const auto& mb : b ) {
    ...
    std::copy( bank_container.begin(), bank_container.end(), std::back_inserter( tileAndTDC ) );
  }
  putStatusOnTES();
  return StatusCode::SUCCESS;
}
...
std::vector<std::pair<MuonTell1Header, unsigned int>> MuonRawBuffer::getHeaders( LHCb::RawEvent* raw ) {
  ...
  // first decode data and insert in buffer
  ret.reserve( b.size() );
  std::transform( b.begin(), b.end(), std::back_inserter( ret ), [&]( const RawBank* rb ) {
