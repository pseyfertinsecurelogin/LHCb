...
//=============================================================================
MuonRec::MuonRec( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
...
StatusCode MuonRec::initialize() {
  ...
  if ( !m_muonDetector ) return Error( "Could not read /dd/Structure/LHCb/DownstreamRegion/Muon from TDS" );
  ...
  if ( !prop ) { return Error( "Unable to locate IProperty interface of MuonRawBuffer" ); }
  ...
  for ( int station = 0; station < m_NStation; station++ ) {
    for ( int region = 0; region < m_NRegion; region++ ) {
      ...
      if ( 1 == NLogicalMap ) {
        ...
        if ( !sc.isSuccess() ) {
          error() << "Failed to map digits to coords in a one to one manner" << endmsg;
          return StatusCode::SUCCESS;
        }
      } else {
        // need to cross the input strips to get output strips
        StatusCode sc = addCoordsCrossingMap( *coords, decoding, station, region );
        if ( !sc.isSuccess() ) {
          error() << "Failed to map digits to coords by crossing strips" << endmsg;
          return StatusCode::SUCCESS;
        }
      }
    }
...
StatusCode MuonRec::addCoordsNoMap( MuonCoords&                                                   coords,
                                    const std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& digits, int station,
                                    int region ) const {
...
StatusCode MuonRec::addCoordsCrossingMap( MuonCoords&                                                   coords,
                                          const std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& digits,
...
StatusCode MuonRec::makeStripLayouts( int station, int region, MuonLayout& layout1, MuonLayout& layout2 ) const {
