...
StatusCode MuonDigitToRawBuffer::ProcessDC06() {
  ...
  for ( unsigned int i = 0; i < (unsigned int)m_TotL1Board; i++ ) {
    ...
    if ( i >= m_M1Tell1 ) {
      ...
      bank << padSize;
      for ( const auto& padADD : m_padInL1[i] ) {
    ...
    for ( unsigned int iODE = 0; iODE < ODE_in_L1; iODE++ ) {
      ...
      for ( const auto& digit : m_digitsInODE[odenumber] ) {
  ...
  for ( unsigned int i = m_M1Tell1; i < (unsigned int)m_TotL1Board; i++ ) {
    ...
    for ( iODE = 0; iODE < ODE_in_L1; iODE++ ) {
      ...
      for ( auto itDigit = m_digitsInODE[odenumber].begin(); itDigit < m_digitsInODE[odenumber].end(); itDigit++ ) {
        ...
        if ( channelInTS * ( TSnumberInODE + 1 ) <= address || itDigit == m_digitsInODE[odenumber].end() - 1 ) {
          // info()<<" entra qui "<<TSnumberInODE<<endmsg;
          if ( itDigit == ( m_digitsInODE[odenumber].end() - 1 ) && address < channelInTS * ( TSnumberInODE + 1 ) ) {
