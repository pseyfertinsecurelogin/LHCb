...
DECLARE_COMPONENT( MuonTAERec )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
MuonTAERec::MuonTAERec( const std::string& name, ISvcLocator* pSvcLocator )
    : GaudiAlgorithm( name, pSvcLocator ), m_NStation( 0 ), m_NRegion( 0 ) {
...
StatusCode MuonTAERec::initialize() {
  ...
  if ( m_muonDetector == NULL ) {
    error() << "Could not read /dd/Structure/LHCb/DownstreamRegion/Muon from TDS " << endmsg;
    ...
    for ( int station = 0; station < m_NStation; station++ ) {
      for ( int region = 0; region < m_NRegion; region++ ) {
        ...
        if ( 1 == NLogicalMap ) {
          ...
          StatusCode sc = addCoordsCrossingMap( coords, m_logChannels, station, region );
          if ( !sc.isSuccess() ) {
...
StatusCode MuonTAERec::addCoordsNoMap( MuonCoords*                                             coords,
                                       std::vector<std::pair<LHCb::MuonTileID, unsigned int>>& digits,
...
StatusCode MuonTAERec::makeStripLayouts( int station, int region, MuonLayout& layout1, MuonLayout& layout2 ) {
