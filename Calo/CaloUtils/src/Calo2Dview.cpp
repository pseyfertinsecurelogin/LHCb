...
namespace {
  ...
  std::vector<double> cell_sizes( const Calo2Dview::CaloParams& cp ) {
    auto result = std::vector<double>();
    for ( const auto& cell : cp.refCell ) { result.push_back( cp.calo->cellSize( cell ) ); };
    return result;
  }
} // namespace
...
Calo2Dview::Calo2Dview( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiHistoAlg( name, pSvcLocator ) {
...
StatusCode Calo2Dview::initialize() {
  ...
  m_caloParams[0] = {getDetIfExists<DeCalorimeter>( DeCalorimeterLocation::Spd ),
                     64,
                     32,
                     6,
                     0,
  ...
  m_caloParams[1] = {getDetIfExists<DeCalorimeter>( DeCalorimeterLocation::Prs ),
                     64,
                     32,
                     6,
                     0,
  ...
  m_caloParams[2] = {getDet<DeCalorimeter>( DeCalorimeterLocation::Ecal ),
                     32,
                     32,
                     6,
                     128,
  ...
  m_caloParams[3] = {getDet<DeCalorimeter>( DeCalorimeterLocation::Hcal ),
                     32,
                     16,
                     2,
                     352,
  ...
  if ( m_caloParams[0].calo == nullptr ) Warning( "Spd Element does not exists !", StatusCode::SUCCESS ).ignore();
  if ( m_caloParams[1].calo == nullptr ) Warning( "Prs Element does not exists !", StatusCode::SUCCESS ).ignore();
...
std::string Calo2Dview::getTitle( std::string title, int calo, int area ) const {
...
void Calo2Dview::resetTitle( const HistoID& unit, std::string title ) const {
...
void Calo2Dview::reset( const HistoID& unit, std::string title ) const {
  // Abort if invalid unit given
  auto it = m_caloViewMap.find( unit );
  if ( it == m_caloViewMap.end() ) return;
...
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::MCCaloHit mchit, const std::string title ) const {
  double weight = m_energyWeighted ? mchit.activeE() : 1.;
  fillCalo2D( unit, mchit.cellID(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::MCCaloDigit mcdigit, const std::string title ) const {
  double weight = m_energyWeighted ? mcdigit.activeE() : 1.;
  fillCalo2D( unit, mcdigit.cellID(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::CaloDigit digit, const std::string title ) const {
  double weight = m_energyWeighted ? digit.e() : 1.;
  fillCalo2D( unit, digit.cellID(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::CaloAdc adc, const std::string title ) const {
  double weight = m_energyWeighted ? (double)adc.adc() : 1.;
  fillCalo2D( unit, adc.cellID(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::L0CaloAdc l0adc, const std::string title ) const {
  double weight = m_energyWeighted ? (double)l0adc.adc() : 1.;
  fillCalo2D( unit, l0adc.cellID(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::L0PrsSpdHit hit, const std::string title ) const {
  fillCalo2D( unit, hit.cellID(), 1., title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::L0CaloCandidate l0calo, const std::string title ) const {
  setL0ClusterView( true );
  double weight = m_energyWeighted ? l0calo.et() : 1.;
  fillCalo2D( unit, l0calo.id(), weight, title );
}
void Calo2Dview::fillCalo2D( const HistoID& unit, LHCb::CaloCluster cluster, const std::string title ) const {
  setL0ClusterView( true );
...
void Calo2Dview::fillCalo2D( const HistoID& unit, const LHCb::CaloCellID& id, double value,
...
void Calo2Dview::fillCaloPin2D( const HistoID& unit, const LHCb::CaloCellID& id, double value,
                                const std::string title ) const {
  // Validation
