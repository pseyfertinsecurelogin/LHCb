 ...
 */
Digit2ClustersConnector::Digit2ClustersConnector( Clusters* clus, const unsigned int CutOff ) : m_cut( CutOff ) {
  if ( clus ) {
    StatusCode sc = load( clus->begin(), clus->end(), CutOff );
    if ( sc.isFailure() ) { throw CaloException( "Digit2ClustersConnector! corrupted data" ); }
  };
}
// ============================================================================
...
StatusCode Digit2ClustersConnector::addCluster( LHCb::CaloCluster* cluster ) {
  ...
  for ( auto entry = cluster->entries().begin(); cluster->entries().end() != entry; ++entry ) {
