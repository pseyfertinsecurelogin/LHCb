...
namespace LHCb {
  namespace CaloAlgUtils {
    ...
    inline bool hltContext( boost::string_ref context ) { return details::contains_ci( context, "HLT" ); }
    ...
    inline std::string PathFromContext( boost::string_ref context, boost::string_ref def ) {
      ...
      if ( context.empty() || details::toUpper( context ) == "OFFLINE" )
        return details::to_string( def ); // default standard Offline
