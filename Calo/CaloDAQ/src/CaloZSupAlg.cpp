...
//=============================================================================
CaloZSupAlg::CaloZSupAlg( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
...
StatusCode CaloZSupAlg::initialize() {
  ...
  if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) {
    ...
    if ( m_outputType.value().adcOnTES )
      debug() << "CaloZSupAlg will produce CaloAdcs on TES" << rootInTES() + m_outputADCData + m_extension << endmsg;
  ...
  info() << "Calorimeter has " << m_numberOfCells << " cells. Zsup method " << details::toString( m_zsupMethod.value() )
...
StatusCode CaloZSupAlg::execute() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    if ( m_outputType.value().adcOnTES )
      debug() << "Perform zero suppression - return CaloAdcs on TES at " << rootInTES() + m_outputADCData + m_extension
              << endmsg;
    if ( m_outputType.value().digitOnTES )
      debug() << "Perform zero suppression - return CaloDigits on TES at "
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Processing " << adcs.size() << " Digits." << endmsg;

  enum { DefaultFlag, NeighborFlag, SeedFlag };
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    if ( m_outputType.value().adcOnTES ) debug() << format( "Have stored %5d CaloAdcs.", newAdcs->size() ) << endmsg;
    if ( m_outputType.value().digitOnTES )
