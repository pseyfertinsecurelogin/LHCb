...
//=============================================================================
CaloTriggerBitsFromRawAlg::CaloTriggerBitsFromRawAlg( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode CaloTriggerBitsFromRawAlg::execute() {
  ...
  LHCb::Calo::FiredCells l0Cells = ( m_isPrs ? m_l0BitTool->prsCells() : m_l0BitTool->spdCells() );
  ...
  if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) debug() << " L0PrsSpdHits container size " << newL0Bits->size() << endmsg;
  return StatusCode::SUCCESS;
}
