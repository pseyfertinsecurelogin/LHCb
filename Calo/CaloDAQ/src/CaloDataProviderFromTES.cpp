...
//=============================================================================
CaloDataProviderFromTES::CaloDataProviderFromTES( const std::string& type, const std::string& name,
                                                  const IInterface* parent )
    : base_class( type, name, parent ), m_detectorName( details::alg_name_to_detector( name ) ) {
  //
  ...
  case details::DetectorName_t::Spd:
    ...
    break;
  case details::DetectorName_t::Unknown:; // allow for genconf to work ;-(
  }
}
...
StatusCode CaloDataProviderFromTES::initialize() {
  ...
  if ( m_loc.value().empty() ) {
    switch ( m_data.value() ) {
    case details::source_t::from_adc:
      m_loc = m_adcLoc;
      break;
    case details::source_t::from_digit:
      m_loc = m_digLoc;
      break;
    }
  }
  ...
  if ( fromDigit() ) {
    ...
    if ( m_digCont ) {
      ...
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "Found container " << m_loc.value() << " Content size " << m_digCont->size() << endmsg;
    } else
      m_ok = false;
  } else if ( fromAdc() ) {
    ...
    if ( m_adcCont ) {
      m_ok = true;
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "Found container " << m_loc.value() << " Content size " << m_adcCont->size() << endmsg;
    } else
      m_ok = false;
  }
...
bool CaloDataProviderFromTES::checkSrc( int source ) {
  return std::any_of( m_readSources.begin(), m_readSources.end(), [&]( int i ) { return source == i; } );
}
