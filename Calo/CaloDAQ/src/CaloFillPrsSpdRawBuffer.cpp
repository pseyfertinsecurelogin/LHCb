...
//=============================================================================
CaloFillPrsSpdRawBuffer::CaloFillPrsSpdRawBuffer( const std::string& name, ISvcLocator* pSvcLocator )
    : GaudiAlgorithm( name, pSvcLocator ), m_calo( NULL ), m_totDataSize( 0 ), m_totTrigSize( 0 ), m_nbEvents( 0 ) {
...
StatusCode CaloFillPrsSpdRawBuffer::execute() {
  ...
  for ( unsigned int kk = 0; m_banks.size() > kk; kk++ ) {
    ...
    if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) debug() << "Data bank coding type : " << m_dataCodingType << endmsg;
    rawEvent->addBank( board, m_bankType, version, m_banks[kk] );
    totDataSize += m_banks[kk].size();
    m_dataSize[kk] += m_banks[kk].size();
    if ( 3 > m_dataCodingType ) {
      if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) debug() << "Trigger bank coding type : " << m_dataCodingType << endmsg;
  ...
  if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) {
    ...
    debug() << "Bank sizes: ";
    for ( unsigned int kk = 0; m_banks.size() > kk; kk++ ) {
      debug() << format( "%2d:%4d+%4d ", board, m_banks[kk].size(), m_trigBanks[kk].size() );
      board++;
    }
...
void CaloFillPrsSpdRawBuffer::fillDataBankShort() {
  ...
  for ( itD = digs->begin(); digs->end() != itD; ++itD ) {
    ...
    if ( MSG::VERBOSE >= msgLevel() ) { verbose() << id << format( "adc %4d", adc ) << endmsg; }
  }
}
...
void CaloFillPrsSpdRawBuffer::fillTriggerBank() {
  ...
  for ( itT = prs->begin(); prs->end() != itT; ++itT ) {
    ...
    if ( MSG::VERBOSE >= msgLevel() ) {
      verbose() << format( "Set PRS bit %2d in word %4d for id ", bitNum, cellIndex ) << id << endmsg;
    }
  }
  ...
  for ( itT = spd->begin(); spd->end() != itT; ++itT ) {
    ...
    if ( MSG::VERBOSE >= msgLevel() ) {
      verbose() << format( "Set SPD bit %2d in word %4d for id ", bitNum, cellIndex ) << id << endmsg;
    }
    words[cellIndex] |= ( 1 << bitNum );
  }
  for ( std::vector<int>::const_iterator itW = words.begin(); words.end() != itW; ++itW ) {
    if ( 0 == ( *itW ) ) continue;
...
void CaloFillPrsSpdRawBuffer::fillTriggerBankShort() {
  ...
  for ( std::vector<int>::const_iterator itW = tag.begin(); tag.end() != itW; ++itW ) {
    if ( 0 == ( *itW ) ) continue;
