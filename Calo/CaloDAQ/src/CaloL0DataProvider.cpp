...
DECLARE_COMPONENT( CaloL0DataProvider )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
CaloL0DataProvider::CaloL0DataProvider( const std::string& type, const std::string& name, const IInterface* parent )
...
bool CaloL0DataProvider::decodeCell( LHCb::CaloCellID id ) {
  ...
  if ( m_packed ) {
    ...
    read = std::any_of( m_readSources.begin(), m_readSources.end(), [&]( int i ) { return i == tell1; } );
  } else {
    read = std::any_of( m_readSources.begin(), m_readSources.end(), []( int i ) { return i == 0; } );
  }
      ...
      if ( 0 < lenTrig ) {
        ...
        for ( unsigned int bitNum = 0; 32 > bitNum; bitNum++ ) {
          if ( 0 != ( pattern & ( 1 << bitNum ) ) ) {
            ...
            if ( offset == 0 )
              if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) debug() << "Data word : " << lastData << endmsg;
      ...
      while ( 0 != word ) {
        ...
        if ( 0 != spdId ) {
          if ( details::DetectorName_t::Spd == m_detectorName ) {
            ...
            if ( ( item & 1 ) != 0 ) { fillL0ADC( prsId, 1, sourceID ); }
          }
        }
      }
      ++data;
