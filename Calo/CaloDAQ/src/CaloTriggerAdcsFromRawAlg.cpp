...
//=============================================================================
CaloTriggerAdcsFromRawAlg::CaloTriggerAdcsFromRawAlg( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode CaloTriggerAdcsFromRawAlg::execute() {
  ...
  if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) )
    debug() << " L0CaloAdcs container" << m_outputData.value() << " size " << newL0Adcs->size() << endmsg;
  ...
  if ( "None" != m_pinContainer && !l0PinAdcs.empty() ) {
    ...
    if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) )
      debug() << " PinDiode : L0CaloAdcs container " << m_pinContainer << " size " << newL0Adcs->size() << endmsg;
  }
