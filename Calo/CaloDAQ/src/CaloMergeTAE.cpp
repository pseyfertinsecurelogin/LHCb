...
//=============================================================================
CaloMergeTAE::CaloMergeTAE( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
...
StatusCode CaloMergeTAE::initialize() {
  ...
  std::tie( m_outDigit, m_outAdc ) =
      ( m_outputDataLoc.value().empty() ? std::pair{"TAE/" + m_locDigit, "TAE/" + m_locAdc}
