...
def prntCmp(cmp, level=0):
    ...
    if hasattr(cmp, 'Members'):
        for m in cmp.Members:
            result += prntCmp(m, level + 1)
...
def onDemand(location, alg, context=''):
    ...
    log.debug('Data-On-Demand: define the action for location "%s" to be "%s"'
              % (_loc, _alg))
    dod.AlgMap.update({_loc: _alg})
    log.debug('Calo-On-Demand: define the action for location "%s" to be "%s"'
              % (_loc, _alg))
