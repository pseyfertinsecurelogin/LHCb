...
namespace LoKi {
  ...
  public:
    ...
    Keeper( const OBJECT* o ) : m_objects() { addObject( o ); }
    /// constructor form the objects
    Keeper( const Objects& objects ) : m_objects( objects ) { removeObject(); }
    /// templated constructor from another keeper
    template <class OTHER>
    Keeper( const Keeper<OTHER>& o ) : m_objects( o.begin(), o.end() ) {}
    /// templated constructor from another keeper
    template <class OTHER>
    Keeper( const UniqueKeeper<OTHER>& o );
    /// templated constructor from sequence
    template <class OTHER>
    Keeper( OTHER first, OTHER last ) : m_objects( first, last ) {
      removeObject();
    }
    /// templated constructor from the sequence
    template <class OTHER, class PREDICATE>
    Keeper( OTHER first, OTHER last, PREDICATE cut ) : m_objects() {
  ...
  public:
    ...
    const OBJECT* object( const int index ) const { return *( begin() + index ); }
    /// get the object by index
  ...
  public:
    ...
    size_t addObjects( OTHER first, OTHER last, PREDICATE cut ) {
      LoKi::Algs::copy_if( first, last, std::back_inserter( m_objects ), cut );
      return removeObject();
    }
    ...
    size_t removeObjects( PREDICATE cut ) {
      m_objects.erase( std::remove_if( m_objects.begin(), m_objects.end(), cut ), m_objects.end() );
      return size();
    }
  ...
  public:
    ...
    void sort() { std::sort( m_objects.begin(), m_objects.end() ); }
    ...
    size_t unique( COMPARISON cmp ) {
      sort( cmp );
      m_objects.erase( std::unique( m_objects.begin(), m_objects.end(), cmp ), m_objects.end() );
      return size();
    }
    /// remove the duplicates from the container
    size_t unique() {
      sort();
      m_objects.erase( std::unique( m_objects.begin(), m_objects.end() ), m_objects.end() );
      return size();
    }
    // ========================================================================
  private:
...
template <class OBJECT>
template <class OTHER>
inline LoKi::Keeper<OBJECT>::Keeper( const LoKi::UniqueKeeper<OTHER>& o ) : m_objects( o.begin(), o.end() ) {}
// ============================================================================
/// adding the another keeper
// ============================================================================
template <class OBJECT>
template <class OTHER>
inline LoKi::Keeper<OBJECT>& LoKi::Keeper<OBJECT>::operator+=( const LoKi::UniqueKeeper<OTHER>& o ) {
  addObjects( o.begin(), o.end() );
  return *this;
}
// ============================================================================
template <class OBJECT, class OTHER>
inline LoKi::Keeper<OBJECT> operator+( const LoKi::Keeper<OBJECT>& first, const LoKi::Keeper<OTHER>& second ) {
  LoKi::Keeper<OBJECT> result( first );
  return result += second;
}
// ============================================================================
template <class OBJECT, class OTHER>
inline LoKi::Keeper<OBJECT> operator+( const LoKi::Keeper<OBJECT>& first, const LoKi::UniqueKeeper<OTHER>& second ) {
  LoKi::Keeper<OBJECT> result( first );
