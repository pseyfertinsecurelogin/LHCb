...
namespace Decays {
  ...
  namespace Trees {
    ...
    StatusCode factory( typename Decays::Trees::Types_<PARTICLE>::Tree& tree, const Decays::iNode& mother,
                        const Decays::Trees::Oscillation& oscillated, const Decays::Trees::Arrow& arrow,
                        const typename Decays::Trees::Types_<PARTICLE>::TreeList& daughters, const bool inclusive,
                        const typename Decays::Trees::Types_<PARTICLE>::TreeList& optional, std::ostream& stream );
  } // namespace Trees
  // ========================================================================
  namespace Parsers {
    // ========================================================================
     ...
     */
    template <class PARTICLE>
    StatusCode factory( Decays::Tree_<PARTICLE>& tree, const Decays::Parsers::Tree& parsed, std::ostream& stream );
    // =======================================================================
    /** helper function to create the list of trees from
     *  the list of parsed trees
     *  @param tree   (OUTPUT) the list of constructed trees
    ...
    StatusCode factory( Decays::TreeList_<PARTICLE>& trees, const Decays::Parsers::Tree::Trees& parsed,
      ...
      for ( Decays::Parsers::Tree::Trees::const_iterator it = parsed.begin(); parsed.end() != it; ++it ) {
        ...
        if ( sc.isFailure() ) {
          stream << "ERROR: unable to parse element : '" << ( *it ) << "' " << sc << std::endl;
          return sc; // RETURN
        }
        trees += tree; // APPEND
      }
    ...
    inline StatusCode factory( Decays::Tree_<PARTICLE>& tree, const Decays::Parsers::Tree& parsed,
      ...
      if ( !parsed.ored().empty() ) {
        ...
        if ( sc.isFailure() ) {
          stream << "ERROR: unable to parse 'OR'    : '" << parsed << "' " << sc << std::endl;
          return sc; // RETURN
        }
        tree = Or( trees );
        ...
        if ( sc.isFailure() ) {
          stream << "ERROR: unable to parse 'AND'   : '" << parsed << "' " << sc << std::endl;
          return sc; // RETURN
        }
        tree = And( trees );
      } else if ( parsed.stable() && !parsed.children().empty() ) {
        StatusCode sc = StatusCode( Decays::Trees::ChildrenForStable, true );
        stream << "ERROR: Children are specified for stable : '" << parsed << "' " << sc << std::endl;
        return sc; // RETURN
      } else if ( parsed.stable() && !parsed.optional().empty() ) {
        StatusCode sc = StatusCode( Decays::Trees::OptionalForStable, true );
        stream << "ERROR: Optional are specified for stable : '" << parsed << "' " << sc << std::endl;
        return sc; // RETURN
      } else if ( parsed.stable() && parsed.inclusive() ) {
        StatusCode sc = StatusCode( Decays::Trees::InclusiveForStable, true );
        stream << "ERROR: Inclusive  is specified for stable : '" << parsed << "' " << sc << std::endl;
        return sc; // RETURN
      } else if ( parsed.stable() ) {
        //
        tree = Stable( parsed.head() );
        //
      } else if ( parsed.children().empty() && parsed.optional().empty() ) {
        StatusCode sc = Decays::Trees::factory( tree, parsed.head(), parsed.oscillated() );
        if ( sc.isFailure() ) {
          stream << "ERROR: unable to create atomic : '" << parsed << "' " << sc << std::endl;
          return sc; // RETURN
        }
      } else {
        TreeList   children;
        ...
        if ( sc1.isFailure() ) {
          stream << "ERROR: unable to parse children: '" << parsed << "' " << sc1 << std::endl;
          return sc1; // RETURN
        }
        StatusCode sc2 = factory( optional, parsed.optional(), stream );
        if ( sc2.isFailure() ) {
          stream << "ERROR: unable to parse optional: '" << parsed << "' " << sc2 << std::endl;
          return sc2; // RETURN
        }
        StatusCode sc = Decays::Trees::factory( tree, parsed.head(), parsed.oscillated(), parsed.arrow(), children,
                                                parsed.inclusive(), optional, stream );
        if ( sc.isFailure() ) {
          stream << "ERROR: unable to create tree   : '" << parsed << "' " << sc << std::endl;
          return sc; // RETURN
        }
      }
