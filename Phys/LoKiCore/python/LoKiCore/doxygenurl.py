...
_PROJECTS = ("ROOT", "MOORE", "PANORAMIX", "BENDER", "ERASMUS", "DAVINCI",
             "ORWELL", "PANOPTES", "BRUNEL", "ANALYSIS", "PHYS", "HLT", "REC",
             "LBCOM", "LHCB", "LCGCMT", "GAUDI")
...
def getURL(o, projects=None, latest=False):
    ...
    if 2 < len(klass) and 'T' == klass[0] and \
       klass[1].isupper()         and not 'ROOT'  in projects :
        projects += ['ROOT']
    if 0 <= klass.find('Gaudi') and not 'GAUDI' in projects:
        projects += ['GAUDI']
    ...
    if not url:  ## is it namespace ?
        ...
        if type(o) is gbl.PyRootType and not type(
                o.__init__) is gbl.MethodProxy:
            ## it is namespace !!!
...
def lbglimpse(o, project=None, version=None):
    ...
    command = 'Lbglimpse %s %s %s' % (what, project, version)
