...
__date__ = "2010-02-12"
__version__ = ""
__all__ = ('setData', 'setInputData', 'extendfile')
...
if not hasattr(_EvtSel, '_openNew_'):
    ...
    def _openNew_(self,
        ...
        if typ == 'ROOT':
            ...
            self.g.service('EventPersistencySvc').CnvServices = [
                'RootEvtCnvSvc'
            ]
        elif typ == 'POOL_ROOT':
            cacsvc = self.g.service('PoolDbCacheSvc')
            if hasattr(cacsvc, 'Dlls'):
                cacsvc.Dlls += ['lcg_RootStorageSvc', 'lcg_XMLCatalog']
            else:
                cacsvc.Dlls = ['lcg_RootStorageSvc', 'lcg_XMLCatalog']
            cacsvc.OutputLevel = 4
            cacsvc.DomainOpts = [
                'Domain[ROOT_All].CLASS_VERSION=2 TYP=int',
        ...
        for f in stream:
            ...
            if 0 < f.find('.raw') or 0 < f.find('.mdf'):
                ...
                if 'LHCb::RawDataCnvSvc' not in self.g.service(
                        'EventPersistencySvc').CnvServices:
                    self.g.service('EventPersistencySvc').CnvServices.append(
                        'LHCb::RawDataCnvSvc')
