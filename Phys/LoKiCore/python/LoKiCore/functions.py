...
def _getattr_SR_(self, attribute):
    ...
    raise AttributeError, 'SmartRef(%s)(nullptr).%s' % (
        self.__class__.__name__[9:-1].replace('::', '.'), attribute)
...
def cause(fun1, fun2):
    ...
    >>> result = cause ( cond , source )
    """
    return fun1._cause_(fun2) if hasattr(fun1,
                                         '_cause_') else fun2._cause_(fun1)
...
def _make_vct_(results, arg1, func=_append_, *arg):
    """
    ...
    if issubclass(type(arg1), (list, tuple, set)):
        for a in arg1:
            results = func(results, a)
    else:
        results = func(results, arg1)
    ##
...
def _check_arg_(check, arg1, *arg):
    """
    Check the arguments
    """
    if issubclass(type(arg1), (list, tuple, set)):
...
def _has_string(arg1, *arg):
    ...
    Check the presence of at least one string argument
    """
    return _check_arg_(lambda s: issubclass(type(s), str), arg1, *arg)
...
def _has_float(arg1, *arg):
    ...
    Check the presence of at least one float argument
    """
    return _check_arg_(lambda s: issubclass(type(s), float), arg1, *arg)
...
def _has_nega(arg1, *arg):
    ...
    """
    return _check_arg_(lambda s: s < 0, arg1, *arg)
...
for _t in (cpp.DataObject, cpp.ContainedObject):
...
if not hasattr(LHCb.ParticleID, 'absname'):
    LHCb.ParticleID.absname = _pid_abs_name_
if not hasattr(LHCb.ParticleID, 'abs_name'):
    LHCb.ParticleID.abs_name = _pid_abs_name_
if not hasattr(LHCb.Particle, 'absname'): LHCb.Particle.absname = _p_abs_name_
if not hasattr(LHCb.Particle, 'abs_name'):
    LHCb.Particle.abs_name = _p_abs_name_
