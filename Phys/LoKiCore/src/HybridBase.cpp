...
// ============================================================================
LoKi::Hybrid::Base::Base( const std::string& type, const std::string& name, const IInterface* parent )
  ...
  declareProperty( "ShowCode", m_showCode = false, "Flag to display the prepared python code" );
  //
  // use Environment variable
  //
  declareProperty( "MakeCpp", m_makeCpp = "UNKNOWN" != System::getEnv( "LOKI_GENERATE_CPPCODE" ),
                   "Generate C++ code for created functors " );
  //
  declareProperty( "UsePython", m_use_python = "UNKNOWN" == System::getEnv( "LOKI_DISABLE_PYTHON" ),
                   "Use Python as factory for LoKi-functors " );
  //
  declareProperty( "UseCache", m_use_cache = "UNKNOWN" == System::getEnv( "LOKI_DISABLE_CACHE" ),
  ...
  //
  declareProperty( "CppFileName", m_cppname, "File name for C++ code for created functors " );
  //
  declareProperty( "CppLines", m_cpplines, "C++ (header) lines to be included" );
}
// ============================================================================
...
StatusCode LoKi::Hybrid::Base::finalize() {
  ...
  if ( Py_IsInitialized() && m_pyInit ) {
    ...
    line += "                                                           # " + name() + "\n";
    {
      ...
      if ( 0 != result ) {
        Warning( "Error code from PyRun_SimpleString", StatusCode{static_cast<unsigned long>( 1000 + result )} )
            .ignore();
...
StatusCode LoKi::Hybrid::Base::executeCode( const std::string& pycode ) const {
  ...
  // ===========
  // execute Python 'code'
  PyObject* result = PyRun_String( const_cast<char*>( pycode.c_str() ), // EXECUTE CODE
  ...
  if ( !result ) {
    ...
    // SyntaxError
    if ( PyErr_GivenExceptionMatches( o2, PyExc_SyntaxError ) || PyErr_GivenExceptionMatches( o2, PyExc_TypeError ) ||
         PyErr_GivenExceptionMatches( o2, PyExc_NameError ) || PyErr_GivenExceptionMatches( o2, PyExc_IndexError ) ||
      ...
      if ( prntfal ) { info() << "prntfal : " << toString( *prntfal ) << endmsg; }
      {
        ...
        stream << " Python error in Code\n";
        if ( pyErr ) { stream << "Python error  : " << toString( *pyErr ) << '\n'; }
        if ( !_msg.empty() ) { stream << "PyErr msg     : " << _msg << '\n'; }
        if ( !_message.empty() ) { stream << "PyErr message : " << _message << '\n'; }
        if ( 0 <= _lineno ) { stream << " Line #" << _lineno << '\n'; }
        if ( !_text.empty() ) { stream << "       " << _text; }
        if ( 0 < _offset ) { stream << "      " << std::string( _offset, ' ' ) << '^'; }
        stream << endmsg;
      }
  ...
  if ( !ok ) {
    err() << " Error has occured in Python: the problematic code is : " << endmsg << pycode << endmsg;
 ...
 */
// ============================================================================
std::string LoKi::Hybrid::Base::makeCode( const LoKi::Hybrid::Base::Strings& modules, const std::string& actor,
                                          const std::string& code, const LoKi::Hybrid::Base::Strings& lines,
  ...
  if ( msgLevel( MSG::DEBUG ) || showCode() ) { info() << "Generated Python code:\n" << result << endmsg; }
  ///
  return result;
}
// ============================================================================
namespace {
  ...
  // ==========================================================================
  std::unique_ptr<std::ostream> openFile( std::string namebase, const unsigned short findex,
    ...
    *file << '\n';
    ...
    for ( const auto& ifunc : allfuncs ) {
      for ( auto& icode : ifunc.second ) {
        for ( const auto& s :
              {std::pair{"LoKiPhys", "#include \"LoKi/LoKiPhys.h\""},
               ...
               std::pair{"TC_", "#include \"LoKi/LoKiTrigger.h\""}, std::pair{"TS_", "#include \"LoKi/LoKiTrigger.h\""},
               std::pair{"ODIN_", "#include \"LoKi/LoKiHlt.h\""}, std::pair{"L0_", "#include \"LoKi/LoKiHlt.h\""},
               std::pair{"HDR_", "#include \"LoKi/LoKiHlt.h\""}, std::pair{"HLT_", "#include \"LoKi/LoKiHlt.h\""}} ) {
          if ( std::string::npos != icode.first.find( s.first ) ) { morelines.insert( s.second ); }
        }
      }
    }
...
void LoKi::Hybrid::Base::writeCpp() const {
  ...
  auto ftotal =
      std::accumulate( m_allfuncs.begin(), m_allfuncs.end(), 0,
                       []( int i, const std::pair<std::string, FUNCTIONS>& fs ) { return i + fs.second.size(); } );
  //
  // number of functors per file
  const unsigned int i_split = 0 > split ? std::abs( split ) : 0 < split ? int( double( ftotal ) / split ) + 1 : 0;
  //
  for ( const auto& ia : m_allfuncs ) {
    ...
    for ( const auto& ic : ia.second ) {
      ...
      //
      *file << "\n// FUNCTOR #" << ++iwrite << "/" << ia.second.size() << "" << '\n';
      //
      // write actual C++ code
      LoKi::Cache::makeCode( *file, cpptype, cppcode, pycode, pytype );
      *file << '\n';
      //
...
LoKi::Context LoKi::Hybrid::Base::make_context() const {
  ...
  //
  return LoKi::Context( Gaudi::Utils::getGaudiAlg( cntx ), Gaudi::Utils::getIDVAlgorithm( cntx ) );
}
// ============================================================================
