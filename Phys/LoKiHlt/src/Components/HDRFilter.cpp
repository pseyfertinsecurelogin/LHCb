...
namespace LoKi {
  ...
  public:
     ...
     */
    HDRFilter( const std::string& name, // the algorithm instance name
  ...
  private:
    ...
    LoKi::Types::HLT_Cut m_cut = {
        LoKi::BasicFunctors<const LHCb::HltDecReports*>::BooleanConstant( false )}; // the functor itself
...
namespace {
  ...
  inline
      //
      bool
      _ok_( const std::string& n1, const std::string& n2 ) {
    const bool h1_1 = _hlt1_( n1 );
    const bool h1_2 = _hlt1_( n2 );
    const bool h2_1 = _hlt2_( n1 );
    const bool h2_2 = _hlt2_( n2 );
    //
    return ( h1_1 && h1_2 && !h2_1 && !h2_2 ) || ( h2_1 && h2_2 && !h1_1 && !h1_2 );
  }
  bool _ok_( const std::string& n1, const std::string& n2, const std::string& n3 ) {
    const bool h1_1 = _hlt1_( n1 );
    const bool h1_2 = _hlt1_( n2 );
    ...
    //
    return ( h1_1 && h1_2 && h1_3 && !h2_1 && !h2_2 && !h2_3 ) || ( h2_1 && h2_2 && h2_3 && !h1_1 && !h1_2 && !h1_3 );
  }
  // special case: Hlt2*Hlt1*
  inline bool special_case( const std::string& name ) {
    return 4 < name.size() && boost::algorithm::starts_with( name, "Hlt2" ) && _hlt1_( name );
  }
  // ==========================================================================
  std::string location( const std::string& name ) {
    ...
    } else if ( std::string::npos != name.find( "Hlt1" ) || std::string::npos != name.find( "HLT1" ) ) {
      loc = LHCb::HltDecReportsLocation::Hlt1Default;
    } else if ( std::string::npos != name.find( "Hlt2" ) || std::string::npos != name.find( "HLT2" ) ) {
      loc = LHCb::HltDecReportsLocation::Hlt2Default;
    } else if ( std::string::npos != name.find( "Strip" ) || std::string::npos != name.find( "STRIP" ) ) {
 ...
 */
// ============================================================================
LoKi::HDRFilter::HDRFilter( const std::string& name, // the algorithm instance name
                            ISvcLocator*       pSvc )      // pointer to the service locator
  ...
  const_cast<Property&>( prop ).declareUpdateHandler( [=]( Property& ) {
    ...
    if ( special_case( nam ) ) {
      std::string s( nam, 5 );
      if ( !_ok_( s, loc ) ) { Error( "Inconsistent setting of name&location/2" ).ignore(); }
      if ( !_ok_( code(), loc ) ) { Error( "Inconsistent setting of code&location/2" ).ignore(); }
      if ( !_ok_( s, code(), loc ) ) { Warning( "Inconsistent setting of name&code&location/2" ).ignore(); }
    } else if ( !_hlt1_( nam ) && !_hlt1_( this->code() ) && !_hlt1_( loc ) && !_hlt2_( nam ) &&
                !_hlt2_( this->code() ) && !_hlt2_( loc ) ) { /* stripping case? */
    } else {
      if ( !_ok_( this->code(), loc ) ) { Error( "Inconsistent setting of code&location      " ).ignore(); }
      if ( !_ok_( nam, loc ) ) { Warning( "Inconsistent setting of name&location      " ).ignore(); }
      if ( !_ok_( nam, this->code(), loc ) ) { Warning( "Inconsistent setting of name&code&location " ).ignore(); }
    }
  } );

  StatusCode sc = setProperty( "Code", "HLT_NONE" );
  Assert( sc.isSuccess(), "Unable (re)set property 'Code'" );
  sc = setProperty( "Factory", boost::algorithm::starts_with( name, "Hlt1" )
