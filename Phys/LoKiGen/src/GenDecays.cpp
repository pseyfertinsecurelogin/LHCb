...
namespace {
   ...
   *  @date   2008-05-25
   */
  inline void makeSections( const HepMC::GenParticle* p, const Decays::Trees::Alg alg, Decays::GenSections& sect ) {
    if ( 0 == p ) { return; }
    //
...
// ============================================================================
size_t Decays::Trees::GenExclusive::collect( Decays::iTree_<const HepMC::GenParticle*>::Collection& output ) const {
...
// ============================================================================
bool Decays::Trees::GenExclusive::operator()( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  if ( p_match( p ) ) {
    return true;
  } else {
...
// ============================================================================
bool Decays::Trees::GenExclusive::p_match( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
...
// ============================================================================
void Decays::Trees::GenExclusive::addDaughter( const Decays::iTree_<const HepMC::GenParticle*>& tree ) {
  m_children += tree;
}
// ============================================================================
// add one more node to the tree
 ...
 */
// ============================================================================
const HepMC::GenParticle* Decays::Trees::GenExclusive::ok( const HepMC::GenParticle* p ) const {
...
// the only one essential method
// ============================================================================
bool Decays::Trees::GenInclusive::p_match( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  const HepMC::GenParticle* good = ok( p );
  if ( !good ) { return false; } // RETURN
  // perform the real matching:
...
// ============================================================================
bool Decays::Trees::GenOptional::p_match( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  const HepMC::GenParticle* good = ok( p );
  if ( !good ) { return false; } // RETURN
  // perform the real matching:
...
// ============================================================================
void Decays::Trees::GenOptional::addOptional( const Decays::iTree_<const HepMC::GenParticle*>& tree ) {
  m_optional += tree;
}
// ============================================================================
// add one more daughter to the decay
...
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Trees::GenPhotos::p_match( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  const HepMC::GenParticle* good = ok( p );
  if ( !good ) { return false; } // RETURN
  // perform the real matching:
...
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Trees::GenPhotosOptional::p_match( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  const HepMC::GenParticle* good = ok( p );
  if ( !good ) { return false; } // RETURN
  // perform the real matching:
...
// ============================================================================
template <>
bool Decays::Trees::Stable_<const HepMC::GenParticle*>::
     operator()( Decays::iTree_<const HepMC::GenParticle*>::argument p ) const {
  //
  if ( 0 == p ) { return false; }
  ...
  //
  return ev->particles_begin( HepMC::children ) == ev->particles_end( HepMC::children ) ||
         0 == std::distance( ev->particles_begin( HepMC::children ), ev->particles_end( HepMC::children ) );
}
