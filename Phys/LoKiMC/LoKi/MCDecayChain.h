...
namespace LoKi {
  ...
  public:
     ...
     */
    MCDecayChain( const size_t maxDepth = 5, const bool vertex = false, const Mode mode = LV_WITHPT,
     ...
     */
    MCDecayChain( const LoKi::DecayChainBase& base, const bool vertexe = true, const bool vertexd = true );
    // ========================================================================
  public:
    // ========================================================================
    ...
    void setVertexD( const bool value ) { m_vertexd = value; }
    // ========================================================================
  public:
    // ========================================================================
  ...
  public:
    ...
    STREAM& print( const LHCb::MCParticle* particle, STREAM& stream, TERMINATOR term, const ACCEPT& accept,
      ...
      if ( particle->hasKey() ) { stream << " #" << toString( particle->key() ); }
      //
      ...
      if ( m_vertex && 0 != vertex ) {
        ...
        if ( vertex->hasKey() ) { stream << " #" << toString( vertex->key() ); }
        stream << " " << vertex->type();
      }
      //
      ...
      for ( EndVertices::const_iterator ev = endVertices.begin(); endVertices.end() != ev; ++ev ) {
        ...
        if ( m_vertex && m_vertexe ) {
          ...
          if ( evertex->hasKey() ) { stream << " #" << toString( evertex->key() ); }
          stream << " " << evertex->type();
        }
        //
        print( evertex->products().begin(), evertex->products().end(), stream, term, accept, mark, prefix + "   ",
               depth + 1 ); // RECURSION
      }
      //
      return stream;
     ...
     */
    template <class STREAM, class TERMINATOR, class ACCEPT, class MARK>
    STREAM& print( const LHCb::MCParticle::ConstVector& particles, STREAM& stream, TERMINATOR term,
      ...
      LoKi::MCTrees::buildTrees( particles.begin(), particles.end(), std::back_inserter( trees ) );
     ...
     */
    template <class STREAM, class TERMINATOR, class ACCEPT, class MARK>
    STREAM& print( const LHCb::MCParticle::Container* particles, STREAM& stream, TERMINATOR term, const ACCEPT& accept,
                   const MARK& mark, const std::string& prefix = " ", const size_t depth = 0 ) const {
      // invalid particle
      if ( 0 == particles ) {
  ...
  public:
    // ========================================================================
    /// predefined printout
    std::string print_( const LHCb::MCParticle* p, const LoKi::Types::MCCuts& accept = AcceptMC( true ),
                        const LoKi::Types::MCCuts& mark = AcceptMC( false ) ) const;
    /// predefined printout
    std::string print_( const LoKi::Types::MCRange& p, const LoKi::Types::MCCuts& accept = AcceptMC( true ),
                        const LoKi::Types::MCCuts& mark = AcceptMC( false ) ) const;
    /// predefined printout
    std::string print_( const LHCb::MCParticle::ConstVector& p, const LoKi::Types::MCCuts& accept = AcceptMC( true ),
                        const LoKi::Types::MCCuts& mark = AcceptMC( false ) ) const;
    /// predefined printout
    std::string print_( const LHCb::MCParticle::Container* p, const LoKi::Types::MCCuts& accept = AcceptMC( true ),
                        const LoKi::Types::MCCuts& mark = AcceptMC( false ) ) const;
    // ========================================================================
  private:
    // ========================================================================
...
// ============================================================================
template <class PARTICLE, class STREAM, class TERMINATOR, class ACCEPT, class MARK>
STREAM& LoKi::MCDecayChain::print( PARTICLE first, PARTICLE last, STREAM& stream, TERMINATOR term, const ACCEPT& accept,
                                   const MARK& mark, const std::string& prefix, const size_t depth ) const {
