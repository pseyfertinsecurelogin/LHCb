...
namespace {
   ...
   *  @date   2008-05-25
   */
  inline void makeSections( const LHCb::MCParticle* p, const Decays::Trees::Alg alg, const bool decay,
  ...
  struct NotPid {
    ...
    inline bool operator()( const LHCb::MCParticle* p ) const { return 0 != p && !m_pid.check( p->particleID() ); }
  ...
  bool match( Decays::MCSection::iterator first, Decays::MCSection::iterator last,
              Decays::Trees::MCExclusive::TreeList::const_iterator begin,
              Decays::Trees::MCExclusive::TreeList::const_iterator end ) {
    //
    if ( std::distance( first, last ) != std::distance( begin, end ) ) { return false; }
    //
    //    (1) sort the sections
 ...
 */
// ============================================================================
Decays::Trees::MCExclusive::MCExclusive( const Decays::iNode&                        mother,
                                         const Decays::Trees::MCExclusive::TreeList& children,
                                         const Decays::Trees::Alg alg, const bool decayOnly,
 ...
 */
// ============================================================================
Decays::Trees::MCExclusive::MCExclusive( const Decays::iNode& mother, const Decays::Trees::Alg alg,
                                         const bool decayOnly, const Decays::Trees::Oscillation oscillation )
    : Decays::iTree_<const LHCb::MCParticle*>()
    , m_mother( mother )
    , m_children()
    , m_alg( alg )
 ...
 */
// ============================================================================
Decays::Trees::MCExclusive::MCExclusive( const Decays::Trees::Oscillation oscillation, const Decays::iNode& mother )
    : Decays::iTree_<const LHCb::MCParticle*>()
    , m_mother( mother )
    , m_children()
 ...
 */
// ============================================================================
Decays::Trees::MCExclusive::MCExclusive( const Decays::Decay& decay, const Decays::Trees::Alg alg, const bool decayOnly,
                                         const Decays::Trees::Oscillation oscillation )
    : Decays::iTree_<const LHCb::MCParticle*>()
  ...
  for ( Decays::Decay::Items::const_iterator item = items.begin(); items.end() != item; ++item ) {
    addDaughter( *item );
  }
}
// ============================================================================
...
// ============================================================================
bool Decays::Trees::MCExclusive::valid() const { return m_mother.valid() && Decays::valid( childBegin(), childEnd() ); }
// ============================================================================
// MANDATORY: validate the tree
// ============================================================================
StatusCode Decays::Trees::MCExclusive::validate( const LHCb::IParticlePropertySvc* svc ) const {
...
// ============================================================================
size_t Decays::Trees::MCExclusive::collect( Decays::iTree_<const LHCb::MCParticle*>::Collection& output ) const {
  size_t size = 0;
  for ( TreeList::const_iterator ichild = childBegin(); childEnd() != ichild; ++ichild ) {
    size += ichild->collect( output );
  }
  return size;
}
// ============================================================================
...
// ============================================================================
bool Decays::Trees::MCExclusive::operator()( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
...
// ============================================================================
bool Decays::Trees::MCExclusive::p_match( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  ...
  for ( Decays::MCSections::iterator isect = sections.begin(); sections.end() != isect; ++isect ) {
    // (3) try to match the section
    if ( match( isect->begin(), isect->end(), childBegin(), childEnd() ) ) { return true; } // RETURN
  }                                                                                         // next section
  // no match
  return false; // RETURN
}
...
// ============================================================================
std::ostream& Decays::Trees::MCExclusive::fillStream( std::ostream& s ) const {
  //
  s << ( ( 0 != nChildren() ) ? " (" : " " );
  //
  switch ( oscillation() ) {
...
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const Decays::iTree_<const LHCb::MCParticle*>& tree ) {
  m_children += tree;
}
// ============================================================================
// add one more node to the tree
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const Decays::iNode& node ) { addDaughter( MCExclusive( node ) ); }
// ============================================================================
// add one more node to the tree
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const Decays::Decay::Item& node ) {
  addDaughter( Decays::Nodes::Pid( node ) );
}
// ============================================================================
// add one more node to the tree
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const std::string& node ) { addDaughter( Decays::Nodes::Pid( node ) ); }
// ============================================================================
// add one more node to the tree
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const LHCb::ParticleID& node ) {
  addDaughter( Decays::Nodes::Pid( node ) );
}
// ============================================================================
// add one more node to the tree
// ============================================================================
void Decays::Trees::MCExclusive::addDaughter( const LHCb::ParticleProperty* node ) {
  addDaughter( Decays::Nodes::Pid( node ) );
}
// ============================================================================
 ...
 */
// ============================================================================
Decays::Trees::MCInclusive::MCInclusive( const Decays::iNode&                        mother,
                                         const Decays::Trees::MCExclusive::TreeList& children,
                                         const Decays::Trees::Alg alg, const bool decayOnly,
                                         const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCExclusive( mother, children, alg, decayOnly, oscillation ) {}
// ============================================================================
 ...
 */
// ============================================================================
Decays::Trees::MCInclusive::MCInclusive( const Decays::iNode& mother, const Decays::Trees::Alg alg,
                                         const bool decayOnly, const Decays::Trees::Oscillation oscillation )
 ...
 */
// ============================================================================
Decays::Trees::MCInclusive::MCInclusive( const Decays::Decay& decay, const Decays::Trees::Alg alg, const bool decayOnly,
                                         const Decays::Trees::Oscillation oscillation )
 ...
 */
// ============================================================================
Decays::Trees::MCInclusive::MCInclusive( const Decays::Trees::MCExclusive& right )
...
// ============================================================================
bool Decays::Trees::MCInclusive::valid() const { return MCExclusive::valid() && 0 != nChildren(); }
// ============================================================================
// the only one essential method
// ============================================================================
bool Decays::Trees::MCInclusive::p_match( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  if ( !ok( p ) ) { return false; } // RETURN
  // perform the real matching:
  ...
  if ( 1 == nChildren() ) {
    ...
    // in trees
    if ( Decays::Trees::Sections == alg() ) {
      return LoKi::MCAlgs::found( d.begin(), d.end(), *childBegin(), decayOnly() );
    }
    // direct daughters :
...
// MANDATORY: the specific printout
// ============================================================================
std::ostream& Decays::Trees::MCInclusive::fillStream( std::ostream& s ) const {
  //
  s << " (";
  //
  switch ( oscillation() ) {
 ...
 */
// ============================================================================
Decays::Trees::MCOptional::MCOptional( const Decays::iNode&                        mother,
                                       const Decays::Trees::MCExclusive::TreeList& children,
                                       const Decays::Trees::MCExclusive::TreeList& optional,
                                       const Decays::Trees::Alg alg, const bool decayOnly,
                                       const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCExclusive( mother, children, alg, decayOnly, oscillation ), m_optional( optional ) {}
// ============================================================================
 ...
 */
// ============================================================================
Decays::Trees::MCOptional::MCOptional( const Decays::Decay& decay, const Decays::Trees::MCExclusive::TreeList& optional,
                                       const Decays::Trees::Alg alg, const bool decayOnly,
                                       const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCExclusive( decay, alg, decayOnly, oscillation ), m_optional( optional ) {}
// ============================================================================
// constructor form the constructed tree
// ============================================================================
Decays::Trees::MCOptional::MCOptional( const Decays::Trees::MCExclusive&           right,
                                       const Decays::Trees::MCExclusive::TreeList& optional )
    : Decays::Trees::MCExclusive( right ), m_optional( optional ) {}
// ============================================================================
// MANDATORY: check the validity of the tree
// ============================================================================
bool Decays::Trees::MCOptional::valid() const {
  return MCExclusive::valid() && 0 != nChildren() && 0 != nOptional() && Decays::valid( optBegin(), optEnd() );
}
// ============================================================================
// reset the collection cache
// ============================================================================
void Decays::Trees::MCOptional::reset() const {
  ...
  std::for_each( optBegin(), optEnd(), []( const _Tree_<PARTICLE>& p ) { p.reset(); } );
}
// ============================================================================
// MANDATORY: the proper validation of the tree
// ============================================================================
StatusCode Decays::Trees::MCOptional::validate( const LHCb::IParticlePropertySvc* svc ) const {
...
// MANDATORY: the specific printout
// ============================================================================
std::ostream& Decays::Trees::MCOptional::fillStream( std::ostream& s ) const {
  //
  s << " (";
  //
  switch ( oscillation() ) {
...
// ============================================================================
bool Decays::Trees::MCOptional::p_match( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  if ( !ok( p ) ) { return false; } // RETURN
  // perform the real matching:
  // (1) get the proper decay sections:
...
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const Decays::iTree_<PARTICLE>& tree ) { m_optional += tree; }
// ============================================================================
// add one more daughter to the decay
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const Decays::iNode& node ) { addOptional( MCExclusive( node ) ); }
// ============================================================================
// add one more daughter to the decay
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const Decays::Decay::Item& node ) {
  addOptional( Decays::Nodes::Pid( node ) );
}
// ============================================================================
// add one more daughter to the decay
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const std::string& node ) { addOptional( Decays::Nodes::Pid( node ) ); }
// ============================================================================
// add one more daughter to the decay
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const LHCb::ParticleID& node ) {
  addOptional( Decays::Nodes::Pid( node ) );
}
// ============================================================================
// add one more daughter to the decay
// ============================================================================
void Decays::Trees::MCOptional::addOptional( const LHCb::ParticleProperty* node ) {
  addOptional( Decays::Nodes::Pid( node ) );
}
// ============================================================================
void Decays::Trees::MCOptional::setOptional( const std::vector<std::string>& optional ) {
  m_optional.clear();
  for ( std::vector<std::string>::const_iterator item = optional.begin(); optional.end() != item; ++item ) {
    addOptional( *item );
  }
}
// ============================================================================
void Decays::Trees::MCOptional::setOptional( const std::vector<LHCb::ParticleID>& optional ) {
  m_optional.clear();
  for ( std::vector<LHCb::ParticleID>::const_iterator item = optional.begin(); optional.end() != item; ++item ) {
    addOptional( *item );
  }
}
 ...
 */
// ============================================================================
Decays::Trees::Photos::Photos( const Decays::iNode& mother, const Decays::Trees::Photos::TreeList& children,
                               const Decays::Trees::Alg alg, const bool decayOnly,
                               const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCExclusive( mother, children, alg, decayOnly, oscillation ), m_photon( s_GAMMA ) {}
// ============================================================================
/** constructor from the decay descriptor and flags
 *  @param decay the decay descriptor
 *  @param alg the matching algorithm
 ...
 */
// ============================================================================
Decays::Trees::Photos::Photos( const Decays::Decay& decay, const Decays::Trees::Alg alg, const bool decayOnly,
                               const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCExclusive( decay, alg, decayOnly, oscillation ), m_photon( s_GAMMA ) {}
// ============================================================================
// constructor from the tree
// ============================================================================
Decays::Trees::Photos::Photos( const Decays::Trees::MCExclusive& right )
    : Decays::Trees::MCExclusive( right ), m_photon( s_GAMMA ) {}
// ============================================================================
// MANDATORY: the proper validation of the tree
// ============================================================================
StatusCode Decays::Trees::Photos::validate( const LHCb::IParticlePropertySvc* svc ) const {
...
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Trees::Photos::p_match( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  if ( !ok( p ) ) { return false; } // RETURN
  // perform the real matching:
  // (1) get the proper decay sections:
  ...
  for ( Decays::MCSections::iterator isect = sections.begin(); sections.end() != isect; ++isect ) {
    ...
    // (4) count photons
    const size_t nGamma = std::count_if( isect->begin(), isect->end(), gamma );
    ...
    if ( 0 == nGamma || isect->size() == nChildren() ) {
      if ( match( isect->begin(), isect->end(), childBegin(), childEnd() ) ) { return true; } // RETURN
      //
      continue; // CONTINUE
    }
    ...
    //
    Decays::MCSection::iterator iphys = std::partition( isect->begin(), isect->end(), not_gamma );
    //
    const std::size_t nPhys = iphys - isect->begin();
    //      a. no room for photons
    if ( nPhys == nChildren() ) {
      if ( match( isect->begin(), iphys, childBegin(), childEnd() ) ) { return true; } // RETURN
      //
      continue; // CONTINUE
    } else if ( nPhys > nChildren() ) {
      continue;
    } // CONTINUE
    //
    // C. general case:
    ...
    LoKi::GetN_<Decays::MCSection, std::less<>> getN( nPhotons, iphys, isect->end() );
    //
    ...
    while ( nPhotons == getN.next( std::back_inserter( sect ) ) ) {
      ...
      //
      if ( match( sect.begin(), sect.end(), childBegin(), childEnd() ) ) { return true; }
      sect.clear();
      sect.reserve( nChildren() );
    }
 ...
 */
// ============================================================================
Decays::Trees::PhotosOptional::PhotosOptional( const Decays::iNode&                        mother,
                                               const Decays::Trees::MCExclusive::TreeList& children,
                                               const Decays::Trees::MCExclusive::TreeList& optional,
                                               const Decays::Trees::Alg alg, const bool decayOnly,
                                               const Decays::Trees::Oscillation oscillation )
    : Decays::Trees::MCOptional( mother, children, optional, alg, decayOnly, oscillation ), m_photon( s_GAMMA ) {}
// ============================================================================
/*  constructor from decay descriptor, optional and flags
 *  @param decay the decay descriptor
 *  @param optional the list of optional components
 ...
 */
// ============================================================================
Decays::Trees::PhotosOptional::PhotosOptional( const Decays::Decay&                        decay,
                                               const Decays::Trees::MCExclusive::TreeList& optional,
                                               const Decays::Trees::Alg alg, const bool decayOnly,
                                               const Decays::Trees::Oscillation oscillated )
    : Decays::Trees::MCOptional( decay, optional, alg, decayOnly, oscillated ), m_photon( s_GAMMA ) {}
// ============================================================================
Decays::Trees::PhotosOptional::PhotosOptional( const Decays::Trees::MCOptional& right )
    : Decays::Trees::MCOptional( right ), m_photon( s_GAMMA ) {}
// ============================================================================
Decays::Trees::PhotosOptional::PhotosOptional( const Decays::Trees::MCExclusive&           right,
                                               const Decays::Trees::MCExclusive::TreeList& optional )
    : Decays::Trees::MCOptional( right, optional ), m_photon( s_GAMMA ) {}
// ============================================================================

// ============================================================================
// MANDATORY: check the validity of the tree
// ============================================================================
bool Decays::Trees::PhotosOptional::valid() const { return MCOptional::valid() && m_photon.valid(); }
// ============================================================================
// MANDATORY: the proper validation of the tree
// ============================================================================
StatusCode Decays::Trees::PhotosOptional::validate( const LHCb::IParticlePropertySvc* svc ) const {
  StatusCode sc = m_photon.validate( svc );
...
// MANDATORY: the specific printout
// ============================================================================
std::ostream& Decays::Trees::PhotosOptional::fillStream( std::ostream& s ) const {
  //
  s << " (";
  //
  switch ( oscillation() ) {
...
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Trees::PhotosOptional::p_match( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  // check the validness and oscillation criteria
  if ( !ok( p ) ) { return false; } // RETURN
  // perform the real matching:
  // (1) get the proper decay sections:
  ...
  for ( Decays::MCSections::iterator isect = sections.begin(); sections.end() != isect; ++isect ) {
    ...
    do //  make all possible permutations:
    {
      ...
      if ( std::equal( childBegin(), childEnd(), isect->begin(), Equal() ) ) {
        ...
        do { // make the nesessary permutations
...
// ============================================================================
template <>
bool Decays::Trees::Stable_<const LHCb::MCParticle*>::
     operator()( Decays::iTree_<const LHCb::MCParticle*>::argument p ) const {
  //
  if ( 0 == p ) { return false; }
  ...
  //
  for ( MCVs::const_iterator imcv = vertices.begin(); vertices.end() != imcv; ++imcv ) {
