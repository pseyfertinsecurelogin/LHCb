...
def test1():
    ...
    print 'particle: ( p>>MCID , p>>cos(MCE) )=(%s,%s)' % (p >> MCID,
                                                           p >> cos(MCE))
    ...
    print ' v>>process(MCID)>>min_value(MCP)  : %s' % (
        v >> process(MCID) >> min_value(MCP))
    print ' v>>(process(MCID)>>min_value(MCP)): %s' % (
        v >> (process(MCID) >> min_value(MCP)))
    print '(v>>min_element(MCID))[0].name()   : %s' % (
        v >> min_element(MCID))[0].name()
    print ' v>>min_value(MCID)                : %s' % (v >> min_value(MCID))
    print 'v>>("e+"!=MCABSID)>>MCSIZE         : %s' % (
        v >> ('e+' != MCABSID) >> MCSIZE)
    print 'v>>("e+"!=MCABSID)>>MCSIZE         : %s' % (
        v >> ('e+' != MCABSID) >> MCSIZE)
    print 'v>>select("mu+"!=MCABSID)>>MCEMPTY : %s' % (
        v >> select('mu+' != MCABSID) >> ~MCEMPTY)

    f1 = in_list(MCID, ["e+", "e-", 'mu+', 'mu-'])
    f2 = in_list(MCABSID, ["e+", "e-", 'mu+', 'mu-'])
    print ' in_list ( MCID    , [ "e+" , "e-" , "mu+" , "mu-"] ) : %s %s ' % (
        f1, f1(p1))
    print ' in_list ( MCABSID , [ "e+" , "e-" , "mu+" , "mu-"] ) : %s %s ' % (
        f2, f2(p1))
