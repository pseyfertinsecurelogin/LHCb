...
def isConfigurable(alg):
    ...
    """
    return issubclass(type(alg), Configurable) or issubclass(
        type(alg), ConfigurableMeta)
...
def isIterable(obj):
    ...
    return hasattr(obj, "__iter__") and not isinstance(
        obj, basestring) and not isConfigurable(obj)
...
def getConfigurablesFromModule(confModule):
    ...
    algNames = filter(lambda x: x[0] != '_', confModule.__dict__.keys())
