...
StatusCode FTRawBankEncoder::execute() {
  ...
  for ( const auto& cluster : *clusters ) {
    ...
    data.push_back( ( linkID << FTRawBank::linkShift ) | ( id.channel() << FTRawBank::cellShift ) |
                    ( cluster->fractionBit() << FTRawBank::fractionShift ) |
                    ( ( cluster->isLarge() > 0 ) << FTRawBank::sizeShift ) );
    if ( msgLevel( MSG::VERBOSE ) ) {
      verbose() << format( "Bank%3d sipm%4d channel %4d frac %3.1f isLarge %2d code %4.4x", bankNumber, linkID,
  ...
  for ( unsigned int iBank = 0; iBank < sipmData.size(); ++iBank ) {
    ...
    if ( msgLevel( MSG::VERBOSE ) ) {
      size_t offset = 0;
      for ( const auto& d : bank ) { verbose() << format( "    at %5d data %8.8x", offset++, d ) << endmsg; }
    }
