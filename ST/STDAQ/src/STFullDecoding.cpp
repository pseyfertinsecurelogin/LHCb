...
//=============================================================================
STFullDecoding::STFullDecoding( const std::string& name, ISvcLocator* pSvcLocator ) : ST::AlgBase( name, pSvcLocator ) {
  ...
  declareSTConfigProperty( "OutputLocation", m_outputLocation, STTELL1DataLocation::TTFull );
  declareSTConfigProperty( "EventInfoLocation", m_eventInfoLocation, STTELL1EventInfoLocation::TTEventInfo );
...
StatusCode STFullDecoding::execute() {
  // Get the raw data
  if ( !exist<RawEvent>( m_inputLocation ) ) { return Warning( "Failed to find raw data", StatusCode::SUCCESS, 1 ); }
  ...
  for ( auto itB = itf.begin(); itB != itf.end(); ++itB ) {
    ...
    for ( STTELL1Data::Data::iterator i = tell1Data.begin(); i != tell1Data.end(); ++i ) {
    ...
    for ( ; j != tell1Header.end(); ++j ) { j->resize( nports * nheaders, 0 ); }
    ...
    const RawBank* p = *itB;
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "Decoding bank number [" << itB - itf.begin() << "] of type " << detType()
              << "Full (TELL1 ID: " << p->sourceID() << ", Size: " << p->size() << " bytes)" << endmsg;
    }
    ...
    for ( ; w != p->end<unsigned int>(); ++w ) {
      ...
      if ( cntWD % 224 == 0 ) { // Each 224 words we have a new PP-FPGA
        cntWD = 0;
        cntPP = ( *( w + 219 ) & STTELL1EventInfo::ChipAddrMask ) >> STTELL1EventInfo::ChipAddrBits;
    ...
    STTELL1Data* myData = new STTELL1Data( tell1Data, tell1Header, sentPP, eventInfo );
