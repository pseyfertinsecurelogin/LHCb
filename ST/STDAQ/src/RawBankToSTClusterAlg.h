...
typedef Gaudi::Functional::MultiTransformer<std::tuple<LHCb::STClusters, LHCb::STSummary>( const LHCb::ODIN&,
                                                                                           const LHCb::RawEvent& ),
                                            Gaudi::Functional::Traits::BaseClass_t<STDecodingBaseAlg>>
    RawBankToSTClusterAlgBaseClass;
...
private:
  LHCb::STSummary decodeBanks( const LHCb::RawEvent& rawEvt, LHCb::STClusters& clusCont ) const;

  void createCluster( const STClusterWord& aWord, const STTell1Board* aBoard, LHCb::span<const SiADCWord> adcValues,
                      const STDAQ::version& bankVersion, LHCb::STClusters& clusCont ) const;

  double mean( LHCb::span<const SiADCWord> adcValues ) const;

  LHCb::STLiteCluster word2LiteCluster( const STClusterWord aWord, const LHCb::STChannelID chan,
                                        const unsigned int fracStrip ) const;

  LHCb::STSummary createSummaryBlock( const LHCb::RawEvent& rawEvt, const unsigned int& nclus, const unsigned int& pcn,
