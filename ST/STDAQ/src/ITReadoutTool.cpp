...
ITReadoutTool::ITReadoutTool( const std::string& type, const std::string& name, const IInterface* parent )
    : STReadoutTool( type, name, parent ) {
  declareProperty( "conditionLocation", m_conditionLocation = "/dd/Conditions/ReadoutConf/IT/ReadoutMap" );
...
StatusCode ITReadoutTool::createBoards() {
  ...
  const std::vector<std::string>& stations = rInfo->param<std::vector<std::string>>( "stations" );
