...
STDecodingBaseAlg::STDecodingBaseAlg( const std::string& name, ISvcLocator* pSvcLocator )
...
bool STDecodingBaseAlg::checkDataIntegrity( STDecoder& decoder, const STTell1Board* aBoard, const unsigned int bankSize,
...
std::unique_ptr<LHCb::STTELL1BoardErrorBanks> STDecodingBaseAlg::decodeErrors( const LHCb::RawEvent& raw ) const {
  ...
  for ( const auto& bank : itf ) {
    ...
    if ( bankEnd < STDAQ::minErrorBankWords ) {
      warning() << "Error bank length is " << bankEnd << " and should be at least " << STDAQ::minErrorBankWords
                << endmsg;
      Warning( "Error bank too short --> not decoded for TELL1 " + ST::toString( bank->sourceID() ),
    ...
    if ( bankEnd > STDAQ::maxErrorBankWords ) {
      warning() << "Error bank length is " << bankEnd << " and should be at most " << STDAQ::maxErrorBankWords
                << endmsg;
      Warning( "Error bank too long --> not decoded for TELL1 " + ST::toString( bank->sourceID() ), StatusCode::SUCCESS,
               2 )
          .ignore();
      continue;
    }
