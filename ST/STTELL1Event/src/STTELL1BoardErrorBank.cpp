...
double LHCb::STTELL1BoardErrorBank::fractionOK( const unsigned int pcn ) const {
  ...
  auto frac = std::accumulate( m_errorInfo.begin(), m_errorInfo.end(), 0.0,
                               [&]( double f, const auto* eI ) { return f + eI->fractionOK( pcn ); } );
...
bool LHCb::STTELL1BoardErrorBank::isIT() const { return regName().find( "IT" ) != std::string::npos; }

bool LHCb::STTELL1BoardErrorBank::isTT() const { return regName().find( "TT" ) != std::string::npos; }
