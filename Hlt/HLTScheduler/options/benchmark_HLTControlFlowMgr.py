...
#create Lines
CNodeConfs = [('decision', 'NONLAZY_OR',
               ['L{}'.format(i) for i in range(nlines)], False)]
for i in range(nlines):
    CNodeConfs.append(
        ('L{}'.format(i), 'LAZY_AND',
         ['L{}C{}'.format(i, j) for j in range(linespread)], True))

if enableCallgrind:
    HLTControlFlowMgr().AdditionalCFEdges = [[p.name(), i]
                                             for i in CNodeConfs[0][2]]
    CNodeConfs[0][2].insert(0,
                            p.name())  #insert callgrindprofile to the decision
...
BNodesNames = list(
    {x
     for CNode in CNodeConfs for x in CNode[2] if x not in CNodeNames})
