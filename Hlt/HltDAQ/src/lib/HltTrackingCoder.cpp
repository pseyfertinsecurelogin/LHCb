...
void encodeTracks( const LHCb::Tracks& tracks, std::vector<unsigned int>& rawBank, bool writeStates ) {
  ...
  auto nWords =
      std::accumulate( std::begin( tracks ), std::end( tracks ), rawBank.size(),
  ...
  for ( const LHCb::Track* Tr : tracks ) {
    ...
    *out++             = nhits;
    ...
    out = std::transform( std::begin( Tr->lhcbIDs() ), std::end( Tr->lhcbIDs() ), out,
    ...
    *out++ = states.size();
    ...
    for ( const LHCb::State* state : states ) {
      ...
      *out++  = pp;
      ...
      std::array<double, 5> err{{std::sqrt( state->errX2() ), std::sqrt( state->errY2() ), std::sqrt( state->errTx2() ),
                                 std::sqrt( state->errTy2() ), std::sqrt( state->errQOverP2() )}};
      ...
      *out++ = pac.energy( 1.e5 * fabs( p ) * err[4] ); //== 1.e5 * dp/p (*1.e2)
      for ( unsigned i = 1; i < 5; ++i )
        for ( unsigned j = 0; j < i; ++j ) { *out++ = pac.fraction( state->covariance()( i, j ) / err[i] / err[j] ); }
...
unsigned int decodeTracks( const unsigned int* rawit, unsigned int nentries, LHCb::Tracks& tracks ) {
  // Packer
  const StandardPacker pac = {};
  ...
  while ( k < nentries ) {
    ...
    auto nid = rawit[k++];
    for ( unsigned int i = 0; i < nid; i++ ) { track->addToLhcbIDs( LHCbID{rawit[k++]} ); }
    ...
    if ( k + nstates * 22 > nentries ) {
      ...
      for ( unsigned int i = 0; i < nhits; ++i ) { std::cout << track->lhcbIDs()[i] << ",\n"; }
