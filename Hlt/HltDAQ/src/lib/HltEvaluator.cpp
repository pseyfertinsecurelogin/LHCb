...
StatusCode HltEvaluator::times( double& t, double& w, double& et ) const {
  ...
  et = odin->gpsTime() / 1e6;
  t  = odin->gpsTime() >= startOfRun() ? double( odin->gpsTime() - startOfRun() )
                                      : -double( startOfRun() - odin->gpsTime() );
  // t in seconds
  t /= 1e6;
...
StatusCode HltEvaluator::initialize() {
  ...
  if ( m_useCondDB ) {
    ...
    m_updMgrSvc->registerCondition( this, "Conditions/Online/LHCb/RunParameters", &HltEvaluator::i_updateConditions,
                                    m_runpars );
  ...
  if ( !m_hltMonSvc.isValid() ) { info() << "Could not retrieve " << m_monSvc << endmsg; }
...
void HltEvaluator::handle( const Incident& incident ) {
  if ( !m_useCondDB && ( incident.type() == IncidentType::BeginRun || incident.type() == "RunChange" ) ) {
    m_startOfRun = 0;
  }
  m_data.clear();
}
