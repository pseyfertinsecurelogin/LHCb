...
namespace PackedDataPersistence {
  ...
  template <typename Buffer, typename T>
  auto load( Buffer& buf, T& object ) -> decltype( object.save( buf ), void() ) {
    object.load( buf );
  }
  ...
  public:
    ...
    template <typename T>
    typename std::enable_if<std::is_scalar<T>::value>::type save( const std::vector<T>& v ) {
      saveSize( v.size() );
    ...
    void saveAOS( const std::vector<T>& v ) {
      saveSize( v.size() );
      for ( const auto& object : v ) PackedDataPersistence::save( *this, object );
    }
    ...
    void saveSOA( const std::vector<T>& v ) {
      ...
      for ( const auto& object : v ) PackedDataPersistence::save( soabuf, object );
    }
    /// Save a vector of serializable objects in the default format.
    template <typename T>
    auto save( const std::vector<T>& v ) -> decltype( PackedDataPersistence::save( *this, v[0] ), void() ) {
      saveSOA( v );
    }
  ...
  public:
    ...
    template <typename T>
    typename std::enable_if<std::is_scalar<T>::value>::type load( T& x ) {
      m_buffer.read( x );
    }
    ...
    template <typename T>
    auto load( T& object ) -> decltype( PackedDataPersistence::load( *this, object ), std::size_t() ) {
      auto index0 = m_buffer.pos();
    ...
    void loadAOS( std::vector<T>& v, unsigned int version ) {
      v.resize( loadSize() );
      for ( auto& object : v ) PackedDataPersistence::load( *this, object, version );
    }
    ...
    void loadSOA( std::vector<T>& v, unsigned int version ) {
      ...
      for ( auto& object : v ) PackedDataPersistence::load( soabuf, object, version );
    }
