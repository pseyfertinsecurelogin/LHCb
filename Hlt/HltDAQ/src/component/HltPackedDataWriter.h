...
public:
  ...
  enum SourceIDBits { PartIDBits = 0, ReservedBits = 8, CompressionBits = 13 };
  /// Bit masks in the SourceID word
  enum SourceIDMasks { PartIDMask = 0x00FF, ReservedMask = 0x1F00, CompressionMask = 0xE000 };
  /// Compression algorithms
  enum Compression { NoCompression = 0, ZLIB = 1, LZMA = 2, MaxCompression = 7 };
  ...
  static unsigned int partID( uint16_t sourceID ) { return ( sourceID & PartIDMask ) >> PartIDBits; }
...
private:
  ...
  template <typename T>
  size_t saveObject( const DataObject& dataObject, const std::string& location );
  ...
  PackedDataPersistence::CompressionAlgorithm m_compressionAlg;
  ...
  Gaudi::Property<int> m_compression{this, "Compression", LZMA,
                                     [=]( Property& ) {
                                       ...
                                       constexpr auto kZLIB      = ROOT::kZLIB;
                                       constexpr auto kLZMA      = ROOT::kLZMA;
                                       constexpr auto kUndefined = ROOT::kUndefinedCompressionAlgorithm;
#else
                                       constexpr auto kZLIB      = ROOT::RCompressionSetting::EAlgorithm::kZLIB;
                                       constexpr auto kLZMA      = ROOT::RCompressionSetting::EAlgorithm::kLZMA;
                                       constexpr auto kUndefined = ROOT::RCompressionSetting::EAlgorithm::kUndefined;
#endif
                                       switch ( this->m_compression ) {
                                       case NoCompression: {
                                         this->m_compressionAlg = kUndefined;
                                         break;
                                       }
                                       case ZLIB: {
                                         this->m_compressionAlg = kZLIB;
                                         break;
                                       }
                                       case LZMA: {
                                         this->m_compressionAlg = kLZMA;
                                         break;
                                       }
                                       default:
                                         throw GaudiException( "Unrecognized compression algorithm.", this->name(),
                                                               StatusCode::FAILURE );
                                       }
                                     },
