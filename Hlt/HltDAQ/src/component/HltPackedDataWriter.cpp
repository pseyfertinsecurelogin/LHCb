...
void HltPackedDataWriter::register_object() {
  m_savers[PackedData::classID()] = [this]( const DataObject& dataObject, const std::string& location ) {
...
StatusCode HltPackedDataWriter::initialize() {
  ...
  for ( const auto& path : m_packedContainers ) { info() << " '" << path << "',"; }
  info() << endmsg;

  m_hltANNSvc = service( "HltANNSvc" );

  if ( UNLIKELY( m_enableChecksum ) ) { m_checksum.emplace(); }
  return sc;
}
...
StatusCode HltPackedDataWriter::execute() {
  ...
  for ( const auto& containerPath : m_packedContainers ) {
    ...
    if ( !dataObject ) { return Error( "Container " + containerPath + " does not exist." ); }
    ...
    for ( unsigned int id = 0; id < nlinks; ++id ) {
      ...
      if ( packedLocation != std::end( m_containerMap ) ) { persistedLocation = packedLocation->second; }
      ...
      if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) {
        debug() << "Packed link " << id << "/" << nlinks << " to " << location << " (" << persistedLocation
    ...
    if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) {
      debug() << "Packed " << containerPath << " with ID " << locationID->second << " and CLID " << classID << " into "
              << objectSize << " bytes" << endmsg;
...
void HltPackedDataWriter::addBanks( LHCb::RawEvent& rawEvent, const std::vector<uint8_t>& data,
  ...
  if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) { debug() << "Writing " << nbanks << " banks" << endmsg; }
  ...
  for ( unsigned int ibank = 0; ibank < nbanks; ++ibank ) {
    ...
    if ( UNLIKELY( msgLevel( MSG::DEBUG ) ) ) {
      debug() << "Adding raw bank with sourceID=" << sourceID << ", length=" << length << ", offset=" << offset
              << endmsg;
    }
    auto bank = rawEvent.createBank( sourceID, LHCb::RawBank::DstData, kVersionNumber, length, &( data[offset] ) );
