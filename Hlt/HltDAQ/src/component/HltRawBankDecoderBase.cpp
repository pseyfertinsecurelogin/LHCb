...
std::vector<const LHCb::RawBank*> selectRawBanks_(unsigned int sourceID, LHCb::span<const LHCb::RawBank*> rawbanks )
{
  ...
  if (mybanks.empty() && ( sourceID == HltRawBankDecoderBase::kSourceID_Hlt1 ||
                           sourceID == HltRawBankDecoderBase::kSourceID_Hlt2 ) ) {
      std::copy_if( std::begin(rawbanks), std::end(rawbanks),
                    std::back_inserter(mybanks),
...
HltRawBankDecoderBase::fetch_info2string(unsigned int tck, const IANNSvc::major_key_type& major, Table_t& table) const
{
    ...
    if (tck==0) {
       ...
       for ( const auto& item : m_hltANNSvc->item_map( major ) ) {
