...
// that is the case. Not sure it catches all possible cases...
  ...
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedTrack& x ) {
    processMany( key, x.key, x.chi2PerDoF, x.nDoF, x.flags, x.firstId, x.lastId, x.firstState, x.lastState,
                 x.firstExtra, x.lastExtra, x.likelihood, x.ghostProba );
  }
  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedRichPID& x ) {
    processMany( key, x.pidResultCode, x.dllEl, x.dllMu, x.dllPi, x.dllKa, x.dllPr, x.track, x.dllBt, x.key, x.dllDe );
  }
  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedMuonPID& x ) {
    processMany( key, x.MuonLLMu, x.MuonLLBg, x.nShared, x.status, x.idtrack, x.mutrack, x.key, x.chi2Corr, x.muonMVA1,
                 x.muonMVA2, x.muonMVA3, x.muonMVA4 );
  }
  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedCaloClusterEntry& x ) {
    processMany( key, x.digit, x.status, x.fraction );
  }
  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedCaloCluster& x ) {
    processMany( key, x.key, x.type, x.seed, x.pos_x, x.pos_y, x.pos_z, x.pos_e, x.pos_c0, x.pos_c1, x.pos_cov00,
                 x.pos_cov11, x.pos_cov22, x.pos_cov10, x.pos_cov20, x.pos_cov21, x.pos_spread00, x.pos_spread11,
                 x.pos_spread10, x.firstEntry, x.lastEntry );
  }

  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedRecVertex& x ) {
    processMany( key, x.key, x.technique, x.chi2, x.nDoF, x.x, x.y, x.z, x.cov00, x.cov11, x.cov22, x.cov10, x.cov20,
                 x.cov21, x.firstTrack, x.lastTrack, x.firstInfo, x.lastInfo, x.container );
  }

  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedTagger& x ) {
    processMany( key, x.type, x.decision, x.omega, x.firstTagP, x.lastTagP, x.mvaValue, x.charge );
  }

  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedParticle& x ) {
    processMany( key, x.key, x.particleID, x.measMass, x.measMassErr, x.lv_px, x.lv_py, x.lv_pz, x.lv_mass, x.refx,
                 x.refy, x.refz, x.momCov00, x.momCov11, x.momCov22, x.momCov33, x.momCov10, x.momCov20, x.momCov21,
                 x.momCov30, x.momCov31, x.momCov32, x.posCov00, x.posCov11, x.posCov22, x.posCov10, x.posCov20,
                 x.posCov21, x.pmCov00, x.pmCov01, x.pmCov02, x.pmCov10, x.pmCov11, x.pmCov12, x.pmCov20, x.pmCov21,
                 x.pmCov22, x.pmCov30, x.pmCov31, x.pmCov32, x.firstExtra, x.lastExtra, x.vertex, x.proto,
                 x.firstDaughter, x.lastDaughter );
  }

  template <>
  void PackedDataChecksum::process( const std::string& key, const LHCb::PackedVertex& x ) {
    processMany( key, x.key, x.technique, x.chi2, x.nDoF, x.x, x.y, x.z, x.cov00, x.cov11, x.cov22, x.cov10, x.cov20,
                 x.cov21, x.firstOutgoingPart, x.lastOutgoingPart, x.firstInfo, x.lastInfo );
  }
