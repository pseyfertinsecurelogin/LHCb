...
//=============================================================================
HltRawDataMonitor::HltRawDataMonitor( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode HltRawDataMonitor::fillRawBank() {
  ...
  if ( !bankSize ) {
    return Warning( " No HltSelReports RawBank for requested SourceID in RawEvent. Quiting. ", StatusCode::SUCCESS,
                    10 );
  }
...
int HltRawDataMonitor::nLength( const LHCb::HltObjectSummary& p ) {
  ...
  for ( const auto& elem : p.substructure() ) { nlength += nLength( *elem ); }
  return nlength;
}
...
StatusCode HltRawDataMonitor::selectionDiagnostics() {
  ...
  mess << boost::format( " | %1$=20.20s || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "TotalSize" %
              m_bankSize->allEntries() % m_bankSize->mean() % m_bankSize->rms() % nMedian( *m_bankSize );

  mess << boost::format( " | %1$=20.20s  || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "HitSize" %
              m_hitSize->allEntries() % m_hitSize->mean() % m_hitSize->rms() % nMedian( *m_hitSize );

  mess << boost::format( " | %1$=20.20s  || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "ObjTypSize" %
              m_objtypSize->allEntries() % m_objtypSize->mean() % m_objtypSize->rms() % nMedian( *m_objtypSize );

  mess << boost::format( " | %1$=20.20s  || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "SubStrSize" %
              m_substrSize->allEntries() % m_substrSize->mean() % m_substrSize->rms() % nMedian( *m_substrSize );

  mess << boost::format( " | %1$=20.20s  || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "StdInfoSize" %
              m_stdinfoSize->allEntries() % m_stdinfoSize->mean() % m_stdinfoSize->rms() % nMedian( *m_stdinfoSize );

  mess << boost::format( " | %1$=20.20s  || %2$=15.1f  %3$=15.1f  %4$=15.1f  %5$=15.1f | \n" ) % "ExtInfoSize" %
              m_extinfoSize->allEntries() % m_extinfoSize->mean() % m_extinfoSize->rms() % nMedian( *m_extinfoSize );

  // print-out results
  mess << boost::format( " | %1$=35.15s | | %2$=16.16s | %3$=16.16s | | %4$=14.12s | %5$=14.12s | | %6$=8s | | "
                         "%7$=14.8s | %8$=14.8s | \n" ) %
              "trigger name" % "mean(per event)" % "mean(per trigger)" % "median(evt)" % "median(trg)" % "entries" %
              "rms(evt)" % "rms(trg)";

  for ( std::vector<HltSortedSelName>::const_iterator j = m_hltRankedSelName.begin(); j != m_hltRankedSelName.end();
        ++j ) {
    mess << boost::format( " | %1$-35.35s | | %2$16.1f | %3$16.1f | | %4$14.1f | %5$14.1f | | %6$8.1f | | %7$14.1f | "
                           "%8$14.1f | \n" ) %
                j->first.first % j->second % ( j->first.second )->mean() % nMedian2( m_event, *j->first.second ) %
                nMedian( *j->first.second ) % j->first.second->allEntries() % nRMS2( m_event, *j->first.second ) %
                j->first.second->rms();
  }
