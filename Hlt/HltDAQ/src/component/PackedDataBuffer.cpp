...
namespace PackedDataPersistence {
  ...
  bool ByteBuffer::compress( CompressionAlgorithm algorithm, int level, buffer_type& output ) const {
    ...
    R__zipMultipleAlgorithm( level, &input_size, (char*)&( m_buffer[0] ), &buffer_size, (char*)&( output[0] ),
  ...
  bool ByteBuffer::init( const buffer_type& data, bool compressed ) {
    ...
    while ( true ) {
      ...
      R__unzip( &input_size, (unsigned char*)&data[0], &buffer_size, (unsigned char*)&m_buffer[0], &output_size );
      if ( output_size > 0 ) {
