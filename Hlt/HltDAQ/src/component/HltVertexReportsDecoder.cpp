...
//=============================================================================
HltVertexReportsDecoder::HltVertexReportsDecoder( const std::string& name, ISvcLocator* pSvcLocator )
    : HltRawBankSplittingDecoder<LHCb::VertexBase::Container>(
          name, pSvcLocator,
...
// Main execution
//=============================================================================
Gaudi::Functional::vector_of_optional_<VertexBase::Container> HltVertexReportsDecoder::
  ...
  if ( hltvertexreportsRawBanks.empty() ) {
    throw GaudiException( " No HltVertexReports RawBank for requested SourceID in RawEvent. Quiting. ", name(),
                          StatusCode::SUCCESS );
  }
  if ( hltvertexreportsRawBanks.size() != 1 ) {
    Warning( " More then one HltVertexReports RawBanks for requested SourceID in RawEvent. Will only process the first "
             "one. ",
  ...
  if ( hltvertexreportsRawBank->magic() != RawBank::MagicPattern ) {
    throw GaudiException( " HltVertexReports RawBank has wrong magic number. Return without decoding.", name(),
                          StatusCode::FAILURE );
  }
  const unsigned int bankVersionNumber = hltvertexreportsRawBank->version();
  if ( bankVersionNumber > kVersionNumber ) {
    Warning( " HltVertexReports Raw Bank version number " + std::to_string( bankVersionNumber ) +
  ...
  if ( nSel != 0 ) { error() << "Unexpected banksize while decoding (case 1).... " << endmsg; }
  if ( i != end ) { error() << "Unexpected banksize while decoding (case 2).... " << endmsg; }
