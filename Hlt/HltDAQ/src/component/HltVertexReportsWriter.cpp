...
StatusCode HltVertexReportsWriter::execute() {
  ...
  if ( !inputSummary ) {
    return Warning( " No HltVertexReports at " + m_inputHltVertexReportsLocation.value(), StatusCode::SUCCESS, 20 );
  }
  ...
  for ( const auto& s : *inputSummary ) {
    ...
    if ( !si ) {
      Error( " selectionName=" + s.first + " from HltVertexReports not found in HltANNSvc. Skipped. ",
  ...
  if ( msgLevel( MSG::VERBOSE ) ) {
    ...
    for ( unsigned int i = 0; i != hltVertexReportsRawBank[0]; ++i ) {
      ...
      verbose() << " selection id " << (unsigned int)( hltVertexReportsRawBank[iWord] >> 16 ) << " number of vertices "
                << n << endmsg;
      ++iWord;
