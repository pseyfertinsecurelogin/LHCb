...
bool equal( const Container& c1, const Container& c2, Cmp cmp ) {
  return std::equal( std::begin( c1 ), std::end( c1 ), std::begin( c2 ), std::end( c2 ), cmp );
}

// Check for equal LHCbIDs
bool equalLHCbIDs( const LHCb::Tracks& tracks, const LHCb::Tracks& reftracks ) {
  return equal( tracks, reftracks,
                []( const LHCb::Track* t1, const LHCb::Track* t2 ) { return t1->lhcbIDs() == t2->lhcbIDs(); } );
}

// Check for equal types
bool equalMeta( const LHCb::Tracks& tracks, const LHCb::Tracks& reftracks ) {
  return equal( tracks, reftracks, []( const LHCb::Track* t1, const LHCb::Track* t2 ) {
    return t1->type() == t2->type() && t1->flags() == t2->flags();
  } );
}

// Check for equal States
bool equalStates( const LHCb::Tracks& tracks, const LHCb::Tracks& reftracks ) {
...
struct ExampleTracks {
  ...
  ExampleTracks() {
    ...
    double                          val[15] = {1.1, 0.21, 0.31, 0.41, 0.51, // first row
                      1.2, 0.22, 0.32, 0.42, 1.3,  0.23, 0.33, 1.4, 0.24, 1.5};
    ...
    std::vector<unsigned int> bank = {
        ...
        9912,  7407,       5115,      11911,      9391,      7090,       4968, 1,    2200000,
        10000, 4294957296, 100000000, 4194967296, 100,                               // // state at location 1
        10488, 10954,      114017543, 118321596,  12247449,  5483,       7777, 5284, // its cov
...
BOOST_AUTO_TEST_CASE( encodeNoStates ) {
  ...
  for ( auto tr : myTracks.m_tracks ) { tr->clearStates(); }
  std::vector<unsigned int> bank = {
      1, 3, 1, 3, 5, 0, 9, 3, 2, 4, 6, 0, 3, 4, 7, 11, 13, 17,
