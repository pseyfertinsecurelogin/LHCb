...
class HltLinePersistenceSvc : public extends<Service, ILinePersistenceSvc>, private LinePersistenceSvcCommon {
public:
...
private:
  Gaudi::Property<NameListPerLine> m_locations{
      this, "Locations", {}, "Map of line decisison name to list of output locations."};
  Gaudi::Property<std::vector<std::string>> m_turboPPLines{
      this, "TurboPPLines", {}, "List of Turbo++ line decision names."};
  Gaudi::PropertyWithReadHandler<NameListPerLine> m_rawBankTypes{
      this, "RawBankTypes", {}, "Map of line decisison name to list of rawbank type names."};
...
HltLinePersistenceSvc::HltLinePersistenceSvc( const std::string& name, ISvcLocator* svcLocator )
  ...
  m_turboPPLines.declareUpdateHandler( [this]( Property& ) {
    this->m_turboPPLinesSet =
        ILinePersistenceSvc::LineDecNames{this->m_turboPPLines.begin(), this->m_turboPPLines.end()};
  } );
  m_rawBankTypes.declareUpdateHandler(
      [=]( Property& ) { this->m_rawBanks = typeNamesToBitset( this->m_rawBankTypes ); } );
}

ILinePersistenceSvc::Locations
HltLinePersistenceSvc::locationsToPersist( const LHCb::HltDecReports&               hdr,
                                           const ILinePersistenceSvc::LineDecNames& lines ) const {
  return locationsToPersistImpl( hdr, lines, m_locations );
}

ILinePersistenceSvc::Locations
HltLinePersistenceSvc::turboPPLocationsToPersist( const LHCb::HltDecReports&               hdr,
                                                  const ILinePersistenceSvc::LineDecNames& lines ) const {
  return turboPPLocationsToPersistImpl( hdr, lines, m_turboPPLinesSet, m_locations );
}

ILinePersistenceSvc::RawBanks
HltLinePersistenceSvc::rawBanksToPersist( const LHCb::HltDecReports&               hdr,
                                          const ILinePersistenceSvc::LineDecNames& lines ) const {
