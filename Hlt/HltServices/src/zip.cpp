...
namespace {
  ...
  ptime ZipToTime( uint16_t zip_date, uint16_t zip_time ) {
    return {date( 1980 + ( ( zip_date >> 9 ) & 0x3f ), ( ( zip_date >> 5 ) & 0xf ), zip_date & 0x1f ),
            hours( ( zip_time >> 11 ) & 0x1f ) + minutes( ( zip_time >> 5 ) & 0x1f ) +
...
namespace ConfigZipFileAccessSvc_details {
  ...
  void ZipFile::ZipInfo::print( void ) const {
    cout << hex << vmade << ' ' << vneed << ' ' << flag << ' ' << compress << ' ' << time << ' ' << date << " ("
         << ZipToTime( date, time ) << ") " << crc << ' ' << dec << data_size << ' ' << file_size << ' ' << name_length
         << ' ' << extra_length << ' ' << comment_length << ' ' << disk << ' ' << hex << iattr << ' ' << eattr << ' '
         << file_offset << ' ' << " '" << name << "' "
         << " '" << extra << "' "
         << " '" << comment << "' " << endl;
  }

  void ZipFile::ZipInfo::make_from( const string& a_name, uint32_t a_file_offset, const string& is ) {
  ...
  bool ZipFile::ZipInfo::read_from( istream& is ) { /* here 'is' is the ZIP file
  ...
  bool ZipFile::EndRec::read_from( istream& is ) {
    ...
    for ( er_offset = filesize - streamsize( sizeEndCentDir );
          er_offset >= max( filesize - streamsize( 0xffff ) - streamsize( sizeEndCentDir ), std::streamoff( 0 ) );
          er_offset -= 1 ) {
      is.seekg( er_offset, ios::beg );
      if ( ::get<uint8_t>( is ) == 0x50 && ::get<uint8_t>( is ) == 0x4b && ::get<uint8_t>( is ) == 0x05 &&
           ::get<uint8_t>( is ) == 0x06 ) {
        break;
      }
    }
