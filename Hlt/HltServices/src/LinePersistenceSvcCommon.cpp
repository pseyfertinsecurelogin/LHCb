...
LinePersistenceSvcCommon::locationsToPersistImpl( const LHCb::HltDecReports& hdr, const std::set<std::string>& lines,
  ...
  for ( const auto& pair : hdr ) {
    if ( pair.second.decision() && lines.count( pair.first ) ) {
      ...
      if ( lineLocations == std::end( locationsMap ) ) {
        throw GaudiException( "Decision name not in Locations map.", this->name(), StatusCode::FAILURE );
      }
      locations.insert( std::begin( lineLocations->second ), std::end( lineLocations->second ) );
    }
  }
  return locations;
}

ILinePersistenceSvc::Locations LinePersistenceSvcCommon::turboPPLocationsToPersistImpl(
    const LHCb::HltDecReports& hdr, const std::set<std::string>& lines, const std::set<std::string>& turboPPLines,
    const NameListPerLine& locationsMap ) const {
  ILinePersistenceSvc::Locations locations;
  for ( const auto& pair : hdr ) {
    if ( pair.second.decision() && lines.count( pair.first ) && turboPPLines.count( pair.first ) ) {
      auto lineLocations = locationsMap.find( pair.first );
      if ( lineLocations == std::end( locationsMap ) ) {
        throw GaudiException( "Decision name not in Locations map.", this->name(), StatusCode::FAILURE );
      }
...
LinePersistenceSvcCommon::rawBanksToPersistImpl( const LHCb::HltDecReports& hdr, const std::set<std::string>& lines,
  ...
  for ( const auto& pair : hdr ) {
    if ( pair.second.decision() && lines.count( pair.first ) ) {
      ...
      if ( lineRawBanks == std::end( rawBanksMap ) ) {
        throw GaudiException( "Decision name not in RawBankTypes map.", this->name(), StatusCode::FAILURE );
      }
...
LinePersistenceSvcCommon::typeNamesToBitset( const NameListPerLine& rawBankTypesMap ) const {
  ...
  for ( const auto& pair : rawBankTypesMap ) {
    ...
    for ( const auto& typeName : pair.second ) {
      ...
      if ( bankid < 0 ) {
        throw GaudiException( "Rawbank type name does not exist.", this->name(), StatusCode::FAILURE );
      }
      rawBanks.set( bankid );
    }
