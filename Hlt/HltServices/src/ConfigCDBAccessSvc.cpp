...
namespace {
  ...
  std::string propertyConfigPath( const PropertyConfig::digest_type& digest ) { return "PC/" + digest.str(); }

  std::string configTreeNodePath( const ConfigTreeNode::digest_type& digest ) { return "TN/" + digest.str(); }

  std::string configTreeNodeAliasPath( const ConfigTreeNodeAlias::alias_type& alias ) { return "AL/" + alias.str(); }
  ...
  uid_t read_uid( std::istream& s ) { return read32( s ); }

  std::time_t read_time( std::istream& s ) { return read32( s ); }
  ...
  public:
    ...
    public:
      ...
      friend bool operator!=( const iterator& lhs, const iterator& rhs ) {
        return lhs.atEnd() != rhs.atEnd() || ( lhs.m_cpos != rhs.m_cpos && ( !lhs.atEnd() || !rhs.atEnd() ) );
      }
  ...
  public:
    ocdb( std::string fname ) : m_name( std::move( fname ) ) {
      ...
      auto fd = open( m_name.c_str(), O_WRONLY | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH );
      if ( fd < 0 ) {
        cdb_fileno( &m_db ) = -1;
  ...
  std::optional<T> unpack( cdb_record data ) {
    ...
    {
      unsigned int version [[gnu::unused]] =
          read8( value ); // C++17: replace by [[maybe_unused]]; note: both clang and gcc understand [[gnu::unused]]
      assert( version == 0 );
    }
  ...
  public:
    template <typename F>
    Listener( std::string incident, F&& f ) : m_incident{std::move( incident )}, m_f{std::forward<F>( f )} {
      addRef();
...
namespace ConfigCDBAccessSvc_details {
  ...
  const std::string& toString( const Mode& mode ) {
    ...
    default:
      throw std::runtime_error( "unknown CDB file mode" );
    }
  }

  class CDB {
  public:
    explicit CDB( const std::string& name, std::ios::openmode mode = std::ios::in ) : m_icdb( name ) {
      if ( mode & std::ios::out ) { m_ocdb.emplace( fs::unique_path( name + "-%%%%-%%%%-%%%%-%%%%" ).string() ); }
      // if not writing, then inability to open is a fatal error!
      if ( !m_icdb && !m_ocdb ) { throw std::runtime_error( "Error opening file " + name + ": " + strerror( errno ) ); }
    ...
    ~CDB() {
      if ( m_ocdb && m_ocdb->close() ) { fs::rename( m_ocdb->name(), m_icdb.name() ); }
    }
    ...
    std::vector<std::string> files( const SELECTOR& selector ) {
      ...
      if ( m_icdb ) {
        for ( auto key_value : m_icdb ) {
          if ( selector( key_value.string_key() ) ) { keys.emplace_back( key_value.string_key() ); }
        }
      }
...
ConfigCDBAccessSvc_details::CDB* ConfigCDBAccessSvc::file() const {
  if ( UNLIKELY( !m_file ) ) {
    ...
    if ( LIKELY( !m_file ) ) {
      ...
      try {
        m_file = std::make_unique<CDB>(
            name, ( m_mode == Mode::ReadWrite ? ( std::ios::in | std::ios::out ) : std::ios::in ) );
...
StatusCode ConfigCDBAccessSvc::finalize() {
  if ( m_initListener ) {
    auto incSvc = service<IIncidentSvc>( "IncidentSvc" );
    if ( incSvc ) { incSvc->removeListener( m_initListener.get(), m_incident ); }
  }
  m_file.reset(); // close file if still open
...
std::optional<PropertyConfig> ConfigCDBAccessSvc::readPropertyConfig( const PropertyConfig::digest_type& ref ) {
  return read<PropertyConfig>( propertyConfigPath( ref ) );
}

std::optional<ConfigTreeNode> ConfigCDBAccessSvc::readConfigTreeNode( const ConfigTreeNode::digest_type& ref ) {
...
ConfigCDBAccessSvc::configTreeNodeAliases( const ConfigTreeNodeAlias::alias_type& alias ) {
  ...
  for ( const auto& i : f->files( PrefixFilenameSelector( "AL/" + alias.major() ) ) ) {
    ...
    if ( msgLevel( MSG::DEBUG ) ) debug() << " configTreeNodeAliases: content:" << x.back() << endmsg;
  }
  return x;
}

PropertyConfig::digest_type ConfigCDBAccessSvc::writePropertyConfig( const PropertyConfig& config ) {
  PropertyConfig::digest_type digest = config.digest();
  return write( propertyConfigPath( digest ), config ) ? digest : PropertyConfig::digest_type::createInvalid();
}

ConfigTreeNode::digest_type ConfigCDBAccessSvc::writeConfigTreeNode( const ConfigTreeNode& config ) {
  ConfigTreeNode::digest_type digest = config.digest();
  return write( configTreeNodePath( digest ), config ) ? digest : ConfigTreeNode::digest_type::createInvalid();
}

ConfigTreeNodeAlias::alias_type ConfigCDBAccessSvc::writeConfigTreeNodeAlias( const ConfigTreeNodeAlias& alias ) {
  auto f = file();
  if ( !f ) {
    error() << " container file not found during attempted write of " << alias;
