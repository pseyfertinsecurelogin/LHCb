...
StatusCode HltGenConfig::initialize() {
  ...
  for ( const auto& var : m_envVars ) {
    ...
    if ( val.empty() ) {
      throw GaudiException( std::string{"Failed to obtain environment variable "} + var, name(), StatusCode::FAILURE );
    }
...
std::vector<PropertyConfig::digest_type> HltGenConfig::gatherDependencies( const INamedInterface& obj ) const {
  ...
  if ( seq ) {
    ...
    std::transform( begin( *subs ), end( *subs ), std::back_inserter( depRefs ), [&]( const auto dep ) {
      debug() << "adding sub-algorithm " << dep->name() << " as dependant to " << obj.name() << endmsg;
      auto digest = generateConfig( *dep );
      if ( digest.invalid() ) {
        throw std::runtime_error( "problem creating dependant configuration for " + dep->name() );
      }
      return digest;
    } );
  }
  ...
  std::transform( tools.first, tools.second, std::back_inserter( depRefs ),
                  [&]( const std::pair<const std::string, const IAlgTool*>& i ) {
                    ...
                    if ( digest.invalid() ) {
                      throw std::runtime_error( "problem creating dependant configuration for " + i.second->name() );
                    }
                    return digest;
                  } );
  return depRefs;
...
ConfigTreeNode::digest_type HltGenConfig::generateConfig( const INamedInterface& obj ) const {
  ...
  if ( currentConfig.digest().invalid() ) { throw std::runtime_error( "got bad configuration for " + obj.name() ); }
  ...
  for ( const auto& valVar : m_envVarValues ) {
    for ( const auto& prop : currentConfig.properties() ) {
      ...
      for ( ; it != iter_t(); ++it ) {
        overrule.push_back( prop.first + ":" +
                            ba::replace_range_copy( prop.second, *it, std::string{"$"} + valVar.second ) );
      }
    }
  }
  ...
  if ( !overrule.empty() ) {
    ...
    if ( !currentConfig.digest().valid() ) { throw std::runtime_error( " overruling of " + obj.name() + " failed " ); }
  }

  // Write the actual config
  auto propRef = m_accessSvc->writePropertyConfig( currentConfig );
  if ( propRef.invalid() ) { throw std::runtime_error( "problem writing PropertyConfig for " + obj.name() ); }
  ...
  if ( nodeRef.invalid() ) { throw std::runtime_error( "problem writing ConfigTreeNode for " + obj.name() ); }
  return nodeRef;
}
...
StatusCode HltGenConfig::generateConfig() const {
  ...
  StatusCode sc = getDependencies(
      begin( m_svcConfig ), end( m_svcConfig ), std::back_inserter( depRefs ), [&]( const std::string& name ) {
        IService* svc{nullptr};
        return sl->getService( Gaudi::Utils::TypeNameString( name ), svc, false ).isSuccess() ? svc : nullptr;
      } );
  if ( sc.isFailure() ) return sc;
  sc = getDependencies( begin( m_topConfig ), end( m_topConfig ), std::back_inserter( depRefs ),
  ...
  auto topAlias =
      m_accessSvc->writeConfigTreeNodeAlias( ConfigTreeNodeAlias::createTopLevel( m_release, m_hltType, topConfig ) );
  if ( topAlias.invalid() ) {
