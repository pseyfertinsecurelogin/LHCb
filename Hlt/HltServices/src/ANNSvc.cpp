...
namespace ANNSvcUtilities {
  ...
  bimap_t<KEY, VALUE>::bimap_t( const bimap_t& rhs )
      : m_map( rhs.m_map ), m_invmap( rhs.m_invmap ), m_property( rhs.m_property ), m_parent( rhs.m_parent ) {
    m_property.declareUpdateHandler( &bimap_t<KEY, VALUE>::updateHandler, this );
  }

  template <typename KEY, typename VALUE>
  void bimap_t<KEY, VALUE>::insert( const mapped_type& value ) {
    // verify each entry to be unique
    if ( m_map.find( value.first ) != m_map.end() || m_invmap.find( value.second ) != m_invmap.end() ) {
      if ( m_parent ) {
        m_parent->error() << " Entry \"" << value.first << "\" -> \"" << value.second
  ...
  template <typename KEY, typename VALUE>
  void bimap_t<KEY, VALUE>::updateHandler( Property& /*prop*/ ) {
...
//=============================================================================
bool ANNSvc::hasMajor( const IANNSvc::major_key_type& major ) const { return m_maps.find( major ) != m_maps.end(); }

std::optional<IANNSvc::minor_value_type> ANNSvc::handleUndefined( const IANNSvc::major_key_type&, int ) const {
  return {};
}

std::optional<IANNSvc::minor_value_type> ANNSvc::handleUndefined( const IANNSvc::major_key_type&,
                                                                  const std::string& ) const {
  return {};
}

std::optional<IANNSvc::minor_value_type> ANNSvc::value( const IANNSvc::major_key_type& major,
                                                        const string&                  minor ) const {
...
std::optional<IANNSvc::minor_value_type> ANNSvc::value( const IANNSvc::major_key_type& major, int minor ) const {
  auto i = m_maps.find( major );
  if ( i == m_maps.end() ) return {};
...
std::vector<IANNSvc::minor_key_type> ANNSvc::keys( const IANNSvc::major_key_type& major ) const {
  auto                                 i = items( major );
  std::vector<IANNSvc::minor_key_type> k;
  k.reserve( i.size() );
  std::transform( i.begin(), i.end(), std::back_inserter( k ), []( const minor_value_type& i ) { return i.first; } );
  return k;
}
...
std::vector<IANNSvc::minor_value_type> ANNSvc::items( const IANNSvc::major_key_type& major ) const {
  ...
  if ( i != m_maps.end() ) {
    ...
    r.insert( r.end(), i->second->mapping().begin(), i->second->mapping().end() );
  }
  return r;
}
...
std::vector<IANNSvc::major_key_type> ANNSvc::majors() const {
  ...
  r.reserve( m_maps.size() );
  std::transform( m_maps.begin(), m_maps.end(), std::back_inserter( r ),
