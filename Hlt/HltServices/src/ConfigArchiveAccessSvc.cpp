...
//=============================================================================
ConfigArchiveAccessSvc::ConfigArchiveAccessSvc( const std::string& name, ISvcLocator* pSvcLocator )
...
string ConfigArchiveAccessSvc::propertyConfigPath( const PropertyConfig::digest_type& digest ) const {
  string sref = digest.str();
  return ( ( string( "PropertyConfigs/" ) += sref.substr( 0, 2 ) ) += "/" ) += sref;
}

string ConfigArchiveAccessSvc::configTreeNodePath( const ConfigTreeNode::digest_type& digest ) const {
  string sref = digest.str();
  return ( ( string( "ConfigTreeNodes/" ) += sref.substr( 0, 2 ) ) += "/" ) += sref;
}

string ConfigArchiveAccessSvc::configTreeNodeAliasPath( const ConfigTreeNodeAlias::alias_type& alias ) const {
...
std::optional<T> ConfigArchiveAccessSvc::read( const string& path ) const {
  ...
  if ( !c && msgLevel( MSG::DEBUG ) ) { debug() << "file " << path << " not found in container " << endmsg; }
  return c;
}
...
std::optional<PropertyConfig> ConfigArchiveAccessSvc::readPropertyConfig( const PropertyConfig::digest_type& ref ) {
  return this->read<PropertyConfig>( propertyConfigPath( ref ) );
}

std::optional<ConfigTreeNode> ConfigArchiveAccessSvc::readConfigTreeNode( const ConfigTreeNode::digest_type& ref ) {
  return this->read<ConfigTreeNode>( configTreeNodePath( ref ) );
}

std::optional<ConfigTreeNode>
ConfigArchiveAccessSvc::readConfigTreeNodeAlias( const ConfigTreeNodeAlias::alias_type& alias ) {
...
ConfigArchiveAccessSvc::configTreeNodeAliases( const ConfigTreeNodeAlias::alias_type& alias ) {
...
PropertyConfig::digest_type ConfigArchiveAccessSvc::writePropertyConfig( const PropertyConfig& config ) {
  PropertyConfig::digest_type digest = config.digest();
  return this->write( propertyConfigPath( digest ), config ) ? digest : PropertyConfig::digest_type::createInvalid();
}

ConfigTreeNode::digest_type ConfigArchiveAccessSvc::writeConfigTreeNode( const ConfigTreeNode& config ) {
  ConfigTreeNode::digest_type digest = config.digest();
  return this->write( configTreeNodePath( digest ), config ) ? digest : ConfigTreeNode::digest_type::createInvalid();
}

ConfigTreeNodeAlias::alias_type ConfigArchiveAccessSvc::writeConfigTreeNodeAlias( const ConfigTreeNodeAlias& alias ) {
