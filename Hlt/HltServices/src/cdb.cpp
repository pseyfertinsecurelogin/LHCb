...
int _cdb_make_add( struct cdb_make* cdbmp, unsigned hval, const unsigned char* key, unsigned klen,
  ...
  for ( ;; ) {
    ...
    if ( cdb_unpack( htp ) == hval ) {
      ...
      if ( cdb_unpack( cdbp->cdb_mem + pos ) == klen ) {
        if ( cdbp->cdb_dend - klen < pos + 8 ) return errno = EPROTO, -1;
        if ( memcmp( key, cdbp->cdb_mem + pos + 8, klen ) == 0 ) {
          n = cdb_unpack( cdbp->cdb_mem + pos + 4 );
          pos += 8;
          if ( cdbp->cdb_dend < n || cdbp->cdb_dend - n < pos + klen ) return errno = EPROTO, -1;
          cdbp->cdb_kpos = pos;
          cdbp->cdb_klen = klen;
...
int cdb_findinit( struct cdb_find* cdbfp, struct cdb* cdbp, const void* key, unsigned klen ) {
  ...
  if ( n > ( cdbp->cdb_fsize >> 3 ) || pos < cdbp->cdb_dend || pos > cdbp->cdb_fsize ||
...
int cdb_findnext( struct cdb_find* cdbfp ) {
  ...
  while ( cdbfp->cdb_httodo ) {
    ...
    if ( ( cdbfp->cdb_htp += 8 ) >= cdbfp->cdb_htend ) cdbfp->cdb_htp = cdbfp->cdb_htab;
    cdbfp->cdb_httodo -= 8;
    if ( n ) {
      if ( pos > cdbp->cdb_fsize - 8 ) return errno = EPROTO, -1;
      if ( cdb_unpack( cdbp->cdb_mem + pos ) == klen ) {
        if ( cdbp->cdb_fsize - klen < pos + 8 ) return errno = EPROTO, -1;
        if ( memcmp( cdbfp->cdb_key, cdbp->cdb_mem + pos + 8, klen ) == 0 ) {
          n = cdb_unpack( cdbp->cdb_mem + pos + 4 );
          pos += 8;
          if ( cdbp->cdb_fsize < n || cdbp->cdb_fsize - n < pos + klen ) return errno = EPROTO, -1;
          cdbp->cdb_kpos = pos;
          cdbp->cdb_klen = klen;
          cdbp->cdb_vpos = pos + klen;
          cdbp->cdb_vlen = n;
...
int cdb_seqnext( unsigned* cptr, struct cdb* cdbp ) {
  ...
  if ( pos > dend - 8 ) return 0;
  klen = cdb_unpack( mem + pos );
  vlen = cdb_unpack( mem + pos + 4 );
  pos += 8;
  if ( dend - klen < pos || dend - vlen < pos + klen ) return errno = EPROTO, -1;
  cdbp->cdb_kpos = pos;
  cdbp->cdb_klen = klen;
  cdbp->cdb_vpos = pos + klen;
  cdbp->cdb_vlen = vlen;
...
int internal_function _cdb_make_add( struct cdb_make* cdbmp, unsigned hval, const unsigned char* key, unsigned klen,
                                     const unsigned char* val, unsigned vlen ) {
  unsigned char  rlen[8];
  struct cdb_rl* rl;
  unsigned       i;
  if ( klen > 0xffffffff - ( cdbmp->cdb_dpos + 8 ) || vlen > 0xffffffff - ( cdbmp->cdb_dpos + klen + 8 ) )
    return errno = ENOMEM, -1;
  i  = hval & 255;
  rl = cdbmp->cdb_rec[i];
  if ( !rl || rl->cnt >= sizeof( rl->rec ) / sizeof( rl->rec[0] ) ) {
    rl = (struct cdb_rl*)malloc( sizeof( struct cdb_rl ) );
    if ( !rl ) return errno = ENOMEM, -1;
    rl->cnt           = 0;
  ...
  if ( _cdb_make_write( cdbmp, rlen, 8 ) < 0 || _cdb_make_write( cdbmp, key, klen ) < 0 ||
       _cdb_make_write( cdbmp, val, vlen ) < 0 )
    return -1;
  return 0;
}

int cdb_make_add( struct cdb_make* cdbmp, const unsigned char* key, unsigned klen, const unsigned char* val,
                  unsigned vlen ) {
...
static int remove_record( struct cdb_make* cdbmp, unsigned rpos, unsigned rlen ) {
  ...
  cdbmp->cdb_dpos -= rlen;
  if ( !len ) return 0; /* it was the last record, nothing to do */
  pos = rpos;
  fd  = cdbmp->cdb_fd;
  do {
    r = len > sizeof( cdbmp->cdb_buf ) ? sizeof( cdbmp->cdb_buf ) : len;
    if ( lseek( fd, pos + rlen, SEEK_SET ) < 0 || ( r = read( fd, cdbmp->cdb_buf, r ) ) <= 0 ) return -1;
    if ( lseek( fd, pos, SEEK_SET ) < 0 || _cdb_make_fullwrite( fd, cdbmp->cdb_buf, r ) < 0 ) return -1;
    pos += r;
    len -= r;
  } while ( len );
...
static int zerofill_record( struct cdb_make* cdbmp, unsigned rpos, unsigned rlen ) {
  ...
  if ( lseek( cdbmp->cdb_fd, rpos, SEEK_SET ) < 0 ) return -1;
  ...
  for ( ;; ) {
    ...
    if ( _cdb_make_fullwrite( cdbmp->cdb_fd, cdbmp->cdb_buf, rpos ) < 0 ) return -1;
    rlen -= rpos;
    if ( !rlen ) return 0;
...
static unsigned match( struct cdb_make* cdbmp, unsigned pos, const unsigned char* key, unsigned klen ) {
  int      len;
  unsigned rlen;
  if ( lseek( cdbmp->cdb_fd, pos, SEEK_SET ) < 0 ) return 1;
  if ( read( cdbmp->cdb_fd, cdbmp->cdb_buf, 8 ) != 8 ) return 1;
  if ( cdb_unpack( cdbmp->cdb_buf ) != klen ) return 0;

  /* record length; check its validity */
  rlen = cdb_unpack( cdbmp->cdb_buf + 4 );
  if ( rlen > cdbmp->cdb_dpos - pos - klen - 8 ) return errno = EPROTO, 1; /* someone changed our file? */
  rlen += klen + 8;
  ...
  while ( klen ) {
    ...
    if ( len <= 0 ) return 1;
    if ( memcmp( cdbmp->cdb_buf, key, len ) != 0 ) return 0;
    key += len;
    klen -= len;
  }
...
static int findrec( struct cdb_make* cdbmp, const unsigned char* key, unsigned klen, unsigned hval,
  ...
  for ( rl = cdbmp->cdb_rec[hval & 255]; rl; rl = rl->next )
    for ( rs = rl->rec, rp = rs + rl->cnt; --rp >= rs; ) {
       ...
       * (above), not keys */
      if ( !seeked && _cdb_make_flush( cdbmp ) < 0 ) return -1;
      seeked = 1;
      ...
      case CDB_FIND_REMOVE:
        if ( remove_record( cdbmp, rp->rpos, r ) < 0 ) return -1;
        break;
      case CDB_FIND_FILL0:
        if ( zerofill_record( cdbmp, rp->rpos, r ) < 0 ) return -1;
        break;
      default:
        goto finish;
      }
...
finish:
  if ( seeked && lseek( cdbmp->cdb_fd, cdbmp->cdb_dpos, SEEK_SET ) < 0 ) return -1;
  return ret;
}

int cdb_make_find( struct cdb_make* cdbmp, const unsigned char* key, unsigned klen, enum cdb_put_mode mode ) {
  return findrec( cdbmp, key, klen, cdb_hash( key, klen ), mode );
}

int cdb_make_exists( struct cdb_make* cdbmp, const unsigned char* key, unsigned klen ) {
  return cdb_make_find( cdbmp, key, klen, CDB_FIND );
}

int cdb_make_put( struct cdb_make* cdbmp, const unsigned char* key, unsigned klen, const unsigned char* val,
                  unsigned vlen, enum cdb_put_mode mode ) {
  ...
  case CDB_PUT_REPLACE0:
    ...
    if ( r < 0 ) return -1;
    if ( r && mode == CDB_PUT_INSERT ) return errno = EEXIST, 1;
    break;
  ...
  if ( _cdb_make_add( cdbmp, hval, key, klen, val, vlen ) < 0 ) return -1;
...
int internal_function _cdb_make_flush( struct cdb_make* cdbmp ) {
  ...
  if ( len ) {
    if ( _cdb_make_fullwrite( cdbmp->cdb_fd, cdbmp->cdb_buf, len ) < 0 ) return -1;
...
int internal_function _cdb_make_write( struct cdb_make* cdbmp, const unsigned char* ptr, unsigned len ) {
  ...
  if ( len > l ) {
    ...
    cdbmp->cdb_bpos += l;
    if ( _cdb_make_flush( cdbmp ) < 0 ) return -1;
    ptr += l;
    len -= l;
    l = len / sizeof( cdbmp->cdb_buf );
    if ( l ) {
      l *= sizeof( cdbmp->cdb_buf );
      if ( _cdb_make_fullwrite( cdbmp->cdb_fd, ptr, l ) < 0 ) return -1;
      ptr += l;
      len -= l;
    }
  }
...
static int cdb_make_finish_internal( struct cdb_make* cdbmp ) {
  ...
  if ( ( ( 0xffffffff - cdbmp->cdb_dpos ) >> 3 ) < cdbmp->cdb_rcnt ) return errno = ENOMEM, -1;
  ...
  if ( lseek( cdbmp->cdb_fd, 0, 0 ) != 0 || _cdb_make_fullwrite( cdbmp->cdb_fd, p, 2048 ) != 0 ) return -1;
