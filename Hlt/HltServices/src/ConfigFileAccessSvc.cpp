...
bool ConfigFileAccessSvc::create_directories( fs::path dir ) const {
  ...
  } catch ( const fs::filesystem_error& x ) { error() << x.what() << endmsg; }
  return false;
}
...
fs::path ConfigFileAccessSvc::configTreeNodePath( const ConfigTreeNode::digest_type& digest ) const {
  std::string sref = digest.str();
  return dir() / "ConfigTreeNodes" / sref.substr( 0, 2 ) / sref;
}

fs::path ConfigFileAccessSvc::configTreeNodeAliasPath( const ConfigTreeNodeAlias::alias_type& alias ) const {
  return dir() / "Aliases" / alias.str();
}

std::optional<PropertyConfig> ConfigFileAccessSvc::readPropertyConfig( const PropertyConfig::digest_type& ref ) {
...
std::optional<ConfigTreeNode> ConfigFileAccessSvc::readConfigTreeNode( const ConfigTreeNode::digest_type& ref ) {
...
PropertyConfig::digest_type ConfigFileAccessSvc::writePropertyConfig( const PropertyConfig& config ) {
...
ConfigTreeNode::digest_type ConfigFileAccessSvc::writeConfigTreeNode( const ConfigTreeNode& config ) {
...
bool ConfigFileAccessSvc::isCompatible( const ConfigTreeNodeAlias& /*alias*/, const fs::path& /*dirName*/ ) const {
...
ConfigTreeNodeAlias::alias_type ConfigFileAccessSvc::writeConfigTreeNodeAlias( const ConfigTreeNodeAlias& alias ) {
  ...
  if ( fs::exists( fdir ) ) {
    if ( !isCompatible( alias, fdir ) ) {
      error() << " current TOPLEVEL is not compatible with other config in " << fdir.string()
              << " refusing to write TOPLEVEL alias " << endmsg;
      return ConfigTreeNodeAlias::alias_type();
    }
...
ConfigFileAccessSvc::configTreeNodeAliases( const ConfigTreeNodeAlias::alias_type& alias ) {
  ...
  dirs.push_back( basedir / alias.major() );
