...
class TCKLinePersistenceSvc : public extends<Service, ILinePersistenceSvc>,
...
const PropertyConfig& TCKLinePersistenceSvc::hltSvcPropertyConfig(const TCK& tck) const {
  ...
  const ConfigTreeNode* tree = m_propertyConfigSvc->resolveConfigTreeNode(
    ConfigTreeNodeAlias::alias_type{std::string("TCK/") + tck.str()});
  if (!tree) {
...
T TCKLinePersistenceSvc::parseProperty(const PropertyConfig& config, const char* name) const {
  ...
  if (status.isFailure()) {
    error() << "Error interpreting requested property " << name << " in  configuration of " << m_instanceName
            << endmsg;
    throw GaudiException("FATAL error",
                         this->name(), StatusCode::FAILURE);
  }
