// $Id: SeedHit.h,v 1.2 2007-02-28 18:32:11 marcocle Exp $
#ifndef SEEDHIT_H 
#define SEEDHIT_H 1
...
public: 
  ...
  //const std::vector<SeedTrack*>& seeds() const;
...
inline void SeedHit::setUse( const bool value ) { 
  m_use1 = value;
  m_use2 = value;
}
...
//inline const std::vector<SeedTrack*>& SeedHit::seeds() const{ 
//  return m_seeds; 
//}
...
inline bool SeedHit::sameLayer(const LHCb::OTChannelID chan) const{
  return(chan.layer() == m_OTChan.layer() && chan.station() == m_OTChan.station() );
}
...
inline bool SeedHit::onTrack() const{
  //return !seeds().empty();
  return clus()->onTrack();
}
