...
public:
  ...
  void setYParams(const double& sy, const double& y0  );
    ...
    std::vector<Tsa::Cluster*> xClusters();

    std::vector<Tsa::Cluster*> yClusters();
...
inline std::vector<Tsa::Cluster*> SeedTrack::clusters() {
  std::vector<SeedPnt> pnts = usedPnts();
  std::vector<Tsa::Cluster*> tVec;
  tVec.reserve(pnts.size());
  for (std::vector<SeedPnt>::iterator iter = pnts.begin(); iter != pnts.end(); ++iter){
    tVec.push_back((*iter).hit()->clus());
  }
  return tVec;
}

inline std::vector<Tsa::Cluster*> SeedTrack::xClusters() {
  std::vector<Tsa::Cluster*> tVec;
  tVec.reserve(m_xPnts.size());
  for (std::vector<SeedPnt>::iterator iter = m_xPnts.begin(); iter != m_xPnts.end(); ++iter){
    if ((*iter).skip() == false) tVec.push_back((*iter).hit()->clus());
  } //iter
  return tVec;
}

inline std::vector<Tsa::Cluster*> SeedTrack::yClusters() {
  std::vector<Tsa::Cluster*> tVec;
  tVec.reserve(m_yPnts.size());
  for (std::vector<SeedPnt>::iterator iter = m_yPnts.begin(); iter != m_yPnts.end(); ++iter){
    if ((*iter).skip() == false) tVec.push_back((*iter).hit()->clus());
  } //iter
  return tVec;
}
