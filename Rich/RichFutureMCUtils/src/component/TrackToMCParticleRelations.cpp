...
TrackToMCParticleRelations::TrackToMCParticleRelations( const std::string &name,
...
TrackToMCParticleRelations::operator()( const LHCb::Tracks &tks, const LHCb::MCParticles & ) const
{
  ...
  else
  {
    ...
    for ( auto *tk : tks )
    {
      ...
      if ( m_allowMultMPs )
      {
        ...
        while ( mcp )
        {
          _ri_verbo << std::setprecision( 20 ) << "Track " << tk->key() << " MCP " << mcp->key()
      ...
      else
      {
        ...
        if ( bestMCP )
        {
          _ri_verbo << std::setprecision( 20 ) << "Track " << tk->key() << " MCP " << bestMCP->key()
    ...
    if ( msgLevel( MSG::DEBUG ) )
    {
      ...
      {
        verbose() << std::setprecision( 20 ) << "  TK " << i.from()->key() << "  MCP "
                  << i.to()->key() << "  Weight " << i.weight() << endmsg;
      }
    }
