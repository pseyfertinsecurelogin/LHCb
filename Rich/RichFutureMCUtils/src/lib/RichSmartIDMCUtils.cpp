...
LHCb::MCParticle::ConstVector SmartIDUtils::mcParticles( const LHCb::RichSmartID id ) const {
...
const LHCb::MCParticle* SmartIDUtils::trueCherenkovRadiation( const Rich::PDPixelCluster& cluster,
                                                              const Rich::RadiatorType    rad ) const {
  // primary ID
...
bool SmartIDUtils::isBackground( const LHCb::RichSmartID id ) const {
  ...
  if ( iEn != m_sumMap.end() ) {
    ...
    isback = std::none_of( ( *iEn ).second.begin(), ( *iEn ).second.end(),
                           []( const auto* sum ) { return sum->history().hasSignal(); } );
  }
  // return final flag
  return isback;
}

bool SmartIDUtils::isCherenkovRadiation( const LHCb::RichSmartID id, const Rich::RadiatorType rad ) const {
  bool isCKRad = false;
...
bool SmartIDUtils::isCherenkovRadiation( const Rich::PDPixelCluster& cluster, const Rich::RadiatorType rad ) const {
