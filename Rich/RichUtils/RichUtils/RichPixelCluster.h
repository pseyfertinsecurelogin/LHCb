...
namespace Rich {
  ...
  public:
    ...
    explicit PDPixelCluster( const Rich::DetectorType rich, const Rich::Side side, const LHCb::RichSmartID id,
    ...
    /// In addition pass in the rich and side enum, to avoid re-computing them from the ID
    explicit PDPixelCluster( const Rich::DetectorType rich, const Rich::Side side, const LHCb::RichSmartID id,
                             SmartIDVector&& secIDs, const DeRichPD* dePD = nullptr )
        : m_rich( rich )
        , m_side( side )
        , m_primaryID( id )
        , m_empty( false )
    ...
    /// and optional DePD pointer.
    /// In addition pass in the rich and side enum, to avoid re-computing them from the ID
    explicit PDPixelCluster( const LHCb::RichSmartID id, SmartIDVector&& secIDs, const DeRichPD* dePD = nullptr )
        : m_rich( id.rich() )
        , m_side( id.panel() )
        , m_primaryID( id )
        , m_empty( false )
  ...
  public:
    ...
    inline decltype( auto ) size() const noexcept { return ( !empty() ? 1u + secondaryIDs().size() : 0u ); }
  ...
  public:
    ...
    friend inline MsgStream& operator<<( MsgStream& os, const PDPixelClusters& clus ) { return clus.fillStream( os ); }
  ...
  public:
    ...
    inline decltype( auto ) colNumber( const LHCb::RichSmartID id ) const noexcept { return id.pixelCol(); }
  ...
  public:
    ...
    inline decltype( auto ) getCluster( const std::int16_t row, const std::int16_t col ) const noexcept {
      return ( isOn( row, col ) ? ( m_clusters )[index( row, col )] : nullptr );
    }

    /// Set cluster for given pixel
    inline void setCluster( const LHCb::RichSmartID id, const std::int16_t row, const std::int16_t col,
     ...
     */
    PDPixelClusters::Cluster* mergeClusters( PDPixelClusters::Cluster*& clus1, PDPixelClusters::Cluster*& clus2 );
  ...
  protected: // methods
    ...
    {
      return ( row >= 0 && row < nPixelRows() && col >= 0 && col < nPixelCols() && ( m_data )[index( row, col )] );
    }
  ...
  public:
    /// Overload output to MsgStream
    friend inline MsgStream& operator<<( MsgStream& os, const PDPixelClustersBuilder& b ) { return b.fillStream( os ); }
  ...
  private:
    ...
    inline void setAliceMode( const bool mode ) noexcept {
      ...
      if ( reallocate ) {
        ...
        m_aliceMode = mode;
        // set data size
        m_nPixRows = ( m_aliceMode ? Rich::DAQ::HPD::MaxDataSizeALICE : Rich::DAQ::HPD::MaxDataSize );
        // reallocate storage
        allocate();
      }
    }
