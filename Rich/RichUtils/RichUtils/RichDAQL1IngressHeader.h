...
namespace Rich::DAQ
{
  ...
  private:
    ...
    static const LongType MaskActiveHPDs = ( LongType )( ( 1 << BitsActiveHPDs ) - 1 )
                                           << ShiftActiveHPDs;
    static const LongType MaskIngressID = ( LongType )( ( 1 << BitsIngressID ) - 1 )
                                          << ShiftIngressID;
    static const LongType MaskHPDsSuppressed = ( LongType )( ( 1 << BitsHPDsSuppressed ) - 1 )
                                               << ShiftHPDsSuppressed;
    ...
    inline void dataInRange( const ShortType value, const ShortType max ) const
    {
      if ( value > max )
      {
        throw GaudiException(
          "Data out of range", "*RichDAQL1IngressHeader*", StatusCode::FAILURE );
      }
    }

    /// Set a given bit in a data word on
    inline void setBit( Rich::DAQ::ShortType &     data,
