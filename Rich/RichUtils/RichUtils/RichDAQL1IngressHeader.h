...
namespace Rich::DAQ {
  ...
  public: // getters
    ...
    inline L1IngressID ingressID() const { return L1IngressID( ( ( data() & MaskIngressID ) >> ShiftIngressID ) ); }

    /// Access the Active HPD bits
    inline ShortType activeHPDbits() const { return ( ( data() & MaskActiveHPDs ) >> ShiftActiveHPDs ); }

    /// Access the suppression flag
    inline bool hpdsSuppressed() const { return ( 0 != ( data() & MaskHPDsSuppressed ) >> ShiftHPDsSuppressed ); }
  ...
  public: // setters
    ...
    inline void setIngressID( const L1IngressID num ) { set( num.data(), ShiftIngressID, MaskIngressID ); }
  ...
  private:
    ...
    inline void dataInRange( const ShortType value, const ShortType max ) const {
      if ( value > max ) {
        throw GaudiException( "Data out of range", "*RichDAQL1IngressHeader*", StatusCode::FAILURE );
      }
    }

    /// Set a given bit in a data word on
    inline void setBit( Rich::DAQ::ShortType& data, const Rich::DAQ::ShortType pos,
                        const Rich::DAQ::ShortType value = 1 ) const {
      data |= value << pos;
    }

    /// Test if a given bit in a word is set on
    inline bool isBitOn( const Rich::DAQ::LongType data, const Rich::DAQ::ShortType pos ) const {
      return ( 0 != ( data & ( 1 << pos ) ) );
    }
