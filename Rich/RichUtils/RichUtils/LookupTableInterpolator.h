...
namespace Rich {
  ...
  protected:
    ...
    inline void init( const TYPE minX, const TYPE maxX, FUNC&& func ) {
      ...
      for ( std::size_t i = 0; i < NBINS; ++i ) { m_data.emplace_back( binLowX( i ), binHighX( i ), func ); }
    }

  protected:
    /// Get the low x value for a given bin index
    inline TYPE binLowX( const std::size_t i ) const noexcept { return m_minX + ( i / this->m_incXinv ); }

    /// Get the high x value for a given bin index
    inline TYPE binHighX( const std::size_t i ) const noexcept { return m_minX + ( ( i + 1 ) / this->m_incXinv ); }
  ...
  private:
    ...
    template <typename PARAM, typename std::enable_if<std::is_arithmetic<PARAM>::value>::type* = nullptr>
  ...
  public:
    ...
    inline void init( const TYPE maxX, FUNC&& func ) {
      BaseLookupTableInterpolator<TYPE, NBINS>::init( TYPE( 0 ), maxX, std::forward<FUNC>( func ) );
    }
