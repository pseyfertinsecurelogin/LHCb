  ...
  public:
    ...
    inline void angleToDirection( const VECTOR &direction, TYPE &theta, TYPE &phi ) const
    {
      if constexpr ( std::is_arithmetic< TYPE >::value )
      {
        ...
        theta = Rich::Maths::fast_atan2( TYPE( std::sqrt( ( r.x() * r.x() ) + ( r.y() * r.y() ) ) ),
                                         r.z() );
      ...
      else
      {
        ...
        phi = Rich::Maths::fast_atan2( r.y(), r.x() );
        theta =
          Rich::Maths::fast_atan2( std::sqrt( ( r.x() * r.x() ) + ( r.y() * r.y() ) ), r.z() );
        // correct phi to range 0 - 2PI
    ...
    inline decltype( auto ) bestPoint( const TYPE fractDist ) const
    {
      ...
      {
        // SIMD version
        auto p = middlePointSIMD() +
                 ( m_exitMidVSIMD * ( ( fractDist - m_midFrac2SIMD ) / m_midFrac2SIMD ) );
