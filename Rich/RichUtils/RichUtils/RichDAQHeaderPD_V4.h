...
namespace Rich::DAQ {
  ...
  namespace RichDAQHeaderV4 {
    ...
    public:
      ...
      inline bool setL0ID( const Level0ID l0id ) {
        return ( dataInRange( l0id.data(), MaxL0ID ) ? set( l0id.data(), ShiftL0ID, MaskL0ID ) : false );
      }
      ...
      inline ShortType nEightBitBlocks() const noexcept {
        ...
        return ( inhibit() ? 0 : ( ( primaryHeaderWord().data() & MaskNeightBit ) >> ShiftNeightBit ) );
      }

      /// Set the number of "8-bit data blocks" with at least one hit
      inline bool setNEightBitBlocks( const ShortType value ) {
        return ( dataInRange( value, MaxNeightBit ) ? set( value, ShiftNeightBit, MaskNeightBit ) : false );
      }
  ...
  inline std::ostream& operator<<( std::ostream& os, const RichDAQHeaderV4::RichDAQHeaderPD& header ) {
    os << "HPD Header V4 : nHeadW=" << header.nHeaderWords() << " L0ID=" << header.l0ID()
       << " EventID=" << header.eventID() << " ZS=" << header.zeroSuppressed() << " AliceMode=" << header.aliceMode()
       << " ExtendedFormat=" << header.extendedFormat() << " Inhibit=" << header.inhibit()
