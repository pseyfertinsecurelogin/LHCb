...
namespace Rich::DAQ {
  ...
  public: // reading and writing
    ...
    inline void readFromDataStream( const LongType*& data ) {
      for ( FooterWords::size_type i = 0; i < nFooterWords(); ++i ) { footerWords()[i] = WordType( *( data++ ) ); }
    }
  ...
  protected: // methods
    ...
    inline bool dataInRange( const ShortType value, const ShortType max ) const { return ( value <= max ); }
