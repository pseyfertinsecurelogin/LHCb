...
void HPDPixelClustersBuilder::initialise( const PDPixelCluster::SmartIDVector& smartIDs ) {
...
void PMTPixelClustersBuilder::initialise( const PDPixelCluster::SmartIDVector& smartIDs ) {
  // reset the first cluster ID to 0
  m_lastID = 0;
  // clear the pixel data
  m_pdClus.clear();
  ...
  if ( !smartIDs.empty() ) {
    ...
    memset( m_data.get(), 0, nPixelRows() * nPixelCols() );
    // set the hit pixels as "on"
    for ( const auto S : smartIDs ) { setOn( rowNumber( S ), colNumber( S ) ); }
  }
}

PDPixelClusters::Cluster* PDPixelClustersBuilder::mergeClusters( PDPixelClusters::Cluster*& clus1,
...
MsgStream& PDPixelClustersBuilder::fillStream( MsgStream& os ) const {
  ...
  // column numbers
  os << " c     |";
  for ( std::int16_t col = 0; col < nPixelCols(); ++col ) { os << boost::format( "%3i" ) % col; }
  os << endmsg;
...
void PDPixelClustersBuilder::splitClusters( const PDPixelClusters::Cluster::PtnVector& clusters ) {
...
void PDPixelClustersBuilder::removeCluster( PDPixelClusters::Cluster* clus ) {
  const auto iF = std::find_if( m_pdClus.clusters().begin(), m_pdClus.clusters().end(),
                                [clus]( const auto& c ) { return &c == clus; } );
...
void PDPixelClusters::suppressIDs( PDPixelCluster::SmartIDVector& smartIDs, const unsigned int maxSize ) const {
...
const PDPixelClusters::Cluster* PDPixelClusters::getCluster( const LHCb::RichSmartID id ) const {
...
MsgStream& PDPixelClusters::fillStream( MsgStream& os ) const {
  for ( const auto& clus : clusters() ) { os << "ID=" << clus.id() << " " << clus.pixels(); }
  return os;
}
