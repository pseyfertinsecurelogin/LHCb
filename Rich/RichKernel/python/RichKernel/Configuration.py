...
class RichConfigurableUser(LHCbConfigurableUser):
    ...
    def checkRadData(self, option, default):
        ...
        if len(self.getProp(option)) != 3:
            raise RuntimeError("Invalid radiator option '%s'=%s" % option %
                               self.getProp(option))
...
class RichTools(RichConfigurableUser):
    ...
    def initialize(self):
        # default values
        self.setRichDefaults("PixelCreatorType", {
            "Offline": "RawBuffer",
            "HLT": "RawBuffer"
        })
        self.setRichDefaults("PhotonCreatorType", {
            "Offline": "RecoPhotons",
            "HLT": "RecoPhotons"
        })
        self.setRichDefaults("PhotonRecoType", {
            "Offline": "Quartic",
            "HLT": "Quartic"
        })
        #self.setRichDefaults( "PhotonRecoType",      { "Offline" : "Adaptive",        "HLT" : "Adaptive" } )
        self.setRichDefaults("PhotonPredictorType", {
            "Offline": "CKthetaBands",
            "HLT": "CKthetaBands"
        })
        self.setRichDefaults("TrackCreatorType", {
            "Offline": "RecoTracks",
            "HLT": "RecoTracks"
        })
        self.setRichDefaults("TrackSelectorType", {
            "Offline": "Reco",
            "HLT": "Reco"
        })
        self.setRichDefaults("TrSegMakerType", {
            "Offline": "Detailed",
            "HLT": "Detailed"
        })
        self.setRichDefaults("GeomEffType", {
            "Offline": "CKMassRing",
            "HLT": "CKMassRing"
        })
        self.setRichDefaults("CkResType", {
            "Offline": "Functional",
            "HLT": "Functional"
        })
        self.setRichDefaults("SignalDetEffType", {
            "Offline": "Tabulated",
            "HLT": "Tabulated"
        })
    ...
    def sigDetEff(self, nickname="RichSignalDetectionEff", private=False):
        ...
        if type == "Tabulated":
            ...
            tool = self.__makeRichTool(Rich__Rec__TabulatedSignalDetectionEff,
                                       nickname, private)
        elif type == "NominalTabulated":
            from Configurables import Rich__Rec__NominalTabulatedSignalDetectionEff
            tool = self.__makeRichTool(
                Rich__Rec__NominalTabulatedSignalDetectionEff, nickname,
                private)
        else:
            raise RuntimeError(
                "Unknown signal detection efficiency tool '%s'" % type)
        return tool
    ...
    def ckResolution(self, nickname="RichCherenkovResolution", private=False):
        ...
        if type == "Binned":
            ...
            tool = self.__makeRichTool(
                Rich__Rec__BinnedCKResVthetaForRecoTracks, nickname, private)
        elif type == "Interpolated":
            from Configurables import Rich__Rec__InterpCKResVthetaForRecoTracks
            tool = self.__makeRichTool(
                Rich__Rec__InterpCKResVthetaForRecoTracks, nickname, private)
        elif type == "Functional":
            from Configurables import Rich__Rec__FunctionalCKResForRecoTracks
            tool = self.__makeRichTool(Rich__Rec__FunctionalCKResForRecoTracks,
                                       nickname, private)
        else:
    ...
    def geomEff(self, nickname="RichGeomEff", private=False):
        ...
        if type == "CKMassRing":
            ...
            tool = self.__makeRichTool(Rich__Rec__GeomEffPhotonTracing,
                                       nickname, private)
    ...
    def trackSelector(self, nickname="TrackSelector", private=False):
        ...
        if selType == "Reco":
            ...
            tool = self.__makeRichTool(Rich__Rec__MC__MCTrueTrackSelector,
                                       nickname, private)
        else:
    ...
    def trackCreator(self, nickname="RichTrackCreator", private=False):
        ...
        if trackType == "RecoTracks":
            ...
            tool = self.__makeRichTool(Rich__Rec__TrackCreatorFromRecoTracks,
                                       nickname, private)
    ...
    def photonPredictor(self, nickname="RichPhotonPredictor", private=False):
        ...
        if predType == "CKthetaBands":
            ...
            tool = self.__makeRichTool(Rich__Rec__CKthetaBandsPhotonPredictor,
                                       nickname, private)
        elif predType == "Simple":
            from Configurables import Rich__Rec__SimplePhotonPredictor
            tool = self.__makeRichTool(Rich__Rec__SimplePhotonPredictor,
                                       nickname, private)
            ...
            tool = self.__makeRichTool(
                Rich__Rec__MC__PhotonPredictorUsingMCTruth, nickname, private)
        else:
    ...
    def photonCreator(self, nickname="RichPhotonCreator", private=False):
        ...
        if phottype == "RecoPhotons":
            ...
            tool = self.__makeRichTool(
                Rich__Rec__MC__PhotonCreatorCheatedTrackDir, nickname, private)
            self.__makeRichTool(Rich__Rec__PhotonCreator,
                                "RichDelPhotonCreator", False)
        elif phottype == "SmearCKTheta":
            from Configurables import (
                Rich__Rec__PhotonCreator,
                Rich__Rec__MC__PhotonCreatorWithGaussianCKSmear)
            tool = self.__makeRichTool(
                Rich__Rec__MC__PhotonCreatorWithGaussianCKSmear, nickname,
                private)
            self.__makeRichTool(Rich__Rec__PhotonCreator,
                                "RichDelegatedPhotonCreator", False)
        else:
    ...
    def photonReco(self, nickname="PhotonParams", private=True):
        ...
        if recoType == "Quartic":
            ...
            tool = self.__makeRichTool(Rich__Rec__PhotonRecoUsingQuarticSoln,
                                       nickname, private)
        elif recoType == "EstiFromRadius":
            from Configurables import Rich__Rec__PhotonRecoUsingCKEstiFromRadius
            tool = self.__makeRichTool(
                Rich__Rec__PhotonRecoUsingCKEstiFromRadius, nickname, private)
        elif recoType == "Adaptive":
            from Configurables import (
                Rich__Rec__AdaptivePhotonReco,
                Rich__Rec__PhotonRecoUsingCKEstiFromRadius)
            slowName = "AdaptiveSlowPhotonReco"
            fastName = "AdaptiveFastPhotonReco"
            tool = self.__makeRichTool(Rich__Rec__AdaptivePhotonReco, nickname,
                                       private)
            self.toolRegistry().Tools += [
                "Rich::Rec::PhotonRecoUsingQuarticSoln/" + slowName,
    ...
    def smartIDTool(self):
        ...
        smarts = Rich__DAQ__RawBufferToSmartIDsTool(
            "ToolSvc.RichSmartIDDecoder")
    ...
    def expectedTrackSignal(self,
                            nickname="RichExpectedTrackSignal",
                            private=False):
        from Configurables import Rich__Rec__ExpectedTrackSignal
        return self.__makeRichTool(Rich__Rec__ExpectedTrackSignal, nickname,
                                   private)
    ...
    def massHypoRingCreator(self, nickname="RichMassHypoRings", private=False):
        ...
        return self.__makeRichTool(Rich__Rec__MassHypothesisRingCreator,
                                   nickname, private)
