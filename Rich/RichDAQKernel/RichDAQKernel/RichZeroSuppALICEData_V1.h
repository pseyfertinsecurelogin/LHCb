...
namespace Rich::DAQ {
  ...
  namespace RichZeroSuppALICEDataV1 {
    ...
    public:
       ...
       */
      explicit RichZeroSuppALICEData( const Level0ID l0ID, const LHCb::RichSmartID::Vector& digits,
                                      const bool extendedFormat = false, const LHCb::ODIN* odin = nullptr )
    ...
    private: // methods
      ...
      inline void updateOffset( const int address, const bool firstWord, int& last_address, int& last_offset ) const {
        if ( !firstWord ) {
          if ( 0 == address ) {
            ++last_offset;
          } else if ( 1 == address ) {
