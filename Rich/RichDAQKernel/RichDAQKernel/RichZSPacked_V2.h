...
namespace Rich::DAQ {
  ...
  namespace RichZSPacked_V2 {
    ...
    public: // methods
      /// Copy Constructor
      RichZSPacked( const RichZSPacked& word ) : m_data( word.data() ), m_aliceMode( word.aliceMode() ) {}
    ...
    public:
       ...
       */
      inline ShortType addressFromRowCol( const ShortType row, const ShortType col, const ShortType subPix = 0 ) const {
        return ( !aliceMode() ? ( ( MaxDataSize - 1 - row ) * 4 + col / BitsField )
                              : ( 4 * ( NumAlicePixelsPerLHCbPixel - 1 - subPix ) +
                                  4 * ( MaxDataSize - 1 - row ) * NumAlicePixelsPerLHCbPixel + col / BitsField ) %
                                    ( 1 + MaxZSAddress ) );
      }
       ...
       */
      inline ShortType rowFromAddress( const ShortType address, const ShortType aliceModeOffset = 0 ) const {
        return ( !aliceMode() ? ( MaxDataSize - 1 - ( address / 4 ) )
                              : ( MaxDataSize - 1 -
