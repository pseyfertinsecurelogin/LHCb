...
StatusCode Rich::Future::Converter_Imp::Error( const std::string& Message, const StatusCode Status ) const {
  return this->Print( Message, MSG::ERROR, Status );
}

// print and count the warning
StatusCode Rich::Future::Converter_Imp::Warning( const std::string& Message, const StatusCode Status ) const {
  return this->Print( Message, MSG::WARNING, Status );
}

// print info message
StatusCode Rich::Future::Converter_Imp::Info( const std::string& Message, const StatusCode Status ) const {
  return this->Print( Message, MSG::INFO, Status );
}

// print Debug message
StatusCode Rich::Future::Converter_Imp::Debug( const std::string& Message, const StatusCode Status ) const {
  return this->Print( Message, MSG::DEBUG, Status );
}

// print verbose message
StatusCode Rich::Future::Converter_Imp::Verbose( const std::string& Message, const StatusCode Status ) const {
  return this->Print( Message, MSG::VERBOSE, Status );
}

// print the message
StatusCode Rich::Future::Converter_Imp::Print( const std::string& Message, const MSG::Level level,
                                               const StatusCode Status ) const {
  this->msgStream( level ) << Message << endmsg;
  return Status;
}

IToolSvc* Rich::Future::Converter_Imp::toolSvc() const { return m_toolSvc; }

IDataProviderSvc* Rich::Future::Converter_Imp::detSvc() const { return m_detSvc; }

IChronoStatSvc* Rich::Future::Converter_Imp::chronoSvc() const { return m_chronoSvc; }
...
StatusCode Rich::Future::Converter_Imp::release( const IInterface* interface ) const {
  if ( nullptr == interface ) { return Error( "release(IInterface):: IInterface* points to NULL!" ); }
...
StatusCode Rich::Future::Converter_Imp::releaseTool( const IAlgTool* algTool ) const {
  ...
  if ( this->toolSvc() == nullptr ) { return Error( "releaseTool(IAlgTool):: IToolSvc* points to NULL!" ); }
  // find a tool in the list of active tools
  auto it = std::find( m_tools.rbegin(), m_tools.rend(), algTool );
  if ( m_tools.rend() == it ) { return Warning( "releaseTool(IAlgTool):: IAlgTool* is not active" ); } // get the tool
  IAlgTool* t = *it;
  // cache name
...
StatusCode Rich::Future::Converter_Imp::releaseSvc( const IInterface* Svc ) const {
  ...
  if ( svc.isValid() ) {
    ...
    if ( it == m_services.end() ) { return Warning( "releaseSvc(IInterface):: IInterface* is not active" ); }
    if ( msgLevel( MSG::DEBUG ) ) { debug() << "Releasing service '" << it->first << "'" << endmsg; }
    m_services.erase( it );
