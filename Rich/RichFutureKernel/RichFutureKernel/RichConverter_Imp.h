...
namespace Rich::Future {
  ...
  /** @class Converter_Imp RichFutureKernel/RichConverter.h
   *
  ...
  public: // tools and services
    ...
    template <class TOOL>
    TOOL* tool( const std::string& type, const std::string& name = "", const IInterface* parent = nullptr,
    ...
    inline SmartIF<SERVICE> svc( const std::string& name, const bool create = true ) const {
    ...
    // ============================================================================
    inline void addToToolList( IAlgTool* tool ) const { m_tools.push_back( tool ); }
    // ============================================================================
  ...
  public: // messaging
    ...
    inline MsgStream& msgStream( const MSG::Level level ) const {
      if ( !m_msgStream ) { m_msgStream.reset( new MsgStream( this->msgSvc(), this->name() ) ); }
      return *m_msgStream << level;
    }

    /// Delete the current messaging object
    inline void resetMsgStream() const { m_msgStream.reset( nullptr ); }
    ...
    /// print info message
    StatusCode Info( const std::string& Message, const StatusCode Status = StatusCode::SUCCESS ) const;

    /// print debug message
    StatusCode Debug( const std::string& Message, const StatusCode Status = StatusCode::SUCCESS ) const;

    /// print verbose message
    StatusCode Verbose( const std::string& Message, const StatusCode Status = StatusCode::SUCCESS ) const;

    /// print the message
    StatusCode Print( const std::string& Message, const MSG::Level level = MSG::INFO,
  ...
  protected:
     ...
     */
    template <class TYPE>
    void declareProperty( const std::string& /* name */, TYPE& /* property */, const std::string& /* doc = "none" */ ) {
    }
