...
void
RawDataFormatTool::handle( const Incident &incident )
{
  // Update prior to start of new event.
  if ( IncidentType::BeginEvent == incident.type() ) { InitEvent(); }
  // End of event
...
void
RawDataFormatTool::printL1Stats( const L1TypeCount &count, const std::string &title ) const
{
  ...
  if ( !count.empty() )
  {
    ...
    // Printout
    info() << LINES << endmsg << "                             " << title << " : " << m_evtCount
           << " events" << endmsg;
    ...
    {
      const double evtCount1 = ( s[ Rich::Rich1 ] > 0 ? (double)( nEvFills[ Rich::Rich1 ] ) /
                                                          (double)( s[ Rich::Rich1 ] ) :
                                                        0 );
      info() << " Rich1 Average                   | L1 size ="
             << occ1( totBanks[ Rich::Rich1 ], evtCount1 )
             << " pds :" << occ2( totWordSize[ Rich::Rich1 ], evtCount1 )
             << " words :" << occ2( totHits[ Rich::Rich1 ], evtCount1 ) << " hits / event"
             << endmsg;
    }
    if ( totBanks[ Rich::Rich2 ] > 0 )
    {
      const double evtCount2 = ( s[ Rich::Rich2 ] > 0 ? (double)( nEvFills[ Rich::Rich2 ] ) /
                                                          (double)( s[ Rich::Rich2 ] ) :
                                                        0 );
      info() << " Rich2 Average                   | L1 size ="
             << occ1( totBanks[ Rich::Rich2 ], evtCount2 )
             << " pds :" << occ2( totWordSize[ Rich::Rich2 ], evtCount2 )
             << " words :" << occ2( totHits[ Rich::Rich2 ], evtCount2 ) << " hits / event"
             << endmsg;
    }
                                   ...
                                   const LHCb::ODIN *               odin ) const
{
  ...
  if ( LHCb5 == version )
  {
    ...
    if ( !m_zeroSupp || zsData->tooBig() )
    {
      delete zsData;
      dataBank =
        (PDDataBank *)new RichDAQ_LHCb5::NonZeroSuppLHCb( l0ID, smartIDs, m_extendedFormat, odin );
    }
    else
    {
  ...
  if ( msgLevel( MSG::VERBOSE ) )
  {
    ...
    verbose() << endmsg << "   Creating encoding HPD " << smartIDs.front().pdID() << " bank from "
              << smartIDs.size() << " RichSmartIDs :-" << endmsg;
...
void
RawDataFormatTool::fillRawEvent( const LHCb::RichSmartID::Vector &smartIDs,
                                 const BankVersion                version ) const
{
  ...
  for ( const auto &L1 : L1Data )
  {
    ...
    for ( auto iIngress = L1.second.begin(); iIngress != L1.second.end(); ++iIngress, ++ingress )
    {
      ...
      if ( version != LHCb0 && version != LHCb1 && version != LHCb2 && version != LHCb3 &&
           version != LHCb4 && version != MaPMT0 )
      {
      ...
      _ri_debug << "  Found " << ( *iIngress ).second.pdData().size() << " HPD(s) with data"
                << endmsg;
      ...
      for ( const auto &HPD : ( *iIngress ).second.pdData() )
      {
        ...
        if ( version != MaPMT0 )
        {
          ...
          std::unique_ptr< const PDDataBank > hpdData(
            createDataBank( HPD.second.smartIDs(), version, odin() ) );
          if ( hpdData.get() )
          {
    ...
    _ri_debug << " : Size " << boost::format( "%4i" ) % dataBank.size() << " words : Version "
              << version << endmsg;
...
void
RawDataFormatTool::decodeToSmartIDs( const LHCb::RawBank &bank, L1Map &decodedData ) const
{
  ...
  { // All OK so decode
    ...
    else if ( m_richIsActive[ rich ] )
    {
      ...
      if ( version == LHCb5 || version == FlatList ) { decodeToSmartIDs_2007( bank, decodedData ); }
...
void
RawDataFormatTool::decodeToSmartIDs_2007( const LHCb::RawBank &bank, L1Map &decodedData ) const
{
  ...
  if ( bankSize > 0 )
  {
    ...
    while ( lineC < bankSize )
    {
      ...
      const bool odinOK =
        ( !m_decodeUseOdin ?
            true :
            ( !m_checkODINEventsIDs ||
              ingressWord.eventID() == EventID( odin()->eventNumber() ) ) &&
...
void
RawDataFormatTool::suppressHotPixels( const LHCb::RichSmartID &  pdID,
...
void
RawDataFormatTool::decodeToSmartIDs_2006TB( const LHCb::RawBank &bank, L1Map &decodedData ) const
{
  using namespace Rich::DAQ::HPD;
...
void
RawDataFormatTool::decodeToSmartIDs_DC0406( const LHCb::RawBank &bank, L1Map &decodedData ) const
{
  ...
  if ( bankSize > 0 )
  {
    ...
    while ( lineC < bankSize )
    {
      ...
      if ( header.startPD() )
      {
        ...
        // L1 input number
        const Level1Input l1Input =
          ( m_useFakeHPDID ? fake_l1Input : m_richSys->level1InputNum( pdID ) );
                                     ...
                                     L1Map &                  decodedData ) const
{
  ...
  for ( const auto &loc : taeLocations )
  {
      ...
      if ( bank )
      {
        ...
        catch ( const GaudiException &expt )
        {
          ...
          mess << "Error decoding bank ID=" << bank->sourceID()
               << " version=" << bankVersion( *bank ) << " '" << expt.message() << "' '"
               << expt.tag() << "'";
          Error( mess.str() ).ignore();
          // dump the full bank
    ...
    if ( !m_useFakeHPDID )
    {
      _ri_debug << "Decoded in total " << richBanks.size() << " RICH Level1 bank(s) for RawEvent '"
                << loc << "'" << endmsg;
    }
...
LHCb::RawEvent *
RawDataFormatTool::rawEvent() const
{
...
RawDataFormatTool::dummyMap() const
{
  ...
  if ( dummyMap.empty() )
  {
    ...
    _ri_debug << "Created " << dummyMap.size()
              << " entries in empty L1 map : L1HardwareIDs = " << m_richSys->level1HardwareIDs()
              << endmsg;
  }
  return dummyMap;
}
...
void
RawDataFormatTool::dumpRawBank( const LHCb::RawBank &bank, MsgStream &os ) const
{
