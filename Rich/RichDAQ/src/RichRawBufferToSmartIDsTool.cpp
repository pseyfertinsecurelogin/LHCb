...
RawBufferToSmartIDsTool::RawBufferToSmartIDsTool( const std::string& type, const std::string& name,
                                                  const IInterface* parent )
    : ToolBase( type, name, parent ) {
  // Defined interface
  declareInterface<IRawBufferToSmartIDsTool>( this );
  // JOs
  declareProperty( "RawEventLocations", m_rawEventLocs = IRawDataFormatTool::RawEventLocations( 1, "" ) );
}
...
RawBufferToSmartIDsTool::richSmartIDs( const IRawBufferToSmartIDsTool::RawEventLocations& taeLocs,
                                       const LHCb::RichSmartID pdID, const bool createIfMissing ) const {
...
const LHCb::RichSmartID::Vector& RawBufferToSmartIDsTool::richSmartIDs( const LHCb::RichSmartID pdID,
...
unsigned int RawBufferToSmartIDsTool::nTotalHits( const Rich::DetectorType rich ) const {
  return countTotalHits( allRichSmartIDs(), rich );
}

unsigned int RawBufferToSmartIDsTool::nTotalHits( const RawEventLocations& taeLocs,
...
unsigned int RawBufferToSmartIDsTool::countTotalHits( const Rich::DAQ::L1Map&  data,
  ...
  for ( const auto& L1 : data ) {
    ...
    } catch ( const GaudiException& excpt ) { Warning( excpt.message() ).ignore(); }
  }
