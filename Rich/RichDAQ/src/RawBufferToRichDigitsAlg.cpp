...
RawBufferToRichDigitsAlg::RawBufferToRichDigitsAlg( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode RawBufferToRichDigitsAlg::initialize() {
  ...
  if ( m_decodeOnly ) { info() << "Will only decode Raw Buffer -> No RichDigits produced" << endmsg; } // Acquire tools
    ...
    for ( const auto& L1 : data ) {
      ...
      for ( const auto& In : L1.second ) {
        ...
        for ( const auto& HPD : In.second.pdData() ) {
          ...
          for ( const auto& ID : HPD.second.smartIDs() ) { digits->insert( new LHCb::RichDigit(), ID ); }
        }
      }
    }

    // Final printout
    _ri_debug << "Successfully registered " << digits->size() << " RichDigits at " << m_richDigitsLoc << endmsg;
