...
namespace OTDAQ {
  ...
  public:
    ...
    GolHeader( int iWord, int iStation, int iLayer, int iQuarter, int iModule, int iOtisErFlag, int iNumberOfHits );
  ...
  inline GolHeader::GolHeader( int iWord, int iStation, int iLayer, int iQuarter, int iModule, int iOtisErFlag,
                               int iNumberOfHits ) {

    m_data = ( iWord << wordBits ) + ( iStation << stationBits ) + ( iLayer << layerBits ) +
             ( iQuarter << quarterBits ) + ( iModule << moduleBits ) + ( iOtisErFlag << otisErFlagBits ) +
  ...
  inline unsigned int GolHeader::module() const { return (unsigned int)( ( m_data & moduleMask ) >> moduleBits ); }

  inline unsigned int GolHeader::quarter() const { return (unsigned int)( ( m_data & quarterMask ) >> quarterBits ); }

  inline unsigned int GolHeader::layer() const { return (unsigned int)( ( m_data & layerMask ) >> layerBits ); }

  inline unsigned int GolHeader::station() const { return (unsigned int)( ( m_data & stationMask ) >> stationBits ); }

  inline bool GolHeader::word() const { return 0 != ( ( m_data & wordMask ) >> wordBits ); }

  inline unsigned int GolHeader::returnInt( const GolHeader& gol ) { return gol.m_data >> 0; }

  inline std::ostream& operator<<( std::ostream& os, const GolHeader& header ) {
    return os << "[ data=" << header.m_data << ", buffer size=" << header.hitBufferSize()
              << ", num hits=" << header.numberOfHits() << ", module ID=(" << header.station() << "," << header.layer()
