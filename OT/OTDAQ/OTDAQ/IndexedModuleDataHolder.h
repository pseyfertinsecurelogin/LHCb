...
namespace OTDAQ {
  ...
  public:
    ...
    friend bool operator==( const IndexedModuleCounter& lhs, const IndexedModuleCounter& rhs ) {
      return lhs.m_counter == rhs.m_counter;
    }
    IndexedModuleCounter& operator++() {
      ++m_counter;
      return *this;
    }
    IndexedModuleCounter& operator--() {
  ...
  public:
    ...
    MODULE& module( const unsigned int station, const unsigned int layer, const unsigned int quarter,
    ...
    bool isvalidID( const unsigned int station, const unsigned int layer, const unsigned int quarter,
                    const unsigned int module ) const {
      return station - 1 <= NumStations - 1 && layer <= NumLayers - 1 && quarter <= NumQuadrants - 1 &&
             module - 1 <= NumModules - 1;
    }
