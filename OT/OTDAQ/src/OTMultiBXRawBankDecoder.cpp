...
public:
  ...
  OTMultiBXRawBankDecoder( const std::string& type, const std::string& name, const IInterface* parent );
  ...
  /// Decode all gol headers.
  StatusCode decodeGolHeaders( const LHCb::RawEvent& rawevent ) const override {
    return m_decoder->decodeGolHeaders( rawevent );
  }
...
private:
  ...
  Gaudi::Property<std::vector<std::string>> m_rawEventLocations{
      this, "RawEventLocations", {LHCb::RawEventLocation::Default, "Prev1/DAQ/RawEvent", "Next1/DAQ/RawEvent"}};
  ...
  public:
    ...
    size_t totalNumberOfHits() const {
      return std::accumulate( begin(), end(), size_t( 0 ),
                              []( size_t n, const LocalHelpers::ModuleHitData& mod ) { return n + mod.size(); } );
    }
...
//=============================================================================
OTMultiBXRawBankDecoder::OTMultiBXRawBankDecoder( const std::string& type, const std::string& name,
...
StatusCode OTMultiBXRawBankDecoder::initialize() {
  ...
  for ( auto ilocation = m_rawEventLocations.begin(); ilocation != m_rawEventLocations.end(); ++ilocation )
    msg << "\'" << *ilocation << "\', ";
  info() << msg.str() << endmsg;
...
StatusCode OTMultiBXRawBankDecoder::decode( LHCb::OTLiteTimeContainer& ottimes ) const {
  ...
  for ( const auto& mod : *m_hitdata ) ottimes.insert( ottimes.end(), mod.begin(), mod.end() );
  return StatusCode::SUCCESS;
}

StatusCode OTMultiBXRawBankDecoder::decodeAll() const {
  StatusCode sc = StatusCode::SUCCESS;
  for ( auto ilocation = m_rawEventLocations.begin(); ilocation != m_rawEventLocations.end() && sc.isSuccess();
        ++ilocation ) {
    ...
    if ( NULL == event ) {
      Warning( std::string( "No raw event buffer at location " ) + *ilocation, StatusCode::SUCCESS, 0 ).ignore();
    } else {
      ...
      if ( sc.isSuccess() ) {
        ...
        if ( sc.isSuccess() ) {
          ...
          for ( LHCb::OTLiteTimeContainer::const_iterator ihit = ottimes.begin(); ihit != ottimes.end(); ++ihit ) {
            ...
            if ( m_selectEarliestHit ) {
              for ( ; jhit != module.end() && newhit.channel().straw() != jhit->channel().straw(); ++jhit ) {}
              if ( jhit != module.end() && jhit->calibratedTime() > newhit.calibratedTime() ) *jhit = newhit;
            } else {
              for ( ; jhit != module.end() && !( newhit.channel().straw() == jhit->channel().straw() &&
                                                 std::abs( jhit->calibratedTime() - newhit.calibratedTime() ) < 1 );
                    ++jhit ) {}
            }
            if ( jhit == module.end() ) module.push_back( newhit );
          }
        }
      }
    }
...
LHCb::OTLiteTime OTMultiBXRawBankDecoder::time( LHCb::OTChannelID channel ) const {
  ...
  auto jhit = std::find_if( moduledata.begin(), moduledata.end(),
                            [&]( const LHCb::OTLiteTime& t ) { return t.channel().straw() == channel.straw(); } );
