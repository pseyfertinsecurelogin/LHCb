...
namespace OTDAQ {
  ...
  public:
    ...
    GolHeaderDC06( int iWord, int iStation, int iLayer, int iQuarter, int iModule, int iOtisErFlag, int iSize );
  ...
  inline GolHeaderDC06::GolHeaderDC06( int iWord, int iStation, int iLayer, int iQuarter, int iModule, int iOtisErFlag,
                                       int iSize ) {
  ...
  inline unsigned int GolHeaderDC06::size() const { return (unsigned int)( ( m_data & sizeMask ) >> sizeBits ); }

  inline unsigned int GolHeaderDC06::otisErFlag() const {
    return (unsigned int)( ( m_data & otisErFlagMask ) >> otisErFlagBits );
  }

  inline unsigned int GolHeaderDC06::module() const { return (unsigned int)( ( m_data & moduleMask ) >> moduleBits ); }

  inline unsigned int GolHeaderDC06::quarter() const {
    return (unsigned int)( ( m_data & quarterMask ) >> quarterBits );
  }

  inline unsigned int GolHeaderDC06::layer() const { return (unsigned int)( ( m_data & layerMask ) >> layerBits ); }

  inline unsigned int GolHeaderDC06::station() const {
    return (unsigned int)( ( m_data & stationMask ) >> stationBits );
  }

  inline bool GolHeaderDC06::word() const { return 0 != ( ( m_data & wordMask ) >> wordBits ); }

  inline unsigned int GolHeaderDC06::returnInt( const GolHeaderDC06& gol ) { return gol.m_data >> 0; }
