...
namespace OTDAQ {
  ...
  struct CompareChannel {
    bool operator()( const OTDAQ::RawHit& lhs, const OTDAQ::RawHit& rhs ) { return lhs.channel() < rhs.channel(); }
  };
  ...
  class OTGol {
  public:
    OTGol( size_t id ) : m_id( id ), m_channels(), m_encode( false ){};
  ...
  class OTBank {
  public:
    OTBank( size_t id, size_t nGols )
        : m_id( id ), m_nGols( nGols ), m_gols(), m_nChannels( 0u ), m_encode( false ), m_nGolsToEncode( 0u ) {
...
const OTRawBankEncoder::OTRawBank& OTRawBankEncoder::createRawBank( const OTDAQ::OTBank& bank ) const {
  ...
  if ( isDebug )
    debug() << "Creating OTRawBank for OTBank with id = " << bank.id() << " containing " << bank.nGols()
            << " GOLS of which " << bank.nGolsToEncode() << " are non-empty and contain " << bank.nChannels()
            << " channels." << endmsg;
  ...
  for ( const auto& gol : bank ) {
    ...
    if ( gol.encode() || m_addEmptyBanks.value() ) {
      if ( !gol.encode() && m_addEmptyBanks.value() ) { // Empty. Always add empty gols in hardware
        ...
        if ( isDebug )
          debug() << "Going to add empty gol with id " << gol.id() << " to bank with id " << bank.id() << endmsg;
        ...
        if ( isDebug )
          debug() << "Creating gol header with id = " << module << " station = " << station << " layer = " << layer
                  << " quarter = " << quarter << endmsg;
        /// Create gol header.
        ...
        pipeToBuffer( OTDAQ::GolHeader( 0u, gol.firstChannel()->station(), gol.firstChannel()->layer(),
                                        gol.firstChannel()->quarter(), gol.firstChannel()->module(), 0u,
                                        gol.nChannels() ),
                      buffer );
      ...
      for ( auto firstChannel = gol.firstChannel(); firstChannel != gol.lastChannel(); ++firstChannel ) {
        if ( isVerbose )
          verbose() << " Gol ID  = " << gol.id() << " Station = " << firstChannel->station()
                    << " Layer   = " << firstChannel->layer() << " Quarter = " << firstChannel->quarter()
                    << " Module  = " << firstChannel->module() << " nHits   = " << gol.nChannels() << endmsg;
...
StatusCode OTRawBankEncoder::encodeChannels( const std::vector<LHCb::OTChannelID>& channels ) const {
  ...
  for ( const auto& chan : channels ) {
    ...
    if ( bankID == 0u || bankID > m_banks.size() ) {
      ...
      if ( isVerbose )
        verbose() << "Added channel " << chan << " to bank with id 0x0" << std::hex << m_banks[bankID - 1u].id()
                  << endmsg;
    }
  }
