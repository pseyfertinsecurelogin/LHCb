...
namespace OTRawBankDecoderHelpers {
  ...
  public:
    ...
    void setData( unsigned int size, const unsigned short* data, int bankversion, double tdcconversion,
  ...
  public:
    Detector( const OTRawBankDecoder& parent, const DeOTDetector& det, const OTDAQ::ChannelMap::Detector& channelmap )
        : m_parent( parent ) {
      ...
      for ( const auto& mod : det.modules() ) {
        ...
        amodule.setChannelMap(
            channelmap.module( mod->stationID(), mod->layerID(), mod->quarterID(), mod->moduleID() ) );
        maxidx = std::min( maxidx, size_t( std::min( std::numeric_limits<decltype( amodule.m_idx.first )>::max(),
    ...
    size_t totalNumberOfHits() const { return totalNumberOfHitsInWindow(); }

    size_t totalNumberOfHitsInWindow() const {
      if ( UNLIKELY( m_totalNumberOfHits == 0 ) ) {
        m_totalNumberOfHits = std::accumulate(
            begin(), end(), size_t( 0 ), []( size_t acc, const Module& m ) { return acc + m.countHitsInWindow(); } );
      }
  ...
  inline LHCb::OTLiteTimeRange Module::ottimes() const {
    return {m_parent->m_ottimes.begin() + m_idx.first, m_parent->m_ottimes.begin() + m_idx.second};
  }
  ...
  inline bool Module::isHitInWindow( unsigned short data, bool vetoOOTPairs ) const {
    ...
    if ( UNLIKELY( hit.channel() == ostraw1 ) )
      ostraw1 = ostraw2;
    else if ( UNLIKELY( hit.channel() == ostraw2 ) )
      ostraw2 = ostraw1;
    bool haveOutOfTimePair = false;
    for ( const unsigned short *ptr = std::lower_bound( m_data, m_data + m_size, ostraw1,
                                                        []( const unsigned short data, unsigned straw ) {
                                                          OTDAQ::RawHit hit( data );
                                                          return hit.channel() < straw;
                                                        } );
      ...
      if ( ostraw2 < otherhit.channel() ) {
        ...
        else if ( UNLIKELY( ptr == ( m_data + m_size - 2 ) && *ptr == 0 ) )
          continue;
        else
          break;
      }
  ...
  inline void Module::addHit( unsigned short data, const bool vetoOOTPairs ) const {
    ...
    m_parent->m_ottimes.emplace_back( LHCb::OTChannelID{m_station, m_layer, m_quarter, m_module, straw, tdctime}, t );
    ++m_idx.second;
  }
  ...
  inline size_t Module::countHitsInWindowDC06() const {
    ...
    auto n = std::count_if( m_data, end, [&]( const unsigned short d ) { return isHitInWindow( d, vetoOOTPairs ); } );
  ...
  inline size_t Module::decodeDC06() const {
    if ( !m_isdecoded ) {
      ...
      if ( m_size != 0 ) {
        ...
        std::for_each( begin, end, [&]( unsigned int hit ) { this->addHit( hit, vetoOOTPairs ); } );
  ...
  inline size_t Module::countHitsInWindowV3() const {
    ...
    if ( m_window.second < m_window.first ) return size();
    const bool vetoOOTPairs = m_parent->m_parent.m_vetoOutOfTimeHitPairs;
    return std::count_if( m_data, m_data + m_size,
                          [&]( const unsigned short d ) { return isHitInWindow( d, vetoOOTPairs ); } );
  }
  ...
  inline size_t Module::decodeV3() const {
    if ( !m_isdecoded ) {
      ...
      std::for_each( m_data, m_data + m_size, [=]( unsigned short hit ) { this->addHit( hit, vetoOOTPairs ); } );
      m_isdecoded = true;
    }
    return m_idx.second - m_idx.first;
  }
...
//=============================================================================
OTRawBankDecoder::OTRawBankDecoder( const std::string& type, const std::string& name, const IInterface* parent )
    : base_class( type, name, parent ) {
  // new for decoders, initialize search path, and then call the base method
  m_rawEventLocations = {LHCb::RawEventLocation::Tracker, LHCb::RawEventLocation::Other,
...
StatusCode OTRawBankDecoder::initialize() {
  ...
  info() << " countsPerBX = " << m_countsPerBX.value() << " numberOfBX  = " << m_numberOfBX.value()
         << " timePerBX = " << m_timePerBX.value() << " ForceBankVersion = " << m_forcebankversion.value();
  if ( m_timewindow.value().first < m_timewindow.value().second ) {
    ...
    if ( m_vetoOutOfTimeHitPairs ) info() << ", vetoing out of time hit pairs";
  }
  info() << endmsg;
  ...
  for ( idata = begin; idata < end; ++idata ) {
    ...
    if ( numhits > 0 ) {
      ...
      if ( !m_detectordata->isvalidID( station, layer, quarter, module ) ) {
        ...
        m_detectordata->module( station, layer, quarter, module )
            .setData( numhits, firsthit, bankversion, m_nsPerTdcCount, m_timewindow );
...
StatusCode OTRawBankDecoder::decodeGolHeadersV3( const LHCb::RawBank& bank, int bankversion ) const {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "OTSpecificHeader in bank:" << otheader << endmsg;
  if ( otheader.error() ) {
  ...
  if ( numgols != otheader.numberOfGOLs() ) {
    std::ostringstream mess;
    mess << "Found " << otheader.numberOfGOLs() << " in bank header, but read only " << numgols << " from bank.";
    Warning( mess.str(), StatusCode::FAILURE, 0 ).ignore();
    decodingerror = true;
  }
...
StatusCode OTRawBankDecoder::decodeGolHeaders( const LHCb::RawEvent& event ) const {
  if ( &event != m_detectordata->rawEvent() ) {
    ...
    if ( msgLevel( MSG::DEBUG ) )
      debug() << "Decoding GOL headers in OTRawBankDecoder. Number of OT banks is " << OTBanks.size() << endmsg;
    ...
    for ( const auto& ibank : OTBanks ) {
      ...
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "OT Bank sourceID= " << ibank->sourceID() << " size=" << ibank->size() / 4
                << " bankversion=" << ibank->version() << endmsg;
      ...
      case OTBankVersion::v3:
        ...
        break;
      default:
        Warning( "Cannot decode OT raw buffer bank version " + std::to_string( bVersion ) +
                     " with this version of OTDAQ",
...
StatusCode OTRawBankDecoder::decode( LHCb::OTLiteTimeContainer& ottimes ) const {
  ...
  size_t numhits =
      std::accumulate( m_detectordata->begin(), m_detectordata->end(), size_t{0},
                       [&]( size_t n, const OTRawBankDecoderHelpers::Module& m ) { return n + decodeModule( m ); } );
  // reserve and copy
  ottimes.clear();
...
StatusCode OTRawBankDecoder::decode( OTDAQ::RawEvent& otrawevent ) const {
  ...
  for ( const auto& bank : banks ) {
    ...
    otbankcontainer.emplace_back( bank->version(), bank->size(), OTDAQ::OTSpecificHeader( *idata ) );
    ...
    // check that everything is well aligned
    if ( idata != end ) {
      Warning( "GOL headers do not add up to buffer size.", StatusCode::FAILURE, 0 ).ignore();
      decodingerror = true;
    }
    // check that we have read the right number of GOLs
    if ( numgols != otspecificbank.header().numberOfGOLs() ) {
      Warning( "Found " + std::to_string( otspecificbank.header().numberOfGOLs() ) + " in bank header, but read " +
                   std::to_string( numgols ) + " from bank.",
               StatusCode::FAILURE, 0 )
          .ignore();
      decodingerror = true;
    }
...
LHCb::OTLiteTime OTRawBankDecoder::time( LHCb::OTChannelID channel ) const {
