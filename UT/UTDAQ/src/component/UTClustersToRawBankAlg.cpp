...
StatusCode UTClustersToRawBankAlgT<IReadoutTool>::execute() {
  ...
  for ( unsigned int iBoard = 0u; iBoard < nBoard; ++iBoard ) {
    ...
    RawBank* tBank = tEvent->createBank( UTDAQ::rawInt( aBoardID.id() ), m_bankType, UTDAQ::v4, bWriter.byteSize(),
                                         &( bWriter.dataBank()[0] ) );
...
void UTClustersToRawBankAlgT<IReadoutTool>::initEvent() {
  ...
  std::for_each( m_clusVectors.begin(), m_clusVectors.end(), []( UTClustersOnBoard& i ) { i.clear(); } );
  m_overflow = 0;
...
unsigned int UTClustersToRawBankAlgT<IReadoutTool>::bankSize( UTClustersOnBoard::ClusterVector& clusCont ) const {
  ...
  unsigned int nADC =
      std::accumulate( clusCont.begin(), clusCont.end(), 0u,
                       []( unsigned n, const UTClustersOnBoard::boardPair& p ) { return n + p.first->size(); } );
...
template <class IReadoutTool>
void UTClustersToRawBankAlgT<IReadoutTool>::writeBank( const UTClustersOnBoard::ClusterVector& clusCont,
                                                       LHCb::BankWriter& bWriter, const UTTell1ID aBoardID ) {
  ...
  for ( const auto& clus : clusCont ) {
    ...
    bWriter << UTClusterWord( clus.second, isf, aClus->size(), aClus->highThreshold() );
  } // clusCont
