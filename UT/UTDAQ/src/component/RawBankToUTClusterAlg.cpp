...
RawBankToUTClusterAlg::RawBankToUTClusterAlg( const std::string& name,
...
std::tuple<LHCb::UTClusters,LHCb::UTSummary>
RawBankToUTClusterAlg::operator()(const LHCb::ODIN& odin, const LHCb::RawEvent& rawEvt) const {
...
LHCb::UTSummary RawBankToUTClusterAlg::decodeBanks(const RawEvent& rawEvt,
...
void RawBankToUTClusterAlg::createCluster( const UTClusterWord& aWord,
  ...
  UTTell1Board::chanPair nearestChan = aBoard->DAQToOffline(fracStrip,bankVersion,UTDAQ::UTStripRepresentation(aWord.channelID()));

  aBoard->ADCToOffline(aWord.channelID(),adcs,bankVersion,offset,fracStrip);

  // make cluster +set things
  auto newCluster = std::make_unique<UTCluster>(this->word2LiteCluster(aWord,
                                                         nearestChan.first,
                                                         nearestChan.second),
...
LHCb::UTSummary RawBankToUTClusterAlg::createSummaryBlock(const RawEvent& rawEvt,
  ...
  return UTSummary(nclus, pcn, pcnsync, totalBytes,
