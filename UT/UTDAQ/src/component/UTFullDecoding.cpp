...
//=============================================================================
UTFullDecoding::UTFullDecoding( const std::string& name,
...
StatusCode UTFullDecoding::execute()
{
  ...
  for( auto itB = itf.begin(); itB != itf.end(); ++itB ) {
    ...
    for( UTTELL1Data::Data::iterator i =tell1Data.begin();
                                     i!=tell1Data.end(); ++i) {
    ...
    for(; w != p->end<unsigned int>(); ++w )  {    
      ...
      if(cntWD % 224 == 0){	// Each 224 words we have a new PP-FPGA        
        cntWD = 0;
        cntPP = (*(w + 219) & UTTELL1EventInfo::ChipAddrMask) 
          >> UTTELL1EventInfo::ChipAddrBits;
    ...
    UTTELL1Data* myData = new UTTELL1Data( tell1Data, tell1Header, 
                                           sentPP, eventInfo );
