...
public:
  ...
  /// constructer
  UTTell1Board( const UTTell1ID aBoard, const unsigned int stripsPerHybrid, const std::string& type );
  ...
  bool isInside( const LHCb::UTChannelID aOfflineChan, unsigned int& sectorIndex ) const;

  /// construct LHCb::UTChannelID from DAQ Channel
  chanPair DAQToOffline( const unsigned int fracStrip, const UTDAQ::version& version,
                         const UTDAQ::UTStripRepresentation aDAQChan ) const;

  /// construct LHCb::UTChannelID from DAQ Channel
  std::tuple<UTTell1Board::ExpandedChannelID, unsigned int, int>
  DAQToOfflineFull( const unsigned int fracStrip, const UTDAQ::version& version, unsigned int aDAQChan ) const;

  /// fill adc values offline
  void ADCToOffline( const unsigned int aDAQChan, LHCb::UTCluster::ADCVector& adcs, const int version,
                     const unsigned int offset, const unsigned int fracStrip ) const;

  /// construct DAQChannel from LHCb::UTChannelID
  unsigned int offlineToDAQ( const LHCb::UTChannelID aOfflineChan, const unsigned int sectorIndex, double isf ) const;
  ...
  friend std::ostream& operator<<( std::ostream& s, const UTTell1Board& obj ) { return obj.fillStream( s ); }
...
inline UTTell1ID UTTell1Board::boardID() const { return m_boardID; }

inline bool UTTell1Board::sameID( const UTTell1ID& id ) const { return m_boardID == id ? true : false; }

inline const std::vector<int>& UTTell1Board::orientation() const { return m_orientation; }

inline const std::vector<std::string>& UTTell1Board::serviceBoxes() const { return m_serviceBoxVector; }

inline const std::vector<LHCb::UTChannelID>& UTTell1Board::sectorIDs() const { return m_sectorsVector; }
