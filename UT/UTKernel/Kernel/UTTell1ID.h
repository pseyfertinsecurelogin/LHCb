...
public:
  ...
  UTTell1ID(unsigned int aRegion,
            unsigned int aSubID){
  ...
  explicit UTTell1ID(unsigned int id):
  m_id(id){
    m_isUT = false;
  }

  explicit UTTell1ID(unsigned int id, bool isUT):
  m_id(id){
    m_isUT = isUT;
  }
  ...
  /// comparison equality
  friend bool operator==(const UTTell1ID& lhs, const UTTell1ID& rhs)
  ...
  friend std::ostream& operator<< (std::ostream& s, const UTTell1ID& obj)
  {
...
inline unsigned int UTTell1ID::id() const
{
  return m_id;
}

inline unsigned int UTTell1ID::isUT() const
{
  return m_isUT;
}
