...
struct IUTReadoutTool: extend_interfaces<IAlgTool>{
  ...
  virtual UTDAQ::chanPair offlineChanToDAQ(const LHCb::UTChannelID aOfflineChan,
                                           double isf) const = 0 ;

  /// convert offline interStripFraction to DAQ interStripFraction
  virtual double interStripToDAQ(const LHCb::UTChannelID aOfflineChan, 
                                 const UTTell1ID aBoardID,
                                 const double isf) const = 0 ;

  virtual bool ADCOfflineToDAQ(const LHCb::UTChannelID aOfflineChan,
			       const UTTell1ID aBoardID,
