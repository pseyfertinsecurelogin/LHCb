...
std::ostream& LHCb::UTTELL1Data::fillStream( std::ostream& s ) const {
  ...
  s << "Sent PPs ";
  for ( std::vector<unsigned int>::const_iterator iter = m_sentPPs.begin(); iter != m_sentPPs.end(); ++iter ) {
    s << *iter << " " << std::endl;
  } // sent pp
  s << std::endl;
  ...
  for ( int unsigned i = 0; i < 24; ++i ) {
    ...
    s << "-----------|-----------|-----------|-----------#" << std::endl;
    ;
    s << outputvector_header[i][0] << " " << outputvector_header[i][1] << " " << outputvector_header[i][2] << "|"
      << outputvector_header[i][3] << " " << outputvector_header[i][4] << " " << outputvector_header[i][5] << "|"
      << outputvector_header[i][6] << " " << outputvector_header[i][7] << " " << outputvector_header[i][8] << "|"
      << outputvector_header[i][9] << " " << outputvector_header[i][10] << " " << outputvector_header[i][11] << "#"
      << std::endl;
  } // loop links
  s << "########################################################################" << std::endl;
  return s;
