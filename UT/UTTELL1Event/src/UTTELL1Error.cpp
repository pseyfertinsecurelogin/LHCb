...
//-----------------------------------------------------------------------------
// Implementation file for class : UTTELL1Error
//
// 2008-06-24 : M Needham
//-----------------------------------------------------------------------------

std::ostream& LHCb::UTTELL1Error::fillStream( std::ostream& s ) const {

  s << "Tell1 Sent " << sentWords() << " "
    << " words " << std::endl;
  ...
  if ( ErrorBankLength() == 0x14 ) {
    ...
    s << "(Event Info) Beetle3 PCN (dec): " << pcnBeetle3() << " |Beetle2 PCN (dec): " << pcnBeetle2()
      << " |Beetle1 PCN (dec): " << pcnBeetle1() << " |Beetle0 PCN (dec): " << pcnBeetle0() << std::endl;

    s << "(Event Info) Reserved bits (hex): " << std::hex << R3() << " |Beetle5 PCN (dec): " << std::dec << pcnBeetle5()
      << " |Beetle4 PCN (dec): " << pcnBeetle4() << std::endl;

    s << "(Event Info) Reserved bits (hex): " << std::hex << R4() << " |I Headers: |Beet.5 (dec): " << std::dec
      << iHeaderBeetle5() << " |Beet.4: " << iHeaderBeetle4() << " |Beet.3: " << iHeaderBeetle3()
      << " |Beet.2: " << iHeaderBeetle2() << " |Beet.1: " << iHeaderBeetle1() << " |Beet.0: " << iHeaderBeetle0()
      << std::endl;
  }

  // optional nsz and pedestal bank words
  if ( hasNZS() == true ) { s << "NSZ bank length: " << nzsBankLength() << std::endl; }
  if ( hasPed() == true ) { s << "Pedestal bank length: " << PedBankLength() << std::endl; }
...
bool LHCb::UTTELL1Error::badLink( const unsigned int beetle, const unsigned int port,
                                  const unsigned int testpcn ) const {

  return ( linkInfo( beetle, port, testpcn ) == UTTELL1Error::FailureMode::kNone ? false : true );
}

bool LHCb::UTTELL1Error::addLinkInfo( const unsigned int key, const LHCb::UTTELL1Error::FailureMode& mode ) {
  return m_badLinks.insert( key, mode ).second;
}

double LHCb::UTTELL1Error::fractionOK( const unsigned int pcn ) const {
...
LHCb::UTTELL1Error::FailureMode LHCb::UTTELL1Error::linkInfo( const unsigned int beetle, const unsigned int port,
                                                              const unsigned int testpcn ) const {

  // if link is not in error we have nothhing to do
  if ( hasErrorInfo() == false ) return UTTELL1Error::FailureMode::kNone;

  const unsigned int                        key = linkID( beetle, port );
  LHCb::UTTELL1Error::FailureInfo::iterator i   = m_badLinks.find( key );
  if ( m_badLinks.end() != i ) { return i->second; }

  // we have to look at the pcn
  const unsigned int pcn = findPCN( beetle );
  return ( pcn != testpcn ? UTTELL1Error::FailureMode::kWrongPCN : UTTELL1Error::FailureMode::kNone );
}

unsigned int LHCb::UTTELL1Error::findPCN( const unsigned int beetle ) const {
...
void LHCb::UTTELL1Error::fillErrorInfo() {
...
bool LHCb::UTTELL1Error::correctPatterns() const {
  if ( !( ( m_word4 & 0xFF ) == 0 ) && ( ( ( m_word4 & 0xFF00 ) / 0x100 ) == 0x8e ) ) return false;

  if ( !( ( m_word4 & 0xFF ) == 0 ) && ( ( ( m_word4 & 0xFF00 ) / 0x100 ) == 0x8e ) ) return false;

  if ( !( ( m_word11 & 0xFF ) == 2 ) && ( ( ( m_word11 & 0xFF00 ) / 0x100 ) == 0x8e ) ) return false;

  if ( hasNZS() ) {
    if ( !( ( m_word12 & 0xFF ) == 3 ) && ( ( ( m_word12 & 0xFF00 ) / 0x100 ) == 0x8e ) ) return false;
  }

  if ( hasPed() ) {
    if ( !( ( m_word13 & 0xFF ) == 4 ) && ( ( ( m_word13 & 0xFF00 ) / 0x100 ) == 0x8e ) ) return false;
  }
...
unsigned int LHCb::UTTELL1Error::pcnVote() const {
