...
\*****************************************************************************/
// ============================================================================
#ifndef CALOFUTUREUTILS_SHAREDCELLS_H
#define CALOFUTUREUTILS_SHAREDCELLS_H 1
// ============================================================================
// Include files
// ============================================================================
// STD & STL
...
/** @namespace SharedCells SharedCells.h CaloFutureUtils/SharedCells.h
 *
 *  set of inline functions used for CaloFutureSharedCellAlg
 *
...
namespace SharedCells {
  ...
  inline StatusCode seedDistanceAlgorithm( L& List, const DE& Det, const T& type, const LHCb::CaloCellID& ID,
                                           const SIZE& size ) {
    ...
    for ( auto iC = List.begin(); List.end() != iC; ++iC ) {
      ...
      const double dist = ( position - Det->cellCenter( seed->cellID() ) ).R(); // mag->R
      /// set the weight
      const unsigned index = iC - List.begin();
      /// calculate weight
      weight[index] = e * exp( -1.0 * dist / size[ID.area()] );
    }
  ...
  inline StatusCode summedDistanceAlgorithm( L& List, const DE& Det, const LHCb::CaloCellID& ID, const SIZE& size,
                                             int NiT ) {
    /// does one need more iterations ?
    if ( 0 >= NiT ) { return StatusCode::SUCCESS; }
    /// valid detector information ??
    ...
    for ( auto iC = List.begin(); List.end() != iC; ++iC ) {
      ...
      /// calculate the distance between seed cell and give cell
      const double dist = sqrt( ( pos.x() - x ) * ( pos.x() - x ) + ( pos.y() - y ) * ( pos.y() - y ) );
      /// set the weight
      const unsigned index = iC - List.begin();
      /// calculate weight
      weight[index] = e * exp( -1.0 * dist / size[ID.area()] );
    ...
    if ( !sc.isSuccess() ) return sc;
    /// make iterations
    return SharedCells::summedDistanceAlgorithm( List, Det, ID, size, --NiT );
  }
  //  =========================================================================
