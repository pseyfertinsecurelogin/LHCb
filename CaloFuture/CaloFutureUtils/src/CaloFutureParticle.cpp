...
// ============================================================================
LHCb::CaloFutureParticle::CaloFutureParticle( LHCb::Particle* part )
...
void LHCb::CaloFutureParticle::CaloFutureParticleTree ( const LHCb::Particle* part )
{
  ...
  if( part->isBasicParticle() ){
    if( 0 == part->proto() )                { m_isCaloFuture = false; }
    else if( part->proto()->calo().empty() ){ m_isCaloFuture = false; }
  ...
  for( LHCb::Particle* part : m_parts ) {
    if( !part->isBasicParticle() && LHCb::CaloMomentum::OK == status() ){
      for ( const auto& daughter : part->daughters() ) {
        LHCb::CaloFutureParticle caloDau( const_cast<LHCb::Particle*>(daughter.target()),
                                    referencePoint () ,
                                    pointCovMatrix () ) ;
        caloDau.updateTree();
      }
...
// get the particle
// ============================================================================
LHCb::Particle* LHCb::CaloFutureParticle::particle() const
{
