 ...
 */
// ============================================================================
CovarianceEstimator::CovarianceEstimator( const DeCalorimeter* Detector ) : m_detector( Detector ) {}
...
StatusCode CovarianceEstimator::operator()( LHCb::CaloCluster* cluster ) const {
  ...
  for ( auto it = entries.begin(); entries.end() != it; ++it ) {
    ...
    if ( ( it->status() & LHCb::CaloDigitStatus::UseForEnergy ) ||
         ( it->status() & LHCb::CaloDigitStatus::UseForPosition ) )
  ...
  if ( 0. >= eTE ) parameters( LHCb::CaloPosition::Index::E ) = 0.;
  if ( 0. >= eTP ) {
    parameters( LHCb::CaloPosition::Index::X ) = 0.;
    parameters( LHCb::CaloPosition::Index::Y ) = 0.;
  }
  ...
  parameters( LHCb::CaloPosition::Index::E ) = Ecl; // E
  parameters( LHCb::CaloPosition::Index::X ) = Xcl; // X
  parameters( LHCb::CaloPosition::Index::Y ) = Ycl; // Y
...
MsgStream& CovarianceEstimator::printOut( MsgStream& log ) const {
  log << " Cluster Covariance Estimator: "
      ...
      << "   Coherent Noise is " << ( m_CoherentNoise ) << "]" << endmsg;
  ///
  return log;
}
// ============================================================================
