...
// Main execution
//=============================================================================
std::tuple<LHCb::CaloAdcs,
	   LHCb::CaloDigits,
	   LHCb::RawBankReadoutStatus> CaloFutureRawToDigits::operator()(const LHCb::RawEvent& rawEvt) const {
  ...
  if(m_packed){  // TELL1 format : 1 source per TELL1
    for(const auto& t : m_calo->tell1Params() ) {
      ...
      status.addStatus( t.number()  , ok ? LHCb::RawBankReadoutStatus::Status::OK : LHCb::RawBankReadoutStatus::Status::Missing);
    }
  } else { // Offline format : single source 0
    status.addStatus( 0 , sources.empty() ? LHCb::RawBankReadoutStatus::Status::Missing : LHCb::RawBankReadoutStatus::Status::OK);
  }
  ...
  if ( banks.empty() ) {
    ...
    for( const auto& bank : banks ) {
      ...
      if ( read ) {
	Warning("Another bank with same sourceID " + Gaudi::Utils::toString( sourceID ) + " has already been read").ignore();
...
//=============================================================================
std::vector<LHCb::CaloAdc> CaloFutureRawToDigits::decode ( const LHCb::RawBank& bank, LHCb::RawBankReadoutStatus status, bool getPinData ) const {
  ...
  if ( 1 > version || 3 < version ) {
    ...
    while( 0 != size ) {
	  ...
	  if(m_calo->isPinCard(prevCard)){
        ...
        status.addStatus( sourceID,   LHCb::RawBankReadoutStatus::Status::Corrupted | LHCb::RawBankReadoutStatus::Status::Incomplete);
      }
