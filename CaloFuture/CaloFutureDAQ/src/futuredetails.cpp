...
namespace {
  const std::regex s_outTypeParse{"([\"']?)(?:(?:CALOFUTURE)?(ADC|DIGIT)S?|(BOTH))\\1", std::regex_constants::icase};
}
...
namespace futuredetails {
  ...
  const char* toString( const source_t& source ) {
    switch ( source ) {
    case source_t::from_adc:
      return "ADC";
    case source_t::from_digit:
      return "DIGIT";
    default:
