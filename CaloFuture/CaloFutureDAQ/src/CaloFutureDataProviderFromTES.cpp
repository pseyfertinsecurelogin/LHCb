...
// local
#include "CaloFutureDataProviderFromTES.h"

//-----------------------------------------------------------------------------
// Implementation file for class : CaloFutureDataProviderFromTES
//
// 2008-08-22 : Olivier Deschamps
//-----------------------------------------------------------------------------

// Declaration of the Tool Factory
DECLARE_COMPONENT( CaloFutureDataProviderFromTES )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
CaloFutureDataProviderFromTES::CaloFutureDataProviderFromTES( const std::string& type,
  ...
  , m_detectorName( futuredetails::alg_name_to_detector( name ) )
{
  ...
  case futuredetails::DetectorName_t::Spd :
    ...
    break;
  case futuredetails::DetectorName_t::Unknown :
...
//=============================================================================
StatusCode CaloFutureDataProviderFromTES::finalize ( ) {
  IIncidentSvc* inc = incSvc() ;
  if ( inc ) { inc->removeListener( this ); }
  return base_class::finalize();
}

StatusCode CaloFutureDataProviderFromTES::initialize ( ) {
  ...
  case futuredetails::DetectorName_t::Hcal :
    ...
    break;
  case futuredetails::DetectorName_t::Prs :
  case futuredetails::DetectorName_t::Spd :  // Prs FE for SPD
  ...
  if( m_loc.value().empty() ){
    switch (m_data.value()) {
      case futuredetails::source_t::from_adc   : m_loc = m_adcLoc; break;
      case futuredetails::source_t::from_digit : m_loc = m_digLoc; break;
    }
  }
...
//-------------------------------------
bool CaloFutureDataProviderFromTES::getBanks( ) {
  clear();
  m_getRaw = false;
...
//-------------------------------------
void CaloFutureDataProviderFromTES::clear( ) {
  m_adcs.clear();
  m_digits.clear();
  m_readSources.clear();
...
void CaloFutureDataProviderFromTES::cleanData(int feb ) {
  if(feb<0)return;
...
//---------
double CaloFutureDataProviderFromTES::digit (LHCb::CaloCellID id, double def){
...
//---------
int CaloFutureDataProviderFromTES::adc (LHCb::CaloCellID id,int def){
  double pedShift = m_calo->pedestalShift();
  if(m_getRaw)getBanks();
  if( fromDigit() ){
    if( !m_digCont )return def ;
...
const CaloVector<LHCb::CaloAdc>& CaloFutureDataProviderFromTES::adcs(int source,bool clean){
  return adcs(std::vector<int>(1,source),clean);
}
const CaloVector<LHCb::CaloDigit>& CaloFutureDataProviderFromTES::digits(int source,bool clean){
  return digits(std::vector<int>(1,source),clean);
}


const CaloVector<LHCb::CaloAdc>& CaloFutureDataProviderFromTES::adcs(std::vector<int> sources,bool clean){
  if(m_getRaw)getBanks();
  if(clean)clear();
...
//---------
const CaloVector<LHCb::CaloDigit>& CaloFutureDataProviderFromTES::digits(std::vector<int> sources,bool clean){
  if(m_getRaw)getBanks();
  if(clean)clear();
  for(const auto& source : sources) {
    if(checkSrc( source ))continue;
...
bool CaloFutureDataProviderFromTES::checkSrc(int source){
