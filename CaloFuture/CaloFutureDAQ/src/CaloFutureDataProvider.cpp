...
// local
#include "CaloFutureDataProvider.h"

//-----------------------------------------------------------------------------
// Implementation file for class : CaloFutureDataProvider
//
// 2005-01-10 : Olivier Deschamps
//-----------------------------------------------------------------------------

DECLARE_COMPONENT( CaloFutureDataProvider )
...
//=========================================================================
StatusCode CaloFutureDataProvider::initialize ( ) {
...
//-------------------------------------
void CaloFutureDataProvider::clear( ) {
  m_adcs.clear();
  m_digits.clear();
  m_tell1s = 0;
  m_readSources.clear();
...
//-------------------------------------
void CaloFutureDataProvider::cleanData(int feb ) {
  if(feb<0)return;
...
//  Get ADC Container
//===================
const CaloVector<LHCb::CaloAdc>& CaloFutureDataProvider::adcs(int source,bool clean){
  if(clean)clear();
  (!m_packed)? decodeTell1(-1) : decodeTell1(source);
  return m_adcs;
}
const CaloVector<LHCb::CaloAdc>& CaloFutureDataProvider::adcs(std::vector<int> sources,bool clean){
  if(clean)clear();
...
void CaloFutureDataProvider::adc2digit(){
...
const CaloVector<LHCb::CaloDigit>& CaloFutureDataProvider::digits(std::vector<int> sources,bool clean){
  if( m_getRaw ) getBanks();
...
const CaloVector<LHCb::CaloDigit>& CaloFutureDataProvider::digits(int source,bool clean){
  adcs(source,clean);
  adc2digit();
  return m_digits;
}
...
//  Get data
//==========
double CaloFutureDataProvider::digit (LHCb::CaloCellID id,double def){
  double pedShift = m_calo->pedestalShift();
  if ( m_getRaw ) getBanks();
...
//-------------------------------------------------------
int CaloFutureDataProvider::adc (LHCb::CaloCellID id, int def){
  if( m_getRaw ) getBanks();
...
// Protected methods :
//-------------------------------------------------------
bool CaloFutureDataProvider::decodeCell(LHCb::CaloCellID id ){
...
//-------------------------------------------------------
bool CaloFutureDataProvider::decodeTell1 (int source) {
  ...
  for( const auto* itB : m_banks) {
    ...
    decoded = ( futuredetails::DetectorName_t::Spd == m_detectorName ? decodePrsTriggerBank( *itB )
                                       : decodeBank ( *itB ) );
    if( !decoded ){
...
//==================================
bool CaloFutureDataProvider::decodeBank( const LHCb::RawBank& bank ){
...
//==================================
bool CaloFutureDataProvider::decodePrsTriggerBank( const LHCb::RawBank& bank ) {
