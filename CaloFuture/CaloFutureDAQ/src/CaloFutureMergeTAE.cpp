...
//=============================================================================
CaloFutureMergeTAE::CaloFutureMergeTAE( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode CaloFutureMergeTAE::initialize() {
  ...
  //
  std::tie( m_outDigit, m_outAdc ) =
      ( m_outputDataLoc.value().empty() ? std::make_pair( "TAE/" + m_locDigit, "TAE/" + m_locAdc )
  ...
  auto loc = [&]() -> decltype( auto ) {
    switch ( m_data.value() ) {
    case futuredetails::source_t::from_adc:
      return m_outAdc;
    case futuredetails::source_t::from_digit:
      return m_outDigit;
    default:
      throw "IMPOSSIBLE";
    }
