...
public:
  ...
  DeRichPD::SIMDFP::MaskType detectionPoint( const SmartIDs &smartID,
...
private:
                                                 ...
                                                 const SIMDUINT &fracPixelRow ) const noexcept
  {
    ...
    const auto xh = ( simd_cast< SIMDFP >( fracPixelCol ) - SIMDFP( m_PmtNumPixColFrac ) ) *
                    SIMDFP( m_PmtEffectivePixelXSize );
    const auto yh = ( simd_cast< SIMDFP >( fracPixelRow ) - SIMDFP( m_PmtNumPixRowFrac ) ) *
                    SIMDFP( m_PmtEffectivePixelYSize );
  ...
  inline SIMDPoint getAnodeHitCoordFromGrandPixelNum( const SIMDUINT &fracPixelCol,
                                                      const SIMDUINT &fracPixelRow ) const noexcept
  {
    const auto xmask = simd_cast< SIMDFP::MaskType >(
      fracPixelCol == SIMDUINT::Zero() || fracPixelCol == SIMDUINT( m_PmtNumPixCol - 1 ) );
    const auto ymask = simd_cast< SIMDFP::MaskType >(
      fracPixelRow == SIMDUINT::Zero() || fracPixelRow == SIMDUINT( m_PmtNumPixRow - 1 ) );
    ...
    const auto xh =
      ( simd_cast< SIMDFP >( fracPixelCol ) - SIMDFP( m_PmtNumPixColFrac ) ) * aXEffPixel;
    const auto yh =
      ( simd_cast< SIMDFP >( fracPixelRow ) - SIMDFP( m_PmtNumPixRowFrac ) ) * aYEffPixel;
  ...
  inline SIMDPoint getAnodeHitCoordFromMultTypePixelNum( const SIMDUINT &fracPixelCol,
                                                         const SIMDUINT &fracPixelRow ) const
    noexcept
  {
    return ( PmtIsGrand() ? getAnodeHitCoordFromGrandPixelNum( fracPixelCol, fracPixelRow ) :
                            getAnodeHitCoordFromPixelNum( fracPixelCol, fracPixelRow ) );
