...
public:
   ...
   */
  BeamPipeIntersectionType intersectionPoints( const Gaudi::XYZPoint& start, const Gaudi::XYZPoint& end,
                                               Gaudi::XYZPoint& entryPoint, Gaudi::XYZPoint& exitPoint ) const;
  /**
   ...
   */
  template <typename POINT, typename std::enable_if<std::is_arithmetic<typename POINT::Scalar>::value>::type* = nullptr>
  ...
  inline decltype( auto ) testForIntersection( const POINT& start, const POINT& end,
    ...
    if ( UNLIKELY( any_of( mask ) ) ) {
      ...
      for ( std::size_t i = 0; i < POINT::Scalar::Size; ++i ) {
        if ( mask[i] ) {
          mask[i] = m_localCone->testForIntersection( Gaudi::XYZPoint{pL.X()[i], pL.Y()[i], pL.Z()[i]},
...
private:
  ...
  inline decltype( auto ) isCloseBy( const POINT& p ) const {
    if constexpr ( std::is_arithmetic<typename POINT::Scalar>::value ) {
      ...
      const auto beamz = ( p.z() > m_endPGlo.z() ? m_endPGlo.z() : p.z() < m_startPGlo.z() ? m_startPGlo.z() : p.z() );
...
inline std::ostream& operator<<( std::ostream& s, const DeRichBeamPipe::BeamPipeIntersectionType& type ) {
