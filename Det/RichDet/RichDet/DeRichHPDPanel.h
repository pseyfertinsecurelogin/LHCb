...
// Check HPD panel acceptance
//=========================================================================
inline LHCb::RichTraceMode::RayTraceResult DeRichHPDPanel::checkPanelAcc( const Gaudi::XYZPoint& point ) const {
  const auto uv =
      ( Rich::Rich1 == rich() ? std::make_pair( point.y(), point.x() ) : std::make_pair( point.x(), point.y() ) );
  return ( ( fabs( uv.first ) >= fabs( m_panelColumnSideEdge ) || fabs( uv.second ) >= m_panelStartColPos )
