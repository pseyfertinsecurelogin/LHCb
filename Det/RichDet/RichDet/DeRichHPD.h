...
public:
   ...
   */
  Gaudi::XYZPoint windowCentreIn() const { return geometry()->toGlobal( m_pvWindow->toMother( {0, 0, m_winInR} ) ); }
  ...
  Gaudi::XYZPoint windowCentreInIdeal() const {
    return ( geometry()->toGlobalMatrixNominal() * m_pvWindow->toMother( {0, 0, m_winInR} ) );
  }
   ...
   */
  inline const Gaudi::XYZPoint hpdWinInsideCentreInMother() const noexcept { return m_windowInsideCentreMother; }

  /**
   * Get the point on the centre of the HPD window on the outside surface
   * @return Inside window surface centre point
   */
  Gaudi::XYZPoint windowCentreOut() const { return geometry()->toGlobal( m_pvWindow->toMother( {0, 0, m_winOutR} ) ); }
   ...
   */
  inline const Gaudi::Transform3D& fromPanelToHPDWindow() const noexcept { return m_fromPanelToWindow; }

  /** Get the tranformation from the HPD quartz window to the HPD
   *  @return The HPD window to HPD tranformation
   */
  inline const Gaudi::Transform3D& fromHPDWindowToHPD() const noexcept { return m_fromWindowToHPD; }
   ...
   */
  inline const Gaudi::Transform3D& fromHPDWindowToGlobal() const noexcept { return m_fromWindowToGlobal; }
   ...
   */
  inline const Gaudi::Transform3D& fromHPDToPanel() const noexcept { return m_fromHPDToPanel; }

  /** Get the solid for the HPD window (for intersection purposes)
   *  @return The quartz window solid
   */
  inline const ISolid* HPDWindowSolid() const noexcept { return m_windowSolid; }
   ...
   */
  bool testKaptonShadowing( const Gaudi::XYZPoint& pInPanel, const Gaudi::XYZVector& vInPanel ) const;
   ...
   */
  bool intersectEntryWindow( const Gaudi::XYZPoint& position, const Gaudi::XYZVector& direction,
...
public:
   ...
   */
  inline const Rich::TabulatedFunction1D* demagnification_RtoR( const FieldPolarity field = FieldPolarity::Down ) const
      noexcept {
   ...
   */
  inline const Rich::TabulatedFunction1D* magnification_RtoR( const FieldPolarity field = FieldPolarity::Down ) const
      noexcept {
   ...
   */
  inline const Rich::TabulatedFunction1D* magnification_RtoPhi( const FieldPolarity field = FieldPolarity::Down ) const
      noexcept {
...
private: // functions
  ...
  bool magnifyToGlobalMagnetON( Gaudi::XYZPoint& detectPoint, const bool photoCathodeSide ) const;

  /// go from a point on silicon to a point on the photo-cathode with magnet OFF
  bool magnifyToGlobalMagnetOFF( Gaudi::XYZPoint& detectPoint, const bool photoCathodeSide ) const;
