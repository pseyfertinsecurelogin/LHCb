...
protected:
  ...
  /// Report an error
  inline StatusCode Error( const std::string& mess, const StatusCode sc = StatusCode::FAILURE ) const {
    error() << mess << endmsg;
    return sc;
  }

  /// Report a warning
  inline StatusCode Warning( const std::string& mess, const StatusCode sc = StatusCode::FAILURE ) const {
    warning() << mess << endmsg;
    return sc;
  }
