...
private:
  ...
  inline Rich::DAQ::PDPanelIndex _pdNumber( const LHCb::RichSmartID smartID ) const noexcept
  {
    ...
    return Rich::DAQ::PDPanelIndex( ( smartID.pdCol() * m_NumPmtInRichModule ) +
                                    smartID.pdNumInCol() );
  }
...
private:
  ...
  void ( DeRichPMTPanel::*m_getModuleNumsSIMD )( const SIMDFP &     x,
                                                 const SIMDFP &     y,
...
private:
  ...
  inline decltype( auto ) isInPmtAnodeLateralAcc( const SIMDFP           X,
                                                  const SIMDFP           Y,
                                                  const SIMDFP::MaskType aFlagGrandPMT ) const
    noexcept
  {
  ...
  {
    const auto aPmtH =
      ( ( aFlagGrandPMT && rich() == Rich::Rich2 ) ? ( m_GrandPmtMasterLateralSize * 0.5 ) :
                                                     ( m_PmtMasterLateralSize * 0.5 ) );
    return ( fabs( aPointInPmt.x() ) < aPmtH && fabs( aPointInPmt.y() ) < aPmtH );
  }

  inline decltype( auto )
  isInPmt( const SIMDFP X, const SIMDFP Y, const SIMDFP::MaskType aFlagGrandPMT ) const noexcept
  {
  ...
  inline decltype( auto )
  checkPDAcceptance( SIMDFP X, SIMDFP Y, const SIMDFP::MaskType aFlagGrandPMT ) const noexcept
  {
    X = abs( X );
    Y = abs( Y );
