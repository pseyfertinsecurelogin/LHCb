...
std::shared_ptr< const Rich::TabulatedProperty1D >
DeRich::loadNominalHPDQuantumEff()
{
  ...
  if ( !tabQE )
  {
    throw GaudiException(
      "No information on HPD Quantum Efficiency", "DeRich", StatusCode::FAILURE );
  }
  else
  {
...
std::shared_ptr< const Rich::TabulatedProperty1D >
DeRich::loadNominalPMTQuantumEff()
{
  ...
  {
    throw GaudiException(
      "No information on PMT Quantum Efficiency", "DeRich", StatusCode::FAILURE );
  }
  else
  {
...
Rich::MirrorSegPosition
DeRich::sphMirrorSegPos( const int mirrorNumber ) const
{
...
Rich::MirrorSegPosition
DeRich::secMirrorSegPos( const int mirrorNumber ) const
{
...
//=========================================================================
int
DeRich::sensitiveVolumeID( const Gaudi::XYZPoint &globalPoint ) const
{
...
//=========================================================================
const std::string
DeRich::panelName( const Rich::Side /* panel */ ) const
{
  throw GaudiException(
    "Not implemented for DeRich class" + myName(), "DeRich::panelName", StatusCode::FAILURE );
}
                  ...
                  const LHCb::RichTraceMode       mode ) const
{
  // are we configured to test individual PD acceptance or just interset the plane ?
  return (
    mode.detPlaneBound() == LHCb::RichTraceMode::DetectorPlaneBoundary::RespectPDTubes ?
      // Full PD acceptance
                  ...
                  const LHCb::RichTraceMode       mode ) const
{
  ...
  if ( all_of( m1 ) )
  { return rayTrace( Rich::firstSide, pGlobal, vGlobal, hitPosition, smartID, PDs, mode ); }

  // side 2 mask
  const auto m2 = ( sides == Rich::SIMD::Sides( Rich::secondSide ) );
  if ( all_of( m2 ) )
  { return rayTrace( Rich::secondSide, pGlobal, vGlobal, hitPosition, smartID, PDs, mode ); }
