...
const CLID& DeRichHPDPanel::classID() { return CLID_DeRichHPDPanel; }
  ...
  for ( const auto* detelem : childIDetectorElements() ) {
    if ( std::string::npos != detelem->name().find( "HPD:" ) ) {
      ...
      if ( !deHPD->childIDetectorElements().empty() ) {
        ...
        // register UMS dependency
        updMgrSvc()->registerCondition( this, deHPD->childIDetectorElements().front()->geometry(),
...
bool DeRichHPDPanel::smartID( const Gaudi::XYZPoint& globalPoint, LHCb::RichSmartID& id ) const {
  ...
  const auto subPixel =
      (unsigned int)( ( m_siliconHalfLengthY - inSiliconY - pixelRow * m_pixelSize ) / m_subPixelSize );
...
DeRichHPDPanel::PDWindowPointSIMD( const SIMDPoint& pGlobal, const SIMDVector& vGlobal, SIMDPoint& hitPosition,
...
//=========================================================================
DeRichHPDPanel::SIMDRayTResult::Results
DeRichHPDPanel::detPlanePointSIMD( const SIMDPoint& pGlobal, const SIMDVector& vGlobal, SIMDPoint& hitPosition,
                                   SIMDRayTResult::SmartIDs& smartID, SIMDRayTResult::PDs& PDs,
                                   const LHCb::RichTraceMode mode ) const {
  SIMDRayTResult::Results res;
  // Hit position
...
//=========================================================================
LHCb::RichTraceMode::RayTraceResult DeRichHPDPanel::PDWindowPoint( const Gaudi::XYZPoint&  pGlobal,
                                                                   const Gaudi::XYZVector& vGlobal,
                                                                   Gaudi::XYZPoint&        windowPointGlobal,
                                                                   LHCb::RichSmartID& smartID, const DeRichPD*& pd,
                                                                   const LHCb::RichTraceMode mode ) const {
  ...
  if ( mode.detPrecision() != LHCb::RichTraceMode::DetectorPrecision::FullPDs ) {
    ...
    if ( UNLIKELY( mode.hpdKaptonShadowing() && HPD->testKaptonShadowing( pInPanel, vInPanel ) ) ) {
      ...
      if ( UNLIKELY( 0 == numTicks ) ) {
        ...
        const auto hitRadiusSq = ( std::pow( windowPointInHPD.x(), 2 ) + std::pow( windowPointInHPD.y(), 2 ) );
...
//=========================================================================
bool DeRichHPDPanel::readoutChannelList( LHCb::RichSmartID::Vector& readoutChannels ) const {
    ...
    for ( unsigned int pixRow = 0; pixRow < m_pixelRows; ++pixRow ) {
      for ( unsigned int pixCol = 0; pixCol < m_pixelColumns; ++pixCol ) {
        ...
        if ( radcornSq <= activeRadiusSq ) {
          ...
          readoutChannels.emplace_back( LHCb::RichSmartID( rich(), side(), pdPosInCol, pdCol, pixRow, pixCol ) );
        }
...
//=========================================================================
LHCb::RichTraceMode::RayTraceResult DeRichHPDPanel::detPlanePoint( const Gaudi::XYZPoint&  pGlobal,
                                                                   const Gaudi::XYZVector& vGlobal,
...
// findHPDColAndPos
//=========================================================================
bool DeRichHPDPanel::findHPDColAndPos( const Gaudi::XYZPoint& inPanel, LHCb::RichSmartID& id ) const {
  bool OK = true;
  ...
  auto HPDNumInCol = ( 0 == HPDCol % 2 ? (long int)( ( v - m_panelStartColPosEvenOdd[0] ) * m_OneOverHPDPitch )
...
StatusCode DeRichHPDPanel::geometryUpdate() {
  ...
  if ( msgLevel( MSG::DEBUG, msg ) )
    msg << MSG::DEBUG << "HPDColumns:" << nPDColumns() << " HPDNumberInColumns:" << nPDsPerCol() << endmsg;

  if ( m_HPDColPitch < ( activeRadius * 2 ) ) {
    msg << MSG::WARNING
        << "The active area is bigger by:" << ( activeRadius * 2 - fabs( m_HPDColPitch ) ) / Gaudi::Units::mm
  ...
  if ( msgLevel( MSG::DEBUG, msg ) )
    msg << MSG::DEBUG << "HPDPitch:" << m_HPDPitch << " panelColumnSideEdge:" << m_panelColumnSideEdge << endmsg;
  ...
  m_panelStartColPos = std::max( fabs( m_panelStartColPosEvenOdd[0] ), fabs( m_panelStartColPosEvenOdd[1] ) );
  ...
  if ( msgLevel( MSG::VERBOSE, msg ) ) msg << MSG::VERBOSE << "About to do a dynamic cast SolidBox" << endmsg;
  ...
  auto          windowTicksSize = windowSolid0->intersectionTicks( Gaudi::XYZPoint( 0.0, 0.0, 0.0 ),
  ...
  if ( msgLevel( MSG::DEBUG, msg ) ) msg << MSG::DEBUG << "Centre of HPDPanel : " << centreGlobal << endmsg;
  ...
  if ( msgLevel( MSG::VERBOSE, msg ) ) {
    ...
    msg << MSG::VERBOSE << "Ideal local centre of HPD#" << 2 * nPDsPerCol() - 1 << " "
        << geometry()->toLocal( deHPD( Rich::DAQ::PDPanelIndex( 2 * nPDsPerCol() - 1 ) )->windowCentreInIdeal() )
        << endmsg;
  }
  ...
  if ( msgLevel( MSG::DEBUG, msg ) ) msg << MSG::DEBUG << "Found " << m_DeHPDs.size() << " DeRichHPDs" << endmsg;
  ...
  if ( Rich::Rich1 == rich() ) {
    ...
    localTranslation = ROOT::Math::Translation3D( centreGlobal.x(), sign * localOffset(), -detectPlaneZcoord() );
  } else {
    const int sign   = ( side() == Rich::left ? 1 : -1 );
    localTranslation = ROOT::Math::Translation3D( sign * localOffset(), centreGlobal.y(), -detectPlaneZcoord() );
  }
