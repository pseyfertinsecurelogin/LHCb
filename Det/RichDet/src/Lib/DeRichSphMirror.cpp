...
DeRichSphMirror::DeRichSphMirror( const std::string &name ) : DeRichBase( name )
{
...
const CLID &
DeRichSphMirror::classID()
{
  return CLID_DeRichSphMirror;
}

StatusCode
DeRichSphMirror::initialize()
{
...
//=========================================================================
StatusCode
DeRichSphMirror::intersects( const Gaudi::XYZPoint & globalP,
...
//=========================================================================
StatusCode
DeRichSphMirror::intersects( const Gaudi::XYZPoint &globalP, const Gaudi::XYZVector &globalV ) const
{
  const auto vLocal = geometry()->toLocalMatrix() * globalV;
...
DeRichSphMirror::updateGeometry()
{
  ...
  if ( m_firstUpdate )
    updMgrSvc()->registerCondition( this, geometry(), &DeRichSphMirror::updateGeometry );
  ...
  if ( type == "SolidSphere" ) { sphereSolid = dynamic_cast< const SolidSphere * >( m_solid ); }
  else
  {
  ...
  const auto    sphTicksSize =
    sphereSolid->intersectionTicks( m_localOrigin, toSphCentreXYZ, sphTicks );
  if ( sphTicksSize != 2 )
  {
  ...
  {
    debug() << "Mirror #" << m_mirrorNumber << " " << rich << " Radius:" << m_radius
            << " Centre of curvature " << m_centreOfCurvature << endmsg;
  ...
  const Gaudi::XYZVector localCentreNormal(
    Gaudi::XYZVector( -m_localMirrorCentre.x(), -m_localMirrorCentre.y(), -m_localMirrorCentre.z() )
      .unit() );
  ...
  else
  {
    ...
    if ( !s )
    {
      fatal() << "Could not find surface for mirror " << myName() << " at location " << surfLocation
              << endmsg;
      return StatusCode::FAILURE;
    }
    surf = s;
