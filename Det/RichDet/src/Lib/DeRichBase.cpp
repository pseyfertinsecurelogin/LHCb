...
DeRichSystem* DeRichBase::deRichSys() {
  if ( UNLIKELY( !m_deRichS ) ) {
    ...
    if ( afterMag->exists( "RichDetectorLocations" ) ) {
      ...
      if ( !deRich ) {
        throw GaudiException( "Could not load first RICH at " + firstRichLoc, "DeRichBase::deRichSys()",
                              StatusCode::FAILURE );
      }
      ...
      if ( deRich->exists( "RichSystemDetElemLocation" ) ) {
        ...
        throw GaudiException( "First RICH does not have RichSystem location", "DeRichBase::deRichSys()",
                              StatusCode::FAILURE );
      }
    } else {
    ...
    if ( !deRichS ) {
      throw GaudiException( "Could not load DeRichSystem at " + deRichSysLoc, "DeRichBase::deRichSys()",
                            StatusCode::FAILURE );
    }
