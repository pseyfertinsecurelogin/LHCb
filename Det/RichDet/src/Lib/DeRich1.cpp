...
const CLID& DeRich1::classID() { return CLID_DERich1; }
...
StatusCode DeRich1::initialize() {
  ...
  if ( pvRich1SubMaster ) {
    pvGasWindow =
        pvRich1SubMaster->lvolume()->pvolume( "pvRich1MagShH0:0" )->lvolume()->pvolume( "pvRich1GQuartzWH0:0" );
  }
  if ( pvGasWindow ) {
...
StatusCode DeRich1::updateMirrorParams() {
  ...
  // Load the secondary mirrors to compute the 'nominal' settings
  {
    ...
    m_nominalPlanes[Rich::top] =
        Gaudi::Plane3D( nominalFMirrorPlaneTop[0] / (double)nTop, nominalFMirrorPlaneTop[1] / (double)nTop,
                        nominalFMirrorPlaneTop[2] / (double)nTop, nominalFMirrorPlaneTop[3] / (double)nTop );
    m_nominalPlanes[Rich::bottom] =
        Gaudi::Plane3D( nominalFMirrorPlaneBot[0] / (double)nBot, nominalFMirrorPlaneBot[1] / (double)nBot,
                        nominalFMirrorPlaneBot[2] / (double)nBot, nominalFMirrorPlaneBot[3] / (double)nBot );
  }
...
const std::string DeRich1::panelName( const Rich::Side panel ) const {
  ...
  std::string pname = ( Rich::top == panel ? DeRichLocations::Rich1Panel0 : DeRichLocations::Rich1Panel1 );
