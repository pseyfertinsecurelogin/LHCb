  ...
  if ( pvGasWindow )
  {
    for ( const auto& mat : pvGasWindow->lvolume()->material()->tabulatedProperties() )
    {
      if ( mat )
      {
        if      ( mat->type() == "RINDEX" )
        {
          _ri_debug << "Loaded gas window refIndex from: " << mat->name()
                    << endmsg;
  ...
  // Load the primary mirror segments to compute the 'nominal' settings
  {
    ...
    for ( const auto& loc : paramVect<std::string>("SphericalMirrorDetElemLocations") )
    {
      ...
      _ri_debug << loc
                ...
                << " RoC = " << roc
                << " CoC = " << coc << endmsg;
      if ( cen.y() > 0 ) { ++nTop; m_nominalCentresOfCurvature[Rich::top]    += Gaudi::XYZVector(coc); }
      else               { ++nBot; m_nominalCentresOfCurvature[Rich::bottom] += Gaudi::XYZVector(coc); }
      avroc += roc;
    }
