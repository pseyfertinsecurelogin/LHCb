...
const CLID& DeRichBeamPipe::classID() { return CLID_DERichBeamPipe; }
...
DeRichBeamPipe::BeamPipeIntersectionType DeRichBeamPipe::intersectionPoints( const Gaudi::XYZPoint& start,
  ...
  if ( inter ) {
    ...
    if ( 0 != noTicks ) {
      ...
      const bool frontFaceHit = ( entryLocal.z() < -( m_localCone->zHalfLength() ) + 1 * Gaudi::Units::mm );
      entryPoint              = geometry()->toGlobal( entryLocal );

      // set exit point
      const auto exitLocal   = pLocal + ticks[noTicks - 1] * vLocal;
      const bool backFaceHit = ( exitLocal.z() > m_localCone->zHalfLength() - 1 * Gaudi::Units::mm );
...
//  text conversion
//=========================================================================
std::string DeRichBeamPipe::text( const DeRichBeamPipe::BeamPipeIntersectionType& type ) {
  switch ( type ) {
  case DeRichBeamPipe::NoIntersection:
    return "NoIntersection";
  case DeRichBeamPipe::FrontAndBackFace:
    return "FrontAndBackFace";
  case DeRichBeamPipe::FrontFaceAndCone:
    return "FrontFaceAndCone";
  case DeRichBeamPipe::BackFaceAndCone:
    return "BackFaceAndCone";
  case DeRichBeamPipe::ConeOnly:
    return "ConeOnly";
  default:
