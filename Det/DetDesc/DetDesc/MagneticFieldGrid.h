...
private:
    ...
    std::array<vectype, 3> fetchVectorQ_helper (
      ...
      std::array<Vec4f, 3> Qs = {m_Q_V[indices[Is]]...};
    ...
    std::array<vectype, 3> fetchVectorQ (const vectype& indices) {
      return fetchVectorQ(indices, std::make_index_sequence<vectype::size()>{});
    }
...
public:
    ...
    std::array<vectype, 3> horizontallyVectorizedFieldVector (
      ...
      std::array<vectype, 3> Dxyz {{m_Dxyz_V[0]}, {m_Dxyz_V[1]}, {m_Dxyz_V[2]}};
      std::array<vectype, 3> inverted_Dxyz {{1. / m_Dxyz_V[0]}, {1. / m_Dxyz_V[1]}, {1. / m_Dxyz_V[2]}};
      std::array<vectype, 3> Nxyz {{m_Nxyz_V[0]}, {m_Nxyz_V[1]}, {m_Nxyz_V[2]}};
      ...
      vectype x = rx - vectype{m_min_FL_V[0]};
      vectype y = ry - vectype{m_min_FL_V[1]};
      vectype z = rz - vectype{m_min_FL_V[2]};

      vectype i = floor(x * inverted_Dxyz[0]);
      vectype j = floor(y * inverted_Dxyz[1]);
      vectype k = floor(z * inverted_Dxyz[2]);
      ...
      vectype hx1 = (x - i * Dxyz[0]) * inverted_Dxyz[0];
      vectype hx0 = vectype{1.} - hx1;
      
      vectype hy1 = (y - i * Dxyz[1]) * inverted_Dxyz[1];
      vectype hy0 = vectype{1.} - hy1;

      vectype hz1 = (z - i * Dxyz[2]) * inverted_Dxyz[2];
      vectype hz0 = vectype{1.} - hz1;

      // auto h000 = hx0*hy0*hz0; auto h100 = hx1*hy0*hz0;
      // auto ijk000 = ( m_Nxyz[0]*( m_Nxyz[1]*k     + j )     + i );
      // bf  = h000*m_Q[ijk000]+h100*m_Q[ijk000+1] ;
      vectype h000 = hx0 * hy0 * hz0;
      vectype h100 = hx1 * hy0 * hz0;
      vectype ijk000 = (Nxyz[0] * (Nxyz[1]*k + j) + i);
      ...
      vectype h001 = hx0 * hy0 * hz1;
      vectype h101 = hx1 * hy0 * hz1;
      vectype ijk001 = (Nxyz[0] * (Nxyz[1]*(k+1) + j) + i);
      ...
      vectype h010 = hx0 * hy1 * hz0;
      vectype h110 = hx1 * hy1 * hz0;
      vectype ijk010 = (Nxyz[0] * (Nxyz[1]*k + j + 1) + i);
      ...
      vectype h011 = hx0 * hy1 * hz1;
      vectype h111 = hx1 * hy1 * hz1;
      vectype ijk011 = (Nxyz[0] * (Nxyz[1]*(k+1) + j + 1) + i);
