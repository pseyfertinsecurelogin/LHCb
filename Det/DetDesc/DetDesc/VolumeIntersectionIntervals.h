...
namespace VolumeIntersectionIntervals {
  ...
  inline int intersect( const ILVolume::Interval& i1, const ILVolume::Interval& i2 ) {
  ...
  // ==========================================================================
  /// check the intersection
  inline int intersect( const ILVolume::Intersection& i1, const ILVolume::Intersection& i2 ) {
  ...
  inline unsigned int TicksToIntervals( const ISolid::Ticks& ticks, // input "ticks", assumed to be sorted!
                                        OUTPUTTYPE           out ) {
  ...
  inline StatusCode MergeOwnAndChildContainers( const ILVolume::Intersections& own,
                                                const ILVolume::Intersections& child, OUTPUTTYPE out,
                                                const ILVolume* /* volum */, const double /* length */ ) {
    ...
    for ( Iter iterTop = own.begin(); own.end() != iterTop; ++iterTop ) {
      ...
      for ( Iter iter = child.begin(); child.end() != iter; ++iter ) {
        ...
        if ( intervalLoc.second <= ( intervalTop.first + tol ) ) {
          ...
          log << MSG::ERROR << "VolumeIntersection:Merge error 15 : interval " << intervalLoc.first << " "
              << intervalLoc.second << " master " << intervalTop.first << " " << intervalTop.second << endmsg;
      ...
      for ( IndexCont::const_iterator it = tmpIndex.begin(); tmpIndex.end() != it; ++it ) {
        ...
        ///
        if ( leftTick <= ( intervalLocal.first + tol ) && intervalLocal.first < mostRightTick ) {
        ...
        } else if ( ( matLocal == prevMatLocal ) && ( intervalLocal.second <= ( mostRightTick + tol ) ) ) {
          ...
          DetDesc::ServicesPtr services = DetDesc::services();
          MsgStream            log( services->msgSvc(), "TransportSvc" );
          log << MSG::ERROR << "VolumeIntersection:Merge error 17 : interval " << intervalLocal.first << " "
              << intervalLocal.second << " master " << leftTick << " " << mostRightTick << endmsg;
   ...
   */
  template <class OUTPUTTYPE>
  inline StatusCode MergeOwnAndChildContainers2( const ILVolume::Intersections& own,
                                                 const ILVolume::Intersections& child, OUTPUTTYPE out,
                                                 const ILVolume* volume, const double length ) {
    /*  here we have both containers - own and child containers
     *  try to merge the containers
     */
    typedef std::vector<ILVolume::Intersections::size_type> IndexCont;
    ...
    for ( Iter iterTop = own.begin(); own.end() != iterTop; ++iterTop ) {
      ...
      for ( Iter iter = child.begin(); child.end() != iter; ++iter ) {
        ...
        const int result = VolumeIntersectionIntervals::intersect( intervalTop, intervalLoc );
      ...
      for ( IndexCont::const_iterator it = tmpIndex.begin(); tmpIndex.end() != it; ++it ) {
        ...
        if ( rL <= 0 && rR <= 0 ) {
          //
          if ( rL < 0 ) {
            *out++   = ILVolume::Intersection( ILVolume::Interval( leftTick, intervalLocal.first ), matTop );
            leftTick = intervalLocal.first;
          }
          //
          if ( rR < 0 ) {
            *out++   = ILVolume::Intersection( ILVolume::Interval( leftTick, intervalLocal.second ), matLocal );
            leftTick = intervalLocal.second;
          } else if ( rR == 0 ) {
            *out++   = ILVolume::Intersection( ILVolume::Interval( leftTick, mostRightTick ), matLocal );
            leftTick = mostRightTick;
          }
        } else {
          ...
          if ( DetDesc::IntersectionErrors::recovery() ) {
            ...
            if ( 0 < rL && rR < 0 ) {
              *out++ = ILVolume::Intersection( ILVolume::Interval( leftTick, intervalLocal.second ), matLocal );
              // report the problem
              DetDesc::IntersectionErrors::recovered( volume, matTop, matLocal,
            ...
            // try to recover (3)
              ...
              *out++ = ILVolume::Intersection( ILVolume::Interval( leftTick, mostRightTick ), matLocal );
              ...
              DetDesc::IntersectionErrors::recovered(
                  ...
                  length * ( intervalLocal.second - mostRightTick + leftTick - intervalLocal.first ) );
              //
              leftTick = mostRightTick;
            }
          } // recover allowed?
      ...
      if ( rF < 0 ) { *out++ = ILVolume::Intersection( ILVolume::Interval( leftTick, mostRightTick ), matTop ); }
   ...
   *  @date 2007-12-13
   */
  inline int correct( const ILVolume* volume, ILVolume::Intersections& cnt, const double tick,
                      unsigned int index = 0 ) {
    // nothing to correct?
    if ( 2 + index > cnt.size() ) { return 0; } // RETURN
    ILVolume::Intersections::iterator i1 = cnt.begin() + index;
    // loop over all the intersections
    for ( ILVolume::Intersections::iterator i2 = i1 + 1; i2 != cnt.end(); ++i1, ++i2, ++index ) {
      ...
      if ( 0 < length ) {
        if ( DetDesc::IntersectionErrors::recovery() ) {
          DetDesc::IntersectionErrors::skip( volume, i1->second, tick * ( i1->first.second - i1->first.first ) );
          cnt.erase( i1 );
        ...
        DetDesc::IntersectionErrors::skip( volume, i1->second, 0 );
        cnt.erase( i1 );
        return correct( volume, cnt, tick, index ) + 1; // RETURN
      }
      // ======================================================================
      ...
      if ( 0 < result && i1->second != i2->second ) {
        if ( DetDesc::IntersectionErrors::recovery() ) {
          DetDesc::IntersectionErrors::recovered( volume, i1->second, i2->second,
