...
static const InterfaceID IID_ILVolume( 153, 3, 2 );
...
struct ILVolume : virtual IInterface {
   ...
   *  @return pointer to daughter (Physical Volume)
   */
  virtual const IPVolume* operator[]( std::string_view name ) const = 0;
   ...
   *  @return pointer to daughter (Physical Volume)
   */
  virtual const IPVolume* pvolume( std::string_view name ) const = 0;
   ...
   *  @return status code
   */
  virtual StatusCode belongsTo( const Gaudi::XYZPoint& localPoint, const int level,
                                ILVolume::PVolumePath& volumePath ) const = 0;
 ...
 */
// ============================================================================
inline MsgStream& operator<<( MsgStream& os, const ILVolume& lv ) { return lv.printOut( os ); }
