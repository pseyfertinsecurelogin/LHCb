...
namespace DetDesc {
  ...
  const Gaudi::Transform3D localDeltaMatrix( const IGeometryInfo* GI, const Gaudi::Transform3D& toMisaligned_global ) {
  ...
  void localDeltaParameters( const IDetectorElement* DE, std::vector<double>& trans, std::vector<double>& rot,
                             std::vector<double>& pivot ) {

    DetDesc::localDeltaParameters( DE->geometry(), trans, rot, pivot );
  }

  void localDeltaParameters( const IGeometryInfo* GI, std::vector<double>& trans, std::vector<double>& rot,
                             std::vector<double>& pivot ) {
    ...
    std::vector<double> tmp = ( GI->alignmentCondition()->exists( pivotString ) )
                                  ? GI->alignmentCondition()->paramAsDoubleVect( pivotString )
  ...
  const Gaudi::Transform3D localDeltaFromParentTransform( const IDetectorElement* parentDE, const IDetectorElement* DE,
                                                          const Gaudi::Transform3D& parentTransform ) {
    return localDeltaFromParentTransform( parentDE->geometry(), DE->geometry(), parentTransform );
  }

  const Gaudi::Transform3D localDeltaFromParentTransform( const IGeometryInfo* parentGI, const IGeometryInfo* GI,
                                                          const Gaudi::Transform3D& parentTransform ) {
    const Gaudi::Transform3D d_0       = GI->ownToOffNominalMatrix();
  ...
  const Gaudi::Transform3D localDeltaFromAnyFrame( const Gaudi::Transform3D& frame, const IDetectorElement* DE,
                                                   const Gaudi::Transform3D& toGlobalInFrame ) {
    return localDeltaFromAnyFrame( frame, DE->geometry(), toGlobalInFrame );
  }

  const Gaudi::Transform3D localDeltaFromAnyFrame( const Gaudi::Transform3D& frame, const IGeometryInfo* GI,
                                                   const Gaudi::Transform3D& toGlobalInFrame ) {
    const Gaudi::Transform3D d_0       = GI->ownToOffNominalMatrix();
  ...
  const Gaudi::Transform3D localDeltaFromAnyFrameDelta( const Gaudi::Transform3D& frame, const IDetectorElement* DE,
                                                        const Gaudi::Transform3D& deltaInFrame ) {
    return localDeltaFromAnyFrameDelta( frame, DE->geometry(), deltaInFrame );
  }

  const Gaudi::Transform3D localDeltaFromAnyFrameDelta( const Gaudi::Transform3D& frame, const IGeometryInfo* GI,
                                                        const Gaudi::Transform3D& deltaInFrame ) {
    const Gaudi::Transform3D d_0 = GI->ownToOffNominalMatrix();
  ...
  const Gaudi::Transform3D globalDelta( const IDetectorElement* DE ) { return globalDelta( DE->geometry() ); }
