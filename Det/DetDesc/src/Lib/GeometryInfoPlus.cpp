...
namespace {
  void recoverPrecision( Gaudi::Transform3D& matrix ) {
    ...
    matrix.SetComponents( xx, xy, xz, dx, yx, yy, yz, dy, zx, zy, zz, dz );
  }
} // namespace

//=============================================================================
// create "ghost"
GeometryInfoPlus::GeometryInfoPlus( IDetectorElement* de ) : m_gi_has_logical( false ), m_gi_iDetectorElement( de ) {
  if ( !de ) { throw GeometryInfoException( "IDetectorElement* points to NULL!" ); }
  if ( initialize().isFailure() ) { throw GeometryInfoException( "Failed to initialize!" ); }
}
//=============================================================================
// create orphan
GeometryInfoPlus::GeometryInfoPlus( IDetectorElement* de, const std::string& LogVol )
    : m_gi_lvolumeName( LogVol ), m_gi_iDetectorElement( de ) {
  if ( !de ) { throw GeometryInfoException( "IDetectorElement* points to NULL!" ); }
  if ( initialize().isFailure() ) { throw GeometryInfoException( "Failed to initialize!" ); }
}
//=============================================================================
/// create regular  with numeric replica path
GeometryInfoPlus::GeometryInfoPlus( IDetectorElement* de, const std::string& LogVol, const std::string& Support,
...
StatusCode GeometryInfoPlus::initialize() {
  ...
  if ( m_hasAlignmentPath ) {
    ...
    VERBO << "No AlignmentCondition requested. Assigning identity transformation" << endmsg;
  }

  return StatusCode{registerCondition() && registerSupportGI() && ums->update( this )};
}
//=============================================================================
bool GeometryInfoPlus::hasLVolume() const { return m_gi_has_logical; }
//=============================================================================
bool GeometryInfoPlus::hasSupport() const { return m_gi_has_support; }
//=============================================================================
const AlignmentCondition* GeometryInfoPlus::alignmentCondition() const { return m_alignmentCondition; }
//=============================================================================
const Gaudi::Transform3D& GeometryInfoPlus::toLocalMatrix() const { return *m_matrix; }
//=============================================================================
const Gaudi::Transform3D& GeometryInfoPlus::toGlobalMatrix() const { return *m_matrixInv; }
//=============================================================================
const Gaudi::Transform3D& GeometryInfoPlus::toLocalMatrixNominal() const { return *m_idealMatrix; }
//=============================================================================
const Gaudi::Transform3D& GeometryInfoPlus::toGlobalMatrixNominal() const { return *m_idealMatrixInv; }
//=============================================================================
...
bool GeometryInfoPlus::isInside( const Gaudi::XYZPoint& globalPoint ) const {
  return ( ( hasLVolume() && lvolume() ) ? lvolume()->isInside( toLocal( globalPoint ) )
...
StatusCode GeometryInfoPlus::calculateMatrices() {
  ...
  VERBO << "Welcome to calculateMatrices for volume " << gi->lvolumeName() << " support " << m_gi_supportName << endmsg;

  do {
    Assert( gi, " GeometryInfoPlus::calculateMatrices() - support is not found!" );
    Assert( gi->lvolume(), " GeometryInfoPlus::calculateMatrices() - support has no LVolume!" );
    ...
    VERBO << "get support IGeometryInfo* for " << gi->lvolumeName() << endmsg;

    gi = gi->supportIGeometryInfo();

  } while ( gi );

  VERBO << "calculateMatrices: stored " << m_pvMatrices.size() << " ideal and " << m_deltaMatrices.size()
        << " delta matrices for volumes " << endmsg;
  ...
  if ( isVerbose() ) {
    for ( const auto* info : parentGeomInfos ) { VERBO << "\t" << info->lvolumeName() << endmsg; }
  }

  return combineMatrices( deltaBegin(), deltaEnd(), idealBegin(), idealEnd() );
}
//=============================================================================
StatusCode GeometryInfoPlus::combineMatrices( matrix_iterator deltaFirst, matrix_iterator deltaEnd,
                                              matrix_iterator pvFirst, matrix_iterator pvEnd ) {
  if ( !idealMatrixLoaded() ) calculateIdealMatrix( pvFirst, pvEnd );
  return calculateFullMatrices( deltaFirst, deltaEnd, pvFirst );
}
//=============================================================================
StatusCode GeometryInfoPlus::calculateFullMatrices( matrix_iterator deltaFirst, matrix_iterator deltaEnd,
                                                    matrix_iterator pvFirst ) {
  m_matrix = std::inner_product( deltaFirst, deltaEnd, pvFirst, Gaudi::Transform3D{},
                                 std::multiplies<Gaudi::Transform3D>(), std::multiplies<Gaudi::Transform3D>() );
...
//=============================================================================
void GeometryInfoPlus::calculateIdealMatrix( matrix_iterator pvFirst, matrix_iterator pvEnd ) {

  m_idealMatrix    = std::accumulate( pvFirst, pvEnd, Gaudi::Transform3D{}, std::multiplies<Gaudi::Transform3D>() );
...
//=============================================================================
StatusCode GeometryInfoPlus::ownToOffNominalParams( const std::vector<double>& trans, const std::vector<double>& rot,
...
//=============================================================================
StatusCode GeometryInfoPlus::setLocalOffNominalDeltaMatrix( const Gaudi::Transform3D& newDelta ) {
  ...
  if ( !calculateFullMatrices( deltaBegin(), deltaEnd(), idealBegin() ) ) return StatusCode::FAILURE;
...
//=============================================================================
// FIXME: to be removed
StatusCode GeometryInfoPlus::getAlignmentCondition() { return StatusCode::SUCCESS; }
//=============================================================================
StatusCode GeometryInfoPlus::registerCondition() {
  VERBO << "registerCondition" << endmsg;
  updMgrSvc()->registerCondition( this, m_alignmentPath, &GeometryInfoPlus::cache, m_alignmentCondition );
  return StatusCode::SUCCESS;
}
//=============================================================================
...
const Gaudi::Transform3D& GeometryInfoPlus::ownToLocalMatrixNominal() const {
  ...
  if ( ( !gi || !hasLVolume() ) || !hasSupport() || ( this == (const IGeometryInfo*)gi ) ) {
    ...
    StatusCode            sc = lv->traverse( supportPath().begin(), supportPath().end(), volumePath );
    Assert( sc.isSuccess(), " GeometryInfoPlus::localIdealMatrix(): could not traverse the path!" );
...
//=============================================================================
Gaudi::Transform3D GeometryInfoPlus::ownToOffNominalMatrix() const { return ownToNominalMatrix().Inverse(); }
//=============================================================================
IGeometryInfo* GeometryInfoPlus::supportIGeometryInfo() const {

  if ( !hasSupport() ) return nullptr;

  VERBO << "supportIGeometryInfo with support " << m_gi_supportName << endmsg;

  if ( !m_gi_support ) {
    IGeometryInfo* gi = nullptr;
    if ( !m_gi_supportName.empty() ) { gi = geoByName( m_gi_supportName ); }
    m_gi_support = gi;
  }

  if ( isVerbose() ) {
    if ( m_gi_support ) {
      VERBO << "supportIGeometryInfo return IGeometryInfo* with volume " << m_gi_support->lvolumeName() << endmsg;
    } else {
      VERBO << "supportIGeometryInfo found no IGeometryInfo* with support " << m_gi_supportName << endmsg;
    }
  }
...
Gaudi::Transform3D GeometryInfoPlus::accumulateMatrices( const ILVolume::PVolumePath& volumePath ) const {
  return std::accumulate( volumePath.begin(), volumePath.end(), Gaudi::Transform3D{}, IPVolume_accumulateMatrix );
}
...
const ILVolume::ReplicaPath& GeometryInfoPlus::supportPath() const {
  ...
  while ( !aux.empty() && lv ) {
    ...
    if ( !physVol.empty() ) {
      ...
      Assert( lv->pvEnd() != it, "GeometryInfoPlus:: error during retrieve of Replica Path" );
...
//=============================================================================
StatusCode GeometryInfoPlus::fullGeoInfoForPoint( const Gaudi::XYZPoint& point, const int level, IGeometryInfo*& start,
                                                  ILVolume::PVolumePath& volumePath ) {
  // reset output values
  start = nullptr;
...
//=============================================================================
StatusCode GeometryInfoPlus::fullGeoInfoForPoint( const Gaudi::XYZPoint& point, const int level, std::string& start,
                                                  ILVolume::PVolumePath& volumePath ) {
  // reset output values
  start = std::string( "" );
  if ( !volumePath.empty() ) { volumePath.clear(); }
...
std::string GeometryInfoPlus::belongsToPath( const Gaudi::XYZPoint& globalPoint ) {
  ...
  // Look children
  auto it = std::find_if( childBegin(), childEnd(), IGeometryInfo_isInside( globalPoint ) );
  //
  return ( childEnd() == it ) ? std::string{} : *( m_gi_childrensNames.begin() + ( it - childBegin() ) );
}
//=============================================================================
std::string GeometryInfoPlus::belongsToPath( const Gaudi::XYZPoint& globalPoint, const int level ) {
  if ( 0 == level ) { return detElem()->name(); }
  IGeometryInfo* gi = belongsTo( globalPoint );
  return ( !gi ? detElem()->name() : gi->belongsToPath( globalPoint, level - 1 ) );
}
//=============================================================================
...
//=============================================================================
IGeometryInfo* GeometryInfoPlus::belongsTo( const Gaudi::XYZPoint& globalPoint, const int level ) {
...
StatusCode GeometryInfoPlus::loadChildren() const {
  ...
  Assert( m_gi_iDetectorElement, "GeometryInfoPlus::loadChildren IDetectorElement is not available!" );
  ...
  while ( IDEchildrens.end() != it ) {
    ...
    try {
      ...
      m_gi_childrensNames.clear();
      m_gi_childrens.clear();
      Assert( false, "GeometryInfoPlus::loadChildren(): exception caught!", Exception );
    } ///  throw new exception
    catch ( ... ) {
      m_gi_childrensNames.clear();
      m_gi_childrens.clear();
      Assert( false, "GeometryInfoPlus::loadChildren(): unknown exception caught!" );
    } //  throw new exception
  }
  //
...
IGeometryInfo* GeometryInfoPlus::reset() {
  ...
  m_gi_parent       = nullptr;
  /// reset daughters
  std::for_each( m_gi_childrens.begin(), m_gi_childrens.end(), []( IGeometryInfo* gi ) { gi->reset(); } );
  m_gi_childrens.clear();
  m_gi_childrensNames.clear();
  m_gi_childLoaded = false;
  ///
...
IGeometryInfo* GeometryInfoPlus::parentIGeometryInfo() {
  if ( !hasSupport() ) { return nullptr; }
  VERBO << "parentIGeometryInfo with support " << m_gi_supportName << endmsg;
  if ( !m_gi_support && !m_gi_supportName.empty() ) { m_gi_support = geoByName( m_gi_supportName ); }
  return m_gi_support;
}
//=============================================================================
const IGeometryInfo* GeometryInfoPlus::parentIGeometryInfo() const {
  if ( m_gi_parentLoaded ) { return m_gi_parent; }
  Assert( m_gi_iDetectorElement, "GeometryInfoPlus:: IDetectorElement is not available!" );

  auto ide = m_gi_iDetectorElement->parentIDetectorElement();
  Assert( ide, "GeometryInfoPlus:: parent           is not available!" );
  auto igi = ide->geometry();
  Assert( igi, "GeometryInfoPlus:: parent geometry  is not available!" );
  return igi;
}
//=============================================================================
const ILVolume* GeometryInfoPlus::lvolume() const {
  ...
  try {
    ...
    Assert( false, "GeometryInfoPlus::lvolume() : exception caught", Exception );
  } catch ( ... ) { Assert( false, "GeometryInfoPlus::lvolume() : unknown exception caught" ); }
  ///
  Assert( m_gi_lvolume, "GeometryInfoPlus::lvolume() : unable to load ILVolume!" );
  return m_gi_lvolume;
}
//=============================================================================
StatusCode GeometryInfoPlus::location( IGeometryInfo*& start, ILVolume::ReplicaPath& replicaPath ) const {
  replicaPath.clear();
...
//============================================================================
StatusCode GeometryInfoPlus::location( std::string& start, ILVolume::ReplicaPath& replicaPath ) const {
...
//============================================================================
const ILVolume* GeometryInfoPlus::lvolume( IGeometryInfo* start, const ILVolume::ReplicaPath& replicaPath ) {
  if ( !start || !start->lvolume() ) { return nullptr; }
  //
  const auto*           lv = start->lvolume();
  ILVolume::PVolumePath volumePath;
  if ( lv->traverse( replicaPath.begin(), replicaPath.end(), volumePath ).isFailure() ) { return nullptr; }
  //
...
//=============================================================================
std::string GeometryInfoPlus::lvolumePath( IGeometryInfo* start, const ILVolume::ReplicaPath& replicaPath ) {
  // starting point for address is invalid!
  if ( !start || !start->lvolume() ) { return std::string{}; }
  //
  const auto*           lv = start->lvolume();
  ILVolume::PVolumePath volumePath;
  if ( lv->traverse( replicaPath.begin(), replicaPath.end(), volumePath ).isFailure() ) { return ""; }
  if ( volumePath.empty() ) { return start->lvolumeName(); }
  const auto* pv = volumePath.back();
  return pv ? pv->lvolumeName() : std::string{};
}
//=============================================================================
const ILVolume* GeometryInfoPlus::lvolume( const std::string& start, const ILVolume::ReplicaPath& replicaPath ) {
  if ( start.empty() ) { return nullptr; }
  auto* gi = geoByName( start );
  return gi ? lvolume( gi, replicaPath ) : nullptr;
}
//=============================================================================
std::string GeometryInfoPlus::lvolumePath( const std::string& start, const ILVolume::ReplicaPath& replicaPath ) {
...
std::ostream& GeometryInfoPlus::printOut( std::ostream& os ) const {
  ...
  } catch ( ... ) { os << " !!! Unable to print GeometryInfo out !!!"; }
  //
  return os << '\n';
}
//=============================================================================
...
ILVolume* GeometryInfoPlus::findLogical() const {
  ...
  try {
    ...
    Assert( false, "GeometryInfoPlus::findLogical(), exception in retrieving LV=" + lvolumeName(), Exception );
  } ///  throw new exception
  catch ( ... ) {
    // throw new exception
    Assert( false, "GeometryInfoPlus::findLogical(), exception in retrieving LV=" + lvolumeName() );
  }
  Assert( lv, "GeometryInfoPlus::findLogical(), unable to retrieve LV=" + lvolumeName() );
  return lv;
}
//=============================================================================
StatusCode GeometryInfoPlus::queryInterface( const InterfaceID& ID, void** ppI ) {
...
// Destructor
//=============================================================================
GeometryInfoPlus::~GeometryInfoPlus() { updMgrSvc()->unregister( this ); }
//=============================================================================
