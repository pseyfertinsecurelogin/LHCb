...
SmartIF<ISvcLocator>& DetDesc::Services::svcLocator() const {
  ...
  if ( UNLIKELY( !m_svcLocator ) ) {
    ...
    if ( !m_svcLocator ) {
      throw GaudiException( "DetDesc::ISvcLocator* points to NULL!", "*DetDescException*", StatusCode::FAILURE );
    }
  }
  return m_svcLocator;
}
...
IDataProviderSvc* DetDesc::Services::detSvc() const {
  ...
  if ( UNLIKELY( !m_detSvc ) ) {
    ...
    if ( !m_detSvc ) {
      throw GaudiException( "DetDesc::Could not locate IDataProviderSvc='DetectorDataSvc'", "*DetDescException*",
                            StatusCode::FAILURE );
    }
  }
...
IMessageSvc* DetDesc::Services::msgSvc() const {
  ...
  if ( UNLIKELY( !m_msgSvc ) ) {
    ...
    if ( !m_msgSvc ) {
      throw GaudiException( "DetDesc::Could not locate IMessageSvc='MessageSvc'", "*DetDescException*",
                            StatusCode::FAILURE );
    }
  }
  return m_msgSvc;
...
IUpdateManagerSvc* DetDesc::Services::updMgrSvc( bool create ) const {
  ...
  if ( UNLIKELY( !m_updMgrSvc ) ) {
    ...
    if ( !m_updMgrSvc )
      throw GaudiException( "DetDesc::Could not locate IUpdateManagerSvc='UpdateManagerSvc'", "*DetDescException*",
                            StatusCode::FAILURE );
  }
  return m_updMgrSvc;
}
