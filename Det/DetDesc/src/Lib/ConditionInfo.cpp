...
/// Constructor
ConditionInfo::ConditionInfo( IDetectorElement* de, const std::string& condition )
    : m_detElem( de ), m_conditionName( condition ) {
  m_condition = 0;
  m_count     = 0;
...
IDataProviderSvc* ConditionInfo::dataSvc() const { return m_services->detSvc(); }

//----------------------------------------------------------------------------

/// Get a pointer to the detector element to which the ConditionInfo belongs
IDetectorElement* ConditionInfo::detElem() const { return m_detElem; }

//----------------------------------------------------------------------------

/// Get the name of the associated condition
const std::string& ConditionInfo::conditionName() const { return m_conditionName; }
