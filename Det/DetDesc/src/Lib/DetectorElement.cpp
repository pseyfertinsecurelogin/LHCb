 ...
 */
DetectorElement::DetectorElement( const std::string& /* name */ ) : m_services{DetDesc::services()} {}

DetectorElement::~DetectorElement() = default;

IDataProviderSvc* DetectorElement::dataSvc() const { return m_services->detSvc(); }

IUpdateManagerSvc* DetectorElement::updMgrSvc() const { return m_services->updMgrSvc(); }
...
IDetectorElement* DetectorElement::parentIDetectorElement() const {
  ...
  if ( mgr ) {
    ...
    if ( sc.isSuccess() && pRegParent ) { return dynamic_cast<IDetectorElement*>( pRegParent->object() ); }
  }
  return nullptr;
}

unsigned long DetectorElement::addRef() { return ParamValidDataObject::addRef(); }

unsigned long DetectorElement::release() { return ParamValidDataObject::release(); }

StatusCode DetectorElement::queryInterface( const InterfaceID& ID, void** ppI ) {
...
IDetectorElement* DetectorElement::reset() {
  ...
  if ( m_de_childrensLoaded ) std::for_each( childBegin(), childEnd(), []( IDetectorElement* de ) { de->reset(); } );
...
SmartRef<Condition> DetectorElement::condition( const std::string& name ) const {
  ...
  if ( cond == m_de_conditions.end() ) {
    ...
    throw DetectorElementException( oss.str(), this, StatusCode( StatusCode::FAILURE, true ) );
  }
  return cond->second;
}
...
const IGeometryInfo* DetectorElement::createGeometryInfo() {
  Assert( !geometry(), "Could not create GHOST: Geometry already exist!" );
...
const IGeometryInfo* DetectorElement::createGeometryInfo( const std::string& LogVol ) {
  Assert( !geometry(), "Could not create ORPHAN: Geometry already exist!" );
  m_de_iGeometry.reset( GeoInfo::createGeometryInfo( this, LogVol ) );
  return geometry();
}
/////
const IGeometryInfo* DetectorElement::createGeometryInfo( const std::string& LogVol, const std::string& Support,
                                                          const std::string& NamePath ) {
  Assert( !geometry(), "Could not create REGULAR(1): Geometry already exist!" );
...
const IGeometryInfo* DetectorElement::createGeometryInfo( const std::string& LogVol, const std::string& Support,
  ...
  Assert( !geometry(), "Could not create REGULAR(2): Geometry already exist!" );
...
IDetectorElement::IDEContainer& DetectorElement::childIDetectorElements() const {
  /// already loaded?
...
const ParamValidDataObject* DetectorElement::params() const { return this; }
...
const IDetectorElement* DetectorElement::childDEWithPoint( const Gaudi::XYZPoint& globalPoint ) const {
  auto iDE = std::find_if( childBegin(), childEnd(), [&]( IDetectorElement::IDEContainer::const_reference i ) {
