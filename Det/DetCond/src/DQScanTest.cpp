...
namespace {
  inline long long s2ns( unsigned int s ) { return static_cast<long long>( s ) * 1000000000; }
} // namespace
...
namespace DetCondTest {
  std::ostream& operator<<( std::ostream& s, const DetCondTest::IOV& iov ) {
    return Gaudi::Utils::toStream(
        std::make_pair( iov.since.ns() / Gaudi::Time::SEC_NSECS, iov.until.ns() / Gaudi::Time::SEC_NSECS ), s );
  }
} // namespace DetCondTest
...
namespace DetCondTest {
  ...
  StatusCode DQScanTest::finalize() {
    ...
    if ( release( m_scanner ).isFailure() ) { warning() << "Failed to release tool " << m_DQScannerName << endmsg; }
    m_scanner = nullptr;
