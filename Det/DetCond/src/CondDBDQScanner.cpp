...
namespace {
  class EvtTimeMgr {
  public:
    EvtTimeMgr(IDetDataSvc& dds): m_dds{dds} {
      m_orig_time = m_dds.eventTime();
    }
    void set(const Gaudi::Time& time) { m_dds.setEventTime(time); }
    ~EvtTimeMgr() {
      m_dds.setEventTime(m_orig_time);
    }
  private:
    IDetDataSvc& m_dds;
...
IDQFilter::FlagsType CondDBDQScanner::scan(const Gaudi::Time & since, const Gaudi::Time & until) const
{
  ...
  while ( cursor < until ) {
    ...
    if (m_converter->addressCreator()->createAddress(
            XML_StorageType, Condition::classID(),
            par, ipar, addr).isFailure()
