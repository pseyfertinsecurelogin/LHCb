...
// Constructor
// ------------------------------------------------------------------------
XmlBaseDetElemCnv::XmlBaseDetElemCnv( ISvcLocator* svc ) : XmlGenericCnv( svc, XmlBaseDetElemCnv::classID() ) {
...
void XmlBaseDetElemCnv::initStrings() {
  ...
  userParameterVectorString = xercesc::XMLString::transcode( "userParameterVector" );
...
// Constructor
// ------------------------------------------------------------------------
XmlBaseDetElemCnv::XmlBaseDetElemCnv( ISvcLocator* svc, const CLID& clsID ) : XmlGenericCnv( svc, clsID ) {
  m_doGenericCnv = false;
  initStrings();
}
...
StatusCode XmlBaseDetElemCnv::initialize() {
  ...
  if ( sc.isSuccess() ) {
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Initializing converter for class ID " << classID() << endmsg;
    if ( 0 != m_xmlSvc ) {
      m_doGenericCnv = m_xmlSvc->allowGenericCnv();
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Generic conversion status: " << (unsigned int)m_doGenericCnv << endmsg;
    }
  }
  return sc;
}
...
// -----------------------------------------------------------------------
StatusCode XmlBaseDetElemCnv::fillObjRefs( IOpaqueAddress* childElement, DataObject* refpObject ) {
  if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Method fillObjRefs() starting" << endmsg;
  ...
  if ( !dataObj ) {
    ...
    throw GaudiException( msg.str(), "XmlBaseDetElemCnv::fillObjRefs", StatusCode::FAILURE );
  }
  // initializes it
  sc = dataObj->initialize();
  if ( !sc.isSuccess() ) {
    error() << "DetectorElement " << dataObj->name() << ": initialization failed." << endmsg;
    return sc;
  }
  // returns
  if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Method fillObjRefs() ending" << endmsg;
...
// -----------------------------------------------------------------------
StatusCode XmlBaseDetElemCnv::i_createObj( xercesc::DOMElement* element, DataObject*& refpObject ) {
  // creates an object for the node found
  if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Normal generic detector element conversion" << endmsg;
...
// -----------------------------------------------------------------------
StatusCode XmlBaseDetElemCnv::i_fillObj( xercesc::DOMElement* childElement, DataObject* refpObject,
  ...
  if ( ( 0 == xercesc::XMLString::compareString( detelemrefString, tagName ) ) ||
    ...
    if ( mgr ) { sc = mgr->registerAddress( address->registry(), addr->par()[1], addr ); }
    if ( !sc.isSuccess() ) { throw GaudiException( "Unable to register new Address", "XmlBaseDetElemCnv", sc ); }
  } else if ( 0 == xercesc::XMLString::compareString( versionString, tagName ) ||
              0 == xercesc::XMLString::compareString( authorString, tagName ) ) {
    // currently ignored
  } else if ( 0 == xercesc::XMLString::compareString( geometryinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string logVolName    = dom2Std( childElement->getAttribute( lvnameString ) );
    std::string conditionPath = dom2Std( childElement->getAttribute( conditionString ) );
    std::string support       = dom2Std( childElement->getAttribute( supportString ) );
    std::string replicaPath   = dom2Std( childElement->getAttribute( rpathString ) );
    std::string namePath      = dom2Std( childElement->getAttribute( npathString ) );
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "GI volume        : " << logVolName << endmsg << "GI support       : " << support << endmsg
                << "GI rpath         : " << replicaPath << endmsg << "GI npath         : " << namePath << endmsg
    ...
    if ( logVolName.empty() ) {
      ...
      dataObj->createGeometryInfo( logVolName, support, namePath, conditionPath );
        ...
        if ( true == wasColon || *( rp + 1 ) == '\0' ) {
          if ( i > 0 ) {
            *replica = '\0';
            ...
            repPath.push_back( i );
            if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Found replica number " << repPath.back() << endmsg;
            replica = buf;
            i       = 0;
          }
        }
      ...
      error() << "File " << address->par()[0] << ": " << tagNameString << " Missing \"rpath\" or \"npath\" element, "
              << "please correct XML data\n"
              << " Either remove support element or provide proper rpath or npath" << endmsg;
      xercesc::XMLString::release( &tagNameString );
      StatusCode st( CORRUPTED_DATA );
      throw XmlCnvException( " Corrupted XML data", st );
    }
  } else if ( 0 == xercesc::XMLString::compareString( alignmentinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string condition = dom2Std( childElement->getAttribute( conditionString ) );
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Create AlignmentInfo with condition : " << condition << endmsg;
    dataObj->createAlignment( condition );
  } else if ( 0 == xercesc::XMLString::compareString( calibrationinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string condition = dom2Std( childElement->getAttribute( conditionString ) );
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Create CalibrationInfo with condition : " << condition << endmsg;
    dataObj->createCalibration( condition );
  } else if ( 0 == xercesc::XMLString::compareString( readoutinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string condition = dom2Std( childElement->getAttribute( conditionString ) );
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Create ReadOutInfo with condition : " << condition << endmsg;
    dataObj->createReadOut( condition );
  } else if ( 0 == xercesc::XMLString::compareString( slowcontrolinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string condition = dom2Std( childElement->getAttribute( conditionString ) );
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Create SlowControlInfo with condition : " << condition << endmsg;
    dataObj->createSlowControl( condition );
  } else if ( 0 == xercesc::XMLString::compareString( fastcontrolinfoString, tagName ) ) {
    // Everything is in the attributes
    std::string condition = dom2Std( childElement->getAttribute( conditionString ) );
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Create FastControlInfo with condition : " << condition << endmsg;
    ...
    for ( i = 0; i < specificChildren->getLength(); i++ ) {
      if ( specificChildren->item( i )->getNodeType() == xercesc::DOMNode::ELEMENT_NODE ) {
        // gets the current child
        xercesc::DOMNode* childNode = specificChildren->item( i );
        StatusCode        sc        = i_fillSpecificObj( (xercesc::DOMElement*)childNode, dataObj, address );
        if ( sc.isFailure() ) {
          std::string childNodeName = dom2Std( ( (xercesc::DOMElement*)childNode )->getNodeName() );
          warning() << "parsing of specific child " << childNodeName << " raised errors." << endmsg;
        }
      }
    }
              ...
              0 == xercesc::XMLString::compareString( userParameterVectorString, tagName ) ) {

    bool isSingleParam = ( 0 == xercesc::XMLString::compareString( userParameterString, tagName ) ||
    ...
    for ( i = 0; i < nodeChildren->getLength(); i++ ) {
      if ( nodeChildren->item( i )->getNodeType() == xercesc::DOMNode::TEXT_NODE ) {
        ...
        while ( begin < newVal.length() && isspace( newVal[begin] ) ) { begin++; }
        if ( begin < newVal.length() ) {
          unsigned int end = newVal.length() - 1;
          while ( isspace( newVal[end] ) ) { end--; }
          if ( firstWord ) {
            firstWord = false;
          } else {
            value += " ";
    ...
    if ( isSingleParam ) {
      ...
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Added user parameter " << name << " with value " << value << ", type " << type
                  << " and comment \"" << comment << "\"" << endmsg;
    } else {
      ...
      if ( msgLevel( MSG::VERBOSE ) ) {
        verbose() << "Adding user parameter vector " << name << " with values ";
        std::vector<std::string>::iterator it2;
        for ( it2 = vect.begin(); vect.end() != it2; ++it2 ) { verbose() << *it2 << " "; }
        verbose() << ", type " << type << " and comment \"" << comment << "\"." << endmsg;
      }
    ...
    char* tagNameString = xercesc::XMLString::transcode( tagName );
    warning() << "This tag makes no sense to element : " << tagNameString << endmsg;
    xercesc::XMLString::release( &tagNameString );
  }
...
// -----------------------------------------------------------------------
const CLID& XmlBaseDetElemCnv::classID() { return CLID_DetectorElement; }
