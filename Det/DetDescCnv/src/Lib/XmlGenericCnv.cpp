...
XmlGenericCnv::XmlGenericCnv( ISvcLocator* svc, const CLID& clid )
    : Converter( XML_StorageType, clid, svc ), m_have_CONDDB_StorageType( false ) {
...
StatusCode XmlGenericCnv::initialize() {
  ...
  StatusCode        sc        = addressCreator()->createAddress( CONDDB_StorageType, 5, par, &channelId, result );
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::createObj( IOpaqueAddress* addr, DataObject*& refpObject ) {

  StatusCode sc = StatusCode::FAILURE;

  // maked sure the address is not null
  if ( 0 == addr ) { return StatusCode::FAILURE; }
  ...
  if ( isAString ) {
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "Address for string: orig. path = " << addr->par()[2] << ", ObjectName = " << addr->par()[1]
              << ", string = ";
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "Address: filename = " << addr->par()[0] << ", ObjectName = " << addr->par()[1] << endmsg;
  }
  ...
  if ( document ) {
    ...
    unsigned int          index;
    for ( index = 0; index < list->getLength() && 0 == mainNode; ++index ) {
      if ( list->item( index )->getNodeType() == xercesc::DOMNode::ELEMENT_NODE ) {
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "found element " << dom2Std( childElement->getNodeName() ) << " at top level of xml file."
                    << endmsg;
    ...
    if ( mainNode ) {
      ...
      if ( 0 == xercesc::XMLString::compareString( mainNode->getNodeName(), DDDBString ) ) {
      ...
      if ( majorVersion == defaultMajorVersion ) {   // fine
        if ( minorVersion != defaultMinorVersion ) { // not perfect, just warn
          warning() << "DDDB DTD Version " << defaultMajorVersion << "." << defaultMinorVersion << " recommanded. "
                    << "You are currently using Version " << versionAttribute
                    << ". Everything should work fine but you may get some "
                    << "error messages about unknown tags." << endmsg;
        }
        ...
        for ( k = 0; k < macroList->getLength(); k++ ) {
          ...
          if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Added DDDB Macro " << name << " = " << value << endmsg;
        }
        ...
        for ( kk = 0; kk < parameterList->getLength(); kk++ ) {
          ...
          xmlSvc()->addParameter( name, value );
          if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Added DDDB Parameter " << name << " = " << value << endmsg;
        }
        ...
        if ( std::string::npos != slashPosition ) { objectName = objectName.substr( slashPosition + 1 ); }

        if ( mustSubstitute ) { //== Only in the last part of the string... A bit dangerous
          std::string::size_type indx = objectName.find( numeral );
          if ( indx == objectName.size() - numeral.size() ) { objectName = objectName.substr( 0, indx ) + "-KEY-"; }
        }

        if ( msgLevel( MSG::VERBOSE ) ) verbose() << "getElementByID for " << objectName << endmsg;
        ...
        if ( element != NULL ) {
          ...
          try {
            ...
            e.printOut( *m_msg );
            fatal() << endmsg;
            fatal() << "address = " << ( isAString ? addr->par()[2] : addr->par()[0] ) << "#" << addr->par()[1]
                    << endmsg;
            sc = e.code();
          }
        } else { // (element == NULL)
          fatal() << objectName << " : "
                  << "No such object in file " << addr->par()[0] << endmsg;
        }
      } else { // (majorVersion != defaultMajorVersion)
        // this is a problem
        error() << "DDDB DTD Version " << defaultMajorVersion << "." << defaultMinorVersion << " required. "
                << "You are currently using Version " << versionAttribute
                << ". Please update your DTD and XML data files. "
                << "If you are using the XmlDDDB package, please get a new version of it." << endmsg;
      }
    } else { // (mainNode == NULL)
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::internalCreateObj( xercesc::DOMElement* element, DataObject*& refpObject,
    ...
    if ( clsID != this->objType() ) {
      ...
      if ( 0 == conv ) {
        if ( false == this->xmlSvc()->allowGenericCnv() ) {
          error() << "No proper converter found for classID " << clsID << ", the default converter for "
                  << dom2Std( tagName ) << " will be used. This message may be ignored in case you are "
  ...
  while ( childNode ) {
    if ( childNode->getNodeType() == xercesc::DOMNode::ELEMENT_NODE ) {
      ...
      if ( sc2.isFailure() ) {
        error() << "unable to fill " << dom2Std( element->getNodeName() ) << " with its child "
                << dom2Std( childElement->getNodeName() ) << endmsg;
      }
    } else if ( childNode->getNodeType() == xercesc::DOMNode::TEXT_NODE ) {
      // gets the current child
      xercesc::DOMText* textNode = (xercesc::DOMText*)childNode;
      // calls fill_obj on it
      StatusCode sc2 = converter->i_fillObj( textNode, refpObject, address );
      if ( sc2.isFailure() ) {
        error() << "unable to fill " << dom2Std( element->getNodeName() ) << " with text node containing : \""
                << dom2Std( textNode->getData() ) << endmsg;
      }
    }
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::updateObj( IOpaqueAddress* pAddress, DataObject* pObject ) {
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::createRep( DataObject* /*pObject*/, IOpaqueAddress*& /*refpAddress*/ ) {
  return StatusCode::SUCCESS;
}
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::updateRep( IOpaqueAddress* /*pAddress*/, DataObject* /*pObject*/ ) {
  return StatusCode::SUCCESS;
}
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::i_createObj( xercesc::DOMElement* /*childElement*/, DataObject*& /*refpObject*/ ) {
  return StatusCode::FAILURE;
}
...
// Fill an object with a new child element
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::i_fillObj( xercesc::DOMElement* /*childElement*/, DataObject* /*refpObject*/,
                                     IOpaqueAddress* /*address*/ ) {
  return StatusCode::SUCCESS;
}
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::i_fillObj( xercesc::DOMText* /*childText*/, DataObject* /*refpObject*/,
                                     IOpaqueAddress* /*address*/ ) {
  return StatusCode::SUCCESS;
}
...
// -----------------------------------------------------------------------
StatusCode XmlGenericCnv::i_processObj( DataObject* /*refpObject*/, IOpaqueAddress* /*address*/ ) {
  return StatusCode::SUCCESS;
}
...
// -----------------------------------------------------------------------
IOpaqueAddress* XmlGenericCnv::createAddressForHref( std::string href, CLID clid, IOpaqueAddress* parent ) const {

  // expand environment variables in href
  std::string oldPath = href;
  if ( AddressTools::hasEnvironmentVariable( href ) ) {
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Found environment variable in path " << href << endmsg;
    if ( !AddressTools::expandAddress( href ) ) {
      throw XmlCnvException( "XmlGenericCnv : unable to resolve path " + oldPath, StatusCode::FAILURE );
    }
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "path expanded to " << href << endmsg;
  }
  ...
  if ( condDB ) {
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Href points to a conddb URL: " << href << endmsg;
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "Now build a CondDB address for path=" << path << " channelId=" << channelId
                << " and entryName=" << entryName << endmsg;
    // Then build a new Address
    return createCondDBAddress( path, entryName, channelId, clid );
  } else {
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Href points to a regular URL: " << href << endmsg;
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "Now build an XML address for location=" << location << " and entryName=" << entryName << endmsg;
    // Then build a new Address
...
// -----------------------------------------------------------------------
IOpaqueAddress* XmlGenericCnv::createXmlAddress( std::string location, std::string entryName, CLID clid ) const {
  ...
  StatusCode sc = addressCreator()->createAddress( XML_StorageType, clid, par, ipar, result );
  if ( !sc.isSuccess() ) { throw XmlCnvException( "XmlGenericCnv : Unable to create Address from href", sc ); }

  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "New address created : location = " << location << ", entry name = " << entryName
              << " isString : " << isString << endmsg;
  return result;
}
...
// build an address from location and entryName
// -----------------------------------------------------------------------
IOpaqueAddress* XmlGenericCnv::createCondDBAddress( std::string path, std::string entryName, unsigned long channelId,
                                                    CLID clid ) const {
  const std::string par[2] = {path, entryName};
  IOpaqueAddress*   result;
  StatusCode        sc = addressCreator()->createAddress( CONDDB_StorageType, clid, par, &channelId, result );
  if ( !sc.isSuccess() ) { throw XmlCnvException( "XmlGenericCnv : Unable to create Address from href", sc ); }
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "New address created : path = " << path << ", channel id = " << channelId
