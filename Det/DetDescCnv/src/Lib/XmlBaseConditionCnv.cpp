...
namespace {
  ...
  StatusCode _addHisto( Condition* dataObj, const std::string& name, std::string value, const std::string& comment );
} // namespace

// -----------------------------------------------------------------------
// Constructor
// ------------------------------------------------------------------------
XmlBaseConditionCnv::XmlBaseConditionCnv( ISvcLocator* svc ) : XmlGenericCnv( svc, XmlBaseConditionCnv::classID() ) {
...
// Constructor
// ------------------------------------------------------------------------
XmlBaseConditionCnv::XmlBaseConditionCnv( ISvcLocator* svc, const CLID& clsID ) : XmlGenericCnv( svc, clsID ) {
  m_doGenericCnv    = false;
  specificString    = xercesc::XMLString::transcode( "specific" );
  paramString       = xercesc::XMLString::transcode( "param" );
  paramVectorString = xercesc::XMLString::transcode( "paramVector" );
...
StatusCode XmlBaseConditionCnv::initialize() {
  ...
  if ( sc.isSuccess() ) {
    ...
    if ( 0 != m_xmlSvc ) {
      ...
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Generic conversion status: " << (unsigned int)m_doGenericCnv << endmsg;
    }
  }
  return sc;
}
...
// -----------------------------------------------------------------------
StatusCode XmlBaseConditionCnv::fillObjRefs( IOpaqueAddress* /*childElement*/, DataObject* refpObject ) {
  // gets the object
...
// -----------------------------------------------------------------------
StatusCode XmlBaseConditionCnv::updateObjRefs( IOpaqueAddress* /*childElement*/, DataObject* refpObject ) {
  // gets the object
  Condition* dataObj = static_cast<Condition*>( refpObject );
...
// -----------------------------------------------------------------------
StatusCode XmlBaseConditionCnv::i_createObj( xercesc::DOMElement* /*element*/, DataObject*& refpObject ) {
  // creates an object for the node found
  if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Normal generic condition conversion" << endmsg;
...
// -----------------------------------------------------------------------
StatusCode XmlBaseConditionCnv::i_fillObj( xercesc::DOMElement* childElement, DataObject* refpObject,
  ...
  if ( 0 == xercesc::XMLString::compareString( tagName, specificString ) ) {
    ...
    for ( i = 0; i < specificChildren->getLength(); i++ ) {
      if ( specificChildren->item( i )->getNodeType() == xercesc::DOMNode::ELEMENT_NODE ) {
        ...
        if ( sc.isFailure() ) {
          const XMLCh* childNodeName = ( (xercesc::DOMElement*)childNode )->getNodeName();
          char*        nameString    = xercesc::XMLString::transcode( childNodeName );
          warning() << "parsing of specific child " << nameString << " raised errors." << endmsg;
    ...
    if ( 0 == xercesc::XMLString::compareString( paramString, tagName ) ) {
      ...
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Adding user parameter " << name << " with value " << value << ", type " << type
      ...
      if ( type == "int" ) {
        ...
        if ( sc.isFailure() )
          error() << "failed to parse Histo1D name ='" << name << "' value = '" << value << "'" << endmsg;
      } else if ( type == "Histo2D" ) {
        StatusCode sc = _addHisto<DetDesc::Params::Histo2D>( dataObj, name, value, comment );
        if ( sc.isFailure() )
          error() << "failed to parse Histo2D name ='" << name << "' value = '" << value << "'" << endmsg;
      } else {
...
// -----------------------------------------------------------------------
const CLID& XmlBaseConditionCnv::classID() { return CLID_Condition; }
...
namespace {
  ...
  StatusCode _addHisto( Condition* dataObj, const std::string& name, std::string value, const std::string& comment ) {
    ...
    if ( sc.isSuccess() ) dataObj->addParam<HTYPE>( name, histo, comment );
