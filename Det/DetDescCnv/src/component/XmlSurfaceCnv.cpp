...
// -----------------------------------------------------------------------
StatusCode XmlSurfaceCnv::i_createObj( xercesc::DOMElement* element, DataObject*& refpObject ) {
...
#  pragma warning( disable : 1572 ) // Floating-point equality and inequality comparisons are unreliable
  ...
  // model
  {
    ...
    if ( i_value != v_value ) {
      log << MSG::WARNING << " Surface(" << elementName << ")"
          << " parameter '"
          << "model"
          << "'"
          << "(" << value << ")"
          << "=" << v_value << " is truncated to=" << i_value << endmsg;
    }
...
// -----------------------------------------------------------------------
StatusCode XmlSurfaceCnv::i_fillObj( xercesc::DOMElement* childElement, DataObject* refpObject,
  ...
  if ( 0 == xercesc::XMLString::compareString( tabpropsString, tagName ) ) {
    if ( log.level() <= MSG::DEBUG ) log << MSG::VERBOSE << "looking at tabprops" << endmsg;
    ...
    log << MSG::WARNING << "This tag makes no sense to surface : " << tagNameString << endmsg;
