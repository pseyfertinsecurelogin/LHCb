...
XmlTabulatedPropertyCnv::XmlTabulatedPropertyCnv( ISvcLocator* svcs )
    : XmlGenericCnv( svcs, classID() ), m_xunit( 1 ), m_yunit( 1 ) {
...
// -----------------------------------------------------------------------
StatusCode XmlTabulatedPropertyCnv::i_createObj( xercesc::DOMElement* element, DataObject*& refpObject ) {
  m_xunit = 1;
  m_yunit = 1;
...
// -----------------------------------------------------------------------
StatusCode XmlTabulatedPropertyCnv::i_fillObj( xercesc::DOMElement* childElement, DataObject* refpObject,
  ...
  if ( 0 == xercesc::XMLString::compareString( entryString, tagName ) ) {
    ...
    if ( xAttribute.empty() ) {
      throw XmlCnvException( "XmlTabulatedPropertyCnv: 'x' attribute for tag='entry'"
    ...
    if ( yAttribute.empty() ) {
      throw XmlCnvException( "XmlTabulatedPropertyCnv: 'y' attribute for tag='entry'"
                             " could not be omitted! " );
    }
    ...
    log << MSG::WARNING << "This tag makes no sense to tabulated property : " << tagNameString << endmsg;
...
// -----------------------------------------------------------------------
StatusCode XmlTabulatedPropertyCnv::i_fillObj( xercesc::DOMText* childText, DataObject* refpObject,
                                               IOpaqueAddress* /*address*/ ) {
  // gets the object
  TabulatedProperty* dataObj = dynamic_cast<TabulatedProperty*>( refpObject );
  // gets the text
  ...
  if ( !x.empty() || !y.empty() ) {
    throw XmlCnvException( "XmlTabulatedPropertyCnv: Corrupted Data #x does not match #y! " );
  }
  // returns
  return StatusCode::SUCCESS;
} // end i_fillObj
...
// Process an object
// -----------------------------------------------------------------------
StatusCode XmlTabulatedPropertyCnv::i_processObj( DataObject* refpObject, IOpaqueAddress* /*address*/ ) {
  // gets the object
  TabulatedProperty* dataObj = dynamic_cast<TabulatedProperty*>( refpObject );
  // sorts the table
