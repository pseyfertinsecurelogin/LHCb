...
namespace {
  ...
  bool is_any_of( const XMLCh* tag, std::initializer_list<Type> c ) {
    return std::any_of( begin( c ), end( c ),
                        [&]( const auto& i ) { return 0 == xercesc::XMLString::compareString( i, tag ); } );
  }
} // namespace
// -----------------------------------------------------------------------
...
// Constructor
// ------------------------------------------------------------------------
XmlLVolumeCnv::XmlLVolumeCnv( ISvcLocator* svc ) : XmlGenericCnv( svc, CLID_LVolume ) {
...
bool XmlLVolumeCnv::isBooleanSolid( const XMLCh* tag ) const {
  return is_any_of( tag, {std::cref( unionString ), std::cref( subtractionString ), std::cref( intersectionString )} );
} // end isBooleanSolid
...
bool XmlLVolumeCnv::isSimpleSolid( const XMLCh* tag ) const {
  return is_any_of( tag,
                    {std::cref( boxString ), std::cref( trdString ), std::cref( trapString ), std::cref( consString ),
                     std::cref( polyconeString ), std::cref( tubsString ), std::cref( sphereString )} );
} // end isSimpleSolid
...
bool XmlLVolumeCnv::isTransformation( const XMLCh* tag ) const {
  return is_any_of( tag, {std::cref( transformationString ), std::cref( posXYZString ), std::cref( posRPhiZString ),
                          std::cref( posRThPhiString ), std::cref( rotXYZString ), std::cref( rotAxisString )} );
...
bool XmlLVolumeCnv::isParamphysvol( const XMLCh* tag ) const {
  return is_any_of(
      tag, {std::cref( paramphysvolString ), std::cref( paramphysvol2DString ), std::cref( paramphysvol3DString )} );
} // end isTransformation
...
std::string XmlLVolumeCnv::createPvName( const PVolumeItem& pv ) const {
  ...
  return pv.indexed ? pv.physvolName + ":" + std::to_string( pv.tag ) : pv.physvolName;
}

// -----------------------------------------------------------------------
// Create an object corresponding to a DOM element
// -----------------------------------------------------------------------
StatusCode XmlLVolumeCnv::internalCreateObj( xercesc::DOMElement* element, DataObject*& refpObject,
  ...
  unsigned int i = 0;
  while ( i < childNodes->getLength() && childNodes->item( i )->getNodeType() != xercesc::DOMNode::ELEMENT_NODE ) {
    ++i;
  }
  ...
  if ( isSolid( tagName ) ) {
    ...
    if ( i < childNodes->getLength() ) {
      ...
      if ( isTransformation( childTagName ) ) {
        ...
        log << MSG::WARNING << "In logvol " << volName << ", a transformation (" << childTagName
            << ") is applied to the first solid (" << tagNameString
  ...
  if ( solid ) {
    ...
    if ( materialName.empty() || materialName[0] != '/' ) { materialName.insert( 0, "/dd/Materials/" ); }
    ...
    dataObj = std::make_unique<LVolume>( volName,
                                         ...
                                         materialName, sensDetName, magFieldName );
  } else {
    ...
    if ( !materialName.empty() ) {
      ...
      log << MSG::WARNING << "The logical volume " << volName
          ...
          << "defined for it (" << materialName << ") will be ignored." << endmsg;
    }
    // create an Assembly volume
    dataObj = std::make_unique<LAssembly>( volName, sensDetName, magFieldName );
  }
  ...
  while ( i < childNodes->getLength() ) {
    ...
    if ( 0 == xercesc::XMLString::compareString( physvolString, tagName ) ) {
      ...
      dataObj->createPVolume( createPvName( volume ), volume.logvolName,
                              volume.transformation ? volume.transformation->Inverse() : Gaudi::Transform3D() );
      ...
      for ( const auto& vol : *volumes ) {
        dataObj->createPVolume( createPvName( vol ), vol.logvolName,
                                vol.transformation ? vol.transformation->Inverse() : Gaudi::Transform3D() );
      }
      ...
      MsgStream log( msgSvc(), "XmlLVolumeCnv" );
      char*     tagNameString = xercesc::XMLString::transcode( tagName );
      log << MSG::WARNING << "This tag makes no sense to LVolume " << volName << " : " << tagNameString
...
// -----------------------------------------------------------------------
XmlLVolumeCnv::PVolumeItem XmlLVolumeCnv::dealWithPhysvol( const xercesc::DOMElement* element,
  ...
  // to find one
  unsigned int i = 0;
  while ( ( !transformation || *transformation == Gaudi::Transform3D() ) && i < childNodes->getLength() ) {
    while ( i < childNodes->getLength() && childNodes->item( i )->getNodeType() != xercesc::DOMNode::ELEMENT_NODE ) {
      ++i;
    }
    // is there is one, deal with the transformation
    if ( i < childNodes->getLength() ) { transformation = dealWithTransformation( element, &i ); }
  }
  ...
  if ( i < childNodes->getLength() ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "There are too many children in physical volume " << nameAttribute
        << ". The exceeding ones will be ignored." << endmsg;
  }
  ...
  if ( columnPos != std::string::npos ) {
    ...
    indexed            = true;
    if ( !digits.empty() ) {
      if ( 0 != numeral.size() ) { replaceTagInString( digits, numeral ); }
...
// -----------------------------------------------------------------------
std::unique_ptr<XmlLVolumeCnv::PVolumes> XmlLVolumeCnv::dealWithParamphysvol( const xercesc::DOMElement* element,
                                                                              const std::string& numeral ) const {
  ...
  } else if ( 0 == xercesc::XMLString::compareString( paramphysvol2DString, tagName ) ) {
    return dealWithParamphysvol( element, 2, numeral );
  } else if ( 0 == xercesc::XMLString::compareString( paramphysvol3DString, tagName ) ) {
    return dealWithParamphysvol( element, 3, numeral );
  } else {
    // unknown tag
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    char*     tagNameString = xercesc::XMLString::transcode( tagName );
    log << MSG::WARNING << "In " << locateElement( element ) << " : the tag " << tagNameString
...
std::unique_ptr<XmlLVolumeCnv::PVolumes> XmlLVolumeCnv::dealWithParamphysvol( const xercesc::DOMElement* element,
  ...
  {
    unsigned int i = 0;
    for ( i = 0; i < nD; i++ ) { numbers[i] = (int)xmlSvc()->eval( numberAttributes[i], false ); }
  }
  ...
  // some checks : numbers should be positive
  {
    ...
    for ( i = 0; i < nD; i++ ) {
      if ( numbers[i] < 0 ) {
        throw XmlCnvException( " paramphysvolume : number of copies must be positive ! ", CORRUPTED_DATA );
        return 0;
      }
    }
  }
  ...
  if ( i < childNodes->getLength() ) {
    ...
    xercesc::DOMElement* firstChildElement = (xercesc::DOMElement*)firstChildNode;
    ...
    std::vector<std::unique_ptr<Gaudi::Transform3D>> transformations;
    transformations.reserve( nD );

    // while there are still children, and not enough transformations found, try
    // to find more transformations
    unsigned int j = i + 1;
    while ( transformations.size() < nD && j < childNodes->getLength() ) {
      while ( j < childNodes->getLength() && childNodes->item( j )->getNodeType() != xercesc::DOMNode::ELEMENT_NODE ) {
        ++j;
      }
      // is there is one child, deal with the transformation
      if ( j < childNodes->getLength() ) { transformations.push_back( dealWithTransformation( element, &j ) ); }
    }
    ...
    if ( j < childNodes->getLength() ) {
      ...
      log << MSG::WARNING << "In " << locateElement( element )
          ...
          << "volume. The exceeding ones will be ignored." << endmsg;
    }
...
XmlLVolumeCnv::expandParamPhysVol( std::unique_ptr<PVolumes> volumes, const std::vector<int>& numbers,
  ...
  if ( transformations.size() != nD ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::FATAL << "In expandParamPhysVol, the dimensions of the arguments are "
        << "not compatible : I got " << nD << " numbers and " << transformations.size() << " transformations. "
    ...
    for ( unsigned int dimension = 0; dimension < nD; dimension++ ) {
      ...
      for ( unsigned int item = 0; item < numberOfItems; item++ ) {
        for ( int i = 1; i < numbers[dimension]; i++ ) {
          translations[numberOfItems * i + item] = transRot.first + translations[numberOfItems * ( i - 1 ) + item];
...
std::unique_ptr<ISolid> XmlLVolumeCnv::dealWithSolid( const xercesc::DOMElement* element,
  ...
  if ( isSimpleSolid( tagName ) ) {
    ...
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    char*     tagNameString = xercesc::XMLString::transcode( tagName );
    log << MSG::WARNING << "In " << locateElement( element ) << " : this should be a Solid but is a : " << tagName
        << ". It will be ignored" << endmsg;
    xercesc::XMLString::release( &tagNameString );
  }
  return nullptr;
...
std::unique_ptr<SolidBoolean> XmlLVolumeCnv::dealWithBoolean( const xercesc::DOMElement* element,
  ...
  if ( !isBooleanSolid( tagName ) ) {
    ...
    char*     tagNameString = xercesc::XMLString::transcode( tagName );
    log << MSG::WARNING << "In " << locateElement( element )
        << " : this sould be a boolean solid but is a : " << tagNameString << ". It will be ignored" << endmsg;
    xercesc::XMLString::release( &tagNameString );
    return nullptr;
  }
  ...
  if ( solids->size() < 2 ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << " : this boolean solid contains less than 2 solids. "
        << "It will be ignored" << endmsg;
    if ( solids->size() > 0 ) { solids->pop_back(); }
    return nullptr;
  }
  ...
  if ( 0 == xercesc::XMLString::compareString( unionString, tagName ) ) {
    ...
    if ( placedSolid.transformation ) {
      MsgStream log( msgSvc(), "XmlLVolumeCnv" );
      log << MSG::WARNING << "In union " << nameAttribute << ", a transformation"
    ...
    while ( !solids->empty() ) {
      ...
      solids->pop_front();
      StatusCode sc = unionResult->unite( std::move( placedSolid.solid ), placedSolid.transformation.get() );
      if ( !sc.isSuccess() ) {
        MsgStream log( msgSvc(), "XmlLVolumeCnv" );
        log << MSG::WARNING << "unionResult->unite failed. Don't know what to do..." << endmsg;
      }
    }
    result = std::move( unionResult );
  } else if ( 0 == xercesc::XMLString::compareString( subtractionString, tagName ) ) {
    auto subtractionResult = std::make_unique<SolidSubtraction>( nameAttribute, std::move( placedSolid.solid ) );
    // TO BE IMPLEMENTED -- TODO
    if ( placedSolid.transformation ) {
      MsgStream log( msgSvc(), "XmlLVolumeCnv" );
      log << MSG::WARNING << "In subtraction " << nameAttribute << ", a transformation"
          << " is applied to the first solid."
          << " This functionnality is not implemented yet. The "
          << " transformation will be ignored." << endmsg;
    }
    ...
    while ( !solids->empty() ) {
      placedSolid = std::move( solids->front() );
      solids->pop_front();
      StatusCode sc = subtractionResult->subtract( std::move( placedSolid.solid ), placedSolid.transformation.get() );
      if ( !sc.isSuccess() ) {
        MsgStream log( msgSvc(), "XmlLVolumeCnv" );
        log << MSG::WARNING << "unionResult->subtract failed. Don't know what to do.." << endmsg;
      }
    }
    result = std::move( subtractionResult );
  } else if ( 0 == xercesc::XMLString::compareString( intersectionString, tagName ) ) {
    auto intersectionResult = std::make_unique<SolidIntersection>( nameAttribute, std::move( placedSolid.solid ) );
    // TO BE IMPLEMENTED -- TODO
    if ( placedSolid.transformation ) {
      MsgStream log( msgSvc(), "XmlLVolumeCnv" );
      log << MSG::WARNING << "In intersection " << nameAttribute << ", a transformation"
          << " is applied to the first solid."
          << " This functionnality is not implemented yet. The "
          << " transformation will be ignored." << endmsg;
    }
    ...
    while ( !solids->empty() ) {
      placedSolid = std::move( solids->front() );
      solids->pop_front();
      StatusCode sc = intersectionResult->intersect( std::move( placedSolid.solid ), placedSolid.transformation.get() );
      if ( !sc.isSuccess() ) {
        MsgStream log( msgSvc(), "XmlLVolumeCnv" );
        log << MSG::WARNING << "unionResult->intersect failed. Don't know what to do!" << endmsg;
      }
    }
...
XmlLVolumeCnv::dealWithBooleanChildren( const xercesc::DOMElement* element, const std::string& numeral ) const {
  ...
  while ( i < childNodes->getLength() ) {
    if ( childNodes->item( i )->getNodeType() == xercesc::DOMNode::ELEMENT_NODE ) {
      ...
      if ( isSolid( tagName ) ) {
        ...
        MsgStream log( msgSvc(), "XmlLVolumeCnv" );
        char*     tagNameString = xercesc::XMLString::transcode( tagName );
        log << MSG::WARNING << "In " << locateElement( childElement ) << " : this " << tagName
...
std::unique_ptr<ISolid> XmlLVolumeCnv::dealWithSimpleSolid( const xercesc::DOMElement* element,
  ...
  if ( 0 == xercesc::XMLString::compareString( boxString, tagName ) ) {
    ...
    char*     tagNameString = xercesc::XMLString::transcode( tagName );
    log << MSG::WARNING << "In " << locateElement( element )
        << " : this should be a SimpleSolid but is a : " << tagNameString << ". It will be ignored" << endmsg;
    xercesc::XMLString::release( &tagNameString );
  }
  return nullptr;
...
std::unique_ptr<SolidBox> XmlLVolumeCnv::dealWithBox( const xercesc::DOMElement* element,
  ...
  if ( 0 != numeral.size() ) { replaceTagInString( solidName, numeral ); }

  // computes the values
  double sizeX = 0.0;
  double sizeY = 0.0;
  double sizeZ = 0.0;
  if ( !sizeXAttribute.empty() ) { sizeX = xmlSvc()->eval( sizeXAttribute ); }
  if ( !sizeYAttribute.empty() ) { sizeY = xmlSvc()->eval( sizeYAttribute ); }
  if ( !sizeZAttribute.empty() ) { sizeZ = xmlSvc()->eval( sizeZAttribute ); }

  // builds solid
  auto result = std::make_unique<SolidBox>( solidName, sizeX / 2.0, sizeY / 2.0, sizeZ / 2.0 );
...
std::unique_ptr<SolidTrd> XmlLVolumeCnv::dealWithTrd( const xercesc::DOMElement* element ) const {
  // gets attributes
  std::string sizeX1Attribute = dom2Std( element->getAttribute( sizeX1String ) );
  std::string sizeX2Attribute = dom2Std( element->getAttribute( sizeX2String ) );
  std::string sizeY1Attribute = dom2Std( element->getAttribute( sizeY1String ) );
  std::string sizeY2Attribute = dom2Std( element->getAttribute( sizeY2String ) );
  std::string sizeZAttribute  = dom2Std( element->getAttribute( sizeZString ) );
  ...
  double sizeY2 = 0.0;
  double sizeZ  = 0.0;
  if ( !sizeX1Attribute.empty() ) { sizeX1 = xmlSvc()->eval( sizeX1Attribute ); }
  if ( !sizeX2Attribute.empty() ) { sizeX2 = xmlSvc()->eval( sizeX2Attribute ); }
  if ( !sizeY1Attribute.empty() ) { sizeY1 = xmlSvc()->eval( sizeY1Attribute ); }
  if ( !sizeY2Attribute.empty() ) { sizeY2 = xmlSvc()->eval( sizeY2Attribute ); }
  if ( !sizeZAttribute.empty() ) { sizeZ = xmlSvc()->eval( sizeZAttribute ); }

  // builds solid
  auto result =
      std::make_unique<SolidTrd>( trdName, sizeZ / 2.0, sizeX1 / 2.0, sizeY1 / 2.0, sizeX2 / 2.0, sizeY2 / 2.0 );

  // checks there are no children
  if ( element->hasChildNodes() ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << " : trd should not have any child. They will be ignored"
        << endmsg;
  }
...
std::unique_ptr<SolidTrap> XmlLVolumeCnv::dealWithTrap( const xercesc::DOMElement* element ) const {
  ...
  std::string sizeX1Attribute = dom2Std( element->getAttribute( sizeX1String ) );
  std::string sizeX2Attribute = dom2Std( element->getAttribute( sizeX2String ) );
  std::string alp1Attribute   = dom2Std( element->getAttribute( alp1String ) );
  std::string sizeY2Attribute = dom2Std( element->getAttribute( sizeY2String ) );
  std::string sizeX3Attribute = dom2Std( element->getAttribute( sizeX3String ) );
  std::string sizeX4Attribute = dom2Std( element->getAttribute( sizeX4String ) );
  std::string alp2Attribute   = dom2Std( element->getAttribute( alp2String ) );
  ...
  double alp2   = 0.0;
  if ( !sizeZAttribute.empty() ) { sizeZ = xmlSvc()->eval( sizeZAttribute ); }
  if ( !thetaAttribute.empty() ) { theta = xmlSvc()->eval( thetaAttribute, false ); }
  if ( !phiAttribute.empty() ) { phi = xmlSvc()->eval( phiAttribute, false ); }
  if ( !sizeY1Attribute.empty() ) { sizeY1 = xmlSvc()->eval( sizeY1Attribute ); }
  if ( !sizeX1Attribute.empty() ) { sizeX1 = xmlSvc()->eval( sizeX1Attribute ); }
  if ( !sizeX2Attribute.empty() ) { sizeX2 = xmlSvc()->eval( sizeX2Attribute ); }
  if ( !alp1Attribute.empty() ) { alp1 = xmlSvc()->eval( alp1Attribute, false ); }
  if ( !sizeY2Attribute.empty() ) { sizeY2 = xmlSvc()->eval( sizeY2Attribute ); }
  if ( !sizeX3Attribute.empty() ) { sizeX3 = xmlSvc()->eval( sizeX3Attribute ); }
  if ( !sizeX4Attribute.empty() ) { sizeX4 = xmlSvc()->eval( sizeX4Attribute ); }
  if ( !alp2Attribute.empty() ) { alp2 = xmlSvc()->eval( alp2Attribute, false ); }

  // builds solid
  auto result = std::make_unique<SolidTrap>( trapName, sizeZ / 2.0, theta, phi, sizeY1 / 2.0, sizeX1 / 2.0,
                                             sizeX2 / 2.0, alp1, sizeY2 / 2.0, sizeX3 / 2.0, sizeX4 / 2.0, alp2 );

  // checks there are no children
  if ( element->hasChildNodes() ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << "A trap should not have any child. They will be ignored"
        << endmsg;
  }
...
std::unique_ptr<SolidCons> XmlLVolumeCnv::dealWithCons( const xercesc::DOMElement* element ) const {
  // gets attributes
  std::string sizeZAttribute         = dom2Std( element->getAttribute( sizeZString ) );
  std::string outerRadiusPZAttribute = dom2Std( element->getAttribute( outerRadiusPZString ) );
  std::string outerRadiusMZAttribute = dom2Std( element->getAttribute( outerRadiusMZString ) );
  std::string innerRadiusPZAttribute = dom2Std( element->getAttribute( innerRadiusPZString ) );
  std::string innerRadiusMZAttribute = dom2Std( element->getAttribute( innerRadiusMZString ) );
  std::string startPhiAngleAttribute = dom2Std( element->getAttribute( startPhiAngleString ) );
  std::string deltaPhiAngleAttribute = dom2Std( element->getAttribute( deltaPhiAngleString ) );
  ...
  if ( !sizeZAttribute.empty() ) { sizeZ = xmlSvc()->eval( sizeZAttribute ); }
  if ( !outerRadiusPZAttribute.empty() ) { outerRadiusPZ = xmlSvc()->eval( outerRadiusPZAttribute ); }
  if ( !outerRadiusMZAttribute.empty() ) { outerRadiusMZ = xmlSvc()->eval( outerRadiusMZAttribute ); }
  if ( !innerRadiusPZAttribute.empty() ) { innerRadiusPZ = xmlSvc()->eval( innerRadiusPZAttribute ); }
  if ( !innerRadiusMZAttribute.empty() ) { innerRadiusMZ = xmlSvc()->eval( innerRadiusMZAttribute ); }
  if ( !startPhiAngleAttribute.empty() ) { startPhiAngle = xmlSvc()->eval( startPhiAngleAttribute, false ); }
  if ( !deltaPhiAngleAttribute.empty() ) { deltaPhiAngle = xmlSvc()->eval( deltaPhiAngleAttribute, false ); }

  // builds solid
  auto result = std::make_unique<SolidCons>( consName, sizeZ / 2.0, outerRadiusMZ, outerRadiusPZ, innerRadiusMZ,
                                             innerRadiusPZ, startPhiAngle, deltaPhiAngle );

  // checks there are no children
  if ( element->hasChildNodes() ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << "A cons should not have any child. They will be ignored"
        << endmsg;
  }
...
std::unique_ptr<SolidPolycone> XmlLVolumeCnv::dealWithPolycone( const xercesc::DOMElement* element ) const {
  ...
  for ( unsigned int i = 0; i < children->getLength(); i++ ) {
    ...
    // gets attributes
    std::string zAttribute           = dom2Std( child->getAttribute( zString ) );
    std::string outerRadiusAttribute = dom2Std( child->getAttribute( outerRadiusString ) );
    std::string innerRadiusAttribute = dom2Std( child->getAttribute( innerRadiusString ) );

    // computes the values
    double z           = 0.0;
    double outerRadius = 0.0;
    double innerRadius = 0.0;
    if ( !zAttribute.empty() ) { z = xmlSvc()->eval( zAttribute ); }
    if ( !outerRadiusAttribute.empty() ) { outerRadius = xmlSvc()->eval( outerRadiusAttribute ); }
    if ( !innerRadiusAttribute.empty() ) { innerRadius = xmlSvc()->eval( innerRadiusAttribute ); }

    // builds zplane and adds it to the list
    zplanes.push_back( SolidPolycone::Triplet( z, SolidPolycone::Pair( outerRadius, innerRadius ) ) );

    // checks there are no children
    if ( child->hasChildNodes() ) {
      MsgStream log( msgSvc(), "XmlLVolumeCnv" );
      log << MSG::WARNING << "In " << locateElement( child )
          << "A zplane should not have any child. They will be ignored" << endmsg;
    }
  }
  ...
  try {
    ...
    return std::make_unique<SolidPolycone>( polyconeName, zplanes, startPhiAngle, deltaPhiAngle );
...
std::unique_ptr<SolidTubs> XmlLVolumeCnv::dealWithTubs( const xercesc::DOMElement* element ) const {
  // gets attributes
  std::string sizeZAttribute         = dom2Std( element->getAttribute( sizeZString ) );
  std::string outerRadiusAttribute   = dom2Std( element->getAttribute( outerRadiusString ) );
  std::string innerRadiusAttribute   = dom2Std( element->getAttribute( innerRadiusString ) );
  std::string startPhiAngleAttribute = dom2Std( element->getAttribute( startPhiAngleString ) );
  std::string deltaPhiAngleAttribute = dom2Std( element->getAttribute( deltaPhiAngleString ) );
  ...
  // builds solid
  auto result =
      std::make_unique<SolidTubs>( tubsName, sizeZ / 2.0, outerRadius, innerRadius, startPhiAngle, deltaPhiAngle );

  // checks there are no children
  if ( element->hasChildNodes() ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << "A tubs should not have any child. They will be ignored"
        << endmsg;
  }
...
std::unique_ptr<SolidSphere> XmlLVolumeCnv::dealWithSphere( const xercesc::DOMElement* element ) const {
  ...
  std::string startPhiAngleAttribute   = dom2Std( element->getAttribute( startPhiAngleString ) );
  std::string deltaPhiAngleAttribute   = dom2Std( element->getAttribute( deltaPhiAngleString ) );
  std::string startThetaAngleAttribute = dom2Std( element->getAttribute( startThetaAngleString ) );
  std::string deltaThetaAngleAttribute = dom2Std( element->getAttribute( deltaThetaAngleString ) );
  ...
  if ( !startPhiAngleAttribute.empty() ) { startPhiAngle = xmlSvc()->eval( startPhiAngleAttribute, false ); }
  if ( !deltaPhiAngleAttribute.empty() ) { deltaPhiAngle = xmlSvc()->eval( deltaPhiAngleAttribute, false ); }
  if ( !startThetaAngleAttribute.empty() ) { startThetaAngle = xmlSvc()->eval( startThetaAngleAttribute, false ); }
  if ( !deltaThetaAngleAttribute.empty() ) { deltaThetaAngle = xmlSvc()->eval( deltaThetaAngleAttribute, false ); }
  ...
  // builds the solid
  auto result = std::make_unique<SolidSphere>( sphereName, outerRadius, innerRadius, startPhiAngle, deltaPhiAngle,
...
// -----------------------------------------------------------------------
std::unique_ptr<Gaudi::Transform3D> XmlLVolumeCnv::dealWithTransformation( const xercesc::DOMElement* element,
  ...
  while ( *index < childNodes->getLength() &&
          childNodes->item( *index )->getNodeType() != xercesc::DOMNode::ELEMENT_NODE ) {
    ++*index;
  }
  ...
  if ( 0 == xercesc::XMLString::compareString( posXYZString, tagName ) ) {
    ...
    } catch ( const XmlCnvException& e ) { result.emplace(); }
  } else if ( 0 == xercesc::XMLString::compareString( posRThPhiString, tagName ) ) {
    // catches an exception in case the r attribute is negative
    try {
      result = dealWithPosRThPhi( childElement );
    } catch ( const XmlCnvException& e ) { result.emplace(); }
  } else if ( 0 == xercesc::XMLString::compareString( transformationString, tagName ) ) {
    ...
    log << MSG::WARNING << "In " << locateElement( childElement )
        << " : this should be a rotation or a translation but is a : " << tagNameString << ". It will be ignored."
        << endmsg;
    xercesc::XMLString::release( &tagNameString );
...
// Deal with transformation
// -----------------------------------------------------------------------
Gaudi::Transform3D XmlLVolumeCnv::dealWithTransformation( const xercesc::DOMElement* element ) const {
  // gets the children
  xercesc::DOMNodeList* childNodes = element->getChildNodes();
  ...
  if ( n < 2 ) {
    MsgStream log( msgSvc(), "XmlLVolumeCnv" );
    log << MSG::WARNING << "In " << locateElement( element ) << "A transformation should have at least two children."
        << endmsg;
  }
...
Gaudi::Transform3D XmlLVolumeCnv::dealWithPosXYZ( const xercesc::DOMElement* element ) const {
  ...
  double y = 0.0;
  double z = 0.0;
  if ( !xAttribute.empty() ) { x = xmlSvc()->eval( xAttribute ); }
  if ( !yAttribute.empty() ) { y = xmlSvc()->eval( yAttribute ); }
  if ( !zAttribute.empty() ) { z = xmlSvc()->eval( zAttribute ); }
...
Gaudi::Transform3D XmlLVolumeCnv::dealWithPosRPhiZ( const xercesc::DOMElement* element ) const {
  ...
  if ( !rAttribute.empty() ) { rho = xmlSvc()->eval( rAttribute ); }
  if ( !phiAttribute.empty() ) { phi = xmlSvc()->eval( phiAttribute, false ); }
  if ( !zAttribute.empty() ) { z = xmlSvc()->eval( zAttribute ); }
  // checks the validity of the value of r
  if ( 0.0 > rho ) { throw XmlCnvException( " RPhiZTranslation : r must be non-negative value !", CORRUPTED_DATA ); }
...
Gaudi::Transform3D XmlLVolumeCnv::dealWithPosRThPhi( const xercesc::DOMElement* element ) const {
  ...
  double theta = 0.0;
  double phi   = 0.0;
  if ( !rAttribute.empty() ) { r = xmlSvc()->eval( rAttribute ); }
  if ( !thetaAttribute.empty() ) { theta = xmlSvc()->eval( thetaAttribute, false ); }
  if ( !phiAttribute.empty() ) { phi = xmlSvc()->eval( phiAttribute, false ); }
  // checks the validity of the value of r
  if ( 0.0 > r ) { throw XmlCnvException( " RThPhiTranslation : r must be non-negative value !", CORRUPTED_DATA ); }
...
// Deal with rotXYZ
// -----------------------------------------------------------------------
Gaudi::Transform3D XmlLVolumeCnv::dealWithRotXYZ( const xercesc::DOMElement* element ) const {
  // get attributes
  ...
  double ry = 0.0;
  double rz = 0.0;
  if ( !rotXAttribute.empty() ) { rx = xmlSvc()->eval( rotXAttribute, false ); }
  if ( !rotYAttribute.empty() ) { ry = xmlSvc()->eval( rotYAttribute, false ); }
  if ( !rotZAttribute.empty() ) { rz = xmlSvc()->eval( rotZAttribute, false ); }

  // computes the rotation
  Gaudi::Rotation3D result = Gaudi::RotationX( rx ) * Gaudi::RotationY( ry ) * Gaudi::RotationZ( rz );
...
// Deal with rotAxis
// -----------------------------------------------------------------------
Gaudi::Transform3D XmlLVolumeCnv::dealWithRotAxis( const xercesc::DOMElement* element ) const {
  // get attributes
  std::string axThetaAttribute = dom2Std( element->getAttribute( axThetaString ) );
  std::string axPhiAttribute   = dom2Std( element->getAttribute( axPhiString ) );
  std::string angleAttribute   = dom2Std( element->getAttribute( angleString ) );

  // computes the values
  double axTheta = 0.0;
  double axPhi   = 0.0;
  double angle   = 0.0;
  if ( !axThetaAttribute.empty() ) { axTheta = xmlSvc()->eval( axThetaAttribute, false ); }
  if ( !axPhiAttribute.empty() ) { axPhi = xmlSvc()->eval( axPhiAttribute, false ); }
  if ( !angleAttribute.empty() ) { angle = xmlSvc()->eval( angleAttribute, false ); }
  // checks the validity of the theta angle
  if ( axTheta < 0 || axTheta > 180 * Gaudi::Units::degree ) {
    throw XmlCnvException( " AxisRotation : axTheta must be inside 0*degree"
                           " and 180*degree ! ",
                           CORRUPTED_DATA );
  }
  // checks the validity of the phi angle
  if ( axPhi < 0 || axPhi > 360 * Gaudi::Units::degree ) {
    throw XmlCnvException( " AxisRotation : axPhi  must be inside 0*degree"
                           " and 360*degree ! ",
                           CORRUPTED_DATA );
  }
  // Construction of vector with input of theta and phi
  Gaudi::XYZVector axis( sin( axTheta ) * cos( axPhi ), sin( axTheta ) * sin( axPhi ), cos( axTheta ) );

  return Gaudi::Transform3D( Gaudi::AxisAngle( axis, angle ), Gaudi::XYZVector() );
