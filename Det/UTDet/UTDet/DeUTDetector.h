...
public:
  ...
  */
  ...
  /** get the trajectory
   @return trajectory
  */
  LHCb::LineTraj<double> trajectory( const LHCb::LHCbID& id, const double offset ) const;
...
inline unsigned int DeUTDetLocation::detType() { return ( LHCb::UTChannelID::detType::typeUT ); }

#include "UTDet/DeUTLayer.h"
#include "UTDet/DeUTSector.h"
#include "UTDet/DeUTStation.h"

inline unsigned int DeUTDetector::firstStation() const { return m_firstStation; }

inline unsigned int DeUTDetector::lastStation() const { return m_firstStation + m_stations.size() - 1u; }

inline unsigned int DeUTDetector::nStation() const { return m_stations.size(); }
...
inline unsigned int DeUTDetector::nStrip() const { return m_nStrip; }

inline void DeUTDetector::setNstrip( const unsigned int nStrip ) { m_nStrip = nStrip; }

inline const DeUTDetector::Stations& DeUTDetector::stations() const { return m_stations; }

inline const DeUTDetector::Sectors& DeUTDetector::sectors() const { return m_sectors; }
...
inline unsigned int DeUTDetector::nReadoutSector() const { return sectors().size(); }
