...
StatusCode UpdateManagerSvc::Item::update( IDataProviderSvc* dp, const Gaudi::Time& when, MsgStream& log,
  ...
  if ( updateLock ) {
    if ( log.level() <= MSG::VERBOSE ) log << MSG::VERBOSE << "Update lock found: break loop" << endmsg;
    return StatusCode::SUCCESS;
  }
  // check validity
  if ( isValid( when ) ) {
    if ( log.level() <= MSG::VERBOSE ) log << MSG::VERBOSE << "Item valid, not need to update" << endmsg;
    return StatusCode::SUCCESS;
  }
  // prepare for update
  updateLock = true;
  ...
  if ( !ptr && !vdo && !path.empty() ) { // I do not have a VDO ptr (or a void*) but a path: load it
    ...
    if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "Retrieve object " << path << " from data provider" << endmsg;
    ...
    if ( override ) {
      ...
      if ( vdo ) { // Good, we can update in place
        ...
        if ( cond ) { // A condition needs to be initialized too
          sc = cond->initialize();
          if ( !sc.isSuccess() ) {
            log << MSG::ERROR << "Unable to initialize overridden condition at " << path << endmsg;
            return sc;
          }
        }
  ...
  if ( vdo && !vdo->isValid( when ) ) { // I have a VDO ptr and the object is not valid
    ...
    if ( !inInit ) { // during initialization we may have a loaded object that doesn't have a valid
      ...
      if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "Update object " << path << " from data provider" << endmsg;
  ...
  for ( auto mfIt = memFuncs.begin(); mfIt != memFuncs.end(); ++mfIt ) {
    if ( !mfIt->isValid( when ) ) { // only if one the children of the member function need an update
      ...
      if ( log.level() <= MSG::VERBOSE ) log << MSG::VERBOSE << "Loop over dependencies of m.f. " << n << endmsg;
      mfIt->resetIOV();
      ...
      if ( log.level() <= MSG::VERBOSE ) log << MSG::VERBOSE << "Call m.f. " << n << endmsg;
      sc = ( *( mfIt->mf ) )();
      if ( !sc.isSuccess() ) {
        if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "m.f. " << n << " returned a failure" << endmsg;
        return sc;
      }
    }
...
void UpdateManagerSvc::Item::purge( MsgStream* log ) {
  if ( !path.empty() ) {
    if ( log && ( *log ).level() <= MSG::DEBUG ) ( *log ) << MSG::DEBUG << "Purging " << path << endmsg;
    // Clean up pointers
    const bool force = true;
    setPointers( nullptr, force ).ignore();
  } else {
    if ( log && ( *log ).level() <= MSG::DEBUG ) ( *log ) << MSG::DEBUG << "Purging object at " << ptr << endmsg;
  }
