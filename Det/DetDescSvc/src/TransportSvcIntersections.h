 ...
 */
unsigned long TransportSvc::intersections( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& vect,
                                           const ISolid::Tick& tickMin, const ISolid::Tick& tickMax,
                                           ILVolume::Intersections& intersept, double threshold,
                                           IGeometryInfo* alternativeGeometry, IGeometryInfo* geometryGuess ) const {
  return intersections_r( point, vect, tickMin, tickMax, intersept, m_accelCache, threshold, alternativeGeometry,
                          geometryGuess );
}
 ...
 */
// ============================================================================
unsigned long TransportSvc::intersections_r( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& vect,
                                             const ISolid::Tick& tickMin, const ISolid::Tick& tickMax,
                                             ILVolume::Intersections& intersept, std::any& accelCache, double threshold,
                                             IGeometryInfo* alternativeGeometry, IGeometryInfo* guessGeometry ) const {
  ...
  try {
    ...
    if ( point1 == cache.prevPoint1 && point2 == cache.prevPoint2 && threshold == cache.previousThreshold &&
         topGeometry == cache.previousTopGeometry && guessGeometry == cache.previousGuess ) {
    ...
    IGeometryInfo* gi =
        ...
        ( guessGeometry && ( giLocal = findLocalGI( point1, point2, guessGeometry, alternativeGeometry ) ) )
            ? guessGeometry
            :
            // try the previous geometry (only if top-geometry matches)
            ( cache.previousGeometry && topGeometry == cache.previousTopGeometry &&
              ( giLocal = findLocalGI( point1, point2, cache.previousGeometry, topGeometry ) ) )
                ?
                // just take the top geometry
                cache.previousGeometry
                : ( giLocal = findLocalGI( point1, point2, topGeometry, topGeometry ) ) ?
