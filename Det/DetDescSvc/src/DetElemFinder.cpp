...
//=============================================================================
const IDetectorElement* DetElemFinder::detectorElementForPath( const std::string& path ) const { return m_map( path ); }
...
StatusCode DetElemFinder::initialize() {
  ...
  if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "--- initialize ---" << endmsg;

  auto detSvc = service<IDataProviderSvc>( m_detDataSvcName, true );
  if ( !detSvc ) {
    log << MSG::ERROR << "Unable to get a handle to the detector data service (" << m_detDataSvcName.value() << ")"
        << endmsg;
...
StatusCode DetElemFinder::finalize() {
  // local finalization
  MsgStream log( msgSvc(), name() );
  if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "--- finalize ---" << endmsg;

  if ( m_dumpMap ) {
    for ( const auto& i : m_map ) { log << MSG::ALWAYS << i.first << " -> " << i.second->name() << endmsg; }
  }
...
//=========================================================================
StatusCode DetElemFinder::detector_element_path( const IDetectorElement* de, std::string& path,
  ...
  sc = lv->traverse( de->geometry()->supportPath().begin(), de->geometry()->supportPath().end(), volumePath );
  if ( !sc.isSuccess() ) {
    MsgStream log( msgSvc(), name() );
  ...
  path = std::accumulate( volumePath.begin(), volumePath.end(), path,
                          []( std::string p, const IPVolume* vp ) { return p + "/" + vp->name(); } );
  return sc;
}
...
StatusCode DetElemFinder::insert( const IDetectorElement* de, const std::string& parent_path ) {
  ...
  MsgStream log( msgSvc(), name() );
  if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "Processing detector element " << de->name() << endmsg;
  ...
  if ( x != m_map.end() ) {
    ...
    if ( log.level() <= MSG::DEBUG ) log << MSG::DEBUG << "Insert path \"" << path << "\"" << endmsg;
    m_map[path] = de;
  }
