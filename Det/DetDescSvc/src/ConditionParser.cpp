...
namespace {
  ...
  struct TempParameter final {
    ...
    struct SetName {
      ...
      TempParameter& temp;
      void           operator()( char const* b, char const* e ) const { temp.name = std::string( b, e ); }
    };
    ...
    struct SetValue {
      ...
      TempParameter& temp;
      void           operator()( T val ) const { temp.setValue<T>( val ); }
    };
    ...
    struct SetValueString {
      ...
      TempParameter& temp;
      void operator()( char const* b, char const* e ) const { temp.setValue<std::string>( std::string( b, e ) ); }
    };
    ...
    struct AddVector {
      ...
      TempParameter& temp;
      void           operator()( char const*, char const* ) const { temp.newVector<T>(); }
    };
    ...
    struct PushBack {
      ...
      TempParameter& temp;
      void           operator()( T val ) const { temp.param->template get<std::vector<T>>().push_back( val ); }
    };
    ...
    struct AddParameter {
      ...
      void operator()( char const*, char const* ) const { cond.addBasicParam( temp.name, *temp.param ); }
    };
  };
  ...
  struct SetString {
    ...
    std::string& temp;
    void         operator()( char const* b, char const* e ) const { temp = std::string( b, e ); }
  };
  ...
  public:
    ...
    struct definition {
      ...
      /// type of elements
      enum Types { Integer = 0, Double, String, IntegerVector, DoubleVector, StringVector, NTypes };
      ...
      definition( const ConditionGrammar& self )
        ...
        type[IntegerVector] = str_p( "int_v" )[TempParameter::AddVector<int>( tmp )];
        type[DoubleVector]  = ( str_p( "double_v" ) | str_p( "dbl_v" ) )[TempParameter::AddVector<double>( tmp )];
        type[StringVector]  = ( str_p( "string_v" ) | str_p( "str_v" ) )[TempParameter::AddVector<std::string>( tmp )];
        ...
        value[Integer] = int_p[TempParameter::SetValue<int>( tmp )];
        value[Double]  = real_p[TempParameter::SetValue<double>( tmp )];
        value[String]  = ( ( +alnum_p ) % spaces )[TempParameter::SetValueString( tmp )];
