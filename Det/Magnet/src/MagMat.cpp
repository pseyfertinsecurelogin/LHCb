...
MagMat::MagMat( int N, int M ) {
  ...
  for ( int i = 0; i < Nrow; i++ ) {
    for ( int j = 0; j < Ncol; j++ ) { *( ptr_data + i * Ncol + j ) = 0.; }
  }
}
...
void MagMat::copy( const MagMat& m ) {
  ...
  for ( int i = 0; i < n; i++ ) {
    for ( int j = 0; j < l; j++ ) { *( ptr_data + i * Ncol + j ) = *( m.ptr_data + i * m.Ncol + j ); }
  }
}
...
MagMat& MagMat::operator=( const MagMat& m ) {
  ...
  if ( ( Nrow != 0 || Ncol != 0 ) && ( Nrow != m.Nrow || Ncol != m.Ncol ) ) {
    std::cerr << "MagMat MagMat Assignment: size do not match!" << std::endl;
    return *this;
  }
...
MagMat MagMat::operator+( const MagMat& m ) {
  ...
  for ( int i = 0; i < Nrow; i++ ) {
    for ( int j = 0; j < Ncol; j++ ) { *( b.ptr_data + i * b.Ncol + j ) = *( ptr_data + i * Ncol + j ) + m[i][j]; }
  }
...
MagMat& MagMat::operator+=( const MagMat& m ) {
  ...
  for ( int i = 0; i < Nrow; i++ ) {
    for ( int j = 0; j < Ncol; j++ ) { *( ptr_data + i * Ncol + j ) += *( m.ptr_data + i * m.Ncol + j ); };
  };
...
MagMat MagMat::T() const {
  ...
  for ( int i = 0; i < b.Nrow; i++ ) {
    for ( int j = 0; j < b.Ncol; j++ ) { b[i][j] = *( ptr_data + j * Ncol + i ); }
  }
  return b;
}
...
void MagMat::reSize( int Nnew, int Mnew ) {
  ...
  if ( Nnew != Nrow || Mnew != Ncol ) {
    ...
    for ( int i = 0; i < k; i++ ) {
      for ( int j = 0; j < l; j++ ) { *( ptr_data + i * Ncol + j ) = *( p + i * Ncol_old + j ); }
    }
...
int MagMat::nrow() const { return Nrow; }

int MagMat::ncol() const { return Ncol; }
