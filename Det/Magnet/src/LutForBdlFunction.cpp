...
LutForBdlFunction::LutForBdlFunction( int nVar, int* nBinVar, double* minVar, double* maxVar ) {
  ...
  if ( nVar < 1 || nVar > 100 ) {
    ...
    for ( i = 0; i < nVar; i++ ) {
      if ( nBinVar[i] < 1 || ( maxVar[i] - minVar[i] ) < 1.e-10 ) { good_input = 0; }
    }
  }
...
void LutForBdlFunction::createTable() {
  int i;
  int iSize = 1;
  for ( i = 0; i < m_nVar; i++ ) { iSize *= ( m_nPointVar[i] ); }
...
void LutForBdlFunction::resetIndexVector() {
  int i;
  for ( i = 0; i < m_nVar; i++ ) { m_indexVector[i] = 0; }
}

void LutForBdlFunction::resetVariableVector() {
  int i;
  for ( i = 0; i < m_nVar; i++ ) { m_variableVector[i] = 0; }
}
...
void LutForBdlFunction::fillTable( double lutValue ) { m_table[tableLocation()] = (float)lutValue; }
...
void LutForBdlFunction::getVariableVector( double* var ) {
  int i;
  calculateVariableVector();
  for ( i = 0; i < m_nVar; i++ ) { var[i] = m_variableVector[i]; }
}
...
int LutForBdlFunction::tableLocation() {
  ...
  for ( i = m_nVar - 2; i >= 0; i-- ) { location = location * m_nPointVar[i] + m_indexVector[i]; }
  return location;
}

void LutForBdlFunction::calculateVariableVector() {
  int i;
  for ( i = 0; i < m_nVar; i++ ) { m_variableVector[i] = m_minVar[i] + m_indexVector[i] * m_deltaVar[i]; }
}
