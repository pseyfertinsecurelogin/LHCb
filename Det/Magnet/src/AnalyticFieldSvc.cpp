...
//=============================================================================
AnalyticFieldSvc::AnalyticFieldSvc( const std::string& name, ISvcLocator* svc ) : base_class( name, svc ) {
...
StatusCode AnalyticFieldSvc::initialize() {
  ...
  if ( m_useConstField ) {
    log << MSG::WARNING << "using constant magnetic field with field vector " << m_constFieldVector << " (Tesla)"
        << endmsg;
    return StatusCode::SUCCESS;
  }

  if ( 1. != m_scaleFactor ) {
    log << MSG::WARNING << "Field map will be scaled by a factor = " << m_scaleFactor << endmsg;
  }

  status = GetParam();
  if ( status.isSuccess() ) {
    if ( UNLIKELY( log.level() <= MSG::DEBUG ) ) log << MSG::DEBUG << "B maps read successfully" << endmsg;

    return status;
  } else {
    if ( UNLIKELY( log.level() <= MSG::DEBUG ) ) log << MSG::DEBUG << "B maps failed" << endmsg;
...
StatusCode AnalyticFieldSvc::GetParam() {
  ...
  for ( int ifile = 0; ifile < 3; ifile++ ) {
    ...
    if ( !infile ) { log << MSG::ERROR << "Unable to open BMap : " << m_filename[ifile] << endmsg; }

    if ( infile ) {
      sc = StatusCode::SUCCESS;
      log << MSG::INFO << "BMap opened successfully : " << m_filename[ifile] << endmsg;

      // Skip the header till NREG
      do { infile.getline( line, 255 ); } while ( line[0] != 'N' || line[1] != 'R' );
      ...
      do { infile.getline( line, 255 ); } while ( line[0] != 'N' || line[1] != 'Y' );
      ...
      do { infile.getline( line, 255 ); } while ( line[0] != 'M' && line[1] != 'I' );
      ...
      for ( int w = 0; w < m_nREG; w++ ) { zmin[w] = atof( sZmin[w].c_str() ); }

      // Skip the header till ZMAXS
      do { infile.getline( line, 255 ); } while ( line[0] != 'M' && line[1] != 'A' );
      ...
      for ( int x = 0; x < m_nREG; x++ ) { zmax[x] = atof( sZmax[x].c_str() ); }

      for ( int k = 0; k < m_nREG; k++ ) {

        // Skip the header till next REGION
        do { infile.getline( line, 255 ); } while ( line[0] != 'R' );
...
//=============================================================================
StatusCode AnalyticFieldSvc::fieldVector( const Gaudi::XYZPoint& r, Gaudi::XYZVector& b ) const {
...
//=============================================================================
void AnalyticFieldSvc::Bcalculation( const Gaudi::XYZPoint& point, Gaudi::XYZVector& bf ) const {

  // initialize B field value
  bf.SetXYZ( 0, 0, 0 );
  // Use constant field?
  if ( m_useConstField ) {
    bf.SetXYZ( m_constFieldVector[0] * Gaudi::Units::tesla, m_constFieldVector[1] * Gaudi::Units::tesla,
  ...
  for ( int iz = 0; iz < m_nREG; iz++ ) {
    ...
    bool condZ = zmin[iz] <= point.z() / Gaudi::Units::cm && point.z() / Gaudi::Units::cm < zmax[iz] &&
                 coord[0] < ( fabs( coord[2] ) * tan( 0.3 ) ) && coord[1] < ( fabs( coord[2] ) * tan( 0.25 ) );
    ...
    if ( condZ ) {
      ...
      bf.SetXYZ( EvaluateField( coord, *( Bxmap[iReg] ) ), EvaluateField( coord, *( Bymap[iReg] ) ),
                 EvaluateField( coord, *( Bzmap[iReg] ) ) );
    }
  }
...
void AnalyticFieldSvc::ClearMaps() {
  ...
  for ( std::vector<MagMat*>::const_iterator iMatBx = Bxmap.begin(); iMatBx != Bxmap.end(); ++iMatBx ) {
    delete ( *iMatBx );
  }

  for ( std::vector<MagMat*>::const_iterator iMatBy = Bymap.begin(); iMatBy != Bymap.end(); ++iMatBy ) {
    delete ( *iMatBy );
  }

  for ( std::vector<MagMat*>::const_iterator iMatBz = Bzmap.begin(); iMatBz != Bzmap.end(); ++iMatBz ) {
    delete ( *iMatBz );
  }
}
