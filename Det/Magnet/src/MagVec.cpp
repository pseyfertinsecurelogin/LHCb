...
MagVec::~MagVec() { delete[] ptr_data; }

double& MagVec::operator[]( int i ) {

  if ( i < 0 ) {
    std::cerr << "MagVec: Index < zero! " << std::endl;
    return ptr_data[0];
  };
  if ( i >= Nele ) {
    std::cerr << "MagVec: Index too large! " << std::endl;
    return ptr_data[0];
  };
...
MagVec& MagVec::operator=( const MagVec& v ) {
  ...
  if ( Nele != 0 && Nele != v.Nele ) {
    std::cerr << "MagVec Assignment: size does not match!" << std::endl;
    return *this;
  }
...
int MagVec::n_elem() const { return Nele; }
