...
//=============================================================================
MagneticFieldSvc::MagneticFieldSvc( const std::string& name, ISvcLocator* svc ) : base_class( name, svc ) {
  ...
  declareProperty( "NominalCurrent", m_nominalCurrent = 5850, "Nominal magnet current in Amps" );
...
//=============================================================================
{
  ...
  m_updMgrSvc->registerCondition( this, m_UseSetCurrent ? MagnetCondLocations::Set : MagnetCondLocations::Measured,
  ...
  if ( !m_mapFileNames.empty() ) {
    log << MSG::WARNING << "Requested condDB but using manually set field map file name(s) = " << m_mapFileNames
        << endmsg;

    m_mapFromOptions = true;
    MagneticFieldGridReader reader( *msgSvc() );
    const StatusCode sc = ( m_mapFileNames.size() == 1 ? reader.readDC06File( m_mapFileNames.front(), m_magFieldGrid )
  ...
  if ( m_forcedScaleFactor < 9998. ) {
    log << MSG::WARNING << "Requested condDB but using manually set signed scale factor = " << m_forcedScaleFactor
        << endmsg;
...
{
  MsgStream log( msgSvc(), name() );
  if ( UNLIKELY( log.level() <= MSG::DEBUG ) ) log << MSG::DEBUG << "updateConditions called" << endmsg;

  if ( m_forcedToUseDownMap && m_forcedToUseUpMap )
    log << MSG::WARNING << "inconsistent settings, forced to use Down AND Uo map = " << endmsg;

  double polarity = 0;
  if ( m_forcedToUseDownMap ) { polarity = -1.0; }
  if ( m_forcedToUseUpMap ) { polarity = +1.0; }
  if ( !m_forcedToUseDownMap && !m_forcedToUseUpMap ) { polarity = m_currentPtr->param<int>( "Polarity" ); }

  // Update the scale factor
  if ( m_forcedScaleFactor > 9998. ) {
    const double current = m_currentPtr->param<double>( "Current" );

    const auto& coeffs = ( polarity > 0 ? m_scaleUpPtr->param<std::vector<double>>( "Coeffs" )
  ...
  if ( !m_mapFromOptions ) {
    ...
    if ( m_mapFileNames != files ) {
      ...
      // update the field
      MagneticFieldGridReader reader( *msgSvc() );
      sc = ( m_mapFileNames.size() == 1 ? reader.readDC06File( m_mapFileNames.front(), m_magFieldGrid )
                                        : reader.readFiles( m_mapFileNames, m_magFieldGrid ) );
      if ( UNLIKELY( log.level() <= MSG::DEBUG ) )
