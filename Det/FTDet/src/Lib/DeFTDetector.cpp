...
// classID function
//=============================================================================
const CLID& DeFTDetector::clID() const { return DeFTDetector::classID(); }
...
StatusCode DeFTDetector::initialize() {
  ...
  *m_msg << MSG::INFO << "Current FT geometry version =   " << m_FTversion << endmsg;
    ...
    if ( station ) {
      ...
      if ( stationID < 4 && stationID > 0 ) {
        ...
        for ( const auto layer : station->layers() ) {
          for ( const auto quarter : layer->quarters() ) m_quarters[iQuarter++] = quarter;
        }
      }
    }
  }
  ...
  m_nTotChannels      = ( m_nModulesT1 + m_nModulesT2 + m_nModulesT3 ) * m_nLayers * m_nQuarters * m_nChannelsInModule;
...
LHCb::FTChannelID DeFTDetector::getRandomChannelFromSeed( const double seed ) const {
  if ( seed < 0.0 || seed > 1.0 ) return LHCb::FTChannelID( 0u );
  ...
  if ( flatChannel >= m_nModulesT1 + m_nModulesT2 ) {
    station = 3;
    module  = flatChannel - m_nModulesT1 - m_nModulesT2;
  } else if ( flatChannel >= m_nModulesT1 ) {
    station = 2;
    module  = flatChannel - m_nModulesT1;
  }
...
LHCb::FTChannelID DeFTDetector::getRandomChannelFromPseudo( const int pseudoChannel, const double seed ) const {
  if ( seed < 0.0 || seed > 1.0 ) return LHCb::FTChannelID( 0u );
  ...
  const DeFTModule* moduleDet = findModule( LHCb::FTChannelID( station, layer, quarter, module, 0u ) );
