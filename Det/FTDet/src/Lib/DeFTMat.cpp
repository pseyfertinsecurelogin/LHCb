...
LHCb::FTChannelID DeFTMat::calculateChannelAndFrac( float localX, float& frac ) const {
  ...
  // Construct channelID
  return LHCb::FTChannelID( stationID(), layerID(), quarterID(), moduleID(), matID(), hitSiPM,
                            hitChan + ( hitDie * m_nChannelsInDie ) );
}

// Get the relevant channel boundaries
std::vector<std::pair<LHCb::FTChannelID, float>> DeFTMat::calculateChannels( LHCb::FTChannelID thisChannel,
...
DeFTMat::calculateChannels( const float localEntry, const float localExit,
  ...
  if ( thisChannel.channelID() == endChannel.channelID() && std::abs( fracBegin ) > 0.5 && std::abs( fracEnd ) > 0.5 &&
       fracBegin * fracEnd > 0.25 )
...
std::vector<std::pair<LHCb::FTChannelID, float>> DeFTMat::calculateChannels() const {
  ...
  LHCb::FTChannelID endChannel =
      ( stationID(), layerID(), quarterID(), moduleID(), matID(), m_nChannelsInSiPM * m_nSiPMsInMat - 1 );

  return DeFTMat::calculateChannels( thisChannel, endChannel );
}

// Find the local x-position for a given channel+fraction
float DeFTMat::localXfromChannel( const LHCb::FTChannelID channelID, const float frac ) const {
...
float DeFTMat::distancePointToChannel( const Gaudi::XYZPoint& globalPoint, const LHCb::FTChannelID channelID,
...
LHCb::LineTraj<double> DeFTMat::trajectory( const LHCb::FTChannelID channelID, const float frac ) const {
...
std::pair<Gaudi::XYZPointF, Gaudi::XYZPointF> DeFTMat::endPoints( const LHCb::FTChannelID channelID,
                                                                  const float             frac ) const {
  float localX = localXfromChannel( channelID, frac );
