...
StatusCode DeFTFibreMat::initialize(){
  ...
  if( deFibreTracker ){
    if( deFibreTracker->exists("FTversion") )
      FTversion = deFibreTracker->param<int>("FTversion");
  }
  ...
  if (FTversion < 0) {
    ...
    if( deFibreTracker->exists("NumLayers") )
      m_numlayers = deFibreTracker->param<int>("NumLayers");
    else
      m_numlayers = 12;
  }
  ...
  if(m_FTGeomversion < 50) {   //geom < 5x
    ...
    //additional layers have one bit more to handle with two more stations
    if((m_FTGeomversion > 50) && (m_FTGeomversion < 60) && (m_numlayers > 12)){
      
      const unsigned int nFMbits = 10;
      
      std::bitset<nFMbits> bset(std::to_string(m_FibreMatID));
      
      std::bitset<nFMbits> bT(bset.to_string(),0,3);
      std::bitset<nFMbits> bL(bset.to_string(),3,2);
      std::bitset<nFMbits> bQ(bset.to_string(),5,2);
      std::bitset<nFMbits> bM(bset.to_string(),7,3);
    ...
    {
      //geometry with 12 layers, standard 9 bits
    ...
    //Alessio P. TO FIX: please fix to read from DDDB!
    ...
    //fix for additional layers
    if( (dT - 1) < 3 )
      m_layer = dL + 4*(dT - 1);  //standard layer
    else
    {
      //additional layers
      if((dT - 1) == 3)
				       ...
				       unsigned int numOfAdditionalChannels) const 
{
  ...
  if ( (enPSipmID==exPSipmID) && (enPCellID==exPCellID) ) {
    ...
    channelRightEdges.push_back ( this->cellLocalX(channel) - m_cellSizeX/2 );
    channelLeftEdges.push_back ( this->cellLocalX(channel) + m_cellSizeX/2);
  ...
  if (this->cellLocalX(channelIDs[0])<this->cellLocalX(channelIDs[vector_size -1]))
    {
      for (unsigned int i = 0; i < numOfAdditionalChannels; ++i)
	{ channel = this->nextChannelLeft(channelIDs[vector_size -1 +i].channelID());
	  channelIDs.push_back(channel);
	  channelRightEdges.push_back ( this->cellLocalX(channel) - m_cellSizeX/2 );
	  channelLeftEdges.push_back ( this->cellLocalX(channel) + m_cellSizeX/2);
	} 
	...
	{                  
	  ...
	  channel = this->nextChannelRight(channelIDs[0].channelID());
	  it = channelIDs.insert(it, channel);
	  auto itRight = channelRightEdges.begin();
	  itRight = channelRightEdges.insert(itRight, this->cellLocalX(channel) - m_cellSizeX/2 );
	  auto itLeft = channelLeftEdges.begin();
	  itLeft = channelLeftEdges.insert(itLeft, this->cellLocalX(channel) + m_cellSizeX/2 );
	}
    ...
    {
      for (unsigned int i = 0; i < numOfAdditionalChannels; ++i)
	{ channel = this->nextChannelRight(channelIDs[vector_size -1 +i].channelID());
	  channelIDs.push_back(channel);
	  channelRightEdges.push_back ( this->cellLocalX(channel) - m_cellSizeX/2 );
	  channelLeftEdges.push_back ( this->cellLocalX(channel) + m_cellSizeX/2);
	} 
	...
	{                  
	  auto it = channelIDs.begin();
	  channel = this->nextChannelLeft(channelIDs[0].channelID());
	  it = channelIDs.insert(it, channel);
	  auto itRight = channelRightEdges.begin();
	  itRight = channelRightEdges.insert(itRight, this->cellLocalX(channel) - m_cellSizeX/2 );
	  auto itLeft = channelLeftEdges.begin();
