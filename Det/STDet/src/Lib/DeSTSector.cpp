...
StatusCode DeSTSector::initialize() {
  ...
  if ( sc.isFailure() ) {
    ...
    if ( m_versionString != "DC06" ) {
      sc = registerCondition( this, m_statusString, &DeSTSector::updateStatusCondition, true );
      if ( sc.isFailure() ) {
        MsgStream msg( msgSvc(), name() );
      ...
      if ( m_versionString != "DC07" ) {
        sc = registerCondition( this, m_noiseString, &DeSTSector::updateNoiseCondition, true );
...
double DeSTSector::sectorNoise() const {
  ...
  for ( unsigned int chan( 0 ); chan < m_nStrip; ++chan ) {
    if ( statusVector[chan] == DeSTSector::OK || statusVector[chan] == DeSTSector::Pinhole ) {
...
double DeSTSector::portNoise( const unsigned int& beetle, const unsigned int& port ) const {
  if ( beetle > nBeetle() ) {
    MsgStream msg( msgSvc(), name() );
    msg << MSG::WARNING << "You asked for beetle " << beetle << " but there are " << nBeetle() << " of them" << endmsg;
    return 999.99;
...
void DeSTSector::setNoise( const unsigned int& strip, const double& value ) {
  ...
  if ( aCon == 0 ) {
    ...
    std::vector<double>& reference = aCon->param<std::vector<double>>( "SectorNoise" );
...
void DeSTSector::setCMNoise( const unsigned int& strip, const double& value ) {
  ...
  if ( aCon == 0 ) {
    ...
    std::vector<double>& reference = aCon->param<std::vector<double>>( "cmNoise" );
    reference[strip - 1u]          = value;
...
double DeSTSector::cmPortNoise( const unsigned int& beetle, const unsigned int& port ) const {
  if ( beetle > nBeetle() ) {
    MsgStream msg( msgSvc(), name() );
    msg << MSG::WARNING << "You asked for beetle " << beetle << " but there are " << nBeetle() << " of them" << endmsg;
    return 999.99;
...
double DeSTSector::toADC( const double& e, const LHCb::STChannelID& aChannel ) const {
...
double DeSTSector::toElectron( const double& val, const LHCb::STChannelID& aChannel ) const {
  return toElectron( val, aChannel.strip() );
}

double DeSTSector::toElectron( const double& val, const unsigned int& aStrip ) const {
  return val * m_electronsPerADC[aStrip - 1];
}

LHCb::BrokenLineTrajectory DeSTSector::trajectory( const STChannelID& aChan, const double offset ) const {

  if ( !contains( aChan ) ) {
    MsgStream msg( msgSvc(), name() );
    msg << MSG::ERROR << "Failed to link to sector " << nickname() << " test strip  number " << aChan.strip()
        << " strip " << endmsg;
    throw GaudiException( "Failed to make trajectory", "DeSTSector.cpp", StatusCode::FAILURE );
  }

  return createTraj( aChan.strip(), offset );
}

LHCb::BrokenLineTrajectory DeSTSector::trajectoryFirstStrip() const { return createTraj( m_firstStrip, 0. ); }

LHCb::BrokenLineTrajectory DeSTSector::trajectoryLastStrip() const { return createTraj( nStrip(), 0. ); }

LHCb::BrokenLineTrajectory DeSTSector::createTraj( const unsigned int strip, const double offset ) const {
...
STChannelID DeSTSector::nextLeft( const STChannelID testChan ) const {
  if ( ( contains( testChan ) ) && ( isStrip( testChan.strip() - 1u ) == true ) ) {
    return STChannelID( testChan.type(), testChan.station(), testChan.layer(), testChan.detRegion(), testChan.sector(),
                        testChan.strip() - 1u );
  } else {
...
DeSTSensor* DeSTSector::findSensor( const Gaudi::XYZPoint& point ) const {
  ...
  auto iter =
      std::find_if( m_sensors.begin(), m_sensors.end(), [&]( const DeSTSensor* s ) { return s->isInside( point ); } );
  return iter != m_sensors.end() ? *iter : nullptr;
}

bool DeSTSector::globalInActive( const Gaudi::XYZPoint& point, Gaudi::XYZPoint tol ) const {
  return std::any_of( m_sensors.begin(), m_sensors.end(),
                      [&]( const DeSTSensor* s ) { return s->globalInActive( point, tol ); } );
}

bool DeSTSector::globalInBondGap( const Gaudi::XYZPoint& point, double tol ) const {
...
void DeSTSector::setBeetleStatus( const unsigned int beetle, const DeSTSector::Status& newStatus ) {
...
void DeSTSector::setStripStatus( const unsigned int strip, const DeSTSector::Status& newStatus ) {
...
void DeSTSector::setStatusCondition( const std::string& type, const unsigned int entry,
