...
DeITLayer::DeITLayer( const std::string& name )
    : DeSTLayer( name ), m_parent( NULL ), m_firstSector( NULL ), m_lastSector( NULL ) {
  // constructor
}

const CLID& DeITLayer::clID() const { return DeITLayer::classID(); }
...
StatusCode DeITLayer::initialize() {
  ...
  if ( sc.isFailure() ) {
    ...
    m_firstSector = ( *start )->sector();
    m_lastSector  = ( *start )->sector();
...
DeITLadder* DeITLayer::findLadder( const Gaudi::XYZPoint& point ) const {
  ...
  auto iter =
      std::find_if( m_ladders.begin(), m_ladders.end(), [&]( const DeITLadder* l ) { return l->isInside( point ); } );
  return iter != m_ladders.end() ? *iter : nullptr;
}

void DeITLayer::flatten() {

  std::transform( m_ladders.begin(), m_ladders.end(), std::back_inserter( m_sectors ),
                  []( const child_type* i ) { return i->sector(); } );
}

double DeITLayer::fractionActive() const {

  return std::accumulate( m_ladders.begin(), m_ladders.end(), 0.0,
                          []( double fa, const DeITLadder* l ) { return fa + l->fractionActive(); } ) /
