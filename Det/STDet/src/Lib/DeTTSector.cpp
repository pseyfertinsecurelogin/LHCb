...
DeTTSector::DeTTSector( const std::string& name ) : DeSTSector( name ), m_parent( NULL ), m_row( 0u ) {
  // constructer
}

const CLID& DeTTSector::clID() const { return DeTTSector::classID(); }

std::string DeTTSector::hybridType() const { return m_hybridType; }
...
StatusCode DeTTSector::initialize() {
  ...
  if ( sc.isFailure() ) {
    ...
    STChannelID chan( STChannelID::detType::typeTT, parentID.station(), parentID.layer(), parentID.detRegion(), id(),
                      0 );
    setElementID( chan );
    ...
    m_isStereo               = false;
    if ( ( chan.station() == TTNames::Station::TTa && layer == 2 ) ||
         ( chan.station() == TTNames::Station::TTb && layer == 1 ) )
      m_isStereo = true;
    ...
    m_conditionPathName = TTNames().UniqueLayerToString( chan ) + "LayerR" + region + "Module" + col + position() +
                          "Sector" + hybridType();
...
unsigned int DeTTSector::prodID() const { return m_parent->prodID(); }

std::string DeTTSector::conditionsPathName() const { return m_conditionPathName; }

std::string DeTTSector::moduleNumber( const unsigned int& reg, const unsigned int& station ) const {
  using namespace boost;
  if ( reg == 1 ) // => A
