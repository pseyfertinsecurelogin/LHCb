...
DeITBox::DeITBox( const std::string& name ) : DeSTBaseElement( name ) {
  // constructer
}

const CLID& DeITBox::clID() const { return DeITBox::classID(); }
...
StatusCode DeITBox::initialize() {
  ...
  if ( sc.isFailure() ) {
    ...
    STChannelID       chan( STChannelID::detType::typeIT, parentID.station(), parentID.layer(), m_id, 0, 0 );
    setElementID( chan );
...
std::ostream& DeITBox::printOut( std::ostream& os ) const {
  ...
  // stream to cout
  os << " Box : " << name() << std::endl << " id " << id() << std::endl << " nickname " << nickname() << std::endl;
...
MsgStream& DeITBox::printOut( MsgStream& os ) const {
  ...
  os << " Box : " << name() << " id " << id() << " nickname " << nickname() << std::endl;
...
DeITLayer* DeITBox::findLayer( const STChannelID aChannel ) {
  ...
  auto i =
      std::find_if( m_layers.begin(), m_layers.end(), [&]( const DeITLayer* l ) { return l->contains( aChannel ); } );
...
DeITLayer* DeITBox::findLayer( const Gaudi::XYZPoint& point ) {
  ...
  auto i = std::find_if( m_layers.begin(), m_layers.end(), [&]( const DeITLayer* l ) { return l->isInside( point ); } );
  return i != m_layers.end() ? *i : nullptr;
}

double DeITBox::fractionActive() const {

  return std::accumulate( m_layers.begin(), m_layers.end(), 0.0, [&]( double x, const DeITLayer* l ) {
    return x + l->fractionActive() / double( m_layers.size() );
  } );
}

bool DeITBox::contains( const LHCb::STChannelID aChannel ) const {
  return ( ( aChannel.detRegion() == elementID().detRegion() ) && m_parent->contains( aChannel ) );
}
