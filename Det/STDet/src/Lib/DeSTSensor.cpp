...
void DeSTSensor::clear() { m_midTraj.reset(); }
...
double DeSTSensor::localU( const unsigned int strip, const double offset ) const {
  // strip to local
...
bool DeSTSensor::localInActive( const Gaudi::XYZPoint& point, Gaudi::XYZPoint tol ) const {
...
LHCb::LineTraj<double> DeSTSensor::trajectory( const unsigned int strip, const double offset ) const {
