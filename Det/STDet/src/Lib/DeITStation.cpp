...
DeITStation::DeITStation( const std::string& name ) : DeSTStation( name ), m_parent( NULL ) {
  // constructor
}

const CLID& DeITStation::clID() const { return DeITStation::classID(); }
...
DeITBox* DeITStation::findBox( const STChannelID aChannel ) {
  ...
  auto iter =
      std::find_if( m_boxes.begin(), m_boxes.end(), [&]( const DeITBox* b ) { return b->contains( aChannel ); } );
...
DeITBox* DeITStation::findBox( const Gaudi::XYZPoint& point ) {
  ...
  auto iter = std::find_if( m_boxes.begin(), m_boxes.end(), [&]( const DeITBox* b ) { return b->isInside( point ); } );
  return iter != m_boxes.end() ? *iter : nullptr;
}
