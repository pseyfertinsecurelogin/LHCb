...
DeTTDetector::DeTTDetector( const std::string& name ) : DeSTDetector( name ) {
  // constructer
  m_sectors.clear();
}

const CLID& DeTTDetector::clID() const { return DeTTDetector::classID(); }
...
DeSTSector* DeTTDetector::findSector( const Gaudi::XYZPoint& aPoint ) const {
  ...
  if ( tStation ) {
    ...
    if ( aLayer ) {
      ...
      if ( aModule ) { aSector = aModule->findSector( aPoint ); } // module
    }                                                             // layer
  }                                                               // station
  return aSector;
}
...
DeSTBaseElement* DeTTDetector::findTopLevelElement( const std::string& nickname ) const {
  ...
  if ( chan.layer() ) { return findLayer( chan ); }
  if ( chan.station() != 0u ) {
    // its a station
