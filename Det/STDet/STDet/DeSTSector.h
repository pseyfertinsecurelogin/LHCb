...
public:
   ...
   */
  double toElectron( const double& val, const LHCb::STChannelID& aChannel ) const;
   ...
   */
  LHCb::BrokenLineTrajectory trajectory( const LHCb::STChannelID& aChan, const double offset ) const;
...
private:
  ...
  void setStatusCondition( const std::string& type, const unsigned int entry, const DeSTSector::Status& newStatus );
...
inline unsigned int DeSTSector::id() const { return m_id; }

inline void DeSTSector::setID( unsigned int id ) { m_id = id; }

inline bool DeSTSector::contains( const LHCb::STChannelID aChannel ) const {
  return ( aChannel.uniqueSector() == elementID().uniqueSector() );
}

inline double DeSTSector::pitch() const { return m_pitch; }

inline unsigned int DeSTSector::nStrip() const { return m_nStrip; }

inline std::string DeSTSector::versionString() const { return m_versionString; }

inline double DeSTSector::deadWidth() const { return m_deadWidth; }

inline bool DeSTSector::isStrip( const unsigned int strip ) const {
  return ( strip >= m_firstStrip && strip < m_firstStrip + m_nStrip );
}

inline double DeSTSector::capacitance() const { return m_capacitance; }
...
inline double DeSTSector::stripLength() const { return m_stripLength; }

inline double DeSTSector::thickness() const { return m_thickness; }

inline std::string DeSTSector::type() const { return m_type; }

inline double DeSTSector::angle() const { return m_angle; }

inline double DeSTSector::sinAngle() const { return m_sinAngle; }

inline double DeSTSector::cosAngle() const { return m_cosAngle; }

inline bool DeSTSector::isStereo() const { return m_isStereo; }

inline void DeSTSector::trajectory( unsigned int strip, double offset, double& dxdy, double& dzdy, double& xAtYEq0,
...
inline unsigned int DeSTSector::beetle( const LHCb::STChannelID& chan ) const { return beetle( chan.strip() ); }

inline unsigned int DeSTSector::beetle( const unsigned int strip ) const {
  return ( ( strip - 1u ) / LHCbConstants::nStripsInBeetle ) + 1u;
}

inline unsigned int DeSTSector::nBeetle() const { return nStrip() / LHCbConstants::nStripsInBeetle; }
...
inline DeSTSector::Status DeSTSector::sectorStatus() const { return m_status; }

inline DeSTSector::Status DeSTSector::beetleStatus( const LHCb::STChannelID& chan ) const {
  return beetleStatus( beetle( chan ) );
}

inline void DeSTSector::setBeetleStatus( const LHCb::STChannelID& chan, const DeSTSector::Status& newStatus ) {
  // just delegate
  setBeetleStatus( beetle( chan ), newStatus );
}

inline void DeSTSector::setStripStatus( const LHCb::STChannelID& chan, const DeSTSector::Status& newStatus ) {
  // just delegate
...
inline LHCb::STChannelID DeSTSector::stripToChan( const unsigned int strip ) const {
  return isStrip( strip ) ? LHCb::STChannelID( elementID().type(), elementID().station(), elementID().layer(),
                                               elementID().detRegion(), elementID().sector(), strip )
                          : LHCb::STChannelID( 0 );
}

inline const DeSTSector::Sensors& DeSTSector::sensors() const { return m_sensors; }

inline DeSTSensor* DeSTSector::middleSensor() const { return m_sensors[m_sensors.size() / 2u]; }

inline bool DeSTSector::xInverted() const { return middleSensor()->xInverted(); }

inline bool DeSTSector::yInverted() const { return middleSensor()->yInverted(); }
 ...
 */
inline std::ostream& operator<<( std::ostream& os, const DeSTSector* aSector ) { return aSector->printOut( os ); }
 ...
 *  @param aSector reference to DeSTSector object
 */
inline MsgStream& operator<<( MsgStream& os, const DeSTSector* aSector ) { return aSector->printOut( os ); }

inline const std::string& DeSTSector::nickname() const { return m_nickname; }

inline const Condition* DeSTSector::statusCondition() const { return condition( m_statusString ); }

inline const Condition* DeSTSector::noiseCondition() const { return condition( m_noiseString ); }

inline double DeSTSector::measEff() const { return m_measEff; }
#endif // _DeSTSector_H
