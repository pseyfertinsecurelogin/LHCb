...
inline Gaudi::XYZPoint DeSTBaseElement::globalCentre() const { return m_globalCentre; }
...
inline typename STDetTraits<TYPE>::parent* DeSTBaseElement::getParent() const {
  ...
  if ( parent == 0 ) { throw GaudiException( "Orphaned detector element", "DeSTBaseElement", StatusCode::FAILURE ); }

  return parent;
}

inline LHCb::STChannelID DeSTBaseElement::elementID() const { return m_elementID; }

inline void DeSTBaseElement::setElementID( const LHCb::STChannelID& chanID ) { m_elementID = chanID; }
...
inline std::vector<typename STDetTraits<TYPE>::child*> DeSTBaseElement::getChildren() {
  ...
  if ( childVector.empty() ) {
    throw GaudiException( "Sterile detector element", "DeSTBaseElement", StatusCode::FAILURE );
  }
