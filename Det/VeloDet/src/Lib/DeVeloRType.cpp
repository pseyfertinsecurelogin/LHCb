...
//==============================================================================
const CLID& DeVeloRType::clID() const { return DeVeloRType::classID(); }
//==============================================================================
...
StatusCode DeVeloRType::initialize() {
  ...
  updMgrSvc()->registerCondition( this, this->m_geometry, &DeVeloRType::updateGeometryCache );
...
//==============================================================================
StatusCode DeVeloRType::pointToChannel( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& channel, double& fraction,
                                        double& pitch ) const {
  ...
  double       logarithm    = ( m_pitchSlope * ( radius - m_innerR ) + m_innerPitch ) / m_innerPitch;
...
StatusCode DeVeloRType::isInActiveArea( const Gaudi::XYZPoint& point ) const {
  ...
  double y = point.y();
  if ( m_phiGap > y && -m_phiGap < y ) { return StatusCode::FAILURE; }
  // corner cut-offs
...
//==============================================================================
StatusCode DeVeloRType::neighbour( const LHCb::VeloChannelID& start, const int& nOffset,
...
//=============================================================================
StatusCode DeVeloRType::residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel, double& residual,
                                  double& chi2 ) const {
...
//==============================================================================
StatusCode DeVeloRType::residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel,
...

{
  ...
  if ( m_verbose ) {
    msg() << MSG::VERBOSE << "Residual; rPoint = " << rPoint << " strip " << strip << " rStrip = " << rStrip
          << " offset: " << offset << " residual " << residual << " sigma = " << sigma << " chi2 = " << chi2 << endmsg;
  }
  return StatusCode::SUCCESS;
}
...
void DeVeloRType::calcStripLimits() {
  ...
  m_pitchSlope = ( m_outerPitch - m_innerPitch ) / ( m_outerR - m_innerR );
  ...
  if ( m_staticDataInvalid ) {
    ...
    for ( unsigned int zone = 0; zone < m_numberOfZones; ++zone ) {
      for ( unsigned int istrip = 0; istrip < m_stripsInZone; ++istrip ) {
        radius =
            ( vdt::fast_exp( m_pitchSlope * istrip ) * m_innerPitch - ( m_innerPitch - m_pitchSlope * m_innerR ) ) /
            m_pitchSlope;
    ...
    for ( auto spli = m_stripPhiLimits.begin(); spli != m_stripPhiLimits.end() && ri != m_rStrips.end();
          ++spli, ++ri ) {
  ...
  if ( m_debug ) {
    for ( unsigned int i = 0; i < m_phiMin.size(); ++i ) {
      msg() << MSG::DEBUG << "Zone limits; zone " << i << " min " << m_phiMin[i] << " max " << m_phiMax[i] << " phiMin "
            << phiMinZone( i, innerRadius() ) << " max " << phiMaxZone( i, innerRadius() ) << endmsg;
    }
    msg() << MSG::DEBUG << "Radius of first strip is " << m_rStrips[0] << " last strip "
          << m_rStrips[m_rStrips.size() - 1] << endmsg;
    msg() << MSG::DEBUG << "Pitch; inner " << m_rPitch[0] << " outer " << m_rPitch[m_rPitch.size() - 1]
          << " Radius; inner " << m_innerR << " outer " << m_outerR << " slope " << m_pitchSlope << endmsg;
  }
}
//==============================================================================
...
void DeVeloRType::cornerLimits() {
  ...
  if ( m_debug ) {
    msg() << MSG::DEBUG << "Cut off starts at x=" << m_cornerX1 << ",y=" << m_cornerY1
          << " and ends at x=" << m_cornerX2 << ",y=" << m_cornerY2 << " gradient=" << m_gradCutOff
          << " intercept=" << m_intCutOff << endmsg;
  }
}
//==============================================================================
...
void DeVeloRType::intersectCutOff( const double radius, double& x, double& y ) const {
  ...
  if ( B2Minus4AC > 0 ) {
    ...
    if ( m_verbose ) {
      msg() << MSG::VERBOSE << "a=" << QuadA << ",b=" << QuadB << ",c=" << QuadC << " Solution 1: x=" << x1
            << " y=" << y1 << " Solution 2: x=" << x2 << " y=" << y2 << " Chose: x=" << x << " y=" << y << endmsg;
    }
  } else {
...
void DeVeloRType::BuildRoutingLineMap() {
  for ( unsigned int routLine = m_minRoutingLine; routLine <= m_maxRoutingLine / 2; ++routLine ) {
    ...
    if ( m_verbose ) {
      msg() << MSG::VERBOSE << "Routing line " << routLine << " area " << routArea << " strip "
            << m_mapRoutingLineToStrip[routLine] << " +1024 line " << routLine + 1024 << " +1024 strip "
            << m_mapRoutingLineToStrip[routLine + 1024] << endmsg;
      msg() << MSG::VERBOSE << "Routing line " << routLine << " strip " << RoutingLineToStrip( routLine )
            << " chip channel " << RoutingLineToChipChannel( routLine ) << " and back "
            << ChipChannelToRoutingLine( RoutingLineToChipChannel( routLine ) ) << " from strip " << endmsg;
    }
  }
}
//=============================================================================
...
LHCb::CircleTraj DeVeloRType::trajectory( const LHCb::VeloChannelID& id, double offset ) const {
  ...
  if ( !sc ) { throw GaudiException( "The trajectory could not be made", "DeVELO.cpp", StatusCode::FAILURE ); }
...
StatusCode DeVeloRType::updateZoneLimits() {
  for ( unsigned int localZone = 0; localZone < 4; ++localZone ) {
    ...
    auto phiLimits =
        std::array{globalLimitsMin.first.phi(),  globalLimitsMin.second.phi(), globalLimitsMax.first.phi(),
                   globalLimitsMax.second.phi(), globalLimitsMid.first.phi(),  globalLimitsMid.second.phi()};
    ...
    phiLimits = std::array{halfBoxLimitsMin.first.phi(),  halfBoxLimitsMin.second.phi(), halfBoxLimitsMax.first.phi(),
                           halfBoxLimitsMax.second.phi(), halfBoxLimitsMid.first.phi(),  halfBoxLimitsMid.second.phi()};
    // map to [0,2pi] for right hand side sensors
    if ( isRight() ) {
      for ( auto& phiLimit : phiLimits ) {
        if ( phiLimit < 0 ) phiLimit += 2.0 * Gaudi::Units::pi;
...
StatusCode DeVeloRType::distToM2Line( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& vID, double& distToM2,
...
bool DeVeloRType::distToM2Line( double const& x, double const& y, LHCb::VeloChannelID& vID, double& dist ) const {
  double       dist2 = 1.e9;
  ...
  auto iLineMax = lower_bound( iLineMin, iLineMin + m_stripsInZone, std::pair{phi, 0u} );
...
void DeVeloRType::loadM2RoutingLines() {
  ...
  for ( unsigned int strip = 0; strip < 2048; ++strip ) {
    ...
    if ( !exists( param_X ) || !exists( param_Y ) ) {
      msg() << MSG::WARNING << "VELO R sensor M2 routing lines not in conditions DB cond: " << param_X << " and "
            << param_Y << endmsg;
      return;
    }
    m_M2RoutingLines.emplace_back( param<std::vector<double>>( param_X ), param<std::vector<double>>( param_Y ) );
  }
  m_M2RLMinPhi.clear();
  m_M2RLMinPhi.reserve( m_M2RoutingLines.size() );
