...
VeloAlignCond::PositionPaths::PositionPaths( const std::string& _x, const std::string& _y ) {
  ...
  if ( !_x.empty() ) {
    if ( boost::regex_match( _x, what, e ) ) {
      ...
      throw GaudiException( "Bad syntax for location of stepping motor reading (x): " + _x,
  ...
  if ( !_y.empty() ) {
    if ( boost::regex_match( _y, what, e ) ) {
      ...
      throw GaudiException( "Bad syntax for location of stepping motor reading (y): " + _y,
                            "VeloAlignCond::PositionPaths", StatusCode::FAILURE );
    }
  } else { // set values to empty if the string is empty
    y.first  = "";
...
// Main constructor
//-----------------------------------------------------------------------------
VeloAlignCond::VeloAlignCond( const std::vector<double>& translation, const std::vector<double>& rotation,
                              const std::vector<double>& pivot, const std::string& xOffsetLoc,
...
VeloAlignCond::~VeloAlignCond() {
  ...
  if ( m_inUpdMgrSvc ) { m_services->updMgrSvc()->unregister( this ); }
}
...
void VeloAlignCond::i_registerOffsetCond( const PositionPaths::ValueType& offsetCond, Condition*& cond,
  ...
  if ( oldOffsetCond.first != offsetCond.first ) { // do something only if they are not ==
    if ( !oldOffsetCond.first.empty() ) {
      ...
      msg() << MSG::WARNING << "Offset condition changed from '" << oldOffsetCond.first << "[" << oldOffsetCond.second
            << "]"
            << "' to '" << offsetCond.first << "[" << offsetCond.second << "]"
      ...
      m_services->updMgrSvc()->registerCondition( this, offsetCond.first, &VeloAlignCond::makeMatrices, cond );
