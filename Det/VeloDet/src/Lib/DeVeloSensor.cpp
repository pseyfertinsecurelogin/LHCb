...
//==============================================================================
const CLID& DeVeloSensor::clID() const { return DeVeloSensor::classID(); }
//==============================================================================
...
StatusCode DeVeloSensor::initialize() {
  ...
  IDetectorElement* halfBox = this->parentIDetectorElement()->parentIDetectorElement()->parentIDetectorElement();
  if ( m_debug ) msg() << MSG::DEBUG << "Great grandparent of " << this->name() << " is " << halfBox->name() << endmsg;
  m_halfBoxGeom = halfBox->geometry();

  if ( m_debug )
    msg() << MSG::DEBUG << "Module " << m_module << " sensor " << m_sensorNumber << " full type " << m_fullType
          << " z= " << m_z << " R " << isR() << " Phi " << isPhi() << " PU " << isPileUp() << " Left " << m_isLeft
          << " Right " << isRight() << " Downstream " << isDownstream() << endmsg;
...
//=============================================================================
StatusCode DeVeloSensor::channelDistance( const LHCb::VeloChannelID& start, const LHCb::VeloChannelID& end,
                                          int& nOffset ) const {
  nOffset                 = 0;
  ...
  if ( numberOfStrips() < startStrip || numberOfStrips() < endStrip ) { return StatusCode::FAILURE; }
...
void DeVeloSensor::initSensor() {
  ...
  if ( m_type == "R" || m_type == "Phi" || m_type == "Veto" ) {
    ...
    m_fullType     = m_type + ( ( m_isDownstream ) ? "DnStrm" : "UpStrm" ) + ( ( m_isLeft ) ? "Left" : "Right" );
  } else {
    m_isLeft       = ( m_type == "PhiDL" || m_type == "PhiUL" || m_type == "RLeft" || m_type == "VetoL" );
...
//=========================================================================
StatusCode DeVeloSensor::residual( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& dir,
                                   const LHCb::VeloChannelID& channel, const double interStripFraction,
                                   double& residual, double& chi2 ) const {
...
//=========================================================================
StatusCode DeVeloSensor::intersectWithLine( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& dir,
  ...
  if ( std::abs( denom ) < epsilon ) {
    msg() << MSG::ERROR << "Failed to compute intersection. Line parallel to sensor plane." << endmsg;
    return StatusCode::FAILURE;
  }
...
StatusCode DeVeloSensor::registerConditionCallBacks() {
  ...
  updMgrSvc()->registerCondition( this, condition( m_stripNoiseConditionName.c_str() ).path(),
                                  &DeVeloSensor::updateStripNoiseCondition );

  // strip info condition
  updMgrSvc()->registerCondition( this, condition( m_stripInfoConditionName.c_str() ).path(),
                                  &DeVeloSensor::updateStripInfoCondition );

  // readout flag condition
  updMgrSvc()->registerCondition( this, condition( m_readoutConditionName.c_str() ).path(),
                                  &DeVeloSensor::updateReadoutCondition );

  // geometry conditions (just m_z for now)
  updMgrSvc()->registerCondition( this, this->m_geometry, &DeVeloSensor::cacheGeometry );
...
StatusCode DeVeloSensor::updateStripInfoCondition() {
  ...
  const std::vector<int>& tmpStripInfos = m_stripInfoCondition->paramAsIntVect( "StripInfo" );
  m_stripInfos.clear();
  m_stripInfos.resize( tmpStripInfos.size() );
  std::transform( tmpStripInfos.begin(), tmpStripInfos.end(), m_stripInfos.begin(), ConvertIntToStripInfo() );
