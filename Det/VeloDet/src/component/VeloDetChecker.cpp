...
//=============================================================================
VeloDetChecker::VeloDetChecker( const std::string& name, ISvcLocator* pSvcLocator )
    : GaudiTupleAlg( name, pSvcLocator ), m_velo( NULL ) {
...
void VeloDetChecker::TestSensorForwardIterators( std::vector<DeVeloSensor*>::const_iterator sensorsBegin,
  ...
  for ( std::vector<DeVeloSensor*>::const_iterator iSens = sensorsBegin; iSens != sensorsEnd; ++iSens ) {
    debug() << format( "Sensor Number=%d, R=%i, Phi=%i, PU=%i, Left=%i, Right=%i, Down=%i, z=%f",
                       ...
                       ( *iSens )->isLeft(), ( *iSens )->isRight(), ( *iSens )->isDownstream(), ( *iSens )->z() )
            << endmsg;
  }
}
//==============================================================================
...
// Generic method to test contents of sensor containers
//==============================================================================
void VeloDetChecker::TestSensorReverseIterators(
    std::vector<DeVeloSensor*>::const_reverse_iterator sensorsReverseBegin,
    std::vector<DeVeloSensor*>::const_reverse_iterator sensorsReverseEnd ) {
  debug() << "TestSensorReverseIterators" << endmsg;
  for ( std::vector<DeVeloSensor*>::const_reverse_iterator iSens = sensorsReverseBegin; iSens != sensorsReverseEnd;
        ++iSens ) {
    debug() << format( "Sensor Number=%d, R=%i, Phi=%i, PU=%i, Left=%i, Right=%i, Down=%i, z=%f",
                       ( *iSens )->sensorNumber(), ( *iSens )->isR(), ( *iSens )->isPhi(), ( *iSens )->isPileUp(),
                       ( *iSens )->isLeft(), ( *iSens )->isRight(), ( *iSens )->isDownstream(), ( *iSens )->z() )
...
// Generic method to test contents of R sensor containers
//==============================================================================
void VeloDetChecker::TestRSensorReverseIterators(
    std::vector<DeVeloRType*>::const_reverse_iterator rSensorsReverseBegin,
    std::vector<DeVeloRType*>::const_reverse_iterator rSensorsReverseEnd ) {
  debug() << "TestRSensorReverseIterators" << endmsg;
  for ( std::vector<DeVeloRType*>::const_reverse_iterator iSens = rSensorsReverseBegin; iSens != rSensorsReverseEnd;
        ++iSens ) {
    debug() << format( "Sensor Number=%d, R=%i, Phi=%i, PU=%i, Left=%i, Right=%i, Down=%i, z=%f",
                       ( *iSens )->sensorNumber(), ( *iSens )->isR(), ( *iSens )->isPhi(), ( *iSens )->isPileUp(),
                       ( *iSens )->isLeft(), ( *iSens )->isRight(), ( *iSens )->isDownstream(), ( *iSens )->z() )
...
// Generic method to test contents of Phi sensor containers
//==============================================================================
void VeloDetChecker::TestPhiSensorReverseIterators(
    std::vector<DeVeloPhiType*>::const_reverse_iterator phiSensorsReverseBegin,
    std::vector<DeVeloPhiType*>::const_reverse_iterator phiSensorsReverseEnd ) {
  debug() << "TestPhiSensorReverseIterators" << endmsg;
  for ( std::vector<DeVeloPhiType*>::const_reverse_iterator iSens = phiSensorsReverseBegin;
        iSens != phiSensorsReverseEnd; ++iSens ) {
    debug() << format( "Sensor Number=%d, R=%i, Phi=%i, PU=%i, Left=%i, Right=%i, Down=%i, z=%f",
                       ( *iSens )->sensorNumber(), ( *iSens )->isR(), ( *iSens )->isPhi(), ( *iSens )->isPileUp(),
                       ( *iSens )->isLeft(), ( *iSens )->isRight(), ( *iSens )->isDownstream(), ( *iSens )->z() )
            << endmsg;
...
void VeloDetChecker::TestTrajectories() {
  ...
  for ( std::vector<DeVeloSensor*>::const_iterator iSens = m_velo->sensorsBegin(); iSens != m_velo->sensorsEnd();
        ++iSens ) {
...
// and VeloChannelID
//==============================================================================
void VeloDetChecker::PrintTrajectory( const DeVeloSensor& iSens, LHCb::VeloChannelID channel ) {
...
void VeloDetChecker::CheckSensorGeometry() {
  ...
  if ( m_r ) { CheckRSensorGeometry(); }
  if ( m_phi ) { CheckPhiSensorGeometry(); }
  if ( m_r && m_phi ) { CheckAllSensorGeometry(); }
}
//==============================================================================
