...
public:
  ...
  /// and the local pitch.
  StatusCode pointToChannel( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& channel, double& fraction,
                             double& pitch ) const override;

  /// Get the nth nearest neighbour for a given channel
  StatusCode neighbour( const LHCb::VeloChannelID& start, const int& nOffset,
  ...
  StatusCode residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel, double& residual,
                       double& chi2 ) const override;

  /// Residual of a 3-d point to a VeloChannelID + interstrip fraction
  StatusCode residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel,
                       const double interStripFraction, double& residual, double& chi2 ) const override;
  ...
  unsigned int stripsInZone( const unsigned int /*zone*/ ) const override { return m_stripsInZone; }

  /// The minimum radius for a given zone of the sensor
  double rMin( const unsigned int /*zone*/ ) const override { return innerRadius(); }
  ...
  inline double rOfStrip( const unsigned int strip ) const { return m_rStrips[strip]; }
  ...
  inline double rPitch( double radius ) const { return m_innerPitch + m_pitchSlope * ( radius - innerRadius() ); }

  /// Return the radius of the centre of strip plus interstrip fraction*pitch in the global frame
  inline double globalR( unsigned int strip, double isp ) const { return m_globalR[strip] + isp * rPitch( strip ); }

  /// Return the radius of the centre of strip plus interstrip fraction*pitch in the halfbox frame
  inline double halfboxR( unsigned int strip, double isp ) const { return m_halfboxR[strip] + isp * rPitch( strip ); }
  ...
  const std::pair<double, double>& globalPhiRange( unsigned int globalZone ) const {
    return m_globalPhiLimitsZone[globalZone];
  }

  /// The phi range [-pi,pi] of the given global zone in the halfbox frame.
  const std::pair<double, double>& halfboxPhiRange( unsigned int globalZone ) const {
    return m_halfboxPhiLimitsZone[globalZone];
  }

  /// The r range of the given global zone in the global frame.
  const std::pair<double, double>& globalRRange( unsigned int globalZone ) const {
    return m_globalRLimitsZone[globalZone];
  }

  /// The r range of the given global zone in the halfbox frame.
  const std::pair<double, double>& halfboxRRange( unsigned int globalZone ) const {
    return m_halfboxRLimitsZone[globalZone];
  }

  /// The minimum phi of a strip
  double phiMinStrip( unsigned int strip ) const { return m_stripPhiLimits[strip].first; }

  /// The maximum phi of a strip
  double phiMaxStrip( unsigned int strip ) const { return m_stripPhiLimits[strip].second; }

  /// Return the strip limits for panoramix
  inline StatusCode stripLimits( unsigned int strip, double& radius, double& phiMin, double& phiMax ) const {
  ...
  */
  StatusCode distToM2Line( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& vID, double& distToM2Line,
...
private:
  ...
  bool distToM2Line( double const& x, double const& y, LHCb::VeloChannelID& vID, double& dist ) const;
...
public:
  ...
  struct PolyLine {
    ...
    double              m_maxPhi;
    PolyLine( std::vector<double> x, std::vector<double> y ) : m_x( x ), m_y( y ), m_maxPhi( -999. ) {
...
private:
  ...
  inline MsgStream& msg() const {
    std::call_once( m_msgSetFlag, [&] { m_msgStream.reset( new MsgStream( msgSvc(), "DeVeloRType" ) ); } );
    return *m_msgStream;
  }
