...
public:
  ...
  StatusCode pointToChannel( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& channel, double& localOffset,
  ...
  unsigned int zoneOfStrip( const unsigned int strip ) const override { return ( m_nbInner > strip ) ? 0 : 1; }
  ...
  double rMax( const unsigned int zone ) const override;
  ...
  inline double globalDistToOrigin( unsigned int zone ) const { return m_globalDistToOrigin[zone]; }

  /// get the half box frame phi distance to origin
  inline double halfboxDistToOrigin( unsigned int zone ) const { return m_halfboxDistToOrigin[zone]; }

  /// get the ideal frame phi distance to origin
  inline double idealDistToOrigin( unsigned int zone ) const { return m_idealDistToOrigin[zone]; }
  ...
  inline double phiOfStrip( unsigned int strip, double fraction, const double radius ) const {
    double effectiveStrip = fraction + static_cast<double>( strip );
    if ( m_nbInner > strip ) {
  ...
  inline double phiPitch( const double radius ) const {
    return ( m_middleRadius > radius ) ? m_innerPitch * radius : m_outerPitch * radius;
  }

  /// The phi pitch in radians for a given strip
  inline double phiPitch( unsigned int strip ) const { return ( m_nbInner > strip ) ? m_innerPitch : m_outerPitch; }
...
private:
  ...
  inline double safePhiOffset( double const& c0, double const& d0, double const& radius ) const {
  ...
  inline MsgStream& msg() const {
    std::call_once( m_msgSetFlag, [&] { m_msgStream.reset( new MsgStream( msgSvc(), "DeVeloPhiType" ) ); } );
    return *m_msgStream;
  }
