...
public:
  ...
  virtual StatusCode pointToChannel( const Gaudi::XYZPoint& point, LHCb::VeloChannelID& channel, double& localOffset,
                                     double& pitch ) const = 0;

  /// Get the nth nearest neighbour for a given channel
  virtual StatusCode neighbour( const LHCb::VeloChannelID& start, const int& nOffset,
                                LHCb::VeloChannelID& channel ) const = 0;

  /// Returns the number of channels between two channels
  virtual StatusCode channelDistance( const LHCb::VeloChannelID& start, const LHCb::VeloChannelID& end,
                                      int& nOffset ) const;
   ...
   */
  virtual StatusCode residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel, double& residual,
                               double& chi2 ) const = 0;
   ...
   *  actually on the sensor.
   */
  virtual StatusCode residual( const Gaudi::XYZPoint& point, const LHCb::VeloChannelID& channel,
                               const double interStripFraction, double& residual, double& chi2 ) const = 0;
   ...
   */
  StatusCode residual( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& dir, const LHCb::VeloChannelID& channel,
   ...
   */
  StatusCode intersectWithLine( const Gaudi::XYZPoint& point, const Gaudi::XYZVector& dir,
  ...
  unsigned int RoutingLineToStrip( unsigned int routLine ) const { return m_mapRoutingLineToStrip.at( routLine ); }
  /// Convert strip number to routing line
  unsigned int StripToRoutingLine( unsigned int strip ) const { return m_mapStripToRoutingLine.at( strip ); }
   ...
   *   depends on CondDB.
   */
  bool OKStrip( unsigned int strip ) const { return ( strip < m_numberOfStrips && stripInfo( strip ).stripIsOK() ); }
...
public:
  ...
  private:
    enum { LOWGAIN = 2, NOISY = 1, STRIPOK = 0, DEAD = -1, OPEN = -2, PINHOLE = -3, SHORT = -4 };
...
private:
  ...
  inline MsgStream& msg() const {
    std::call_once( m_msgSetFlag, [&] { m_msgStream.reset( new MsgStream( msgSvc(), "DeVeloSensor" ) ); } );
    return *m_msgStream;
  }
