 ...
 */
// ============================================================================
DeCalorimeter::DeCalorimeter( const std::string& name ) : DetectorElement( name ) {}
// ============================================================================
...
StatusCode DeCalorimeter::initialize() {
  ...
  if ( sc.isSuccess() ) {
      ...
      if ( m_caloDet == "EcalDet" || m_caloDet == "HcalDet" ) {
        ...
        if ( UNLIKELY( msg2.level() <= MSG::VERBOSE ) )
          msg2 << MSG::VERBOSE << "Chan : " << id << " [ crate=" << cardCrate( cardNumber( id ) )
               ...
               << "  | actual L0Cte : " << act << " => " << ag << " MeV/ADC" << endmsg;
      }
    }
    // Check gain
    ...
    if ( UNLIKELY( msg3.level() <= MSG::VERBOSE ) ) {
      ...
      long k = 1;
      for ( const auto& ic : m_cells ) {
        msg3 << MSG::VERBOSE << k << " : " << ic.cellID() << " pos : (" << ic.x() << "," << ic.y() << "," << ic.z()
             << ") "
...
Gaudi::Plane3D DeCalorimeter::plane( const double dz ) const {
  if ( fabs( dz - m_zOffset ) > m_zSize / 2. ) {
    ...
    msg << MSG::DEBUG << " THE REQUESTED PLANE IS OUTSIDE THE ACTIVE VOLUME of : " << name() << " dz = " << dz
        << " z-enveloppe of the active area = [" << m_zOffset - m_zSize / 2. << " ; " << m_zOffset + m_zSize / 2.
        << " ] " << endmsg;
  }
...
StatusCode DeCalorimeter::buildCells() {
    ...
    for ( auto& subSubCalo : elem->subSubCalos() ) {
      ...
      for ( int Row = 0; m_maxRowCol >= Row; ++Row ) {
        ...
        for ( int Column = 0; m_maxRowCol >= Column; ++Column ) {
          ...
          if ( 0 > m_cells.index( id ) )
            m_cells.addEntry( CellParam( id ), id ); // store the new cell if it does not already exist
  ...
  for ( auto& cell : m_cells ) {
    ...
    for ( int iColumn = Column - 1; iColumn <= Column + 1; ++iColumn ) {
      if ( iColumn >= 0 && iColumn <= m_maxRowCol ) { // inside calorimeter
        ...
        for ( int iRow = Row - 1; iRow <= Row + 1; ++iRow ) {
          if ( ( iRow >= 0 ) && ( iRow <= m_maxRowCol ) && // inside calorimeter
    ...
    if ( 8 != nNeigh ) {
      ...
      for ( auto pArea = cellSize.begin(); cellSize.end() != pArea; ++pArea ) {
          ...
          if ( ( 0 <= iRow ) && ( m_maxRowCol >= iRow ) ) {
            ...
            for ( int iColumn = cc - 4; iColumn <= cc + 4; ++iColumn ) {
              if ( ( 0 <= iColumn ) && ( m_maxRowCol >= iColumn ) ) {
                ...
                if ( m_cells[id2].valid() ) {
                  if ( ( fabs( m_cells[id2].x() - x ) <= margin ) && ( fabs( m_cells[id2].y() - y ) <= margin ) ) {
                    cell.addNeighbor( id2 );
                  }
                }
              }
...
StatusCode DeCalorimeter::buildCards() {
  ...
  for ( unsigned int kk = 0; kk * aSize < temp.size(); ++kk ) {
    ...
    if ( (int)kk != cardNum ) {
      msg << MSG::ERROR << "FE-Card number not in sequence: Found " << cardNum << " expect " << kk << endmsg;
      return StatusCode::FAILURE;
    }
    ...
    if ( it == maps.end() && mapType != CardParam::None ) {
      msg << MSG::ERROR << "The requested map type = " << mapType << " is not defined in condDB." << endmsg;
      return StatusCode::FAILURE;
    }
    ...
    if ( it != maps.end() ) {
      ...
      if ( cellids.size() != count ) {
        msg << MSG::ERROR << "The mapping with type = " << mapType << " is not consistent with the FEB " << cardNum
            << endmsg;
        return StatusCode::FAILURE;
      }
    }
  ...
  if ( m_readout->exists( "ValidationBoard" ) ) {
    ...
    while ( vb.end() > it ) {
      ...
      while ( 0 < nb-- ) { hcalFe.push_back( *it++ ); }
  ...
  for ( auto& card : m_feCards ) {
    ...
    for ( int row = fRow; lRow >= row; ++row ) {
      for ( int col = fCol; lCol >= col; ++col ) {
        ...
        if ( ( row == fRow ) && ( 0 < fRow ) ) {
          ...
          if ( cardNumber( testID ) >= 0 ) { downCard = cardNumber( testID ); }
        }
        ...
        if ( ( col == fCol ) && ( 0 < col ) && ( m_firstRowUp != col ) ) {
          ...
          if ( ( row == fRow ) && ( 0 < row ) ) {
            ...
            if ( cardNumber( testID2 ) >= 0 ) { cornerCard = cardNumber( testID2 ); }
          }
        }
      }
    }
    ...
    if ( UNLIKELY( msg.level() <= MSG::VERBOSE ) )
      msg << MSG::VERBOSE << " >> boundaries : " << cardFirstValidRow( card.number() ) << " ("
          ...
          << cardFirstColumn( card.number() ) << ")  -> " << cardLastValidColumn( card.number() ) << " ("
...
StatusCode DeCalorimeter::buildTell1s() {
  ...
  while ( temp.end() > it ) {
    ...
    if ( num != m_tell1Boards.size() ) {
      msg << MSG::ERROR << "TELL1 number not in sequence: Found " << num << " expect " << m_tell1Boards.size()
          << endmsg;
      return StatusCode::FAILURE;
    }
    int        nb = ( *it++ );
    ...
    while ( 0 < nb-- ) {
      ...
      if ( m_feCards[feCard].tell1() > 0 ) {
        msg << MSG::ERROR << "**** FE-Card " << feCard << " already linked to Tell1 board " << m_feCards[feCard].tell1()
            << " - Cannot link to Tell1 board " << num << endmsg;
        return StatusCode::FAILURE;
      }
...
StatusCode DeCalorimeter::buildMonitoring() {
  ...
  while ( ll + 1 < (int)temp.size() ) {
    ...
    if ( 0 > iCard ) {
      msg << MSG::ERROR << " No FE-Card defined in crate/slot " << crate << "/" << slot << endmsg;
      return StatusCode::FAILURE;
    }
    LHCb::CaloCellID pinId( m_caloIndex, m_pinArea, m_feCards[iCard].firstRow() + pinRow,
    ...
    for ( int ir = 1; ir != rLed + 1; ir++ ) {
      for ( int ic = 1; ic != cLed + 1; ic++ ) {
        ...
        if ( 0 <= area2 ) {
          if ( 1 != rLed * cLed ) {
            msg << MSG::ERROR << "Don't know how to distributed Leds array on several Calo area" << endmsg;
            return StatusCode::FAILURE;
          }
...
StatusCode DeCalorimeter::getCalibration() {
  ...
  if ( !data.empty() && size > 2 )
    msg << MSG::WARNING << "Reference data are not longer supported within 'Calibration' condition "
    ...
    if ( m_cells[id].valid() ) {
      ...
      if ( UNLIKELY( msg.level() <= MSG::VERBOSE ) ) {
        ...
        msg << MSG::VERBOSE << "    current <PMT> +- RMS from LED signal : " << ledData << " +- " << ledDataRMS
            << endmsg;
        msg << MSG::VERBOSE << "    current <PMT/PIN> +- RMS  from LED signal " << ledMoni << " +- " << ledMoniRMS
            << endmsg;
      }
...
std::ostream& DeCalorimeter::printOut( std::ostream& os ) const {
  ...
  os << std::endl << "\t\tIncrease in Et per radian                  =  ";
  for ( unsigned int i = 0; i < m_nArea; i++ ) { os << " area= " << i << " :: " << m_maxEtSlope[i] << " | "; }
  os << std::endl
     << "\t\tMaximum codage in the ADC                  =  " << m_adcMax << std::endl
     << "\t\tConversion from activeE() to energy seen   =  " << m_activeToTotal << std::endl
     << "\t\tZ of the shower maximum in the local frame =  " << m_zShowerMax << std::endl
     << "\t\tMaximum value for Row/Column               =  " << m_maxRowCol << std::endl
     << "\t\tFirst Row or Column  over center           =  " << m_firstRowUp << std::endl
     << "\t\tCentral Value = m_maxRowCol/2                =  " << m_centerRowCol << std::endl;

  for ( const auto& cell : m_cells ) {
    LHCb::CaloCellID id = cell.cellID();
    os << "Cell " << id << " Neighbors ";
    std::copy( neighborCells( id ).begin(), neighborCells( id ).end(),
...
MsgStream& DeCalorimeter::printOut( MsgStream& os ) const {
  ...
  os << "\t Parameters" << endmsg << "\t\tEt value for maximum ADC value at theta(0) =  " << m_maxEtInCenter << endmsg
     << "\t\tIncrease in Et per radian                  =  " << ( m_maxEtSlope ) << endmsg
     << "\t\tMaximum codage in the ADC                  =  " << ( m_adcMax ) << endmsg
     << "\t\tConversion from activeE() to energy seen   =  " << ( m_activeToTotal ) << endmsg
     << "\t\tZ of the shower maximum in the local frame =  " << ( m_zShowerMax ) << endmsg
     << "\t\tMaximum value for Row/Column               =  " << ( m_maxRowCol ) << endmsg
     << "\t\tFirst Row or Column  over center           =  " << ( m_firstRowUp ) << endmsg
     << "\t\tCentral Value = m_maxRowCol/2                =  " << ( m_centerRowCol ) << endmsg;
...
StatusCode DeCalorimeter::updGain() {
  ...
  m_zSupMeth =
      m_gain->exists( "ZeroSupMethod" ) ? m_gain->paramAsDouble( "ZeroSupMethod" ) : 1.; // 1D 0-sup per default
