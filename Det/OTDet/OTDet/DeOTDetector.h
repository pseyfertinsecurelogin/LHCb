...
namespace DeOTDetectorLocation {
  ...
  inline const std::string Default = "/dd/Structure/LHCb/AfterMagnetRegion/T/OT";
} // namespace DeOTDetectorLocation

constexpr CLID CLID_DeOTDetector = 8101;
...
public:
  ...
  int sensitiveVolumeID( const Gaudi::XYZPoint& aPoint ) const override;
   ...
   * @return distance
   */
  double distanceAlongWire( const LHCb::OTChannelID aChannel, double xHit, double yHit ) const;
   ...
   */
  double propagationTime( const LHCb::OTChannelID aChannel, const double x, const double y ) const;
   ...
   * @return trajectory
   */
  std::unique_ptr<LHCb::Trajectory<double>> trajectoryFirstWire( const LHCb::LHCbID& id, int monolayer = 0 ) const;

  /** Get trajectory representing the most right wire in (second=0) monolayer
   * @return trajectory
   */
  std::unique_ptr<LHCb::Trajectory<double>> trajectoryLastWire( const LHCb::LHCbID& id, int monolayer = 1 ) const;

  /** Get trajectory representing the wire identified by the LHCbID
   * The offset is zero for all OT Trajectories
   * @return trajecory
   */
  std::unique_ptr<LHCb::Trajectory<double>> trajectory( const LHCb::LHCbID& id, double = 0 /** offset */ ) const;
...
public:
  ...
  const Calibration* globalCalibration() const { return m_calibration ? &m_calibration.value() : nullptr; }
...
inline unsigned int DeOTDetector::firstStation() const { return m_firstStation; }

inline unsigned int DeOTDetector::lastStation() const { return m_firstStation + m_stations.size() - 1u; }

inline unsigned int DeOTDetector::nStation() const { return m_stations.size(); }
...
inline double DeOTDetector::resolution() const { return m_resolution; }
...
inline double DeOTDetector::propagationDelay() const { return m_propagationDelay; }
...
inline double DeOTDetector::driftDelay() const { return m_maxDriftTime / m_cellRadius; }

inline double DeOTDetector::deadTime() const { return m_deadTime; }

inline const DeOTDetector::Stations& DeOTDetector::stations() const { return m_stations; }

inline const DeOTDetector::Layers& DeOTDetector::layers() const { return m_layers; }

inline const DeOTDetector::Quarters& DeOTDetector::quarters() const { return m_quarters; }

inline const DeOTDetector::Modules& DeOTDetector::modules() const { return m_modules; }

inline unsigned int DeOTDetector::nChannels() { return m_nChannels; }

inline unsigned int DeOTDetector::nMaxChanInModule() { return m_nMaxChanInModule; }
