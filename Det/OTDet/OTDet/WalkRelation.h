...
namespace OTDet {
  ...
  public:
    ...
    inline float walk_fast_error_estimate() const noexcept { return m_approx.error_estimate(); }
  ...
  inline float WalkRelation::walk_fast( float l ) const noexcept {
    ...
    return UNLIKELY( l > m_upperend ) ? ( m_fdptfactor * l + m_foff ) : m_approx( std::max( 0.f, l ) );
  }
} // namespace OTDet
