...
namespace LHCb {
  ...
  private:
    ...
    public:
      Segment() {}
      Segment( const Point& begin, const Point& end ) : m_begin( begin ) {
        m_dir = ( end - begin ) / ( end.y() - begin.y() );
      }

      const Point& begin() const { return m_begin; }

      Point position( double mu ) const { return m_begin + ( mu - m_begin.y() ) * m_dir; }
      ...
      void applyTranslation( const Vector& translation ) { m_begin += translation; }
    };
  ...
  public:
    ...
    OTWireTrajImp( const std::array<Point, 2 * N>& points, const double beginyrange, const double endyrange )
      ...
      for ( size_t i = 0; i < N; ++i ) m_segments[i] = Segment( points[2 * i], points[2 * i + 1] );
    ...
    std::unique_ptr<OTWireTraj> cloneOTWireTraj() const override { return std::make_unique<OTWireTrajImp<N>>( *this ); }

    Index numSegments() const override { return N; }

  public: // implementation for member functions of the interface
    std::unique_ptr<Trajectory<double>> clone() const override { return std::make_unique<OTWireTrajImp<N>>( *this ); }

    /// trajectory is parameterized along y
    double muEstimate( const Point& p ) const override { return p.y(); }

    /// Point on the trajectory
    Point position( double mu ) const override { return m_segments[index( mu )].position( mu ); }

    Vector direction( double mu ) const override { return m_segments[index( mu )].direction(); }

    Vector curvature( double ) const override { return Vector( 0.0, 0.0, 0.0 ); }
    ...
    double distTo1stError( double, double, int ) const override { return 10 * Gaudi::Units::km; }

    double distTo2ndError( double, double, int ) const override { return 10 * Gaudi::Units::km; }

    /// arclength for given range
    double arclength( double mu1, double mu2 ) const override { return ( mu2 - mu1 ) * m_dndy; }

    /// arclength
    double arclength() const override { return arclength( beginRange(), endRange() ); }
  };
