...
namespace OTDet {
  ...
  public:
    ...
    inline const std::vector<float>& tcoeff() const noexcept { return m_tcoeff; }

    /// return coefficients of polynimial sigma_t(r)
    inline const std::vector<float>& terrcoeff() const noexcept { return m_terrcoeff; }
    ...
    inline float error_estimate( WhichError which ) const noexcept {
      return ( Rt == which ) ? m_rt.error_estimate()
                             : ( Drdt == which ) ? m_drdt.error_estimate() : m_sigmar.error_estimate();
    }
    ...
    inline float drdt() const noexcept { return ( rmax() - rmin() ) / ( tmax() - tmin() ); }
  ...
  inline float RtRelation::drifttime( float radius ) const noexcept {
    return PolyEval::eval( std::min( rmax(), std::max( rmin(), radius ) ) * m_rconv, m_tcoeff );
  }

  inline float RtRelation::dtdr( float radius ) const noexcept {
    return PolyEval::eval( std::min( rmax(), std::max( rmin(), radius ) ) * m_rconv, m_dtdrcoeff );
  }

  inline float RtRelation::drifttimeError( float radius ) const noexcept {
    return PolyEval::eval( std::min( rmax(), std::max( rmin(), radius ) ) * m_rconv, m_terrcoeff );
  }
  ...
  inline DriftTimeWithError RtRelation::drifttimeWithError( float radius ) const noexcept {
    ...
    return DriftTimeWithError( PolyEval::eval( radius, m_tcoeff ), PolyEval::eval( radius, m_terrcoeff ) );
  }
  ...
  inline float RtRelation::extrapolatedradius( float time ) const noexcept {
    return UNLIKELY( time > tmax() )
               ? ( rmax() + m_drdtAtRmax * ( time - tmax() ) )
               : UNLIKELY( time < tmin() ) ? ( rmin() + m_drdtAtRmin * ( time - tmin() ) ) : m_rt( time );
  }
