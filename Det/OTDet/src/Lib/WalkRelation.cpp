...
namespace OTDet {
  ...
  WalkRelation::WalkRelation( double off, double amp, double tau, double dpt )
      ...
      , m_foff( off )
      , m_fdptfactor( ( 0. != tau ) ? ( 1e-3 * dpt ) : 0. )
      , m_approx( 0., 9.1 * std::abs( tau ),
                  [off, amp, tau, dpt]( double l ) {
                    return off + amp * ( std::tanh( ( ( 0. != tau ) ? ( 1. / std::abs( tau ) ) : 0. ) * l ) - 1.0 ) +
                           ( ( 0. != tau ) ? ( 1e-3 * dpt ) : 0. ) * l;
                  } )
      , m_famp( amp )
      , m_fovertau( ( 0. != tau ) ? ( 1. / std::abs( tau ) ) : 0. )
      , m_off( off )
      , m_amp( amp )
      , m_tau( tau )
      , m_dpt( dpt )
  ...
  inline double WalkRelation::fastTanh( const double x ) noexcept {
    // table of tanh
    static const double tantable[101] = {0.0000000000e+00,
                                         4.9958374958e-02,
                                         9.9667994625e-02,
                                         1.4888503362e-01,
                                         1.9737532022e-01,
  ...
  inline double vdt_tanh( const double x ) noexcept {
    ...
    const double exptwox = ( _e > 100 ? 1e43 : _e < -100 ? 0.0 : vdt::fast_exp( _e ) );
