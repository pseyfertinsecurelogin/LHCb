...
namespace OTDet {
  ...
  RtRelation::RtRelation( float rmax, const std::vector<float>& tcoeff, const std::vector<float>& terrcoeff )
      : m_rmax( rmax )
      , m_rconv( 1 / rmax )
      ...
      , m_tmin( drifttime( 0. ) )
      , m_tmax( drifttime( m_rmax ) )
      , m_rt( ChebyshevApprox<float, s_ChebTerms>::inverse( 0.f, rmax,
                                                            [this]( float rr ) { return float( drifttime( rr ) ); } ) )
      , m_drdt( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0.f, rmax, [this]( float rr ) { return 1.f / dtdr( rr ); },
            [this]( float rr ) { return float( drifttime( rr ) ); } ) )
      , m_sigmar( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0.f, rmax, [this]( float rr ) { return float( drifttimeError( rr ) / std::abs( dtdr( rr ) ) ); },
            [this]( float rr ) { return float( drifttime( rr ) ); } ) )
  ...
  RtRelation::RtRelation( float rmax, const std::vector<float>& tcoeff, float sigmaR )
      : m_rmax( rmax )
      , m_rconv( 1 / rmax )
      , m_tcoeff( std::begin( tcoeff ), std::end( tcoeff ) )
      , m_dtdrcoeff( derivative<float, float>( tcoeff, 1 / rmax ) )
      ...
      , m_drdt( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            ...
            [this]( float rr ) { return float( drifttime( rr ) ); } ) )
      , m_sigmar( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0.f, rmax, [sigmaR]( float /* rr */ ) { return sigmaR; },
            [this]( float rr ) { return float( drifttime( rr ) ); } ) )
      , m_drdtAtRmin( 1 / std::abs( dtdr( 0.f ) ) )
      , m_drdtAtRmax( 1 / std::abs( dtdr( m_rmax ) ) ) {
    // "normalise" polynomials by chopping off zero coefficients at the back
  ...
  RtRelation::RtRelation( double rmax, const std::vector<double>& tcoeff, const std::vector<double>& terrcoeff )
      : m_rmax( rmax )
      , m_rconv( 1 / rmax )
      , m_tcoeff( std::begin( tcoeff ), std::end( tcoeff ) )
      ...
      , m_tmin( drifttime( 0. ) )
      , m_tmax( drifttime( m_rmax ) )
      , m_rt( ChebyshevApprox<float, s_ChebTerms>::inverse(
            0., rmax, [this]( double rr ) { return double( drifttime( rr ) ); } ) )
      , m_drdt( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0., rmax, [this]( double rr ) { return 1. / dtdr( rr ); },
            [this]( double rr ) { return double( drifttime( rr ) ); } ) )
      , m_sigmar( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0., rmax, [this]( double rr ) { return double( drifttimeError( rr ) / std::abs( dtdr( rr ) ) ); },
            [this]( double rr ) { return double( drifttime( rr ) ); } ) )
      , m_drdtAtRmin( 1 / std::abs( dtdr( 0.f ) ) )
      , m_drdtAtRmax( 1 / std::abs( dtdr( m_rmax ) ) ) {
    // "normalise" polynomials by chopping off zero coefficients at the back
  ...
  RtRelation::RtRelation( double rmax, const std::vector<double>& tcoeff, double sigmaR )
      : m_rmax( rmax )
      , m_rconv( 1 / rmax )
      , m_tcoeff( std::begin( tcoeff ), std::end( tcoeff ) )
      , m_dtdrcoeff( derivative<double, float>( tcoeff, 1 / rmax ) )
      ...
      , m_drdt( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            ...
            [this]( double rr ) { return double( drifttime( rr ) ); } ) )
      , m_sigmar( ChebyshevApprox<float, s_ChebTerms - 1>::implicit(
            0., rmax, [sigmaR]( double /* rr */ ) { return sigmaR; },
            [this]( double rr ) { return double( drifttime( rr ) ); } ) )
      , m_drdtAtRmin( 1 / std::abs( dtdr( 0.f ) ) )
      , m_drdtAtRmax( 1 / std::abs( dtdr( m_rmax ) ) ) {
    // "normalise" polynomials by chopping off zero coefficients at the back
  ...
  //     : m_tmin(tmin), m_tmax(tmax), m_rtable(radius), m_dt( (tmax-tmin) / (radius.size()-1)), m_rmin(radius.front()),
  //     m_rmax(radius.back())
  //   {
