...
const CLID& DeOTModule::clID() const { return classID(); }
...
StatusCode DeOTModule::initialize() {
  ...
  try {
    if ( msg.level() <= MSG::VERBOSE ) msg << MSG::VERBOSE << "Registering conditions" << endmsg;
    ...
    if ( hasCondition( m_monoalignConditionName ) ) {
      updMgrSvc()->registerCondition( this, condition( m_monoalignConditionName ).path(),
                                      &DeOTModule::monoalignCallback );
    }
    if ( msg.level() <= MSG::VERBOSE ) msg << MSG::VERBOSE << "Start first update of conditions" << endmsg;
    StatusCode sc = updMgrSvc()->update( this );
    if ( !sc.isSuccess() ) {
      msg << MSG::WARNING << "Failed to update detector element " << this->name() << "!" << endmsg;
      return sc;
    }
...
void DeOTModule::findStraws( const Gaudi::XYZPoint& entryPoint, const Gaudi::XYZPoint& exitPoint,
                             Straws& straws ) const {
...
void DeOTModule::calculateHits( const Gaudi::XYZPoint& entryPoint, const Gaudi::XYZPoint& exitPoint,
      ...
      for ( auto straw : straws ) {
        ...
        if ( notParallel ) {
          ...
          if ( isEfficientA( mu.y() ) && std::abs( dist ) < m_cellRadius ) {
            chanAndDist.emplace_back( OTChannelID{m_stationID, m_layerID, m_quarterID, m_moduleID, straw}, dist );
          }
        }
      }
      /// Second monolayer
      ...
      for ( auto straw : straws ) {
        ...
        if ( notParallel ) {
          ...
          if ( isEfficientB( mu.y() ) && std::abs( dist ) < m_cellRadius ) {
            chanAndDist.emplace_back( OTChannelID{m_stationID, m_layerID, m_quarterID, m_moduleID, straw + m_nStraws},
                                      dist );
          }
        }
      }
...
void DeOTModule::sCircle( const double z1, const double u1, const double z2, const double u2, const double z3c,
...
double DeOTModule::distanceToWire( const unsigned int aStraw, const Gaudi::XYZPoint& aPoint, const double tx,
...
void DeOTModule::monoLayerIntersection( int monolayer, const Gaudi::XYZPoint& aPoint, const double tx, const double ty,
  ...
  for ( unsigned char mono = 0; mono < 2; ++mono ) {
    ...
    if ( monoDx.size() == 2 ) {
      ...
      for ( unsigned char iseg = 0; iseg < 3; ++iseg ) {
        ...
        points[2 * iseg + 1] =
            globalPoint( x0 + monosign * monoDx[2 * iseg + 1] / 2, segy0 + ( iseg + 1 ) * segdy, z0 );
      }
...
StatusCode DeOTModule::calibrationCallback() {
  ...
  MsgStream msg( msgSvc(), name() );
  if ( msg.level() <= MSG::DEBUG ) msg << MSG::DEBUG << "Updating Calibration parameters" << endmsg;
  try {
    const std::vector<double>& trParameters     = m_calibration->param<std::vector<double>>( "TRParameters" ); // in ns
    const std::vector<double>& sigmaTParameters = m_calibration->param<std::vector<double>>( "STParameters" ); // in ns
    std::vector<double>        t0Parameters     = m_calibration->param<std::vector<double>>( "TZero" );
    ...
    //    if(m_calibration->exists("PropagationVelocity")) propagationVelocity =
    //    m_calibration->param<double>("PropagationVelocity"); m_propagationVelocity = propagationVelocity;
    ...
    if ( m_calibration->exists( "WalkParameters" ) ) {
      ...
      if ( walkParameters.size() == 4 ) {
        m_walkrelation =
            OTDet::WalkRelation( walkParameters[0], walkParameters[1], walkParameters[2], walkParameters[3] );
      } else {
    ...
    if ( t0Parameters.size() == nChannels() || t0Parameters.size() == MAXNUMCHAN ) // 1 t0 per channel
      ...
      for ( size_t ichan = 0; ichan < nChannels(); ++ichan ) m_strawt0[ichan] = t0Parameters[ichan / 32];
...
StatusCode DeOTModule::statusCallback() {
  ...
  /// Callback will only be registered with ums if condition exists
  MsgStream msg( msgSvc(), name() );
  if ( msg.level() <= MSG::DEBUG ) msg << MSG::DEBUG << "Updating Status parameters" << endmsg;
  try {
    ...
    if ( statusflags.size() == nChannels() || statusflags.size() == MAXNUMCHAN ) // 1 t0 per channel
      ...
      for ( size_t ichan = 0; ichan < nChannels(); ++ichan ) m_strawStatus[ichan] = statusflags[ichan / 32];
  ...
  if ( hasCondition( m_monoalignConditionName ) ) {
    ...
    if ( monoaligncondition->exists( m_monoalignParametersName ) ) {
      ...
      if ( m_monoDx.size() != 1 && m_monoDx.size() != 2 && m_monoDx.size() != 4 && m_monoDx.size() != 6 ) {
        MsgStream msg( msgSvc(), name() );
        msg << MSG::ERROR << "There should be 1, 2, 4 or 6 mono alignment parameters: " << m_monoDx.size()
            << " provided" << endmsg;
      }
    }
  }
...
std::unique_ptr<LHCb::Trajectory<double>> DeOTModule::trajectoryFirstWire( int monolayer ) const {
  /// Default is 0 -> straw 1
  return m_trajFirstWire[monolayer]->cloneOTWireTraj();
}

std::unique_ptr<LHCb::Trajectory<double>> DeOTModule::trajectoryLastWire( int monolayer ) const {
...
std::unique_ptr<LHCb::Trajectory<double>> DeOTModule::trajectory( const OTChannelID& aChan,
                                                                  const double /*offset*/ ) const {
  if ( !contains( aChan ) ) {
    throw GaudiException( "Failed to make trajectory!", "DeOTModule.cpp", StatusCode::FAILURE );
  }
...
StatusCode DeOTModule::setWalkRelation( const OTDet::WalkRelation& walkRelation ) {
  ...
  if ( m_calibration->exists( "WalkParameters" ) )
    m_calibration->param<std::vector<double>>( "WalkParameters" ) = walkParameters;
  else
    m_calibration->addParam( "WalkParameters", walkParameters, "Walk parameters" );
...
StatusCode DeOTModule::setMonoAlignment( const std::vector<double>& pars ) {
  ...
  if ( monoaligncondition->exists( m_monoalignParametersName ) )
    ...
    monoaligncondition->addParam( m_monoalignParametersName, pars, "Monolayer alignment parameters" );
...
void DeOTModule::getMonoAlignment( std::vector<double>& pars ) const { pars = m_monoDx; }
...
void DeOTModule::checkWalkRelation( MsgStream& msg ) const {
  ...
  if ( msg.level() <= MSG::VERBOSE ) {
    msg << MSG::VERBOSE << "Fast walk relation approximation error: " << err << endmsg;
  }
  if ( err > thresh ) {
    msg << MSG::ERROR
        << "Fast walk relation approximation error "
           "larger than one OTIS bin ("
        << err
        << "), please check "
           "your calibration parameters and/or increase Chebyshev order "
           "in Det/OTDet/WalkRelation."
        << endmsg;
  } else if ( err > 0.5 * thresh ) {
    msg << MSG::WARNING
        << "Fast walk relation approximation error "
           "larger than half an OTIS bin ("
        << err
        << "), please check "
           "your calibration parameters and/or increase Chebyshev order "
           "in Det/OTDet/WalkRelation."
...
void DeOTModule::checkRtRelation( MsgStream& msg ) const {
  ...
  const decltype( thresh ) threshs[3] = {thresh, thresh / ( m_rtrelation.rmax() - m_rtrelation.rmin() ), thresh};
  const decltype( m_rtrelation.error_estimate( OTDet::RtRelation::Rt ) ) errs[3] = {
      m_rtrelation.error_estimate( OTDet::RtRelation::Rt ), m_rtrelation.error_estimate( OTDet::RtRelation::Drdt ),
  ...
  for ( unsigned i = 0; i < sizeof( threshs ) / sizeof( threshs[0] ); ++i ) {
    ...
    if ( msg.level() <= MSG::VERBOSE ) {
      msg << MSG::VERBOSE << "Fast " << name << " relation approximation error: " << err << endmsg;
    }
    if ( err > th ) {
