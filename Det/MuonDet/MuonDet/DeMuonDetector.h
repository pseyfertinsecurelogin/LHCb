...
public:
  ...
  StatusCode Hit2ChamberNumber( Gaudi::XYZPoint myPoint, int station, int& chamberNumber, int& regNum ) const;

  StatusCode Hit2GapNumber( Gaudi::XYZPoint myPoint, int station, int& gapNumber, int& chamberNumber,
                            int& regNum ) const;

  StatusCode Pos2ChamberNumber( const double x, const double y, const double z, int& chamberNumber, int& regNum ) const;

  StatusCode Pos2GapNumber( const double x, const double y, const double z, int& gapNumber, int& chamberNumber,
                            int& regNum ) const;

  StatusCode Pos2StChamberNumber( const double x, const double y, int station, int& chamberNumber, int& regNum ) const;

  StatusCode Pos2StGapNumber( const double x, const double y, int station, int& gapNumber, int& chamberNumber,
                              int& regNum ) const;

  StatusCode Pos2ChamberTile( const double x, const double y, const double z, LHCb::MuonTileID& tile ) const;

  StatusCode Pos2StChamberPointer( const double x, const double y, int station, DeMuonChamber*& chamberPointer ) const;

  StatusCode Pos2ChamberPointer( const double x, const double y, const double z, DeMuonChamber*& chamberPointer ) const;

  StatusCode Chamber2Tile( int chaNum, int station, int region, LHCb::MuonTileID& tile ) const;
  ...
  std::vector<std::pair<MuonFrontEndID, std::array<float, 4>>>
             listOfPhysChannels( Gaudi::XYZPoint my_entry, Gaudi::XYZPoint my_exit, int region, int chamber );
  StatusCode getPCCenter( MuonFrontEndID fe, int chamber, int station, int region, double& xcenter, double& ycenter,
                          double& zcenter );
  ...
  DeMuonChamber* getChmbPtr( const int station, const int region, const int chmb ) const;
...
inline bool DeMuonDetector::isM1defined() const { return m_isM1defined; }
...
inline bool DeMuonDetector::testForFilter( const IDetectorElement* station ) const {
  ...
  if ( stationName.find( "/MF" ) != stationName.npos ) return true;
