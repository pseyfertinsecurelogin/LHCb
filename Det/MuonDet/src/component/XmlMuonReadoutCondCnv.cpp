...
protected:
   ...
   */
  StatusCode i_fillSpecificObj( xercesc::DOMElement* childElement, MuonReadoutCond* dataObj,
                                IOpaqueAddress* address ) override;

private:
  /// fills the X and Y cluster sizes and probabilities
  StatusCode setClusterSizes( MuonReadoutCond* dataObj, const int& index, std::string& clSzX, std::string& clPrX,
                              std::string& clSzY, std::string& clPrY );

  /// filles the time jitter PDF
  StatusCode setJitterVector( MuonReadoutCond* dataObj, const int& index, const std::string& jitterMin,
                              const std::string& jitterInc, std::string& jitterValues );
...
// Constructor
// ------------------------------------------------------------------------
XmlMuonReadoutCondCnv::XmlMuonReadoutCondCnv( ISvcLocator* svc ) : XmlUserConditionCnv<MuonReadoutCond>( svc ) {
...
// ------------------------------------------------------------------------
StatusCode XmlMuonReadoutCondCnv::i_fillSpecificObj( xercesc::DOMElement* childElement, MuonReadoutCond* dataObj,
  ...
  */
  ...
  if ( 0 == xercesc::XMLString::compareString( ReadoutString, tagName ) ) {
    const std::string rType      = dom2Std( childElement->getAttribute( ReadoutTypeString ) );
    const std::string eff        = dom2Std( childElement->getAttribute( EfficiencyString ) );
    const std::string syncD      = dom2Std( childElement->getAttribute( SyncDriftString ) );
    const std::string chamNoise  = dom2Std( childElement->getAttribute( ChamberNoiseString ) );
    const std::string elecNoise  = dom2Std( childElement->getAttribute( ElectronicsNoiseString ) );
    const std::string meanDead   = dom2Std( childElement->getAttribute( MeanDeadTimeString ) );
    const std::string rmsDead    = dom2Std( childElement->getAttribute( RMSDeadTimeString ) );
    const std::string timeOffset = dom2Std( childElement->getAttribute( TimeGateStartString ) );

    const std::string padESX   = dom2Std( childElement->getAttribute( PadEdgeSizeXString ) );
    const std::string padESigX = dom2Std( childElement->getAttribute( PadEdgeSigmaXString ) );
    std::string       clSzX    = dom2Std( childElement->getAttribute( ClusterSizeXString ) );
    std::string       clPrX    = dom2Std( childElement->getAttribute( ClusterProbXString ) );

    const std::string padESY   = dom2Std( childElement->getAttribute( PadEdgeSizeYString ) );
    const std::string padESigY = dom2Std( childElement->getAttribute( PadEdgeSigmaYString ) );
    std::string       clSzY    = dom2Std( childElement->getAttribute( ClusterSizeYString ) );
    std::string       clPrY    = dom2Std( childElement->getAttribute( ClusterProbYString ) );

    const std::string jitterMin    = dom2Std( childElement->getAttribute( JitterMinString ) );
    const std::string jitterMax    = dom2Std( childElement->getAttribute( JitterMaxString ) );
    std::string       jitterValues = dom2Std( childElement->getAttribute( JitterValuesString ) );

    if ( !( ( rType == "Anode" ) || ( rType == "Cathode" ) ) ) {
      log << MSG::WARNING << "Readout type claimed to be " << rType << endmsg;
    ...
    log << MSG::WARNING << "Can not interpret specific type :" << dom2Std( tagName ) << endmsg;
  }
  return StatusCode::SUCCESS;
}

StatusCode XmlMuonReadoutCondCnv::setClusterSizes( MuonReadoutCond* dataObj, const int& index, std::string& clSzX,
                                                   std::string& clPrX, std::string& clSzY, std::string& clPrY ) {
  ...
  if ( clProb.size() != clSize.size() ) {
    log << MSG::ERROR << "Found " << clProb.size() << " probabilities and " << clSize.size() << " sizes (X)" << endmsg;
    return StatusCode::FAILURE;
  }
  unsigned int i;
...
StatusCode XmlMuonReadoutCondCnv::setJitterVector( MuonReadoutCond* dataObj, const int& index,
                                                   const std::string& jitterMin, const std::string& jitterMax,
  ...
  dataObj->setTimeJitter( jitterVector, atof( jitterMin.c_str() ), atof( jitterMax.c_str() ), index );
