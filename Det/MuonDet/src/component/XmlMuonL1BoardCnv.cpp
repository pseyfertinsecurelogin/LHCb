...
protected:
   ...
   */
  StatusCode i_fillSpecificObj( xercesc::DOMElement* childElement, MuonL1Board* dataObj,
...
//=============================================================================
XmlMuonL1BoardCnv::XmlMuonL1BoardCnv( ISvcLocator* svc ) : XmlUserConditionCnv<MuonL1Board>( svc ) {
...
StatusCode XmlMuonL1BoardCnv::i_fillSpecificObj( xercesc::DOMElement* childElement, MuonL1Board* dataObj,
                                                 IOpaqueAddress* ) {
  ...
  if ( 0 == xercesc::XMLString::compareString( L1String, tagName ) ) {
    ...
    //          ->getNodeValue());
    const std::string L1Number =
        dom2Std( childElement->getAttributes()->getNamedItem( L1NumberString )->getNodeValue() );
    long              L1NumberValue = atol( L1Number.c_str() );
    const std::string L1Station =
        dom2Std( childElement->getAttributes()->getNamedItem( L1StationString )->getNodeValue() );
    long L1StationValue = atol( L1Station.c_str() );

    const std::string ODENumber =
        dom2Std( childElement->getAttributes()->getNamedItem( ODENumberString )->getNodeValue() );
    long ODENumberValue = atol( ODENumber.c_str() );

    std::string       ODEList = dom2Std( childElement->getAttributes()->getNamedItem( ODEListString )->getNodeValue() );
    ...
    for ( unsigned int i = 0; i < nodeChildren->getLength(); ++i ) {
      ...
      if ( dom2Std( nodeChildren->item( i )->getNodeName() ) == dom2Std( ODEReferenceString ) ) {
        ...
        xercesc::DOMNamedNodeMap* attributes    = nodeChildren->item( i )->getAttributes();
        xercesc::DOMNode*         TSRegionNode  = attributes->getNamedItem( TSLayoutRegionString );
        long                      TSRegionValue = atol( dom2Std( TSRegionNode->getNodeValue() ).c_str() );

        xercesc::DOMNode* TSLayoutXNode = attributes->getNamedItem( TSLayoutXString );
        long              TSXValue      = atol( dom2Std( TSLayoutXNode->getNodeValue() ).c_str() );
...
StatusCode XmlMuonL1BoardCnv::splitList( std::string& stringList, std::vector<long>& vectorList ) {
