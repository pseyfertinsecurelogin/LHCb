...
protected:
   ...
   */
  StatusCode i_fillSpecificObj( xercesc::DOMElement* childElement, MuonStationCabling* dataObj,
...
// ------------------------------------------------------------------------
StatusCode XmlMuonCablingCnv::i_fillSpecificObj( xercesc::DOMElement* childElement, MuonStationCabling* dataObj,
  ...
  if ( 0 == xercesc::XMLString::compareString( CablingString, tagName ) ) {
    ...
    const std::string l1NumberString = dom2Std( childElement->getAttribute( L1NumberString ) );
    ...
    for ( i = 0; i < nodeChildren->getLength(); ++i ) {
      if ( dom2Std( nodeChildren->item( i )->getNodeName() ) == dom2Std( L1ListString ) ) {
        xercesc::DOMNamedNodeMap* attributes    = nodeChildren->item( i )->getAttributes();
        xercesc::DOMNode*         L1Node        = attributes->getNamedItem( L1ListRefString );
...
StatusCode XmlMuonCablingCnv::splitList( std::string& stringList, std::vector<long>& vectorList ) {
