...
protected:
   ...
   */
  StatusCode i_fillSpecificObj( xercesc::DOMElement* childElement, MuonODEBoard* dataObj,
                                IOpaqueAddress* address ) override;

private:
  StatusCode splitList( std::string& stringList, std::vector<long>& vectorList );
...
//=============================================================================
XmlMuonODECnv::XmlMuonODECnv( ISvcLocator* svc ) : XmlUserConditionCnv<MuonODEBoard>( svc ) {
...
StatusCode XmlMuonODECnv::i_fillSpecificObj( xercesc::DOMElement* childElement, MuonODEBoard* dataObj,
                                             IOpaqueAddress* ) {
  ...
  if ( 0 == xercesc::XMLString::compareString( ODEString, tagName ) ) {
    const std::string odeNumberString  = dom2Std( childElement->getAttribute( ODENumberString ) );
    long              ODENumberValue   = atol( odeNumberString.c_str() );
    const std::string odeECSNameString = dom2Std( childElement->getAttribute( ODEECSNameString ) );
    //   long ODEECSNameValue=(odeECSNameString.c_str());
    std::string ODEECSNameValue = ( odeECSNameString.c_str() );

    const std::string regionNumberString = dom2Std( childElement->getAttribute( RegionString ) );
    ...
    const std::string tsLayoutXString   = dom2Std( childElement->getAttribute( TSLayoutXString ) );
    long              tsLayoutXValue    = atol( tsLayoutXString.c_str() );
    const std::string tsLayoutYString   = dom2Std( childElement->getAttribute( TSLayoutYString ) );
    long              tsLayoutYValue    = atol( tsLayoutYString.c_str() );
    const std::string tsNumberString    = dom2Std( childElement->getAttribute( TSNumberString ) );
    long              tsNumberValue     = atol( tsNumberString.c_str() );
    std::string       tsGridXListString = dom2Std( childElement->getAttribute( TSGridXListString ) );
    std::vector<long> tsGridXValue;
    StatusCode        sc = splitList( tsGridXListString, tsGridXValue );
    if ( sc.isFailure() ) return sc;
    std::string       tsGridYListString = dom2Std( childElement->getAttribute( TSGridYListString ) );
    std::vector<long> tsGridYValue;
    sc = splitList( tsGridYListString, tsGridYValue );
    if ( sc.isFailure() ) return sc;
    std::string       tsQuadrantListString = dom2Std( childElement->getAttribute( TSQuadrantListString ) );
    std::vector<long> tsQuadrantValue;
    sc = splitList( tsQuadrantListString, tsQuadrantValue );
    if ( sc.isFailure() ) return sc;
    sc = dataObj->update( tsLayoutXValue, tsLayoutYValue, tsNumberValue, tsGridXValue, tsGridYValue, tsQuadrantValue );
    if ( sc.isFailure() ) return sc;
    ...
    for ( i = 0; i < nodeChildren->getLength(); ++i ) {
      if ( dom2Std( nodeChildren->item( i )->getNodeName() ) == dom2Std( TSMapString ) ) {
        xercesc::DOMNamedNodeMap* attributes    = nodeChildren->item( i )->getAttributes();
        xercesc::DOMNode*         TSNode        = attributes->getNamedItem( TSMapRefString );
...
StatusCode XmlMuonODECnv::splitList( std::string& stringList, std::vector<long>& vectorList ) {
