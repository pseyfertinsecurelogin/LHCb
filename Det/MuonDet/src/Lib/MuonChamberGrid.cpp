...
MuonChamberGrid::listOfPhysChannels( double x_enter, double y_enter, double x_exit, double y_exit ) {
  ...
  if ( debug )
    std::cout << "Returning List of Phys Channels for grid G" << m_number_of_grid
              << " . A Details. Vectors rd1x: " << m_x_pad_rdout1 << " rd1y: " << m_y_pad_rdout1
  ...
  for ( int iRd = 0; iRd < RdN; iRd++ ) {
    ...
    if ( debug )
      std::cout << "Looping on readout n. " << iRd << " whith vct dim nX: " << PhNx << " nY: " << PhNy
                << ". Entry/Exit of hit. x_e: " << x_enter << " y_e: " << y_enter << " x_x:  " << x_exit
                << " y_x: " << y_exit << " " << std::endl;
    ...
    if ( debug )
      std::cout << "Returning List of Phys Channels. TmpCha_entry: " << tmpNxChaEntry
                << " TmpCha_exit: " << tmpNxChaExit << std::endl;
    ...
    if ( debug )
      std::cout << "Returning List of Phys Channels. Cha_entry: " << nxChaEntry << " Cha_exit: " << nxChaExit
                << std::endl;
    ...
    for ( unsigned int Xloop = nxChaEntry; Xloop <= nxChaExit; Xloop++ ) {
      ...
      if ( debug )
        std::cout << "Computing Linear Parameters. Xbeg " << xBegin << "; xEnd: " << xEnd << " Ybeg " << yBegin
                  << "; yEnd: " << yEnd << " int: " << intercept << " slope: " << slopeY << std::endl;
      ...
      if ( debug ) std::cout << "Debugging tmpY_beg: " << tmpYBegin << "; tmpY_end: " << tmpYEnd << std::endl;
      ...
      if ( debug ) std::cout << "Debugging Y_beg: " << nyBegin << "; Y_end: " << nyEnd << std::endl;
      ...
      for ( unsigned int Yloop = nyBegin; Yloop <= nyEnd; Yloop++ ) {
        ...
        if ( nyBegin == nyEnd ) {
          ...
          if ( parallelFlag ) {
            ...
            myX = ( ( retLenght( Yloop, y_rdout ) - intercept ) / slopeY +
                    ( retLenght( Yloop + 1, y_rdout ) - intercept ) / slopeY ) /
                  2;
          }
        ...
        keepTemporary.emplace_back( input, std::array{float( myX - retLenght( Xloop, x_rdout ) ),
        ...
        if ( debug )
          std::cout << "Hit processing.  RT:: " << m_readoutType[iRd] << " ; Xl = "
                    << " " << Xloop << " ; Bd = "
                    << " " << myBoundary[0] << " ; Bd2 = "
                    << " " << myBoundary[1] << " ; Bd3 = "
                    << " " << myBoundary[2] << " ; Bd4 = "
                    << " " << myBoundary[3] << " ; Yl = " << Yloop << "\n";
      }
    }
  }
...
StatusCode MuonChamberGrid::getPCCenter( MuonFrontEndID fe, double& xcenter, double& ycenter ) {
  ...
  if ( chx > 0 ) {
    ...
    xcenter = ( x_rdout[chx] ) / 2;

  if ( chy > 0 ) {
    ycenter = ( y_rdout[chy] + y_rdout[chy - 1] ) / 2;
  } else
    ycenter = ( y_rdout[chy] ) / 2;
