...
MuonReadoutCond::MuonReadoutCond() : Condition(), m_RList() {}

/// Copy constructor
MuonReadoutCond::MuonReadoutCond( MuonReadoutCond& obj )
    : IValidity(), Condition( (Condition&)obj ), m_RList( obj.getRList() ) {}
...
StatusCode MuonReadoutCond::initialize() {
  ...
  for ( iRout = m_RList.begin(); iRout != m_RList.end(); iRout++ ) {
    ...
    for ( probIt = iRout->ClusterX.begin(); probIt != iRout->ClusterX.end(); ++probIt ) {
      iRout->CumProbX[( *probIt ).clSize - 1] = ( *probIt ).clProb;
    }

    double                        rsum = 0.;
    std::vector<double>::iterator cumIt;
    for ( cumIt = iRout->CumProbX.begin(); cumIt != iRout->CumProbX.end(); ++cumIt ) {
      rsum += *cumIt;
      *cumIt = rsum;
    }

    // alright paranoia here, normalise the cumlative sum
    for ( cumIt = iRout->CumProbX.begin(); cumIt != iRout->CumProbX.end(); ++cumIt ) { *cumIt /= rsum; }
    ...
    for ( probIt = iRout->ClusterY.begin(); probIt != iRout->ClusterY.end(); ++probIt ) {
      iRout->CumProbY[( *probIt ).clSize - 1] = ( *probIt ).clProb;
    }

    rsum = 0.;
    for ( cumIt = iRout->CumProbY.begin(); cumIt != iRout->CumProbY.end(); ++cumIt ) {
      rsum += *cumIt;
      *cumIt = rsum;
    }

    // alright paranoia here, normalise the cumlative sum
    for ( cumIt = iRout->CumProbY.begin(); cumIt != iRout->CumProbY.end(); ++cumIt ) { *cumIt /= rsum; }
  }
...
int MuonReadoutCond::maxCluster( const _readoutParameter& readout, const char& xy ) {
  ...
  for ( it = ( *cluster ).begin(); it != ( *cluster ).end(); ++it ) {
    if ( max < ( *it ).clSize ) { max = ( *it ).clSize; }
  }
  return max;
}

/// Return the size of cluster corresponding to a random number (0->1) in X
int MuonReadoutCond::singleGapClusterX( const double& randomNumber, const double& xDistPadEdge, const int& i ) {
...
int MuonReadoutCond::singleGapClusterY( const double& randomNumber, const double& yDistPadEdge, const int& i ) {
...
int MuonReadoutCond::singleGapCluster( const int& xy, const double& randomNumber, const double& xpos, const int& i ) {
...
void MuonReadoutCond::addClusterX( const int& size, const double& prob, const int& i ) {
  _clus tmp;
  tmp.clSize = size;
  tmp.clProb = prob;
...
void MuonReadoutCond::addClusterY( const int& size, const double& prob, const int& i ) {
  _clus tmp;
  tmp.clSize = size;
  tmp.clProb = prob;
...
void MuonReadoutCond::setTimeJitter( const std::vector<double>& jitterVec, const double& min, const double& max,
                                     const int& i ) {
