...
IPVolume* DeMuonChamber::getFirstGasGap() {
  ...
  for ( auto pvIterator = geometry()->lvolume()->pvBegin(); pvIterator != geometry()->lvolume()->pvEnd();
        ++pvIterator ) {
    ...
    if ( boost::ends_with( geoCh->name(), "/lvGasGap" ) ) {
      ...
      for ( auto pvGapIterator = ( geoCh->pvBegin() ); pvGapIterator != ( geoCh->pvEnd() ); ++pvGapIterator ) {
        if ( !( ( *pvGapIterator )->lvolume()->sdName().empty() ) ) { return ( *pvGapIterator ); }
      }
    }
  }
  return nullptr;
...
StatusCode DeMuonChamber::isPointInGasGap( Gaudi::XYZPoint pointInChamber, Gaudi::XYZPoint& pointInGap, int& number,
                                           IPVolume*& gasVolume ) {
...
StatusCode DeMuonChamber::isPointInGasGap( Gaudi::XYZPoint pointInChamber, Gaudi::XYZPoint& pointInGap,
                                           IPVolume*& gasVolume ) {
  int number = 0;
