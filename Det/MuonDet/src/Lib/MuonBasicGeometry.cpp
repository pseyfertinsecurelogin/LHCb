...
namespace {
  std::string findName( const std::string& allname, const std::string& /*rootName*/ ) {
...
//=============================================================================
MuonBasicGeometry::MuonBasicGeometry( IDataProviderSvc* detSvc, IMessageSvc* msgSvc ) {
  ...
  for ( auto itStation = muonSystem->childBegin(); itStation != muonSystem->childEnd(); itStation++ ) {
    ...
    if ( !muStation ) { log << MSG::ERROR << "Could not read station" << ( *itStation )->name() << endmsg; }
...
int MuonBasicGeometry::getStations() { return m_stationNumber; }

int         MuonBasicGeometry::getRegions() { return m_regionNumber; }
int         MuonBasicGeometry::getPartitions() { return m_partition; }
std::string MuonBasicGeometry::getStationName( int station ) { return ( m_stations.at( station ) ).first; }
...
int MuonBasicGeometry::retrieveM1status() { return m_isM1defined; }
