...
//=============================================================================
MuonChamberLayout::MuonChamberLayout() : m_msgSvc( NULL ), m_detSvc( NULL ), m_isM1defined( false ) {
...
MuonChamberLayout::MuonChamberLayout( MuonLayout R1, MuonLayout R2, MuonLayout R3, MuonLayout R4,
...
StatusCode MuonChamberLayout::initialize() {
  ...
  if ( sc.isFailure() ) {
    if ( debug ) msgStream() << MSG::ERROR << "Failure to initialize DetectorElement" << endmsg;
    return sc;
  }

  SmartDataPtr<DeMuonDetector> MuonDe( m_detSvc, DeMuonLocation::Default );
  m_isM1defined = MuonDe->isM1defined();
  if ( debug ) msgStream() << MSG::INFO << "Retrieved M1 definition status: " << m_isM1defined << endmsg;
...
void MuonChamberLayout::Copy( MuonChamberLayout& lay ) {
  ...
  for ( int iL = 0; iL < 4; iL++ ) {
    if ( debug ) { msgStream() << MSG::DEBUG << "My Copied layout: " << iL << endmsg; }
...
std::vector<int> MuonChamberLayout::neighborChambers( float myX, float myY, int stat, int x_direction,
  ...
  if ( debug ) {
    msgStream() << MSG::DEBUG << "Position in the grid " << sC_idX << " " << sC_idY << " " << reg << " " << x_direction
                << " " << y_direction << endmsg;
  }
  ...
  for ( int ix = 0; ix <= scX; ix++ ) {
    for ( int iy = 0; iy <= scY; iy++ ) {
      ...
      if ( ix || iy ) { chamberXY( sC_idX, sC_idY, ix * x_direction, iy * y_direction, reg, myChambers ); }
    }
  }
...
void MuonChamberLayout::returnChambers( int sta, float st_x, float st_y, int x_dir, int y_dir, std::vector<int>& regs,
  ...
  for ( const auto& idTile : neighborChambers( st_x, st_y, sta, x_dir, y_dir ) ) {
    ...
    if ( cTile > 0 ) {
      ...
      if ( debug ) {
        msgStream() << MSG::DEBUG << "Returned chambers:: " << sta + 1 << " " << region + 1 << " "
                    << cTile - m_offSet.at( region ) << endmsg;
      }

      regs.push_back( region );
      chs.push_back( cTile - m_offSet.at( region ) - 1 );
    }
  }
  if ( debug ) { msgStream() << MSG::DEBUG << "Exiting from chamber creation " << endmsg; }
}

void MuonChamberLayout::chamberXY( int sx, int sy, int shx, int shy, int reg, std::vector<int>& chamberNumber ) const {
  ...
  if ( fx < 0 || fy < 0 || fx >= 4 * (int)m_cgX.at( reg ) || fy >= 4 * (int)m_cgY.at( reg ) ) {
    ...
    if ( reg + 1 < 4 ) {
      ...
      if ( debug ) { msgStream() << MSG::INFO << " Chamber not Found (>3) " << chN << endmsg; }
    ...
    if ( vSize > 0 ) {
      if ( debug ) {
        msgStream() << MSG::INFO << " Encode and chamber in first call: " << chN
                    << " :: " << chamberNumber.at( vSize - 1 ) << " " << chamberNumber.size() << " " << reg + 1
                    << endmsg;
      }
    ...
    if ( debug ) {
      msgStream() << MSG::INFO << " Encode and chamber in first call: " << chN << " " << enc << " " << reg << " " << fx
                  << " " << fy << endmsg;
    }
  }
  ...
  if ( chN < 0 && shouldLowReg( fx, fy, reg ) ) {
    ...
    if ( debug ) msgStream() << MSG::INFO << " chamber number is <0  , REGION : " << reg << endmsg;

    if ( reg - 1 >= 0 ) {
      // When dropping down the region number
      if ( reg < 3 ) {
        if ( fx < 2 * (int)m_cgX.at( reg ) ) {
          shx = -1;
        } else {
          shx = 1;
        }
      ...
      shy = m_cgY.at( reg );
      if ( debug ) {
        msgStream() << MSG::DEBUG << " Subcall 2: " << fx << " " << fy << " " << shx << " " << -shy << " " << reg - 1
                    << endmsg;
      }
      ...
      if ( debug ) { msgStream() << MSG::INFO << " Chamber not Found (<0) " << chN << endmsg; }
    ...
    if ( vSize > 0 ) {
      if ( debug ) {
        msgStream() << MSG::INFO << " Encode and chamber in second call: " << chN
                    << " :: " << chamberNumber.at( vSize - 1 ) << " " << chamberNumber.size() << " " << endmsg;
      }

      if ( chamberNumber.at( vSize - 1 ) > -1 ) { chN = chamberNumber.at( vSize - 1 ); }
    }
    ...
    if ( reg < 3 ) {
      if ( ( sy / m_cgY.at( reg ) < 1 ) || ( sy / m_cgY.at( reg ) > 2 ) ) {
        if ( reg - 1 > 0 ) {
          if ( debug ) { msgStream() << MSG::INFO << " Third call " << endmsg; }
          chamberXY( fx, fy, 0, 0 - m_cgY.at( reg ), reg - 1, chamberNumber );
        } else {
          if ( debug ) { msgStream() << MSG::INFO << " Chamber not Found " << endmsg; }
    ...
    if ( debug ) {
      msgStream() << MSG::INFO << "Closing Chamber " << chN << " in R" << reg << " xIndex " << fx << " yIndex " << fy
                  << endmsg;
    }
...
void MuonChamberLayout::chamberMostLikely( float x, float y, int station, int& chmb, int& reg ) const {
...
LHCb::MuonTileID MuonChamberLayout::tileChamber( DeMuonChamber* chmb ) const {
  ...
  if ( debug ) { msgStream() << MSG::INFO << "TileChamber:: " << m_cgX.at( reg ) << " " << m_cgY.at( reg ) << endmsg; }
    ...
    for ( fy = 0; fy < 4 * m_cgY.at( reg ); fy++ ) {
      ...
      if ( chN == mychN ) {
        ...
        if ( fx >= m_cgX.at( reg ) * 2 ) {
          myQuarter = 1;
          if ( fy >= m_cgY.at( reg ) * 2 ) { myQuarter = 0; }
        } else {
          myQuarter = 2;
          if ( fy >= m_cgY.at( reg ) * 2 ) { myQuarter = 3; }
        }

        if ( debug ) {
          msgStream() << MSG::INFO << "X, Y " << fx << " " << fy << " for chamber " << chN << " " << reg << endmsg;
        }
        ...
        if ( debug ) {
          msgStream() << MSG::INFO << "X, Y and Q " << myTile.nX() << " " << myTile.nY() << " " << myQuarter
...
LHCb::MuonTileID MuonChamberLayout::tileChamberNumber( int sta, int reg, int chmbNum ) const {
  ...
  if ( deChmb ) {
    ...
    msgStream() << MSG::INFO << " Could not find the chamber " << chmbNum << " in the TES. Check the configuration!"
                << endmsg;
  }
  return myTile;
}

void MuonChamberLayout::gridPosition( float x, float y, int iS, int& idx, int& idy, int& reg ) const {
  ...
  rY    = y - ys * myIdy;
  if ( debug ) {
    msgStream() << MSG::INFO << "Rest: " << rX << " " << rY << " " << xs << " " << ys << " " << x << " " << y << endmsg;
  }

  for ( int iRx = 0; iRx < 4; iRx++ ) {
    if ( debug ) {
      msgStream() << MSG::INFO << "Idx in loop grid: " << myIdx + 2 * (int)m_cgX.at( iRx ) << " "
                  << myIdy + 2 * (int)m_cgY.at( iRx ) << " for region: " << iRx << endmsg;
    }
  ...
  if ( myReg > -1 ) {
    ...
    reg = myReg;
    if ( debug ) {
      msgStream() << MSG::INFO << "Final Idxs grid: " << idx << " " << idy << " for region: " << myReg << endmsg;
    }
  }
}
...
void MuonChamberLayout::setGridStep() {
  ...
  for ( int iDum = ( m_isM1defined ? 0 : 1 ); iDum < 5; iDum++ ) {
    m_xS.push_back( dimX[iDum] );
    m_yS.push_back( dimY[iDum] );
  }
}
...
std::vector<DeMuonChamber*> MuonChamberLayout::fillChambersVector( IDataProviderSvc* detSvc ) {
  ...
  m_logHorizGridX.resize( 20 );
  m_logHorizGridY.resize( 20 );
  m_logVertGridX.resize( 20 );
  m_logVertGridY.resize( 20 );
  m_padGridX.resize( 20 );
  m_padGridY.resize( 20 );
  for ( int ir = 0; ir < 20; ir++ ) {
    m_logHorizGridX.at( ir ) = m_logHorizGridY.at( ir ) = m_logVertGridX.at( ir ) = m_logVertGridY.at( ir ) =
  ...
  for ( itSt = muonSys->childBegin(); itSt < muonSys->childEnd(); itSt++ ) {
    ...
    if ( debug ) { msgStream() << MSG::INFO << "Station Name: " << ( *itSt )->name() << " ::  " << obtIS << endmsg; }
    ...
    for ( itSide = ( *itSt )->childBegin(); itSide < ( *itSt )->childEnd(); itSide++ ) {
      ...
      for ( itRg = ( *itSide )->childBegin(); itRg < ( *itSide )->childEnd(); itRg++ ) {
        ...
        if ( debug ) { msgStream() << MSG::INFO << "Region Name: " << ( *itRg )->name() << " ::  " << obtIR << endmsg; }

        while ( iR != obtIR - 1 ) {
          msgStream() << MSG::INFO << "There is/are void regions. " << endmsg;
          for ( int ich = 0 + iSide * MaxRegions[iR] / 2; ich < ( iSide + 1 ) * MaxRegions[iR] / 2; ich++ ) {
       ...
       x <---+----+----o----+----+
          ...
          */
          ...
          if ( debug ) {
            msgStream() << MSG::INFO << "Chamber initialization: " << enc << " " << deChmb->chamberNumber() << " "
                        << idx << " " << idy << " " << m_cgX.at( reg ) << " " << m_offSet.at( reg ) << " " << myX << " "
                        << myY << endmsg;
          }
          ...
          if ( vIdx != myvIdx ) {
            ...
            if ( sc.isFailure() ) {
              msgStream() << MSG::INFO << "Failed to fill the system grid for chamber " << deChmb->chamberNumber()
  ...
  if ( debug ) { msgStream() << MSG::INFO << "Filled chamber vector of size: " << encode << endmsg; }
...
StatusCode MuonChamberLayout::Tile2XYZpos( const LHCb::MuonTileID& tile, double& x, double& deltax, double& y,
                                           double& deltay, double& z, double& deltaz ) {
  ...
  if ( m_debug ) {
    msgStream() << MSG::INFO << "Tile det: " << tile.layout().xGrid() << ", " << tile.layout().yGrid() << " in station "
                << station << " and region " << region << ";Grid details. Reg " << m_cgX.at( region ) << ", "
                << m_cgY.at( region ) << " ; Pad " << m_padGridX[station * 4 + region] << ", "
                << m_padGridY[station * 4 + region] << " ; LogH " << m_logHorizGridX[station * 4 + region] << ", "
                << m_logHorizGridY[station * 4 + region] << " ; LogV " << m_logVertGridX[station * 4 + region] << ", "
  ...
  if ( m_cgX.at( region ) == tile.layout().xGrid() && m_cgY.at( region ) == tile.layout().yGrid() ) {
    ...
    if ( m_debug ) { msgStream() << MSG::DEBUG << "Found a tile laying out pads" << endmsg; }
    ...
    if ( m_debug ) { msgStream() << MSG::DEBUG << "Found a tile laying out horizontal logical channels" << endmsg; }
    ...
    if ( m_debug ) { msgStream() << MSG::DEBUG << "Found a tile laying out vertical logical channels" << endmsg; }
    ...
    if ( m_debug ) { msgStream() << MSG::DEBUG << "Found a tile laying out Twelfths" << endmsg; }
    ...
    msgStream() << MSG::ERROR << "Did not understand the LHCb::MuonTileID encoding"
                << " xGrid=" << tile.layout().xGrid() << " yGrid=" << tile.layout().yGrid() << endmsg;
    return StatusCode::FAILURE;
  }

  if ( m_debug )
    msgStream() << MSG::DEBUG << "Output " << x << " " << deltax << " " << y << " " << deltay << " " << z << " "
                << deltaz << endmsg;

  return StatusCode::SUCCESS;
}

StatusCode MuonChamberLayout::fillSystemGrids( DeMuonChamber* deChmb, int vIdx, int reg ) {
  ...
  if ( debug ) {
    ...
    msgStream() << MSG::INFO << "Stat/Region " << stat << "/" << reg << " Grid " << deChmb->getGridName()
                << "  data:: " << grX / m_cgX.at( reg ) << " " << grY / m_cgY.at( reg ) << " " << SgrX / m_cgX.at( reg )
                << " " << SgrY / m_cgY.at( reg ) << " ; Map:: " << xm0 << " " << ym0 << " " << xm1 << " " << ym1
                << endmsg;
  }
...
StatusCode MuonChamberLayout::getXYZChamberTile( const LHCb::MuonTileID& tile, double& x, double& deltax, double& y,
                                                 double& deltay, double& z, double& deltaz, bool toGlob ) {
  ...
  if ( !sc.isSuccess() ) { msgStream() << MSG::ERROR << "Failed to get chamber " << chamberNum << endmsg; }

  return StatusCode::SUCCESS;
}

StatusCode MuonChamberLayout::getXYZChamber( const int& station, const int& region, const int& chamberNum, double& x,
                                             double& deltax, double& y, double& deltay, double& z, double& deltaz,
                                             bool toGlob ) {
  StatusCode sc = getXYZ( station, region, chamberNum, -1, x, deltax, y, deltay, z, deltaz, toGlob );
  return sc;
}

StatusCode MuonChamberLayout::getXYZ( const int& station, const int& region, const int& chamberNum, const int& gapNum,
                                      double& x, double& deltax, double& y, double& deltay, double& z, double& deltaz,
                                      bool toGlob ) {

  int            encode, re( 0 );
  DeMuonChamber* muChamber;
  encode            = 276 * station + chamberNum;
  int MaxRegions[4] = {12, 24, 48, 192};
  if ( region ) {
    re = region;
    while ( re >= 1 ) {
      encode += MaxRegions[re - 1];
      re--;
    }
  }
  ...
  if ( -1 == gapNum ) {
    ...
    Gaudi::XYZVector  vtrans;
    Gaudi::Rotation3D vrota;
    ...
    if ( !box ) {
      msgStream() << MSG::ERROR << "Could not cast gas gap solid to box" << endmsg;
      return StatusCode::FAILURE;
    }
    ...
    if ( !gap ) {
      ...
      msgStream() << MSG::ERROR << "Could not read gas gaps  from TDS" << endmsg;
      return StatusCode::FAILURE;
    }
...
StatusCode MuonChamberLayout::getXYZPad( const LHCb::MuonTileID& tile, double& x, double& deltax, double& y,
                                         double& deltay, double& z, double& deltaz ) {
  ...
  if ( m_debug ) {
    msgStream() << MSG::DEBUG << "Pad xOffset within chamber:" << xOffset << " yOffset within chamber:" << yOffset
                << " xratio: " << xRatio << " yratio: " << yRatio << endmsg;
  }
  ...
  if ( !sc.isSuccess() ) {
    msgStream() << MSG::ERROR << "Failed to get chamber for pad tile: " << tile << endmsg;
    return sc;
  }

  // now make the calculations of x,y and z (and dx, dy and dz)
  // in chamber reference frame.
  x = ( cx - cDeltax ) + ( ( static_cast<double>( xOffset ) / static_cast<double>( xRatio ) ) * ( 2.0 * cDeltax ) );
  y = ( cy - cDeltay ) + ( ( static_cast<double>( yOffset ) / static_cast<double>( yRatio ) ) * ( 2.0 * cDeltay ) );

  if ( m_debug )
    msgStream() << MSG::DEBUG << " getXYZPad:: XY tem info  " << xOffset << " " << yOffset << " " << cx << " "
                << cDeltax << " " << cy << " " << cDeltay << " " << x << " " << y << endmsg;
  ...
  if ( m_debug )
    msgStream() << MSG::DEBUG << " getXYZPad:: XY info  " << x << " " << Dx << " " << y << " " << Dy << endmsg;
  ...
  if ( m_debug )
    msgStream() << MSG::DEBUG << " getXYZPad:: loc to glob " << Dx << " " << Dy << " " << Dz << " "
                << myChs.at( 0 )->name() << " " << deltax << " " << deltay << " " << deltaz << endmsg;

  return StatusCode::SUCCESS;
}

StatusCode MuonChamberLayout::getXYZLogical( const LHCb::MuonTileID& tile, double& x, double& deltax, double& y,
                                             double& deltay, double& z, double& deltaz ) {
  ...
  if ( tile.layout().xGrid() >= m_layout[region].xGrid() && tile.layout().yGrid() >= m_layout[region].yGrid() ) {
  ...
  } else if ( tile.layout().xGrid() >= m_layout[region].xGrid() && tile.layout().yGrid() < m_layout[region].yGrid() ) {
    ...
    if ( m_debug ) {
      msgStream() << MSG::DEBUG << "Making " << nTile << " temporary LHCb::MuonTileIDs"
                  << " to get all chamber locations" << endmsg;
    }
    ...
    msgStream() << MSG::WARNING << "You requested a logical channel wider than a chamber, failing" << endmsg;
    return StatusCode::FAILURE;
  }
...
StatusCode MuonChamberLayout::getXYZTwelfth( const LHCb::MuonTileID& tile, double& x, double& deltax, double& y,
                                             double& deltay, double& z, double& deltaz ) {
  unsigned int station = tile.station();
  unsigned int region  = tile.region();
  ...
  for ( chamCorner = 0; chamCorner < 4; chamCorner++ ) {
    ...
    // get the chamber
    StatusCode sc = getXYZChamber( station, region, nChamber, x, deltax, y, deltay, z, deltaz, true );
    if ( !sc.isSuccess() ) {
...
int MuonChamberLayout::getChamberNumber( const LHCb::MuonTileID& tile ) {
  ...
  // Look inside the grid
  if ( m_debug )
    msgStream() << MSG::DEBUG << " getChamberNumber:: Bef corr.   " << tile.region() << " " << tile.nX() << " "
                << tile.nY() << " " << m_cgX.at( tile.region() ) << " " << m_offSet.at( tile.region() ) << endmsg;
  int fx( 0 ), fy( 0 );
  ...
  if ( m_debug ) {
    msgStream() << MSG::DEBUG << " getChamberNumber:: chNum  " << chamberNumber << " " << enc << " " << fx << " " << fy
                << " " << m_cgX.at( tile.region() ) << " " << m_offSet.at( tile.region() ) << endmsg;

    msgStream() << MSG::DEBUG << "Chamber " << chamberNumber << " in R" << tile.region() + 1 << " xIndex " << tile.nX()
                << " yIndex " << tile.nY() << " was found for tile " << tile << endmsg;
  }
  if ( chamberNumber > 1380 ) {
    msgStream() << MSG::ERROR << "Chamber " << chamberNumber << " in R" << tile.region() + 1 << " xIndex " << tile.nX()
                << " yIndex " << tile.nY() << " was found for tile " << tile << endmsg;
    chamberNumber = 0;
  }
...
int MuonChamberLayout::getTwelfthCorner( const int& region, const int& twelfth, const int& chamberNum ) {
  int xPos = 0;
  int yPos = 0;
...
void MuonChamberLayout::getTwelfthCornerIndex( const int& region, const int& twelfth, const int& chamberNum, int& xPos,
                                               int& yPos ) {
...
void MuonChamberLayout::localToglobal( const IGeometryInfo* gInfo, const Gaudi::XYZPoint& cent,
                                       const Gaudi::XYZPoint& corn, double& dx, double& dy, double& dz ) {
...
std::vector<DeMuonChamber*> MuonChamberLayout::createChambersFromTile( std::vector<LHCb::MuonTileID> mytiles ) {
  ...
  for ( const auto& idTile : mytiles ) {
    ...
    if ( cTile > 0 ) {
      if ( debug )
        msgStream() << MSG::DEBUG << "Returned chambers:: " << station << " " << region << " " << chamb << endmsg;
...
std::vector<unsigned int> MuonChamberLayout::Tile2ChamberNum( const LHCb::MuonTileID& tile ) {
  ...
  if ( m_debug )
    msgStream() << MSG::DEBUG << "Tile det: " << tile.layout().xGrid() << ", " << tile.layout().yGrid()
                << " in station " << station << " and region " << region << ";Grid details. Reg " << m_cgX.at( region )
                << ", " << m_cgY.at( region ) << " ; Pad " << m_padGridX[station * 4 + region] << ", "
                << m_padGridY[station * 4 + region] << " ; LogH " << m_logHorizGridX[station * 4 + region] << ", "
                << m_logHorizGridY[station * 4 + region] << " ; LogV " << m_logVertGridX[station * 4 + region] << ", "
                << m_logVertGridY[station * 4 + region] << endmsg;
  ...
  if ( m_cgX.at( region ) == tile.layout().xGrid() && m_cgY.at( region ) == tile.layout().yGrid() ) {
    ...
    if ( m_debug ) msgStream() << MSG::DEBUG << "Found a tile covering a chamber" << endmsg;
...
std::vector<unsigned int> MuonChamberLayout::Logical2ChamberNum( const LHCb::MuonTileID& tile ) {
  ...
  if ( tile.layout().xGrid() >= m_layout[region].xGrid() && tile.layout().yGrid() >= m_layout[region].yGrid() ) {
    ...
    for ( i = 0; i < nTile; i++ ) {
      ...
      LHCb::MuonTileID tTile( station, tempPadLayout, region, tile.quarter(), tile.nX(), yTile );
      if ( m_debug ) {
        msgStream() << MSG::DEBUG << "i: " << i << "  nX: " << tile.nX() << "  nY: " << yTile << endmsg;
      }
      tempTiles.push_back( tTile );
    }
...
std::vector<unsigned int> MuonChamberLayout::Twelfth2ChamberNum( const LHCb::MuonTileID& tile ) {
  unsigned int              region  = tile.region();
  unsigned int              quarter = tile.quarter();
  std::vector<unsigned int> chamberVect;
