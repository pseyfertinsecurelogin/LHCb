...
StatusCode DeMuonDetector::Hit2GapNumber( Gaudi::XYZPoint myPoint, int station, int& gapNumber, int& chamberNumber,
                                          int& regNum ) const {
  ...
  StatusCode scGap     = theChmb->isPointInGasGap( myPointInChFrame, pointInGap, gapNumber, gasVolume );
  if ( scGap.isSuccess() ) {
    isIn = true;
    sc   = StatusCode::SUCCESS;
  }

  if ( !isIn ) { msgStream() << MSG::ERROR << "Gap not found! " << endmsg; }
  return sc;
}

StatusCode DeMuonDetector::Hit2ChamberNumber( Gaudi::XYZPoint myPoint, int station, int& chamberNumber,
                                              int& regNum ) const {
  ...
  if ( regNum >= 0 && chamberNumber >= 0 ) {
    ...
    if ( isIn ) {
      ...
      if ( debug ) std::cout << "Starting point::  " << station << " " << chamberNumber << " " << regNum << std::endl;
      ...
      if ( debug )
        std::cout << "Hit not found Try to look in corner " << x_ref << " " << x << " " << x_sgn << " " << y_ref << " "
                  << y << " " << y_sgn << std::endl;

      std::vector<int> regs;
      std::vector<int> myChams;
      m_chamberLayout->returnChambers( station, (float)x_ref, (float)y_ref, x_sgn, y_sgn, regs, myChams );
      ...
      for ( auto aChamber = myChams.begin(); aChamber < myChams.end(); aChamber++ ) {
        ...
        if ( geoOthChm ) {
          ...
          msgStream() << MSG::WARNING << "Could not find Geometry info of a given chamber!" << endmsg;
        }
      }
      ...
      if ( debug ) {
        ...
        std::cout << "Chmb ret.  : ";
        for ( const auto& aChamber : myChams ) { std::cout << aChamber << " "; }
        std::cout << " " << std::endl;
      }
    }
  }
...
StatusCode DeMuonDetector::Pos2StChamberNumber( const double x, const double y, int station, int& chamberNumber,
                                                int& regNum ) const {
...
StatusCode DeMuonDetector::Pos2StGapNumber( const double x, const double y, int station, int& gapNumber,
...
StatusCode DeMuonDetector::Pos2StChamberPointer( const double x, const double y, int station,
  ...
  if ( ( regNum > -1 ) && ( chamberNumber > -1 ) ) { chamberPointer = getChmbPtr( station, regNum, chamberNumber ); }
  return sc;
}

StatusCode DeMuonDetector::Pos2ChamberPointer( const double x, const double y, const double z,
                                               DeMuonChamber*& chamberPointer ) const {
  // Dummy conversion z <-> station
  return Pos2StChamberPointer( x, y, getStation( z ), chamberPointer );
}

StatusCode DeMuonDetector::Pos2ChamberNumber( const double x, const double y, const double z, int& chamberNumber,
                                              int& regNum ) const {
...
StatusCode DeMuonDetector::Pos2GapNumber( const double x, const double y, const double z, int& gapNumber,
                                          int& chamberNumber, int& regNum ) const {
...
StatusCode DeMuonDetector::Pos2ChamberTile( const double x, const double y, const double z,
...
void DeMuonDetector::CountDetEls() {
  ...
  for ( auto itSt = this->childBegin(); itSt < this->childEnd(); itSt++ ) {
    ...
    auto mreg              = std::count_if( ( *itSide )->childBegin(), ( *itSide )->childEnd(),
                               []( const IDetectorElement* rg ) -> bool { return rg; } );
...
DeMuonChamber* DeMuonDetector::getChmbPtr( const int station, const int region, const int chmb ) const {
  DeMuonChamber* myPtr;
  int            encode, re( 0 );
  ...
  if ( !myPtr )
    msgStream() << MSG::WARNING
                << "DeMuonDetector::getChmbPtr"
                   "is returning NULL pointer"
                << endmsg;
...
DeMuonDetector::listOfPhysChannels( Gaudi::XYZPoint my_entry, Gaudi::XYZPoint my_exit, int region, int chamber ) {

  // Pair vector;
  std::vector<std::pair<MuonFrontEndID, std::array<float, 4>>> tmpPair;
  ...
  if ( dx && dy ) {
    ...
    msgStream() << MSG::ERROR << "Null chamber dimensions. Returning a void list." << endmsg;
    return tmpPair;
  }
  ...
  std::vector<std::pair<MuonFrontEndID, std::array<float, 4>>> myPair;
  if ( theGrid ) {
    // Gets list of channels

    for ( const auto& tmpPair : theGrid->listOfPhysChannels( mod_xen, mod_yen, mod_xex, mod_yex ) ) {
      auto myFE  = tmpPair.first;
      auto myVec = tmpPair.second;
      myFE.setLayer( gapCnt / 2 );

      for ( int iDm = 0; iDm < 4; iDm++ ) {
        myVec[iDm] = iDm % 2 ? myVec[iDm] * 2 * (float)dy : myVec[iDm] * 2 * (float)dx;
        // Added resolution effect
        if ( fabs( myVec.at( iDm ) ) < 0.0001 ) myVec.at( iDm ) = 0;
      }
      myPair.emplace_back( myFE, myVec );
    }
  } else {
    msgStream() << MSG::ERROR << "No grid found. Returning a void list." << endmsg;
    return tmpPair;
  }
  return myPair;
}
...
StatusCode DeMuonDetector::Chamber2Tile( int chaNum, int station, int region, LHCb::MuonTileID& tile ) const {
  // Convert chamber number into a tile
    ...
    for ( auto itSide = ( *itSt )->childBegin(); itSide < ( *itSt )->childEnd(); itSide++ ) { // loop over sides
      ...
      for ( auto itRg = ( *itSide )->childBegin(); itRg < ( *itSide )->childEnd(); itRg++ ) { // loop over regions
        ...
        for ( auto pvIterator = ( ( ( ( *itCh )->geometry() )->lvolume() )->pvBegin() );
              pvIterator != ( ( ( ( *itCh )->geometry() )->lvolume() )->pvEnd() ); pvIterator++ ) {
        ...
        if ( nreadout == 1 ) {
          for ( int i = 0; i < maps; i++ ) {
            ...
            //              if(debug)msgStream()<<MSG::INFO<<" red and maps "<<i<<" "<<
        ...
        //        }
        int chamber = 0;
        for ( itCh = ( *itRg )->childBegin(); itCh < ( *itRg )->childEnd(); itCh++ ) { chamber++; }
  ...
  for ( itSt = this->childBegin(); itSt < this->childEnd(); itSt++ ) {
    ...
    for ( unsigned int nx = 0; nx < layoutInner.xGrid(); nx++ ) {
      for ( int qua = 0; qua < 4; qua++ ) {
        LHCb::MuonTileID chTile( station, layoutInner, 0, qua, nx, layoutInner.yGrid() );
        ...
        if ( debug ) msgStream() << MSG::ERROR << myGapVol->toMother( Gaudi::XYZPoint( -dx, -dy, 0 ) ) << endmsg;

        if ( debug )
          msgStream() << MSG::ERROR << geoCh->toGlobal( myGapVol->toMother( Gaudi::XYZPoint( -dx, -dy, 0 ) ) )
                      << endmsg;
    ...
    for ( unsigned int ny = 0; ny < layoutInner.yGrid(); ny++ ) {
      for ( int qua = 0; qua < 4; qua++ ) {
        LHCb::MuonTileID chTile( station, layoutInner, 0, qua, layoutInner.xGrid(), ny );
        int              chNumber = m_chamberLayout->getChamberNumber( chTile );
        DeMuonChamber*   chP      = getChmbPtr( station, 0, chNumber );
        IPVolume*        myGapVol = chP->getGasGapLayer( 0 );
    ...
    for ( unsigned int nx = 0; nx < 2 * layoutOuter.xGrid(); nx++ ) {
      for ( int qua = 0; qua < 4; qua++ ) {
        ...
        LHCb::MuonTileID chTile( station, layoutOuter, 3, qua, nx, 2 * layoutOuter.yGrid() - 1 );
        int              chNumber = m_chamberLayout->getChamberNumber( chTile );
    ...
    for ( unsigned int ny = 0; ny < 2 * layoutOuter.yGrid(); ny++ ) {
      for ( int qua = 0; qua < 4; qua++ ) {
        LHCb::MuonTileID chTile( station, layoutOuter, 3, qua, 2 * layoutOuter.xGrid() - 1, ny );
        int              chNumber = m_chamberLayout->getChamberNumber( chTile );
        DeMuonChamber*   chP      = getChmbPtr( station, 3, chNumber );
        IPVolume*        myGapVol = chP->getGasGapLayer( 0 );
...
int DeMuonDetector::sensitiveVolumeID( const Gaudi::XYZPoint& myPoint ) const {
  ...
  nqua = tile.quarter();
  // pack the integer:
  int id = ( nqua << PackMCMuonHit::shiftQuadrantID ) | ( nsta << PackMCMuonHit::shiftStationID ) |
...
DetectorElement* DeMuonDetector::Tile2Station( LHCb::MuonTileID aTile ) const {
  ...
  if ( !station ) { msgStream() << MSG::ERROR << "Error in retrieving DetectorElement!" << endmsg; }
  return station;
}
...
std::vector<DeMuonChamber*> DeMuonDetector::Tile2Chamber( LHCb::MuonTileID aTile ) const {
  ...
  std::transform( chamberVector.begin(), chamberVector.end(), ChmbPtr.begin(),
                  [this, istat = aTile.station(), ireg = aTile.region()]( unsigned int chamNum ) {
                    return this->getChmbPtr( istat, ireg, chamNum );
                  } );
  return ChmbPtr;
}
...
DeMuonChamber* DeMuonDetector::Hit2Chamber( Gaudi::XYZPoint myPoint ) {
  ...
  StatusCode sc = this->Pos2StChamberPointer( myPoint.x(), myPoint.y(), istat, ChmbPtr );
