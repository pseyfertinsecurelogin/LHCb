 ...
 */
// ============================================================================
DetDesc::MaterialBudget::MaterialBudget( const std::string& name, ISvcLocator* svc )
  ...
  declareProperty( "ShootingPoint", m_vertex, "Position of shooting vertex" );
  declareProperty( "Shots", m_shots, "Number of random short per event" );
  declareProperty( "zPlane", m_z, "Z-position of reference plane" );
  declareProperty( "xMax", m_xMax, "Maximal X at reference plane" );
  declareProperty( "xMin", m_xMin, "Minimal X at reference plane" );
  declareProperty( "yMax", m_yMax, "Maximal Y at reference plane" );
  declareProperty( "yMin", m_yMin, "Minimal Y at reference plane" );
...
StatusCode DetDesc::MaterialBudget::execute() {
  ...
  if ( m_grid ) {
    return makeGridShots();
  } else if ( m_psrap ) {
    return makePsrapShots();
  }
...
StatusCode DetDesc::MaterialBudget::makeRandomShots() {
  ...
  for ( int shot = 0; shot < m_shots; ++shot, ++progress ) {
    ...
    const double dist = m_trSvc->distanceInRadUnits_r( m_vertex, point, accelCache );
