 ...
 */
// ============================================================================
DetDesc::VolumeCheck::VolumeCheck( const std::string& name, ISvcLocator* svcloc )
...
StatusCode DetDesc::VolumeCheck::initialize() {
  ...
  Assert( randSvc() != nullptr, "randSvc() points to NULL!" );
  Assert( detSvc() != nullptr, "detSvc()  points to NULL!" );
  ...
  if ( !m_volume->pvolumes().empty() ) {
    info() << " Has " << m_volume->pvolumes().size() << " daughter volumes " << endmsg;
    std::size_t i = 0;
    for ( auto pv = m_volume->pvBegin(); m_volume->pvEnd() != pv; ++pv ) {
      info() << " **** Daughter Physical Volume #" << i++ << endmsg;
      info() << *pv << endmsg;
    }
  }
...
StatusCode DetDesc::VolumeCheck::execute() {
  // spherical (3d) shots
  {
    ...
    for ( int shot = 0; shot < m_shotsSphere; ++shot ) {
      ...
      const double radLength = std::accumulate( intersections.begin(), intersections.end(), 0.0,
                                                VolumeIntersectionIntervals::AccumulateIntersections() );

      plot2D( vect.phi() / Gaudi::Units::degree, vect.theta() / Gaudi::Units::degree, 1,
              " 3D-Material Budget     for " + m_volumeName, -180.0, 180.0, 0.0, 180.0, 50, 50, radLength );

      plot2D( vect.phi() / Gaudi::Units::degree, vect.theta() / Gaudi::Units::degree, 2,
              " 3D-Material Budget (N) for " + m_volumeName, -180.0, 180.0, 0.0, 180.0, 50, 50 );
  ...
  // xy - shots
  {
    ...
    for ( int shot = 0; shot < m_shotsXY; ++shot ) {
      ...
      plot2D( point.x(), point.y(), 3, " XY-Material Budget     for " + m_volumeName, m_minx, m_maxx, m_miny, m_maxy,
              50, 50, radLength );

      plot2D( point.x(), point.y(), 4, " XY-Material Budget (N) for " + m_volumeName, m_minx, m_maxx, m_miny, m_maxy,
              50, 50 );
  ...
  // yz - shots
  {
    ...
    for ( int shot = 0; shot < m_shotsYZ; ++shot ) {
      ...
      plot2D( point.y(), point.z(), 5, " YZ-Material Budget     for " + m_volumeName, m_miny, m_maxy, m_minz, m_maxz,
              50, 50, radLength );

      plot2D( point.y(), point.z(), 6, " YZ-Material Budget (N) for " + m_volumeName, m_miny, m_maxy, m_minz, m_maxz,
              50, 50 );
  ...
  // zx - shoots
  {
    ...
    for ( int shot = 0; shot < m_shotsZX; ++shot ) {
      ...
      plot2D( point.y(), point.z(), 7, " ZX-Material Budget     for " + m_volumeName, m_minz, m_maxz, m_minx, m_maxx,
              50, 50, radLength );

      plot2D( point.y(), point.z(), 8, " ZX-Material Budget (N) for " + m_volumeName, m_minz, m_maxz, m_minx, m_maxx,
              50, 50 );
