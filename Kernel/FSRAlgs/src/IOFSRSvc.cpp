...
StatusCode IOFSRSvc::mergeIOFSRs()
{
  ...
  debug() << "My Maps Before Merging: prov " << m_provenanceMap.size()
  ...
  debug() << "My Maps Before Leading Info: prov " << m_provenanceMap.size()
      << ", seen " << m_seenByMap.size() << ", read " << m_readFromMap.size()
      << ", write " << m_writtenToMap.size() << endmsg;
  ...
  debug() << "My Maps After Leading Info: prov " << m_provenanceMap.size()
      << ", seen " << m_seenByMap.size() << ", read " << m_readFromMap.size()
      << ", write " << m_writtenToMap.size() << endmsg;
...
StatusCode IOFSRSvc::cleanTopFSR()
{
  ...
  if (sc.isSuccess()) debug() << "Removed existing FSR at " << LHCb::IOFSRLocation::Default << endmsg;
  else debug() << "No IOFSR to remove from " << LHCb::IOFSRLocation::Default << endmsg;
  return sc;
