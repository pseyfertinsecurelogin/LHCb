...
//=============================================================================
LumiMergeFSR::LumiMergeFSR( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  declareProperty( "EventCountFSRName",
                   m_EventCountFSRName = "/EventCountFSR" ); // LHCb::EventCountFSRLocation::Default );
...
StatusCode LumiMergeFSR::initialize() {
  ...
  if ( m_subtractBXTypes.empty() ) { m_subtractBXTypes = {"Beam1", "Beam2", "NoBeam"}; }
  // ensure consistency
...
StatusCode LumiMergeFSR::merge() {
  ...
  auto               tsAddresses  = m_navigatorTool->navigate( fileRecordRoot, m_TimeSpanFSRName );
