      ...
      if ( status.isSuccess() ) {
        ...
        for ( DataObject* obj : *sel ) {
          try {
            ...
            MsgStream         log( msgSvc(), name() );
            const std::string loc = ( obj->registry() ? obj->registry()->identifier() : "UnRegistered" );
            log << MSG::FATAL << "std::exception during createRep for '" << loc << "' "
                << System::typeinfoName( typeid( *obj ) ) << endmsg;
        ...
        for ( DataObject* obj : *sel ) {
          try {
            ...
            MsgStream         log( msgSvc(), name() );
            const std::string loc = ( obj->registry() ? obj->registry()->identifier() : "UnRegistered" );
            log << MSG::FATAL << "std::exception during fillRepRefs for '" << loc << "'"
                << System::typeinfoName( typeid( *obj ) ) << endmsg;
            log << MSG::FATAL << excpt.what() << endmsg;
            throw;
          }
...
StatusCode LHCbOutputStream::collectObjects() {
  ...
  for ( const auto& item : m_itemList ) {
    ...
    if ( StatusCode iret = m_pDataProvider->retrieveObject( m_currentItem->path(), obj ); iret.isSuccess() ) {
      ...
      log << MSG::ERROR << "Cannot write mandatory object(s) (Not found) " << m_currentItem->path() << endmsg;
      status = iret;
    }
  }
  ...
  for ( const auto& item : m_optItemList ) {
    ...
    if ( iret.isSuccess() ) { iret = m_pDataManager->traverseSubTree( obj, bind( &LHCbOutputStream::collect, this ) ); }
    if ( !iret.isSuccess() ) {
      ON_DEBUG
      log << MSG::DEBUG << "Ignore request to write non-mandatory object(s) " << m_currentItem->path() << endmsg;
    }
  }
...
void LHCbOutputStream::clearSelection() { m_objects.erase( m_objects.begin(), m_objects.end() ); }

// Find single item identified by its path (exact match)
DataStoreItem* LHCbOutputStream::findItem( const std::string& path ) {
  auto matchesPath = [&]( const DataStoreItem* dsi ) { return dsi->path() == path; };
  auto i           = std::find_if( m_itemList.begin(), m_itemList.end(), matchesPath );
  if ( i != m_itemList.end() ) return *i;
  i = std::find_if( m_optItemList.begin(), m_optItemList.end(), matchesPath );
...
void LHCbOutputStream::addItem( Items& itms, const std::string& descriptor ) {
  ...
  ON_DEBUG
  log << MSG::DEBUG << "Adding LHCbOutputStream item " << item->path() << " with " << item->depth() << " level(s)."
      << endmsg;
  itms.push_back( item );
}
...
StatusCode LHCbOutputStream::connectConversionSvc() {
  ...
  if ( dbType.length() == 0 && svc.length() == 0 ) {
    log << MSG::FATAL << "Unable to locate IConversionSvc interface (Unknown technology) " << endmsg
...
Algorithm* LHCbOutputStream::decodeAlgorithm( const std::string& theName ) {
  ...
  if ( !theAlgorithm ) {
    MsgStream log( msgSvc(), name() );
    log << MSG::WARNING << "Failed to decode Algorithm name " << theName << endmsg;
  }

  return theAlgorithm;
}

StatusCode LHCbOutputStream::decodeAlgorithms( StringArrayProperty& theNames, std::vector<Algorithm*>* theAlgs ) {
