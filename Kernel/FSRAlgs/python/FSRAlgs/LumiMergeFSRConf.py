...
def _ext(name):
    return path.splitext(name)[-1].lstrip('.')
...
class LumiMergeFSRConf(LHCbConfigurableUser):
    ...
    __slots__ = {
        "InputType":
        "DST"  # Data type, can be ['MDF','DST'], works only on DST types
        ,
        "LumiSequencer":
        None  # The sequencer to add the Lumi Accounting to - essential input
        ,
        "outputFile":
        ''  # output filename
        ,
        "Persistency":
        None  # change the default persistency for testing
    }
    ...
    def _configureOutput(self):
        ...
        IOHelper(persistency, persistency).outStream(
            filename=outputFile, writer=dstWriter)
