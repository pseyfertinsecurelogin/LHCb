...
// ============================================================================
StatusCode Decays::Nodes::Any::validate( const LHCb::IParticlePropertySvc* /*svc */ ) const {
  return StatusCode( StatusCode::SUCCESS, true );
}
// ============================================================================
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Nodes::Any::operator()( const LHCb::ParticleID& /* p */ ) const { return true; }
// ============================================================================
...
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::JSpin::validate( const LHCb::IParticlePropertySvc* /*svc */ ) const {
  return valid() ? StatusCode( StatusCode::SUCCESS, true ) : StatusCode( InvalidSpin, true );
}
// ============================================================================
...
// ============================================================================
Decays::Nodes::Pid::Pid( const LHCb::ParticleProperty* item ) : Decays::iNode(), m_item( item ) {}
// ============================================================================
// constructor from ParticleID
// ============================================================================
Decays::Nodes::Pid::Pid( const LHCb::ParticleID& item ) : Decays::iNode(), m_item( item ) {}
// ============================================================================
...
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::Pid::validate( const LHCb::IParticlePropertySvc* svc ) const {
...
// constructor from Particle Property
// ============================================================================
Decays::Nodes::CC::CC( const LHCb::ParticleProperty* item ) : Decays::Nodes::Pid( item ) {}
// ============================================================================
// constructor from ParticleID
// ============================================================================
Decays::Nodes::CC::CC( const LHCb::ParticleID& item ) : Decays::Nodes::Pid( item ) {}
// ============================================================================
// constructor from Particle name
// ============================================================================
...
// ============================================================================
Decays::Nodes::HasQuark::HasQuark( LHCb::ParticleID::Quark quark ) : Decays::Nodes::Any(), m_quark( quark ) {}
// ============================================================================
...
// ============================================================================
Decays::Nodes::CTau::CTau( const double low, const double high, const LHCb::IParticlePropertySvc* svc )
    : Decays::iNode(), m_ppSvc(), m_accepted(), m_rejected(), m_low( low ), m_high( high ) {
  if ( 0 != svc ) { m_ppSvc = const_cast<LHCb::IParticlePropertySvc*>( svc ); }
}
// ============================================================================
// MANDATORY: clone method ("virtual constructor")
// ============================================================================
...
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::CTau::validate( const LHCb::IParticlePropertySvc* svc ) const { return setService( svc ); }
// ============================================================================
// MANDATORY: the only one essential method
// ============================================================================
...
void Decays::Nodes::CTau::addToAccepted( const LHCb::ParticleID& pid ) const {
  PIDs::iterator ifind = std::lower_bound( m_accepted.begin(), m_accepted.end(), pid );
  if ( m_accepted.end() == ifind || pid < ( *ifind ) ) { m_accepted.insert( ifind, pid ); }
}
// ============================================================================
void Decays::Nodes::CTau::addToRejected( const LHCb::ParticleID& pid ) const {
  PIDs::iterator ifind = std::lower_bound( m_rejected.begin(), m_rejected.end(), pid );
  if ( m_rejected.end() == ifind || pid < ( *ifind ) ) { m_rejected.insert( ifind, pid ); }
}
// ============================================================================
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::CTau::setService( const LHCb::IParticlePropertySvc* svc ) const {
  ...
  //
  return StatusCode( m_ppSvc.isValid() ? StatusCode::SUCCESS : StatusCode::FAILURE, true );
}
// ============================================================================
// constructor from c-tau range & Service
// ============================================================================
Decays::Nodes::Mass::Mass( const double low, const double high, const LHCb::IParticlePropertySvc* svc )
...
// ===========================================================================
Decays::Nodes::ShortLived_::ShortLived_( const double high, const LHCb::IParticlePropertySvc* svc )
    : Decays::Nodes::CTau( -1 * Gaudi::Units::km, high, svc ) {}
// ===========================================================================
// constructor with high edge and service
// ===========================================================================
Decays::Nodes::ShortLived_::ShortLived_( const LHCb::IParticlePropertySvc* svc )
...
std::ostream& Decays::Nodes::ShortLived_::fillStream( std::ostream& s ) const {
  if ( LHCb::Math::lomont_compare_double( s_SHORTLIVED, high(), 1000 ) ) { return s << " ShortLived "; }
...
// constructor with high edge and service
// ===========================================================================
Decays::Nodes::LongLived_::LongLived_( const double low, const LHCb::IParticlePropertySvc* svc )
    : Decays::Nodes::CTau( low, s_INFINITY, svc ) {}
// ===========================================================================
// constructor with high edge and service
// ===========================================================================
Decays::Nodes::LongLived_::LongLived_( const LHCb::IParticlePropertySvc* svc )
...
std::ostream& Decays::Nodes::LongLived_::fillStream( std::ostream& s ) const {
  if ( LHCb::Math::lomont_compare_double( s_LONGLIVED, low(), 1000 ) ) { return s << " LongLived "; }
...
// ===========================================================================
Decays::Nodes::Stable::Stable( const LHCb::IParticlePropertySvc* svc ) : Decays::Nodes::LongLived_( s_STABLE, svc ) {}
// ===========================================================================
// MANDATORY: clone method ("virtual constructor")
// ===========================================================================
...
// constructor with the service
// ===========================================================================
Decays::Nodes::StableCharged::StableCharged( const LHCb::IParticlePropertySvc* svc ) : Decays::Nodes::Stable( svc ) {}
// ===========================================================================
// MANDATORY: clone method ("virtual constructor")
// ===========================================================================
...
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Nodes::StableCharged::operator()( const LHCb::ParticleID& pid ) const {
  /// check for accepted pids:
  if ( binary_search( accepted(), pid ) ) { return true; } // RETURN
  /// check for rejected pids:
  if ( binary_search( rejected(), pid ) ) { return false; } // RETURN
...
// constructor with high edge and service
// ===========================================================================
Decays::Nodes::Light::Light( const double high, const LHCb::IParticlePropertySvc* svc )
...
// constructor with high edge and service
// ===========================================================================
Decays::Nodes::Heavy::Heavy( const double low, const LHCb::IParticlePropertySvc* svc )
...
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::PosID::validate( const LHCb::IParticlePropertySvc* /*svc */ ) const {
  return StatusCode( StatusCode::SUCCESS, true );
}
// ============================================================================
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Nodes::PosID::operator()( const LHCb::ParticleID& p ) const { return 0 < p.pid(); }
// ============================================================================
// MANDATORY: the specific printout
// ===========================================================================
...
// MANDATORY: the proper validation of the node
// ============================================================================
StatusCode Decays::Nodes::NegID::validate( const LHCb::IParticlePropertySvc* /*svc */ ) const {
  return StatusCode( StatusCode::SUCCESS, true );
}
// ============================================================================
// MANDATORY: the only one essential method
// ============================================================================
bool Decays::Nodes::NegID::operator()( const LHCb::ParticleID& p ) const { return 0 > p.pid(); }
// ============================================================================
// MANDATORY: the specific printout
// ===========================================================================
...
// MANDATORY: the only one essential method
// ===========================================================================
bool Decays::Nodes::Symbol::operator()( const LHCb::ParticleID& pid ) const { return m_symbol.node( pid ); }
// ===========================================================================
// MANDATORY: the specific printout
// ===========================================================================
...
// MANDATORY: validate
// ===========================================================================
StatusCode Decays::Nodes::Symbol::validate( const LHCb::IParticlePropertySvc* svc ) const {
