...
for t in [
    ...
    for i in range(0, 10):
        ...
        if not equal(i1, i2):
            print 'Integrals   1/2 are not equal: %s %g %g %g' % (
                _t.__name__, delta(i1, i2), i1, i2)
        if not equal(i1, i3):
            print 'Integrals   1/3 are not equal: %s %g %g %g' % (
                _t.__name__, delta(i1, i3), i1, i3)
        if not equal(d1, d2):
            print 'Derivatives 1/2 are not equal: %s %g %g %g' % (
                _t.__name__, delta(d1, d2), d1, d2)
        if not equal(d1, d3):
            print 'Derivatives 1/3 are not equal: %s %g %g %g' % (
                _t.__name__, delta(d1, d3), d1, d3)
