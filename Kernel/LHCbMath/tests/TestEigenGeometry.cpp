...
public:
  DataROOT() {
    ...
    static std::uniform_real_distribution<double> r_emiss_x( -800, 800 ), r_emiss_y( -600, 600 ),
...
public:
  ...
  EigenXYZPoint  CoC;
  float          radius;
  DataEigen( const DataROOT& d ) : position( d.position ), direction( d.direction ), CoC( d.CoC ), radius( d.radius ) {}
};
...
bool reflectSpherical_ROOT( Gaudi::XYZPoint& position, Gaudi::XYZVector& direction, const Gaudi::XYZPoint& CoC,
                            const float radius ) {
...
bool reflectSpherical_Eigen( EigenXYZPoint& position, EigenXYZVector& direction, const EigenXYZPoint& CoC,
                             const float radius ) {
  // find intersection point
  // for line sphere intersection look at http://www.realtimerendering.com/int/
...
float testEigenDotProduct( const EigenXYZVector& v ) { return v.dot( v ); }

float testROOTDotProduct( const Gaudi::XYZVector& v ) { return v.Dot( v ); }
...
int main( int /*argc*/, char** /*argv*/ ) {
  ...
  // test dot product
  for ( const auto& data : dataV_ROOT ) { testROOTDotProduct( data.direction ); }
  for ( const auto& data : dataV_Eigen ) { testEigenDotProduct( data.direction ); }
