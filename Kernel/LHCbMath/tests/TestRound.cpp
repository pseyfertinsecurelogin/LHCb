...
int main() {
  ...
  for ( int i = 0; i < 10000; ++i ) {
    ...
    if ( i1 != i2 || i2 != i3 || i3 != i4 || i1 != i3 || i2 != i4 || i1 != i4 ) {
      ++bad;
      std::cout << i1 << " " << i2 << " " << i3 << "  " << i4 << " " << v << std::endl;
    }
  }
  ...
  for ( unsigned int m = 0; m < nMax; ++m ) {
    ...
    { // LHCb::Math::round
      for ( int j = 0; j < 500; ++j ) {
        Chrono cnt( &counter1 );
        for ( int k = 0; k < 500; ++k ) { value1 += roundOld( v ) + ( j - k ); }
      }
    }
    { // LHCb::Math::round using lomont
      for ( int j = 0; j < 500; ++j ) {
        Chrono cnt( &counter2 );
        for ( int k = 0; k < 500; ++k ) { value2 += roundLomont( v ) + ( j - k ); }
      }
    }
    { // boost::numeric
      for ( int j = 0; j < 500; ++j ) {
        Chrono cnt( &counter3 );
        for ( int k = 0; k < 500; ++k ) { value3 += LHCb::Math::round( v ) + ( j - k ); }
      }
    }
    ++show_progress;
  }

  std::cout << " round-Knuth     " << value1 << std::endl
