...
for T in (cpp.Gaudi.Math.Polynomial, cpp.Gaudi.Math.Bernstein,
          cpp.Gaudi.Math.LegendreSum, cpp.Gaudi.Math.ChebyshevSum):
...
def summary(cnts):
    ...
    for k in keys:
        ...
        print " %12s -> %-12s : %-20s %.3g min/max=%.3g/%.3g" % (k[0], k[1], m,
                                                                 r, mn, mx)
