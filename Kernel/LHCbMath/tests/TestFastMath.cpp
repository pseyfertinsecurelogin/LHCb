...
const char* type_name() {
  return helper_type_name( static_cast<T*>( nullptr ) );
}
...
constexpr bool is_scalar() {
  return std::is_same<T, decltype( inner_type<T>() )>::value;
}
...
#ifdef __x86_64__
void static inline __attribute( ( always_inline ) ) keep( float& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( double& r ) noexcept { asm volatile( "" : "+v"( r ) ); }

void static inline __attribute( ( always_inline ) ) keep( __m128& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( __m128d& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
#endif

#ifdef __AVX__
void static inline __attribute( ( always_inline ) ) keep( __m256& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( __m256d& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
#endif

#ifdef __AVX512F__
void static inline __attribute( ( always_inline ) ) keep( __m512& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( __m512d& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
#endif

#ifdef __ARM_NEON
void static inline __attribute( ( always_inline ) ) keep( float& r ) noexcept { asm volatile( "" : "+w"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( double& r ) noexcept { asm volatile( "" : "+w"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( float32x2_t& r ) noexcept { asm volatile( "" : "+w"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( float32x4_t& r ) noexcept { asm volatile( "" : "+w"( r ) ); }
#  ifdef __aarch64__
void static inline __attribute( ( always_inline ) ) keep( float64x2_t& r ) noexcept { asm volatile( "" : "+w"( r ) ); }
#  endif
#endif

#ifdef __ALTIVEC__
void static inline __attribute( ( always_inline ) ) keep( float& r ) noexcept { asm volatile( "" : "+f"( r ) ); }
void static inline __attribute( ( always_inline ) ) keep( double& r ) noexcept { asm volatile( "" : "+d"( r ) ); }
#  ifndef __VSX__
void static inline __attribute( ( always_inline ) ) keep( vector float& r ) noexcept { asm volatile( "" : "+v"( r ) ); }
#  else
...
template <class T>
constexpr int card() {
  return sizeof( T ) / sizeof( inner_type<T>() );
}
...
#ifdef __SSE2__
template <>
struct helper_get_vec<float, 4> {
  using type = __m128;
};
template <>
struct helper_get_vec<double, 2> {
  using type = __m128d;
};
#endif
#ifdef __AVX__
template <>
struct helper_get_vec<float, 8> {
  using type = __m256;
};
template <>
struct helper_get_vec<double, 4> {
  using type = __m256d;
};
#endif
#ifdef __AVX512F__
template <>
struct helper_get_vec<float, 16> {
  using type = __m512;
};
template <>
struct helper_get_vec<double, 8> {
  using type = __m512d;
};
#endif
#ifdef __ARM_NEON
template <>
struct helper_get_vec<float, 2> {
  ...
  namespace Math {
    ...
    namespace details {
      ...
      struct RSQRT<naive> : RSQRT<ieee> {
        ...
        static float  doit( float x ) { return 1.f / std::sqrt( x ); }
        static double doit( double x ) { return 1. / std::sqrt( x ); }
      };
...
double __attribute( ( noinline ) ) time( F&& f ) {
  ...
  for ( int i = 0; i < N; i++ ) { in[i] = init<scalar_t>( 1.23456789e6 ); }
...
#endif
    for ( int j = 0; j < repeat / N; j++ ) { Transform<T>( +in, +in + N, +in, f ); }
#ifdef RDTSC
    auto t1 = _rdtsc();
#else
    auto   t1 = std::chrono::high_resolution_clock::now();
#endif
    for ( int j = 0; j < N; j++ ) { keep( in[j] ); }
#ifdef RDTSC
...
struct Error {
  ...
  explicit __attribute( ( noinline ) ) Error( F&& f, E&& error ) {
    ...
    for ( long long i = 0; i < n; i += N ) {
      for ( int j = 0; j < N; j++ ) { input[j] = distribution( mt ); }
...
struct Error<float, Compute> {
  ...
  explicit __attribute( ( noinline ) ) Error( F&& f, E&& error ) {
    ...
    for ( int j = 1; j < N; j++ ) { iinput[j] = iinput[0] + j; }

    for ( long long i = 0; i < n; i += N ) {
      for ( int j = 0; j < N; j++ ) { input[j] = bit_cast<float, int>( iinput[j] ); }
      Transform<Compute>( +input, +input + N, +output, f );
      for ( int j = 0; j < N; j++ ) {
...
double bits( double e ) { return e == 0 ? std::numeric_limits<double>::infinity() : -std::log2( e ); }
...
void csv_time( F&& ) {
  ...
  for ( int i = 0; i < N; i++ ) { std::cout << "," << times[i]; }
  std::cout << std::endl;
}
...
void csv_accuracy( F&& ) {
  ...
  Error<Storage, Compute> errors[N]{Error<Storage, Compute>{typename F::template fn<Tags>{}, typename F::error{}}...};

  std::cout << type_name<Compute>();
  for ( int i = 0; i < N; i++ ) { std::cout << "," << bits( errors[i].max ); }
  std::cout << std::endl;
}
...
void csv_time() {
  ...
  std::cout << "type";
  for ( int i = 0; i < N; i++ ) { std::cout << "," << names[i]; }
  std::cout << std::endl;

  std::cout << "scalar ";
  csv_time<float, Tags...>( F<true>{} );
...
void csv_accuracy() {
  ...
  std::cout << "scalar ";
  csv_accuracy<float, Tags...>( F<true>{} );
  ...
  std::cout << "scalar ";
  csv_accuracy<double, Tags...>( F<true>{} );
...
struct time_functor {
  static double call() { return time<T>( typename F::template fn<Tag>{} ); }
};
...
void check_accuracies( F&& ) {
  ...
  for ( int i = 1; i < N; i++ ) {
    if ( max_error > errors[i].max * max_error_tol ) {
      ...
      std::cout << F::name << "<" << names[i] << ">(" << type_name<Compute>() << ")";
      std::cout << ":\tmin accuracy " << bits( max_error ) << " / " << bits( errors[i].max ) << "\t(error)"
                << std::endl;
    ...
    if ( max_quad_error > errors[i].quad * quad_error_tol ) {
      ...
      std::cout << " is less accurate than ";
      std::cout << F::name << "<" << names[i] << ">(" << type_name<Compute>() << ")";
      std::cout << ":\tmean accuracy " << bits( max_quad_error ) << " / " << bits( errors[i].quad ) << "\t(error)"
                << std::endl;
...
int main( int argc, char* argv[] ) {
  ...
  if ( failed > 0 || warning > 0 ) {
    ...
    if ( warning > 0 ) { std::cout << "\t(warnings: " << warning << "/" << failed + warning << ")"; }
    std::cout << std::endl;
