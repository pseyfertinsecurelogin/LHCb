...
void fillSMatrixSymWithCondNumber( Gaudi::SymMatrix5x5& F, TRandom& r, double condNumber ) {
  ...
  Gaudi::Matrix5x5 D;
  D( 0, 0 ) = 1;
  for ( int i = 1; i < 5; i++ ) { D( i, i ) = r.Rndm() * condNumber; }
 ...
 */
template <typename M>
std::pair<M, bool> compareSMatrix( M& A, M& B, bool symetric = true, double diffThreshold = 1e-5 ) {
  M    cmpres;
  bool hasDiff = false;
...
TestResults compareInstructionSets( Mat& Ftype, SymMat& Otype, VTable& simFuncs, TRandom& r, const int nbentries,
  ...
  auto runit = [&]( ISet iset, std::vector<SymMat>& res ) {
    ...
    for ( int i = 0; i < nbentries; i++ ) { ( *f )( to_span( O[i] ), to_span( F[i] ), to_span( res[i] ) ); }
    ...
    for ( int i = 0; i < nbentries; i++ ) {
      {
        ...
        if ( tmpres > results.maxDiff[GENERIC] ) results.maxDiff[GENERIC] = tmpres;
      }
      {
        double tmpres = matrixMaxDiff( resClassic[i], resSSE3[i] );
        if ( tmpres > results.maxDiff[SSE3] ) results.maxDiff[SSE3] = tmpres;
      }
      {
        double tmpres = matrixMaxDiff( resClassic[i], resAVX[i] );
        if ( tmpres > results.maxDiff[AVX] ) results.maxDiff[AVX] = tmpres;
      }
      {
        double tmpres = matrixMaxDiff( resClassic[i], resAVX2[i] );
        if ( tmpres > results.maxDiff[AVX2] ) results.maxDiff[AVX2] = tmpres;
      }
    }
...
int main() {
  ...
  for ( int i = 0; i < nbres; i++ ) {
    std::cout << results[i].conditionNumber << "\t" << results[i].maxDiff[ISet::GENERIC] << "\t"
              << results[i].maxDiff[ISet::SSE3] << "\t" << results[i].maxDiff[ISet::AVX] << "\t"
              << results[i].maxDiff[ISet::AVX2] << std::endl;
  }
  ...
  for ( int i = 0; i < testcount; i++ ) {
    ...
    std::cout << tresults[i].timing[ISet::CLASSIC] << "\t" << tresults[i].timing[ISet::GENERIC] << "\t"
              << tresults[i].timing[ISet::SSE3] << "\t" << tresults[i].timing[ISet::AVX] << "\t"
              << tresults[i].timing[ISet::AVX2] << std::endl;
  }
