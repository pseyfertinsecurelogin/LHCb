...
if not 0.75 < _eps_ * 2**52 < 1.25:
    import warnings
    warnings.warn(
        '"epsilon" in not in the expected range!Math could be suboptimal')
# =============================================================================
...
def _h8_(func, x, h, der=False):
    ...
    d1 = 672.0 * (fp1 - fm1) - 168 * (fp2 - fm2) + 32 * (fp3 - fm3) - 3 * (
        fp4 - fm4)
    d1 /= 840 * h
    ...
    fm5 = func(x - 5 * h)
    fp5 = func(x + 5 * h)
    d9 = (-fm5 + 8 * fm4 - 27 * fm3 + 48 * fm2 - 42 * fm1 + 42 * fp1 - 48 * fp2
          + 27 * fp3 - 8 * fp4 + fp5) / (2 * h**9)

    d9 = 42.0 * (fp1 - fm1) - 48 * (fp2 - fm2) + 27 * (fp3 - fm3) - 8 * (
        fp4 - fm4) + (fp5 - fm5)
    d9 /= 2 * h**9
...
def _h10_(func, x, h, der=False):
    ...
    d1 = 2100.0 * (fp1 - fm1) - 600 * (fp2 - fm2) + 150 * (fp3 - fm3) - 25 * (
        fp4 - fm4) + 2 * (fp5 - fm5)
    d1 /= 2520.0 * h
    ...
    d11 = -132.0 * (fp1 - fm1) + 165 * (fp2 - fm2) - 110 * (fp3 - fm3) + 44 * (
        fp4 - fm4) - 10 * (fp5 - fm5) + (fp6 - fm6)
    d11 /= 2 * h**11
...
def _h12_(func, x, h, der=False):
    ...
    d1 = 23760.0 * (fp1 - fm1) - 7425 * (fp2 - fm2) + 2200 * (
        fp3 - fm3) - 495 * (fp4 - fm4) + 72 * (fp5 - fm5) - 5 * (fp6 - fm6)
    d1 /= 27720.0 * h
    ...
    d13 = 429.0 * (fp1 - fm1) - 572 * (fp2 - fm2) + 429 * (fp3 - fm3) - 208 * (
        fp4 - fm4) + 65 * (fp5 - fm5) - 12 * (fp6 - fm6) + (fp7 - fm7)
    d13 /= 2 * h**13
...
def _h14_(func, x, h, der=False):
    ...
    d1 = 315315.0 * (fp1 - fm1) - 105105 * (fp2 - fm2) + 35035 * (
        fp3 - fm3) - 9555 * (fp4 - fm4) + 1911 * (fp5 - fm5) - 245 * (
            fp6 - fm6) + 15 * (fp7 - fm7)
    d1 /= 360360.0 * h
    ...
    d15 = -1430.0 * (fp1 - fm1) + 2002 * (fp2 - fm2) - 1638 * (
        fp3 - fm3) + 910 * (fp4 - fm4) - 350 * (fp5 - fm5) + 90 * (
            fp6 - fm6) - 14 * (fp7 - fm7) + (fp8 - fm8)
    d15 /= 2 * h**15
...
def _h16_(func, x, h, der=False):
    ...
    d1 = 640640.0 * (fp1 - fm1) - 224224 * (fp2 - fm2) + 81536 * (
        fp3 - fm3) - 25480 * (fp4 - fm4) + 6272 * (fp5 - fm5) - 1120 * (
            fp6 - fm6) + 128 * (fp7 - fm7) - 7 * (fp8 - fm8)
    d1 /= 720720.0 * h
    ...
    d17 = 4862.0 * (fp1 - fm1) - 7072 * (fp2 - fm2) + 6188 * (
        fp3 - fm3) - 3808 * (fp4 - fm4) + 1700 * (fp5 - fm5) - 544 * (
            fp6 - fm6) + 119 * (fp7 - fm7) - 16 * (fp8 - fm8) + (fp9 - fm9)
    d17 /= 2 * h**17
...
_numbers_ = (
    (0.5 * 10**(-10. / 3), 0.5 * 10**(-10. / 3), 0.5 * 10**(-10. / 5),
     0.5 * 10**(-10. / 7), 0.5 * 10**(-10. / 9), 0.5 * 10**(-10. / 11),
     0.5 * 10**(-10. / 13), 0.5 * 10**(-10. / 15), 0.5 * 10**(-10. / 17)),
...
def derivative(fun, x, h=0, I=2, err=False):
    ...
    e2 = e * e * (J * _eps_ + abs(f0) + abs(x * d1))**(2 - 2. / J) * abs(dJ)**(
        2. / J)
    return VE(d1, 4 * e2)
...
class Derivative(object):
    ...
    def __call__(self, x):
        ...
        >>> print deriv(0.1)
        """
        return derivative(self._func, x, self._h, self._I, self._err)
...
class IntegralCache(Integral):
    ...
    def __call__(self, x, *args):
        ...
        if not args and isinstance(x, float) and self._prev:
            if isinstance(self._xmin,
                          float) and (self._prev[0] - x) < abs(self._xmin - x):
                xmn = self._prev[0]
                dlt = self._prev[1]
...
class EvalVE(object):
    ...
    def __call__(self, x, *args):
        ...
        elif 0 >= x.cov2() or is_zero(x.cov2()):
            return VE(val, 0)
        # evaluate the derivative
...
class Moment(object):
    ...
    ## print it!
    def __str__(self):
        return "Moment(%d,%s,%s,%s,%s)" % (self._N, self._xmin, self._xmax,
                                           self._err, self._x0)
...
class CentralMoment(Moment):
    ...
    ## print it!
    def __str__(self):
        return "CentralMoment(%d,%s,%s,%s)" % (self._N, self._xmin, self._xmax,
                                               self._err)
...
class Mean(Moment):
    ...
    def __str__(self):
        return "Mean(%s,%s,%s)" % (self._xmin, self._xmax, self._err)
...
class Variance(Mean):
    ...
    def __str__(self):
        return "Variance(%s,%s,%s)" % (self._xmin, self._xmax, self._err)
...
class RMS(Variance):
    ...
    def __str__(self):
        return "RMS(%s,%s,%s)" % (self._xmin, self._xmax, self._err)
...
class Skewness(Variance):
    ...
    def __str__(self):
        return "Skewness(%s,%s,%s)" % (self._xmin, self._xmax, self._err)
...
class Kurtosis(Skewness):
    ...
    def __str__(self):
        return "Kurtosis(%s,%s,%s)" % (self._xmin, self._xmax, self._err)
...
class Mode(Median):
    ...
    def __call__(self, func, *args):
        ...
        result = optimize.minimize(
            ifun, x0=float(m0), bounds=[(self._xmin, self._xmax)], args=args)
...
class CL_symm(object):
    ...
    def __str__(self):
        return "CL_sym(%s,%s,%s,%s)" % (self._prob, self._xmin, self._xmax,
                                        self._x0)
    ...
    def __call__(self, func, *args):
        ...
        if not xmin <= x0 <= xmax:
            raise AttributeError(
                "Invalid x0 value %s<=%s<=%s" % (xmin, x0, xmax))
        ...
        if 0 >= norm:
            raise AttributeError(
                "Normalization integral is not positive %s" % norm)
...
class CL_asymm(object):
    ...
    def __call__(self, func, *args):
        ...
        if not xmin <= xmode <= xmax:
            raise AttributeError(
                "Invalid mode value %s<=%s<=%s" % (xmin, xmode, xmax))
...
if '__main__' == __name__:
    ...
    functions = [
        ...
        (lambda x: math.tanh(10. * x),
         lambda x: 10 * (1. - math.tanh(10. * x)**2), 0.1),
        (lambda x: math.tan(10. * x),
         lambda x: 10 * (1. + math.tan(10. * x)**2), 2.1),
        (lambda x: 1. / math.sin(2. * x),
         lambda x: -2. / math.sin(2. * x)**2 * math.cos(2. * x), 0.2),
    ...
    for i in range(0, 20):
        ...
        print fmt % (x, func(x), 1 - deriv_1(x) / deriv_a(x),
                     1 + deriv_2(x) / func(x))
    ...
    print '1@[0,1]         0.501-quantile: %s ' % quantile(
        lambda x: 1, 0.501, 0, 1)
    print '1@[0,1]         0.201-quantile: %s ' % quantile(
        lambda x: 1, 0.201, 0, 1)
