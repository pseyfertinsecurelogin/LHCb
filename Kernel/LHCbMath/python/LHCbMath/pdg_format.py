...
def pdg_format(value, error, latex=False):
    ...
    q = abs(q)
    short = (0 == n) or (1 == n and abs(v) < 10000) or (-1 == n
                                                        and abs(v) > 0.1)
...
def pdg_format2(value, error1, error2, latex=False, mode='total'):
    ...
    if not short:
        ...
        if latex:
            fmt = "$(%%.%df \pm %%.%df \pm %%.%df)\cdot10^{%%d}$" % (q, q, q)
        else:
            fmt = "(%%.%df +/- %%.%df +/- %%.%df)*10^{%%d}" % (q, q, q)
...
def pdg_format3(value, error1, error2, error3, latex=False, mode='total'):
    ...
    if not short:
        ...
        if latex:
            fmt = "$(%%.%df \pm %%.%df \pm %%.%df \pm %%.%df)\cdot10^{%%d}$" % (
                q, q, q, q)
        else:
            fmt = "(%%.%df +/- %%.%df +/- %%.%df +/- %%.%df)*10^{%%d}" % (q, q,
                                                                          q, q)
        ...
        if latex:
            fmt = "$%%.%df \pm %%.%df \pm %%.%df \pm %%.%df$" % (q, q, q, q)
        else:
            fmt = "%%.%df +/- %%.%df +/- %%.%df +/- %%.%df" % (q, q, q, q)
        ##
...
if '__main__' == __name__:
    ...
    for s in (1.e-6, 1.e-3, 1.e3, 1.e6):
        for e in (0.000123, 0.000456, 0.000900, 0.000986):
            ve = VE(1. / 3, e * e) * s
            print ' Value %s,\tround: %s,\tTeX: %s' % (ve, ve.pdg(),
                                                       ve.pdg_format(True))
