...
## Geometry vectors
cpp.Gaudi.XYZPoint = _RM.PositionVector3D(
    'ROOT::Math::Cartesian3D<double>,ROOT::Math::DefaultCoordinateSystemTag')
cpp.Gaudi.XYZVector = _RM.DisplacementVector3D(
    'ROOT::Math::Cartesian3D<double>,ROOT::Math::DefaultCoordinateSystemTag')
...
def _sym_matrix_(i, typ='double'):
    ...
    """
    m = _RM.SMatrix(
        '%s,%d,%d,ROOT::Math::MatRepSym<%s,%d>' % (typ, i, i, typ, i))
...
## Gaudi::Math
cpp.Gaudi.Math.XYZLine = cpp.Gaudi.Math.Line(cpp.Gaudi.XYZPoint,
                                             cpp.Gaudi.XYZVector)
...
cpp.Gaudi.Math.ValueWithError.Vector = cpp.std.vector(
    cpp.Gaudi.Math.ValueWithError)
...
def _p_str_(self):
    ...
    >>> print plance
    """
    return "(%s,%s)" % (self.ProjectOntoPlane(cpp.Gaudi.XYZPoint()),
                        self.Normal())
...
for t in (cpp.Gaudi.Math.ValueWithError, cpp.Gaudi.Math.Point3DWithError,
          cpp.Gaudi.Math.Vector3DWithError,
          cpp.Gaudi.Math.LorentzVectorWithError,
          cpp.Gaudi.Math.SVector2WithError, cpp.Gaudi.Math.SVector3WithError,
          cpp.Gaudi.Math.SVector4WithError, cpp.Gaudi.Math.SVector5WithError,
          cpp.Gaudi.Math.SVector6WithError, cpp.Gaudi.Math.SVector8WithError):
...
def _eigen_2_(self, sorted=True):
    ...
    st = cpp.Gaudi.Math.EigenSystems.eigenVectors(self, _values, _vectors,
                                                  sorted)
    if st.isFailure():
...
for m in (cpp.Gaudi.SymMatrix2x2, cpp.Gaudi.SymMatrix3x3,
...
if not hasattr(cpp.Gaudi.Math, 'XYZGeomFun'):
    cpp.Gaudi.Math.XYZGeomFun = cpp.Gaudi.Math.GF(
        cpp.Gaudi.XYZPoint, cpp.Gaudi.XYZLine, cpp.Gaudi.Plane3D)
...
def _fit_mass_(particle, mass):
    ...
    _chi2 = cpp.Gaudi.Math.FitMass.fit(particle, mass, _fitted)
...
if not hasattr ( cpp.Gaudi.Math.Splines , 'DATA'    ) or \
   ...
   not hasattr ( cpp.Gaudi.Math.Splines , 'DATAERR' ) :
    ...
    cpp.Gaudi.Math.Splines.PAIR = std.pair('double',
                                           cpp.Gaudi.Math.ValueWithError)
...
def _ve_eq_(self, other):
    ...
    if isinstance(other, VE):
        ...
        raise NotImplementedError, ' Equality for %s and  %s is not implemented' % (
            self, other)
...
def _ve_ne_(self, other):
    ...
    try:
        ...
        raise NotImplementedError, ' Inequality for %s and  %s is not implemented' % (
            self, other)
