...
namespace Gaudi {
  namespace Math {
    ...
    public:
       ...
       */
      aPoint position( const double mu ) const { return beginPoint() + direction() * (float)mu; }
      /** Return a point on the line tick direction vectors away
       *  from point of origin:
       *  \f$ \vec{\mathbf{P}}\left(\mu\right) =
       *      \vec{\mathbf{P}}_0 + \mu\vec{\mathbf{V}} \f$
       */
      aPoint operator()( const double mu ) const { return beginPoint() + direction() * (float)mu; }

    public:
      inline std::ostream& fillStream( std::ostream& os ) const {
        os << "\np0 (" << m_p0.x() << " " << m_p0.y() << " " << m_p0.z() << ") direction (" << m_v0.x() << " "
           << m_v0.y() << " " << m_v0.z() << ")\n"
           << std::endl;
        return os;
      }
    ...
    // =========================================================================
    template <typename aPoint, typename aVector>
    inline std::ostream& operator<<( std::ostream& os, const Line<aPoint, aVector>& rhs ) {
