...
namespace LHCb {
  namespace Math {
    ...
    namespace Eigen {
      ...
      template <typename FTYPE = float, typename E4Vector = typename ::Eigen::Matrix<FTYPE, 4, 1>,
      ...
      public:
        ...
        XYZVector( const FTYPE x = 0.0, const FTYPE y = 0.0, const FTYPE z = 0.0 ) : E4Vector( x, y, z, 0 ) {}
      ...
      public:
        /// Dot product
        template <class TYPE>
        inline FTYPE dot( const TYPE& v ) const {
          return E4Vector::dot( v );
        }
        ...
        /// 3D Cross product
        template <class TYPE>
        inline XYZVector Cross( const TYPE& v ) const {
          return cross3( v );
        }
      ...
      public:
        /// Standard constructor
        XYZPoint( const FTYPE x = 0.0, const FTYPE y = 0.0, const FTYPE z = 0.0 ) : E4Vector( x, y, z, 0 ) {}
      ...
      public: /// Alias methods for accessors and setters
        ...
        inline void SetCoordinates( const FTYPE x, const FTYPE y, const FTYPE z ) noexcept {
          *this = E4Vector( x, y, z, 0 );
        }
      ...
      template <typename FTYPE = float, typename EPlane = typename ::Eigen::Hyperplane<FTYPE, 3>>
      ...
      public:
        ...
        Plane3D( const FTYPE a, const FTYPE b, const FTYPE c, const FTYPE d ) : EPlane( Vec3f( a, b, c ), d ) {}
        ...
        Plane3D( const VECTOR& v, const POINT& p )
            : EPlane( Vec3f( v.x(), v.y(), v.z() ), Vec3f( p.x(), p.y(), p.z() ) ) {}

        /// Constructor from three points in the plane
        template <class POINT>
        Plane3D( const POINT& p1, const POINT& p2, const POINT& p3 )
            : EPlane( EPlane::Through( Vec3f( p1.x(), p1.y(), p1.z() ), Vec3f( p2.x(), p2.y(), p2.z() ),
