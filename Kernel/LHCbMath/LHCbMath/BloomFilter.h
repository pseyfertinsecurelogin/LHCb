...
#  endif
  ...
  static constexpr double log2_tiny( const double xm1 ) {
    return ( ( ( ( ( ( ( ( ( ( ( ( ( ( ( ( ( ( ( -xm1 / 20. + 1. / 19. ) * xm1 - 1. / 18. ) * xm1 + 1. / 17. ) * xm1 -
                                           1. / 16. ) *
                                             xm1 +
                                         1. / 15. ) *
                                           xm1 -
  ...
  static constexpr double log2_small( const double x ) {
    return ( abs( x - 1. ) < 0.18 )
               ...
               : ( ( ( ( ( ( ( ( ( ( 6.9334366458438446E+10 * x + 3.3811243166750752E+12 ) * x +
                                   3.5283588930136203E+13 ) *
                                     x +
                                 1.1756441702477491E+14 ) *
                                   x +
                               1.133968217106985E+14 ) *
                                 x -
                             5.7135488589275375E+13 ) *
                               x -
                           1.3605784210515297E+14 ) *
                             x -
                         6.4737928386652203E+13 ) *
                           x -
                       1.1115029743498373E+13 ) *
                         x -
                     6.4096316616289868E+11 ) *
                       x -
                   8.0343580013031902E+9 ) /
                     ( ( ( ( ( ( ( ( ( ( 1.23046875E+10 * x + 9.84375E+11 ) * x + 1.5946875E+13 ) * x + 9.072E+13 ) *
                                       x +
                                   2.22264E+14 ) *
                                     x +
                                 2.56048128E+14 ) *
  ...
  static constexpr double log2_finite( const double x ) {
    return ( abs( x - 1. ) <= 0.5 )
               ? log2_small( x )
               : ( ( x > 1. ) ? ( 1. + log2_finite( 0.5 * x ) ) : ( -1. + log2_finite( 2.0 * x ) ) );
  }
  ...
  static constexpr double log2( const double x ) {
    return ( x < 0 || x != x )
               ...
               : ( ( x > std::numeric_limits<double>::max() )
                       ...
                       : ( ( x == 0. ) ? -std::numeric_limits<double>::infinity() : log2_finite( x ) ) );
  }

  /// log implementation (computable at compile time)
  static constexpr double log( const double x ) { return 0.69314718055995 * log2( x ); }
#endif
   ...
   * @date 2014-09-10
   */
  static constexpr unsigned _nBits( const unsigned capacity, const unsigned pnumer, const unsigned pdenom ) {
   ...
   * value...
   */
  static constexpr bool _canUseVeryFastHash( uint64_t MAX, uint64_t NBITS, unsigned NHASHES ) {
    return ( 0 == NHASHES ) ? ( 0 != MAX ) : _canUseVeryFastHash( MAX / NBITS, NBITS, NHASHES - 1 );
  }

  // the abomination pays us a visit...
#  define _nBits( capacity, pnumer, pdenom ) BloomFilterImpl::_nBits( capacity, pnumer, pdenom )

  // the abomination pays us a visit...
#  define _nHashes( pnumer, pdenom ) BloomFilterImpl::_nHashes( pnumer, pdenom )

  // the abomination pays us a visit...
#  define _canUseVeryFastHash( max, nbits, nhashes ) BloomFilterImpl::_canUseVeryFastHash( max, nbits, nhashes )
...
#  warning "Intel compilers before 14.x are really difficult to support"
  ...
  namespace log2impl {
    ...
    struct flog2 {
      enum {
        value = ( ( 1u << ( 4 * 6 ) ) * _N ) / 1u - ( ( 1u << ( 3 * 6 ) ) * _N * _N ) / 2u +
                ( ( 1u << ( 2 * 6 ) ) * _N * _N * _N ) / 3u - ( ( 1u << ( 1 * 6 ) ) * _N * _N * _N * _N ) / 4u +
  ...
  public:
    ...
    enum {
      numer = int( denom - ( ( N != ( 1u << log2impl::ilog2<N>::value ) ) *
                                 log2impl::flog2<( 64u * ( ( 1u << log2impl::ilog2up<N>::value ) - N ) ) /
                                                 ( 1u << log2impl::ilog2up<N>::value )>::value -
                             ( D != ( 1 << log2impl::ilog2<D>::value ) ) *
                                 log2impl::flog2<( 64u * ( ( 1u << log2impl::ilog2up<D>::value ) - D ) ) /
                                                 ( 1u << log2impl::ilog2up<D>::value )>::value ) )
    };
    /// integer part of log2(N/D)
    enum { integer = int( log2impl::ilog2up<N>::value - log2impl::ilog2up<D>::value - 1 ) };

    /// return result as double
    constexpr static double value() { return double( integer ) + double( numer ) / double( denom ); }
  };
   ...
   * @date 2014-08-15
   */
  template <unsigned N, unsigned PNUMER, unsigned PDENOM>
  struct nbits {
    static_assert( PNUMER < PDENOM, "Probability >= 1, error." );
    enum {
      value =
          int( ( ( -int64_t( N ) * ( int64_t( log2<PNUMER, PDENOM>::denom ) * int64_t( log2<PNUMER, PDENOM>::integer ) +
   ...
   * @date 2014-08-15
   */
  template <unsigned PNUMER, unsigned PDENOM>
  struct nhashes {
    static_assert( PNUMER < PDENOM, "Probability >= 1, error." );
    enum {
      value = int( -( int64_t( log2<PNUMER, PDENOM>::denom ) * int64_t( log2<PNUMER, PDENOM>::integer ) +
                      int64_t( log2<PNUMER, PDENOM>::numer ) ) /
                   int64_t( 594482189u ) )
    };
  };
  ...
  struct canUseVeryFastHash {
    enum { value = canUseVeryFastHash<MAX / NBITS, NBITS, NHASHES - 1>::value };
  };
  ...
  // the abomination pays us a visit...
#  define _nBits( capacity, pnumer, pdenom ) BloomFilterImpl::nbits<capacity, pnumer, pdenom>::value

  // the abomination pays us a visit...
#  define _nHashes( pnumer, pdenom ) BloomFilterImpl::nhashes<pnumer, pdenom>::value

  // the abomination pays us a visit...
#  define _canUseVeryFastHash( max, nbits, nhashes ) BloomFilterImpl::canUseVeryFastHash<max, nbits, nhashes>::value

#endif

  /// very dumb FNV1a implementation
  template <class T>
  struct __doFNV1a { /** empty, resolve template only for specialised known types */
  };
   ...
   */
  template <>
  struct __doFNV1a<uint32_t> {
    static const uint32_t     hashinit = 2166136261u;
    static constexpr uint32_t doByte( uint32_t hash, uint8_t val ) { return ( hash ^ val ) * 16777619u; }
    uint32_t operator()( const unsigned char* begin, const unsigned char* end, uint32_t hash = hashinit ) const {
      return std::accumulate( begin, end, hash, doByte );
    }
    constexpr uint32_t operator()( uint8_t val, uint32_t hash = hashinit ) const { return doByte( hash, val ); }
   ...
   */
  template <>
  struct __doFNV1a<uint64_t> {
    static const uint64_t     hashinit = 14695981039346656037ull;
    static constexpr uint64_t doByte( uint64_t hash, uint8_t val ) { return ( hash ^ val ) * 1099511628211ull; }
    uint64_t operator()( const unsigned char* begin, const unsigned char* end, uint64_t hash = hashinit ) const {
      return std::accumulate( begin, end, hash, doByte );
    }
    constexpr uint64_t operator()( uint8_t val, uint64_t hash = hashinit ) const { return doByte( hash, val ); }
   ...
   * See http://www.isthe.com/chongo/tech/comp/fnv/ for details.
   */
  template <class T, class HASHTYPE = uint64_t>
  struct HashFNV1a {
    /// type of hash value
    ...
    constexpr result_type operator()( const T& val, const result_type hash = __doFNV1a<result_type>::hashinit ) const {
      return __doFNV1a<result_type>()( reinterpret_cast<const unsigned char*>( &val ),
                                       reinterpret_cast<const unsigned char*>( &val ) + sizeof( T ), hash );
    }
  };
  /// specialise HashFNV1a for basic types
#define BLOOMFILTER_DOHASHOVERLOAD( typearg )                                                                           \
  template <>                                                                                                           \
  struct HashFNV1a<typearg, uint32_t> {                                                                                 \
    typedef uint32_t result_type;                                                                                       \
    typedef typearg  argument_type;                                                                                     \
    result_type      operator()( const typearg val, const result_type hash = __doFNV1a<result_type>::hashinit ) const { \
      return __doFNV1a<result_type>()( val, hash );                                                                \
    }                                                                                                                   \
  };                                                                                                                    \
  template <>                                                                                                           \
  struct HashFNV1a<typearg, uint64_t> {                                                                                 \
...
#define BLOOMFILTER_DOHASHOVERLOAD_CSTRING( CTYPE, HASHTYPE )                                                          \
  template <>                                                                                                          \
  struct HashFNV1a<const CTYPE*, HASHTYPE> {                                                                           \
    typedef HASHTYPE     result_type;                                                                                  \
    typedef const CTYPE* argument_type;                                                                                \
    result_type          operator()( argument_type str, result_type hash = __doFNV1a<result_type>::hashinit ) const {  \
...
#define BLOOMFILTER_DOHASHOVERLOAD_POINTER( HASHTYPE )                                                                 \
  template <class T>                                                                                                   \
  struct HashFNV1a<const T*, HASHTYPE> {                                                                               \
    typedef HASHTYPE result_type;                                                                                      \
    typedef const T* argument_type;                                                                                    \
    result_type      operator()( argument_type val, result_type hash = __doFNV1a<result_type>::hashinit ) const {      \
      return __doFNV1a<result_type>()( reinterpret_cast<const void*>( val ), hash );                              \
    }                                                                                                                  \
  }
  ...
  struct HashFNV1a<std::basic_string<CharT, Traits, Allocator>, HASHTYPE> {
    ...
    result_type operator()( const argument_type& str, result_type hash = __doFNV1a<result_type>::hashinit ) const {
      return std::accumulate( std::begin( str ), std::end( str ), hash,
                              []( result_type h, CharT c ) { return __doFNV1a<result_type>()( c, h ); } );
    }
  };
} // namespace BloomFilterImpl
 ...
 */
template <class T, unsigned CAPACITY = 128, unsigned PNUMER = 1, unsigned PDENOM = 64,
...
public:
  ...
  constexpr static double pFalsePositiveTarget() { return double( PNUMER ) / double( PDENOM ); }
  ...
  constexpr static double pFalsePositive() { return std::pow( 2., -nBits * BloomFilterImpl::log( 2. ) / capacity ); }
...
private:
  ...
  typedef typename BloomFilterImpl::HashFNV1a<hashresult1_type> HASH2;
  ...
  enum { canUseVeryFastHash = _canUseVeryFastHash( std::numeric_limits<hashresult1_type>::max(), nBits, nHashes ) };
...
public:
   ...
   */
  inline int popcount() const { return m_bits.count(); }
  ...
  inline constexpr uint32_t getword( unsigned nword ) const {
    return ( 32 * nword >= nBits ) ? 0 :           /* read past the end of the bitset */
               ( ( 32 * ( nword + 1 ) <= nBits ) ? /* word fully contained in bitset */
                     ( ( uint32_t( m_bits[32 * nword + 0] ) << 0 ) | ( uint32_t( m_bits[32 * nword + 1] ) << 1 ) |
                       ( uint32_t( m_bits[32 * nword + 2] ) << 2 ) | ( uint32_t( m_bits[32 * nword + 3] ) << 3 ) |
                       ( uint32_t( m_bits[32 * nword + 4] ) << 4 ) | ( uint32_t( m_bits[32 * nword + 5] ) << 5 ) |
                       ( uint32_t( m_bits[32 * nword + 6] ) << 6 ) | ( uint32_t( m_bits[32 * nword + 7] ) << 7 ) |
                       ( uint32_t( m_bits[32 * nword + 8] ) << 8 ) | ( uint32_t( m_bits[32 * nword + 9] ) << 9 ) |
                       ( uint32_t( m_bits[32 * nword + 10] ) << 10 ) | ( uint32_t( m_bits[32 * nword + 11] ) << 11 ) |
                       ( uint32_t( m_bits[32 * nword + 12] ) << 12 ) | ( uint32_t( m_bits[32 * nword + 13] ) << 13 ) |
                       ( uint32_t( m_bits[32 * nword + 14] ) << 14 ) | ( uint32_t( m_bits[32 * nword + 15] ) << 15 ) |
                       ( uint32_t( m_bits[32 * nword + 16] ) << 16 ) | ( uint32_t( m_bits[32 * nword + 17] ) << 17 ) |
                       ( uint32_t( m_bits[32 * nword + 18] ) << 18 ) | ( uint32_t( m_bits[32 * nword + 19] ) << 19 ) |
                       ( uint32_t( m_bits[32 * nword + 20] ) << 20 ) | ( uint32_t( m_bits[32 * nword + 21] ) << 21 ) |
                       ( uint32_t( m_bits[32 * nword + 22] ) << 22 ) | ( uint32_t( m_bits[32 * nword + 23] ) << 23 ) |
                       ( uint32_t( m_bits[32 * nword + 24] ) << 24 ) | ( uint32_t( m_bits[32 * nword + 25] ) << 25 ) |
                       ( uint32_t( m_bits[32 * nword + 26] ) << 26 ) | ( uint32_t( m_bits[32 * nword + 27] ) << 27 ) |
                       ( uint32_t( m_bits[32 * nword + 28] ) << 28 ) | ( uint32_t( m_bits[32 * nword + 29] ) << 29 ) |
                       ( uint32_t( m_bits[32 * nword + 30] ) << 30 ) | ( uint32_t( m_bits[32 * nword + 31] ) << 31 ) )
                                                 : /* partially contained word */
  ...
  inline void setword( unsigned nword, uint32_t word ) {
    if ( 32 * ( nword + 1 ) <= nBits ) {
      m_bits[nword * 32 +
             0]                     = bool( ( word >> 0 ) & 1 ),
             m_bits[nword * 32 + 1] = bool( ( word >> 1 ) & 1 ), m_bits[nword * 32 + 2] = bool( ( word >> 2 ) & 1 ),
             m_bits[nword * 32 + 3] = bool( ( word >> 3 ) & 1 ), m_bits[nword * 32 + 4] = bool( ( word >> 4 ) & 1 ),
             m_bits[nword * 32 + 5] = bool( ( word >> 5 ) & 1 ), m_bits[nword * 32 + 6] = bool( ( word >> 6 ) & 1 ),
             m_bits[nword * 32 + 7] = bool( ( word >> 7 ) & 1 ), m_bits[nword * 32 + 8] = bool( ( word >> 8 ) & 1 ),
             m_bits[nword * 32 + 9] = bool( ( word >> 9 ) & 1 ), m_bits[nword * 32 + 10] = bool( ( word >> 10 ) & 1 ),
             m_bits[nword * 32 + 11] = bool( ( word >> 11 ) & 1 ), m_bits[nword * 32 + 12] = bool( ( word >> 12 ) & 1 ),
             m_bits[nword * 32 + 13] = bool( ( word >> 13 ) & 1 ), m_bits[nword * 32 + 14] = bool( ( word >> 14 ) & 1 ),
             m_bits[nword * 32 + 15] = bool( ( word >> 15 ) & 1 ), m_bits[nword * 32 + 16] = bool( ( word >> 16 ) & 1 ),
             m_bits[nword * 32 + 17] = bool( ( word >> 17 ) & 1 ), m_bits[nword * 32 + 18] = bool( ( word >> 18 ) & 1 ),
             m_bits[nword * 32 + 19] = bool( ( word >> 19 ) & 1 ), m_bits[nword * 32 + 20] = bool( ( word >> 20 ) & 1 ),
             m_bits[nword * 32 + 21] = bool( ( word >> 21 ) & 1 ), m_bits[nword * 32 + 22] = bool( ( word >> 22 ) & 1 ),
             m_bits[nword * 32 + 23] = bool( ( word >> 23 ) & 1 ), m_bits[nword * 32 + 24] = bool( ( word >> 24 ) & 1 ),
             m_bits[nword * 32 + 25] = bool( ( word >> 25 ) & 1 ), m_bits[nword * 32 + 26] = bool( ( word >> 26 ) & 1 ),
             m_bits[nword * 32 + 27] = bool( ( word >> 27 ) & 1 ), m_bits[nword * 32 + 28] = bool( ( word >> 28 ) & 1 ),
             m_bits[nword * 32 + 29] = bool( ( word >> 29 ) & 1 ), m_bits[nword * 32 + 30] = bool( ( word >> 30 ) & 1 ),
  ...
  operator std::string() const {
    ...
    do {
      acc = getword( iw ), ina = std::min( 32u, unsigned( nBits ) - 32 * iw );
      ++iw;
      switch ( inof ) {
      case 0: // no overflow bits from last iteration
        oflo = acc >> 30;
        if ( ina ) {
          retVal += conv( acc & 0x3f );
          acc >>= 6, ina -= std::min( 6u, ina );
          if ( ina ) {
            retVal += conv( acc & 0x3f );
            acc >>= 6, ina -= std::min( 6u, ina );
            if ( ina ) {
  ...
  void set( const std::string& str ) {
    static const auto conv = []( char c ) {
      ...
      if ( 'A' <= c && c <= 'Z' )
        return uint32_t( 0 + c - 'A' );
      else if ( 'a' <= c && c <= 'z' )
        return uint32_t( 26 + c - 'a' );
      else if ( '0' <= c && c <= '9' )
        return uint32_t( 52 + c - '0' );
      else if ( '+' == c )
        return uint32_t( 62 );
      else if ( '/' == c )
        return uint32_t( 63 );
    ...
    while ( ipos < str.size() ) { // convert while there's input left
      ...
      case 2: // oflo contains 2 bits
        acc = oflo | ( ( ipos + 0 < str.size() ) ? ( conv( str[ipos + 0] ) << 2 ) : 0 ) |
      ...
      case 4: // oflo contains 4 bits
        acc = oflo | ( ( ipos + 0 < str.size() ) ? ( conv( str[ipos + 0] ) << 4 ) : 0 ) |
   ...
   */
  BloomFilter( const std::string& str ) { set( str ); }
  ...
  inline size_t size() const {
    return std::round( -double( nBits ) / double( nHashes ) * std::log( 1. - double( popcount() ) / double( nBits ) ) );
  }
    ...
    if ( !canUseVeryFastHash ) {
...
#endif
      for ( unsigned i = 0; i < nHashes; ++i ) { m_bits[( h1 + i * h2 ) % nBits] = true; }
    } else {
      auto h = HASH()( val );
      ...
      if ( 2 == std::numeric_limits<double>::radix &&
           std::pow( static_cast<decltype( h )>( nBits ), int( nHashes ) ) <
               ( static_cast<decltype( h )>( 1 ) << std::numeric_limits<double>::digits ) ) {
        h = static_cast<decltype( h )>( double( h ) *
                                        ( double( std::pow( static_cast<decltype( h )>( nBits ), int( nHashes ) ) ) /
   ...
   */
  template <class RC>
  inline typename std::enable_if<std::is_convertible<decltype( *std::begin( RC() ) ), const T&>::value, void>::type
  insert( const RC& r ) {
   ...
   */
  template <class RC>
  inline BloomFilter( typename std::enable_if<std::is_convertible<decltype( *std::begin( RC() ) ), const T&>::value,
                                              const RC&&>::type r ) {
   ...
   */
  bool operator==( const BloomFilter& other ) const { return m_bits == other.m_bits; }
...
BloomFilter<T, CAPACITY, PNUMER, PDENOM, HASH> operator&( const BloomFilter<T, CAPACITY, PNUMER, PDENOM, HASH>& s1,
