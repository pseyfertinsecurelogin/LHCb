...
namespace ve {
    ...
    namespace detail {
        ...
        struct arr_type {
            ...
            T m_arr[SZ];
            constexpr T& operator[](size_type idx) noexcept
            {
                return m_arr[idx];
            }
            constexpr const T& operator[](size_type idx) const noexcept
            {
                return m_arr[idx];
            }
        ...
        template <typename T, size_t SZ>
        using mask_type =
                typename __vectype<typename _mask_type<sizeof(T)>::type,
                                   SZ>::type;
        ...
        static_assert(!is_indexable<float>::value,
                      "is_indexable not correct");
        static_assert(is_indexable<vect_type<float, 4>>::value,
                      "is_indexable not correct");
        static_assert(!is_linear<empty_for_linear_test>::value,
                      "is_linear not correct");
        static_assert(is_linear<float>::value,
                      "is_linear not correct");
        static_assert(is_linear<vect_type<float, 4>>::value,
                      "is_linear not correct");
        static_assert(!is_vector_like<float>::value,
                      "is_vector_like not correct");
        static_assert(is_vector_like<vect_type<float, 4>>::value,
                      "is_vector_like not correct");
        ...
        template <typename T, size_t SZ, size_t... IDXs>
        constexpr vect_type<T, SZ> shuffle(const vect_type<T, SZ>& v,
                                           const vect_type<T, SZ>& w) noexcept
        {
            static_assert(sizeof...(IDXs) == SZ, "wrong number of indices");
    ...
    namespace detail {
        ...
        T _hcmp(const CMP& cmp, const T& _v) noexcept
        {
            ...
            switch (T::size()) {
            case 1024: __hcmp<512>(cmp, v, SEQ()); // fallthrough
            case 512: __hcmp<256>(cmp, v, SEQ()); // fallthrough
            case 256: __hcmp<128>(cmp, v, SEQ()); // fallthrough
            case 128: __hcmp<64>(cmp, v, SEQ()); // fallthrough
            case 64: __hcmp<32>(cmp, v, SEQ()); // fallthrough
            case 32: __hcmp<16>(cmp, v, SEQ()); // fallthrough
            case 16: __hcmp<8>(cmp, v, SEQ());  // fallthrough
            case 8: __hcmp<4>(cmp, v, SEQ());   // fallthrough
            case 4: __hcmp<2>(cmp, v, SEQ());   // fallthrough
            case 2: __hcmp<1>(cmp, v, SEQ());   // fallthrough
            default: break;
            }
            return v;
        }
        template <std::size_t NROT, std::size_t N, typename T,
                  std::size_t... IDXs>
        constexpr simdv<T, N> hrotate(const simdv<T, N>& v,
        ...
        template <typename T>
        struct is_proxy<T, std::void_t<typename T::proxy_tag>>
                : std::true_type {};
        /// detect if T is proxy type
        template <typename T, typename RET = void>
        using enable_if_proxy_t =
                typename std::enable_if<is_proxy<T>::value, RET>::type;
        ...
        namespace {
            ...
            struct dummy_vector {
                ...
                friend inline dummy_vector
                operator+(const dummy_vector&, const dummy_vector&) noexcept;
                friend inline dummy_vector
                operator-(const dummy_vector&, const dummy_vector&) noexcept;
                friend inline dummy_vector
                operator*(const int, const dummy_vector&) noexcept;
                friend inline dummy_vector
                operator*(const dummy_vector&, const dummy_vector&) noexcept;
                friend inline dummy_vector
                operator/(const dummy_vector&, const dummy_vector&) noexcept;
        ...
        static_assert(is_vector<dummy_vector>::value,
                      "incorrect implementation of is_vector");
        static_assert(!is_vector<int>::value,
                      "incorrect implementation of is_vector");
        static_assert(is_proxy<typename dummy_vector::dummy_proxy>::value,
                      "incorrect implementation of is_proxy");
        static_assert(!is_proxy<int>::value,
                      "incorrect implementation of is_proxy");
        static_assert(is_vector_like<dummy_vector>::value,
                      "incorrect implementation of is_vector_like");
        ...
        using enable_if_mask_t =
                typename std::enable_if<is_mask<M>::value, RET>::type;

        /// extract the value_type from vectors and scalars alike
        template <typename T, unsigned FLAGS = 4 * is_proxy<T>::value +
                                               2 * is_vector<T>::value +
                                               1 * is_vector_like<T>::value>
        struct value_type;
        /// extract value_type from vectors and scalars alike (scalar)
        template <typename T>
        struct value_type<T, 0> { using type = T; };
        /// extract value_type from vectors and scalars alike (vector extension)
        template <typename T>
        struct value_type<T, 1> { using type = decltype(0 + std::declval<const T&>()[0]); };
        /// extract value_type from vectors and scalars alike (vector)
        template <typename T>
        struct value_type<T, 2> { using type = typename T::value_type; };
        /// extract value_type from vectors and scalars alike (vector)
        template <typename T>
        struct value_type<T, 3> { using type = typename T::value_type; };
        /// extract value_type from vectors and scalars alike (vector's proxy)
        template <typename T>
        struct value_type<T, 4> { using type = typename T::value_type; };
        /// extract the value_type from vectors and scalars alike
        template <typename T>
        using value_type_t = typename value_type<T>::type;

        static_assert(std::is_same<value_type_t<int>, int>::value,
                      "incorrect implementation of value_type_t");
        static_assert(std::is_same<value_type_t<dummy_vector>, int>::value,
                      "incorrect implementation of value_type_t");
        static_assert(
                std::is_same<value_type_t<typename dummy_vector::dummy_proxy>,
                             int>::value,
                "incorrect implementation of value_type_t");
        ...
        template <typename S, typename T,
                  bool BOTH_ARITH = std::is_arithmetic<S>::value&&
                          std::is_arithmetic<T>::value>
        struct is_first_common_type : std::false_type {};
        /// given two types, check that the first is the common type of both
        template <typename S, typename T>
        struct is_first_common_type<S, T, true>
                : std::is_same<S, typename std::common_type<S, T>::type> {};

        /// given two types (vectors or scalars), return the common vector type
        template <typename S, typename T, typename SV = value_type_t<S>,
                  typename TV = value_type_t<T>>
        using common_vector_type_t = typename std::conditional<
                is_vector<S>::value,
                typename std::conditional<
                        is_vector<T>::value,
                        typename std::conditional<
                                is_first_common_type<SV, TV>::value, S,
                                T>::type,
                        S>::type,
                typename std::conditional<is_vector<T>::value, T,
                                          void>::type>::type;

        /// return v's value as a mask (for vectors)
        template <typename T>
        constexpr typename std::enable_if<is_vector<T>::value,
                                          typename T::mask_type>::type
        as_mask(const T& v) noexcept
        { return v.as_mask(); }
        /// return v's value as a mask (for scalars)
        template <typename T>
        constexpr typename std::enable_if<std::is_scalar<T>::value &&
        ...
        union _U {
            ...
            template <typename MM = _M,
                      typename = typename std::enable_if<
                              std::is_same<MM, _M>::value &&
                              !std::is_same<_M, _V>::value>::type>
            ...
            template <typename AA = _A,
                      typename std::enable_if<
                              std::is_same<AA, _A>::value &&
                              !std::is_same<_V, _A>::value>::type>
    ...
    public:
        ...
        template <typename V, typename = typename std::enable_if<
                                      detail::is_vector_like<V>::value &&
                                      !detail::is_vector<V>::value &&
                                      std::is_convertible<V, _V>::value &&
                                      (!std::is_same<V, _M>::value ||
                                       (std::is_same<V, _M>::value &&
                                        std::is_same<_V, _M>::value))>::type>
        ...
        template <typename M, typename = M,
                  typename = typename std::enable_if<
                          detail::is_vector_like<M>::value &&
                          !detail::is_vector<M>::value &&
                          std::is_same<M, _M>::value &&
                           !std::is_same<_V, _M>::value>::type>
        ...
        static_assert(sizeof(typename _mask_type<sizeof(T)>::type) ==
                              sizeof(T),
                      "Incorrect mask type");
    ...
    private:
        ...
        using enable_if_mask_t = typename std::enable_if<
                std::is_scalar<S>::value && sizeof(value_type) == sizeof(S) &&
                        std::is_integral<S>::value &&
                        std::is_integral<U>::value,
                TYPE>::type;

        template <typename U, typename V, typename RETVAL = U,
                  bool ALLARITHMETIC = std::is_arithmetic<U>::value&&
                          std::is_arithmetic<V>::value&&
                                  std::is_arithmetic<RETVAL>::value,
                  bool NONEVECTOR = !detail::is_vector<RETVAL>::value &&
                                    !detail::is_vector<U>::value &&
                                    !detail::is_vector<V>::value &&
                                    !detail::is_vector_like<RETVAL>::value &&
                                    !detail::is_vector_like<U>::value &&
                                    !detail::is_vector_like<V>::value>
        ...
        struct _is_non_narrowing<U, V, RETVAL, true, true>
                : std::integral_constant<
                          bool,
                          std::is_same<RETVAL, typename std::common_type<
                                                       U, V>::type>::value ||
                                  std::is_same<
                                          RETVAL,
        ...
        template <typename U, typename V, typename RETVAL = U>
        using is_narrowing = std::integral_constant<
                bool, !_is_non_narrowing<U, V, RETVAL>::value>;
        ...
        template <typename U, typename V,
                  typename RETVAL = detail::common_vector_type_t<U, V>,
                  typename UV = detail::value_type_t<U>,
                  typename VV = detail::value_type_t<V>,
                  bool EXTRACOND = is_non_narrowing<
                          UV, VV, typename RETVAL::value_type>::value>
        using enable_if_non_narrowing2_t = typename std::enable_if<
                // at least one of the arguments is a vector
                (detail::is_vector<U>::value ||
                 detail::is_vector<V>::value) &&
                        // neither is a proxy
                        (!detail::is_proxy<U>::value &&
                         !detail::is_proxy<V>::value) &&
                        // at least one of the arguments is derived from
                        // self_type
                        (std::is_base_of<self_type, U>::value ||
                         std::is_base_of<self_type, V>::value) &&
                        // they value_types must be arithmetic ones
                        std::is_arithmetic<UV>::value &&
                        std::is_arithmetic<VV>::value &&
                        // if we have two different vector arguments, break
                        // the tie
                        ((detail::is_vector<U>::value +
                          detail::is_vector<V>::value) == 1 ||
                         ((detail::is_vector<U>::value +
                           detail::is_vector<V>::value) == 2 &&
                          std::is_base_of<self_type, U>::value)) &&
        ...
        template <typename U, typename V,
                  ...
                  typename UV = detail::value_type_t<U>,
                  typename VV = detail::value_type_t<V>>
        using enable_if_integral2_t = enable_if_non_narrowing2_t<
                U, V, RETVAL, UV, VV,
                std::is_integral<UV>::value && std::is_integral<VV>::value &&
                        is_non_narrowing<UV, VV,
                                         typename RETVAL::value_type>::value>;
         ...
         */
        template <typename U, typename V,
                  typename RETVAL = typename detail::common_vector_type_t<
                          U, V>::mask_type,
                  typename UV = detail::value_type_t<U>,
                  typename VV = detail::value_type_t<V>>
        using enable_if_mask2_t = enable_if_non_narrowing2_t<
                U, V, RETVAL, UV, VV,
                (std::is_integral<UV>::value ||
                 std::is_integral<VV>::value) &&
        ...
        template <typename U, typename V,
                  ...
                  typename UV = detail::value_type_t<U>,
                  typename VV = detail::value_type_t<V>>
        using enable_if_mask3_t = enable_if_non_narrowing2_t<
                U, V, RETVAL, UV, VV,
                sizeof(UV) <= sizeof(typename RETVAL::value_type) &&
                        sizeof(VV) <= sizeof(typename RETVAL::value_type)>;
        ...
        using enable_if_derived_t = typename std::enable_if<
                detail::is_vector<S>::value &&
                        std::is_base_of<self_type, S>::value && EXTRACOND,
                RETVAL>::type;
    ...
    public:
        ...
        constexpr simdv(from_mask /* unused */, const mask_type& m) noexcept
                : simdv(m.m_arr._m)
        {}

        constexpr mask_type as_mask() const noexcept
        {
            return mask_type(m_arr._m);
        }
        ...
        template <typename S, typename = S, typename = S,
                  typename = typename std::enable_if<
                          !detail::is_vector_like<S>::value &&
                          !detail::is_vector<S>::value &&
        ...
        template <typename S, typename = S, typename = S, typename = S,
                  typename = typename std::enable_if<
                          std::is_same<bool, S>::value &&
                          std::is_integral<value_type>::value>::type>
        constexpr simdv(const S& val) noexcept
        {
            for (size_type i = 0; N != i; ++i) m_arr._a[i] = -value_type(val);
        }
        ...
        constexpr simdv(IT first, IT last, const T& val = T()) noexcept
                : simdv(val)
        {
            const size_type jend = (std::size_t(last - first) < N)
        ...
        explicit constexpr simdv(std::initializer_list<S> ilist,
                                 const T& val = T()) noexcept
                : simdv(ilist.begin(), ilist.end(), val)
        {}

        /// from other vector (non-narrowing)
        template <
                typename V, typename = V, typename = V, typename = V,
                typename = V, typename = V,
                typename = typename std::enable_if<
                        detail::is_vector<V>::value &&
                        std::is_convertible<typename V::value_type,
                                            value_type>::value &&
                        N == V::size() &&
                        is_non_narrowing<value_type, typename V::value_type>::
                                value>::type>
        constexpr simdv(const V& v) noexcept : simdv(v.begin(), v.end())
        {}
        /// from other vector (potentially narrowing, therefore explicit)
        template <typename V, typename = V, typename = V, typename = V,
                  typename = V, typename = V, typename = V,
                  typename = typename std::enable_if<
                          detail::is_vector<V>::value &&
                          std::is_convertible<typename V::value_type,
                                              value_type>::value &&
                          N == V::size() &&
                          !is_non_narrowing<value_type,
    ...
    public:
        ...
        public:
            ...
            explicit constexpr const_proxy(_U& arr, size_type idx) noexcept
                    : proxy_base{arr, idx}
            {}
            ...
            {
                return m_arr._v[m_idx];
            }
            friend constexpr const_pointer operator&(const const_proxy& p) noexcept
            { return &p.m_arr._a[p.m_idx]; }
        };
        static_assert(detail::is_proxy<const_proxy>::value,
                      "const_proxy must be proxy");
        ...
        public:
            constexpr explicit proxy(_U& arr, size_type idx) noexcept
                    : const_proxy(arr, idx)
            {}
            ...
            friend constexpr pointer operator&(const proxy& p) noexcept
            { return &p.m_arr._a[p.m_idx]; }
        };
        static_assert(detail::is_proxy<proxy>::value, "proxy must be proxy");

        constexpr proxy operator[](size_type idx) noexcept
        {
        ...
        constexpr const_proxy front() const noexcept
        {
            return operator[](0);
        }
        constexpr proxy back() noexcept { return operator[](N - 1); }
        constexpr const_proxy back() const noexcept
        {
            return operator[](N - 1);
        }
        ...
        template <typename U, typename V>
        friend constexpr enable_if_non_narrowing2_t<U, V, U>&
        operator+=(U& u, const V& v) noexcept
        {
        ...
        template <typename U, typename V>
        friend constexpr enable_if_integral2_t<U, V, U>&
        operator%=(U& u, const V& v) noexcept
        {
            u.m_arr._v %= U(v).m_arr._v;
            return u;
        ...
        template <typename U, typename V>
        friend constexpr enable_if_mask2_t<U, V, U>&
        operator&=(U& u, const V& v) noexcept
        {
        ...
        template <typename U, typename V>
        friend constexpr enable_if_non_narrowing2_t<U, V>
        operator+(const U& u, const V& v) noexcept
        {
        ...
        template <typename U, typename V>
        friend constexpr enable_if_integral2_t<U, V>
        operator%(const U& u, const V& v) noexcept
        {
        ...
        template <typename U, typename V>
        friend constexpr enable_if_mask2_t<U, V>
        operator&(const U& u, const V& v) noexcept
        {
        ...
        template <typename S>
        friend constexpr enable_if_derived_t<S, typename S::mask_type>
        ...
        template <typename U, typename V>
        friend constexpr enable_if_mask3_t<U, V>
        operator==(const U& u, const V& v) noexcept
        {
            ...
            return mask(reinterpret_cast<internal_mask>(type(u).m_arr._v ==
                                                        type(v).m_arr._v));
        }
        template <typename U, typename V>
        friend constexpr enable_if_mask3_t<U, V>
                ...
                gather(const T* base, const simdv<M, N>& offsets) noexcept
        {
            ...
            for (unsigned i = 0; N != i; ++i)
                retVal[i] = base[int32_t(offsets[i])];
            return retVal;
        }

        template <typename M>
        typename std::enable_if<std::is_integral<M>::value &&
                                sizeof(M) == sizeof(uint32_t)>::type
        scatter(T* base, const simdv<M, N>& offsets) noexcept
        {
            for (unsigned i = 0; N != i; ++i)
                base[int32_t(offsets[i])] = operator[](i);
        }
    };
    ...
    template <size_t... IDXS, typename T>
    constexpr detail::enable_if_vector_t<T, T> shuffle(const T& v,
                                                       const T& w) noexcept
    ...
    constexpr typename std::enable_if<
            !detail::is_vector<T>::value && std::is_pod<T>::value, T>::type
    ...
    constexpr typename std::enable_if<
            detail::is_mask<M>::value && detail::is_vector<T>::value &&
                    (sizeof(typename M::value_type) ==
    ...
    constexpr typename std::enable_if<
            detail::is_mask<M>::value && detail::is_vector<S>::value &&
                    !detail::is_vector<T>::value &&
                    (sizeof(typename M::value_type) ==
                     sizeof(typename S::value_type)) &&
                    (M::size() == S::size()) &&
                    std::is_same<typename std::common_type<
                                         typename S::value_type, T>::type,
    ...
    constexpr detail::enable_if_vector_t<T, T> hrotate(const T& v) noexcept
    {
        ...
        return detail::hrotate<NROTATE>(
                v, std::make_index_sequence<T::size()>());
    }
    ...
    constexpr detail::enable_if_vector_t<T, T> haddv(const T& _v) noexcept
    {
        ...
        T v(_v);
        switch (T::size()) {
        case 1024: _hadd<512>(v, SEQ()); // fallthrough
        case 512: _hadd<256>(v, SEQ());  // fallthrough
        case 256: _hadd<128>(v, SEQ());  // fallthrough
        case 128: _hadd<64>(v, SEQ());   // fallthrough
        case 64: _hadd<32>(v, SEQ());    // fallthrough
        case 32: _hadd<16>(v, SEQ());    // fallthrough
        case 16: _hadd<8>(v, SEQ());     // fallthrough
        case 8: _hadd<4>(v, SEQ());      // fallthrough
        case 4: _hadd<2>(v, SEQ());      // fallthrough
        case 2: _hadd<1>(v, SEQ());      // fallthrough
        case 1: break;
        default: UNREACHABLE();
        }
        return v;
    ...
    /// horizontal add - adds up all elements in the vector v
    template <typename T>
    constexpr detail::enable_if_vector_t<T, typename T::value_type>
    hadd(const T& _v) noexcept
    {
        return haddv(_v)[0];
    }
    ...
    {
        return detail::_hcmp(
                [](const auto& v, const auto& w) noexcept { return v < w; },
                _v);
    }
    ...
    constexpr simdv<T, N>
    map(OP op, IT first, IT last,
        T default_value = T()) noexcept(noexcept(op(*first)) &&
                                        noexcept(T(T())))
    {
    ...
    namespace detail { ///< implementation details
        ...
        struct SortStage {
             ...
             *                  as v
             */
            template <std::size_t... IDXPERM, std::size_t... IDXMASK,
                      typename T, std::size_t N, typename... ARGS>
            void operator()(indices<IDXPERM...> /* unused */,
                            indices<IDXMASK...> /* unused */,
                            ve::simdv<T, N>& v,
                            ARGS&... args) const noexcept
            {
                using mask_type = typename ve::simdv<T, N>::mask_type;
                using mask_value_type = typename mask_type::value_type;
                constexpr mask_type flip{
                        mask_value_type(-mask_value_type(bool(IDXMASK)))...};
                const auto vperm = shuffle<IDXPERM...>(v);
                const auto mask = m_cmp(v, vperm) ^ flip;
                v = sel(mask, v, vperm);
                std::forward_as_tuple(
                        ((args = sel(mask, args, shuffle<IDXPERM...>(args))),
                         0)...);
            }
        };
         ...
         */
        template <std::size_t N, typename STAGE, typename... ARGS,
                  std::size_t... IDXs>
        void bitonicsort_step(const STAGE& stage, indices<IDXs...> /* unused */,
                            ARGS&... args) noexcept
        {
            static_assert(0 == (N & (N - 1)), "N must be a power of 2");
            constexpr auto MASK = 2 * N - 1;
            stage(indices<((IDXs & (~MASK)) | (MASK - (IDXs & MASK)))...>(),
                  indices<bool(IDXs & N)...>(), args...);
            switch (N) {
            case 512:
                stage(indices<(IDXs ^ 256)...>(),
                      indices<bool(IDXs & 256)...>(),
                      args...); /* fallthrough */
            case 256:
                stage(indices<(IDXs ^ 128)...>(),
                      indices<bool(IDXs & 128)...>(),
                      args...); /* fallthrough */
            case 128:
                stage(indices<(IDXs ^ 64)...>(),
                      indices<bool(IDXs & 64)...>(),
                      args...); /* fallthrough */
            case 64:
                stage(indices<(IDXs ^ 32)...>(),
                      indices<bool(IDXs & 32)...>(),
                      args...); /* fallthrough */
            case 32:
                stage(indices<(IDXs ^ 16)...>(),
                      indices<bool(IDXs & 16)...>(),
                      args...); /* fallthrough */
            case 16:
                stage(indices<(IDXs ^ 8)...>(), indices<bool(IDXs & 8)...>(),
                      args...); /* fallthrough */
            case 8:
                stage(indices<(IDXs ^ 4)...>(), indices<bool(IDXs & 4)...>(),
                      args...); /* fallthrough */
            case 4:
                stage(indices<(IDXs ^ 2)...>(), indices<bool(IDXs & 2)...>(),
                      args...); /* fallthrough */
            case 2:
                stage(indices<(IDXs ^ 1)...>(), indices<bool(IDXs & 1)...>(),
                      args...); /* fallthrough */
            default: break;
            };
        }
        ...
        void bitonicsort(const STAGE& stage, simdv<T, N>& v, ARGS&... args) noexcept
        {
            ...
            if (N >= 2)
                bitonicsort_step<1>(stage, std::make_index_sequence<N>(), v,
                                    args...);
            if (N >= 4)
                bitonicsort_step<2>(stage, std::make_index_sequence<N>(), v,
                                    args...);
            if (N >= 8)
                bitonicsort_step<4>(stage, std::make_index_sequence<N>(), v,
                                    args...);
            if (N >= 16)
                bitonicsort_step<8>(stage, std::make_index_sequence<N>(), v,
                                    args...);
            if (N >= 32)
                bitonicsort_step<16>(stage, std::make_index_sequence<N>(), v,
                                     args...);
            if (N >= 64)
                bitonicsort_step<32>(stage, std::make_index_sequence<N>(), v,
                                     args...);
            if (N >= 128)
                bitonicsort_step<64>(stage, std::make_index_sequence<N>(), v,
                                     args...);
            if (N >= 256)
                bitonicsort_step<128>(stage, std::make_index_sequence<N>(), v,
                                      args...);
            if (N >= 512)
                bitonicsort_step<256>(stage, std::make_index_sequence<N>(), v,
                                      args...);
            if (N >= 1024)
                bitonicsort_step<512>(stage, std::make_index_sequence<N>(), v,
                                      args...);
        }
    } // namespace detail
    ...
    {
        static_assert(detail::ALL((T::size() == ARGS::size())...),
                      "all arguments must be same size");
        detail::bitonicsort(detail::SortStage<CMP>{std::move(cmp)}, v,
                            args...);
    }
    ...
    namespace detail {
        ...
        void __insertionsort(const iter& start, const iter& end,
                             const compar& cmp, value v)
        {
            iter i = start + 1, j;
            do {
                j = i - 1;
        ...
        void __shellsort_doit(iter start, iter end, compar cmp,
                        val v, const size_t *incs)
        {
                size_t h;
                iter it, jt;
                // skip too large increments
                while ((size_t(1) < *incs) && (size_t(end - start) <= *incs))
                        ++incs;
                while ((h = *incs++) > 1) {
        ...
        static constexpr std::size_t __shellsort_incs[] = {
                                                  11117101561907004825ULL,
    ...
    void sort(IT first, IT last, const CMP& cmp)
    {
        ...
        const std::size_t align_offs =
                static_cast<char*>(static_cast<void*>(&*first)) -
    ...
    namespace detail { /// implementation details
        ...
        using pointee_t =
                typename std::decay<decltype(*std::declval<JT>())>::type;
        ...
        using vecttype_t = typename std::conditional<
                std::is_const<typename std::remove_reference<decltype(
                        *std::declval<JT>())>::type>::value,
                const simdv<pointee_t<JT>, SIMDWIDTH>,
                simdv<pointee_t<JT>, SIMDWIDTH>>::type;
    } // namespace detail
     ...
     */
    template <std::size_t SIMDWIDTH, typename IT, typename OUTIT, typename OP,
              typename... ITS>
    __attribute__((noinline)) OUTIT
    vtransform(IT first, IT last, OUTIT newfirst, OP op,
               ITS... others) noexcept(noexcept(*newfirst = op(*first,
                                                               *others...)))
    {
        using detail::ALL;
        ...
        static_assert(sizeof(pointee_t<IT>) == sizeof(pointee_t<OUTIT>) &&
                              ALL((sizeof(pointee_t<IT>) ==
                                   sizeof(pointee_t<ITS>))...),
                      "sizes of pointees must match");
        // work out mask to check alignment constraints
        constexpr auto alignmask =
                sizeof(typename std::decay<decltype(*first)>::type) *
                        SIMDWIDTH -
                1;
        ...
        if (std::size_t(std::distance(first, last)) >= SIMDWIDTH &&
            ...
            ALL((0 ==
                 (reinterpret_cast<std::size_t>(&*others) & alignmask))...)) {
            // the full vectorised crazyness
            for (; std::size_t(std::distance(first, last)) >= SIMDWIDTH;
                 std::forward_as_tuple((first += SIMDWIDTH, 0),
                                       (newfirst += SIMDWIDTH, 0),
                                       (others += SIMDWIDTH, 0)...)) {
                reinterpret_cast<vecttype_t<OUTIT, SIMDWIDTH>&>(*newfirst) =
                        op(reinterpret_cast<vecttype_t<IT, SIMDWIDTH>&>(
                                   *first),
                           reinterpret_cast<vecttype_t<ITS, SIMDWIDTH>&>(
                                   *others)...);
            }
        }
        for (; last != first; std::forward_as_tuple(
                     (++first, 0), (++newfirst, 0), (++others, 0)...)) {
            *newfirst = op(*first, *others...);
        }
        return newfirst;
    }
