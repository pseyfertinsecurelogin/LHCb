...
namespace LHCb {
  namespace Math {
    ...
    namespace detail {
      ...
      template <typename T, unsigned int N, unsigned int M>
      inline span<const double, N * M>
      to_span( const ROOT::Math::SMatrix<T, N, M, ROOT::Math::MatRepStd<T, N, M>>& x ) {
        return {x.Array(), N * M};
      }

      // Forward declare pointer-to-worker-function type
      template <std::ptrdiff_t N, std::ptrdiff_t M>
      using similarity_t = void ( * )( span<const double, N*( N + 1 ) / 2> Ci, span<const double, N * M> Fi,
    ...
    inline void Similarity( const Gaudi::Matrix5x5& F, const Gaudi::SymMatrix5x5& origin,
                            Gaudi::SymMatrix5x5& target ) {
      ( *detail::similarity_5_5 )( detail::to_span( origin ), detail::to_span( F ), detail::to_span( target ) );
    }

    inline Gaudi::SymMatrix5x5 Similarity( const Gaudi::Matrix5x5& F, const Gaudi::SymMatrix5x5& origin ) {
      Gaudi::SymMatrix5x5 target;
    ...
    // (actually, in the SSE3 and AVX versions it is possible, but not in
    // the generic version -- hence this could be a very confusing bug)
    inline void Similarity( const ROOT::Math::SMatrix<double, 7, 5>& F, const Gaudi::SymMatrix5x5& origin,
    ...
    // 'target' and 'origin' are NOT allowed to be the same object.
    inline void Similarity( const ROOT::Math::SMatrix<double, 5, 2>& F, const Gaudi::SymMatrix2x2& origin,
                            Gaudi::SymMatrix5x5& target ) {
    ...
    namespace detail {
      ...
      // Forward declare pointer-to-worker-function type
      using average_t = bool ( * )( span<const double, 5> X1, span<const double, 15> C1, span<const double, 5> X2,
    ...
    inline bool Average( const Gaudi::Vector5& X1, const Gaudi::SymMatrix5x5& C1, const Gaudi::Vector5& X2,
                         const Gaudi::SymMatrix5x5& C2, Gaudi::Vector5& X, Gaudi::SymMatrix5x5& C ) {
      //
      ...
      //
      return ( *detail::average )( detail::to_span( X1 ), detail::to_span( C1 ), detail::to_span( X2 ),
                                   detail::to_span( C2 ), detail::to_span( X ), detail::to_span( C ) );
    }

    namespace detail {
      // Forward declare pointer-to-worker-function type
      using filter_t = double ( * )( span<double, 5> X, span<double, 15> C, span<const double, 5> Xref,
    ...
    inline double Filter( Gaudi::Vector5& X, Gaudi::SymMatrix5x5& C, const Gaudi::Vector5& Xref,
                          const Gaudi::Matrix1x5& H, double refResidual, double errorMeas2 ) {
      ...
      return ( *detail::filter )( detail::to_span( X ), detail::to_span( C ), detail::to_span( Xref ),
                                  detail::to_span( H ), refResidual, errorMeas2 );
    }
