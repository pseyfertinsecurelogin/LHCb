...
template <typename TYPE>
inline double ModeFunctions::halfSampleWidth(const TYPE start, const TYPE stop){
  // trivial cases
  if (start == stop){
    return 0.0;
  }
  ...
  const auto bestpos = halfSample(start,stop);
  return fabs(*bestpos.first - *bestpos.second);
}

template<typename TYPE>
inline double ModeFunctions::shorth(const TYPE start, const TYPE stop){
  // trivial cases
  if (start == stop) {
    return 0.0;
  }
  ...
  const auto bestpos = halfSample(start,stop);
  const auto halfSize = (unsigned int)((stop-start)/2u);
  return std::accumulate(bestpos.first, bestpos.second,0.0)/double(halfSize);
}

template <typename TYPE>
inline double ModeFunctions::LMS(const TYPE start, const TYPE stop)
{
  // trivial cases
  if (start == stop) {
    return 0.0;
...
template <typename TYPE>
inline double ModeFunctions::generalizedMean(const TYPE start, const TYPE stop, const double power)
{
  double sum = 0.0; 
  ...
  return size != 0u ? std::pow(sum/double(size),1.0/power) : 0.0;
}
