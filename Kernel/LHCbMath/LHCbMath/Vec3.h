...
#define VEC3_SOA_ACCESSOR( name, locX, locY, locZ ) \
  template<typename T>\
  inline Vec3<T> name( const int key ) const { return { locX + key, locY + key, locZ + key }; }\
...
#define VEC3_XY_SOA_ACCESSOR( name, locX, locY, valZ ) \
  template<typename T>\
  inline Vec3<T> name( const int key ) const { return { locX + key, locY + key, valZ }; }\
  template<typename T, typename I> \
  inline Vec3<T> gather_##name( const I& key ) const { \
  ...
  template<typename T>\
  inline Vec3<T> name( const int key, __VA_ARGS__ ) const { return { locX + key, locY + key, locZ + key }; }\
  template<typename T, typename I> \
...
#define VEC3_XY_SOA_ACCESSOR_VAR(name, locX, locY, valZ, ...) \
  template<typename T>\
  inline Vec3<T> name( const int key, __VA_ARGS__ ) const { return { locX + key, locY + key, valZ }; }\
  template<typename T, typename I> \
  inline Vec3<T> gather_##name( const I& key, __VA_ARGS__ ) const { \
    return { gather( locX, key ), gather( locY, key ), valZ }; \
  } \
