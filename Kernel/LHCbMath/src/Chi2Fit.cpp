...
namespace {
  ...
  public: // GSL-specific stuff
    ...
    double fdfddf( const gsl_vector* x, gsl_vector* g,
  ...
  inline Chi2::VE term_chi2( Chi2::VE res, const Chi2::CMPS& cmps, const unsigned short index, const gsl_vector* x ) {
    //
  ...
  inline void term_grad( const Chi2::VE& res, const Chi2::CMPS& cmps, const unsigned short index, const gsl_vector* x,
                         gsl_vector* g ) {
    //
  ...
  inline void term_hesse( const Chi2::VE& res, const Chi2::CMPS& cmps, const unsigned short index, const gsl_vector* x,
                          gsl_matrix* h ) {
    //
    const double rv   = res.value();
    const double rv2  = rv * rv;
  ...
  // ==========================================================================
  double Chi2::fdfddf( const gsl_vector* x, gsl_vector* g, gsl_matrix* h ) const {
    ++m_calls_fdfddf;
    //
    m_points = 0;
  ...
  Chi2::~Chi2() // destructor
  {
    ...
    if ( 0 != m_solution ) {
      gsl_vector_free( m_solution );
      m_solution = 0;
    }
    if ( 0 != m_hessian ) {
      gsl_matrix_free( m_hessian );
      m_hessian = 0;
    }
    if ( 0 != m_covariance ) {
  ...
  StatusCode Chi2::fit() {
    ...
    do {
      ...
      if ( status ) {
        gsl_error( "Chi2Fit error from minimization", __FILE__, __LINE__, status );
        /// break ;                                                  // BREAK
        continue; // BREAK
      }
      //
    ...
    //
    //
    if ( GSL_SUCCESS != status && 27 != status ) { return StatusCode{static_cast<unsigned long>( 300 + status )}; }
    //
...
// ============================================================================
namespace {
  Gaudi::Math::Chi2Fit::VE _adjust_( const Gaudi::Math::Chi2Fit::DATA& data, Gaudi::Math::Chi2Fit::DATA& comp,
...
// ============================================================================
Gaudi::Math::Chi2Fit::Chi2Fit( const Gaudi::Math::Chi2Fit::DATA& data, const Gaudi::Math::Chi2Fit::DATA& cmps )
    //
    : m_data( data )
    , m_cmps( 1, cmps )
    , m_init()
  ...
  if ( m_code.isSuccess() ) {
    m_chi2   = c2.chi2();
    m_calls  = c2.calls_f() + c2.calls_df() + c2.calls_fdf() + c2.calls_fdfddf();
    m_iters  = c2.iters();
    m_points = c2.points();
    //
    if ( 0 != c2.covariance() ) {
...
// ============================================================================
Gaudi::Math::Chi2Fit::Chi2Fit( const Gaudi::Math::Chi2Fit::DATA& data, const Gaudi::Math::Chi2Fit::CMPS& cmps )
    : m_data( data )
    , m_cmps( cmps )
    , m_init()
    //
...
// get the parameter
// ============================================================================
Gaudi::Math::Chi2Fit::VE Gaudi::Math::Chi2Fit::param( const unsigned int index ) const {
  //
  ...
  //
  return VE( gsl_vector_get( v, index ), gsl_matrix_get( m, index, index ) );
  //
}
// ============================================================================
// get all parameters at once
// ============================================================================
Gaudi::Math::Chi2Fit::DATA Gaudi::Math::Chi2Fit::params() const // get all parameters at once
{
  ...
  //
  for ( unsigned int i = 0; i < size(); ++i ) { pars.push_back( param( i ) ); }
  //
  return pars;
}
// ============================================================================
...
// ============================================================================
double Gaudi::Math::Chi2Fit::cov2( const unsigned int i1, const unsigned int i2 ) const {
  //
...
// output
// ============================================================================
std::ostream& Gaudi::Math::Chi2Fit::fillStream( std::ostream& s ) const {
  ...
  if ( m_code.isSuccess() && 0 != m_solu && 0 != m_cov2 ) {
    gsl_vector* v = (gsl_vector*)m_solu;
    gsl_matrix* m = (gsl_matrix*)m_cov2;
    //
    if ( v->size == m->size1 && m->size1 == m->size2 ) {
      for ( unsigned int i = 0; i < size(); ++i ) { s << "  Parameter : " << param( i ) << std::endl; }
      s << "  Solution  : " << ( *v ) << std::endl << "  Covariance: \n" << ( *m ) << std::endl;
      //
    }
  }
  return s;
