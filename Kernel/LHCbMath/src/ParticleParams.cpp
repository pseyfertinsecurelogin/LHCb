...
namespace {
  ...
  void fillCov8( Gaudi::SymMatrix8x8& result, const Gaudi::SymMatrix3x3& posCovMatrix,
                 const Gaudi::SymMatrix4x4& momCovMatrix, const double lenCovMatrix,
                 const Gaudi::Matrix4x3& momPosCovMatrix, const Gaudi::Vector3& lenPosCovMatrix,
    ...
    for ( int imom = 0; imom < 4; ++imom ) {
      for ( int ipos = 0; ipos < 3; ++ipos ) { result( imom + 3, ipos ) = momPosCovMatrix( imom, ipos ); }
    }

    for ( int imom = 0; imom < 4; ++imom ) { result( 7, imom + 3 ) = lenMomCovMatrix( imom ); }

    for ( int ipos = 0; ipos < 3; ++ipos ) { result( 7, ipos ) = lenPosCovMatrix( ipos ); }
  }
  // ==========================================================================
  ...
  */
  ...
  void fillCov5( Gaudi::SymMatrix5x5& result, const Gaudi::Math::LorentzVectorWithError& momentum,
                 const Gaudi::Math::ValueWithError& length, const Gaudi::Vector4& lenMomCov ) {
    result.Place_at( momentum.cov2(), 0, 0 );
    result( 4, 4 ) = length.cov2();
    //
    for ( unsigned int i = 0; i < 4; ++i ) { result( 4, i ) = lenMomCov( i ); }
  }
  // ===========================================================================
} // namespace
// ============================================================================
...
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams( const Gaudi::XYZPoint& position, const Gaudi::LorentzVector& momentum,
                                             const double length, const Gaudi::SymMatrix8x8& cov8 )
    ...
    , m_lenPosCovMatrix()
    , m_lenMomCovMatrix() {
  for ( unsigned int i = 0; i < 3; ++i ) { m_lenPosCovMatrix( i ) = cov8( i, 7 ); }
  for ( unsigned int i = 0; i < 4; ++i ) { m_lenMomCovMatrix( i ) = cov8( i + 3, 7 ); }
}
// ============================================================================
// constructor from a full parameter set
// ============================================================================
...
// constructor from a full parameter set
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams( const Gaudi::Math::Point3DWithError&       position,
                                             const Gaudi::Math::LorentzVectorWithError& momentum,
                                             const Gaudi::Math::ValueWithError& length, const Gaudi::Matrix4x3& posmom,
                                             const Gaudi::Vector3& lenpos, const Gaudi::Vector4& lenmom )
    : m_length( length )
    , m_position( position )
    , m_momentum( momentum )
...
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams( const Gaudi::XYZPoint& position, const Gaudi::LorentzVector& momentum,
                                             const double length, const Gaudi::SymMatrix3x3& poscov,
                                             const Gaudi::SymMatrix4x4& momcov, const double lencov,
                                             const Gaudi::Matrix4x3& posmom, const Gaudi::Vector3& lenpos,
...
// constructor from a full parameter set
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams( const Gaudi::Math::SVectorWithError<8, double>& params )
  ...
  //
  for ( unsigned int i = 0; i < 3; ++i ) { m_lenPosCovMatrix( i ) = params.cov2( i, 7 ); }
  for ( unsigned int i = 0; i < 4; ++i ) { m_lenMomCovMatrix( i ) = params.cov2( i + 3, 7 ); }
}
// ============================================================================
// constructor from a full parameter set
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams( const Gaudi::Vector8& par, const Gaudi::SymMatrix8x8& cov )
    : m_length( cov( 7, 7 ) )
  ...
  //
  for ( unsigned int i = 0; i < 3; ++i ) { m_lenPosCovMatrix( i ) = cov( i, 7 ); }
  for ( unsigned int i = 0; i < 4; ++i ) { m_lenMomCovMatrix( i ) = cov( i + 3, 7 ); }
}
// ============================================================================
// constructor from a full parameter set
// ============================================================================
Gaudi::Math::ParticleParams::ParticleParams()
    : m_length(), m_position(), m_momentum(), m_momPosCovMatrix(), m_lenPosCovMatrix(), m_lenMomCovMatrix() {}
// ============================================================================
// virtual destructor
// ============================================================================
Gaudi::Math::ParticleParams::~ParticleParams() {}
...
//  get all parameters
// ============================================================================
void Gaudi::Math::ParticleParams::all( Gaudi::Math::SVectorWithError<8, double>& a ) const {
  par8( a.value() );
  cov8( a.cov2() );
}
// ============================================================================
...
void Gaudi::Math::ParticleParams::cov8( Gaudi::SymMatrix8x8& cov ) const {
  fillCov8( cov, m_position.cov2(), m_momentum.cov2(), m_length.cov2(), m_momPosCovMatrix, m_lenPosCovMatrix,
            m_lenMomCovMatrix );
}
// ============================================================================
// get all covariances
...
Gaudi::Math::ValueWithError Gaudi::Math::ParticleParams::ctau() const {
  ...
  //
  fillCov5( cov5, m_momentum, m_length, m_lenMomCovMatrix );
  // now: we actually measure momentum and mass both in units of
  // energy. that makes a factor of c difference. in the formula.
  return Gaudi::Math::ValueWithError( tau, ROOT::Math::Similarity( jacobian, cov5 ) );
}
// ========================================================================
 ...
 */
// ========================================================================
double Gaudi::Math::FitMass::fit( const Gaudi::Math::ParticleParams& input, const double mass,
  ...
  fillCov8( cov8, input.position().cov2(), ROOT::Math::Similarity( p4jacobian, input.momentum().cov2() ),
            input.decayLength().cov2(), p4jacobian * input.momPosCov(), input.lenPosCov(),
  ...
  const double new_E = std::sqrt( std::fabs( new_px * new_px + new_py * new_py + new_pz * new_pz + new_m2 ) );
  ...
  output = Gaudi::Math::ParticleParams(
      Gaudi::XYZPoint( new_x, new_y, new_z ), Gaudi::LorentzVector( new_px, new_py, new_pz, new_E ), new_len,
      cov8.Sub<Gaudi::SymMatrix3x3>( 0, 0 ),
      ROOT::Math::Similarity( new_p4jacobian, cov8.Sub<Gaudi::SymMatrix4x4>( 3, 3 ) ), cov8( 7, 7 ),
      new_p4jacobian * cov8.Sub<Gaudi::Matrix4x3>( 3, 0 ), v3, v4 * ROOT::Math::Transpose( new_p4jacobian ) );
 ...
 */
// ============================================================================
double Gaudi::Math::FitMass::fit( const double mass, Gaudi::Math::ParticleParams& particle ) {
 ...
 */
// ============================================================================
Gaudi::Math::ParticleParams Gaudi::Math::FitMass::fit( const Gaudi::Math::ParticleParams& input, const double mass,
                                                       double& chi2 ) {
  Gaudi::Math::ParticleParams fitted;
