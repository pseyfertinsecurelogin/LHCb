...
namespace {
  ...
  decltype(auto) dispatch_fn( const Vtbl& vtbl, Trampoline& t, Args&&... args )
  {
    ...
    const auto i = std::find_if( std::begin(vtbl), std::end(vtbl),
                                 [&level]( const auto j )
...
namespace LHCb {
  namespace Math {
    ...
    namespace dispatch {
      ...
      {
        auto vtbl = { std::make_pair( 8, avx2::similarity_5_1 ),
                      std::make_pair( 7, avx::similarity_5_1 ),
      ...
      {
        auto vtbl = { std::make_pair( 8, avx2::similarity_5_5 ),
                      std::make_pair( 7, avx::similarity_5_5 ),
      ...
      {
        auto vtbl = { std::make_pair( 8, avx2::similarity_5_7 ),
                      std::make_pair( 7, avx::similarity_5_7 ),
      ...
      bool average(const double* X1, const double* C1,
                   ...
                   double *X, double *C )
      {
        auto vtbl = { std::make_pair( 8, avx2::average ),
                      std::make_pair( 7, avx::average ),
      ...
      double filter(double* X, double* C,
                    const double* Xref, const double* H,
                    double refResidual, double errorMeas2 ) {
        auto vtbl = { std::make_pair( 8, avx2::filter ),
                      std::make_pair( 7, avx::filter ),
