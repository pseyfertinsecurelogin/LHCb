...
namespace LHCb::cxx {
  ...
  namespace details {
    ...
    using require_clone_t =
        std::enable_if_t<has_clone_v<Interface, Interface*> || has_clone_v<Interface, std::unique_ptr<Interface>>, T>;
  ...
  public:
    ...
    PolymorphicValue& operator=( PolymorphicValue const& other ) {
      m_ptr = details::clone( other.m_ptr.get() );
      return *this;
    }
    ...
    PolymorphicValue& operator=( std::unique_ptr<Interface>&& other ) noexcept {
    ...
    Interface& operator*() noexcept( noexcept( *m_ptr ) ) {
      assert( m_ptr.get() );
      return *m_ptr;
    }
    Interface const& operator*() const noexcept( noexcept( *m_ptr ) ) {
      assert( m_ptr.get() );
      return *m_ptr;
    }
...
namespace std {
  ...
  struct hash<LHCb::cxx::PolymorphicValue<IFace>> {
    std::size_t operator()( const LHCb::cxx::PolymorphicValue<IFace>& t ) const {
      return hash<const IFace*>{}( t.ptr() );
    };
  };
} // namespace std
