  ...
  namespace Container {
    ...
    public:
      ...
      std::pair<iterator, iterator> range_( Args&&... args ) {
        ...
        return {std::next( std::begin( m_hits ), rng.first ), std::next( std::begin( m_hits ), rng.second )};
      }
      ...
      void insert( I&& b, I&& e, Args&&... args ) {
        ...
        assert( std::none_of( std::next( begin( m_offsets ), subDetectorId + 1 ), std::end( m_offsets ),
                              []( typename Offsets::value_type& offset ) { return offset.first != offset.second; } ) &&
