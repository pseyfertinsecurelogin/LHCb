...
namespace LHCb {
  ...
  namespace Vector {
    ...
    namespace Mem {
      ...
      struct Store {
        typename std::list<std::vector<PRECISION, aligned_allocator<PRECISION, VectorConfiguration::bytewidth()>>>
            backends;
        typename std::list<std::vector<PRECISION, aligned_allocator<PRECISION, VectorConfiguration::bytewidth()>>>::
            iterator currentBackend;
         ...
         */
        inline Store( const size_t& width, const unsigned& numberOfNodes ) : width( width ) {
        ...
        void printState( const std::string& position ) {
          std::cout << "State at " << position
                    << std::endl
                    // << " currentBackend " << ((unsigned) (currentBackend - backends.begin()))
                    << ", currentVector " << currentVector << ", currentElement " << currentElement << std::endl
                    << " backend capacity " << backends.size() << ", element capacity " << elementCapacity
        ...
        inline void align() {
          if ( currentElement != 0 ) {
            ...
            if ( currentVector == vectorCapacity ) {
              ...
              if ( std::next( currentBackend ) == backends.end() ) { backends.emplace_back( elementCapacity * width ); }
        ...
        inline PRECISION* getLastVector() {
          ...
          if ( currentVector == 0 ) { return getPointer( std::prev( currentBackend ), vectorCapacity - 1 ); }
          return getPointer( currentBackend, currentVector - 1 );
        }

        inline PRECISION* getFirstVector() { return backends.front().data(); }
         ...
         */
        inline PRECISION*
        getPointer( const typename std::list<std::vector<
                        PRECISION, aligned_allocator<PRECISION, VectorConfiguration::bytewidth()>>>::iterator& backend,
                    const unsigned& vector, const unsigned& element ) {
          assert( backend != backends.end() and vector < vectorCapacity and
                  element < VectorConfiguration::width<PRECISION>() );
