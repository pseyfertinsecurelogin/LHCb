...
namespace LHCb::Vector::Mem {
  ...
  struct Iterator {
    ...
    Iterator( Store<PRECISION>& storeParam ) : store( &storeParam ) {
      backend        = store->backends.begin();
      vectorCapacity = store->vectorCapacity;
    }

    Iterator( Store<PRECISION>& storeParam, const bool& fromCurrent ) : store( &storeParam ) {
      if ( fromCurrent ) {
    ...
    friend inline std::ostream& operator<<( std::ostream& cout, const Iterator& iterator ) {
    ...
    PRECISION* nextElement() {
      assert( backend != ( store->currentBackend ) or vector != ( store->currentVector ) or
              element != ( store->currentElement ) );
  ...
  struct ReverseIterator {
    ...
    ReverseIterator( Store<PRECISION>& storeParam ) : store( &storeParam ) {
      backend        = store->currentBackend;
      vector         = store->currentVector;
      vectorCapacity = store->vectorCapacity;
    }

    friend inline std::ostream& operator<<( std::ostream& cout, const ReverseIterator& iterator ) {
      if ( iterator.store != nullptr ) {
