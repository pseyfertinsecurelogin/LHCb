...
std::ostream &
LHCb::RichSmartID::dumpBits( std::ostream &s ) const
{
...
std::ostream &
LHCb::RichSmartID::fillStream( std::ostream &s, const bool dumpSmartIDBits ) const
{
  s << "{";
  ...
  if ( isValid() )
  {
    ...
    if ( panelIsSet() )
    {
      const std::string PANEL =
        ( rich() == Rich::Rich1 ? ( panel() == Rich::top ? "Top     " : "Bottom  " ) :
...
void
LHCb::RichSmartID::rangeError( const DataType     value,
...
std::string
LHCb::RichSmartID::toString() const
{
...
void
LHCb::RichSmartID::setPixelSubRow( const DataType pixelSubRow )
{
  if ( HPDID == idType() )
  {
#ifndef NDEBUG
  ...
  else
  {
    ...
    throw GaudiException(
      "MaPMTs cannot have their sub-pixel field set", "*RichSmartID*", StatusCode::FAILURE );
  }
}
