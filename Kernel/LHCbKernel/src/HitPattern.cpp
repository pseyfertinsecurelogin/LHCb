...
namespace LHCb {
  ...
  size_t HitPattern::numVeloStationsOverlap() const {
    return ( ( m_veloRA | m_veloPhiA ) & ( m_veloRC | m_veloPhiC ) ).count();
  }
  ...
  size_t HitPattern::numITOTStationsOverlap() const { return ( hitStations( it() ) & hitStations( ot() ) ).count(); }
  ...
  size_t HitPattern::numVeloHoles() const {
    ...
    if ( velo.any() ) {
      ...
      for ( int n = firstbit + 1; n < lastbit; ++n ) rc += ( veloPhi.test( n ) ? 0 : 1 ) + ( veloR.test( n ) ? 0 : 1 );
  ...
  size_t HitPattern::numTHoles() const {
    std::bitset<Number::NumT> layers = m_itAC | m_itTopBottom | m_ot1stMonoLayer | m_ot2ndMonoLayer;
    size_t                    rc( 0 );
    if ( layers.any() ) {
      int firstbit = firstbitset( layers );
      int lastbit  = lastbitset( layers );
      for ( int n = firstbit + 1; n < lastbit; ++n ) rc += layers.test( n ) ? 0 : 1;
    }
    return rc;
  }
  ...
  bool HitPattern::operator==( const HitPattern& hitPat ) const {
    ...
    return ( m_veloRA == hitPat.veloRA() && m_veloRC == hitPat.veloRC() && m_veloPhiA == hitPat.veloPhiA() &&
             m_veloPhiC == hitPat.veloPhiC() && m_ot1stMonoLayer == hitPat.ot1stMonoLayer() &&
             m_ot2ndMonoLayer == hitPat.ot2ndMonoLayer() && m_itAC == hitPat.itAC() &&
             m_itTopBottom == hitPat.itTopBottom() && m_tt == hitPat.tt() && m_muon == hitPat.muon() );
  }
} // namespace LHCb
