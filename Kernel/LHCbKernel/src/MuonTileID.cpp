...
LHCb::MuonTileID::MuonTileID( const MuonTileID& id, const IMuonLayout& lay, const unsigned int x,
...
std::string LHCb::MuonTileID::toString() const {
  ...
  sprintf( bufnm, "S%u(%u,%u)Q%u,R%u,%u,%u", station(), layout().xGrid(), layout().yGrid(), quarter(), region(), nX(),
           nY() );
