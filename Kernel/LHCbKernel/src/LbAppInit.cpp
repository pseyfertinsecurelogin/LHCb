...
StatusCode LbAppInit::initialize() {
  ...
  if ( sc.isFailure() ) return Error( " Fatal error while retrieving Property AppName " );
  m_appName = value;

  sc = propMgr->getProperty( "AppVersion", value );
  if ( sc.isFailure() ) return Error( " Fatal error while retrieving Property AppVersion " );
  m_appVersion = value;
  ...
  if ( m_memPurgeLimit != -1 ) warning() << "MemPurgeLimit property is obsolete, please remove it" << endmsg;
...
StatusCode LbAppInit::finalize() {
  ...
  if ( ( -1 != m_eventMax ) && ( m_eventMax != eventCounter() ) ) {
    warning() << "Should have processed " << m_eventMax << " events" << endmsg;
    warning() << "==================================================================" << endmsg;
  }
...
void LbAppInit::printEventRun( long long event, int run, std::vector<long int>* seeds, Gaudi::Time time ) const {
  if ( this->okToPrint() ) {
...
StatusCode LbAppInit::initRndm( const std::vector<long int>& seeds ) const {
  ...
  if ( 0 < m_skipFactor ) {
    if ( this->okToPrint() ) { info() << "Skipping " << m_skipFactor << " random numbers" << endmsg; }
