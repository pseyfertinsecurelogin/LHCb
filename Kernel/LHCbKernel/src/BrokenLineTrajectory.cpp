...
void LHCb::BrokenLineTrajectory::push_back( LHCb::LineTraj<double> t ) {
  const auto& i = m_traj.emplace_back( std::move( t ), endRange() ).first;
  setRange( beginRange(), endRange() + ( i.endRange() - i.beginRange() ) );
}

void LHCb::BrokenLineTrajectory::push_front( LHCb::LineTraj<double> t ) {
...
std::pair<const LHCb::LineTraj<double>*, double> LHCb::BrokenLineTrajectory::loc( double mu ) const {
  assert( !m_traj.empty() );
...
LHCb::Trajectory<double>::Point LHCb::BrokenLineTrajectory::position( double s ) const {
  return local( s, []( const LHCb::LineTraj<double>& traj, double t ) { return traj.position( t ); } );
}

LHCb::Trajectory<double>::Vector LHCb::BrokenLineTrajectory::direction( double s ) const {
  return local( s, []( const LHCb::LineTraj<double>& traj, double t ) { return traj.direction( t ); } );
}

LHCb::Trajectory<double>::Vector LHCb::BrokenLineTrajectory::curvature( double ) const { return {0, 0, 0}; }

void LHCb::BrokenLineTrajectory::expansion( double s, Point& p, Vector& dp, Vector& ddp ) const {
  local( s, [&]( const LHCb::LineTraj<double>& traj, double t ) { traj.expansion( t, p, dp, ddp ); } );
}

double LHCb::BrokenLineTrajectory::muEstimate( const Point& p ) const {
  return std::accumulate(
             begin( m_traj ), end( m_traj ), std::pair{0., std::numeric_limits<double>::max()},
             [&p]( std::pair<double, double> est, const auto& traj_mu ) {
               ...
               return ( dist2 < est.second ) ? std::pair{glob_mu + ( loc_mu - loc_traj.beginRange() ), dist2} : est;
             } )
      .first;
}

double LHCb::BrokenLineTrajectory::distTo1stError( double s, double, int pathDirection ) const {
  auto l = loc( s );
  if ( l.first != ( pathDirection >= 0 ? &m_traj.back().first : &m_traj.front().first ) ) {
...
double LHCb::BrokenLineTrajectory::distTo2ndError( double s, double tolerance, int pathDirection ) const {
  return distTo1stError( s, tolerance, pathDirection );
}

std::ostream& LHCb::BrokenLineTrajectory::print( std::ostream& os ) const {
