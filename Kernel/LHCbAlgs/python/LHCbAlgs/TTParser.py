...
def parse(afile):
    ...
    if afile.endswith(".csv"):
        ...
        raise TypeError("I do not understand files of the type " +
                        afile.split('.')[-1])
    f = open(afile)
    if not f:
...
def event_av_dict(TT):
    ...
    if type(TT) is str:
        ...
        raise TypeError(
            "You must send me a list of timing-table-like objects, or a string with the filename, but you sent me a "
            + str(type(TT)))
    ret = {}
    for f in TT:
