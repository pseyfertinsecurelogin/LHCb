...
public:
  inline unsigned int getTCK() const override { return m_triggerTCK; } ///< retrieve TCK
...
private:
  /// Get prescale of algorithm given its instance name (See TCKsh)
  double parameterFromTCK( const std::string& instanceName, const std::string& code, const std::string& sub ) const;
...
double RateFromTCK::parameterFromTCK( const std::string& instanceName, const std::string& code,
  ...
  if ( !tree ) {
    ...
    error() << "Obtained TCK " << std::hex << _tck << " from the database which could not be resolved" << endmsg;
    return rate;
  }
...
//=============================================================================
StatusCode RateFromTCK::runUpdate() { return GaudiTool::runUpdate(); }
