...
//=============================================================================
OfflineRateLimiter::OfflineRateLimiter( const std::string& name, ISvcLocator* pSvcLocator )
    : OfflineDeterministicPrescaler( name, pSvcLocator ) {
  declareProperty( "HltLimiter", m_hltLimiter = "Hlt1MBNoBiasODINFilter", "Reference Hlt rate limiter" );
...
void OfflineRateLimiter::updateRate() {
  ...
  if ( m_accFrac != newacc ) {
    info() << "Rate is now " << m_storedRate << " Hz and precale " << m_storedPrescale << " (=" << triggerRate << " Hz)"
           << " -> Need to reduce by factor " << newacc << " to reach target " << m_rate << " Hz." << endmsg;
    m_accFrac = newacc;
...
void OfflineRateLimiter::updateRateFromTCK() {
  if ( handleTCK() ) {
    ...
    if ( randomRate <= 0 ) {
      warning() << "Rate of " << m_hltLimiter << " in TCK " << std::hex << m_tck << " is " << randomRate << " Hz"
                << endmsg;
    }
...
void OfflineRateLimiter::updatePrescaleFromTCK() {
  if ( handleTCK() ) {
    ...
    if ( prescale <= 0 ) {
      warning() << "Prescale of " << m_prescaler << " in TCK " << std::hex << m_tck << " is " << prescale << endmsg;
    }
