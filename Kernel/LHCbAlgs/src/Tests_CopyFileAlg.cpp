...
namespace LHCbAlgsTests {
  ...
  public:
    CopyFileAlg( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
    ...
    StatusCode execute() override {
      ...
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Copying " << m_source << " -> " << m_destination << endmsg;
