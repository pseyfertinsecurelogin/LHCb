...
namespace {
  struct modify_fn {
    ...
    Properties operator()( Properties props, std::string_view key, std::string_view value ) const {
      auto i = std::find_if( begin( props ), end( props ), [key]( const auto& p ) { return p.first == key; } );
      if ( i == end( props ) ) {
        throw std::invalid_argument( "PropertyConfig: trying to update unknown property '" + std::string{key} + "'" );
      }
      i->second = value;
      return props;
    }
    ...
    Properties operator()( Properties props, std::string_view keyAndValue ) const {
      ...
      if ( c == std::string_view::npos ) throw std::invalid_argument( "PropertyConfig: keyAndValue must contain ':'" );
...
public:
  ...
  PropertyConfig( std::string name, std::string type, kind_t kind, Properties props )
      : m_properties( std::move( props ) ), m_type( std::move( type ) ), m_name( std::move( name ) ), m_kind( kind ) {}

  PropertyConfig( const PropertyConfig& orig, Properties properties )
      : m_properties( std::move( properties ) ), m_type( orig.m_type ), m_name( orig.m_name ), m_kind( orig.m_kind ) {}
