...
class TCK final : public boost::equality_comparable<TCK>, boost::equality_comparable2<TCK, unsigned int> {
public:
  TCK() = default;
  explicit TCK( unsigned int i ) : m_unsigned( i ) { set( i ); }
  explicit TCK( std::string s ) { set( s ); }
  friend bool        operator<( const TCK& lhs, const TCK& rhs ) { return lhs.m_unsigned < rhs.m_unsigned; }
  friend bool        operator==( const TCK& lhs, const TCK& rhs ) { return lhs.m_unsigned == rhs.m_unsigned; }
  friend bool        operator==( const TCK& lhs, unsigned int rhs ) { return lhs.m_unsigned == rhs; }
  friend bool        operator==( unsigned int& lhs, const TCK& rhs ) { return lhs == rhs.m_unsigned; }
  ...
  friend std::ostream& operator<<( std::ostream& os, const TCK& tck ) { return os << tck.str(); }

  // allow Gaudi::Property<TCK> :
  friend std::ostream& toStream( const TCK& tck, std::ostream& os ) { return os << '\'' << tck << '\''; }
