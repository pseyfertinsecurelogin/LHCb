...
try:
    ...
    __schema__ = __imp__.load_source(
        'schema',
        __os__.sep.join(__file__.split(__os__.sep)[:-1] + ['schema.py']))
...
class Summary(__schema__.VTree):
    ...
    def __init__(self, schemafile=__default_schema__, construct_default=True):
        ...
        if construct_default:
            self.__element__ = self.__schema__.create_default(
                self.__schema__.root()).__element__
        else:
    ...
    def __file_exists__(self, GUID, filename, isOutput=False):
        ...
        if (GUID is not None) and (GUID in self.__file_dict__[mother]):
            return self.__file_dict__[mother][GUID]
              ...
              not None) and (filename in self.__file_dict__[mother]):
            return self.__file_dict__[mother][filename]

        return None

    def __file_merger__(self,
                        destination,
                        filename=None,
                        GUID=None,
                        status=None,
        ...
        if status is not None:
            if destination.__element__.attrib['status'] == 'fail':
                ...
                #mult is mult!
                pass
            elif destination.__element__.attrib[
                    'status'] == 'full':  # and status!='full':
    ...
    def __fill_file__(self,
        ...
        return self.__file_merger__(open_file, filename, GUID, status,
                                    addevents, isOutput)
    ...
    def fill_VTree_file(self, file, isOutput=False):
        ...
        if file.__element__.attrib['name'] != "":
            if "LFN:" not in file.__element__.attrib['name'].upper():
                if "PFN:" not in file.__element__.attrib['name'].upper():
                    file.__element__.attrib[
                        'name'] = "PFN:" + file.__element__.attrib['name']
        ...
        #else merge:
        return self.__file_merger__(open_file, filename, GUID,
    ...
    def fill_VTree_counter(self, counter, isLumi=False):
        ...
        if 'VTree' not in str(type(counter)):
            raise TypeError, 'expected VTree, got ' + str(
                type(counter)) + ' instead'
        if counter.tag() not in ['statEntity', __count_tag__]:
            raise TypeError, 'expected counter, got ' + counter.tag(
            ) + ' instead'
        ...
        if name not in self.__count_dict__[mother]:
            counters = __schema__.VTree([
                bt for bt in self.__element__.getchildren() if mother in bt.tag
            ][0], self.__schema__, self, False)
        ...
        if 'statentity' not in counter.tag().lower():
            cnt.__element__.text = str(
                int(cnt.__element__.text) + int(counter.__element__.text))
            return True
    ...
    def fill_memory(self, memory, unit='b'):
        ...
        if not astat or len(astat) == 0:
            ...
            if astat.attrib('unit') != unit:
                raise AttributeError, 'I cannot compare two MemoryMaxima when they have different units! ' + astat.attrib(
                    'unit') + " " + unit
            ...
            for mother in mothers:
                ...
                for amother in self.children(mother):
                    for count in amother.children():
                        self.__count_dict__[mother][count.attrib(
                            'name')] = count
            ...
            for mother in mothers:
                ...
                for amother in self.children(mother):
                    for file in amother.children():
                        if (file.attrib('name') != ""):
                            self.__file_dict__[mother][file.attrib(
                                'name')] = file
                        if (file.attrib('GUID') != ""):
                            self.__file_dict__[mother][file.attrib(
                                'GUID')] = file
...
def Merge(summaries, schemafile=__default_schema__):
    ...
    if type([]) != type(summaries):
        raise TypeError, 'you should send a list into the merger, I got a ' + str(
            type(summaries)) + ' object instead'
    sum_objects = []

    if type("") == type(summaries[0]) and str(
            type(Summary(schemafile=schemafile))) == str(type(summaries[0])):
        raise TypeError, 'you should send strings or Summaries into the merger, I got a ' + str(
            type(summaries[0])) + ' object instead'
    ...
    try:
        ...
        if type("") == type(summaries[0]):
            for asummary in summaries:
                ...
                asummary = __os__.path.expanduser(
                    __os__.path.expandvars(asummary))
                if not __os__.path.exists(asummary):
                    raise IOError, 'file does not exist ' + str(asummary)
                sum_object.__element__ = schemafile.__fast_parse__(
                    asummary).__element__
        ...
        steps = merged.__schema__.Tag_enumeration(
            merged.children('step')[0].tag())
        flag = ''
        for asummary in sum_objects:
        ...
        for asummary in sum_objects:
            ...
            for stat in asummary.children('usage')[0].children('stat'):
                merged.fill_memory(stat.value(),
                                   stat.__element__.attrib['unit'])
            #print 'input'
            ...
            #merge counters
            #print 'counters'
            for cnt in asummary.children('counters')[0].children(
                    __count_tag__):
                #print 'counter'
                #print cnt
            ...
            #merge counters
            #print 'lumiCounters'
            for cnt in asummary.children('lumiCounters')[0].children(
                    __count_tag__):
                name = cnt.__element__.attrib['name']
