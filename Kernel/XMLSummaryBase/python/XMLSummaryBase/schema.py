...
id = 'xxx schema.py,v 1.5 2009-10-07 13:51:18 rlambert Exp $'
...
class VTree(object):
    ...
    def __append_element__(self, child):
        ...
        self.__element__.getchildren()[-1].tail = str(
            self.__element__.text)[:-1]
        if len(self.__element__.getchildren()) > 1:
            if self.__element__.text is None or self.__element__.text.strip(
            ) == '':
                self.__element__.getchildren()[-2].tail = str(
                    self.__element__.text)
        return True
    ...
    def __insert_element__(self, child, index):
        ...
        if real_index != element_size - 1:
            self.__element__.getchildren(
            )[real_index].tail = self.__element__.getchildren()[real_index +
                                                                1].tail
        else:
            self.__element__.getchildren(
            )[real_index].tail = self.__element__.getchildren()[real_index +
    ...
    def xml(self):
        ...
        return self.__schema__.header() + '\n' + __ElementTree__.tostring(
            self.__element__).replace(
                'ns0:',
                self.__schema__.__ns__.rstrip(':') + 'i:').replace(
                    ':ns0',
                    ':' + self.__schema__.__ns__.rstrip(':') + 'i').replace(
                        self.__schema__.__schemafile_long__,
                        self.__schema__.__schemafile_short__)
    ...
    def attrib(self, att=None, val=None):
        ...
        if att is None:
            ...
            for c in self.__element__.attrib:
                list[c] = self.__schema__.__cast_from_tag__(
                    c, self.__element__.attrib[c])
            return list
        #print the given attrib
        if val is None:
            try:
                return self.__schema__.__cast_from_tag__(
                    att, self.__element__.attrib[att])
            except KeyError:
        ...
        self.__element__.attrib[att] = str(
            self.__schema__.Tag_castValue(att, val))
        if self.__schema__.Tag_whitespace(att) == 'collapse':
            self.__element__.attrib[att] = self.__element__.attrib[att].rstrip(
            ).lstrip()
        return True
        #else:
    ...
    def add(self, child, index=None):
        ...
        if name not in self.__schema__.Tag_children(self.tag()):
            ...
            return False
        if self.nChildren(name) == self.__schema__.Tag_nChild(
                self.tag(), name)[1]:
            raise TypeError, 'cannot add' + name + ' to ' + self.tag(
        ...
        if 'VTree' in child.__str__():
            ...
            if index is None:
                return self.__append_element__(
                    self.__schema__.create_default(name))
            else:
                return self.__insert_element__(
                    self.__schema__.create_default(name), index)
    ...
    def remove(self, child):
        ...
        if child.__element__ not in self.__element__.getchildren():
            ...
            return False
        if self.nChildren(child.tag()) == self.__schema__.Tag_nChild(
                self.tag(), child.tag())[0]:
            raise TypeError, 'cannot remove ' + child.tag(
            ) + ' to ' + self.tag(
    ...
    def value(self, val=None):
        ...
        if val is None:
            ...
            if self.__schema__.Tag_whitespace(self.tag()) == 'collapse':
                return self.__schema__.__cast_from_tag__(
                    self.tag(),
                    self.__element__.text.rstrip().lstrip())
            return self.__schema__.__cast_from_tag__(self.tag(),
                                                     self.__element__.text)
        #set the value
...
class Schema(object):
    ...
    def __init__(self, schemafile, ns='xs', root=''):
        ...
        self.__header__ = ''
        self.__schemafile_long__ = __os__.path.expanduser(
            __os__.path.expandvars(schemafile))
        ...
        self.__tagelement__ = {}
        #all known types
        self.__basetypes__ = set([
            "integer", "long", "unsignedLong", "double", "string",
            "normalizedString", "boolean"
        ])
        ...
        if self.__schemafile_short__:
            ...
            self.__rootattribs__[
                "xmlns:" + self.__ns__.split(':')[0] + "i"] = (
                    self.__uri__.split('}')[0].split('{')[-1] + "-instance")
            self.__rootattribs__[self.__ns__.split(':')[0] +
                                 "i:noNamespaceSchemaLocation"] = (
                                     self.__schemafile_long__)
            self.__rootattribs__[self.__uri__.split('}')[0] + "-instance}" +
    ...
    def __check__(self, element):
        ...
        for att, val in element.items():
            ...
            if not self.Tag_hasAttrib(element.tag, att):
                ...
                return False
            if self.Tag_isRoot(
                    element.tag) and 'noNamespaceSchemaLocation' in att:
                if element.attrib[att].split(
                        '/')[-1] != self.__schemafile_long__.split('/')[-1]:
                    raise AttributeError, (
                        'root element ' + element.tag +
                        ' must be from the same schema!! ' + ' attribute ' +
                        att + ' is ' + val + ' versus ' +
                        self.__schemafile_long__)
                    return False
            elif not self.Tag_canHaveValue(att, val):
                raise AttributeError, (
                    'element ' + element.tag + ' cannot have attribute ' + att
                    + ' with value ' + val + ' in the schema')
                return False
        ...
        for att in self.Tag_attribs(element.tag):
            if self.Tag_isAttribRequired(element.tag, att):
                if att not in element.attrib:
                    raise AttributeError, (
                        'element ' + element.tag + ' must have attribute ' +
                        att + ' in the schema')
                    return False
        ...
        for child in element.getchildren():
            ...
            if child.tag not in self.Tag_children(element.tag):
                raise AttributeError, (
                    'element ' + element.tag + 'cannot have child ' + child +
                    ' in the schema')
                return False
                ...
                try:
                    if self.Tag_nChild(element.tag, child)[0] > kiddic[child]:
                        raise AttributeError, ('element ' + element.tag +
                                               ' has not enough copies of ' +
                                               child + ' for the schema')
                        return False
                except KeyError:
                    raise AttributeError, (
                        'element ' + element.tag + ' requires child ' + child +
                        ' for the schema')
        ...
        if __os__.path.exists(parsefile):
            ...
            if rt:
                ...
                for i in list(self.__basetypes__):
                    ...
                    self.__basetypes__.add(self.__ns__ + i)
                    self.__basetypes__.add(self.__uri__ + i)
                    self.__basetypes__.remove(i)
                    self.__type_remove_namespace__[self.__uri__ +
                                                   i] = i.lower()
    ...
    def create_default(self, tag, level=0):
        ...
        if self.Tag_isRoot(tag):
            def_e.attrib[self.__rootattribs__.keys(
            )[1]] = self.__rootattribs__[self.__rootattribs__.keys()[1]]
    ...
    def xml(self):
        '''dump the object to an xml string'''
        return self.__header__ + '\n' + __ElementTree__.tostring(
            self.__tree__.getroot()).replace('ns0:', self.__ns__).replace(
                ':ns0', ':' + self.__ns__.rstrip(':')).replace(
    ...
    def Tag_constraints(self, tag):
        ...
        if (tag not in self.__tags__ and tag not in self.__attribs__
    ...
    def __cast_from_tag__(self, tag, val):
        ...
        if self.Tag_isList(tag):
            ...
            for aval in nval:
                try:
                    ...
                    raise ValueError, 'This list does not accept ' + str(
                        type(aval)) + ' values'
        ...
        if ret is None:
            raise ValueError, 'This tag does not accept ' + str(
                type(aval)) + ' values'
        return ret
    ...
    def Tag_castValue(self, tag, val):
        ...
        if f is not None:
            ...
            if ret != f:
                raise ValueError, 'This value is fixed, and not equal to this one ' + str(
                    ret)
        e = self.Tag_enumeration(tag)
        if e is not None and len(e) > 0:
            #print 'recognised enum'
            if ret not in e:
                raise ValueError, 'This value is not one of the enumerated list ' + str(
                    ret)

        if ret is None:
            raise ValueError, 'This tag ' + tag + ' does not accept ' + str(
                type(val)) + ' values'
        else:
            return ret
