...
StatusCode XMLSummarySvc::initialize() {
  ...
  for ( const auto& conn : connections ) {
    if ( conn && !conn->pfn().empty() ) filenames.push_back( conn->pfn() );
  }

  {
    ...
    for ( const auto& fn : filenames ) {
      PyObject_CallMethod( m_summary, chr( "fill_input" ), chr( "s" ), chr( fn.c_str() ) );
    }
  }
...
StatusCode XMLSummarySvc::finalize() {
  ...
  // const IntegerProperty returnCode=(const IntegerProperty)
  // getProperty("ReturnCode");//Gaudi::Utils::getProperty(&returnCode);
...
void XMLSummarySvc::addCounter( const std::string alg, const std::string name, const StatEntity& count,
...
void XMLSummarySvc::addCounter( const std::string alg, const std::string name, const Stat& count,
                                const Gaudi::CounterSummary::SaveType saveType ) {
...
void XMLSummarySvc::handle( const Incident& incident ) {
  ...
  if ( !isConfigured() ) {
    DEBUG_MSG << "the summary object is no longer available " << incident.type() << " Ignored" << endmsg;
    return;
  }

  ON_VERBOSE if ( incident.type() != m_endIncident && incident.type() != m_beginIncident ) verbose()
      << incident.type() << ":" << incident.source() << endmsg;
  ...
  if ( incident.type() != m_endIncident || m_hasinput ) {
    ...
    ON_VERBOSE if ( incident.type() != m_endIncident && incident.type() != m_beginIncident ) verbose()
        << method << "(" << filename << "," << GUID << "," << status << "," << addevents << ")" << endmsg;

    PyGILGuard gil;
    PyObject_CallMethod( m_summary, chr( method.c_str() ), chr( "s,s,s,d" ), chr( filename.c_str() ),
                         chr( GUID.c_str() ), chr( status.c_str() ), double( addevents ) );
  }
  ...
  if ( incident.type() != m_beginIncident &&
       (
           ...
           ( incident.type() == IncidentType::EndInputFile || incident.type() == IncidentType::FailInputFile ||
             incident.type() == IncidentType::FailOutputFile || incident.type() == IncidentType::BeginInputFile ||
             m_handled.flag() == 1 )
...
StatusCode XMLSummarySvc::fillcounter( const NameStatTypePair& count ) {
  ...
  if ( stat_ent ) {
    ...
    PyObject_CallMethod( m_summary, chr( type_name.c_str() ), chr( "s,d,d,d,d,d" ), chr( count.first.first.c_str() ),
                         double( count.first.second.flag() ), double( count.first.second.nEntries() ),
                         double( count.first.second.flag2() ), double( count.first.second.flagMin() ),
                         double( count.first.second.flagMax() ) );
    // printXML(MSG::VERBOSE).ignore();
    return StatusCode::SUCCESS;
  }
  // log << MSG::VERBOSE << "making stimple counter "  << endmsg;
  PyObject_CallMethod( m_summary, chr( type_name.c_str() ), chr( "s,d" ), chr( count.first.first.c_str() ),
...
StatusCode XMLSummarySvc::fillUsage() {
  ...
  PyGILGuard gil;
  PyObject_CallMethod( m_summary, chr( "fill_memory" ), chr( "d,s" ), double( mem ), chr( "KB" ) );
  return StatusCode::SUCCESS;
}
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
