...
// Constructor
//=============================================================================
HCRawBankDecoderHlt::HCRawBankDecoderHlt( const std::string& name, ISvcLocator* pSvcLocator )
    : Decoder::HistoAlgBase( name, pSvcLocator ) {

  declareProperty( "DigitLocation", m_digitLocation = "Raw/HC/Sums" );

  // Initialize search path, and then call the base method.
  m_rawEventLocations = {LHCb::RawEventLocation::HC, LHCb::RawEventLocation::Default, LHCb::RawEventLocation::Other};
  initRawEventSearch();
}
...
StatusCode HCRawBankDecoderHlt::initialize() {
  ...
  if ( !existDet<Condition>( location ) ) {
    return Error( "Cannot find " + location + " in database", StatusCode::SUCCESS );
  }
  registerCondition( location, m_cond, &HCRawBankDecoderHlt::cacheMapping );
  // First update.
  sc = updMgrSvc()->update( this );
  if ( sc.isFailure() ) { return Error( "Cannot update mapping.", StatusCode::SUCCESS ); }
...
StatusCode HCRawBankDecoderHlt::execute() {
  ...
  if ( banks.empty() ) { return Warning( "Cannot find HC raw banks", StatusCode::SUCCESS ); }
  ...
  for ( const auto& bank : banks ) {
    ...
    if ( version == 2 || version == 3 ) {
      ...
      return Error( "Unknown raw bank version: " + std::to_string( version ), StatusCode::SUCCESS );
    }
  }
...
//=============================================================================
bool HCRawBankDecoderHlt::decode( const LHCb::RawBank& bank, std::vector<int>& sums ) const {
...
StatusCode HCRawBankDecoderHlt::cacheMapping() {
  ...
  for ( unsigned int i = 0; i < nStations; ++i ) {
    ...
    if ( channels.size() < 4 || masks.size() < 4 ) { return Error( "Invalid configuration for station " + st ); }
