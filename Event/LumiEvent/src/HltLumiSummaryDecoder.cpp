...
//=============================================================================
HltLumiSummaryDecoder::HltLumiSummaryDecoder( const std::string& name, ISvcLocator* pSvcLocator )
    : Transformer(
          name, pSvcLocator,
...
HltLumiSummary HltLumiSummaryDecoder::operator()( const RawEvent& event ) const {
  ...
  for ( const auto& ibank : event.banks( RawBank::HltLumiSummary ) ) {
    ...
    if ( msgLevel( MSG::DEBUG ) ) {
      debug() << "Bank size: ";
      debug() << format( "%4d ", ibank->size() ) << "Total Data bank size " << totDataSize << endmsg;
    }
  }
...
StatusCode HltLumiSummaryDecoder::finalize() {
  if ( 0 < m_nbEvents ) {
    info() << "Average event size : " << format( "%7.1f words", m_totDataSize / m_nbEvents ) << endmsg;
  }
