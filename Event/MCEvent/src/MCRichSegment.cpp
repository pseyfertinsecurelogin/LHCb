...
Gaudi::XYZPoint LHCb::MCRichSegment::bestPoint( const double fraction ) const {
  // Check data is OK
  if ( trajectoryPoints().size() < 2 ) {
    throw GaudiException( "Too few trajectory data points", "MCRichSegment", StatusCode::FAILURE );
  }
  ...
  } else { // Need to find which two state points to use
...
Gaudi::XYZVector LHCb::MCRichSegment::bestMomentum( const double fraction ) const {

  // Check data is OK
  if ( trajectoryPoints().size() < 2 || trajectoryMomenta().size() != trajectoryPoints().size() ) {
    throw GaudiException( "Too few trajectory data points", "MCRichSegment", StatusCode::FAILURE );
  }
...
Gaudi::XYZVector LHCb::MCRichSegment::averageMomentum( const unsigned int nsamples ) const {
