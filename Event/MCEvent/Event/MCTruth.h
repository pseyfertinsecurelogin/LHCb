...
private:
  ...
  const DataObject* object( const ContainedObject* p ) const { return ( !p ) ? nullptr : p->parent(); }
...
public:
  ...
  /// Standard templated constructor taking any object pointer as source address
  template <class T>
  MCTruth( const SmartRef<T>& t ) {
    setup( t.target() );
  }
  /// Standard templated constructor taking any object pointer as source address
  template <class T>
  MCTruth( SmartDataPtr<T>& t ) {
    setup( t.ptr() );
  }
  ...
  template <class T>
  StatusCode set( const T* t ) {
    return makeMCTruthLink( m_dobj, object( t ) );
  }
  /// Retrieve Monte Carlo truth information from passed object
  template <class T>
  static TYPE* get( const T* t ) {
    return (TYPE*)accessMCTruth( cast, t );
  }

  /// Adapter: Set Monte Carlo truth link to specified object
  template <class T>
  StatusCode set( SmartDataPtr<T>& t ) {
    return set( t.ptr() );
  }
  /// Adapter: Set Monte Carlo truth link to specified object
  template <class T>
  StatusCode set( const SmartRef<T>& t ) {
    return set( t.target() );
  }
  /// Adapter: Retrieve Monte Carlo truth information from passed object
  template <class T>
  static TYPE* get( SmartDataPtr<T>& t ) {
    return get( t.ptr() );
  }
  /// Adapter: Retrieve Monte Carlo truth information from passed object
  template <class T>
  static TYPE* get( const SmartRef<T>& t ) {
    return get( t.target() );
  }
};
