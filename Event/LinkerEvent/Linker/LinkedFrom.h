...
public:
  ...
  LinkedFrom( IDataProviderSvc* eventSvc, IMessageSvc* msgSvc, std::string containerName ) {
    m_eventSvc       = eventSvc;
    ...
    if ( !links ) {
      ...
      if ( links->targetClassID() != TARGET::classID() && CLID_ContainedObject != TARGET::classID() ) {
        std::ostringstream message;
