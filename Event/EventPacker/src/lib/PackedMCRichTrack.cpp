  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( const auto* track : tracks ) {
      ...
      for ( const auto& S : track->mcSegments() ) {
        ptrack.mcSegments.emplace_back( UNLIKELY( 0 == ver ) ? m_pack.reference32( &ptracks, S->parent(), S->key() )
                                                             : m_pack.reference64( &ptracks, S->parent(), S->key() ) );
      }

      if ( track->mcParticle() ) {
        ptrack.mcParticle =
            ( UNLIKELY( 0 == ver )
                  ? m_pack.reference32( &ptracks, track->mcParticle()->parent(), track->mcParticle()->key() )
                  : m_pack.reference64( &ptracks, track->mcParticle()->parent(), track->mcParticle()->key() ) );
      }
    }
  }
}
