...
void ParticlePacker::pack( const Data& part, PackedData& ppart, PackedDataVector& pparts ) const {
  ...
  if ( 1 == pparts.packingVersion() ) {
    ...
    for ( const auto& E : part.extraInfo() ) {
      pparts.extra().emplace_back( PackedDataVector::PackedExtraInfo( E.first, m_pack.fltPacked( E.second ) ) );
    }
    ppart.lastExtra = pparts.extra().size();

    // end vertex
    if ( part.endVertex() ) {
      ppart.vertex = m_pack.reference64( &pparts, part.endVertex()->parent(), part.endVertex()->key() );
    }

    // protoparticle
    if ( part.proto() ) { ppart.proto = m_pack.reference64( &pparts, part.proto()->parent(), part.proto()->key() ); }

    // daughters
    ppart.firstDaughter = pparts.daughters().size();
    for ( const auto& P : part.daughters() ) {
      if ( P.target() ) { pparts.daughters().push_back( m_pack.reference64( &pparts, P->parent(), P->key() ) ); }
    }
...
void ParticlePacker::pack( const DataVector& parts, PackedDataVector& pparts ) const {
...
void ParticlePacker::unpack( const PackedData& ppart, Data& part, const PackedDataVector& pparts,
  ...
  if ( 0 == pparts.packingVersion() || 1 == pparts.packingVersion() ) {
    ...
    const auto px   = ( isVZero ? m_pack.slope( ppart.lv_px ) * pz : m_pack.energy( ppart.lv_px ) );
    const auto py   = ( isVZero ? m_pack.slope( ppart.lv_py ) * pz : m_pack.energy( ppart.lv_py ) );
    ...
    // reference point
    part.setReferencePoint( Gaudi::XYZPoint( m_pack.position( ppart.refx ), m_pack.position( ppart.refy ),
                                             m_pack.position( ppart.refz ) ) );

    // Mom Cov
    auto&      momCov = *( const_cast<Gaudi::SymMatrix4x4*>( &part.momCovMatrix() ) );
    const auto merr00 = ( isVZero ? m_pack.slope( ppart.momCov00 ) * px : m_pack.energy( ppart.momCov00 ) );
    const auto merr11 = ( isVZero ? m_pack.slope( ppart.momCov11 ) * py : m_pack.energy( ppart.momCov11 ) );
...
StatusCode ParticlePacker::check( const DataVector& dataA, const DataVector& dataB ) const {
...
StatusCode ParticlePacker::check( const Data& dataA, const Data& dataB ) const {
  ...
  // Mass
  ok &= ch.compareEnergies( "MeasuredMass", dataA.measuredMass(), dataB.measuredMass() );

  ok &= ch.compareEnergies( "MeasuredMassError", dataA.measuredMassErr(), dataB.measuredMassErr() );
  ...
  if ( extraSizeOK ) {
    ...
    for ( ; iEA != dataA.extraInfo().end() && iEB != dataB.extraInfo().end(); ++iEA, ++iEB ) {
      ...
      const bool valueOK = ch.compareDoubles( mess.str(), iEA->second, iEB->second, tol );
      ok &= valueOK;
    }
  } else {
