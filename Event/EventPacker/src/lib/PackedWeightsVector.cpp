...
void WeightsVectorPacker::unpack( const PackedDataVector& pweightsV, DataVector& weightsV ) const {
  ...
  if ( isSupportedVer( pVer ) ) {
    ...
    for ( const PackedData& pweights : pweightsV.data() ) {
      ...
      auto& wWeights = *( const_cast<LHCb::WeightsVector::WeightDataVector*>( &weights->weights() ) );
      ...
      for ( auto iW = pweights.firstWeight; iW < pweights.lastWeight; ++iW ) {
        ...
        wWeights.emplace_back(
            LHCb::WeightsVector::WeightData( pweight.key, (float)m_pack.fraction( pweight.weight ) ) );
      }
    }
  }
}
