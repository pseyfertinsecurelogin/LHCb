    ...
    pvert.firstTrack = pverts.refs().size();
    {
      ...
      for ( auto itT = vert.tracks().begin(); vert.tracks().end() != itT; ++itT, ++iW ) {
        if ( *itT ) {
          ...
          parent()
              .Warning( "Null Track SmartRef in '" + LHCb::Packer::Utils::location( vert.parent() ) + "'" )
              .ignore();
        }
      }
    }
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( auto kk = pvert.firstTrack; pvert.lastTrack > kk; ++kk ) {
      ...
      if ( ( 0 != ver && m_pack.hintAndKey64( trk, &pverts, &verts, hintID, tKey ) ) ||
        ...
        const auto weight = (float)( (int)pverts.version() > 1 ? m_pack.fraction( pverts.weights()[kk] ) : 1.0 );
        // save with weight
        vert.addToTracks( ref, weight );
      } else {
        parent().Error( "Corrupt RecVertex Track SmartRef detected." ).ignore();
      }
    }
