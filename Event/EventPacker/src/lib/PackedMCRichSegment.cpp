    ...
    for ( const auto* seg : segs ) {
      ...
      if ( seg->mcParticle() ) {
        pseg.mcParticle = ( UNLIKELY( 0 == ver )
                                ? m_pack.reference32( &psegs, seg->mcParticle()->parent(), seg->mcParticle()->key() )
                                : m_pack.reference64( &psegs, seg->mcParticle()->parent(), seg->mcParticle()->key() ) );
      }

      if ( seg->mcRichTrack() ) {
        pseg.mcRichTrack =
            ( UNLIKELY( 0 == ver )
                  ? m_pack.reference32( &psegs, seg->mcRichTrack()->parent(), seg->mcRichTrack()->key() )
                  : m_pack.reference64( &psegs, seg->mcRichTrack()->parent(), seg->mcRichTrack()->key() ) );
      }

      pseg.mcPhotons.reserve( seg->mcRichOpticalPhotons().size() );
      for ( const auto& P : seg->mcRichOpticalPhotons() ) {
        pseg.mcPhotons.push_back( UNLIKELY( 0 == ver ) ? m_pack.reference32( &psegs, P->parent(), P->key() )
                                                       : m_pack.reference64( &psegs, P->parent(), P->key() ) );
      }

      pseg.mcHits.reserve( seg->mcRichHits().size() );
      for ( const auto& H : seg->mcRichHits() ) {
        pseg.mcHits.push_back( UNLIKELY( 0 == ver ) ? m_pack.reference32( &psegs, H->parent(), H.linkID() )
                                                    : m_pack.reference64( &psegs, H->parent(), H.linkID() ) );
      }
    }
  }
}
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( const auto& pseg : psegs.data() ) {
      ...
      for ( ; ix != pseg.trajPx.end(); ++ix, ++iy, ++iz ) {
        seg->addToTrajectoryPoints(
            Gaudi::XYZPoint( m_pack.position( *ix ), m_pack.position( *iy ), m_pack.position( *iz ) ) );
      }

      auto jx( pseg.trajMx.begin() ), jy( pseg.trajMy.begin() ), jz( pseg.trajMz.begin() );
      for ( ; jx != pseg.trajMx.end(); ++jx, ++jy, ++jz ) {
        seg->addToTrajectoryMomenta(
            Gaudi::XYZVector( m_pack.energy( *jx ), m_pack.energy( *jy ), m_pack.energy( *jz ) ) );
      }
