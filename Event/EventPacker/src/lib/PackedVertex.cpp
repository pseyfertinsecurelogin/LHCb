...
void VertexPacker::pack( const Data& vert, PackedData& pvert, PackedDataVector& pverts ) const {
  if ( isSupportedVer( pverts.packingVersion() ) ) {
    ...
    for ( const auto& P : vert.outgoingParticles() ) {
      if ( P.target() ) {
        pverts.outgoingParticles().push_back( m_pack.reference64( &pverts, P->parent(), P->key() ) );
      }
    }
    ...
    for ( const auto& E : vert.extraInfo() ) { pverts.addExtra( E.first, m_pack.fltPacked( E.second ) ); }
    pvert.lastInfo = pverts.extras().size();
  }
}

void VertexPacker::pack( const DataVector& verts, PackedDataVector& pverts ) const {
...
void VertexPacker::unpack( const PackedData& pvert, Data& vert, const PackedDataVector& pverts,
  ...
  if ( isSupportedVer( pverts.packingVersion() ) ) {
    ...
    vert.setPosition(
        Gaudi::XYZPoint( m_pack.position( pvert.x ), m_pack.position( pvert.y ), m_pack.position( pvert.z ) ) );
...
StatusCode VertexPacker::check( const DataVector& dataA, const DataVector& dataB ) const {
