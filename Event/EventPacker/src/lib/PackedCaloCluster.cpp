  ...
  if ( isSupportedVer( pclus.packingVersion() ) ) {
    ...
    for ( const auto* clu : clus ) {
      ...
      for ( const auto& En : clu->entries() ) {
        ...
        if ( En.digit().target() ) {
          pEnt.digit = m_pack.reference64( &pclus, En.digit()->parent(), En.digit()->key().all() );
        }
...
void CaloClusterPacker::unpack( const PackedDataVector& pclus, DataVector& clus ) const {
  if ( isSupportedVer( pclus.packingVersion() ) ) {
    ...
    for ( const auto& pclu : pclus.data() ) {
      ...
      clu->position().setParameters( CaloP::Parameters( m_pack.position( pclu.pos_x ), m_pack.position( pclu.pos_y ),
                                                        m_pack.energy( pclu.pos_e ) ) );

      clu->position().setCenter( CaloP::Center( m_pack.position( pclu.pos_c0 ), m_pack.position( pclu.pos_c1 ) ) );
      ...
      for ( auto iE = pclu.firstEntry; iE < pclu.lastEntry; ++iE ) {
        ...
        if ( -1 != pEnt.digit ) {
          ...
          if ( m_pack.hintAndKey64( pEnt.digit, &pclus, &clus, hintID, key ) ) {
            ...
            parent().Error( "Corrupt CaloCluster Digit SmartRef found" ).ignore();
          }
        }
...
StatusCode CaloClusterPacker::check( const Data& dataA, const Data& dataB ) const {
  ...
  // 'positions'
  ok &= ch.compareDoubles( "Position-X", dataA.position().x(), dataB.position().x() );
  ok &= ch.compareDoubles( "Position-Y", dataA.position().y(), dataB.position().y() );
  ok &= ch.compareDoubles( "Position-Z", dataA.position().z(), dataB.position().z() );
  ...
  if ( entsSizeOK ) {
    ...
    for ( ; iEA != dataA.entries().end() && iEB != dataB.entries().end(); ++iEA, ++iEB ) {
      ...
      ok &= ch.compareDoubles( "Entry-Fraction", ( *iEA ).fraction(), ( *iEB ).fraction() );
    }
  }
