...
void LHCb::PackedClusters::addSTCluster( const LHCb::STCluster* sCl, const unsigned int key ) {
  m_clusters.emplace_back( PackedCluster() );
  auto& t        = m_clusters.back();
...
void LHCb::PackedClusters::addUTCluster( const LHCb::UTCluster* sCl, const unsigned int key ) {
  m_clusters.emplace_back( PackedCluster() );
  auto& t        = m_clusters.back();
  t.id           = key + sCl->liteCluster().channelID().channelID();
  const int size = ( 4 * sCl->interStripFraction() ) + 0.5;
