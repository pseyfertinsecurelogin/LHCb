  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( const auto* H : hypos ) {
      ...
      for ( const auto& clu : H->clusters() ) {
        if ( clu.target() ) {
          phypos.refs().push_back( UNLIKELY( 0 == ver ) ? m_pack.reference32( &phypos, clu->parent(), clu->key() )
      ...
      for ( const auto& iH : H->hypos() ) {
        if ( iH.target() ) {
          phypos.refs().push_back( UNLIKELY( 0 == ver ) ? m_pack.reference32( &phypos, iH->parent(), iH->key() )
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( const auto& src : phypos.hypos() ) {
      ...
      if ( 0 != src.z ) {
        ...
        pos->setParameters( LHCb::CaloPosition::Parameters( m_pack.position( src.posX ), m_pack.position( src.posY ),
        ...
        pos->setCenter( LHCb::CaloPosition::Center( m_pack.position( src.centX ), m_pack.position( src.centY ) ) );
