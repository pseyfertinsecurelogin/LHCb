...
void RelatedInfoRelationsPacker::pack( const DataVector& rels, PackedDataVector& prels ) const {
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    for ( const auto& rel : rels.relations() ) {
      ...
      prel.reference = m_pack.reference64( &prels, rel.from()->parent(), rel.from()->key() );
      ...
      for ( const auto& data : rMap ) { prels.info().emplace_back( std::make_pair( data.first, data.second ) ); }
    ...
    if ( location.empty() ) {
      ...
      for ( const auto& cont : prels.containers() ) {
        ...
        if ( containerName == location ) {
          ...
          for ( auto kk = cont.first; cont.last > kk; ++kk ) { unpack( prels.relations()[kk], prels, rels ); }
        }
      }
    }
  }
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    if ( UNLIKELY( !srcContainer ) ) {
      ...
      if ( from ) {
        ...
        for ( auto jj = pmap.first; pmap.last > jj; ++jj ) { to.insert( prels.info()[jj] ); }
        // Save the relation
