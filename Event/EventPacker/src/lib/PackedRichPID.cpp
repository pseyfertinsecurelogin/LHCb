...
void RichPIDPacker::pack( const Data& pid, PackedData& ppid, PackedDataVector& ppids ) const {
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    if ( nullptr != pid.track() ) {
      ppid.track = ( UNLIKELY( ver < 3 ) ? m_pack.reference32( &ppids, pid.track()->parent(), pid.track()->key() )
                                         : m_pack.reference64( &ppids, pid.track()->parent(), pid.track()->key() ) );
    }
  }
}

void RichPIDPacker::pack( const DataVector& pids, PackedDataVector& ppids ) const {
  const auto ver = ppids.packingVersion();
  if ( isSupportedVer( ver ) ) {
...
void RichPIDPacker::unpack( const PackedData& ppid, Data& pid, const PackedDataVector& ppids, DataVector& pids ) const {
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    if ( ver > 3 ) pid.setParticleDeltaLL( Rich::Deuteron, (float)m_pack.deltaLL( ppid.dllDe ) );
    if ( -1 != ppid.track ) {
      int hintID( 0 ), key( 0 );
...
void RichPIDPacker::unpack( const PackedDataVector& ppids, DataVector& pids ) const {
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    if ( ver > 3 && 0 == pids.version() ) {
      ...
      if ( !ppids.data().empty() ) {
        parent()
            .Warning( "Incorrect data version 0 for packing version > 3."
                      " Correcting data to version 2.",
                      StatusCode::SUCCESS )
            .ignore();
      }
      // reset to V2
...
StatusCode RichPIDPacker::check( const DataVector& dataA, const DataVector& dataB ) const {
