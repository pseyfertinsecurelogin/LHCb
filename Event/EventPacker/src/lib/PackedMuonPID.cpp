...
void MuonPIDPacker::pack( const Data& pid, PackedData& ppid, PackedDataVector& ppids ) const {
  ...
  if ( isSupportedVer( ver ) ) {
    ...
    if ( pid.idTrack() ) {
      ppid.idtrack =
          ( UNLIKELY( 1 >= ver ) ? m_pack.reference32( &ppids, pid.idTrack()->parent(), pid.idTrack()->key() )
                                 : m_pack.reference64( &ppids, pid.idTrack()->parent(), pid.idTrack()->key() ) );
    }
    if ( pid.muonTrack() ) {
      ppid.mutrack =
          ( UNLIKELY( 1 >= ver ) ? m_pack.reference32( &ppids, pid.muonTrack()->parent(), pid.muonTrack()->key() )
                                 : m_pack.reference64( &ppids, pid.muonTrack()->parent(), pid.muonTrack()->key() ) );
    }
  }
}

void MuonPIDPacker::pack( const DataVector& pids, PackedDataVector& ppids ) const {
  const auto ver = ppids.packingVersion();
  if ( isSupportedVer( ver ) ) {
...
StatusCode MuonPIDPacker::check( const DataVector& dataA, const DataVector& dataB ) const {
