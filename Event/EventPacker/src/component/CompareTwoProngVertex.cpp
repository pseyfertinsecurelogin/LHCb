...
//=============================================================================
CompareTwoProngVertex::CompareTwoProngVertex( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  while ( old->end() != itOld ) {
    ...
    if ( !isOK || MSG::DEBUG >= msgLevel() ) {
      ...
      for ( kk = 0; oExtra.size() != kk; ++kk, ++oIt, ++tIt ) {
        info() << format( "   old Extra %5d %12.4f     new %5d %12.4f", ( *oIt ).first, ( *oIt ).second, ( *tIt ).first,
                          ( *tIt ).second )
               << endmsg;
      }
      //== Compatible PID
