...
//=============================================================================
UnpackTwoProngVertex::UnpackTwoProngVertex( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode UnpackTwoProngVertex::execute() {
  ...
  const auto* dst = getOrCreate<LHCb::PackedTwoProngVertices, LHCb::PackedTwoProngVertices>( m_inputName );

  if ( msgLevel( MSG::DEBUG ) ) debug() << "Size of PackedTwoProngVertices = " << dst->vertices().size() << endmsg;
  ...
  for ( const auto& src : dst->vertices() ) {
    ...
    for ( int kk = src.firstTrack; src.lastTrack > kk; ++kk ) {
      ...
      if ( ( 0 == pVer && pack.hintAndKey32( trk, dst, newTwoProngVertices, hintID, key ) ) ||
        ...
        Error( "Corrupt TwoProngVertex Track SmartRef detected." ).ignore();
      }
    }
