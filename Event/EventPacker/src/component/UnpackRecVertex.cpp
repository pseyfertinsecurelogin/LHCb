...
//=============================================================================
UnpackRecVertex::UnpackRecVertex( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  if ( (int)dst->version() < 2 ) {
    ...
    if ( weightsV ) {
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "Old pre-Weights PVs found. Will load weights vector and update" << endmsg;
      ...
      for ( auto* RV : *newRecVertices ) {
        ...
        if ( weights ) {
          ...
          for ( const auto& TW : trksWeights ) {
            if ( msgLevel( MSG::DEBUG ) )
              debug() << " -> Setting Track " << TW.first->key() << " weight " << TW.second << endmsg;
