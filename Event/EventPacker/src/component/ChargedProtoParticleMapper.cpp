...
Gaudi::Utils::TypeNameString ChargedProtoParticleMapper::algorithmForPath( const std::string& path ) {
  ...
  if ( pathIsHandled( path ) ) {
    ...
    auto sc = joSvc()->addPropertyToCatalogue(
        unpackName, StringProperty( "InputName", packedProtoLocation( streamRoot( path ) ) ) );
    if ( sc )
      sc = joSvc()->addPropertyToCatalogue( unpackName,
                                            StringProperty( "OutputName", protoLocation( streamRoot( path ) ) ) );
    ...
    if ( !m_pidTune.empty() ) {
      ...
      for ( const auto& tkType : m_tkTypes ) {
        for ( const auto& pidType : m_pidTypes ) {
          ...
          if ( sc )
            sc = joSvc()->addPropertyToCatalogue(
                name, StringProperty( "ProtoParticleLocation", protoLocation( streamRoot( path ) ) ) );
    ...
    if ( m_unpackersOutputLevel > 0 ) {
      ...
      if ( sc ) sc = joSvc()->addPropertyToCatalogue( seqName, StringProperty( "OutputLevel", lvl.str() ) );
