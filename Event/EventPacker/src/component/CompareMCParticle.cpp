...
//=============================================================================
CompareMCParticle::CompareMCParticle( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  while ( old->end() != itOld ) {
    ...
    if ( !isOK || MSG::VERBOSE >= msgLevel() ) {
      ...
      debug() << format( "  old pid %6d ", oPart->particleID().pid() ) << " endVert " << oPart->originVertex()
              << endmsg;
      debug() << format( " test pid %6d ", tPart->particleID().pid() ) << " endVert " << tPart->originVertex() << endmsg
              << "  old endVertices ";
