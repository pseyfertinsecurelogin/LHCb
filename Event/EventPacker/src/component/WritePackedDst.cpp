...
StatusCode WritePackedDst::initialize() {
  ...
  for ( std::vector<std::string>::const_iterator itS = m_containers.begin(); m_containers.end() != itS; ++itS ) {
    info() << " '" << *itS << "',";
  }
  info() << endmsg;
...
StatusCode WritePackedDst::execute() {
  ...
  *dataPt++                        = padd->svcType();
  ...
  for ( std::vector<std::string>::const_iterator itC = m_containers.begin(); m_containers.end() != itC; ++itC ) {
    //== Try to get the container, a data object
    DataObject* myObj = getIfExists<DataObject>( *itC );
    if ( !myObj ) { return Warning( "Container " + *itC + " does not exist." ); }
    ...
    if ( LHCb::CLID_PackedTracks == myClID ) { //== PackedTracks
      ...
      for ( LHCb::RecSummary::SummaryData::const_iterator iS = in->summaryData().begin(); iS != in->summaryData().end();
            ++iS ) {
      ...
      warning() << "--- Unknown class ID " << myClID << " for container " << *itC << " --" << endmsg;
      return StatusCode::FAILURE;
    }
  }
...
void WritePackedDst::storeInBlob( PackedBank& pBnk, const void* data, unsigned int nb, unsigned int bSize ) {
  ...
  for ( unsigned int k = 0; iSize > k; ++k ) {
    for ( unsigned int l = 0; nb > l; ++l ) { temp.push_back( *( start + iSize * l + k ) ); }
  }
  ...
  while ( MAXBANK < len ) {
    LHCb::RawBank* bank = m_dst->createBank( m_blobNumber++, LHCb::RawBank::DstData, 0, MAXBANK, start );
    m_dst->adoptBank( bank, true );
    start = start + MAXBANK / 4;
    len -= MAXBANK;
  }
  if ( 0 < len ) {
    LHCb::RawBank* bank = m_dst->createBank( m_blobNumber++, LHCb::RawBank::DstData, 0, len, start );
    m_dst->adoptBank( bank, true );
  }
}
