...
namespace DataPacking {
  ...
  public:
    ...
    inline bool compareMatrices( const std::string& name, const TYPE& a, const TYPE& b,
    ...
    inline bool compareCovMatrices( const std::string& name, const TYPE& a, const TYPE& b,
                                    const boost::array<double, N> tolOnDiag, const double tolOffDiag ) const {
      bool ok = true;
      for ( unsigned int n = 0; n < N; ++n ) {
        for ( unsigned int m = n; m < N; ++m ) {
          ...
          text << name << ":" << n << m;
          if ( m == n ) { // On diagonal
            ok &= compareDoubles( text.str(), std::sqrt( a( n, m ) ), std::sqrt( b( n, m ) ), tolOnDiag[n] );
          } else { // Off diagonal
            const auto testA =
                ( fabs( a( n, n ) ) > 0 && fabs( a( m, m ) ) > 0 ? a( n, m ) / std::sqrt( a( n, n ) * a( m, m ) ) : 0 );
            const auto testB =
                ( fabs( b( n, n ) ) > 0 && fabs( b( m, m ) ) > 0 ? b( n, m ) / std::sqrt( b( n, n ) * b( m, m ) ) : 0 );
    ...
    template <class TYPE>
    inline bool compareInts( const std::string& name, const TYPE& a, const TYPE& b ) const {
      const bool ok = ( a == b );
    ...
    inline bool compareEnergies( const std::string& name, const double& a, const double& b,
                                 const double tol = 5.0e-3 ) const {
      return compareDoubles( name, a, b, tol );
    }

    /// Compare two double 'energy' vectors (e.g. Momentum vectors)
    inline bool compareEnergies( const std::string& name, const Gaudi::XYZVector& a, const Gaudi::XYZVector& b,
                                 const double tol = 5.0e-3 ) const {
      return compareVectors( name, a, b, tol );
    }

    template <class TYPE>
    inline bool comparePointers( const std::string& name, const TYPE* a, const TYPE* b ) const {
      const bool ok = ( a == b );
      if ( UNLIKELY( !ok && parent ) ) {
        parent->warning() << name << " comparison failed :-" << endmsg << " Original = " << a << endmsg
                          << " Unpacked = " << b << endmsg;
