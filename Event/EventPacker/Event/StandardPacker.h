...
public:
  ...
  inline int energy( const double e ) const { return packDouble( e * Packer::ENERGY_SCALE ); }

  /** returns an int for a double position */
  inline int position( const double x ) const { return packDouble( x * Packer::POSITION_SCALE ); }

  /** returns an int for a double slope */
  inline int slope( const double x ) const { return packDouble( x * Packer::SLOPE_SCALE ); }

  /** returns an short int for a double fraction f */
  inline short int fraction( const double f ) const { return shortPackDouble( f * Packer::FRACTION_SCALE ); }
  ...
  inline int mva( const double x ) const { return packDouble( x * Packer::MVA_SCALE ); }

  /** returns an int for a double time (TOF) value */
  inline int time( const double x ) const { return packDouble( x * Packer::TIME_SCALE ); }

  /** returns an int for a double delta log likelihood value */
  inline int deltaLL( const double x ) const { return packDouble( x * Packer::DELTALL_SCALE ); }

  /** returns an int for a double mass */
  int mass( const double mass ) const { return packDouble( mass * Packer::MASS_SCALE ); }
  ...
  /// Returns the 'LinkID'
  inline long linkID( DataObject* out, const DataObject* parent ) const {
  ...
  /// Returns the 'LinkID'
  inline long linkID( DataObject* out, const std::string& targetName ) const {
   ...
   */
  int reference32( DataObject* out, const DataObject* parent, const int key ) const;
   ...
   *  @arg  key : returned by the method .linkID() of the SmartRef
   */
  int reference32( DataObject* out, const std::string& targetName, const int key ) const;

  /// Extracts the key and index from a packed data word
  inline void indexAndKey32( const int data, int& indx, int& key ) const {
    indx = data >> 28;
    key  = data & 0x0FFFFFFF;
  }

  /// Extracts the key and hint from a packed data word
  bool hintAndKey32( const int data, const DataObject* source, DataObject* target, int& hint, int& key ) const;
   ...
   *  @arg  key    : returned by the method .linkID() of the SmartRef
   */
  inline long long reference64( DataObject* out, const DataObject* parent, const int key ) const {
  ...
  inline void indexAndKey64( const long long data, int& indx, int& key ) const {
    indx                     = data >> 32;
  ...
  bool hintAndKey64( const long long data, const DataObject* source, DataObject* target, int& hint, int& key ) const;
