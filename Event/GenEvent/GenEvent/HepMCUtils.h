...
inline bool HepMCUtils::commonTrees( HepMC::GenVertex* V1, const HepMC::GenVertex* V2 ) {
  ...
  for ( iter = V1->vertices_begin( HepMC::descendants ); iter != V1->vertices_end( HepMC::descendants ); ++iter ) {
    if ( V2 == ( *iter ) ) return true;
  }
  for ( iter = V1->vertices_begin( HepMC::ancestors ); iter != V1->vertices_end( HepMC::ancestors ); ++iter ) {
    if ( V2 == ( *iter ) ) return true;
  }
  return false;
}
