...
std::string LHCb::L0DUTrigger::summary() {
  ...
  std::ostringstream s( " " );
  s << description() << "  ===== <** EMULATED DECISION **> = <" << emulatedDecision() << ">" << std::endl;
  return s.str();
}
...
std::string LHCb::L0DUTrigger::description() {
  ...
  GaudiUtils::details::ostream_joiner(
      s << "  ====  Trigger '" << L0DUDecision::Name[m_decisionType] << "|" << m_name << "' = [", m_channels, " || ",
      []( std::ostream& os, const auto& c ) -> std::ostream& {
        return os << L0DUDecision::Name[c.second->decisionType()] << "|" << c.second->name();
      } )
      << "]\n";
  return s.str();
}
