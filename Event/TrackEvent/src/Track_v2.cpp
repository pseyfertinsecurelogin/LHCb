...
namespace LHCb::Event::v2 {
  ...
  const State& Track::closestState( const Gaudi::Plane3D& plane ) const {
    ...
    if ( iter == m_states.end() ) throw GaudiException( "No state closest to plane", "Track.cpp", StatusCode::FAILURE );
    return *iter;
  }
  ...
  State* Track::stateAt( const State::Location& location ) {
    auto iter =
        std::find_if( m_states.begin(), m_states.end(), [&]( const State& s ) { return s.checkLocation( location ); } );
  ...
  Track& Track::addToStates( const State& state ) {
    auto ipos = with_order( useDecreasingOrder{checkFlag( Flag::Backward )}, [&]( auto order ) {
      return std::upper_bound( m_states.begin(), m_states.end(), state, order );
    } );
  ...
  Track& Track::setFlag( Track::Flag const flag, bool const ok ) {
    uint32_t val =
        ( ( static_cast<uint32_t>( flag ) ) << details::trailing_zeros( flagsMasks::flagMask ) ) & flagsMasks::flagMask;
    if ( ok )
      m_flags |= val;
    else
      m_flags &= ~val;
  ...
  std::ostream& Track::fillStream( std::ostream& os ) const {
    ...
    if ( !m_states.empty() ) {
      os << " p  : " << (float)firstState().p() << "\n pt : " << (float)firstState().pt() << "\n " << nStates()
         << " states at z =";
      for ( const auto& s : states() ) { os << " " << s.z(); }
      os << "  :-\n";
