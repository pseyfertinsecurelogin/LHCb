...
namespace LHCb::Event {
  ...
  public:
    ...
    TracksUT(const TracksUT&) = delete;

    TracksUT(TracksUT&& other) {
      m_data = other.m_data;
    ...
    // Refined Velo state
    VEC3_SOA_ACCESSOR(statePos,
                      (float*)&m_data[1*max_tracks],
                      (float*)&m_data[2*max_tracks],
                      (float*)&m_data[3*max_tracks])

    VEC3_XY_SOA_ACCESSOR(stateDir,
    ...
    inline void copy_back(const TracksUT& from, int at, maskT mask) {
