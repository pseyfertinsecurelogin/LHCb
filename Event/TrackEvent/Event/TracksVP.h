...
namespace LHCb::Event {
  ...
  public:
    ...
    TracksVP(const TracksVP&) = delete;

    TracksVP(TracksVP&& other) {
      m_data = other.m_data;
    ...
    VEC3_SOA_ACCESSOR_VAR(statePos,
                          (float*)&m_data[(max_hits+1+state*8  )*max_tracks],
                          (float*)&m_data[(max_hits+1+state*8+1)*max_tracks],
                          (float*)&m_data[(max_hits+1+state*8+2)*max_tracks],
                          int state)

    VEC3_XY_SOA_ACCESSOR_VAR(stateDir,
                             (float*)&m_data[(max_hits+1+state*8+3)*max_tracks],
                             (float*)&m_data[(max_hits+1+state*8+4)*max_tracks],
                             1.f,
                             int state)
    
    VEC3_SOA_ACCESSOR_VAR(stateCov,
                          (float*)&m_data[(max_hits+1+state*8+5)*max_tracks],
                          (float*)&m_data[(max_hits+1+state*8+6)*max_tracks],
                          (float*)&m_data[(max_hits+1+state*8+7)*max_tracks],
                          int state)
    ...
    inline void copy_back(const TracksVP& from, int at, maskT mask) {
