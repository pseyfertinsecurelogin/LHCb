...
namespace TrackFunctor {
  ...
  constexpr auto distanceAlongZ = []( double z0 = 0. ) {
    return details::add_deref(
        [=]( const auto& t1, const auto& t2 ) { return ( ( std::abs( t1.z() - z0 ) < std::abs( t2.z() - z0 ) ) ); } );
  };
  ...
  constexpr auto orderByZ = []( int order = +1 ) {
    return details::add_deref( [=]( const auto& t1, const auto& t2 ) { return order * t1.z() < order * t2.z(); } );
  };
  ...
  constexpr auto increasingByZ = []() {
    return details::add_deref( []( const auto& t1, const auto& t2 ) { return t1.z() < t2.z(); } );
  };
  ...
  public:
    HasKey( ptr_memfun check, E key ) : m_pmf( check ), m_key( key ) {}
    bool operator()( const T& t ) const { return ( t.*m_pmf )( m_key ); }
  ...
  LHCb::State& closestState( LHCb::Track& track, const Fun& fun ) {
    ...
    if ( iter == allstates.end() ) throw GaudiException( "No states", "TrackFunctor.h", StatusCode::FAILURE );
    return *( *iter );
  }
