...
StatusCode ChargedProtoCombineDLLsAlg::initialize() {
  ...
  if ( 0 == m_elCombDll ) Warning( "Not creating Combined DLL for electron hypothesis", StatusCode::SUCCESS ).ignore();
  ...
  if ( 0 == m_deCombDll ) Warning( "Not creating Combined DLL for deuteron hypothesis", StatusCode::SUCCESS ).ignore();
...
StatusCode ChargedProtoCombineDLLsAlg::execute() {
  ...
  for ( auto* proto : *protos ) {
    if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Creating Combined DLLs for ProtoParticle " << proto->key() << endmsg;
