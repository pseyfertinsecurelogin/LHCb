...
namespace LHCb {
  ...
  struct LHCbIDAdder {
    unsigned int operator()( const unsigned int& lhs, const LHCb::LHCbID& rhs ) { return lhs + rhs.lhcbID(); }
  };
  ...
  PrimaryVertex::PrimaryVertex( const LHCb::RecVertex& recvertex, bool refit ) : VertexBase( recvertex ) {
  ...
  int PrimaryVertex::unbias( const std::vector<const LHCb::Track*>& tracksToRemove, LHCb::VertexBase& target ) const {
  ...
  int PrimaryVertex::unbiasedPosition( const std::vector<const LHCb::Track*>& tracksToRemove, Gaudi::XYZPoint& pos,
  ...
  int PrimaryVertex::unbiasedPosition( const std::vector<VeloSegmentID>& tracksToRemove, Gaudi::XYZPoint& pos,
                                       Gaudi::SymMatrix3x3& cov ) const {
    int nremoved( 0 );
    // subtract all the tracks
    auto W  = m_sumhalfD2Chi2DX2;
  ...
  void PrimaryVertex::updateTukeyWeights( double maxchi2 ) {
    const Vector3 pos( position().x(), position().y(), position().z() - m_refZ );
