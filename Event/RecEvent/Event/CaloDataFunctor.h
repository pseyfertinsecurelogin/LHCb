 ...
 *  @date    26/11/1999
 */
namespace LHCb::CaloDataFunctor {
  // ==========================================================================
  /** @class CellID
   *
  ...
  // ==========================================================================
  struct CellID_t {
    template <class TYPE, typename = typename std::enable_if_t<!std::is_pointer_v<TYPE>>>
  ...
  public:
    ...
    bool operator()( const LHCb::CaloClusterEntry& obj ) const {
  ...
  public:
    ...
    double operator()( const TYPE& obj ) const {
  ...
  class Over_Et_Threshold {
    ...
    using ET = EnergyTransverse<DETECTOR>;
  ...
  struct Less_by_Energy_t {
    ...
    bool operator()( const LHCb::CaloClusterEntry& obj1, const LHCb::CaloClusterEntry& obj2 ) const {
  ...
  public:
     ...
     */
    template <class TYPE>
    double operator()( double& Energy, const TYPE& object ) const {
  ...
  public:
     ...
     */
    template <typename TYPE>
    double operator()( double& Energy, const TYPE& obj ) const {
  ...
  inline double clusterEnergy( IT begin, IT end ) {
    return std::accumulate( begin, end, 0., []( double e, const auto& entry ) {
      const LHCb::CaloDigit* digit = entry.digit();
      /// skip nulls and useless digits
      if ( digit && ( ( entry.status() & LHCb::CaloDigitStatus::UseForEnergy ) != 0 ) ) {
  ...
  inline std::pair<IT, IT> clusterCommonDigit( IT begin1, IT end1, IT begin2, IT end2 ) {
    // check arguments
    if ( begin2 == end2 ) { return {end1, end2}; }
    // loop over the sequences
    for ( ; begin1 != end1; ++begin1 ) {
      if ( !( begin1->digit() ) ) { continue; } // skip NULLS !
      auto it2 = std::find_if( begin2, end2, [&]( const auto& i2 ) { return i2.digit() == begin1->digit(); } );
  ...
  public:
     ...
     */
    explicit DigitFromCalo( CaloCellCode::CaloIndex calo ) : m_calo( calo ) {}
     ...
     */
    bool operator()( const LHCb::CaloDigit* digit ) const { return digit && digit->cellID().calo() == m_calo; }

  private:
    CaloCellCode::CaloIndex m_calo;
  };
  // ==========================================================================
  ...
  public:
    ...
    explicit IsFromCalo( CaloCellCode::CaloIndex index ) : m_index( index ) {}
    /// constructor from calorimeter name
    explicit IsFromCalo( const std::string& name ) : IsFromCalo( CaloCellCode::CaloNumFromName( name ) ) {}
    template <class TYPE>
    bool operator()( const TYPE& obj ) const {
  ...
  private:
    CaloCellCode::CaloIndex m_index;
  };
  // ==========================================================================
