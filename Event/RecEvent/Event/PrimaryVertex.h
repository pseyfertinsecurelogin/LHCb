...
namespace LHCb {
  ...
  public:
    ...
    using Vector      = std::vector<PrimaryVertex*>;
    using ConstVector = std::vector<const PrimaryVertex*>;
    ...
    using Selection = SharedObjectsContainer<LHCb::PrimaryVertex>;
    ...
    using TrackContainer = std::vector<LHCb::PrimaryVertexTrack>;

    /// Constructor
    PrimaryVertex( const LHCb::RecVertex& recvertex, bool refit = false );

    /// Default Constructor
    PrimaryVertex() = default;

    /// clone PrimaryVertex
    [[nodiscard]] PrimaryVertex* clone() const override;
    ...
    static VeloSegmentID uniqueVeloSegmentID( const LHCb::Track& ttrack );

    /// return weight of track in vertex
    [[nodiscard]] double weight( const VeloSegmentID& id ) const;

    /// return weight of track in vertex
    [[nodiscard]] double weight( const LHCb::Track& track ) const;

    /// tell whether given track is in the vertex
    [[nodiscard]] bool contains( const VeloSegmentID& id ) const;

    /// tell whether given track is in the vertex
    [[nodiscard]] bool contains( const LHCb::Track& track ) const;
    ...
    [[nodiscard]] float refZ() const;

    /// Retrieve const  Tracks in PV
    [[nodiscard]] const TrackContainer& tracks() const;
  ...
  private:
    ...
    [[nodiscard]] PrimaryVertex::TrackContainer::const_iterator find( const VeloSegmentID& id ) const;
    ...
    float          m_refZ{0.0};        ///< Reference position for track states
    ...
    double         m_sumchi2{0.0};     ///< Cached sum of chi2 contributions of tracks
