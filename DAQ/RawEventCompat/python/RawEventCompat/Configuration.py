...
def _replaceWrap(aloc):
    ...
    for pattern in c.getProp("GenericReplacePatterns"):
        ...
        if pattern in aloc:
            ...
            aloc = aloc.replace(pattern,
                                c.getProp("GenericReplacePatterns")[pattern])
...
def RecombineEventByMap(version,
                        regex=".*",
                        DoD=True,
                        locations=None,
                        recodict=None):
    ...
    try:
        ...
        raise ImportError(
            "You don't have RawEventMapCombiner available, choose another method, or getpack the correct version of DAQ/DAQUtils (v1r5p1, v1r7b, head)"
        )
...
def RecombineWholeEvent(version,
                        DoD=True,
                        regex=".*",
                        locations=None,
                        recodict=None):
    ...
    try:
        ...
        raise ImportError(
            "You don't have RawEventSimpleCombiner available, choose another method, or getpack the correct version of DAQ/DAQUtils (v1r5p1, v1r7b, head)"
        )
    myCombiner = RawEventSimpleCombiner("resurectRawEvent")
    myCombiner.InputRawEventLocations = [
        _replaceWrap(aloc) for aloc in ReverseDict(version, locations)
        if re.match(regex, aloc)
    ]
...
class RecombineRawEvent(ConfigurableUser):
    ...
    _propertyDocDct = {
        "Method":
        "Simple or Map combiner, see the documentation of the methods in __known_methods__, default Simple"  #Simple or Map combiner
        ,
        "Regex":
        "locations or RawBanks to copy. A regular expression such that you can ignore certain locations or bank names. Default .*"  #locations or RawBanks to copy.
        ,
        "Version":
        "Version to copy from, float. Default 2.0"  #version to copy from, can be string or float
    }
    ...
    def __apply_configuration__(self):
        ...
        if version in [0.0, 1.0]:
            raise KeyError(
                "Versions 0.0 and 1.0 anyway have a combined raw event... why would you want to do this? ",
                self.getProp("Version"))
...
class RawEventJuggler(ConfigurableUser):
    ...
    __slots__ = {
        "Input":
        None  #Raw event location to start with
        ,
        "Output":
        None  #Raw Event location to go to
        ,
        "KillInputBanksBefore":
        None  #Regex of banks to kill BEFORE copying around, from input locations
        ,
        "KillInputBanksAfter":
        None  #Regex of banks to kill AFTER copying around, from input locations
        ,
        "KillExtraBanks":
        False  #Bool, whether or not to remove banks which don't exist in target format
        ,
        "KillExtraNodes":
        False  #Bool, whether to remove nodes which don't exist in target format
        ,
        "KillExtraDirectories":
        False  #Kill not only the RawEvent, but also unlink the parent directories. Requires KillExtraNodes.
        ,
        "WriterOptItemList":
        None  #append a writer here to add output locations as OptItemList
        ,
        "WriterItemList":
        None  #append a writer here to add output locations as ItemList
        ,
        "Sequencer":
        None  #send in a sequencer to add the juggling into
        ,
        "DataOnDemand":
        False  #Add juggling into DoD?
        ,
        "Depth":
        "#1",
        "TCK":
                    ...
                    for loc in input_locations:
                        if loc not in output_locations:
                            ...
                            if self.getProp("KillExtraDirectories"):
                                ...
                                if (True not in flags) and (
                                        dloc not in enk.Nodes):
