...
def dirThisFile(afile, causes, result, testname):
    ...
    if not f:
        ...
        result[testname] = result.Quote("File was not openable by ROOT " +
                                        afile)
        return []
    try:
        names = [
            b.GetName().replace("_", "/").lstrip("/").rstrip("/").rstrip(".")
            for b in f.Event.GetListOfBranches()
        ]
    except:
        names = []
    #print names
...
def checkIsSplit(afile, version, causes, result, testname):
    ...
    required = [
        _replaceWrap(r).lstrip("/").rstrip("/")
        for r in ReverseDict(version).keys()
    ]
    shouldbegone = []
    ...
    required = [("Event/" + r).replace("Event/Event/", "Event/")
                for r in required]
    shouldbegone = [("Event/" + r).replace("Event/Event/", "Event/")
                    for r in shouldbegone]
