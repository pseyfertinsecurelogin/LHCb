...
namespace OdinBCIDFilter_details {
    ...
    const char* toString( Comparator_t cmp ) {
        switch(cmp) {
            case Comparator_t::eq : return "==";
            case Comparator_t::neq: return "!=";
            case Comparator_t::lt : return "<";
            case Comparator_t::le : return "<=";
            case Comparator_t::gt : return ">";
            case Comparator_t::ge : return ">=";
        }
        throw "IMPOSSIBLE!";
    }

    std::ostream& toStream( Comparator_t cmp, std::ostream& os) {
        return os << std::quoted( toString(cmp), '\'');
    }

    StatusCode parse(Comparator_t& cmp, const std::string& in) {
        for (Comparator_t ref : { Comparator_t::eq, Comparator_t::neq,
                                  Comparator_t::lt, Comparator_t::le,
                                  Comparator_t::gt, Comparator_t::ge } ) {
            if (in != toString(ref)) continue;
            cmp = ref;
            return StatusCode::SUCCESS;
        }
        if ( in == "=" ) { cmp = Comparator_t::eq; return StatusCode::SUCCESS; }
...
private:
  ...
  Gaudi::Property<OdinBCIDFilter_details::Comparator_t>  m_comparator { this, "Comparator", OdinBCIDFilter_details::Comparator_t::eq };
