  ...
  , m_rawEvt(0)
{
  ...
  declareProperty("KillSourceID", m_sourceKill=0, "Kill bank type with specific source IDs, see KillSourceIDMask.") ;
  declareProperty("KillSourceIDMask", m_sourceKillMask=0, "AND mask for sourceID before comparison with KillSourceID.") ;
}
...
void  bankKiller::killBankType( LHCb::RawBank::BankType bankType,
  ...
  for(std::vector<LHCb::RawBank*>::const_iterator itB = banks.begin() ; itB !=  banks.end() ; ++itB ) {
    if (m_sourceKillMask != 0 &&
        m_sourceKill != ((*itB)->sourceID() & m_sourceKillMask)) {
      continue;
    }
