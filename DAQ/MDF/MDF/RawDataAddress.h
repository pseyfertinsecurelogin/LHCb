...
namespace LHCb {
  ...
  private:
    ...
    RawDataAddress& operator=( const RawDataAddress& ) { return *this; }

  public:
    /// Dummy constructor
    RawDataAddress() : GenericAddress(), m_data( 0, 0 ), m_ownData( 0 ) { m_par[1] = "0 "; }
    /// Copy Constructor
    RawDataAddress( const RawDataAddress& c ) : GenericAddress( c ), m_data( c.m_data ), m_ownData( 0 ) {}
    /// Standard Constructor
    RawDataAddress( long svc, const CLID& clid, const std::string& p1 = "", const std::string& p2 = "",
    ...
    void adoptData( const std::pair<char*, int>& val ) {
      m_data    = val;
      m_ownData = 1;
    }
    /// Access file offset
