...
namespace {
  ...
  long long ip_seek( const Access& /* con */, long long /* offset */, int /* where */ ) { return -1; }
  long long posix_seek( const Access& con, long long offset, int where ) {
    if ( con.ioFuncs && con.ioFuncs->lseek64 ) { return con.ioFuncs->lseek64( con.ioDesc, offset, where ); }
    return -1;
  }
...
LHCb::StreamDescriptor::StreamDescriptor() : m_allocated( 0 ), m_length( 0 ), m_data( 0 ) {}

// Standard Destructor
LHCb::StreamDescriptor::~StreamDescriptor() { allocate( 0 ); }

/// Allocate data block
char* LHCb::StreamDescriptor::allocate( int len ) {
  m_length = 0;
  if ( len > 0 ) {
    if ( len < m_allocated ) { return m_data; }
...
void LHCb::StreamDescriptor::getInetConnection( const std::string& con, std::string& host, Networking::in_addr* ip,
                                                unsigned short& port ) {
  std::string proto;
  ...
  if ( ::isalpha( host[0] ) ) {
    ...
    if ( h ) {
      if ( h->h_addr_list[0] ) { ip->s_addr = *(unsigned long*)h->h_addr_list[0]; }
    }
  } else {
...
Access LHCb::StreamDescriptor::connect( const std::string& specs ) {
  ...
  default:  //  DATA='rfio:/castor/cern.ch/......
    ...
    if ( !proto.empty() ) {
      ...
      if ( result.type == 'F' ) {
        result.ioDesc  = file_open( file.c_str(), O_WRONLY | O_BINARY | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG );
        ...
        if ( io && io->open && io->close && io->write && io->read && io->lseek64 ) {
          result.ioFuncs = io;
          result.ioDesc  = io->open( file.c_str(), O_WRONLY | O_BINARY | O_CREAT | O_TRUNC, S_IRWXU | S_IRWXG );
  ...
  default:
    ...
    if ( !proto.empty() ) {
      ...
      if ( result.type == 'F' ) {
        ...
        if ( proto == "dcap" || proto == "dcache" || proto == "gsidcap" || proto == "castor" || proto == "rfio" ||
             proto == "gfal" ) {
          proto = "root";
          file  = specs;
        } else if ( proto == "raw" || proto == "mdf" ) {
