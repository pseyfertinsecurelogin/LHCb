...
namespace VeloTELL1 {
  ...
  enum dbConfig { STATIC = 0, DYNAMIC = 1 };

  enum sumBeetle { BEETLE_CHIPS = 4 };

  enum sumLink { BEETLE_ALINKS = 4 };

  enum strips { HEADER_STRIPS = 4 };
  ...
  enum fir { FIR_COEFF = 4, REAL_ALINKS = 8 };
  ...
  enum clusterCoding { CLUSTER_CNT = 1, CLUSTER_PER_WORD = 2, ADCS_PER_WORD = 4, ADC_SHIFT = 8, CLUSTER_SHIFT = 16 };

  enum remaindeADCS { ADC_REMAINDER_1 = 1, ADC_REMAINDER_2 = 2 };
  ///------------------------------------------------------------------
