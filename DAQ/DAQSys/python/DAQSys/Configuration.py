...
def SetEvtClock(bank, db=None):
    ...
    if len(odinconfs):
        ...
        EventClockSvc(
            EventTimeDecoder=publicTool.replace("ToolSvc.", "") + ":PUBLIC")
...
class DecodeRawEvent(ConfigurableUser):
    ...
    __slots__ = {
        "DataOnDemand":
        False  #Add decoding into DoD?
        ,
        "Sequencer":
        None  #Add decoding to this sequence?
        ,
        "DecoderDB":
        None  #if none, then decoder_db is used
        ,
        "OverrideInputs":
        None  #use raw event version to override input locations
        ,
        "EvtClockBank":
        "ODIN"  #use the ODIN bank tool to configure the EvtClock Svc, needs to be an option such that we can override it with RecHeader info
    }
                ...
                for loc in locs:
                    if loc in DataOnDemandSvc().AlgMap:
                        ...
                        if testname == v.FullName or (
                                testname.split("/")[0] == testname.split(
