...
thednames = [
    "GaudiSequencer/SomeCrazyAlg", "GaudiSequencer/SomeOtherCrazyAlg",
    "GaudiSequencer/Acounter", "GaudiSequencer/FishSticks",
    "GaudiSequencer/FishSticks2"
]
...
if not myC.isPropertySet("OutputLevel") or not myC.isPropertySet(
        "Members") or myC.getProp("OutputLevel") != 6 or myC.getProp(
            "Members") != ["Blah", "Blah"]:
...
for dec in [
        "L0CaloAlg/L0Calo", "HltTrackReportsDecoder",
        "CaloDigitsFromRaw/EcalFromRaw", "CaloDigitsFromRaw/HcalFromRaw"
]:
    if dec not in DecoderDB:
        raise KeyError(
            "The decoder called " + dec +
            " isn't in the database any more, update options/test-decoder-db.py"
        )
