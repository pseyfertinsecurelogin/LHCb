...
known_extras = [
    'Raw/Prs/Adcs', 'Hlt1/DecReports', 'Hlt2/DecReports', 'Hlt/SelReports',
    'Raw/Velo/PUClustersNZS', 'Hlt1/VertexReports', 'Raw/Hcal/Adcs',
    'Raw/Ecal/Adcs', 'Hlt/VertexReports', 'Raw/Spd/Adcs', 'Hlt2/SelReports',
    'Hlt1/Track/Velo', 'Hlt/DecReports', 'Trig/L0/L0DUData', 'Hlt1/SelReports',
    'Raw/Velo/PUClusters', 'Hlt2/VertexReports', 'Hlt/LumiSummary',
    'Hlt1/SelReports/Candidates', 'Hlt2/SelReports/Candidates',
    'Hlt/VertexReports/PV3D', 'Hlt/VertexReports/ProtoPV3D', 'Rec/TT/Summary',
    'Rec/IT/Summary', 'Rec/UT/Summary'
]
...
def parse(stdout):
    ...
    for block in blocks:
        ...
        algname = lines[1]
        #print lines[2]
        exec ("algprops=" + lines[2].replace(
            "PublicToolHandle", "'PublicToolHandle'+").replace(
    ...
    for dest, v in d1.iteritems():
        ...
        for alg, props in v.iteritems():
            if alg not in d2[dest]:
                ...
                raise KeyError(dest + " uses different algorithm, expected " +
                               alg)
            for prop, val in props.iteritems():
                if prop not in d2[dest][alg]:
                    print props.keys(), d2[dest][alg].keys()
                    raise KeyError(
                        alg + " somehow has two different property lists? ")
...
for dest, algs in diff.iteritems():
    ...
    for alg, props in algs.iteritems():
        for prop, val in props.iteritems():
            ...
            if [dest, alg, prop] in found or prop in ('DataInputs',
                                                      'DataOutputs'):
                continue
