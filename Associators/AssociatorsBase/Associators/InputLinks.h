...
namespace {
  ...
  struct GetObject {
    const Source* operator()( const LHCb::LinksByKey& links, const int linkID, int index ) const {
      if ( linkID != -1 ) {
...
public:
  ...
  InputLinks( const LHCb::LinksByKey& links ) {
    ...
    if ( links.sourceClassID() != Source::classID() && CLID_ContainedObject != Source::classID() ) {
      ...
      message << "Incompatible Source type; template classID is " << Source::classID() << " expected "
              << links.sourceClassID();
      throw GaudiException( message.str(), "LinkedTo", StatusCode::FAILURE );
    }
    if ( links.targetClassID() != Target::classID() ) {
      std::ostringstream message;
      message << "Incompatible Target type; template classID is " << Target::classID() << " expected "
              << links.targetClassID();
      throw GaudiException( message.str(), "LinkedTo", StatusCode::FAILURE );
    }
    ...
    for ( auto [srcKey, refIndex] : links.keyIndex() ) {
      do {
        ...
        if ( target ) { m_keyRelations.i_push( srcKey, target, ref.weight() ); }
  ...
  template <typename Object,
            typename std::enable_if<
                ( !std::is_same<Source, Object>::value && std::is_same<ContainedObject, Object>::value ) ||
                ( !std::is_same<ContainedObject, Object>::value && std::is_same<Source, Object>::value )>::type>
  ...
  const typename LHCb::RelationWeighted2D<Source, Target, double>& relations() const { return m_relations; }

  const typename LHCb::RelationWeighted2D<unsigned int, Target, double>& keyRelations() const { return m_keyRelations; }
