...
StatusCode XmlParserSvc::initialize()
{
  ...
  try {
    ...
    Service::warning() << "Error during Xerces-c Initialization.\n"
              << "  Exception message:"
              << message << endmsg;
  ...
  if ( !m_resolverName.empty() ) {
    ...
    if ( !m_toolSvc ) {
      Service::error() << "Could not locate the ToolSvc " << endmsg;
      return sc;
    }
...
void XmlParserSvc::clearCache()
{
  ...
  for ( auto& i : m_cache ) {
    if ( i.second.lock > 0 ) {
      Service::warning() << "Item in cache with lock count = " << i.second.lock << " " << i.first << endmsg;
    }
  }
...
void XmlParserSvc::releaseDoc( IOVDOMDocument* doc )
{
  ...
  auto it = std::find_if( m_cache.begin(), m_cache.end(),
                          [&]( cacheType::const_reference i ) { return i.second.document.get() == doc; } );
  if ( it == m_cache.end() ) {
    // the item was never cached (parsed from a string) ==> delete it
    delete doc;
  } else {
    if ( --it->second.lock < 0 ) {
      Service::warning() << "XmlParserSvc::release request for an object which wasn't locked!" << endmsg;
      it->second.lock = 0;
    }
  }
}
...
void XmlParserSvc::warning( const xercesc::SAXParseException& exception )
{
  ...
  Service::warning() << "DOM>> File " << aSysId << ", line " << exception.getLineNumber() << ", column "
                     << exception.getColumnNumber() << ": " << aMsg << endmsg;
...
void XmlParserSvc::error( const xercesc::SAXParseException& exception )
{
  ...
  Service::error() << "DOM>> File " << aSysId << ", line " << exception.getLineNumber() << ", column "
                   << exception.getColumnNumber() << ": " << aMsg << endmsg;
  xercesc::XMLString::release( &aSysId );
  xercesc::XMLString::release( &aMsg );
}
...
// CacheItem
// -----------------------------------------------------------------------
IOVDOMDocument* XmlParserSvc::cacheItem( std::string fileName, std::unique_ptr<IOVDOMDocument> document )
{
  ...
  if ( m_maxDocNbInCache <= m_cache.size() ) {
    ...
    for ( auto it = m_cache.begin(); it != m_cache.end(); ++it ) {
      unsigned long score = it->second.birthDate + m_cacheBehavior * it->second.utility;
    ...
    if ( m_cache.end() == winner ) {
      ...
      Service::warning() << "The cache is full and I cannot delete anything: I increase the max size to "
