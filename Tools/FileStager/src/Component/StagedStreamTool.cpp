...
//=============================================================================
StagedStreamTool::StagedStreamTool( const std::string& type, const std::string& name, const IInterface* parent )
...
StatusCode StagedStreamTool::addStreams( const StreamSpecs& inputs ) {
  ...
  for ( const std::string& descriptor : inputs ) {
    ...
    } catch ( const std::exception& ) { return StatusCode::FAILURE; }
  }
  ...
  for ( const Descriptor& descriptor : descriptors ) {
    if ( descriptor.type() == LFN ) {
      ...
      debug() << "Adding: " << descriptor.descriptor() << " to the list of files to be staged." << endmsg;
      files.push_back( descriptor.descriptor() );
    }
  }

  if ( status.isSuccess() ) { status = m_stagerSvc->addFiles( files ); }
...
//=============================================================================
StagedStreamTool::Descriptor StagedStreamTool::extractFilename( const std::string& descriptor ) {
  ...
  if ( boost::regex_search( descriptor.begin(), descriptor.end(), match, m_regex, flags ) ) {
    if ( match.size() >= 1 ) {
      ...
      error() << "could not extract filename from descriptor: " << descriptor << endmsg;
      throw std::exception();
    }
  } else {
