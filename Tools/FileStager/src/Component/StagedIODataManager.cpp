...
namespace Gaudi {
  ...
  StatusCode StagedIODataManager::initialize() {
    ...
    if ( !m_catalog.isValid() ) {
      log << MSG::ERROR << "Unable to localize interface IFileCatalog from service:" << m_catalogSvcName.value()
          << endmsg;
  ...
  StagedIODataManager::Connections StagedIODataManager::connections( const IInterface* owner ) const {
    ...
    for ( ConnectionMap::const_iterator i = m_connectionMap.begin(); i != m_connectionMap.end(); ++i ) {
      ...
      if ( 0 == owner || c->owner() == owner ) conns.push_back( c );
    }
    MsgStream log( msgSvc(), name() );
    ...
    if ( con ) {
      ...
      if ( j != m_fidMap.end() ) {
        ...
        while ( j != m_fidMap.end() ) {
          ...
          if ( j->second == j->first ) { break; }
          j = m_fidMap.find( j->second );
        }
        if ( j != m_fidMap.end() ) { fid = j->second; }
      ...
      if ( sc.isSuccess() && e->ioType == Connection::READ ) {
        ...
        for ( auto& i : m_connectionMap ) {
          ...
          if ( e->connection != c && c->isConnected() && !i.second->keepOpen ) {
            c->ageFile();
            if ( c->age() > m_ageLimit.value() ) { to_retire.push_back( i.second ); }
          }
        }
        if ( !to_retire.empty() ) {
          MsgStream log( msgSvc(), name() );
  ...
  //=============================================================================
  StatusCode StagedIODataManager::connectDataIO( int typ, IoType rw, CSTR dataset, CSTR technology, bool keep_open,
    ...
    try {
      ...
      if ( j == m_fidMap.end() ) {
        ...
        case LFN:
          ...
          log << MSG::DEBUG << "connectDataIO (LFN) fid: " << fid << endmsg;
          break;
        case PFN:
          ...
          log << MSG::DEBUG << "connectDataIO (PFN) fid: " << fid << endmsg;
          break;
        }
      } else {
        fid = ( *j ).second;
      ...
      if ( typ == PFN ) {
        ...
        if ( fi == m_connectionMap.end() ) {
          log << MSG::DEBUG << "connectDataIO (PFN): no entry in connectionMap" << endmsg;
          ...
          log << MSG::DEBUG << "connectDataIO (PFN)" << endmsg << "connection fid: " << connection->fid() << endmsg
              << "fid: " << fid << endmsg << "dsn: " << dsn << endmsg;
          return S_OK;
        } else {
          // Here we open the file!
          Connection* c = fi->second->connection;
          log << MSG::DEBUG << "connectDataIO (PFN): using entry in connectionMap" << endmsg
              << "connection fid: " << c->fid() << endmsg << "fid: " << fid << endmsg << "dsn: " << dsn << endmsg;
