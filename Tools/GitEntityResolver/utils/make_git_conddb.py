...
def extract_tags_infos(notes, partition):
    ...
    committers = dict((el.find('lhcb:name', ns).text, '{0} <{1}>'.format(
        el.find('lhcb:name', ns).text,
...
def main():
    ...
    parser.add_option(
        '--tag-prefix', help='prefix to use for the generated tags')
    parser.add_option(
        '--clean-iovs',
        action='store_true',
        dest='clean_iovs',
    ...
    if notes is None:
        tags_infos = dict((opts.tags[i],
                           ('1970-01-01T00:00:00', 'nobody <nobody@no.where>',
    ...
    def tag_time(t):
        ...
        t = int(
            root_node.tagInsertionTime(t).coralTimeStamp().total_nanoseconds()
            * 1.e-9)
    ...
    if os.path.exists(repo_dir):
        ...
        existing_tags = set(
            check_output(['git', 'tag'], cwd=repo_dir).splitlines())
        tags_to_copy = [
            t for t in tags_to_copy
            if (opts.tag_prefix + t.name) not in existing_tags
        ]
    else:
    ...
    single_version_nodes = set(
        n for n in db.getAllNodes() if db.isSingleVersionFolder(n))
    ...
    for count, tag in enumerate(tags_to_copy, 1):
        ...
        for node in nodes:
            ...
            # print node
            data = db.getPayloadList(
                node, IOV_MIN, IOV_MAX, None,
            ...
            for payload, since, until, channel, _ in data:
                for key in payload:
                    ...
                    if key == 'data':
                        ...
                        path = (repo_dir + '{0}/{1}@{2}'.format(
                            os.path.dirname(node), key,
                            os.path.basename(node)))
                    if (channel
                            or os.path.basename(path) in ('MagnetScale.xml',
                                                          'RichHpdSurface.xml',
                                                          'HpdLogVol.xml')):
                    ...
                    if (since == IOV_MIN
                            and until == IOV_MAX) and not opts.always_iovs:
        ...
        if check_output(['git', 'status', '--porcelain'],
            ...
            check_output([
                'git', 'commit', '-m',
                opts.head_tag_name if tag == 'HEAD' else tag, '--author',
                author, '--date', date
            ],
                         cwd=repo_dir)
        else:
        ...
        if tag != 'HEAD':
            ...
            check_output([
                'git', 'tag', '-m', '---\n{}---\n'.format(
                    yaml.dump({
                        'datatypes': [
                            int(dt) if dt.isdigit() else str(dt)
                            for dt in datatypes
                        ]
                    })), opts.tag_prefix + tag
            ],
            ...
            for branch in branches:
                print 'creating branch', branch, '(from %s)' % (
                    opts.tag_prefix + tag)
