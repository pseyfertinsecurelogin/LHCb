...
class Tree(object):
    ...
    def leaves(self):
        ...
        '''
        return sorted(
            e for e in self._entries if not isinstance(self._entries[e], Tree))
...
def _useFiles(repository, tag):
    ...
    # bare repository
    def is_not_bare():
        return (isdir(join(repository, '.git'))
                or not (isdir(join(repository, 'object'))
                        and isdir(join(repository, 'refs'))))
...
def listFiles(repository, tag='HEAD', path='.'):
    ...
    if _useFiles(repository, tag):
        ...
        out = _git(
            repository,
            ['ls-tree', '-r', '--name-only', '-z', tag or 'HEAD', '--', path
             ]).rstrip('\0').split('\0')
    out.sort()
    return out
...
def gitOpen(repository, path, tag='HEAD'):
    ...
    if _useFiles(repository, tag):
        ...
        return StringIO(
            _git(repository,
                 ['cat-file', 'blob', (tag or 'HEAD') + ':' + path]))
...
def rev_parse(repository, *args):
    ...
    try:
        return _git(
            repository, ['rev-parse'] + list(args), stderr=PIPE).strip()
