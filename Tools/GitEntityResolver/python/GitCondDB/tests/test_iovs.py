...
def make_entries(iterable):
    ...
    for data in iterable:
        if isinstance(data[1], tuple):
            output.append(
                I.NestedEntry(data[0], data[1][0], make_entries(data[1][1])))
        else:
...
TMP_DIR = None
Cond1_IOVs = make_entries([(0, 'zero')] + zip(
    range(1270049171000000000, 1270116165000000000, 15000000000000),
    ('abc{:03d}'.format(i) for i in itertools.count())))
