...
#--------------------------------------------------------------------------------
    ...
    def parseImport(self, godClass, include, stdIncludes, forwardDeclLHCb,
                    forwardDeclGlob, forwardIncl):
        for imp in godClass.get('import', []):
            impAtt = imp['attrs']
            impName = impAtt['name']
            impSoft = impAtt['soft']
            if impAtt['std'] == 'TRUE' and impName not in stdIncludes:
                stdIncludes.append(impName)
            else:
                if impSoft == 'TRUE' or impSoft == 'FORWARDONLY':  # do forward declaration stuff
                    ...
                    if impNS and impNS != myNS:
                        if impNS not in forwardDeclGlob:
                            forwardDeclGlob[impNS] = [impName]
                        elif impName not in forwardDeclGlob[impNS]:
                            forwardDeclGlob[impNS].append(impName)
                    elif impName not in forwardDeclLHCb:
                        forwardDeclLHCb.append(impName)
                    if impSoft == 'TRUE' and impName not in forwardIncl:
                        forwardIncl.append(impName)
...
#--------------------------------------------------------------------------------
    ...
    def genForwardDeclsLHCb(self):
        ...
        for decl in self.forwardDeclLHCb:
            if decl not in self.include:
                s += 'class %s;\n' % decl.split('/')[-1]
        for imp in self.verbatimLHCb:
            s += '%s\n' % imp
        return s
#--------------------------------------------------------------------------------
        ...
        for k in self.forwardDeclGlob.keys():
            ...
            for sk in k.split('::'):
                if sk:
                    s += '%snamespace %s {\n' % (' ' * ind, sk)
                    ind += 2
            for ns in self.forwardDeclGlob[k]:
                s += '%sclass %s;\n' % (' ' * ind,
                                        ns.split('/')[-1].split('.')[0])
            ind -= 2
            while (ind >= 0):
