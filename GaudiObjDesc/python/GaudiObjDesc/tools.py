...
class tools:
    ...
    def __init__(self):
        ...
        self.integerT = [
            'short int', 'short', 'signed short', 'short signed',
            'signed short int', 'short signed int', 'int', 'signed',
            'signed int', 'long int', 'long', 'signed long', 'long signed',
            'signed long int', 'long signed int', 'unsigned short int',
            'unsigned short', 'short unsigned', 'short unsigned int',
            'unsigned int', 'unsigned', 'unsigned long int', 'unsigned long',
            'long unsigned', 'long unsigned int', 'long long', 'long long int',
        ...
        self.cppKeywords = [
            ...
            'typedef', 'typeid', 'typename', 'union', 'unsigned', 'usign',
...
#--------------------------------------------------------------------------------
    ...
    def isFundamentalT(self, type):
        if (self.isBooleanT(type) or self.isCharacterT(type)
                or self.isIntegerT(type) or self.isFloatingPointT(type)):
            return 1
        return 0
#--------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------
    ...
    def genParamsFromStrg(self, parameters):
        ...
        for p in paramList:
            ps = p.split()
            pList.append('%s %s' % (self.genParamFromStrg(
                (' ').join(ps[:-1])), ps[-1]))
        return pList
#--------------------------------------------------------------------------------

    def genParamTypeName(
            self, fullParam):  # this is a very simple minded implementation
        return (
            (' ').join(fullParam.split()[:-1]),
            fullParam.split()[:-1])  # assuming that every parameter has a name
#--------------------------------------------------------------------------------
    ...
    def genParamFromElem(self, p):
        ...
        if p['inout'] == 'BYVALUE': s += p['type']
            ...
            s += p['type']
            if not (self.isFundamentalT(p['type'])
                    or self.isPointerT(p['type'])):
                s += '&'
        elif p['inout'] in ['INOUT', 'BOTH']:
            s += p['type'] + '&'
        s += ' '
        s += p.get('name', 'value')
        return s
#--------------------------------------------------------------------------------
    ...
    def genParamsFromElem(self, pDict):
        pList = []
        for p in pDict:
            pList.append(self.genParamFromElem(p['attrs']))
        return pList
#--------------------------------------------------------------------------------
...
#--------------------------------------------------------------------------------
    ...
    def getTypeTokens(self, completeType):
        ...
        for x in tList2:
            ...
            tList3.extend(sList)
        tList4 = [
            self.removeCppKeywords(x) for x in tList3
            if self.removeCppKeywords(x).strip()
        ]
    ...
    def lowerGetterName(self, metName):
        if metName[0].isupper():
            ...
            for i in range(0, lname):
                if i != (lname - 1) and metName[i].isupper() and metName[
                        i + 1].isupper():
                    metName = metName[:i] + metName[i].lower() + metName[i +
                                                                         1:]
...
#--------------------------------------------------------------------------------
    ...
    def printCode(self, code):
        codeList = code.split('\n')
        basicIndent = 0
        while not codeList[0].strip():
            codeList = codeList[1:]
        while not codeList[-1].strip():
            codeList = codeList[:-1]
        for c in codeList[0]:
