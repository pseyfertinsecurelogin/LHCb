...
class genClassDicts:
    ...
    def __init__(self, godRoot, dictOutputDir, srcOutputDir):
        ...
        if srcOutputDir == '.': self.srcOutputDir = 'Event/'
            ...
            self.srcOutputDir = srcOutputDir.replace('\\', '/').replace(
                '../', '')
        ...
        for ns in godPackage['namespace']:
            ...
            for typ, tag in typetags:
                try:
                    things = ns[typ]
                    for thing in things:
                        xml_str = '  <%s name="%s::%s::%s"/>' % (
                            tag, pkg_namespace, ns_name,
                            thing['attrs']['name'])
                        self.sClassSelections = self.conc(
                            self.sClassSelections, xml_str)
                except KeyError:
                    pass
...
#--------------------------------------------------------------------------------
    ...
    def genPackageDictClass(self, godPackage):
        ...
        for cl in godPackage['class']:
            ...
            self.sIncludes = self.conc(
                self.sIncludes,
                '#include "%s%s.h"' % (self.srcOutputDir, cl['attrs']['name']))
            # get the class id
            if 'id' in cl['attrs']:
                id = cl['attrs']['id']
            ...
            for attr in cl.get('attribute', []):
                if attr['attrs'].get('transient') == 'TRUE':
                    if trans == False:
                        cnstr += '>'
                        trans = True
                    cnstr += ' <field name="m_%s" transient="true"/>' % (
                        attr['attrs']['name'])
            if trans:
                cnstr += ' </class>'
            else:
            ...
            for t in cl.get('template', []):
                ...
                if tcname.lower() == 'vector':
                    # include header file
                    self.sIncludes = self.conc(self.sIncludes,
                                               '#include <vector>')
                    # include template instantiation
                    self.sDictInstances = self.conc(
                        self.sDictInstances, 'std::vector<%s> m_std_vector_%s;'
                        % (t1name, self.clean(t1name)))
                    # include element for selection file
                    self.sClassSelections = self.conc(
                        self.sClassSelections,
                        '  <class name="std::vector<%s>"/>' % t1name)
                    ...
                    if t1name != 'int':
                        ...
                        self.sDictInstances = self.conc(
                            self.sDictInstances,
                            'KeyedObject<%s> m_KeyedObject_%s;' %
                            (t1name, self.clean(t1name)))
                        # include element for selection file
                        ko = '  <class name="KeyedObject<%s>" rootmap="false">' % (
                            t1name)
                        ko += ' <field name="m_hasKey" transient="true"/>'
                        ko += ' <field name="m_refCount" transient="true"/>'
                        ko += ' </class>'
                        self.sClassSelections = self.conc(
                            self.sClassSelections, ko)
                elif tcname == "KeyedContainer":
                    # "GaudiKernel" hardcoded here to avoid usage of the inclusion mechanism (not nice)
                    self.sIncludes = self.conc(
                        self.sIncludes, '#include "GaudiKernel/%s.h"' % tcname)
                    # include template instantiation
                    self.sDictInstances = self.conc(
                        self.sDictInstances,
                        '%s<%s> m_%s_%s;' % (tcname, t1name, tcname,
                                             self.clean(t1name)))
                    # Add container id to class id
                    if 'id' in cl['attrs']:
                        sid = ' id="%08x-0000-0000-0000-000000000000"' % (
                            int(id) + 0x60000)
                    # include element for selection file
                    kc = '  <class name="%s<%s,Containers::KeyedObjectManager<Containers::hashmap> >"%s>' % (
                        tcname, t1name, sid)
                    kc += ' <field name="m_cont" transient="true"/>'
                    kc += ' <field name="m_random" transient="true"/>'
                    kc += ' </class>'
                    self.sClassSelections = self.conc(self.sClassSelections,
                                                      kc)
                elif tcname == "SmartRef":
                    # "GaudiKernel" hardcoded here to avoid usage of the inclusion mechanism (not nice)
                    self.sIncludes = self.conc(
                        self.sIncludes, '#include "GaudiKernel/SmartRef.h"')
                    # include template instantiation
                    self.sDictInstances = self.conc(
                        self.sDictInstances,
                        'SmartRef<%s> m_SmartRef_%s;' % (t1name,
                                                         self.clean(t1name)))
                    # include element for selection file
                    ...
                    self.sDictInstances = self.conc(
                        self.sDictInstances,
                        'SmartRefVector<%s> m_SmartRefVector_%s;' %
                        (t1name, self.clean(t1name)))
                    self.sDictInstances = self.conc(
                        self.sDictInstances,
                        'std::vector<SmartRef<%s> > m_std_vector_SmartRef_%s;'
                        % (t1name, self.clean(t1name)))
                    # include element for selection file
                    ...
                    self.sClassSelections = self.conc(
                        self.sClassSelections,
                        '  <class name="std::vector<SmartRef<%s> >"/>' %
                        (t1name))
                    ...
                    self.sClassSelections = self.conc(
                        self.sClassSelections,
                        '  <class name="%s<%s>"%s/>' % (tcname, t1name, sid))
                else:
                    # "GaudiKernel" hardcoded here to avoid usage of the inclusion mechanism (not nice)
                    self.sIncludes = self.conc(
                        self.sIncludes, '#include "GaudiKernel/%s.h"' % tcname)
    ...
    def appendFile(self, fileName, tmplname):
        ...
        if tmplname == 'dictionary_h':
            ...
            for line in file.readlines():
                if line.find('// begin include files') != -1:
                    appendIncludes = 1
                elif line.find('// end include files') != -1:
                    appendIncludes = 0
                elif line.find('// begin instantiations') != -1:
                    appendInstances = 1
                elif line.find('// end instantiations') != -1:
                    appendInstances = 0
                elif (appendIncludes):
                    self.sIncludes = self.conc(self.sIncludes, line[:-1])
                elif (appendInstances):
                    self.sDictInstances = self.conc(
                        self.sDictInstances, ' '.join(line[:-1].split()))
