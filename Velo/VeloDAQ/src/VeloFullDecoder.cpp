...
VeloFullDecoder::VeloFullDecoder( const int decoderType ) { decoderIni( decoderType ); }
//=============================================================================
      ...
      for ( int stream = 0; stream < VeloTELL1::DataStreamPerSection; stream++ ) {
        ...
        if ( !( ( aSection == EmptyData ) && ( stream == EmptyData ) ) ) {
          ...
          for ( int aLink = 0; aLink < VeloTELL1::ALinkPerDataStream; aLink++ ) {
            ...
            int count           = 0;
            //
            for ( auto secIt = section.begin() + beginDecode; secIt != section.begin() + endDecode; ++secIt ) {
