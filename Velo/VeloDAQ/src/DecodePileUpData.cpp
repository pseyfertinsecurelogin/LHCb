...
StatusCode DecodePileUpData::initialize() {
  ...
  if ( m_rawEventLocations[0] != LHCb::RawEventLocation::Trigger &&
...
StatusCode DecodePileUpData::decodePileUpBinaryNZS( LHCb::span<const LHCb::RawBank*> banksNZS ) {
  ...
  for ( const LHCb::RawBank* aBank : banksNZS ) {
    ...
    for ( int PP = 0; PP < PuTell1::NUM_PP; PP++ ) {
      ...
      for ( int linkIt = 0; linkIt < PuTell1::NUM_PPLINKS; linkIt++ ) { linkIsGood[linkIt] = false; }
...
//=========================================================================
void DecodePileUpData::Fill( unsigned int wordIt, unsigned int word_Tot, const unsigned int* data_Ptr, int step,
...
StatusCode DecodePileUpData::findPileUpHitsBee( PuTell1::dataObject OneBeetleData, int sens, int bee,
                                                VeloClusters* clusters ) {
  ...
  for ( int i = 0; i < PuTell1::NUMBITS; i++ ) {
    ...
    if ( checkBee ) {
      ...
      if ( OneBeetleData.isOrdered ) {
        // not flipped word
        stripNum =
            (PuTell1::STRIPS_PER_CHANNEL)*i + bee * ( PuTell1::STRIPS_PER_BEETLE ); // channel numbering starts from 0
      } else {
        // flipped
        stripNum =
            ( PuTell1::STRIPS_PER_CHANNEL ) * ( PuTell1::NUMBITS - 1 - i ) + bee * ( PuTell1::STRIPS_PER_BEETLE );
      }
      ...
      if ( msgLevel( MSG::DEBUG ) ) {
        debug() << "findPileUpHitsBee : VeloLiteCluster lc(" << fracStrip << ", " << pseudoSize << ", " << hasHighThre
                << ", with channelId strip " << ( lc.channelID() ).strip() << endmsg;
        debug() << "******************** clusters size is " << clusters->size() << ")*********************************"
                << endmsg;
      }
...
StatusCode DecodePileUpData::findPileUpHitsBeeNZS( PuTell1::dataObject OneBeetleData, int sens, int bee,
                                                   VeloClusters* clustersNZS ) {
  unsigned int maskBee = 1;
  unsigned int inBee   = OneBeetleData.dataWord;
  ...
  for ( int i = 0; i < PuTell1::NUMBITS; i++ ) {
    ...
    if ( checkBee ) {
      ...
      VeloChannelID          vcid( sensorNum, stripNum );
      VeloLiteCluster        lc( fracStrip, pseudoSize, hasHighThre, VeloChannelID( sensorNum, stripNum ) );
      VeloCluster::ADCVector adcs;
      adcs.reserve( 1 );
      adcs.push_back( std::pair<int, unsigned int>( static_cast<int>( stripNum ), static_cast<unsigned int>( 1. ) ) );
      ...
      if ( msgLevel( MSG::DEBUG ) ) {
        debug() << "findPileUpHitsBeeNZS : VeloLiteCluster lc(" << fracStrip << ", " << pseudoSize << ", "
                << hasHighThre << ", with channelId strip " << ( lc.channelID() ).strip() << endmsg;
