...
StatusCode PrepareVeloRawBuffer::initialize() {
  ...
  default: // not supported, bail out
    error() << "VELO raw buffer version " << m_bankVersion << " is not supported." << endmsg;
...
StatusCode PrepareVeloRawBuffer::execute() {
  ...
  while ( firstOnSensor != m_sortedClusters.end() ) {
    ...
    while ( currentSensorNumber != m_sensorNumbers[sensorIndex] ) {
      // store an empty bank
      storeBank( m_sensorNumbers[sensorIndex], lastOnSensor, lastOnSensor, rawEvent );
      if ( isDebug ) debug() << "Added empty bank for sensor " << m_sensorNumbers[sensorIndex] << endmsg;
      sensorIndex++; // move to next on list, try again
    }

    while ( lastOnSensor != m_sortedClusters.end() && ( *lastOnSensor )->sensor() == currentSensorNumber ) {
      ++lastOnSensor;
    }
...
void PrepareVeloRawBuffer::storeBank( int sensor, std::vector<const LHCb::InternalVeloCluster*>::const_iterator begin,
  ...
  if ( m_velo->tell1IdBySensorNumber( sensor, sourceId ) ) {
    ...
    LHCb::RawBank* newBank = rawEvent->createBank( static_cast<SiDAQ::buffer_word>( sourceId ), LHCb::RawBank::Velo,
                                                   m_bankVersion, m_bankSizeInBytes, &( m_rawData[0] ) );
    ...
    warning() << "Plot to create bank for sensor " << sensor << " with no TELL1 Id foiled." << endmsg;
  }
}

unsigned int PrepareVeloRawBuffer::makeBank( std::vector<const LHCb::InternalVeloCluster*>::const_iterator begin,
                                             std::vector<const LHCb::InternalVeloCluster*>::const_iterator end ) {
  ...
  if ( isVerbose ) { verbose() << "Number of clusters:" << numClu << endmsg; }
  ...
  for ( ; iC != end; ++iC ) {
    ...
    if ( isVerbose ) {
      verbose() << "Reading cluster from sensor " << clu->sensor() << " with " << numStrips << " strips." << endmsg;
    }
    ...
    if ( numStrips == 1 ) { // single strip cluster
      ...
      if ( isVerbose ) { verbose() << "strip " << clu->strip( 0 ) << ",adc= " << aw.value() << endmsg; }
    } else {
      ...
      for ( ; i < stripSignals.size(); ++i ) {
        ...
        if ( isVerbose ) { verbose() << "ADC COUNT:" << adcCount << " STRIP:" << stripNumber << endmsg; }
      ...
      if ( isVerbose ) {
        verbose() << "CPOS: " << cPos << " CHPOS: " << channelPos << " ISPOS: " << interStripPos
                  << " WORD: " << format( "%8X", packedCluster ) << endmsg;
      }
    }
  ...
  if ( nClu % VeloDAQ::clu_per_buffer != 0 ) { m_clusterPosBuffer.push_back( cluRowData ); }
  // check row data is not empty, store remaining adcs
  if ( nAdc % VeloDAQ::adc_per_buffer != 0 ) { m_clusterADCBuffer.push_back( rowData ); }

  // write to buffer or return ?

  if ( isVerbose ) {
    verbose() << "Raw  output of cluster start" << endmsg;
    verbose() << "PCN error flag and number of clusters[" << format( "%10X", pcnAndNumClu ) << "]" << endmsg;
  }
  ...
  for ( ; tmpCp != m_clusterPosBuffer.end(); ++tmpCp ) {
    if ( isVerbose ) { verbose() << "Cluster position[" << format( "%10X", *tmpCp ) << "]" << endmsg; }
  ...
  for ( ; tmpAdc != m_clusterADCBuffer.end(); ++tmpAdc ) {
    if ( isVerbose ) { verbose() << "Adc row[" << format( "%10X", *tmpAdc ) << "]" << endmsg; }
  ...
  if ( isVerbose ) { verbose() << "Raw  output of cluster end" << endmsg; }
...
void PrepareVeloRawBuffer::dumpInputClusters() const {
  for ( std::vector<const LHCb::InternalVeloCluster*>::const_iterator ci = m_sortedClusters.begin();
    ...
    for ( int adci = 0; adci < clu->size(); ++adci ) { info() << " " << adcValues[adci] << "@" << clu->strip( adci ); }
    info() << endmsg;
  }
  return;
}
