...
StatusCode VeloClustersToRaw::storeBank( const unsigned int                                     sensor,
  ...
  if ( m_velo->tell1IdBySensorNumber( sensor, sourceId ) ) {
    ...
    LHCb::RawBank* newBank =
        m_rawEventOut->createBank( static_cast<SiDAQ::buffer_word>( sourceId ), LHCb::RawBank::Velo, m_bankVersion,
                                   m_bankSizeInBytes, &( m_rawData[0] ) );
    ...
    error() << " Attempt to create bank for sensor " << sensor << " with no TELL1 Id detected." << endmsg;
...
unsigned int VeloClustersToRaw::makeBank( const unsigned int                                     sensor,
  ...
  std::copy( m_clusterPosBuffer.begin(), m_clusterPosBuffer.end(), std::back_inserter( m_rawData ) );

  // add adc values
  std::copy( m_clusterADCBuffer.begin(), m_clusterADCBuffer.end(), std::back_inserter( m_rawData ) );
...
bool VeloClustersToRaw::selfTest() {
  ...
  for ( const auto* outBank : m_rawEventOut->banks( LHCb::RawBank::Velo ) ) {
    ...
    VeloDAQ::decodeRawBankToClustersV3( static_cast<const SiDAQ::buffer_word*>( outBank->data() ), sensor, false,
                                        clusters, byteCount, errMsg, false );
  }
  ...
  std::sort( decodedClusters.begin(), decodedClusters.end(), VeloDAQ::veloClusterPtrLessThan() );
  ...
  for ( ; iOut != decodedClusters.end(); ++iIn, ++iOut ) {
    ...
    if ( inClu->size() != outClu->size() ) {
      error() << "Cluster sizes do not match: ref=" << inClu->size() << " out=" << outClu->size() << endmsg;
      passed = false;
    } else {
      for ( unsigned int i = 0; i < outClu->size(); ++i ) {
        if ( inClu->strip( i ) != outClu->strip( i ) ) {
          error() << "Cluster strips do not match: ref=" << inClu->strip( i ) << " out=" << outClu->strip( i )
                  << endmsg;
          passed = false;
        }

        if ( inClu->adcValue( i ) != outClu->adcValue( i ) ) {
          error() << "Strip ADC values do not match: ref=" << inClu->adcValue( i ) << " out=" << outClu->adcValue( i )
                  << endmsg;
          passed = false;
        }
      }
    ...
    if ( inClu->totalCharge() != outClu->totalCharge() ) {
      error() << "Cluster charges do not match: ref=" << inClu->totalCharge() << " out=" << outClu->totalCharge()
              << endmsg;
      passed = false;
    }

    if ( inClu->highThreshold() != outClu->highThreshold() ) {
      error() << "Threshold flags do not match: ref=" << inClu->highThreshold() << " out=" << outClu->highThreshold()
              << endmsg;
      passed = false;
    }

    if ( inClu->pseudoSize() != outClu->pseudoSize() ) {
      error() << "Cluster pseudo sizes do not match: ref=" << inClu->pseudoSize() << " out=" << outClu->pseudoSize()
              << endmsg;
      passed = false;
    }
