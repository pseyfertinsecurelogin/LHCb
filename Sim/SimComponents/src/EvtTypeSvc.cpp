...
StatusCode EvtTypeSvc::finalize()
{
  ...
  if( msg.level() <= MSG::DEBUG ) {
    ...
    debug() << "Table size before clean up" << m_evtTypeInfos.size()
        << endmsg;
  }
  ...
  if( msg.level() <= MSG::DEBUG )
    debug() << "Table size after clean up" << m_evtTypeInfos.size()
        << endmsg;
...
StatusCode EvtTypeSvc::parseFile( const std::string input )
{
  ...
  if ( !infile ) {
    fatal() << "File with event types information cannot be opened :" << input
            << endmsg;
    return StatusCode::FAILURE;
  }
  info() << "Opened file with the event types information : " << inputFile()
         << endmsg;
  ...
  if( msgLevel(MSG::DEBUG) ) {
    ...
    debug() << "Stored " << m_evtTypeInfos.size()
