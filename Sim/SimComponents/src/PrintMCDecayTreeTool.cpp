...
StatusCode PrintMCDecayTreeTool::initialize() {
  ...
  do {
    ...
    if ( tok == "Name" )
      ...
      m_keys.push_back( flags );
    else if ( tok == "fromSignal" )
      m_keys.push_back( fromSignal );
    else if ( tok == "IDCL" )
      m_keys.push_back( idcl );
    else
      err() << "Unknown output key '" << tok << "'. Ignoring it." << endmsg;
  ...
  if ( m_energyUnit == TeV )
    m_energyUnitName = "TeV";
  else if ( m_energyUnit == GeV )
    m_energyUnitName = "GeV";
  else if ( m_energyUnit == MeV )
  ...
  if ( m_lengthUnit == mm )
    m_lengthUnitName = "mm";
  else if ( m_lengthUnit == cm )
    m_lengthUnitName = "cm";
  else if ( m_lengthUnit == m )
    m_lengthUnitName = "m";
  else if ( m_lengthUnit <= 0 ) {
    err() << "You have chosen a unit for lengths: " << m_lengthUnit << endmsg;
    return StatusCode::FAILURE;
  } else {
...
MsgStream& PrintMCDecayTreeTool::printHeader( MsgStream& log ) const {
  ...
  if ( left >= 0 ) log << '<' << std::string( left, '-' );
  log << *title;
  if ( right >= 0 ) log << std::string( right, '-' ) << '>';
  ...
  for ( const auto& i : m_keys ) switch ( i ) {
    ...
    case eta:
      ...
      break;
    case flags:
      log << std::setw( m_fWidth ) << "Flags ";
      break;
    case fromSignal:
      log << std::setw( m_fWidth ) << "fromSignal";
      break;
    case idcl:
...
//=============================================================================
MsgStream& PrintMCDecayTreeTool::printInfo( const std::string& prefix, const LHCb::MCParticle* part,
                                            MsgStream& log ) const {
  ...
  for ( const auto& i : m_keys ) switch ( i ) {
    ...
    case PID:
      log << std::setw( m_fWidth ) << std::setprecision( m_fPrecision ) << part->particleID().pid();
      break;
    case E:
    ...
    case eta:
      ...
      break;
    case flags:
      log << std::setw( m_fWidth ) << std::setprecision( m_fPrecision ) << part->flags();
      break;
    case fromSignal:
      log << std::setw( m_fWidth ) << std::setprecision( m_fPrecision ) << part->fromSignal();
      break;
    case idcl:
...
//=============================================================================
void PrintMCDecayTreeTool::printTree( const LHCb::MCParticle* mother, int maxDepth ) const {
...
void PrintMCDecayTreeTool::printAsTree( const LHCb::MCParticle::ConstVector& event ) const {
  ...
  for ( const auto& i : event ) {
    if ( !i->originVertex() || !i->originVertex()->mother() ) printDecayTree( i, "", m_depth, log );
  }
  log << endmsg;
}
//=============================================================================
...
//=============================================================================
MsgStream& PrintMCDecayTreeTool::printDecayTree( const LHCb::MCParticle* mother, const std::string& prefix, int depth,
                                                 MsgStream& log ) const {
  printInfo( prefix, mother, log );
  if ( depth ) {
    for ( auto iv = mother->endVertices().begin(); iv != mother->endVertices().end(); iv++ ) {
      for ( auto idau = ( *iv )->products().begin(); idau != ( *iv )->products().end(); idau++ ) {
        bool last = ( ( *idau == ( *iv )->products().back() ) && ( *iv == mother->endVertices().back() ) );
