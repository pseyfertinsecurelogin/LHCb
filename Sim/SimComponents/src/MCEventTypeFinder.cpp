...
namespace {
  bool vec2set( const std::vector<long unsigned int>& avec, LHCb::EventTypeSet& aset ) {
    aset.clear();
    aset.insert( avec.begin(), avec.end() );
    return !aset.empty();
  }
} // namespace
...
StatusCode MCEventTypeFinder::fillMCTools() {
  ...
  for ( auto iType = m_allTypes.begin(); iType != m_allTypes.end(); iType++ ) {
    ...
    // is it a DecProdCut?
    m_decProdCut.push_back( sname.find( "DecProdCut" ) != std::string::npos ||
                            sname.find( "InAcc" ) != std::string::npos );
    ...
    if ( ( anIMCD->setDecay( sdecay ) ).isFailure() ) {
      ...
      m_mcFinders.push_back( anIMCD );
  }
  if ( !m_mcFinders.empty() && m_mcFinders.size() == m_decProdCut.size() && m_mcFinders.size() == m_allTypes.size() )
...
StatusCode MCEventTypeFinder::findDecayType( LHCb::EventTypeSet& found, const LHCb::MCParticle* mc_mother ) {
  ...
  for ( auto iType = m_allTypes.begin();
        ...
        iType++, iD++, iB++ ) // lowest to highest
  {
    ...
    {
      bool nJPsi = false, nmuon = false, pmuon = false, nmudec = false, pmudec = false, nelectron = false, dec = true,
           neutrino = false;
...
StatusCode MCEventTypeFinder::findDecayType( LHCb::EventTypeSet&                  found,
  ...
  for ( const auto& iParticle : mc_mothers ) {
...
long int MCEventTypeFinder::constructDecayType( const LHCb::MCParticle* mc_mother, bool& dimuon ) {
  ...
  // if(mc_mother->pseudoRapidity()<m_acc_min || mc_mother->pseudoRapidity()>m_acc_max) return 0; //don't check if a
  // particle is explicitly passed amuon?
...
long int MCEventTypeFinder::determineDecay( const LHCb::MCParticle* mc_mother, bool& dimuon ) {
  ...
  if ( ncharm >= 2 && !anymuon && !nelectron )
    dctnxu += 90000; // from the LHCb note :)
  else if ( ncharm >= 1 && anymuon )
    dctnxu += 70000;
  else if ( ncharm >= 1 && nelectron )
    dctnxu += 80000;
  else if ( ncharm >= 1 )
    dctnxu += 60000;
  else if ( nJPsi && anymuon )
    dctnxu += 40000;
  else if ( nJPsi && nelectron )
    dctnxu += 50000;
  else if ( nJPsi )
    dctnxu += 30000;
  else if ( anymuon )
...
int MCEventTypeFinder::determineDecay( const LHCb::MCParticle* mc_mother, bool& nJPsi, int& ncharm, bool& nmuon,
                                       bool& pmuon, bool& nmudec, bool& pmudec, bool& nelectron, int& ntracks,
                                       int& neut, bool& dec, bool& neutrino ) {
  if ( !mc_mother ) return 0;
  const LHCb::ParticleID mID = mc_mother->particleID();
  if ( isStable( mc_mother ) ) // it's a track
  {
    if ( abs( mID.pid() ) == 22 )
      return 0; // don't follow photons
    else if ( abs( mID.pid() ) == 12 || abs( mID.pid() ) == 14 || abs( mID.pid() ) == 16 )
      neutrino = true;
    else
      dec = ( dec && mc_mother->pseudoRapidity() > m_acc_min &&
              mc_mother->pseudoRapidity() < m_acc_max ); // check if it's not a neutrino
    ...
    if ( mID.threeCharge() != 0 ) {
      ...
      if ( mID.isLepton() ) {
        if ( mID.pid() == m_muonm ) {
          nmuon = true;
          if ( dec ) nmudec = true;
        } else if ( mID.pid() == m_muonp ) {
          pmuon = true;
          if ( dec ) pmudec = true;
        } else if ( abs( mID.pid() ) == 11 )
          nelectron = true;
      }
    }
...
bool MCEventTypeFinder::isStable( const LHCb::MCParticle* mc_mother ) {
  ...
  if ( anID == 11 || anID == 13 || anID == 22 || anID == 12 || anID == 111 || anID == 211 || anID == 130 ||
       anID == 321 || anID == 2112 || anID == 2212 )
    return true;
  return false;
}
...
bool MCEventTypeFinder::dimuon( const LHCb::MCParticles& mc_mothers ) {
  ...
  for ( const auto& p : mc_mothers ) {
    ...
    // in acceptance?
    if ( p->pseudoRapidity() < m_acc_min || p->pseudoRapidity() > m_acc_max )
      continue; // don't check if a particle is explicitly passed
    if ( p->particleID().pid() == m_muonp )
      pmuon = true;
    else if ( p->particleID().pid() == m_muonm )
      nmuon = true;
  }
  return ( pmuon && nmuon );
}
...
bool MCEventTypeFinder::dimuon( const LHCb::MCParticle::ConstVector& mc_mothers ) {
  ...
  for ( LHCb::MCParticle::ConstVector::const_iterator iParticle = mc_mothers.begin(); iParticle != mc_mothers.end();
        iParticle++ ) {
    if ( !( *iParticle ) ) continue;

    // in acceptance?
    if ( ( *iParticle )->pseudoRapidity() < m_acc_min || ( *iParticle )->pseudoRapidity() > m_acc_max )
      continue; // don't check if a particle is explicitly passed
    if ( ( *iParticle )->particleID().pid() == m_muonp )
      pmuon = true;
    else if ( ( *iParticle )->particleID().pid() == m_muonm )
      nmuon = true;
  }
  return ( pmuon && nmuon );
}
StatusCode MCEventTypeFinder::setEventTypes( const LHCb::EventTypeSet events ) {
  m_allTypes = events;
  if ( m_allTypes.count( m_mbias ) )
    warning() << "No min bias in all types, excecution may fail on certain events, unless you have another min bias"
              << endmsg;
  m_mcFinders.clear();
  m_decProdCut.clear();
...
long int MCEventTypeFinder::categoriseTau( const LHCb::MCParticle* mc_mother, bool& dimuon ) {
  ...
  if ( msgLevel( MSG::VERBOSE ) )
    verbose() << "found the decay type " << ( g * 10000000 + s * 1000000 + d * 100000 + ctnxu ) << endmsg;
  if ( dimuon && msgLevel( MSG::VERBOSE ) ) verbose() << "--which is a dimuon type " << endmsg;

  return ( g * 10000000 + s * 1000000 + d * 100000 + ctnxu );
}

int MCEventTypeFinder::determineTauDecay( const LHCb::MCParticle* mc_mother, int& nV0, bool& nmuon, bool& pmuon,
                                          bool& nmudec, bool& pmudec, bool& nelectron, int& ntracks, int& neut,
                                          bool& dec, int& nPip, int& nPim, int& nKp, int& nKm )

{
  ...
  if ( isStable( mc_mother ) ) // it's a track
  {
    ...
    if ( mID.threeCharge() != 0 ) {
      ...
      if ( nV0 == 0 ) {
        if ( abs( mID.pid() ) == 333 )
          nV0 = 4; // Phi
        else if ( abs( mID.pid() ) == 313 )
          nV0 = 5; // K*(892)0
        else if ( abs( mID.pid() ) == 113 )
          nV0 = 6; // rho0
        else if ( abs( mID.pid() ) == 223 )
          nV0 = 7; // omega0
      }
  ...
  for ( SmartRefVector<LHCb::MCVertex>::const_iterator iV = mc_mother->endVertices().begin();
    ...
    for ( SmartRefVector<LHCb::MCParticle>::const_iterator iP = ( *iV )->products().begin();
      ...
      determineTauDecay( *iP, nV0, nmuon, pmuon, nmudec, pmudec, nelectron, ntracks, neut, dec, nPip, nPim, nKp, nKm );
...
long int MCEventTypeFinder::determineTauDecay( const LHCb::MCParticle* mc_mother, bool& dimuon ) {
  ...
  determineTauDecay( mc_mother, nV0, nmuon, pmuon, nmudec, pmudec, nelectron, ntracks, neut, dec, nPip, nPim, nKp,
                     nKm );
