...
namespace {
  ...
  void addGenCrossSections( LHCb::GenFSR& genFSR, std::ostream& xmlOutput ) {
    ...
    for ( auto const& counter : genFSR.genCounters() ) {
      ...
      double C =
          key >= LHCb::GenCountersFSR::CounterKey::OnebGen
              ? genFSR.getCrossSectionInfo( LHCb::CrossSectionsFSR::CrossSectionKeyToType( "MBCrossSection" ) ).second
              : 1;
...
//=============================================================================
GenFSRLog::GenFSRLog( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {}
