...
def testthisiox(iox):
    ...
    for ext in exts:
        iox.outputAlgs(
            basename + ext, writer='OutputStream/Alg' + ext.replace('.', ''))
        iox.outStream(
            basename + ext, writer='OutputStream/Str' + ext.replace('.', ''))
    ...
    for setPersistency in [True, False]:
        print commands.getstatusoutput("python -c '" + iox.extensionString(
