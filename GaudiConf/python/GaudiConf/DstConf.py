...
class DstConf(LHCbConfigurableUser):
    ...
    _propertyDocDct = {
        'DstType':
        """ Type of dst, can be ['DST', 'RDST', 'XDST','MDST', 'LDST' ] """,
        'SimType':
        """ Type of simulation output, can be ['None','Minimal','Full'] """,
        'EnableUnpack':
        """ List of DST container types to be set up for on demand unpacking. See KnownUnpackingTypes """,
        'EnablePackingChecks':
        """ Flag to turn on the running of various unpacking checks, to test the quality of the data packing """,
        'PackType':
        """ Type of packing for the DST, can be ['NONE','TES','MDF'] """,
        'PackSequencer':
        """ Sequencer in which to run the packing algorithms """,
        'AlwaysCreate':
        """ Flags whether to create output packed objects even if input missing """,
        'Turbo':
        """ Propagate Turbo stream locations """,
        'Writer':
        """ Name of OutputStream writing the DST """,
        'OutputName':
        """ Name of the output file, for MDF writing """,
        'SpilloverPaths':
        """ Paths to write to XDST if available on input file """,
        'DataType':
        """ Flag for backward compatibility with old data """,
        'WriteFSR':
        """ Flags whether to write out an FSR """,
        "SplitRawEventOutput":
        "Which version fo the split raw event to write out, default 2.0, same as Reco 14/Stripping 20 DSTs"
    }
    ...
    def _defineOutputData(self, dType, pType, sType, writer):
        ...
        if pType == 'NONE':
            ...
            if sType != "None":
                ...
                SimConf().setProp("EnablePack",
                                  DigiConf().getProp("EnablePack"))
        else:
        ...
        writer.ItemList += [
            "/Event/Rec/Header" + depth, "/Event/Rec/Status" + depth
        ]
        ...
        if (self.getProp("PVPersistence") is "RecVertex"):
            ...
            if (self.getProp("DataType") in self.Run2DataTypes):
                writer.ItemList += [
                    "/Event/" + recDir + "/Track/FittedHLT1VeloTracks" + depth
                ]
        elif (self.getProp("PVPersistence") is "PrimaryVertex"):
            writer.ItemList += [
                "/Event/" + recDir + "/Vertex/PrimaryVertices" + depth
            ]
        else:
        ...
        if pType != "MDF":
            ...
            if dType != "RDST":
                ...
                locs, rec = _getDict()
                output_locations = ReverseDict(
                    self.getProp("SplitRawEventOutput"), locs, rec)
    ...
    def _doWriteROOT(self, items, optItems):
        ...
        log.info(
            "%s.OptItemList=%s" % (self.getProp("Writer"), writer.OptItemList))
    ...
    def _doPack(self):
        ...
        from Configurables import (
            PackTrack, PackCaloHypo, PackProtoParticle, PackRecVertex,
            PackTwoProngVertex, DataPacking__Pack_LHCb__RichPIDPacker_,
            DataPacking__Pack_LHCb__MuonPIDPacker_, PackPrimaryVertices,
        ...
        if "Tracking" not in self.getProp("EnableUnpack"):
            ...
            if ((self.getProp("DataType") in self.Run2DataTypes)
                ...
                packFittedVelo = PackTrack(
                    name="PackTracksFittedVelo",
            ...
            ###
        richpidpack = DataPacking__Pack_LHCb__RichPIDPacker_(
            name="PackRichPIDs",
            AlwaysCreateOutput=alwaysCreate,
            EnableCheck=doChecks)
        muonpidpack = DataPacking__Pack_LHCb__MuonPIDPacker_(
            name="PackMuonPIDs",
            AlwaysCreateOutput=alwaysCreate,
            EnableCheck=doChecks)
        ...
        protoPidClean = ChargedProtoParticleRemovePIDInfo(
            "ProtoParticlePIDClean")
        packDST.Members += [protoPidClean]

        # packed the charged protos
        packChargedPs = PackProtoParticle(
            name="PackChargedProtos",
            AlwaysCreateOutput=alwaysCreate,
        ...
        packNeutralPs = PackProtoParticle(
            name="PackNeutralProtos",
            AlwaysCreateOutput=alwaysCreate,
    ...
    def _doUnpack(self):
        ...
        # Muon Tracks
        if "Muon" in self.getProp("Detectors"):
            unpackMuons = UnpackTrack(
                name="UnpackMuonTracks",
    ...
    def _unpackMuonPIDs(self):
        ...
        # Unpacking alg
        unpackmuonpid = DataPacking__Unpack_LHCb__MuonPIDPacker_(
            "UnpackMuonPIDs")
    ...
    def _unpackRichPIDs(self):
        ...
        unpackrichpid = DataPacking__Unpack_LHCb__RichPIDPacker_(
            "UnpackRichPIDs")
    ...
    def _unpackStripping(self):
        ...
        if "Calo" in self.getProp("Detectors"):
            ...
            mapper.Algorithms[
                1541] = "DataPacking::Unpack<LHCb::CaloClusterPacker>"
            mapper.Algorithms[
                1542] = "DataPacking::Unpack<LHCb::CaloDigitPacker>"
            mapper.Algorithms[
                1543] = "DataPacking::Unpack<LHCb::CaloAdcPacker>"
        if "Rich" in self.getProp("Detectors"):
            mapper.Algorithms[
                1561] = "DataPacking::Unpack<LHCb::RichPIDPacker>"
        if "Muon" in self.getProp("Detectors"):
            mapper.Algorithms[
                1571] = "DataPacking::Unpack<LHCb::MuonPIDPacker>"
        mapper.Algorithms[1553] = "UnpackRecVertex"
        mapper.Algorithms[
            1555] = "DataPacking::Unpack<LHCb::WeightsVectorPacker>"
        ...
        lclusmapper.Transformations = [('(.*)/Raw/(.*)/LiteClusters',
                                        '$1/Raw/$2/Clusters')]
    ...
    def __apply_configuration__(self):
        ...
        if sType != "None":
            ...
            SimConf().setProp("EnableUnpack",
                              DigiConf().getProp("EnableUnpack"))
        ...
        self.setOtherProps(DigiConf(),
                           ["SpilloverPaths", "DataType", "Detectors"])
        ...
        if "Reconstruction" in self.getProp(
                "EnableUnpack") or "Tracking" in self.getProp("EnableUnpack"):
            self._doUnpack()
