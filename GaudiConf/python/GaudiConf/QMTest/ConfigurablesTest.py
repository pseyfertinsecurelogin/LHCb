        ...
        if hasattr(Configurables, name):
            try:
                ...
                if callable(aConf):
                    ...
                    raise TypeError(name + " is not callable, Configurables." +
                                    name + " evaluates as type " +
                                    str(type(aConf)))
...
def testThisProjectOnly(output=None, project=None):
    ...
    if project is None:
        raise ValueError(
            "could not determine project automatically, please supply a string"
        )
    proj_pack = moduleList()
    findproj = [
        p for p in proj_pack if p == project or p.lower() == project.lower()
    ][0]
...
def compareConfigurables(reference,
                         testoutput,
                         causes,
                         result,
                         testname,
    ...
    if isinstance(testoutput, basestring):
        ...
        if os.path.exists(testoutput):
            ...
            testoutput = testoutput[testoutput.find(PrintParseLine) +
                                    len(PrintParseLine):].strip().replace(
                                        "\n", " ")
            exec ("testoutput=" + testoutput)
        else:
            raise ValueError(
                "Failed to interpret testoutput as a file or stdout")
    if type(reference) is not dict:
        raise TypeError("expected reference dictionary, got instead " +
                        str(type(reference)))
    if type(testoutput) is not dict:
        raise TypeError("expected testoutput dictionary, got instead " +
                        str(type(testoutput)))
    ...
    if len(packages):
        missingpackages = [
            m for m in reference if m not in testoutput and m in packages
        ]
        addedpackages = [
            m for m in testoutput if m not in reference and m in packages
        ]
        #all None
        brokenpackages = [
            m for m in testoutput if m in packages and [None] *
            len(testoutput[m]) == [testoutput[m][v] for v in testoutput[m]]
        ]
        #all False
        failedpackages = [
            m for m in testoutput if m in packages and [False] *
            len(testoutput[m]) == [testoutput[m][v] for v in testoutput[m]]
        ]
        #Nothing True
        mixedpackages = [
            m for m in testoutput if m in packages
            and True not in [testoutput[m][v] for v in testoutput[m]]
        ]
    ...
    if len(missingtofail):
        ...
        result[testname + ".missingpack"] = result.Quote(
            "Completely cannot see the following packages, even though they were there in svnProjectDeps, indicates some major problem with svnProjectDeps or that the package failed to build: "
            + missingtofail.__str__())
    if len(brokenpackages):
        causes.append("Entire package broken")
        result[testname + ".brokenpack"] = result.Quote(
            "Package existed in confDB, but had no entries in Configurables module? Perhaps the python code was broken in this package : "
            + brokenpackages.__str__())
    if len(failedpackages):
        causes.append("Entire package failed")
        result[testname + ".failedpack"] = result.Quote(
            "All configurables in a package failed to instantiate. Check stderr, definitely a problem with the configurables themselves. : "
            + failedpackages.__str__())
    if len(mixedpackages):
        causes.append("Entire package broken or failed")
        result[testname + ".mixedpack"] = result.Quote(
            "All configurables either did not make it to Configurables module or failed to instantiate, probably the module is broken : "
            + mixedpackages.__str__())
    if len(failedconfigurables):
        causes.append("Configurables failed to instantiate")
        result[testname + ".failed"] = result.Quote(
            "Failed to instantiate : " + mixedpackages.__str__())
    if len(brokenconfigurables):
        causes.append(
            "Configurables which didn't make it to Configurables module")
        result[testname + ".broken"] = result.Quote(
            "Failed to make it to Configurables module : " +
            mixedpackages.__str__())
    #only fail if there are a lot of modifications
    if len(movedpackages + addedpackages) > 20:
        causes.append(
            "More than 20 svn moves have taken place, please update the reference! "
        )
    ...
    if len(missingconfigurables) > threshold:
        causes.append("More than " + str(threshold) +
                      " missing configurable(s)")
    missingtopass = [m for m in missingpackages if m not in failIfMissing]
    if len(missingtopass) > 20:
        causes.append(
            "More than 20 packages are missing and ignored, check and update the reference! "
        )
    #not of themselves a problem
    if len(movedpackages):
        result[testname + ".movedpack"] = result.Quote(
            "Moved packages in svn : " + movedpackages.__str__())
    #not of themselves a problem
    if len(missingtopass):
        result[testname + ".ignoremissingpack"] = result.Quote(
            "Missing packages which were ignored : " + missingtopass.__str__())
    #not of themselves a problem
    if len(addedpackages):
        result[testname + ".addedpack"] = result.Quote(
            "Added packages in svn : " + addedpackages.__str__())
    #not of themselves a problem
    if len(missingconfigurables):
        result[testname + ".missing"] = result.Quote(
            "Missing configurables : " + missingconfigurables.__str__())
    if len(extraconfigurables):
        result[testname + ".added"] = result.Quote(
            "Added configurables : " + extraconfigurables.__str__())
