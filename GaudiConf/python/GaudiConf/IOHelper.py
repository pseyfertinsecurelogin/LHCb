...
class IOHelper(object):
    ...
    _inputSvcTypDict = {
        'ROOT': "SVC='Gaudi::RootEvtSelector'",
        'POOL': "TYP='POOL_ROOT'",
    ...
    _externalPerServices = {
        "FileRecordDataSvc": 'FSR',
    ...
    def __init__(self, Input=None, Output=None):
        ...
        if self._outputPersistency == "POOL" or self._inputPersistency == "POOL":
            raise ValueError(
                "POOL is completely deprecated, fix your options or use an older software stack"
            )

        if self._inputPersistency == 'ROOT' or self._outputPersistency == 'ROOT':
            if not self.isRootSupported():
                raise TypeError(
                    "ROOT persistency is not supported in this Application version"
                    + "Ask your release manager for details or change to POOL")

        if self._outputPersistency == "FSR" and self._inputPersistency not in [
                'ROOT'
        ]:
            raise TypeError(
                "FSR is not a proper persistency type. To configure services, you would need to specify a proper type."
            )
    ...
    def _doConfFileRecords(self, fileSvc):
        ...
        '''
        from Gaudi.Configuration import (FileRecordDataSvc, ApplicationMgr,
                                         PersistencySvc)
    ...
    def _isDressed(self, filename):
        ...
        '''
        return ("DATAFILE='" in filename or " OPT='" in filename
                or "SVC=" in filename or "TYP='" in filename)
    ...
    def setupServices(self):
        ...
        from Gaudi.Configuration import (EventDataSvc, ApplicationMgr,
                                         EventPersistencySvc)
        # Set up the TES
        ...
        if self._inputPersistency == 'ROOT' or self._outputPersistency == 'ROOT':
            ...
            if not rootSvc.isPropertySet("VetoBranches"):
                rootSvc.VetoBranches = ["*"]
            if not rootSvc.isPropertySet("CacheBranches"):
    ...
    def clearServices(self):
        ...
        ApplicationMgr().ExtSvc = [
            svc for svc in ApplicationMgr().ExtSvc if svc not in active
        ]
        ...
        for svc in extsvc:
            if hasattr(svc, "CnvServices") or "CnvServices" in svc.__slots__:
                svc.CnvServices = [
                    asvc for asvc in svc.CnvServices if asvc not in active
                ]
    ...
    def servicesExist(self):
        ...
        return (self._inputPersistency in self.activePersistencies()
                and self._outputPersistency in self.activePersistencies())
    ...
    def convertConnectionStrings(self, filelist, IO, force=False):
        ...
        if not force and IO == "O" and (self._outputPersistency == "MDF"
                                        or self._outputPersistency == "FSR"):
        ...
        for file in filelist:
            ...
            if not force and (self.detectFileType(file) == "MDF" or
                              (self.detectFileType(file) == "FSR"
                               and IO == "O")):
                retlist.append(file)
    ...
    def activeStreams(self):
        ...
        for sequencer in allConfigurables:
            if self._isSequencer(sequencer):
                if (not hasattr(allConfigurables[sequencer], "Members")) or (
                        allConfigurables[sequencer].Members is None):
                    continue
            ...
            if hasattr(stream, 'Output'):
                if stream.Output is not None:
                    ...
                    if len(stream.Output) and type(stream.Output) is str:
                        ...
                        if self.detectFileType(
                                stream.Output) not in ["MDF", "ETC", "FSR"]:
                            stream.Output = self.dressFile(
                                self.undressFile(stream.Output), "O")
                            continue
    ...
    def outputAlgs(self, filename, writer="OutputStream", writeFSR=True):
        ...
        dst_filename = ""
        if hasattr(winstance,
                   "Output") or (hasattr(winstance, '__slots__')
                                 and "Output" in winstance.__slots__):
            #build up the filename
            dst_filename = self.dressFile(filename, 'O')
            winstance.Output = dst_filename
        elif hasattr(
                winstance,
                "Connection") or (hasattr(winstance, '__slots__')
                                  and "Connection" in winstance.__slots__):
        ...
        FSRWriter = self._fsrWriter(
            filename,
            writer=GaudiConfigurables.RecordStream("FSR" + writer.replace(
                '/', '').replace('::', '').replace('__', '')))
    ...
    def _fsrWriter(self, filename=None, writer="RecordStream"):
        ...
        if stype not in ["FSR"]:
            raise TypeError(
                "Expecting a recognised FSR writer, but you have supplied something else"
            )
    ...
    def outStream(self, filename, writer="OutputStream", writeFSR=True):
        ...
        if ApplicationMgr().OutStream is None or len(
                ApplicationMgr().OutStream) == 0:
...
class IOExtension(object):
    ...
    def outputAlgs(self, filename, writer="OutputStream"):
        ...
        return IOHelper(ftype, ftype).outputAlgs(
            filename, writer, writeFSR=(ftype != 'MDF'))
    ...
    def outStream(self, filename, writer="OutputStream"):
        ...
        '''
        ftype = self.detectFileType(filename)
        return IOHelper(ftype, ftype).outStream(
            filename, writer, writeFSR=(ftype != 'MDF'))
