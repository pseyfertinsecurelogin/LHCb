...
class DigiConf(LHCbConfigurableUser):
    ...
    __slots__ = {
        ...
        None,
        "Detectors":
        ['Velo', 'TT', 'IT', 'OT', 'Rich', 'Tr', 'Calo', 'Muon', 'L0'],
        "DataType":
        "",
        "OutputName":
        "",
    ...
    _propertyDocDct = {
        'DigiType':
        """ Type of digi, can be ['Minimal','Default','Extended'] """,
        'Writer':
        """ Name of OutputStream writing the Digi file, if any """,
        'TAEPrev':
        """ Number of Prev Time Alignment Events to write """,
        'TAENext':
        """ Number of Next Time Alignment Events to write """,
        'SpilloverPaths':
        """ Paths to write out when spillover is enabled """,
        'EnablePack':
        """ Turn on/off packing of the DIGI data (where appropriate/available) """,
        'EnableUnpack':
        """ Configure the SIM unpacking via the Data On Demand Service """,
        'PackSequencer':
    ...
    def addMCParticleLinksSpillover(self, writer):
        ...
        if 'FT' in dets:
            writer.ItemList += [
                "/Event/Link/Raw/FT/LiteClustersWithSpillover#1"
            ]
    ...
    def addMCHitLinksSpillover(self, writer):
        ...
        if 'FT' in dets:
            writer.ItemList += [
                "/Event/Link/Raw/FT/LiteClusters2MCHitsWithSpillover#1"
            ]
    ...
    def _defineOutputData(self, dType, writer):
        ...
        if dType == "Minimal":
            ...
            SimConf().addMCVertices(
                writer,
                [''])  # Filtered, only primary vertices with no daughters
    ...
    def _doWritePOOL(self, dType, writer):
        ...
        """
        log.info(
            "%s.ItemList    = %s" % (self.getProp("Writer"), writer.ItemList))
        log.info("%s.OptItemList = %s" % (self.getProp("Writer"),
                                          writer.OptItemList))
    ...
    def _doUnpacking(self):
        if "Rich" in self.getProp("Detectors"):
            ...
            unp = DataPacking__Unpack_LHCb__MCRichDigitSummaryPacker_(
    ...
    def __apply_configuration__(self):
        ...
        self.setOtherProps(
            SimConf(),
            ["SpilloverPaths", "EnableUnpack", "EnablePack", "DataType"])
        self._doWrite()
