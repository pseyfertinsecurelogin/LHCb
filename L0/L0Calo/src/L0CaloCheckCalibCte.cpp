...
DECLARE_COMPONENT( L0CaloCheckCalibCte )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
L0CaloCheckCalibCte::L0CaloCheckCalibCte( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode L0CaloCheckCalibCte::initialize() {
  ...
  // Book 2D histograms
  bookCalo2D( "L0CteComp" + m_detectorName, "L0Constant Mismatch " + m_detectorName, m_detectorName );
...
StatusCode L0CaloCheckCalibCte::execute() {
  ...
  for ( const auto& adc : m_daq->adcs( -1 ) ) {
    LHCb::CaloCellID cellID = adc.cellID();
    if ( !( cellID.isPin() ) ) adcMap[cellID] = adc.adc();
  }

  auto fill = [&]( const LHCb::CaloCellID& id ) {
    // Plot the location of the error
    fillCalo2D( "L0CteComp" + m_detectorName, id, 1., "L0Constant Mismatch " + m_detectorName );
  };

  // now check the L0Calo ADC
  for ( const auto& itAdcMap : adcMap ) {
    // compute expected L0ADC
    if ( l0adcFromAdc( itAdcMap.second, itAdcMap.first ) != m_l0daq->l0Adc( itAdcMap.first ) ) {
      LHCb::CaloCellID id = itAdcMap.first;
      if ( msgLevel( MSG::DEBUG ) ) {
        debug() << "Mismatch ADC / L0ADC " << itAdcMap.first << " " << m_l0daq->l0Adc( itAdcMap.first ) << " "
                << itAdcMap.second << " " << l0adcFromAdc( itAdcMap.second, itAdcMap.first ) << endmsg;
        debug() << "Calib constant should be = " << m_calo->cellParam( id ).l0Constant()
                << " Row = " << m_calo->cellParam( id ).cardRow()
                << " Column = " << m_calo->cellParam( id ).cardColumn()
                << " Crate = " << m_calo->cardCrate( m_calo->cellParam( id ).cardNumber() )
                << " Slot = " << m_calo->cardSlot( m_calo->cellParam( id ).cardNumber() ) << endmsg;
      }
...
// L0ADC from ADC
//=============================================================================
int L0CaloCheckCalibCte::l0adcFromAdc( const int adc, const LHCb::CaloCellID& id ) const {
  if ( adc < 0 ) return 0;
