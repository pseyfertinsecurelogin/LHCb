...
//=============================================================================
L0CaloCandidatesFromRaw::L0CaloCandidatesFromRaw( const std::string& name, ISvcLocator* pSvcLocator )
    : L0FromRawBase( name, pSvcLocator ), m_convertTool( NULL ) {}
...
StatusCode L0CaloCandidatesFromRaw::execute() {
  ...
  if ( !rawEvt ) return Error( "No valid raw event location found", StatusCode::SUCCESS, 50 );
  ...
  if ( ( !errBanks.empty() ) ) {
    for ( const auto* eb : errBanks ) {
      readoutStatus.addStatus( eb->sourceID(), LHCb::RawBankReadoutStatus::Status::ErrorBank );
    }
  }
  ...
  if ( 0 == banks.size() ) {
    ...
    for ( const auto& bnk : banks ) {
      if ( LHCb::RawBank::MagicPattern != bnk->magic() ) {
        ...
        readoutStatus.addStatus( bnk->sourceID(), LHCb::RawBankReadoutStatus::Status::Corrupted );
        continue;
      }

      data.emplace_back( bnk->begin<unsigned int>(), bnk->end<unsigned int>() );
    ...
    if ( 0 == sourceZero ) readoutStatus.addStatus( 0, LHCb::RawBankReadoutStatus::Status::Missing );
    if ( 0 == sourceOne ) readoutStatus.addStatus( 1, LHCb::RawBankReadoutStatus::Status::Missing );
    if ( 1 < sourceZero ) readoutStatus.addStatus( 0, LHCb::RawBankReadoutStatus::Status::NonUnique );
    if ( 1 < sourceOne ) readoutStatus.addStatus( 1, LHCb::RawBankReadoutStatus::Status::NonUnique );
  }

  m_convertTool->convertRawBankToTES( data, outFull, out, version, readoutStatus );

  if ( writeOnTES() && statusOnTES() ) {
    // Now put the status on TES also
    LHCb::RawBankReadoutStatuss* statuss = getOrCreate<LHCb::RawBankReadoutStatuss, LHCb::RawBankReadoutStatuss>(
    ...
    if ( !status ) {
      ...
      if ( status->status() != readoutStatus.status() ) {
        for ( const auto& i : readoutStatus.statusMap() ) { status->addStatus( i.first, i.second ); }
      }
    }
  }
