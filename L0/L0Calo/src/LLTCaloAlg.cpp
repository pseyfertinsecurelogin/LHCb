...
// Constructor
//=============================================================================
LLTCaloAlg::LLTCaloAlg( const std::string& name, ISvcLocator* pSvcLocator ) : GaudiAlgorithm( name, pSvcLocator ) {
...
StatusCode LLTCaloAlg::initialize() {
  ...
  for ( eCard = 0; eCard < m_ecal->nCards(); ++eCard ) { m_ecalFe.push_back( LLTTriggerCard( eCard, m_ecal ) ); }
  ...
  for ( hCard = 0; hCard < m_hcal->nCards(); ++hCard ) m_hcalFe.push_back( LLTTriggerCard( hCard, m_hcal ) );

  info() << m_ecal->nCards() << " Ecal and " << m_hcal->nCards() << " Hcal front end cards." << endmsg;

  // Initialize needed tools
  m_adcsEcal = tool<ICaloTriggerAdcsFromRaw>( m_caloToolName, "EcalTriggerAdcTool", this );
  m_adcsHcal = tool<ICaloTriggerAdcsFromRaw>( m_caloToolName, "HcalTriggerAdcTool", this );
...
StatusCode LLTCaloAlg::execute() {
  ...
  for ( it = m_ecalFe.begin(); it != m_ecalFe.end(); ++it ) {
    ...
    cand = new LHCb::L0CaloCandidate( L0DUBase::CaloType::Electron, it->cellIdMax(), it->etMax(),
                                      it->etMax() * m_etScale, center, tol );
    out->add( cand );
  }
  ...
  for ( it = m_hcalFe.begin(); it != m_hcalFe.end(); ++it ) {
    ...
    center.SetX( center.x() + tol );
    center.SetY( center.y() + tol );
    cand = new LHCb::L0CaloCandidate( L0DUBase::CaloType::Hadron, it->cellIdMax(), it->etMax(), it->etMax() * m_etScale,
                                      center, tol );
    out->add( cand );
  }

  cand = new LHCb::L0CaloCandidate( L0DUBase::CaloType::SpdMult, LHCb::CaloCellID(), ( ecalMult + hcalMult ), 0., dummy,
                                    0. );
...
// Finalize.
//=============================================================================
StatusCode LLTCaloAlg::finalize() { return GaudiAlgorithm::finalize(); }
...
unsigned int LLTCaloAlg::sumEcalData() {
  ...
  for ( std::vector<LHCb::L0CaloAdc>::const_iterator itAdc = adcs.begin(); adcs.end() != itAdc; ++itAdc ) {
    ...
    if ( ( 0 == row ) && ( 0 <= down ) ) m_ecalFe[down].addEt( col, nRowCaloCard, adc, m_ECALThreshold );
    if ( ( 0 == col ) && ( 0 <= left ) ) m_ecalFe[left].addEt( nColCaloCard, row, adc, m_ECALThreshold );
