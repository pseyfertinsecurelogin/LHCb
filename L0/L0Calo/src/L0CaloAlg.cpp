...
int L0CaloAlg::s_ecalLUT[14][2] = {{1, 7},   {2, 8},   {25, 26}, {9, 11},  {10, 12}, {3, 5},   {4, 6},
                                   {15, 17}, {16, 18}, {23, 21}, {24, 22}, {27, 28}, {13, 19}, {14, 20}};

int L0CaloAlg::s_hcalLUT[4][16] = {{0, 0, 1, 3, 5, 7, 9, 11, 13, 5, 9, 14, 18, 0, 0, 0},
                                   {0, 13, 15, 17, 19, 21, 23, 6, 17, 3, 25, 1, 27, 4, 10, 0},
                                   {0, 15, 21, 19, 23, 21, 22, 8, 19, 25, 27, 13, 15, 17, 12, 0},
                                   {0, 0, 7, 11, 16, 20, 1, 3, 5, 7, 9, 11, 23, 0, 0, 0}};

//=============================================================================
// Constructor
//=============================================================================
L0CaloAlg::L0CaloAlg( const std::string& name, ISvcLocator* pSvcLocator )
    : L0AlgBase( name, pSvcLocator ), m_validPrs(), m_validPrsInner(), m_rawOutput( 2 ) {
  ...
  declareProperty( "L0CaloADCTool", m_l0caloadctool = "CaloTriggerAdcsFromRaw" );
...
StatusCode L0CaloAlg::initialize() {
  ...
  for ( hCard = 0; hCard < m_hcal->nCards(); ++hCard ) m_hcalFe.push_back( TriggerCard( hCard, m_hcal ) );
  ...
  // L0Calo emulation
  try {
    registerCondition( "Conditions/Calibration/Ecal/L0Calibration", m_l0Cond, &L0CaloAlg::updateL0Calibration );
  } catch ( GaudiException& e ) {
    error() << e << endmsg;
    error() << "Cannot access Ecal/Calibration/L0Calibration condition" << endmsg;
  ...
  double zRatio = m_hcal->cellSize( m_hcal->firstCellID( 1 ) ) / m_ecal->cellSize( m_ecal->firstCellID( 0 ) ) / 2.;
  ...
  for ( eCard = 0; eCard < m_ecal->nCards(); ++eCard ) {
    ...
    if ( 0 <= hCard ) {
      ...
      int offsetCol = ( m_hcal->cardFirstColumn( hCard ) - 16 ) * mag + 32 - m_ecal->cardFirstColumn( eCard );
      int offsetRow = ( m_hcal->cardFirstRow( hCard ) - 16 ) * mag + 32 - m_ecal->cardFirstRow( eCard );
      m_ecalFe[eCard].setHcalParams( hCard, mag, offsetCol, offsetRow );
      m_hcalFe[hCard].addEcalConnectedCard( eCard );
    } else {
      warning() << "Ecal card " << eCard << " not connected to HCAL " << endmsg;
    }
  }
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    for ( eCard = 0; eCard < m_ecal->nCards(); ++eCard ) {
      ...
      debug() << "Ecal card "
              << format( "Ecal card%4d Area%2d Row%3d Col%3d Validation%3d", eCard, m_ecal->cardArea( eCard ),
                         m_ecal->cardFirstRow( eCard ), m_ecal->cardFirstColumn( eCard ),
                         m_ecalFe[eCard].validationNumber() );

      debug() << " left, corner, down, prev "
              << format( "%3d %3d %3d %3d HCAL card %3d Mag. %1d Offset row %2d col %2d",
                         m_ecal->leftCardNumber( eCard ), m_ecal->cornerCardNumber( eCard ),
                         m_ecal->downCardNumber( eCard ), m_ecal->previousCardNumber( eCard ),
                         m_ecalFe[eCard].hcalCard(), m_ecalFe[eCard].hcalMag(), m_ecalFe[eCard].hcalOffsetRow(),
    ...
    for ( hCard = 0; m_hcal->nCards() > hCard; ++hCard ) {
      ...
      debug() << format( "Hcal card%4d Area%2d Row%3d Col%3d ", hCard, m_hcal->cardArea( hCard ),
                         m_hcal->cardFirstRow( hCard ), m_hcal->cardFirstColumn( hCard ) );
      debug() << " left, corner, down "
              << format( "%3d %3d %3d to %2d ECAL cards: ", m_hcal->leftCardNumber( hCard ),
                         m_hcal->cornerCardNumber( hCard ), m_hcal->downCardNumber( hCard ),
  ...
  info() << m_ecal->nCards() << " Ecal and " << m_hcal->nCards() << " Hcal front end cards." << endmsg;
  ...
  m_adcsEcal    = tool<ICaloTriggerAdcsFromRaw>( m_l0caloadctool, "EcalTriggerAdcTool", this );
  m_adcsHcal    = tool<ICaloTriggerAdcsFromRaw>( m_l0caloadctool, "HcalTriggerAdcTool", this );
...
StatusCode L0CaloAlg::execute() {
  ...
  std::vector<L0Candidate> allElectrons( m_nbValidation, L0Candidate( m_ecal ) ),
      allPhotons( m_nbValidation, L0Candidate( m_ecal ) ), allPi0Local( m_nbValidation, L0Candidate( m_ecal ) ),
  ...
  for ( eCard = 0; m_ecal->nCards() > eCard; ++eCard ) {
    ...
    if ( m_usePsSpd ) {
      if ( 0 < okPrs ) {
        if ( 0 < spdMask ) {
          if ( allElectrons[numVal].et() < etMax ) allElectrons[numVal].setCandidate( etMax, ID );
        } else {
          if ( allPhotons[numVal].et() < etMax ) allPhotons[numVal].setCandidate( etMax, ID );
        }
      }
      // Produces also the 'single card' pi0
      if ( 0 != prsMask ) { //== Request some preshower activity
        if ( allPi0Local[numVal].et() < etTot ) allPi0Local[numVal].setCandidate( etTot, ID );
      }
    } else {
      if ( allElectrons[numVal].et() < etMax ) allElectrons[numVal].setCandidate( etMax, ID );
      if ( allPhotons[numVal].et() < etMax ) allPhotons[numVal].setCandidate( etMax, ID );
    ...
    if ( 0 <= pCard ) {
      ...
      if ( ( ID2.area() != ID.area() ) || ( 1 < abs( diffRow ) ) || ( 1 < abs( diffCol ) ) )
        if ( allPi0Global[numVal].et() < etMaxTot ) allPi0Global[numVal].setCandidate( etMaxTot, ID2 );
    }
  } // eCard
  ...
  int firstColumn( 0 ), firstValidRow( 0 ), firstRow( 0 ), firstValidCol( 0 );
  ...
  for ( hCard = 0; hCard < m_hcal->nCards(); ++hCard ) {
    ...
    if ( 0 == maxHcalEt ) {
      ...
      if ( ( firstRow != firstValidRow ) || ( firstColumn != firstValidCol ) ) continue;
    }
    ...
    for ( eLink = 0; eLink < m_hcalFe[hCard].numberOfEcalCards(); ++eLink ) {
      ...
      if ( m_ecalFe[eCard].match_included( hCol, hRow ) ) {
        if ( m_addEcalToHcal ) ecalEt = m_ecalFe[eCard].etMax();
      } else
        ecalEt = 0;
      ...
      else if ( tvbMap[tvbNumber] < ecalEt )
        tvbMap[tvbNumber] = ecalEt;
    }
  ...
  L0Candidate electron( m_ecal ), photon( m_ecal ), pi0Local( m_ecal ), pi0Global( m_ecal );

  // Now remove the candidates with 0 energy
  std::vector<L0Candidate>::iterator itElectron, itPhoton, itPi0Local, itPi0Global;
  itElectron  = std::remove_if( allElectrons.begin(), allElectrons.end(), L0Calo::zeroEt );
  itPhoton    = std::remove_if( allPhotons.begin(), allPhotons.end(), L0Calo::zeroEt );
  itPi0Local  = std::remove_if( allPi0Local.begin(), allPi0Local.end(), L0Calo::zeroEt );
  itPi0Global = std::remove_if( allPi0Global.begin(), allPi0Global.end(), L0Calo::zeroEt );

  if ( !allElectrons.empty() )
    electron = ( *std::max_element( allElectrons.begin(), itElectron, L0Calo::compareL0Candidate ) );
  if ( !allPhotons.empty() ) photon = ( *std::max_element( allPhotons.begin(), itPhoton, L0Calo::compareL0Candidate ) );
  if ( !allPi0Local.empty() )
    pi0Local = ( *std::max_element( allPi0Local.begin(), itPi0Local, L0Calo::compareL0Candidate ) );
  if ( !allPi0Global.empty() )
    pi0Global = ( *std::max_element( allPi0Global.begin(), itPi0Global, L0Calo::compareL0Candidate ) );
  ...
  L0Candidate hadronMaster( m_hcal ), hadronSlave1( m_hcal ), hadronSlave2( m_hcal );
  if ( !sbHcalInput[0].empty() )
    hadronMaster = ( *std::max_element( sbHcalInput[0].begin(), sbHcalInput[0].end(), L0Calo::compareL0Candidate ) );
  if ( !sbHcalInput[1].empty() )
    hadronSlave1 = ( *std::max_element( sbHcalInput[1].begin(), sbHcalInput[1].end(), L0Calo::compareL0Candidate ) );
  if ( !sbHcalInput[2].empty() )
    hadronSlave2 = ( *std::max_element( sbHcalInput[2].begin(), sbHcalInput[2].end(), L0Calo::compareL0Candidate ) );

  // find the highest hadron
  // with priority: Master, Slave1 , Slave2
  L0Candidate hadron( m_hcal );
  if ( ( hadronMaster.et() >= hadronSlave1.et() ) && ( hadronMaster.et() >= hadronSlave2.et() ) )
    hadron = hadronMaster;
  ...
  // save sum Et
  int sumEt = std::accumulate( sbHcalOutputSum.begin(), sbHcalOutputSum.end(), 0 );
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    ...
    debug() << "== L0ProcessorData Summary: " << L0Calo->size() << " entries." << endmsg;
  ...
  for ( it = allPhotons.begin(); it != itPhoton; ++it ) saveInRawEvent( 0, 0, 0, L0DUBase::CaloType::Photon, *it, 1 );
  ...
  saveInRawEvent( 1, 1, 0, L0DUBase::CaloType::SumEt, sbHcalOutputSum[1], 0 );
  // Slave 2
  ...
  saveInRawEvent( 1, 2, 0, L0DUBase::CaloType::SumEt, sbHcalOutputSum[2], 0 );
  // Master
  ...
  saveInRawEvent( 0, 1, 0, L0DUBase::CaloType::SumEt, sbHcalOutputSum[1], 0 );
  saveInRawEvent( 0, 2, 0, L0DUBase::CaloType::SumEt, sbHcalOutputSum[2], 0 );
  ...
  for ( itSpd = m_spdMult.begin(); itSpd != m_spdMult.end(); ++itSpd )
    saveInRawEvent( 0, 0, 0, L0DUBase::CaloType::SpdMult, kk++, ( *itSpd ), 1 );
  saveInRawEvent( 1, 0, 0, L0DUBase::CaloType::SpdMult, 0, totSpdMult, 1 );
  ...
  if ( writeOnTES() ) {
    ...
    if ( msgLevel( MSG::DEBUG ) ) debug() << "L0CaloCandidatesFromRawBank Registered output in TES" << endmsg;

    LHCb::RawBankReadoutStatus readoutStatus( LHCb::RawBank::L0Calo );
    m_bankToTES->convertRawBankToTES( m_rawOutput, outFull, out, m_bankVersion, readoutStatus );
  }
...
StatusCode L0CaloAlg::finalize() {
  if ( 0 != m_nbEvents ) {
    info() << format( "Average bank size : %7.1f words RAW.", m_totRawSize / m_nbEvents ) << endmsg;
  }
...
void L0CaloAlg::sumEcalData() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Found " << adcs.size() << " ECAL adcs" << endmsg;
  ...
  for ( std::vector<LHCb::L0CaloAdc>::const_iterator itAdc = adcs.begin(); adcs.end() != itAdc; ++itAdc ) {
    ...
    if ( MSG::VERBOSE >= msgLevel() ) verbose() << id << format( " adc %3d", adc ) << endmsg;
    ...
    if ( ( 0 == row ) && ( 0 <= down ) ) m_ecalFe[down].addEt( col, nRowCaloCard, adc );
    if ( ( 0 == col ) && ( 0 <= left ) ) m_ecalFe[left].addEt( nColCaloCard, row, adc );
    if ( ( 0 == col ) && ( 0 == row ) && ( 0 <= corner ) ) m_ecalFe[corner].addEt( nColCaloCard, nRowCaloCard, adc );
  }
}
...
void L0CaloAlg::sumHcalData() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Found " << adcs.size() << " HCAL adcs" << endmsg;
  ...
  for ( std::vector<LHCb::L0CaloAdc>::const_iterator itAdc = adcs.begin(); adcs.end() != itAdc; ++itAdc ) {
    ...
    if ( MSG::VERBOSE >= msgLevel() )
      verbose() << id
                << format( " adc %3d card%3d row%3d col%3d, down%3d left%3d corner%3d", adc, card, row, col, down, left,
                           corner )
                << endmsg;

    m_hcalFe[card].addEt( col, row, adc );
    if ( ( 0 == row ) && ( 0 <= down ) )
      if ( ( ( m_hcal->cardFirstColumn( card ) + col ) >= m_hcal->cardFirstValidColumn( down ) ) &&
           ( ( m_hcal->cardFirstColumn( card ) + col ) <= m_hcal->cardLastValidColumn( down ) ) )
        m_hcalFe[down].addEt( col, nRowCaloCard, adc );
    if ( ( 0 == col ) && ( 0 <= left ) )
      if ( ( ( m_hcal->cardFirstRow( card ) + row ) >= m_hcal->cardFirstValidRow( left ) ) &&
           ( ( m_hcal->cardFirstRow( card ) + row ) <= m_hcal->cardLastValidRow( left ) ) )
        m_hcalFe[left].addEt( nColCaloCard, row, adc );
    if ( ( 0 == col ) && ( 0 == row ) && ( 0 <= corner ) ) m_hcalFe[corner].addEt( nColCaloCard, nRowCaloCard, adc );
  }
}
...
void L0CaloAlg::addPrsData() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Found " << ids.size() << " PRS bits" << endmsg;
  ...
  for ( std::vector<LHCb::CaloCellID>::const_iterator itID = ids.begin(); ids.end() != itID; ++itID ) {
    ...
    if ( ( 0 == row ) && ( 0 <= down ) ) m_ecalFe[down].setPrs( col, nRowCaloCard );
    if ( ( 0 == col ) && ( 0 <= left ) ) m_ecalFe[left].setPrs( nColCaloCard, row );
    if ( ( 0 == col ) && ( 0 == row ) && ( 0 <= corner ) ) m_ecalFe[corner].setPrs( nColCaloCard, nRowCaloCard );
  }
}
//=============================================================================
...
void L0CaloAlg::addSpdData() {
  ...
  for ( unsigned int i = 0; i < m_spdMult.size(); ++i ) m_spdMult[i] = 0;
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Found " << ids.size() << " SPD bits" << endmsg;
  ...
  for ( std::vector<LHCb::CaloCellID>::const_iterator itID = ids.begin(); ids.end() != itID; ++itID ) {
    ...
    if ( ( 0 == row ) && ( 0 <= down ) ) m_ecalFe[down].setSpd( col, nRowCaloCard );
    if ( ( 0 == col ) && ( 0 <= left ) ) m_ecalFe[left].setSpd( nColCaloCard, row );
    if ( ( 0 == col ) && ( 0 == row ) && ( 0 <= corner ) ) m_ecalFe[corner].setSpd( nColCaloCard, nRowCaloCard );
...
//=============================================================================
void L0CaloAlg::saveInRawEvent( int io, int slave, int mask, int type, int cardNumber, int spdMult,
                                unsigned int bank ) {
  ...
  if ( type != L0DUBase::CaloType::SpdMult ) {
    warning() << "Should be of type CaloSpdMult and is of type " << type << endmsg;
    return;
  }

  int word = ( io << 31 ) | ( slave << 29 ) | ( mask << 28 ) | ( type << 24 ) | ( cardNumber << 10 ) | spdMult;
...
//=============================================================================
void L0CaloAlg::saveInRawEvent( int io, int slave, int mask, int type, L0Candidate& cand, unsigned int bank ) {
  ...
  int word = ( io << 31 ) | ( slave << 29 ) | ( mask << 28 ) | ( type << 24 ) | ( cand.ID().index() << 8 ) | cand.et();
  m_rawOutput[bank].push_back( word );
}
...
//=============================================================================
void L0CaloAlg::saveInRawEvent( int io, int slave, int mask, int type, int sumEt, unsigned int bank ) {
  if ( 0 == sumEt ) return;

  if ( L0DUBase::CaloType::SumEt == type || L0DUBase::CaloType::SpdMult == type ) {
    int word = ( io << 31 ) | ( slave << 29 ) | ( mask << 28 ) | ( type << 24 ) | sumEt;
    m_rawOutput[bank].push_back( word );
  } else
    warning() << "Should be of type CaloSumEt or CaloSpdMult.... " << endmsg;
}
...
void L0CaloAlg::createHCALLut() {
  ...
  validMap[0].push_back( 0 );
  validMap[0].push_back( 1 );
  validMap[0].push_back( 2 );
  validMap[0].push_back( 3 );
  validMap[1].push_back( 4 );
  validMap[1].push_back( 5 );
  validMap[1].push_back( 6 );
  validMap[1].push_back( -1 );
  validMap[2].push_back( 0 );
  validMap[2].push_back( 1 );
  validMap[2].push_back( 2 );
  validMap[2].push_back( 3 );
  validMap[3].push_back( 4 );
  validMap[3].push_back( 5 );
  validMap[3].push_back( 6 );
  validMap[3].push_back( -1 );
  validMap[4].push_back( 7 );
  validMap[4].push_back( 8 );
  validMap[4].push_back( 11 );
  validMap[4].push_back( 18 );
  validMap[5].push_back( 17 );
  validMap[5].push_back( 24 );
  validMap[5].push_back( 9 );
  validMap[5].push_back( 10 );
  validMap[6].push_back( 12 );
  validMap[6].push_back( 13 );
  validMap[6].push_back( 14 );
  validMap[6].push_back( -1 );
  validMap[7].push_back( 15 );
  validMap[7].push_back( 16 );
  validMap[7].push_back( 19 );
  validMap[7].push_back( -1 );
  validMap[8].push_back( 12 );
  validMap[8].push_back( 13 );
  validMap[8].push_back( 14 );
  validMap[8].push_back( -1 );
  validMap[9].push_back( 15 );
  validMap[9].push_back( 16 );
  validMap[9].push_back( -1 );
  validMap[9].push_back( 23 );
  validMap[10].push_back( 20 );
  validMap[10].push_back( -1 );
  validMap[10].push_back( -1 );
  validMap[10].push_back( -1 );
  validMap[11].push_back( 22 );
  validMap[11].push_back( 21 );
  validMap[11].push_back( -1 );
  validMap[11].push_back( -1 );
  validMap[12].push_back( 20 );
  validMap[12].push_back( 21 );
  validMap[12].push_back( -1 );
  validMap[12].push_back( -1 );
  validMap[13].push_back( 22 );
  validMap[13].push_back( -1 );
  validMap[13].push_back( -1 );
  validMap[13].push_back( -1 );
  validMap[14].push_back( 27 );
  validMap[14].push_back( -1 );
  validMap[14].push_back( -1 );
  validMap[14].push_back( -1 );
  validMap[15].push_back( 29 );
  validMap[15].push_back( 28 );
  validMap[15].push_back( -1 );
  validMap[15].push_back( -1 );
  validMap[16].push_back( 27 );
  validMap[16].push_back( 28 );
  validMap[16].push_back( -1 );
  validMap[16].push_back( -1 );
  validMap[17].push_back( 29 );
  validMap[17].push_back( -1 );
  validMap[17].push_back( -1 );
  validMap[17].push_back( -1 );
  validMap[18].push_back( 26 );
  validMap[18].push_back( 33 );
  validMap[18].push_back( 34 );
  validMap[18].push_back( -1 );
  validMap[19].push_back( 35 );
  validMap[19].push_back( 36 );
  validMap[19].push_back( 37 );
  validMap[19].push_back( -1 );
  validMap[20].push_back( -1 );
  validMap[20].push_back( 33 );
  validMap[20].push_back( 34 );
  validMap[20].push_back( 30 );
  validMap[21].push_back( 35 );
  validMap[21].push_back( 36 );
  validMap[21].push_back( 37 );
  validMap[21].push_back( -1 );
  validMap[22].push_back( 39 );
  validMap[22].push_back( 40 );
  validMap[22].push_back( 25 );
  validMap[22].push_back( 32 );
  validMap[23].push_back( 31 );
  validMap[23].push_back( 38 );
  validMap[23].push_back( 41 );
  validMap[23].push_back( 42 );
  validMap[24].push_back( 43 );
  validMap[24].push_back( 44 );
  validMap[24].push_back( 45 );
  validMap[24].push_back( 46 );
  validMap[25].push_back( 47 );
  validMap[25].push_back( 48 );
  validMap[25].push_back( 49 );
  validMap[25].push_back( -1 );
  validMap[26].push_back( 43 );
  validMap[26].push_back( 44 );
  validMap[26].push_back( 45 );
  validMap[26].push_back( 46 );
  validMap[27].push_back( 47 );
  validMap[27].push_back( 48 );
  validMap[27].push_back( 49 );
  validMap[27].push_back( -1 );
      ...
      if ( -1 == ( validMap[valid] )[input] ) {
        for ( int ecalCard = 0; ecalCard < 8; ++ecalCard ) {
          for ( int ecalAdd = 0; ecalAdd < 32; ++ecalAdd ) {
            for ( int hcalAdd = 0; hcalAdd < 32; ++hcalAdd ) { theFile << "0 "; }
            theFile << std::endl;
          }
          theFile << "-------------------------------------------------------" << std::endl;
        }
