...
//=============================================================================
CaloTriggerAdcsFromCaloRaw::CaloTriggerAdcsFromCaloRaw( const std::string& type, const std::string& name,
...
StatusCode CaloTriggerAdcsFromCaloRaw::initialize() {
  ...
  if ( "Ecal" == m_detectorName ) {
    ...
    m_adcs = tool<ICaloDataProvider>( "CaloDataProvider", "EcalReadoutTool", this );
  } else if ( "Hcal" == m_detectorName ) {
    m_calo = getDet<DeCalorimeter>( DeCalorimeterLocation::Hcal );
    m_adcs = tool<ICaloDataProvider>( "CaloDataProvider", "HcalReadoutTool", this );
  } else {
...
const std::vector<LHCb::L0CaloAdc>& CaloTriggerAdcsFromCaloRaw::adcs() {
  ...
  if ( rawEvt == nullptr ) Exception( "No CALO Raw bank in the event" );

  bool bank = ( 0 == ( rawEvt->banks( m_bank ) ).size() ) ? false : true;
  if ( !bank ) Exception( "No Calo packed bank in the event" );
...
//=============================================================================
const std::vector<LHCb::L0CaloAdc>& CaloTriggerAdcsFromCaloRaw::adcs( int ) { return adcs(); }

//=============================================================================
const std::vector<LHCb::L0CaloAdc>& CaloTriggerAdcsFromCaloRaw::adcs( const LHCb::RawBank& ) { return adcs(); }

const std::vector<LHCb::L0CaloAdc>& CaloTriggerAdcsFromCaloRaw::pinAdcs() { return adcs(); }

int CaloTriggerAdcsFromCaloRaw::l0adcFromAdc( const int adc, const LHCb::CaloCellID& id ) const {
  if ( adc < 0 ) return 0;
