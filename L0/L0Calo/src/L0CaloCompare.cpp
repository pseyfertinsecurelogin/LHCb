...
// Standard creator
L0CaloCompare::L0CaloCompare( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  m_mapCompareName.reserve( 5 );
  m_mapCompareName.resize( 15, "" );
  m_mapCompareTitle.reserve( 5 );
  m_mapCompareTitle.resize( 15, "" );
  m_mapAllName.reserve( 5 );
  m_mapAllName.resize( 15, "" );
  m_mapAllTitle.reserve( 5 );
  m_mapAllTitle.resize( 15, "" );

  m_errorCounterName.reserve( 5 );
  m_errorCounterName.resize( 15, "" );
  m_errorCounterTitle.reserve( 5 );
  m_errorCounterTitle.resize( 15, "" );
...
StatusCode L0CaloCompare::initialize() {
  ...
  m_histSpdMult_Comp = GaudiHistoAlg::book( "SpdMult_Comp", "SpdMult comparison", -101., 101., 202 );
  m_histSumEt_Comp   = GaudiHistoAlg::book( "SumEt_Comp", "SumEt comparison ", -101., 101., 202 );

  m_histTotalCount = GaudiHistoAlg::book( "TotalCounter_Comp", "Counter of compared candidates", 0, 4, 4 );

  m_histErrorCount = GaudiHistoAlg::book( "ErrorCounter_Comp", "Counter of compared candidates in error", 0, 4, 4 );
...
StatusCode L0CaloCompare::execute() {
  ...
  if ( std::binary_search( m_idles.begin(), m_idles.end(), bcId ) ) return StatusCode::SUCCESS;
  ...
  if ( m_fullMonitoring ) {
    inputLocationReference = LHCb::L0CaloCandidateLocation::Full + m_referenceDataSuffix;
    inputLocationCheck     = LHCb::L0CaloCandidateLocation::Full + m_checkDataSuffix;
  } else {
    inputLocationReference = LHCb::L0CaloCandidateLocation::Default + m_referenceDataSuffix;
    inputLocationCheck     = LHCb::L0CaloCandidateLocation::Default + m_checkDataSuffix;
  }

  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << "Execute will read " << inputLocationReference << " as a reference" << endmsg;
    debug() << "Execute will read " << inputLocationCheck << " to be checked " << endmsg;
  }
  ...
  for ( candRef = candidatesRef->begin(); candidatesRef->end() != candRef; ++candRef ) {
    ...
    case L0DUBase::CaloType::Hadron:
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "Type= " << type << " cellID = " << caloCell << " etCode = " << ( *candRef )->etCode()
                << " rawId= " << rawId << endmsg;
    ...
    case L0DUBase::CaloType::SpdMult:
      ...
      if ( msgLevel( MSG::DEBUG ) )
        debug() << " SpdMult (1) : etCode = " << ( *candRef )->etCode() << " rawId= " << rawId << endmsg;
      SpdMultRef = *candRef;
      break;
    case L0DUBase::CaloType::SumEt:
      ...
      if ( m_fullMonitoring ) break;
      if ( msgLevel( MSG::DEBUG ) )
        debug() << " SumEt : etCode = " << ( *candRef )->etCode() << " rawId= " << rawId << endmsg;
      SumEtRef = *candRef;
      break;
    default:
      break;
  ...
  if ( m_fullMonitoring ) {
    for ( candRef = candidatesDefaultRef->begin(); candidatesDefaultRef->end() != candRef; ++candRef ) {
      type = ( *candRef )->type();

      switch ( type ) {
      case L0DUBase::CaloType::SpdMult:
        if ( msgLevel( MSG::DEBUG ) )
          debug() << " SpdMult (2) : etCode = " << ( *candRef )->etCode() << " rawId= " << rawId << endmsg;
        SpdMultRef = *candRef;
        break;
      case L0DUBase::CaloType::SumEt:
        if ( msgLevel( MSG::DEBUG ) )
  ...
  for ( candCheck = candidatesCheck->begin(); candidatesCheck->end() != candCheck; ++candCheck ) {
    ...
    case L0DUBase::CaloType::Hadron:
      ...
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "Type= " << type << " cellID to check = " << caloCell << " etCode = " << etCodeCheck
                << " rawId= " << rawId << endmsg;
      ...
      if ( iterMap == mapRef[type].end() ) {
        if ( msgLevel( MSG::DEBUG ) )
          debug() << "          " << candidateTypeName( type ) << " L0cand not found ! " << endmsg;
        ...
        if ( !found ) {
          if ( msgLevel( MSG::DEBUG ) )
            debug() << " Same cell but different etCode : ref = " << etCodeRef << " check = " << etCodeCheck << endmsg;
          fillCalo2D( m_mapCompareName[type], caloCell, 1., m_mapCompareTitle[type] );
          m_errorCounterHisto[type]->fill( 1.5 );
    ...
    case L0DUBase::CaloType::SumEt:
      ...
      if ( m_fullMonitoring ) break;
      {
        if ( msgLevel( MSG::DEBUG ) ) debug() << " SumEt : etCode = " << etCodeCheck << endmsg;
        ...
        if ( 0 != SumEtRef ) {
          ...
          if ( msgLevel( MSG::DEBUG ) ) debug() << "SumEtRef etCode = " << sumRef << endmsg;
        }
        diff_SumEt = etCodeCheck - sumRef;
        if ( diff_SumEt > 100. )
          diff_SumEt = 100;
        else if ( diff_SumEt < -100. )
          diff_SumEt = -100;
    ...
    case L0DUBase::CaloType::SpdMult:
      ...
      if ( m_fullMonitoring ) break;
      {
        if ( msgLevel( MSG::DEBUG ) ) debug() << "SpdMult (3) : etCode = " << etCodeCheck << endmsg;
        m_histTotalCount->fill( 3.5 );
  ...
  if ( m_fullMonitoring ) {
    for ( candCheck = candidatesDefaultCheck->begin(); candidatesDefaultCheck->end() != candCheck; ++candCheck ) {
      etCodeCheck = ( *candCheck )->etCode();
      type        = ( *candCheck )->type();
      ...
      break;
      case L0DUBase::CaloType::SpdMult: {
        if ( msgLevel( MSG::DEBUG ) ) debug() << "SpdMult (4) : etCode = " << etCodeCheck << endmsg;
