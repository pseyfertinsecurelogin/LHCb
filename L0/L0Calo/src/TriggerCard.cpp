...
void TriggerCard::reset() {
  ...
  for ( i = 0; nColCaloCard >= i; ++i ) {
    for ( j = 0; nRowCaloCard >= j; ++j ) { et[i][j] = 0; }
  }
  for ( i = 0; nColCaloCard > i; ++i ) {
  ...
  m_etMax  = 0;
  m_etTot  = 0;
  m_colMax = m_detElem->cardFirstValidColumn( m_number ) - m_detElem->cardFirstColumn( m_number );
  m_rowMax = m_detElem->cardFirstValidRow( m_number ) - m_detElem->cardFirstRow( m_number );
  m_empty  = true;
}
