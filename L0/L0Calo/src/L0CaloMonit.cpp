...
//=============================================================================
L0CaloMonit::L0CaloMonit( const std::string& name, ISvcLocator* pSvcLocator )
    : Calo2Dview( name, pSvcLocator ), m_ecal( NULL ), m_hcal( NULL ), m_nEvents( 0 ), m_bcidHist( NULL ) {
  ...
  m_histName.reserve( 15 );
  m_histName.resize( 15, "" );
  ...
  m_histTitle.reserve( 15 );
  m_histTitle.resize( 15, "" );
  ...
  m_maxScale.reserve( 15 );
  m_maxScale.resize( 15, 0 );
...
StatusCode L0CaloMonit::initialize() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "==> Default Monitoring histograms booking " << endmsg;

  // Et and multiplicity spectra histogram first
  m_etHist.reserve( 7 );
  m_etHist.resize( 7, 0 );
  int i = 0;
  for ( i = 0; i <= L0DUBase::CaloType::SpdMult; ++i ) bookEtHist( i, m_etHist, "" );
  ...
  if ( m_lookForHotCells ) {
    ...
    m_freqHist.reserve( 5 );
    m_freqHist.resize( 5, v );
    for ( i = 0; i <= L0DUBase::CaloType::Pi0Global; ++i ) bookFreqHist( i, m_freqHist );
  }

  // Then 2D maps
  m_mapName.reserve( 5 );
  m_mapTitle.reserve( 5 );
  m_mapName.resize( 5, "" );
  m_mapTitle.resize( 5, "" );
  ...
  if ( m_fullMonitoring ) {
    if ( msgLevel( MSG::DEBUG ) ) debug() << "==> Full Monitoring histograms booking" << endmsg;
    AIDA::IHistogram1D*              h( 0 );
    std::vector<AIDA::IHistogram1D*> v( 14, h );
    m_crateHist.reserve( 7 );
    m_crateHist.resize( 7, v );
    for ( i = 0; i <= L0DUBase::CaloType::Pi0Global; ++i ) bookCrateHist( i, m_crateHist );
    m_etFullHist.reserve( 15 );
    m_etFullHist.resize( 15, 0 );
    for ( i = 0; i <= L0DUBase::CaloType::SumEtSlave2In; ++i ) bookEtHist( i, m_etFullHist, "Full" );
  }
...
StatusCode L0CaloMonit::execute() {
  ...
  if ( msgLevel( MSG::DEBUG ) )
    debug() << "Execute will read " << LHCb::L0CaloCandidateLocation::Default + m_inputDataSuffix << endmsg;

  LHCb::L0CaloCandidates* candidates =
      getIfExists<LHCb::L0CaloCandidates>( LHCb::L0CaloCandidateLocation::Default + m_inputDataSuffix );
  if ( NULL == candidates )
    return Warning( "No data at " + LHCb::L0CaloCandidateLocation::Default + m_inputDataSuffix, StatusCode::SUCCESS );
  ...
  for ( cand = candidates->begin(); candidates->end() != cand; ++cand ) {
    ...
    if ( msgLevel( MSG::DEBUG ) )
      debug() << " Event " << event << " Type  = " << theCand->type() << " Et  = " << theCand->etCode() << endmsg;
  ...
  if ( ( m_updateFrequency > 0 ) && ( m_lookForHotCells ) ) {
    ...
    if ( 0 == goForCheck ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "m_nEvents = " << m_nEvents << " go for check ..." << endmsg;
      printHotCellSummary();
    }
  }
  ...
  if ( m_fullMonitoring ) {
    ...
    LHCb::L0CaloCandidates* candidatesF =
        getIfExists<LHCb::L0CaloCandidates>( LHCb::L0CaloCandidateLocation::Full + m_inputDataSuffix );
    if ( NULL == candidatesF )
      return Warning( "No data at " + LHCb::L0CaloCandidateLocation::Full + m_inputDataSuffix, StatusCode::SUCCESS );
...
StatusCode L0CaloMonit::finalize() {
  ...
  if ( ( m_updateFrequency < 0 ) && ( m_lookForHotCells ) ) printHotCellSummary();

  std::vector<std::vector<AIDA::IHistogram1D*>>::iterator it;
  for ( it = m_crateHist.begin(); m_crateHist.end() != it; ++it ) ( *it ).clear();
  m_crateHist.clear();
  m_etHist.clear();
...
void L0CaloMonit::defaultMonitoring( const LHCb::L0CaloCandidate* cand ) {
  ...
  case L0DUBase::CaloType::Pi0Global:
    ...
    if ( m_lookForHotCells ) m_freqHist[type][caloCell.area()]->fill( caloCell.all(), 1. );
...
void L0CaloMonit::printHotCellSummary() {
  ...
  for ( int i = 0; i <= L0DUBase::CaloType::Pi0Global; ++i ) {
    ...
    for ( int j = 0; j < nArea; ++j ) {
      info() << "===        " << fullName( i ) << " Candidates " << area( j ) << " region              ===" << endmsg;
...
//=============================================================================
void L0CaloMonit::SearchForHotCellsAndReset( AIDA::IHistogram1D* hist, const int type ) {
  ...
  for ( i = 0; i < nBin; i++ )
    ...
    if ( data[i] > meanOcc * m_alarmThresholdRatio ) hotChannels.push_back( i );

  if ( !hotChannels.empty() ) {
    // Print the hot cells on screen
    info() << "Number of hit cells : " << nUsedCells << " mean occupancy : " << meanOcc
           << " alarm threshold : " << m_alarmThresholdRatio * meanOcc << endmsg;
    info() << "---------------------------------------------------------------" << endmsg;
    info() << "|   Data     |    Hot(?) Cell    |    Crate    | Slot     |"
           << "   Channel   |" << endmsg;
    info() << "---------------------------------------------------------------" << endmsg;

    int    card( -999 ), crate( -999 ), cardSlot( -999 ), channelNum( 0 ), cellChannel( 0 );
    double idAll( -1. );
    std::vector<LHCb::CaloCellID>::const_iterator itc;

    for ( std::vector<int>::iterator it = hotChannels.begin(); it != hotChannels.end(); ++it ) {
      ...
      for ( itc = detector( type )->cardChannels( card ).begin(); itc != detector( type )->cardChannels( card ).end();
            ++itc ) {
        if ( ( *itc ) == caloCell ) {
...
// Book a "Et" histogram
//=============================================================================
void L0CaloMonit::bookEtHist( const unsigned int i, std::vector<AIDA::IHistogram1D*>& hist,
                              const std::string& suffix ) {
  hist[i] = GaudiHistoAlg::book( m_histName[i] + suffix, m_histTitle[i] + suffix, 0, m_maxScale[i], m_maxScale[i] );
}

//=============================================================================
// Book a "Frequency" histogram
//=============================================================================
void L0CaloMonit::bookFreqHist( const unsigned int i, std::vector<std::vector<AIDA::IHistogram1D*>>& hist ) {
  unsigned int nZone( 0 );
  if ( i != L0DUBase::CaloType::Hadron )
    nZone = 3;
  else
    nZone = 2;
  ...
  for ( unsigned int j = 0; j < nZone; ++j ) {
    ...
    hist[i][j] =
        GaudiHistoAlg::book( "Freq" + abbrev( i ) + area( j ), fullName( i ) + " Candidate occurence " + area( j ),
...
//=============================================================================
void L0CaloMonit::bookCrateHist( const unsigned int i, std::vector<std::vector<AIDA::IHistogram1D*>>& hist ) {
  ...
  for ( unsigned int j = 0; j < nCrates; ++j )
    hist[i][j] = GaudiHistoAlg::book( abbrev( i ) + "Crate" + crateNumber( i, j ),
                                      fullName( i ) + " crate " + crateNumber( i, j ), 0, 16, 16 );
}
