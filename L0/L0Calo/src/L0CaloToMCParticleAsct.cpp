...
//=============================================================================
L0CaloToMCParticleAsct::L0CaloToMCParticleAsct( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode L0CaloToMCParticleAsct::execute() {
  ...
  for ( cand = candidates->begin(); candidates->end() != cand; ++cand ) {
    mcDigs = 0;
    if ( ( L0DUBase::CaloType::Electron == ( *cand )->type() ) || ( L0DUBase::CaloType::Photon == ( *cand )->type() ) ||
    ...
    if ( 0 != mcDigs ) {
      ...
      for ( int jj = 0; 2 > jj; jj++ ) {
        for ( int kk = 0; 2 > kk; kk++ ) {
          LHCb::CaloCellID   tmpCell = LHCb::CaloCellID( cell.calo(), cell.area(), cell.row() + jj, cell.col() + kk );
          LHCb::MCCaloDigit* dig     = mcDigs->object( tmpCell );
          if ( 0 != dig ) {
            SmartRefVector<LHCb::MCCaloHit> hits = dig->hits();
            for ( SmartRefVector<LHCb::MCCaloHit>::const_iterator ith = hits.begin(); hits.end() != ith; ith++ ) {
              const LHCb::MCParticle* part   = ( *ith )->particle();
              double                  energy = ( *ith )->activeE();
              eTot += energy;
              if ( msgLevel( MSG::VERBOSE ) )
                verbose() << "  cell " << tmpCell << " Part " << part->key() << " e " << energy << endmsg;
              bool add = true;
        ...
        for ( unsigned int ll = 0; parts.size() > ll; ll++ ) {
          if ( energies[ll] > m_minimalFraction * eTot ) {
            ...
            if ( msgLevel( MSG::DEBUG ) )
              debug() << " .. Relate MC " << parts[ll]->key() << " with fraction " << energies[ll] / eTot << endmsg;
          }
        }
      }
    }
