...
//=============================================================================
L0CaloPRSResetMonitor::L0CaloPRSResetMonitor( const std::string& name, ISvcLocator* pSvcLocator )
    : GaudiHistoAlg( name, pSvcLocator ), m_nEvents( 0 ), m_alarm( 0 ) {
  declareProperty( "InputData", m_inputData = LHCb::L0CaloCandidateLocation::Full );
  declareProperty( "UpdateFrequency", m_updateFrequency = 80 );

  // define alarm published service
  declareInfo( "L0CaloPRSReset", m_alarm, "Empty board in L0 Electron, PRS Reset needed." );
}
...
StatusCode L0CaloPRSResetMonitor::initialize() {
  ...
  for ( eCard = 0; eCard < m_ecal->nCards(); ++eCard ) {
    if ( ( !m_ecal->isPinCard( eCard ) ) && ( !m_ecal->isParasiticCard( eCard ) ) ) m_electronCounter[eCard] = 0;
  }
...
StatusCode L0CaloPRSResetMonitor::execute() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Execute will read " << m_inputData << endmsg;

  LHCb::L0CaloCandidates* candidates = getIfExists<LHCb::L0CaloCandidates>( m_inputData );
  ...
  for ( cand = candidates->begin(); candidates->end() != cand; ++cand ) {
    ...
    if ( theCand->type() != L0DUBase::CaloType::Electron ) continue;

    oneElectronFound = true;

    // Find the candidate board number
    card = m_ecal->cardNumber( theCand->id() );
    if ( 0 != m_electronCounter.count( card ) ) m_electronCounter[card] = m_electronCounter[card] + 1;

    // Count the candidates in the extreme boards
    crate = m_ecal->cardCrate( card );
    slot  = m_ecal->cardSlot( card );
    if ( ( ( crate == 8 ) && ( slot == 1 ) ) || ( ( crate == 8 ) && ( slot == 13 ) ) ||
         ( ( crate == 21 ) && ( slot == 1 ) ) || ( ( crate == 21 ) && ( slot == 13 ) ) )
      m_nEvents++;
  }
  ...
  if ( m_nEvents > m_updateFrequency ) {
    ...
    if ( found ) {
      m_alarm = 1;
      Error( "No L0Electron candidates, reset of PRS will be done automatically", StatusCode::SUCCESS, 20 ).ignore();
    } else
      m_alarm = 0;
