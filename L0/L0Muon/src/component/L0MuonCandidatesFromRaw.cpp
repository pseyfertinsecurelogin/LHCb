...
L0MuonCandidatesFromRaw::L0MuonCandidatesFromRaw( const std::string& name, ISvcLocator* pSvcLocator )
    : L0FromRawBase( name, pSvcLocator ), m_outputTool( NULL ) {
...
StatusCode L0MuonCandidatesFromRaw::initialize() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    ...
    if ( m_mode > 0 ) {
      ...
      debug() << "  - L0MuonCtrlCand-1st part : controller boards final candidates     - should always be there"
              << endmsg;
...
StatusCode L0MuonCandidatesFromRaw::execute() {
  ...
  for ( int itae = -1 * tae_size; itae <= tae_size; ++itae ) {
    ...
    if ( sc.isFailure() ) {
      Warning( "Error from decodeRawBanks - skip this time slice", StatusCode::SUCCESS, 50 ).ignore();
      sc = m_outputTool->releaseRegisters();
      if ( sc.isFailure() ) {
        return Warning( "Fail to release registers - skip the rest of event", StatusCode::SUCCESS, 50 );
      }
      continue;
    }

    // Print Errors
    if ( msgLevel( MSG::DEBUG ) ) { m_outputTool->errors( debug() ); }
    ...
    if ( writeOnTES() ) {
      ...
      if ( sc.isFailure() ) {
        Warning( "Error from writeOnTES - skip this time slice", StatusCode::SUCCESS, 50 ).ignore();
        sc = m_outputTool->releaseRegisters();
        if ( sc.isFailure() ) {
          return Warning( "Fail to release registers - skip the rest of event", StatusCode::SUCCESS, 50 );
        }
    ...
    if ( writeProcData() ) {
      ...
      if ( sc.isFailure() ) {
        Warning( "Error from writeL0ProcessorData - skip this time slice", StatusCode::SUCCESS, 50 ).ignore();
        sc = m_outputTool->releaseRegisters();
        if ( sc.isFailure() ) {
          return Warning( "Fail to release registers - skip the rest of event", StatusCode::SUCCESS, 50 );
        }
