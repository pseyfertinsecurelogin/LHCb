...
DECLARE_COMPONENT( L0MuonFromRawTrans )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
L0MuonFromRawTrans::L0MuonFromRawTrans( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  for ( int i = 0; i < 2; ++i ) { m_ctrlRaw[i] = new L0Muon::CtrlRawCnv( i ); }
  // Converters for the banks of the 4 TELL1s connected to the processing boards
  for ( int i = 0; i < 4; ++i ) { m_procRaw[i] = new L0Muon::ProcRawCnv( i ); }
}
//=============================================================================
// Destructor
//=============================================================================
L0MuonFromRawTrans::~L0MuonFromRawTrans() {
  // Converters for the banks of the 2 TELL1s connected to the controller boards
  for ( int i = 0; i < 2; ++i ) { delete m_ctrlRaw[i]; }
  // Converters for the banks of the 4 TELL1s connected to the processing boards
  for ( int i = 0; i < 4; ++i ) { delete m_procRaw[i]; }
}
...
StatusCode L0MuonFromRawTrans::execute() {
  ...
  for ( int bx = -7; bx < 8; ++bx ) {
    ...
    sc = decodeRawBanks();
    if ( sc.isFailure() ) { return Error( "Error in decoding transparent bank", StatusCode::SUCCESS, 10 ); }

    if ( m_dumpError ) {
      sc = dumpErrors();
      if ( sc.isFailure() ) { return Error( "Error in dumpErrors", StatusCode::SUCCESS, 10 ); }
    }

    sc = writeOnTES();
    if ( sc.isFailure() ) { return Error( "Error in writeOnTES", StatusCode::SUCCESS, 10 ); }

    sc = releaseRegisters();
    if ( sc.isFailure() ) { return Error( "Error in releaseRegisters", StatusCode::SUCCESS, 10 ); }
...
StatusCode L0MuonFromRawTrans::finalize() {
  ...
  if ( 1 ) {
    ...
    for ( int i = 0; i < 4; ++i ) {
      errorCounters +=
          ( boost::format( "Q%d : %3d banks decoded : " ) % ( i + 1 ) % m_procRaw[i]->numberOfDecodedBanks() ).str();
...
StatusCode L0MuonFromRawTrans::writeOnTES() {
    ...
    for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
      ...
      if ( l0mcand == NULL ) {
        if ( msgLevel( MSG::ERROR ) )
          error() << "writeOnTES: ctrl side " << i << " count# " << ievt << "\n"
                  << ( *itcand )->dump( "\t=> " ) << endmsg;
        continue;
      }
  ...
  for ( int i = 0; i < 2; ++i ) {
    ...
    for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
      ...
      if ( l0mcand == NULL ) {
        if ( msgLevel( MSG::ERROR ) )
          error() << "writeOnTES: ctrl side " << i << " count# " << ievt << " BCSU from CB\n"
                  << ( *itcand )->dump( "\t=> " ) << endmsg;
        continue;
      }
  ...
  for ( int i = 0; i < 4; ++i ) {
    ...
    for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
      ...
      if ( l0mcand == NULL ) {
        if ( msgLevel( MSG::ERROR ) )
          error() << "writeOnTES: proc Q" << ( i + 1 ) << " count# " << ievt << " BCSU from PB\n"
                  << ( *itcand )->dump( "\t=> " ) << endmsg;
        continue;
      }
  ...
  for ( int i = 0; i < 4; ++i ) {
    ...
    for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
      ...
      if ( l0mcand == NULL ) {
        if ( msgLevel( MSG::ERROR ) )
          error() << "writeOnTES: proc Q" << ( i + 1 ) << " count# " << ievt << " PU\n"
                  << ( *itcand )->dump( "\t=> " ) << endmsg;
        continue;
      }
  ...
  for ( int i = 0; i < 2; ++i ) {
    ...
    for ( int ii = 0; ii < 2; ++ii ) {
      ...
      // Fill errors
      if ( m_ctrlRaw[i]->decodingError( ii ) )
        l0mctrlerror->setHeader( 4 );
      else if ( m_ctrlRaw[i]->inError( ii ) )
        l0mctrlerror->setHeader( 2 );
      else
        l0mctrlerror->setHeader( 1 );
  ...
  for ( int iq = 0; iq < 4; ++iq ) {
    ...
    for ( int ib = 0; ib < 12; ++ib ) {
      ...
      // Fill errors
      if ( m_procRaw[iq]->decodingError( ib ) )
        l0mprocerror->setHeader( 4 );
      else if ( m_procRaw[iq]->inError( ib ) )
        l0mprocerror->setHeader( 2 );
      else
        l0mprocerror->setHeader( 1 );
...
LHCb::L0MuonCandidate* L0MuonFromRawTrans::l0muoncandidate( L0Muon::PMuonCandidate cand, int procVersion ) {
  ...
  // debug()<<"l0muoncandidate Seed"<<pads[2].toString()<<" M2: "<<pads[1].toString()<<" M1: "<<pads[0].toString()<<
  // endmsg;
