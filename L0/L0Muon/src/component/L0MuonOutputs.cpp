...
//=============================================================================
L0MuonOutputs::L0MuonOutputs( const std::string& type, const std::string& name, const IInterface* parent )
    : GaudiTool( type, name, parent ) {
  declareInterface<L0MuonOutputs>( this );

  // Converters for the banks of the 2 TELL1s connected to the controller boards
  for ( int i = 0; i < 2; ++i ) { m_ctrlCand[i] = new L0Muon::CtrlCandCnv( i ); }
...
L0MuonOutputs::~L0MuonOutputs() {
  // Converters for the banks of the 2 TELL1s connected to the controller boards
  for ( int i = 0; i < 2; ++i ) { delete m_ctrlCand[i]; }
  // Converters for the banks of the 4 TELL1s connected to the processing boards
  for ( int i = 0; i < 4; ++i ) {
    delete m_procCand[i];
    delete m_procData[i];
...
StatusCode L0MuonOutputs::decodeRawBanks( std::string rawInputEvent, bool useRootInTES, bool statusOnTES ) {
  ...
  if ( NULL == rawEvt ) return Error( "RawEvent not found at " + rawInputEvent, StatusCode::FAILURE, 50 );
...
StatusCode L0MuonOutputs::decodeRawBanks( LHCb::RawEvent* rawEvt, bool statusOnTES ) {
  ...
  if ( banks.size() == 0 ) {
    ...
    for ( auto itBnk = banks.begin(); banks.end() != itBnk; ++itBnk ) {
      ...
      if ( error_bank[srcID] ) { ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::ErrorBank ); }
      if ( LHCb::RawBank::MagicPattern != ( *itBnk )->magic() ) {
        // report an error and return without decoding
        ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Corrupted );
        Error( "L0MuonCtrlCand :  Magic pattern is wrong", StatusCode::FAILURE, 50 ).ignore();
        continue;
      }
      int size = ( *itBnk )->size() / 4;
      if ( size == 0 ) {
        ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Empty );
        continue;
      }
      ...
      if ( bankVersion == 0 ) { // DC06
        ...
        if ( srcID == 0 ) {
          ...
          if ( msgLevel( MSG::VERBOSE ) )
            verbose() << "decodeRawBanks: L0Muon bank (version " << bankVersion << " ) found"
                      << ", sourceID is " << srcID << ", size is " << size << " decoding status= " << decoding_status
                      << endmsg;
          break; // only 1 bank
        }
      }                              // End  DC06
      else if ( bankVersion == 1 ) { // V1 (never used ?)
      ...
      else if ( bankVersion > 1 ) { // V2 and greater
        ...
        if ( decoding_status > 0 ) {
          ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::OK );
        } else if ( decoding_status == 0 ) {
          ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Incomplete );
        } else {
          ctrlCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Corrupted );
        }
      } // End V2 and greater
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "decodeRawBanks: L0Muon bank (version " << bankVersion << " ) found"
                  << ", sourceID is " << srcID << ", size is " << size << " Q" << ( ctrlSourceID( srcID ) + 1 )
  ...
  if ( m_mode > 0 ) {
    ...
    if ( proccandbanks.size() == 0 ) {
      ...
      for ( const LHCb::RawBank* bnk : proccandbanks ) {
        int srcID = bnk->sourceID();
        if ( error_bank[srcID] ) { procCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::ErrorBank ); }
        ...
        if ( decoding_status > 0 ) {
          procCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::OK );
        } else if ( decoding_status == 0 ) {
          procCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Incomplete );
        } else {
          procCandStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::Corrupted );
        }
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "decodeRawBanks: L0MuonProcCand bank (version " << bankVersion << " ) found"
                    << ", sourceID is " << srcID << ", size is " << size << ", Q" << ( procSourceID( srcID ) + 1 )
                    << ", decoding status= " << decoding_status << endmsg;
      }
    }
  }
  ...
  if ( m_mode > 0 ) {
    ...
    if ( procdatabanks.size() == 0 ) {
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "decodeRawBanks: no banks in " << LHCb::RawBank::L0MuonProcData << endmsg;
      procDataStatus.addStatus( 0, LHCb::RawBankReadoutStatus::Status::Missing );
    } else {
      m_procDataFlag = true;
      for ( const LHCb::RawBank* bnk : procdatabanks ) {
        int srcID = bnk->sourceID();
        if ( error_bank[srcID] ) { procDataStatus.addStatus( srcID, LHCb::RawBankReadoutStatus::Status::ErrorBank ); }
        if ( LHCb::RawBank::MagicPattern != bnk->magic() ) {
          // report an error and return without decoding
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "decodeRawBanks: L0MuonProcData bank (version " << bankVersion << " ) found"
                    << ", sourceID is " << srcID << ", size is " << size << ", Q" << ( procSourceID( srcID ) + 1 )
                    << ", decoding status= " << decoding_status << endmsg;
      }
    }
  ...
  if ( statusOnTES ) {
    ...
    if ( !status ) {
      ...
      if ( status->status() != ctrlCandStatus.status() ) {
        for ( const auto& s : ctrlCandStatus.statusMap() ) { status->addStatus( s.first, s.second ); }
      }
    }
    ...
    if ( m_mode > 0 ) {
      ...
      if ( !status ) {
        ...
        if ( status->status() != procCandStatus.status() ) {
          for ( const auto& s : procCandStatus.statusMap() ) { status->addStatus( s.first, s.second ); }
        }
      }
      ...
      if ( !status ) {
        ...
        if ( status->status() != procDataStatus.status() ) {
          for ( const auto& s : procDataStatus.statusMap() ) { status->addStatus( s.first, s.second ); }
        }
      }
    }
  }
...
StatusCode L0MuonOutputs::writeRawBanks() {
  ...
  if ( m_version == 0 ) { // Bank version =0
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << "writeRawBanks: L0Muon bank written (DC06) size is " << data.size() << endmsg;
    ++m_rawBankNorm;
    m_rawBankSizeTot += rawBankSize;
    return StatusCode::SUCCESS;
    ...
    for ( int i = 0; i < 2; ++i ) {
      ...
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "writeRawBanks: L0Muon bank written (version " << m_version << " ) size is " << data.size()
                  << endmsg;
      rawBankSize += data.size();
    }
    ...
    for ( int i = 0; i < 4; ++i ) {
      ...
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "writeRawBanks: L0MuonProcCand bank written (version " << m_version << " ) size is " << data.size()
                  << endmsg;
      rawBankSize += data.size();
    }
    ...
    if ( m_mode > 0 ) {
      for ( int i = 0; i < 4; ++i ) {
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          debug() << "writeRawBanks: L0MuonProcData bank written (version " << m_version << " ) size is " << data.size()
                  << endmsg;
        rawBankSize += data.size();
      }
    }
    ...
    for ( int i = 0; i < 2; ++i ) {
      ...
      if ( encoding_status < 1 ) {
        if ( msgLevel( MSG::DEBUG ) )
          debug() << "writeRawBanks: L0Muon bank "
                  << "with srcID= " << ctrlSourceID_inv( i ) << " ( "
                  << "version " << m_version << ", mode " << m_mode << ", compression " << m_compression << " )"
                  << " size is " << data.size() << " -> encoding error: status= " << encoding_status << endmsg;
        continue;
      }
      raw->addBank( ctrlSourceID_inv( i ), LHCb::RawBank::L0Muon, m_version, data );
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "writeRawBanks: L0Muon bank written "
                  << "with srcID= " << ctrlSourceID_inv( i ) << " ( "
                  << "version " << m_version << ", mode " << m_mode << ", compression " << m_compression << " )"
                  << " size is " << data.size() << " -> encoding error: status= " << encoding_status << endmsg;
      rawBankSize += data.size();
    ...
    if ( m_mode > 0 ) {
      for ( int i = 0; i < 4; ++i ) {
        ...
        if ( encoding_status < 1 ) {
          if ( msgLevel( MSG::DEBUG ) )
            debug() << "writeRawBanks: L0MuonProcCand bank "
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "writeRawBanks: L0MuonProcCand bank written "
                    << "with srcID= " << procSourceID_inv( i ) << " ( "
                    << "version " << m_version << ", mode " << m_mode << ", compression " << m_compression << " )"
                    << " size is " << data.size() << " -> encoding error: status= " << encoding_status << endmsg;
        rawBankSize += data.size();
    ...
    if ( m_mode > 0 ) {
      for ( int i = 0; i < 4; ++i ) {
        ...
        if ( encoding_status < 1 ) {
          if ( msgLevel( MSG::DEBUG ) )
            debug() << "writeRawBanks: L0MuonProcData bank "
                    << "with srcID= " << procSourceID_inv( i ) << " ( "
                    << "version " << m_version << ", mode " << m_mode << ", compression " << m_compression << " )"
                    << " size is " << data.size() << " -> encoding error: status= " << encoding_status << endmsg;
          continue;
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "writeRawBanks: L0MuonProcData bank written "
                    << "with srcID= " << procSourceID_inv( i ) << " ( "
                    << "version " << m_version << ", mode " << m_mode << ", compression " << m_compression << " )"
                    << " size is " << data.size() << " -> encoding error: status= " << encoding_status << endmsg;
        rawBankSize += data.size();
...
StatusCode L0MuonOutputs::writeOnTES( std::string l0context, std::string taeInTes ) {
  ...
  if ( m_ctrlCandFlag ) {
    for ( int i = 0; i < 2; ++i ) {
      for ( int iq = 0; iq < 2; ++iq ) {
        ...
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "writeOnTES: "
                    << " side" << i << " Q" << ( i * 2 + iq ) << " nb of cand= " << cands.size() << endmsg;
    ...
    for ( int i = 0; i < 2; ++i ) {
      for ( int iq = 0; iq < 2; ++iq ) {
        ...
        for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
          ...
          if ( l0mcand == NULL ) {
            if ( msgLevel( MSG::ERROR ) )
              error() << "writeOnTES: ctrl side " << i << " BCSU from CB"
                      << "\n"
                      << ( *itcand )->dump( "\t=> " ) << endmsg;
            continue;
          }
    ...
    for ( int i = 0; i < 4; ++i ) {
      for ( int ib = 0; ib < 12; ++ib ) {
        ...
        for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
          ...
          if ( l0mcand == NULL ) {
            if ( msgLevel( MSG::ERROR ) )
              error() << "writeOnTES: proc Q" << ( i + 1 ) << " BCSU from PB"
                      << "\n"
                      << ( *itcand )->dump( "\t=> " ) << endmsg;
            continue;
          }
    ...
    for ( int i = 0; i < 4; ++i ) {
      for ( int ib = 0; ib < 12; ++ib ) {
        ...
        for ( itcand = cands.begin(); itcand != cands.end(); ++itcand ) {
          ...
          if ( l0mcand == NULL ) {
            if ( msgLevel( MSG::ERROR ) )
              error() << "writeOnTES: proc Q" << ( i + 1 ) << " PU"
                      << "\n"
                      << ( *itcand )->dump( "\t=> " ) << endmsg;
            continue;
          }
  ...
  // Controller boards
  for ( int is = 0; is < 2; ++is ) { // Loop over sides (A&C)
    for ( int i = 0; i < 2; ++i ) {  // Loop over the quarters in side
      int iq = is * 2 + i;
      if ( m_ctrlCand[is]->inError( i ) ) { // Error detected in CU or SU
      ...
      for ( int ib = 0; ib < 12; ++ib ) {         // Loop over processing boards in quarter
        if ( m_ctrlCand[is]->inError( i, ib ) ) { // Error detected on a BCSU
  ...
  for ( int iq = 0; iq < 4; ++iq ) {         // Loop over quarters
    for ( int ib = 0; ib < 12; ++ib ) {      // Loop over processing boards in quarter
      if ( m_procCand[iq]->inError( ib ) ) { // Error detected on a BCSU
      ...
      for ( int ipu = 0; ipu < 4; ++ipu ) { // Loop over PU in a board
        if ( ( m_procCand[iq]->inError( ib, ipu ) ) ||
             ( m_procData[iq]->inError( ib, ipu ) ) ) { // Error detected on a PU
...
std::vector<unsigned int> L0MuonOutputs::DC06RawBanks() {
  ...
  int ncandidates = 0;
  for ( int i = 0; i < 2; ++i ) { ncandidates += m_ctrlCand[i]->muonCandidates().size(); }
...
LHCb::L0MuonCandidate* L0MuonOutputs::l0muoncandidate( L0Muon::PMuonCandidate cand ) {
  ...
  std::vector<LHCb::MuonTileID> pads =
      L0Muon::add2pads( cand->quarter(), cand->board(), cand->pu(), cand->colM3(), cand->rowM3(), cand->offM2(),
                        cand->offM1(), m_version, debugFlag );
  if ( pads.size() == 0 ) {
...
void L0MuonOutputs::statTot( MsgStream& os ) const {
  if ( 0 != m_nCandFinalNorm ) {
    os << "- Total number of candidates                 : " << format( "%8d", m_nCandFinalTot ) << endmsg;
    os << "- Number of Bx with at least 1 candidate     : " << format( "%8d", m_nTriggeredEvt )
    ...
    os << "- Average Pt of the candidates               : " << format( "    %7.2f MeV", m_averagePt ) << endmsg;
    os << "- Average charge of the candidates           : "
       << format( "      %5.2f (+/-%4.2f)", m_averageCh,
                  sqrt( ( 1 - ( m_averageCh * m_averageCh ) ) / m_nCandFinalNorm ) )
       << endmsg;
  }
...
void L0MuonOutputs::errors( MsgStream& msg ) const {
  ...
  for ( int i = 0; i < 2; ++i ) {
    for ( int ii = 0; ii < 2; ++ii ) {
      ...
      msg << " ";
      for ( int ich = 0; ich < 2; ++ich ) { msg << ctrlCandError->ctrl_bcid[ich]; }
      msg << " - ";
      ...
      msg << " ";
      for ( int ib = 0; ib < 12; ++ib ) {
        for ( int ich = 0; ich < 2; ++ich ) { msg << ctrlCandError->link[ib][ich]; }
        msg << " ";
      }
      msg << " ";
      for ( int ib = 0; ib < 12; ++ib ) { msg << ctrlCandError->bcsu_bcid_mismatch[ib]; }
      msg << " - ";
      for ( int ib = 0; ib < 12; ++ib ) { msg << ctrlCandError->bcsu_bcid[ib]; }
      msg << " - ";
      for ( int ib = 0; ib < 12; ++ib ) { msg << ctrlCandError->bcsu_status[ib]; }
      msg << endmsg;
    }
  }
  ...
  for ( int i = 0; i < 4; ++i ) {
    for ( int ib = 0; ib < 12; ++ib ) {
      ...
      msg << " ";
      for ( int ipu = 0; ipu < 4; ++ipu ) { msg << procCandError->pus_bcid[ipu]; }
      msg << " - ";
      msg << procCandError->bcsu_status;
      msg << " ";
      for ( int ipu = 0; ipu < 4; ++ipu ) { msg << procCandError->pus_status[ipu]; }
