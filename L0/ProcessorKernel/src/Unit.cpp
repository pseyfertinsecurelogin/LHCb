...
L0Muon::Unit::~Unit() {
  ...
  for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { delete *iu; }
}

void L0Muon::Unit::setParent( L0Muon::Unit* unit ) { m_parent = unit; }
...
void L0Muon::Unit::addInputRegister( L0Muon::Register* in, std::string rname ) { m_inputs[rname] = in; }
...
void L0Muon::Unit::addOutputRegister( L0Muon::Register* in, std::string rname ) { m_outputs[rname] = in; }
...
void L0Muon::Unit::dumpRegisters() {
  ...
  for ( ir = m_inputs.begin(); ir != m_inputs.end(); ir++ ) {
    ...
    for ( boost::dynamic_bitset<>::size_type i = 0; i < bits.size(); i++ ) { std::cout << bits[i] << std::endl; }
    std::cout << std::endl;
    ic++;
  }
...
void L0Muon::Unit::setDebugMode( bool debug ) {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->setDebugMode( debug ); }
  }
}
void L0Muon::Unit::setProperty( std::string name, L0Muon::Property value ) {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->setProperty( name, value ); }
  }
}
...
void L0Muon::Unit::setProperties( std::map<std::string, L0Muon::Property> properties ) {
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->setProperties( properties ); }
  }
...
void L0Muon::Unit::initialize() {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->initialize(); }
  }
}
...
void L0Muon::Unit::preexecute() {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->preexecute(); }
  }
}
...
void L0Muon::Unit::execute() {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->execute(); }
  }
}
...
void L0Muon::Unit::postexecute() {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->postexecute(); }
  }
}
...
void L0Muon::Unit::finalize() {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->finalize(); }
  }
}
...
L0Muon::Unit* L0Muon::Unit::parentByType( std::string utype ) {
  ...
  if ( uparent ) {
    ...
    if ( uparent->type() != utype ) { uparent = uparent->parentByType( utype ); }
  }
  return uparent;
}
...
void L0Muon::Unit::dumpUnitTree( int offset ) {
  ...
  if ( !m_units.empty() ) {
    std::vector<L0Muon::Unit*>::iterator iu;
    for ( iu = m_units.begin(); iu != m_units.end(); iu++ ) { ( *iu )->dumpUnitTree( offset + 4 ); }
  }
}
