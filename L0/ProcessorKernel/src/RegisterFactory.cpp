...
void L0Muon::RegisterFactory::selectInstance( int i ) { s_sel = i < 3 ? i : 1; }

L0Muon::RegisterFactory* L0Muon::RegisterFactory::instance() {

  if ( s_sel == 0 ) {
    if ( s_instance_0 == 0 ) { s_instance_0 = new L0Muon::RegisterFactory; }
    return s_instance_0;
  }

  if ( s_sel == 1 ) {
    if ( s_instance_1 == 0 ) { s_instance_1 = new L0Muon::RegisterFactory; }
    return s_instance_1;
  }

  if ( s_sel == 2 ) {
    if ( s_instance_2 == 0 ) { s_instance_2 = new L0Muon::RegisterFactory; }
    return s_instance_2;
  }
...
L0Muon::Register* L0Muon::RegisterFactory::createRegister( std::string name, int size ) {
...
L0Muon::Register* L0Muon::RegisterFactory::searchRegister( std::string name ) {
...
L0Muon::TileRegister* L0Muon::RegisterFactory::createTileRegister( std::string name, int size ) {
...
L0Muon::TileRegister* L0Muon::RegisterFactory::searchTileRegister( std::string name ) {
...
int L0Muon::RegisterFactory::createAlias( std::string name, std::string alias ) {
  std::map<std::string, L0Muon::Register*>::iterator ind;
  ind = m_registers.find( name );
  if ( ind != m_registers.end() ) {
  ...
  if ( full ) {
    ...
    for ( itreg = m_registers.begin(); itreg != m_registers.end(); itreg++ ) {
      std::cout << "RegisterFactory:  "
                << " " << itreg->first << " " << ( itreg->second )->name() << " " << ( itreg->second )->type() << " "
                << ( itreg->second )->size() << "\n";
    }
  }
}
...
void L0Muon::RegisterFactory::tileRegisterFromNode( DOMNode* pNode ) {
  ...
  DOMNode* child = pNode->getFirstChild();
  while ( child ) {
    if ( XMLString::equals( child->getNodeName(), TILE ) ) { tileFromNode( child, &ids, &tilestag, &stripstag ); }
    child = child->getNextSibling();
  }
...
void L0Muon::RegisterFactory::tileFromNode( DOMNode* pNode, std::vector<LHCb::MuonTileID>* mids,
                                            boost::dynamic_bitset<>* tilestag, boost::dynamic_bitset<>* stripstag ) {
