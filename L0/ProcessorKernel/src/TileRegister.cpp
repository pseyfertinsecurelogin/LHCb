...
L0Muon::TileRegister::TileRegister( int bits ) : L0Muon::Register( bits ) { m_duplicatedTiles = false; }

L0Muon::TileRegister::TileRegister( int bits, unsigned long pattern ) : L0Muon::Register( bits, pattern ) {
  m_duplicatedTiles = false;
}

L0Muon::TileRegister::TileRegister( int bits, unsigned long pattern, std::vector<LHCb::MuonTileID> ids )
...
void L0Muon::TileRegister::setTileVector( const std::vector<LHCb::MuonTileID>& ids ) { m_ids = ids; }
...
void L0Muon::TileRegister::setTilesTagVector( boost::dynamic_bitset<>& tilestag ) { m_tilestag = tilestag; }

void L0Muon::TileRegister::setStripsTagVector( boost::dynamic_bitset<>& stripstag ) { m_stripstag = stripstag; }
...
void L0Muon::TileRegister::makePads() {
  ...
  if ( !tmp.empty() ) {
    for ( i = 0; i < m_bitset.size(); i++ ) {
      ...
      if ( m_bitset.test( i ) ) {
        if ( m_tilestag[i] == 1 ) { m_pads.push_back( mid ); }
        if ( m_tilestag[i] == 0 ) {
              ...
              for ( j = i + 1; j < m_bitset.size(); j++ ) {
                ...
                if ( m_bitset.test( j ) ) {
                  ...
                  if ( m_stripstag[i] != m_stripstag[j] ) {
                    ...
                    if ( ijp.isValid() ) { m_pads.push_back( ijp ); }
                  }
                }
              }
            }
...
void L0Muon::TileRegister::print_bits( unsigned int event, FILE* file ) {
  ...
  for ( boost::dynamic_bitset<>::size_type i = 0; i < m_bitset.size(); i++ ) {
    ...
    if ( val ) {
      ...
      if ( mid.isValid() ) {
        fprintf( file, "%5u %3u ( %2d,%2d ) Q%1d M%1d R%1d %2d %2d  %d\n", event, (unsigned int)i, mid.layout().xGrid(),
                 mid.layout().yGrid(), mid.quarter() + 1, mid.station() + 1, mid.region() + 1, mid.nX(), mid.nY(),
                 val );
      } else {
...
void L0Muon::TileRegister::print_tiles( FILE* file, int ntiles_per_line ) {
  ...
  for ( std::vector<LHCb::MuonTileID>::iterator im = m_ids.begin(); im != m_ids.end(); im++ ) {
    ...
    if ( im->isValid() )
      fprintf( file, " M%1d(%2d,%2d)Q%1dR%1d %2d-%2d ;", im->station() + 1, im->layout().xGrid(), im->layout().yGrid(),
