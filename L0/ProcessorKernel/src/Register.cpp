...
L0Muon::Register::Register( int bits, unsigned long pattern ) : m_bitset( bits, pattern ) { m_set = false; }

L0Muon::Register::Register( int bits ) : m_bitset( bits ) {
  m_set = false;
  m_bitset.reset();
}
...
void L0Muon::Register::set( boost::dynamic_bitset<> bitpattern, unsigned int nbits, unsigned int shift ) {
  ...
  for ( boost::dynamic_bitset<>::size_type it = 0; it < nbits; it++ ) { m_bitset[it + shift] = bitpattern[it]; }
...
void L0Muon::Register::print_words( FILE* file, int nwords_per_line ) {
  ...
  for ( boost::dynamic_bitset<>::size_type it = m_bitset.size(); it > 0; it-- ) {
    ...
    if ( ib < 0 ) {
      ...
      word = 0;
      iw++;
      if ( ( iw % nwords_per_line ) == 0 ) { fprintf( file, "\n" ); }
    }
  }

  if ( ib != 15 ) { fprintf( file, "!!!\n" ); }
}
...
std::ostream& L0Muon::operator<<( std::ostream& ost, L0Muon::Register& r ) { return ( ost << r.getBitset() ); }
