...
//=============================================================================
L0DUAlg::L0DUAlg( const std::string& name, ISvcLocator* pSvcLocator )
    : L0AlgBase( name, pSvcLocator ), m_config( 0 ), m_confTool( 0 ), m_emulator( 0 ) {
...
StatusCode L0DUAlg::initialize() {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << " loading the configuration for TCK = " << m_tck << " /  " << itck << endmsg;
  ...
  for ( std::vector<std::string>::iterator iloc = m_dataLocations.begin(); m_dataLocations.end() != iloc; iloc++ ) {
    if ( *iloc == locHC ) {
      isHC = true;
      break;
    } // HC location already set in the dataLocations list by user (warning : data must really exist !!)
  }
  if ( m_addHC && !isHC ) {
    isHC = true;
    m_dataLocations.push_back( locHC );
  }
...
StatusCode L0DUAlg::execute() {
  ...
  if ( writeBanks() ) {
    if ( msgLevel( MSG::DEBUG ) ) debug() << "Insert RawBank in rawEvent" << endmsg;
