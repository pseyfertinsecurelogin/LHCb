...
public:
  ...
  L0DUConfigProvider( const std::string& type, const std::string& name, const IInterface* parent );
  ...
  LHCb::L0DUConfig* config( long tck = LHCb::L0DUTemplateConfig::TCKValue, std::string slot = "T0" ) override {
    ...
    if ( m_template && tck == LHCb::L0DUTemplateConfig::TCKValue ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "loading the TEMPLATE Configuration" << endmsg;
      return m_config;
    }

    if ( tck != m_tckopts )
      warning() << "The requested TCK " << tck << " is not consistent with the options definition " << m_tckopts
                << " CHECK your options ! " << endmsg;
    if ( tck < 0 || tck > 0xFFFF ) {
