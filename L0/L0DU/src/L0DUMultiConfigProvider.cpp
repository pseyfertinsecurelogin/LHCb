...
//=============================================================================
L0DUMultiConfigProvider::L0DUMultiConfigProvider( const std::string& type, const std::string& name,
...
StatusCode L0DUMultiConfigProvider::initialize() {
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Initialize L0DUMultiConfigProvider" << endmsg;
...
LHCb::L0DUConfig* L0DUMultiConfigProvider::loadConfig( const std::string& tck, std::string slot ) {
  ...
  std::stringstream s;
  s << "TCK_" << tck;
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Loading L0DUConfigProvider : " << s.str() << endmsg;
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "TCK = '" << tck << "' -> decimal value = " << itck << endmsg;

  LHCb::L0DUConfig* config = m_provider->config( itck, slot );

  if ( !config ) {
    error() << "Failed to load configuration for TCK = " << tck << " ... return empty LODUConfig" << endmsg;
    return config;
  }
