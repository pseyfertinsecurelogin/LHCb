...
// ============================================================================
L0ProcessorDataDecoder::L0ProcessorDataDecoder( const std::string& type, const std::string& name,
...
StatusCode L0ProcessorDataDecoder::initialize() {
  if ( msgLevel( MSG::DEBUG ) ) debug() << "Initialize L0ProcessorDataDecoder" << endmsg;
  ...
  for ( LHCb::L0ProcessorDatas* datas : datass ) {
    ...
    for ( auto& itt : *datas ) {
      if ( m_dataContainer.object( itt->key() ) != nullptr ) {
        Warning( "Fiber " + Gaudi::Utils::toString( itt->key() ) + " is used twice - Data not inserted",
                 StatusCode::SUCCESS )
            .ignore();
        continue;
      }
...
bool L0ProcessorDataDecoder::setL0ProcessorData( const std::vector<std::string>& dataLocs ) {
  ...
  for ( const auto& loc : dataLocs ) {
    if ( loc == LHCb::L0ProcessorDataLocation::HC || loc == LHCb::L0ProcessorDataLocation::L0HC ) {
      ...
      if ( !m_hasHC )
        Warning( "Expected Herschel L0ProcessorData container is not found or is empty", StatusCode::SUCCESS ).ignore();
      break;
    }
  }
  // collect the data
...
std::vector<int> L0ProcessorDataDecoder::bxList( const std::array<unsigned int, L0DUBase::Index::Size>& base ) {
  ...
  if ( !fiber ) {
    Warning( "Fiber " + Gaudi::Utils::toString( base[L0DUBase::Index::Fiber] ) + " not found ", StatusCode::SUCCESS )
        .ignore();
    m_ok = false;
    return def;
  }
...
unsigned long L0ProcessorDataDecoder::digit( const std::array<unsigned int, L0DUBase::Index::Size>& base, int bx ) {
  ...
  if ( !fiber->hasData( bx ) )
    Warning( "Fiber " + Gaudi::Utils::toString( base[L0DUBase::Index::Fiber] ) +
                 " has no defined data for BX=" + Gaudi::Utils::toString( bx ),
  ...
  if ( L0DUBase::Fiber::Empty != base[L0DUBase::Index::Fiber2] ) {
    ...
    if ( 0 == fiber2 ) {
      Warning( "Data ( " + Gaudi::Utils::toString( base ) + " ) not found in the container ", StatusCode::SUCCESS )
          .ignore();
      m_ok = false;
      return 0;
    }
