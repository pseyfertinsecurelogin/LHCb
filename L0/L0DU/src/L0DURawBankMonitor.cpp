...
DECLARE_COMPONENT( L0DURawBankMonitor )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
L0DURawBankMonitor::L0DURawBankMonitor( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode L0DURawBankMonitor::execute() {
  ...
  if ( !rawEvt ) {
    Warning( "rawEvent not found at locations " + Gaudi::Utils::toString( m_rawLocations ), StatusCode::SUCCESS )
        .ignore();
  } else {
  ...
  if ( !ok ) { return Error( "Readout error : unable to monitor the L0DU rawBank", StatusCode::SUCCESS ); }
  ...
  if ( m_data ) {
    ...
    // Muon
    fill( histo1D( toHistoID( "Data/Muon/1" ) ),
          m_fromRaw->data( "M0(Pt)" ) * Sgn[m_fromRaw->data( "M0(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/2" ) ),
          m_fromRaw->data( "M1(Pt)" ) * Sgn[m_fromRaw->data( "M1(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/3" ) ),
          m_fromRaw->data( "M2(Pt)" ) * Sgn[m_fromRaw->data( "M2(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/4" ) ),
          m_fromRaw->data( "M3(Pt)" ) * Sgn[m_fromRaw->data( "M3(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/5" ) ),
          m_fromRaw->data( "M4(Pt)" ) * Sgn[m_fromRaw->data( "M4(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/6" ) ),
          m_fromRaw->data( "M5(Pt)" ) * Sgn[m_fromRaw->data( "M5(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/7" ) ),
          m_fromRaw->data( "M6(Pt)" ) * Sgn[m_fromRaw->data( "M6(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Data/Muon/8" ) ),
          m_fromRaw->data( "M7(Pt)" ) * Sgn[m_fromRaw->data( "M7(Sgn)" )] * muonPt, 1 );
  }
  // data address
  if ( m_addr ) {
    // Calo
  ...
  // L0DU Processed Data
  if ( m_proc ) {
    fill( histo1D( toHistoID( "Process/Muon/1" ) ),
          m_fromRaw->data( "Muon1(Pt)" ) * Sgn[m_fromRaw->data( "Muon1(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Process/Muon/2" ) ),
          m_fromRaw->data( "Muon2(Pt)" ) * Sgn[m_fromRaw->data( "Muon2(Sgn)" )] * muonPt, 1 );
    fill( histo1D( toHistoID( "Process/Muon/3" ) ),
          m_fromRaw->data( "Muon3(Pt)" ) * Sgn[m_fromRaw->data( "Muon3(Sgn)" )] * muonPt, 1 );
  ...
  if ( m_status ) {
    ...
    if ( m_bcidDet && NULL != config ) {
      ...
      for ( LHCb::L0DUChannel::Map::const_iterator ic = channels.begin(); ic != channels.end(); ++ic ) {
        ...
        if ( report.channelDecisionByName( channel ) )
          plot1D( bx, "Status/BCID/Channels/" + channel, "BCID for positive decision from channel " + channel, 0.,
                  3565., 3565 );
      }
      ...
      for ( LHCb::L0DUTrigger::Map::const_iterator it = triggers.begin(); it != triggers.end(); ++it ) {
        ...
        if ( report.triggerDecisionByName( trigger ) )
          plot1D( bx, "Status/BCID/SubTriggers/" + trigger, "BCID for positive decision from subTrigger " + trigger, 0.,
                  3565., 3565 );
      }
    }
    ...
    if ( odBX != 3561 && odBX != 3562 && odBX != 3563 && odBX != 0 && mOk )
      fill( histo1D( toHistoID( "Status/L0DU/BCID/2" ) ), ( 0x7F & m_fromRaw->bcid().first ) - m_fromRaw->bcid().second,
            1 );
    ...
    if ( NULL == config ) {
      ...
      for ( std::vector<std::vector<std::string>>::iterator ilocs = m_locs.begin(); ilocs != m_locs.end(); ilocs++ ) {
        ...
        if ( m_emuTool->process( config, dataLocs ).isSuccess() ) {
          ...
          Warning( "Cannot run the emulator for L0ProcessorData location(s) : " + loc.str(), StatusCode::SUCCESS )
              .ignore();
        }
      }
    ...
    if ( 0 < inputStatus ) {
      ...
      if ( m_warn )
        Warning( " Status::Warning : L0DU bank monitor summary : -- input data error bit -- ", StatusCode::SUCCESS )
            .ignore();
    ...
    if ( mOk && ( 0x7F & m_fromRaw->bcid().first ) != m_fromRaw->bcid().second ) {
      if ( odBX == 3561 || odBX == 3562 || odBX == 3563 || odBX == 0 ) {
        ...
        if ( m_warn )
          Warning( "Status::Warning : L0DU bank monitor summary : -- PGA2/3 BXID misaligned -- ", StatusCode::SUCCESS )
              .ignore();
        if ( m_mask == L0DUBase::L0DUError::BxPGAShift ) setFilterPassed( true );
      }
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "BCID ODIN/L0DU&0x7F/PGA3 : " << odBX << "/" << ( 0x7F & m_fromRaw->bcid().first ) << " / "
                << m_fromRaw->bcid().second << endmsg;
    }
    if ( odBX != m_fromRaw->bcid().first ) {
      fill( histo1D( toHistoID( "Status/Summary/1" ) ), L0DUBase::L0DUError::BxOdinShift, 1 );
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- ODIN/L0DU BXID misaligned -- ", StatusCode::SUCCESS )
            .ignore();
    ...
    if ( ( m_fromRaw->status() & 0x1 ) ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning  : L0DU bank monitor summary : --IddleLink error bit -- ", StatusCode::SUCCESS )
            .ignore();
    }
    if ( ( m_fromRaw->status() & 0x2 ) ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- TLK error bit -- ", StatusCode::SUCCESS ).ignore();
    }
    if ( ( m_fromRaw->status() & 0x4 ) ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- DeMultiplexer error bit -- ", StatusCode::SUCCESS )
            .ignore();
    }
    if ( !check ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- Emulator check error -- ", StatusCode::SUCCESS )
            .ignore();
    }
    if ( NULL == config ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning  : L0DU bank monitor summary : -- unknown TCK -- ", StatusCode::SUCCESS ).ignore();
    }
    ...
    if ( m_fromRaw->data( "Spd(Mult)" ) != nSpdDAQ ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning  : L0DU bank monitor summary : -- Wrong Spd Mult -- ", StatusCode::SUCCESS ).ignore();
    }
    if ( !mOk ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- L0Muon banks missing -- ", StatusCode::SUCCESS )
            .ignore();
    }
    if ( !cOk ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- L0Calo banks missing -- ", StatusCode::SUCCESS )
            .ignore();
    }
    if ( !pOk ) {
      ...
      if ( m_warn )
        Warning( "Status::Warning : L0DU bank monitor summary : -- L0PU banks missing -- ", StatusCode::SUCCESS )
            .ignore();
    }
  }
...
bool L0DURawBankMonitor::emulatorCheck( LHCb::L0DUConfig* config, int unit, std::string txt ) {
  ...
  AIDA::IHistogram1D* h1 =
      plot1D( -1., "Status/L0DU/EmulatorCheck/Channels/" + Gaudi::Utils::toString( unit ),
              "L0DU channels preDecision emulator check (" + txt + ")", -1., (double)cBin, cBin + 1 );
  AIDA::IHistogram1D* h2 =
      plot1D( -1., "Status/L0DU/EmulatorCheck/Conditions/" + Gaudi::Utils::toString( unit ),
  ...
  for ( LHCb::L0DUChannel::Map::iterator it = channels.begin(); it != channels.end(); it++ ) {
    ...
    if ( report.channelPreDecision( id ) != ( ( *it ).second )->emulate()->emulatedPreDecision() ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Emulator check error for channel " << ( *it ).first << endmsg;
  ...
  for ( LHCb::L0DUElementaryCondition::Map::iterator it = conditions.begin(); it != conditions.end(); it++ ) {
    ...
    if ( !hasData )
      Warning( "Cannot monitor condition '" + it->first + "' - data is not set for BX=" + Gaudi::Utils::toString( bx ),
               StatusCode::SUCCESS, 10 )
          .ignore();
    if ( !reported )
      Warning( "Cannot monitor condition '" + it->first + "' - decision bit not reported=", StatusCode::SUCCESS, 10 )
          .ignore();
    ...
    if ( ( report.conditionValueByName( it->first ) != it->second->emulatedValue() ) && hasData && reported ) {
      if ( msgLevel( MSG::DEBUG ) ) debug() << "Emulator check error for condition " << ( *it ).first << endmsg;
...
GaudiAlg::HistoID L0DURawBankMonitor::toHistoID( std::string unit ) {
  ...
  if ( unit.find( "Status/Calo/1" ) != std::string::npos )
    return HistoID( pref + "/Calo/StatusBit" );
  else if ( unit.find( "Calo/1" ) != std::string::npos )
    return HistoID( pref + "/Calo/Electron" );
  else if ( unit.find( "Calo/2" ) != std::string::npos )
    return HistoID( pref + "/Calo/Photon" );
  else if ( unit.find( "Calo/3" ) != std::string::npos )
    return HistoID( pref + "/Calo/LocalPi0" );
  else if ( unit.find( "Calo/4" ) != std::string::npos )
    return HistoID( pref + "/Calo/GlobalPi0" );
  else if ( unit.find( "Calo/5" ) != std::string::npos )
    return HistoID( pref + "/Calo/Hadron" );
  else if ( unit.find( "Calo/6" ) != std::string::npos )
    return HistoID( pref + "/Calo/SpdMult" );
  else if ( unit.find( "Calo/7" ) != std::string::npos )
    return HistoID( pref + "/Calo/SumEt" );
  else if ( unit.find( "Process/Muon/1" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon1" );
  else if ( unit.find( "Process/Muon/2" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon2" );
  else if ( unit.find( "Process/Muon/3" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon3" );
  else if ( unit.find( "Process/Muon/4" ) != std::string::npos )
    return HistoID( pref + "/Muon/DiMuon" );
  else if ( unit.find( "Process/Muon/5" ) != std::string::npos )
    return HistoID( pref + "/Muon/MuonCleaning" );
  else if ( unit.find( "Data/Muon/1" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon00" );
  else if ( unit.find( "Data/Muon/2" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon01" );
  else if ( unit.find( "Data/Muon/3" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon10" );
  else if ( unit.find( "Data/Muon/4" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon11" );
  else if ( unit.find( "Data/Muon/5" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon20" );
  else if ( unit.find( "Data/Muon/6" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon21" );
  else if ( unit.find( "Data/Muon/7" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon30" );
  else if ( unit.find( "Data/Muon/8" ) != std::string::npos )
    return HistoID( pref + "/Muon/Muon31" );
  else if ( unit.find( "Status/Muon/1" ) != std::string::npos )
    return HistoID( pref + "/Muon/StatusBit0" );
  else if ( unit.find( "Status/Muon/2" ) != std::string::npos )
    return HistoID( pref + "/Muon/StatusBit1" );
  else if ( unit.find( "Status/Muon/3" ) != std::string::npos )
    return HistoID( pref + "/Muon/StatusBit2" );
  else if ( unit.find( "Status/Muon/4" ) != std::string::npos )
    return HistoID( pref + "/Muon/StatusBit3" );
  else if ( unit.find( "Status/Pus/1" ) != std::string::npos )
    return HistoID( pref + "/Pus/StatusBit" );
  else if ( unit.find( "Pus/1" ) != std::string::npos )
    return HistoID( pref + "/Pus/Peak1" );
  else if ( unit.find( "Pus/2" ) != std::string::npos )
    return HistoID( pref + "/Pus/Peak2" );
  else if ( unit.find( "Pus/3" ) != std::string::npos )
    return HistoID( pref + "/Pus/Hits" );
  return HistoID( unit );
}
