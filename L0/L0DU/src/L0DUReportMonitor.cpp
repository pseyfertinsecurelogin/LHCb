...
//=============================================================================
L0DUReportMonitor::L0DUReportMonitor( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode L0DUReportMonitor::execute() {
  ...
  if ( NULL == report ) { return Error( "L0DUReport not found at location " + loc, StatusCode::SUCCESS ); }
  ...
  if ( !report->valid() ) { return Error( "Invalid report -> cannot monitor", StatusCode::SUCCESS ); }
  ...
  for ( LHCb::L0DUChannel::Map::iterator it = channels.begin(); it != channels.end(); it++ ) {
    ...
    chanNames[( *it ).second->id()] =
        LHCb::L0DUDecision::Name[( ( *it ).second )->decisionType()] + "|" + ( *it ).first;
  }
  // Condition counter
  for ( LHCb::L0DUElementaryCondition::Map::iterator it = conditions.begin(); it != conditions.end(); it++ ) {
    ...
    if ( !it->second->data()->hasData( bx ) ) {
      ...
      m_condError[_name] += 1;
      Warning( "L0DUElementaryData '" + it->second->data()->name() +
                   "' has no value set for BX=" + Gaudi::Utils::toString( bx ),
  ...
  if ( m_cond ) {
    ...
    label( plot1D( -1., base.str() + "/L0Conditions/Counters/1", ecName.str() + " - BX=T0", -1., (double)ecBin,
                   ecBin + 1 ),
           condNames );
    label( plot1D( -1., base.str() + "/L0Conditions/Counters/Prev2/1", ecName.str() + " - BX=Prev2", -1., (double)ecBin,
                   ecBin + 1 ),
           condNames );
    label( plot1D( -1., base.str() + "/L0Conditions/Counters/Prev1/1", ecName.str() + " - BX=Prev1", -1., (double)ecBin,
                   ecBin + 1 ),
           condNames );
    label( plot1D( -1., base.str() + "/L0Conditions/Counters/Next1/1", ecName.str() + " - BX=Next1", -1., (double)ecBin,
                   ecBin + 1 ),
           condNames );
    label( plot1D( -1., base.str() + "/L0Conditions/Counters/Next2/1", ecName.str() + " - BX=Next2", -1., (double)ecBin,
                   ecBin + 1 ),
           condNames );
    if ( ecBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Conditions/Correlations/1",
                     "L0DU Elementary Conditions 2D INclusive counters (TCK = " + ttck.str() + ")", -1., (double)ecBin,
                     -1., (double)ecBin, ecBin + 1, ecBin + 1 ),
             condNames );
    if ( ecBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Conditions/Correlations/2",
                     "L0DU Elementary Conditions 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)ecBin,
                     -1., (double)ecBin, ecBin + 1, ecBin + 1 ),
             condNames );
    ...
    for ( LHCb::L0DUElementaryCondition::Map::iterator it = conditions.begin(); it != conditions.end(); it++ ) {
      ...
      if ( report->conditionValue( id ) ) {
        plot1D( (double)id, base.str() + "/L0Conditions/Counters/1", ecName.str() + " - BX=T0", -1., (double)ecBin,
                ecBin + 1 );
        m_condSeq[name] |= ( 1 << 2 );
      }
      if ( report->conditionValue( id, -2 ) ) {
        plot1D( (double)id, base.str() + "/L0Conditions/Counters/Prev2/1", ecName.str() + " - BX=Prev2", -1.,
                (double)ecBin, ecBin + 1 );
        m_condSeq[name] |= ( 1 << 0 );
      }
      if ( report->conditionValue( id, -1 ) ) {
        plot1D( (double)id, base.str() + "/L0Conditions/Counters/Prev1/1", ecName.str() + " - BX=Prev1", -1.,
                (double)ecBin, ecBin + 1 );
        m_condSeq[name] |= ( 1 << 1 );
      }
      if ( report->conditionValue( id, +1 ) ) {
        plot1D( (double)id, base.str() + "/L0Conditions/Counters/Next1/1", ecName.str() + " - BX=Next1", -1.,
                (double)ecBin, ecBin + 1 );
        m_condSeq[name] |= ( 1 << 3 );
      }
      if ( report->conditionValue( id, +2 ) ) {
        plot1D( (double)id, base.str() + "/L0Conditions/Counters/Next2/1", ecName.str() + " - BX=Next2", -1.,
                (double)ecBin, ecBin + 1 );
        m_condSeq[name] |= ( 1 << 4 );
      }
      ...
      AIDA::IHistogram1D* histo  = plot1D( m_condSeq[name], base.str() + "/L0Conditions/Sequence/" + suffix,
                                          "Decision sequence over 5 BX for -" + name + "- Elementary Condition (TCK " +
                                              ttck.str() + ")  - LSB = Prev2",
                                          0., 32., 32 );
      ...
      for ( int slot = -2; slot <= 2; slot++ ) {
        ...
        AIDA::IHistogram2D* histo = plot2D(
            (double)slot, (double)id, base.str() + "/L0Conditions/Timing/1",
      ...
      if ( report->conditionValue( id ) ) {
        for ( LHCb::L0DUElementaryCondition::Map::iterator jt = conditions.begin(); jt != conditions.end(); jt++ ) {
          ...
          if ( report->conditionValue( jd ) ) {
            ...
            if ( isX )
              plot2D( (double)id, (double)jd, base.str() + "/L0Conditions/Correlations/2",
                      "L0DU Elementary Conditions 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)ecBin,
                      -1., (double)ecBin, ecBin + 1, ecBin + 1 );
          }
        }
  ...
  if ( m_chan ) {
    ...
    label( plot1D( -1., base.str() + "/L0Channels/Counters/1", tcName.str() + " - BX=T0", -1., (double)cBin, cBin + 1 ),
           chanNames );
    label( plot1D( -1., base.str() + "/L0Channels/Counters/2", fcName.str() + " - BX=T0", -1., (double)cBin, cBin + 1 ),
           chanNames );
    label( plot1D( -1., base.str() + "/L0Channels/Counters/Prev2/1", tcName.str() + " - BX=Prev2", -1., (double)cBin,
                   cBin + 1 ),
           chanNames );
    label( plot1D( -1., base.str() + "/L0Channels/Counters/Prev1/1", tcName.str() + " - BX=Prev1", -1., (double)cBin,
                   cBin + 1 ),
           chanNames );
    label( plot1D( -1., base.str() + "/L0Channels/Counters/Next1/1", tcName.str() + " - BX=Next1", -1., (double)cBin,
                   cBin + 1 ),
           chanNames );
    label( plot1D( -1., base.str() + "/L0Channels/Counters/Next2/1", tcName.str() + " - BX=Next2", -1., (double)cBin,
                   cBin + 1 ),
           chanNames );
    if ( cBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Channels/Correlations/1",
                     "L0DU Channels Decision 2D INclusive counters (TCK = " + ttck.str() + ")", -1., (double)cBin, -1.,
                     (double)cBin, cBin + 1, cBin + 1 ),
             chanNames );
    if ( cBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Channels/Correlations/2",
                     "L0DU Channels Decision 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)cBin, -1.,
                     (double)cBin, cBin + 1, cBin + 1 ),
             chanNames );
    ...
    for ( LHCb::L0DUChannel::Map::iterator it = channels.begin(); it != channels.end(); it++ ) {
      ...
      if ( report->channelDecision( id ) ) {
        plot1D( (double)id, base.str() + "/L0Channels/Counters/1", tcName.str() + " - BX=T0", -1., (double)cBin,
                cBin + 1 );
        m_chanSeq[name] |= ( 1 << 2 );
      }

      if ( report->channelDecision( id, -2 ) ) {
        plot1D( (double)id, base.str() + "/L0Channels/Counters/Prev2/1", tcName.str() + " - BX=Prev2", -1.,
                (double)cBin, cBin + 1 );
        m_chanSeq[name] |= ( 1 << 0 );
      }

      if ( report->channelDecision( id, -1 ) ) {
        plot1D( (double)id, base.str() + "/L0Channels/Counters/Prev1/1", tcName.str() + " - BX=Prev1", -1.,
                (double)cBin, cBin + 1 );
        m_chanSeq[name] |= ( 1 << 1 );
      }
      if ( report->channelDecision( id, +1 ) ) {
        plot1D( (double)id, base.str() + "/L0Channels/Counters/Next1/1", tcName.str() + " - BX=Next1", -1.,
                (double)cBin, cBin + 1 );
        m_chanSeq[name] |= ( 1 << 3 );
      }

      if ( report->channelDecision( id, +2 ) ) {
        plot1D( (double)id, base.str() + "/L0Channels/Counters/Next2/1", tcName.str() + " - BX=Next2", -1.,
                (double)cBin, cBin + 1 );
        m_chanSeq[name] |= ( 1 << 4 );
      }
      if ( report->channelPreDecision( id ) )
        plot1D( (double)id, base.str() + "/L0Channels/Counters/2", fcName.str() + " - BX=T0", -1., (double)cBin,
                cBin + 1 );
      ...
      AIDA::IHistogram1D* histo =
          plot1D( m_chanSeq[name], base.str() + "/L0Channels/Sequence/" + suffix,
                  "Decision sequence over 5 BX for -" + name + "- channel (TCK " + ttck.str() + ") - LSB = Prev2", 0.,
                  32., 32 );
      label( histo, seqNames, false, "v" );
      ...
      for ( int slot = -2; slot <= 2; slot++ ) {
        ...
        AIDA::IHistogram2D* histo = plot2D(
            (double)slot, (double)id, base.str() + "/L0Channels/Timing/1",
      ...
      if ( report->channelDecision( id ) ) {
        for ( LHCb::L0DUChannel::Map::iterator jt = channels.begin(); jt != channels.end(); jt++ ) {
          ...
          if ( report->channelDecision( jd ) ) {
            ...
            if ( isX )
              plot2D( (double)id, (double)jd, base.str() + "/L0Channels/Correlations/2",
                      "L0DU Channels Decision 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)cBin, -1.,
                      (double)cBin, cBin + 1, cBin + 1 );
          }
        }
  ...
  if ( m_trig && triggers.size() != 0 ) {
    ...
    label( plot1D( -1., base.str() + "/L0Triggers/Counters/1", tName.str() + " - BX=T0", -1., (double)tBin, tBin + 1 ),
           trigNames );
    label( plot1D( -1., base.str() + "/L0Triggers/Counters/Prev2/1", tName.str() + " - BX=Prev2", -1., (double)tBin,
                   tBin + 1 ),
           trigNames );
    label( plot1D( -1., base.str() + "/L0Triggers/Counters/Prev1/1", tName.str() + " - BX=Prev1", -1., (double)tBin,
                   tBin + 1 ),
           trigNames );
    label( plot1D( -1., base.str() + "/L0Triggers/Counters/Next1/1", tName.str() + " - BX=Next1", -1., (double)tBin,
                   tBin + 1 ),
           trigNames );
    label( plot1D( -1., base.str() + "/L0Triggers/Counters/Next2/1", tName.str() + " - BX=Next2", -1., (double)tBin,
                   tBin + 1 ),
           trigNames );
    if ( tBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Triggers/Correlations/1",
                     "L0DU Triggers Decision 2D INclusive counters (TCK = " + ttck.str() + ")", -1., (double)tBin, -1.,
                     (double)tBin, tBin + 1, tBin + 1 ),
             trigNames );
    if ( tBin > 1 )
      label( plot2D( -1., -1., base.str() + "/L0Triggers/Correlations/2",
                     "L0DU Triggers Decision 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)tBin, -1.,
                     (double)tBin, tBin + 1, tBin + 1 ),
             trigNames );
    ...
    for ( LHCb::L0DUTrigger::Map::iterator it = triggers.begin(); it != triggers.end(); it++ ) {
      ...
      if ( report->triggerDecisionByName( name ) ) {
        plot1D( (double)id, base.str() + "/L0Triggers/Counters/1", tName.str() + " - BX=T0", -1., (double)tBin,
                tBin + 1 );
        m_trigSeq[name] |= ( 1 << 2 );
      }

      if ( report->triggerDecisionByName( name, -2 ) ) {
        plot1D( (double)id, base.str() + "/L0Triggers/Counters/Prev2/1", tName.str() + " - BX=Prev2", -1., (double)tBin,
                tBin + 1 );
        m_trigSeq[name] |= ( 1 << 0 );
      }

      if ( report->triggerDecisionByName( name, -1 ) ) {
        plot1D( (double)id, base.str() + "/L0Triggers/Counters/Prev1/1", tName.str() + " - BX=Prev1", -1., (double)tBin,
                tBin + 1 );
        m_trigSeq[name] |= ( 1 << 1 );
      }
      if ( report->triggerDecisionByName( name, +1 ) ) {
        plot1D( (double)id, base.str() + "/L0Triggers/Counters/Next1/1", tName.str() + " - BX=Next1", -1., (double)tBin,
                tBin + 1 );
        m_trigSeq[name] |= ( 1 << 3 );
      }

      if ( report->triggerDecisionByName( name, +2 ) ) {
        plot1D( (double)id, base.str() + "/L0Triggers/Counters/Next2/1", tName.str() + " - BX=Next2", -1., (double)tBin,
                tBin + 1 );
        m_trigSeq[name] |= ( 1 << 4 );
      }
      ...
      std::string         suffix = m_generic ? sid.str() : name;
      AIDA::IHistogram1D* histo =
          plot1D( m_trigSeq[name], base.str() + "/L0Triggers/Sequence/" + suffix,
                  "Decision sequence over 5 BX for -" + name + "- trigger (TCK " + ttck.str() + ") - LSB = Prev2", 0.,
                  32., 32 );
      label( histo, seqNames, false, "v" );
      ...
      for ( int slot = -2; slot <= 2; slot++ ) {
        ...
        AIDA::IHistogram2D* histo = plot2D(
            (double)slot, (double)id, base.str() + "/L0Triggers/Timing/1",
      ...
      if ( report->triggerDecisionByName( name ) ) {
        for ( LHCb::L0DUTrigger::Map::iterator jt = triggers.begin(); jt != triggers.end(); jt++ ) {
          ...
          if ( report->triggerDecision( jd ) ) {
            ...
            if ( isX )
              plot2D( (double)id, (double)jd, base.str() + "/L0Triggers/Correlations/2",
                      "L0DU Triggers Decision 2D EXclusive counters (TCK = " + ttck.str() + ")", -1., (double)tBin, -1.,
                      (double)tBin, tBin + 1, tBin + 1 );
          }
        }
  ...
  if ( m_dec ) {
    ...
    // Correlations
    AIDA::IHistogram2D* h2d  = plot2D( 0, 0, base.str() + "/L0Decision/Correlations/1",
                                      "Decisions INclusive correlations", 0., 4., 0., 4., 4, 4 );
    AIDA::IHistogram2D* h2d2 = plot2D( 0, 0, base.str() + "/L0Decision/Correlations/2",
                                       "Decisions EXclusive correlations", 0., 4., 0., 4., 4, 4 );
    ...
    while ( idec < LHCb::L0DUDecision::Any ) {
      while ( jdec < LHCb::L0DUDecision::Any ) {
        ...
        if ( report->decision( idec ) == report->decision( jdec ) ) {
          plot2D( ic, jc, base.str() + "/L0Decision/Correlations/1", "Decisions INclusive correlations", 0., 4., 0., 3.,
                  4, 3 );
          if ( !report->decision( kdec ) )
            plot2D( ic, jc, base.str() + "/L0Decision/Correlations/2", "Decisions EXclusive correlations", 0., 4., 0.,
                    3., 4, 3 );
        }
    ...
    for ( int slot = -2; slot <= 2; slot++ ) {
      ...
      while ( idec < LHCb::L0DUDecision::Any ) {
        ...
        AIDA::IHistogram2D* histo =
            plot2D( (double)slot, ic, base.str() + "/L0Decision/Timing/1",
                    "L0 decisions over +-2 BX around trigger : decisions(BX) - (TCK " + ttck.str() + ")", -2.5, 2.5, 0.,
                    4., 5, 4, dec );
...
StatusCode L0DUReportMonitor::finalize() {
  ...
  for ( std::map<unsigned int, LHCb::L0DUConfig*>::iterator it = m_configs.begin(); it != m_configs.end(); it++ ) {
    ...
    eRate[LHCb::L0DUDecision::Any] =
        ( m_evtCnt > 0 ) ? 100. * sqrt( m_decCnt[LHCb::L0DUDecision::Any] ) / m_evtCnt : 0.;
      ...
      while ( idec < LHCb::L0DUDecision::Any ) {
        ...
        if ( count > 0 ) {
          ...
          for ( CounterMap::iterator it = m_trigRate.begin(); m_trigRate.end() != it; it++ ) {
            ...
            info() << "        --- SubTrigger : "
                   << format( "%25s :  %8.0f events : rate = %6.2f  %%  (rel. rate =  %6.2f %% ) ", tag.c_str(),
                              m_trigCnt[name], m_trigRate[name], m_trigRelRate[idec][name] )
                   << endmsg;
          }
        }
        idec = idec << 1;
    ...
    while ( idec < LHCb::L0DUDecision::Any ) {
      ...
      if ( count > 0 ) {
        info() << "    -- " << count << " L0DU channel(s) associated to decision type  = '"
               << LHCb::L0DUDecision::Name[idec]
               << "'  [OR'ed rate = " << format( " (%6.2f +- %6.2f) %% ", rate[idec], eRate[idec] ) << "] : " << endmsg;
        for ( CounterMap::iterator ic = m_chanRate.begin(); m_chanRate.end() != ic; ic++ ) {
          ...
          info() << "        ---  L0 Channel : "
                 << format( "%25s :  %8.0f events : rate = %6.2f  %%  (rel. rate =  %6.2f %% ) ", tag.c_str(),
                            m_chanCnt[name], m_chanRate[name], m_chanRelRate[idec][name] )
                 << endmsg;
        }
      }
      idec = idec << 1;
    ...
    if ( count != 0 ) {
      ...
      for ( CounterMap::iterator ic = m_chanRate.begin(); m_chanRate.end() != ic; ic++ ) {
        ...
        std::string type = LHCb::L0DUDecision::Name[channel->decisionType()];
        info() << "        --- L0 channel : "
               << format( "%25s :  %8.0f events : rate = %6.2f  %%   ", ( type + "|" + name ).c_str(), m_chanCnt[name],
                          m_chanRate[name] )
               << endmsg;
      }
    }
    ...
    for ( CounterMap::iterator ic = m_condRate.begin(); m_condRate.end() != ic; ic++ ) {
      ...
      if ( cnt < 0 )
        info() << "   *  Elementary Condition  " << stamp
               << format( "%20s :   UNKNOWN (condition unreported)     ", _name.c_str(), m_condCnt[name],
                          m_condRate[name] )
               << endmsg;
      else
        info() << "   *  Elementary Condition  " << stamp
               << format( "%20s :  %8.0f events : rate = %6.2f  %%   ", _name.c_str(), m_condCnt[name],
                          m_condRate[name] )
               << endmsg;
    }
    info() << "======================================================================== " << endmsg;
...
//=============================================================================
void L0DUReportMonitor::label( AIDA::IHistogram1D* h1d, std::map<int, std::string> labels, bool count,
                               std::string opt ) {
  if ( !m_swap ) return;
  ...
  if ( opt != "" )
    xAxis->LabelsOption( opt.c_str() );
  else
    xAxis->LabelsOption( m_lab.c_str() );
}
void L0DUReportMonitor::label( AIDA::IHistogram2D* h2d, std::map<int, std::string> labels, int mask, bool count,
                               std::string opt ) {
  if ( !m_swap ) return;
