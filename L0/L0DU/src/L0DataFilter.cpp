...
//=============================================================================
L0DataFilter::L0DataFilter( const std::string& name, ISvcLocator* pSvcLocator ) : L0AlgBase( name, pSvcLocator ) {
...
StatusCode L0DataFilter::initialize() {
  ...
  for ( const auto& i : m_selection ) {
    if ( ( i.second ).size() != 2 )
      return Error( "Selection syntax is : 'DataName' : {'comparator','threshold'}", StatusCode::FAILURE );
    ...
    info() << "  - (" << name << " " << comp << " " << st << ") ";
    if ( k < m_selection.size() )
      info() << m_logical << endmsg;
    else
      info() << endmsg;
    k++;
  }
...
StatusCode L0DataFilter::execute() {
  ...
  bool accept = true;
  if ( m_logical == "AND" )
    accept = ( m_revert ? !aSel : aSel );
  else if ( m_logical == "OR" )
    accept = ( m_revert ? !oSel : oSel );
