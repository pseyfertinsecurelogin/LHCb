...
//=============================================================================
L0Filter::L0Filter( const std::string& name, ISvcLocator* pSvcLocator )
    : L0AlgBase( name, pSvcLocator ), m_count( 0 ), m_sel( 0 ) {
  m_l0channels.clear();
...
StatusCode L0Filter::initialize() {
  ...
  if ( "Timing" == m_trig ) {
    ...
    if ( !m_l0triggers.empty() ) {
      info() << " - any of : [";
      for ( std::vector<std::string>::const_iterator c = m_l0triggers.begin(); c != m_l0triggers.end(); ++c ) {
        info() << *c << " ";
      }
...
StatusCode L0Filter::execute() {
  ...
  if ( !m_l0channels.empty() ) {
    ...
    for ( std::map<std::string, std::vector<std::string>>::const_iterator cc = m_l0channels.begin();
      ...
      if ( toUpper( sT ).find( "OTHER" ) != std::string::npos && isAnother( sTck.str() ) )
        pass = true; // .. on each other TCK
      if ( !pass ) continue;
  ...
  if ( !m_l0triggers.empty() ) {
    for ( std::vector<std::string>::const_iterator c = m_l0triggers.begin(); c != m_l0triggers.end(); ++c ) {
      if ( l0->triggerDecisionByName( *c ) ) {
        setFilterPassed( accept );
        if ( msgLevel( MSG::VERBOSE ) ) verbose() << "Event is accepted by " << *c << " SubTrigger" << endmsg;
      } else if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Event is not accepted by " << *c << " SubTrigger" << endmsg;
    }
  }
  ...
  if ( m_l0triggers.empty() && m_l0channels.empty() ) {
    if ( l0->decision( m_mask ) ) {
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Event is accepted by L0 decision " << LHCb::L0DUDecision::Name[m_mask] << endmsg;
      setFilterPassed( accept );
    } else {
      setFilterPassed( !accept );
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "Event is rejected by L0 decision " << LHCb::L0DUDecision::Name[m_mask] << endmsg;
    }
  }
