...
DECLARE_COMPONENT( L0DUFromRawAlg )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
L0DUFromRawAlg::L0DUFromRawAlg( const std::string& name, ISvcLocator* pSvcLocator )
...
StatusCode L0DUFromRawAlg::initialize() {
  ...
  if ( m_hlt1 && m_fromRawTool == "L0DUFromRawTool" ) { m_fromRawTool = "L0DUFromRawHlt1Tool"; }
  ...
  if ( !m_hlt1 ) {
    ...
    if ( m_fromRaw->_setProperty( "RawEventLocations", Gaudi::Utils::toString( rawEventLocations() ) ).isFailure() )
      ...
      debug() << "Propagate rawEventLocation to decoder tool" << Gaudi::Utils::toString( rawEventLocations() )
              << endmsg;
...
StatusCode L0DUFromRawAlg::execute() {
  ...
  if ( m_ensureKnownTCK && !m_fromRaw->report().configuration() ) {
    return Error( "TCK not recognized. Run with L0Conf().EnsureKnownTCK=False to ignore this error",
                  StatusCode::FAILURE );
  }
  ...
  if ( writeProcData() ) {
    ...
    for ( const auto& it : *m_fromRaw->L0ProcessorDatas() ) { datas->insert( new LHCb::L0ProcessorData( *it ) ); }
  }
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    debug() << "Bank size : " << m_fromRaw->size() << " (bytes) - Bank version : " << m_fromRaw->report().bankVersion()
            << endmsg;
    ...
    while ( mask != 0x0 ) {
      ...
      debug() << "    o decision bit from raw                          : " << m_fromRaw->report().decision( typ )
              << endmsg;
      debug() << "    o decision re-built from summary report          : "
              << m_fromRaw->report().decisionFromSummary( typ ) << endmsg;
      if ( m_fromRaw->version() != 0 && m_fromRaw->report().configuration() != NULL &&
    ...
    if ( m_fromRaw->report().configuration() != NULL && msgLevel( MSG::VERBOSE ) ) {
      ...
      for ( LHCb::L0DUElementaryCondition::Map::iterator it = conds.begin(); conds.end() != it; it++ ) {
        std::string name = ( ( *it ).second )->name();
        verbose() << "    - Condition Value " << name << " : " << m_fromRaw->report().conditionValueByName( name )
                  << endmsg;
      }
    }
  }
...
void L0DUFromRawAlg::compareReports() {
  ...
  if ( rep.tck() != rep2.tck() ) { info() << "!= tck : hlt " << rep.tck() << " pro " << rep2.tck() << endmsg; }
  ...
  if ( rep.channelsPreDecisionSummaries() != rep2.channelsPreDecisionSummaries() ) {
    info() << "!= channelsPreDecisionSummaries : hlt " << rep.channelsPreDecisionSummaries() << " pro "
           << rep2.channelsPreDecisionSummaries() << endmsg;
  }
  if ( rep.channelsDecisionSummaries() != rep2.channelsDecisionSummaries() ) {
    info() << "!= channelsDecisionSummaries : hlt " << rep.channelsDecisionSummaries() << " pro "
           << rep2.channelsDecisionSummaries() << endmsg;
  }
  if ( rep.conditionsValueSummaries() != rep2.conditionsValueSummaries() ) {
    info() << "!= conditionsValueSummaries : hlt " << rep.conditionsValueSummaries() << " pro "
           << rep2.conditionsValueSummaries() << endmsg;
  }
  ...
  if ( rep.dataMap() != rep2.dataMap() ) {
    for ( std::map<std::string, std::pair<int, double>>::const_iterator imap = rep.dataMap().begin();
      ...
      if ( int( rep2.dataDigit( imap->first ) ) != imap->second.first ) {
        info() << "!= dataMap : For name " << imap->first << " hlt: " << imap->second.first << " pro "
               << rep2.dataDigit( imap->first ) << endmsg;
      }
    }
    for ( std::map<std::string, std::pair<int, double>>::const_iterator imap = rep2.dataMap().begin();
      ...
      if ( int( rep.dataDigit( imap->first ) ) != imap->second.first ) {
        info() << "!= dataMap : For name " << imap->first << " pro: " << imap->second.first << " hlt "
               << rep.dataDigit( imap->first ) << endmsg;
      }
    }
  }
  if ( rep.bcid() != rep2.bcid() ) { info() << "!= bcid : hlt " << rep.bcid() << " pro " << rep2.bcid() << endmsg; }
