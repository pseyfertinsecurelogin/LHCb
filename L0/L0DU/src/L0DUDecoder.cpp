...
namespace {
  ...
  static const std::array<std::string, 8> s_Muon_Add = {"M0(Add)", "M1(Add)", "M2(Add)", "M3(Add)",
                                                        "M4(Add)", "M5(Add)", "M6(Add)", "M7(Add)"};

  static const std::array<std::string, 8> s_Muon_Pt = {"M0(Pt)", "M1(Pt)", "M2(Pt)", "M3(Pt)",
                                                       "M4(Pt)", "M5(Pt)", "M6(Pt)", "M7(Pt)"};

  static const std::array<std::string, 8> s_Muon_Sgn = {"M0(Sgn)", "M1(Sgn)", "M2(Sgn)", "M3(Sgn)",
  ...
  void encode_( std::optional<std::map<std::string, std::pair<unsigned int, double>>>& map, LHCb::L0DUReport& report,
...
//=============================================================================
L0DUDecoder::L0DUDecoder( const std::string& name, ISvcLocator* pSvcLocator )
    : Transformer(
          name, pSvcLocator,
...
LHCb::L0DUReport L0DUDecoder::operator()( const LHCb::RawEvent& rawEvent ) const {
  ...
  auto encode = [&]( auto&&... args ) { encode_( dataMap, report, std::forward<decltype( args )>( args )... ); };
  ...
  if ( l0Banks.size() != 1 ) {
    ...
    throw GaudiException( l0Banks.empty() ? "Missing raw bank" : "More than one raw bank", name(),
                          StatusCode::SUCCESS );
  }
  ...
  if ( 2 != vsn ) {
    throw GaudiException( "== Unsupported version " + std::to_string( vsn ) + " L0DU bank", name(),
                          StatusCode::FAILURE );
  }
  ...
  if ( m_ensureKnownTCK.value() && !tck ) {
    throw GaudiException( "TCK not recognized. Run with L0Conf().EnsureKnownTCK=False to ignore this error", name(),
                          StatusCode::FAILURE );
  }
  report.setTck( tck );
  ...
  if ( size != allSize ) {
    info() << "READOUTSTATUS : the total expected size " << allSize << " does NOT match the bank size " << size
           << " <** POSSIBLE DATA CORRUPTION **>" << endmsg;
  ...
  for ( unsigned int i = 0; i < itc; i++ ) { report.setChannelsPreDecisionSummary( *( ++data ), i, 0 ); }
  for ( unsigned int i = 0; i < itc; i++ ) { report.setChannelsDecisionSummary( *( ++data ), i, 0 ); }
  for ( unsigned int i = 0; i < iec; i++ ) { report.setConditionsValueSummary( *( ++data ), i, 0 ); }
  ...
  for ( int im = nm; im > 0; im-- ) {
    for ( unsigned int i = 0; i < itc; i++ ) { report.setChannelsDecisionSummary( *( ++data ), i, -im ); }
    for ( unsigned int i = 0; i < iec; i++ ) { report.setConditionsValueSummary( *( ++data ), i, -im ); }
  }
  ...
  for ( int ip = 0; ip < np; ip++ ) {
    for ( unsigned int i = 0; i < itc; i++ ) { report.setChannelsDecisionSummary( *( ++data ), i, ip + 1 ); }
    for ( unsigned int i = 0; i < iec; i++ ) { report.setConditionsValueSummary( *( ++data ), i, ip + 1 ); }
  }
