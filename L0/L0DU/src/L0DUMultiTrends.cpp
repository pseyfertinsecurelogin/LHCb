...
//=============================================================================
L0DUMultiTrends::L0DUMultiTrends( const std::string& name, ISvcLocator* pSvcLocator )
  ...
  declareProperty( "NumberOfPredefinedTriggers",
                   m_nPredTriggers = 4 );                   // Must be set according to L0DUConfigProvider list
...
StatusCode L0DUMultiTrends::execute() {
  ...
  if ( m_dMon != 0 ) {
    if ( ( Physics & m_dMon ) != 0 && m_fromRaw->report().decision( Physics ) )
      cFill( "Counters/Decisions/Physics", bin );
  ...
  if ( m_trendPeriod > 0 && m_odin ) {
    ...
    if ( NULL != odin ) {
      ...
      if ( diff < 0 )
        counter( "lost events in trending" ) += 1;
        ...
        if ( bin2 >= m_trendPeriod ) {
          ...
          if ( msgLevel( MSG::DEBUG ) )
            debug() << "Sliding origin = " << m_origin << " time " << time << " s - eventNumber " << odin->eventNumber()
...
void L0DUMultiTrends::cFillSubTriggers() {
  ...
  for ( std::vector<std::string>::iterator i = temp.begin(); i != temp.end(); i++ ) {
    ...
    for ( LHCb::L0DUTrigger::Map::const_iterator it = triggers.begin(); triggers.end() != it; ++it ) {
      ...
      if ( *i == "data" && m_fromRaw->report().triggerDecisionByName( name ) )
        cFill( unit, bin );
      else if ( trigger->emulatedDecision() )
        cFill( unit, bin );
    }
...
void L0DUMultiTrends::cFillChannels() {
  ...
  for ( std::vector<std::string>::iterator i = temp.begin(); i != temp.end(); i++ ) {
    ...
    for ( LHCb::L0DUChannel::Map::const_iterator it = channels.begin(); channels.end() != it; ++it ) {
      ...
      if ( *i == "data" && m_fromRaw->report().channelDecisionByName( name ) )
        cFill( unit, bin );
      else if ( channel->emulatedDecision() )
        cFill( unit, bin );
    }
...
void L0DUMultiTrends::booking( std::string pref ) {
  ...
  if ( m_dMon != 0 ) {
    if ( ( Physics & m_dMon ) != 0 )
      cBook( pref + "Decisions/Physics",
             "L0|Physics decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " versus TCK" );
    if ( ( Beam1 & m_dMon ) != 0 )
      cBook( pref + "Decisions/Beam1",
             "L0|Beam1 decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " versus TCK" );
    if ( ( Beam2 & m_dMon ) != 0 )
      cBook( pref + "Decisions/Beam2",
             "L0|Beam2 decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " versus TCK" );
  }
  ...
  if ( 0 < m_trendPeriod ) {
    ...
    if ( m_dMon != 0 ) {
      if ( ( Physics & m_dMon ) != 0 )
        tBook( pref + "Trending/Decisions/Data/Physics",
               "L0|Physics trending decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " for data TCK" );
      if ( ( Beam1 & m_dMon ) != 0 )
        tBook( pref + "Trending/Decisions/Data/Beam1",
               "L0|Beam1 trending decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " for data TCK" );
      if ( ( Beam2 & m_dMon ) != 0 )
        tBook( pref + "Trending/Decisions/Data/Beam2",
               "L0|Beam2 trending decision " + std::string( ( pref == "Rates/" ) ? "rates" : "" ) + " for data TCK" );
      int ic = 0;
      for ( std::vector<std::string>::iterator it = m_list.begin(); it != m_list.end(); it++ ) {
        int               itck   = tck2int( *it );
        LHCb::L0DUConfig* config = m_config->config( itck );
