...
//=============================================================================
L0DUEmulatorTool::L0DUEmulatorTool( const std::string& type, const std::string& name, const IInterface* parent )
    : base_class( type, name, parent ) {
  declareInterface<IL0DUEmulatorTool>( this );
  declareProperty( "MuonCleaning",
                   m_muCleaning = false ); // set to false as long as the L0Muon cleaning is commented in the firmware.
  declareProperty( "MuonZeroSup", m_muZeroSup = false ); // no matter for the emulation/simulation
  declareProperty( "EmulateBXs",
                   m_emuBX = false ); // Emulate Prev/Next BX (& Sum(Et)) in the bank - filled with 0 on MC!!
...
StatusCode L0DUEmulatorTool::fillData() {
  ...
  if ( msgLevel( MSG::VERBOSE ) ) {
    ...
    for ( const auto& data : dataMap ) { verbose() << "Data :  " << data.second->summary() << endmsg; }
  }
...
StatusCode L0DUEmulatorTool::dataTree( LHCb::L0DUElementaryData* data, LHCb::L0DUElementaryData::Map dataMap ) {
  ...
  for ( const auto& iop : data->componentsName() ) {
    if ( dataMap.find( iop ) == dataMap.end() )
      return Warning( "dataTree: the data '" + iop + "' is not defined in the L0DUConfig data map",
                      StatusCode::FAILURE );
...
StatusCode L0DUEmulatorTool::processing() {
  ...
  // downscaling counters updated once per event (paranoid pretection against multi-call of the emulator processing  /
  // event)
...
const std::vector<unsigned int> L0DUEmulatorTool::bank( unsigned int version ) {
  ...
  if ( 0 == version ) {
    ...
    l0Block.push_back( digit( L0DUBase::PileUp::Peak1 ) | digit( L0DUBase::PileUp::Peak1Pos ) << 8 ); // PileUp Peak1
    l0Block.push_back( digit( L0DUBase::PileUp::Peak2 ) | digit( L0DUBase::PileUp::Peak2Pos ) << 8 ); // PileUp Peak2
    l0Block.push_back( digit( L0DUBase::PileUp::Hits ) );                                             // PileUp #Hits
    l0Block.push_back( digit( L0DUBase::Sum::Et ) | digit( L0DUBase::Spd::Mult ) << 14 );             // SumEt + SpdMult
  ...
  //---------------------------------------------------------
    ...
    // PGA3-Block
    // -----------
    auto ptMu = std::array{digit( L0DUBase::Muon1::Pt ) | ( digit( L0DUBase::Muon1::Sign ) << 7 ),
    ...
    auto adMu =
        std::array{digit( L0DUBase::Muon1::Address ) | ( 0 << 13 ), digit( L0DUBase::Muon2::Address ) | ( 1 << 13 ),
                   digit( L0DUBase::Muon3::Address ) | ( 2 << 13 ), digit( L0DUBase::Muon4::Address ) | ( 3 << 13 ),
                   digit( L0DUBase::Muon5::Address ) | ( 4 << 13 ), digit( L0DUBase::Muon6::Address ) | ( 5 << 13 ),
                   digit( L0DUBase::Muon7::Address ) | ( 6 << 13 ), digit( L0DUBase::Muon8::Address ) | ( 7 << 13 )};
    ...
    // Fill MuonBlock
    l0Block.push_back( digit( L0DUBase::Muon1::Status ) | digit( L0DUBase::Muon3::Status ) << 4 |
                       digit( L0DUBase::Muon5::Status ) << 8 | digit( L0DUBase::Muon7::Status ) << 12 | nmuons << 28 );
    // Processed muon
    ...
    for ( unsigned int i = 0; i < ( nmuons + 1 ) / 2; ++i ) { l0Block.push_back( muAdd[i] ); }
    for ( unsigned int i = 0; i < ( nmuons + 3 ) / 4; ++i ) { l0Block.push_back( muPt[i] ); }
    ...
    unsigned int status = digit( L0DUBase::Electron::Status ) << L0DUBase::Fiber::CaloElectron |
    ...
    if ( 1 == version ) w |= ( nBxm << 28 ) | ( nBxp << 30 );
    ...
    l0Block.push_back( digit( L0DUBase::Sum::Et ) | digit( L0DUBase::Spd::Mult ) << 14 |
                       digit( L0DUBase::PileUp::MoreInfo ) << 28 );
    l0Block.push_back( digit( L0DUBase::Electron::Et ) | digit( L0DUBase::Photon::Et ) << 8 |
                       digit( L0DUBase::Pi0Global::Et ) << 16 | digit( L0DUBase::Pi0Local::Et ) << 24 );
    l0Block.push_back( digit( L0DUBase::Hadron::Et ) | digit( L0DUBase::PileUp::Peak1 ) << 8 |
                       digit( L0DUBase::PileUp::Peak2 ) << 16 | digit( L0DUBase::PileUp::Hits ) << 24 );
#ifdef __INTEL_COMPILER           // Disable ICC warning
#  pragma warning( disable : 68 ) // integer conversion resulted in a change of sign
#  pragma warning( push )
#endif
    l0Block.push_back( digit( L0DUBase::Electron::Address ) | 0x2 << 14 | digit( L0DUBase::Photon::Address ) << 16 |
                       0x2 << 30 );
    l0Block.push_back( digit( L0DUBase::Pi0Global::Address ) | 0x2 << 14 | digit( L0DUBase::Pi0Local::Address ) << 16 |
                       0x2 << 30 );
#ifdef __INTEL_COMPILER // Re-enable ICC warning 68
#  pragma warning( pop )
#endif
    l0Block.push_back( digit( L0DUBase::Hadron::Address ) | 0x3 << 14 | digit( L0DUBase::PileUp::Peak1Pos ) << 16 |
