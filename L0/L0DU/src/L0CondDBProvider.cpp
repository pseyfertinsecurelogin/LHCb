...
double L0CondDBProvider::scale( unsigned int base ) const {
  switch ( base ) {
  case L0DUBase::Type::Digit:
    return 1;
  case L0DUBase::Type::CaloEt:
    return caloEtScale();
  case L0DUBase::Type::MuonPt:
    return muonPtScale();
  }
...
double L0CondDBProvider::caloEtScale() const {
  ...
  if ( msgLevel( MSG::DEBUG ) ) debug() << "CaloEt scale set to " << caloEtScale << " MeV" << endmsg;
  return caloEtScale;
}
...
double L0CondDBProvider::muonPtScale() const {
  ...
  if ( std::abs( caloEtScale() - 24. * Gaudi::Units::MeV ) < 1.e-3 )
    muonPtScale = 50. * Gaudi::Units::MeV; // patch for hardware change on 2015/06/02
  return muonPtScale;
}
