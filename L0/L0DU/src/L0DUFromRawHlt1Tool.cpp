...
//=============================================================================
L0DUFromRawHlt1Tool::L0DUFromRawHlt1Tool( const std::string& type, const std::string& name, const IInterface* parent )
    : base_class( type, name, parent ) {
  declareInterface<IL0DUFromRawTool>( this );
  declareProperty( "RawLocations",
                   m_rawLocations = {LHCb::RawEventLocation::Trigger, LHCb::RawEventLocation::Default} );
...
bool L0DUFromRawHlt1Tool::decodeBank( int iBank ) {
  ...
  if ( !raw ) {
    Warning( "rawEvent not found in  " + Gaudi::Utils::toString( m_rawLocations ) + " locations ", StatusCode::SUCCESS )
        .ignore();
    return false;
  }
  ...
  if ( m_size != allSize ) {
    info() << "READOUTSTATUS : the total expected size " << allSize << " does NOT match the bank size " << m_size
  ...
  for ( unsigned int i = 0; i < itc; i++ ) { m_report.setChannelsPreDecisionSummary( *( ++data ), i, 0 ); }
  for ( unsigned int i = 0; i < itc; i++ ) { m_report.setChannelsDecisionSummary( *( ++data ), i, 0 ); }
  for ( unsigned int i = 0; i < iec; i++ ) { m_report.setConditionsValueSummary( *( ++data ), i, 0 ); }
  ...
  for ( int im = nm; im > 0; im-- ) {
    for ( unsigned int i = 0; i < itc; i++ ) { m_report.setChannelsDecisionSummary( *( ++data ), i, -im ); }
    for ( unsigned int i = 0; i < iec; i++ ) { m_report.setConditionsValueSummary( *( ++data ), i, -im ); }
  }
  ...
  for ( int ip = 0; ip < np; ip++ ) {
    for ( unsigned int i = 0; i < itc; i++ ) { m_report.setChannelsDecisionSummary( *( ++data ), i, ip + 1 ); }
    for ( unsigned int i = 0; i < iec; i++ ) { m_report.setConditionsValueSummary( *( ++data ), i, ip + 1 ); }
  }
