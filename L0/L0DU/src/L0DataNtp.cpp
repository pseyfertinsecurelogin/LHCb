...
DECLARE_COMPONENT( L0DataNtp )

//=============================================================================
// Standard constructor, initializes variables
//=============================================================================
L0DataNtp::L0DataNtp( const std::string& name, ISvcLocator* pSvcLocator )
    : GaudiTupleAlg( name, pSvcLocator ), m_l0( NULL ), m_l0conf( NULL ), m_l0emu( NULL ) {
...
StatusCode L0DataNtp::execute() {
  ...
  for ( std::map<std::string, std::pair<unsigned int, double>>::const_iterator it = datamap.begin();
    ...
    if ( LHCb::CaloAlgUtils::StringMatcher( m_list, name ) ) {
      if ( m_scale )
        sc = ntp->column( "L0Data_" + rename( name ), double( val ) * scale );
      else
        sc = ntp->column( "L0Data_" + rename( name ), val );
    }
  }
