...
//=============================================================================
L0DUFromRawTool::L0DUFromRawTool( const std::string& type, const std::string& name, const IInterface* parent )
    : base_class( type, name, parent )
    ,
    // DO NOT TOUCH !! IF YOU MODIFY THIS VALUE THIS WILL BREAK THE DC06 BACKWARD COMPATIBILITY
    m_tck( 0xDC06 )
    , // default value for DC06 production (TCK was not implemented in Bank)
    m_sumEtPrev1( -1 )
    , m_sumEtPrev2( -1 ) {
  ...
  declareProperty( "FakeConsecutiveEvents",
                   m_fakeSeq = false ); // EXPERT USAGE : fake SumEt,Prev as for consecutive events
...
StatusCode L0DUFromRawTool::initialize() {
  ...
  if ( m_fakeSeq )
    Warning( "L0DU IS FAKING CONSECUTIVE EVENTS : SumEtPrev IS OVERWRITTEN !!", StatusCode::SUCCESS ).ignore();

  //
  // m_slot = ( rootInTES() == "" ) ? "T0" : rootInTES();
  if ( ( rootInTES() == "Prev7/" ) || ( rootInTES() == "Prev6/" ) || ( rootInTES() == "Prev5/" ) ||
       ( rootInTES() == "Prev4/" ) || ( rootInTES() == "Prev3/" ) || ( rootInTES() == "Prev2/" ) ||
       ( rootInTES() == "Prev1/" ) || ( rootInTES() == "Next1/" ) || ( rootInTES() == "Next2/" ) ||
       ( rootInTES() == "Next3/" ) || ( rootInTES() == "Next4/" ) || ( rootInTES() == "Next5/" ) ||
       ( rootInTES() == "Next6/" ) || ( rootInTES() == "Next7/" ) )
    m_slot = rootInTES();
  else
    m_slot = "T0";
  ...
  if ( m_muonNoZsup ) { warning() << " ========> WARNING : Muons are assumed to be non zero-suppressed : " << endmsg; }
...
bool L0DUFromRawTool::getL0DUBanksFromRaw() {
  ...
  if ( !rawEvt ) {
    Warning( "rawEvent not found in  '" + Gaudi::Utils::toString( m_rawEventLocations ) + "' locations",
             StatusCode::SUCCESS )
        .ignore();
  ...
  if ( 1 != m_banks.size() ) {
    ...
    if ( m_warn )
      Warning( "READOUSTATUS : more than one L0DU bank has been found in the RawEvent", StatusCode::SUCCESS ).ignore();
...
bool L0DUFromRawTool::decoding( int ibank ) {
  ...
  for ( auto& i : *m_processorDatas ) {
    for ( const auto& ibx : i->bxList() ) { i->setWord( L0DUBase::EmptyData, ibx ); }
  }
  ...
  if ( msgLevel( MSG::DEBUG ) ) {
    ...
    debug() << "Decoding bank : " << bank << " Source : " << m_source << " Version : " << m_vsn << " Size " << m_size
            << " (bytes) " << endmsg;
  }
  ...
  if ( m_vsn == 0 ) {
    ...
    if ( m_warning ) {
      ...
      if ( m_warning )
        Warning( "L0DU bank version = 0 --> the TCK value is forced to " + tck.str(), StatusCode::SUCCESS ).ignore();
    }
  ...
  //---------------------------------------------------------
    ...
    if ( !config ) {
      std::stringstream tck;
      m_roStatus.addStatus( 0, LHCb::RawBankReadoutStatus::Status::Unknown );
      tck << " Unable to load the configuration for tck = " << format( "0x%04X", m_tck )
          << " --> Incomplete L0DUReport";
      Warning( tck.str(), StatusCode::SUCCESS ).ignore();
    } else {
      m_report.setConfiguration( config );
    }
    ...
    if ( m_muonNoZsup ) {
      std::stringstream msg( "" );
      if ( 8 != nmu ) {
        msg << "READOUTSTATUS : muons are supposed to be NON 0-suppressed but the bank content indicates " << nmu
            << " muons only (will assume 8 muons for decoding)";
    ...
    if ( msgLevel( MSG::VERBOSE ) ) {
      ...
      verbose() << "   ... L0Muon processed data decoded  - Muon cleaning pattern = " << m_muCleanPattern << " ["
                << format( "0x%04X", m_muCleanPattern ) << "] ..." << endmsg;
    }
    ...
    for ( unsigned int imu = 0; imu < nmu; imu++ ) {
      ...
      switch ( n[imu] ) {
      case 0:
        encode( M0_Add, val, L0DUBase::Muon1::Address );
        break;
      case 1:
        encode( M1_Add, val, L0DUBase::Muon2::Address );
        break;
      case 2:
        encode( M2_Add, val, L0DUBase::Muon3::Address );
        break;
      case 3:
        encode( M3_Add, val, L0DUBase::Muon4::Address );
        break;
      case 4:
        encode( M4_Add, val, L0DUBase::Muon5::Address );
        break;
      case 5:
        encode( M5_Add, val, L0DUBase::Muon6::Address );
        break;
      case 6:
        encode( M6_Add, val, L0DUBase::Muon7::Address );
        break;
      case 7:
        encode( M7_Add, val, L0DUBase::Muon8::Address );
        break;
      }
    }
    ...
    if ( msgLevel( MSG::DEBUG ) ) {
      ...
      debug() << "   -> Decisions : [ physics : " << decision << "| beam1 : " << beam1 << " |  beam2 : " << beam2 << "]"
              << endmsg;
      ...
      debug() << "   -> Processors Status : " << ( m_status & 0x1FF ) << " ["
              << format( "0x%04X", ( m_status & 0x1FF ) ) << "]" << endmsg;
    }
    ...
    unsigned int pga2Size =
        4 * ( 7 + iec + 2 * itc + ( itc + iec ) * ( nm + np ) + ( int( ( nm + 1 ) / 2 ) + int( ( np + 1 ) / 2 ) ) );
    ...
    if ( ( 0x7F & m_bcid2 ) == m_bcid3 ) {
      if ( msgLevel( MSG::DEBUG ) )
        debug() << "   -> The PGA3 and PGA2 data are aligned _____________________________________ <** OK **> "
                << endmsg;
    } else {
    ...
    for ( unsigned int i = 0; i < itc; i++ ) {
      ...
      m_tcs[{0, i}] = *m_data;
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "   ->  Channel PreDecision summary[BX=0, Word=" << i << "] = " << *m_data << " [0x"
                  << format( "0x%04X", *m_data ) << "]" << endmsg;
    }
    for ( unsigned int i = 0; i < itc; i++ ) {
      if ( !nextData() ) return false;
      m_cds[{0, i}] = *m_data;
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "   -> Channel Decision summary[BX=0, Word=" << i << "] = " << *m_data << " [0x"
                  << format( "0x%04X", *m_data ) << "]" << endmsg;
    }
    for ( unsigned int i = 0; i < iec; i++ ) {
      ...
      m_ecs[{0, i}] = *m_data;
      if ( msgLevel( MSG::VERBOSE ) )
        verbose() << "   ->  Elementary condition summary[BX=0, Word=" << i << "] = " << *m_data << " [0x"
                  << format( "0x%04X", *m_data ) << "]" << endmsg;
    }
    ...
    for ( int im = nm; im > 0; im-- ) {
      for ( unsigned int i = 0; i < itc; i++ ) {
        ...
        m_cds[{-im, i}] = *m_data;
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "   -> Trigger Channel Summary[BX=" << -im << ", Word=" << i << "] = " << *m_data << " [0x"
                    << format( "0x%04X", *m_data ) << "]" << endmsg;
      }
      for ( unsigned int i = 0; i < iec; i++ ) {
        if ( !nextData() ) return false;
        m_ecs[{-im, i}] = *m_data;
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "   -> Elementary Condition Summary[BX=" << -im << ", Word=" << i << "] = " << *m_data << " [0x"
                    << format( "0x%04X", *m_data ) << "]" << endmsg;
      }
    }
    ...
    for ( int ip = 0; ip < np; ip++ ) {
      for ( unsigned int i = 0; i < itc; i++ ) {
        ...
        m_cds[{ip + 1, i}] = *m_data;
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "   -> Trigger Channel Summary[BX=" << ip + 1 << ", Word=" << i << "] = " << *m_data << " [0x"
                    << format( "0x%04X", *m_data ) << "]" << endmsg;
      }
      for ( unsigned int i = 0; i < iec; i++ ) {
        if ( !nextData() ) return false;
        m_ecs[{ip + 1, i}] = *m_data;
        if ( msgLevel( MSG::VERBOSE ) )
          verbose() << "   -> Elementary Condition Summary[BX=" << ip + 1 << ", Word=" << i << "] = " << *m_data
                    << " [0x" << format( "0x%04X", *m_data ) << "]" << endmsg;
      }
    }
    ...
    // check the size
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << " <============= Decoding completed successfuly =============>" << endmsg;
    ...
    if ( m_emu ) {
      ...
      if ( temp && !temp->channels().empty() ) { m_emuTool->process( temp, m_processorDatas.get() ).ignore(); }
    }
    // add data value to L0DUReport embeded map if not available via L0DUConfig (i.e. status bit or no emulation or
    // unknown TCK)
    ...
    if ( msgLevel( MSG::VERBOSE ) )
      verbose() << " <==== Building output (emulated ProcessorData & L0DUReport) completed successfuly ====>" << endmsg;
...
unsigned int L0DUFromRawTool::data( const std::string& name ) const {
  ...
  return it != m_dataMap.end() ? it->second.first : m_report.dataDigit( name );
}
...
double L0DUFromRawTool::dataScaled( const std::string& name ) const {
  ...
  // if not found : look for in config->data() (e.g. compound)
  auto it = m_dataMap.find( name );
  return it != m_dataMap.end() ? it->second.first * it->second.second : m_report.dataValue( name );
}
...
std::string L0DUFromRawTool::dump() const {
  ...
  info() << " ------------ bank body event " << m_count << "-------------" << std::endl << s.str() << endmsg;
  return s.str();
}
