...
class L0Conf(LHCbConfigurableUser):
    ...
    _propertyDocDct = {
        # Properties
        "RawEventLocations":
        """List of raw event input locations.""",
        "RootInTESOnDemand":
        """List of RootInTES for L0 algorithms decoding called via the on demand service.""",
        "EnableTAEOnDemand":
        """ If True, the data on demand service is set up to decode TAE events.""",
        "ReplaceL0BanksWithEmulated":
        """ If True, run the emulators and replace the existing L0 banks.""",
        "ForceSingleL0Configuration":
        """ Option to be used in Moore to force a single TCK configuration.""",
        "ReplayL0DU":
        """ If True, run the L0DU emulation starting from the L0DU banks.""",
        "SimulateL0":
        """ If True, run the L0 simulation and write L0Banks.""",
        "EmulateL0":
        """ If True, run the L0 emulators and write on TES at a non default location.""",
        "DecodeL0":
        """ If True, run the L0 decoding (decode all L0 banks).""",
        "DecodeL0DU":
        """ If True, run the L0DU decoding.""",
        "MonitorL0":
        """ If True, run the L0 monitoring algorithms.""",
        "FilterL0FromRaw":
        """ If True, run the L0DU decoding and filter according to L0 decision.""",
        "FilterL0":
        """ If True, filter according to L0 decision.""",
        "TCK":
        """ Specifies the TCK to be used in simulation or emulation.""",
        "EnsureKnownTCK":
        """ If True, the L0DU decoding will end with an error if the used TCK is not recognized, otherwise the L0DUReport will miss the L0DU configuration .""",
        "FullPileUpSimulation":
        """ If True, perform the full pileup simulation, filling raw banks from MC/Velo/PuFEs with PuVetoFillRawBuffer.""",
        "EnableL0DecodingOnDemand":
        """ If True, setup the data on demand service for L0.""",
        "FastL0DUDecoding":
        """ If True, activate fast decoding for L0DU.""",
        "FullL0MuonDecoding":
        """ If True, decode all the L0Muon banks. Otherwise, decode only the one with the L0MuonCandidates.""",
        "IgnoreL0MuonCondDB":
        """ If True, the L0Muon emulator parameter are not taken from the condition data base.""",
        "L0MuonUseTCKFromData":
        """ If True, the L0Muon emulator will use the event TCK to get the FOI values.""",
        "L0DecodingContext":
        """ String appended to the default TES location where the results of the decoding are stored.""",
        "L0EmulatorContext":
        """ String appended to the default TES location where the results of the emulation are stored.""",
        "L0MuonForceLUTVersion":
        """ LUT version to enforce for the L0Muon emulation.""",
        "verbose":
        """Obsolete"""
        # Sequencers
        ,
        "L0Sequencer":
        """ Sequencer filled according to the L0Conf properties.""",
        "LinkSequencer":
        """ Sequencer filled with the MC associator algorithms (not configurable).""",
        "MoniSequencer":
        """ Sequencer filled with the L0 monitoring sequence (not configurable).""",
        "FilterSequencer":
        """ Sequencer filled with the L0Filter algorithm (not configurable).""",
        "ETCSequencer":
        """ Sequencer filled with the algorithm and stream to write out a L0-ETC.""",
        "ETCOutput":
        """ Name of ETC output file.""",
        "DataType":
        """ Data type, used to set up default TCK """
        # Herschel-specific options
        ,
        "EmulateHC":
        """ Flag whether or not to emulate Herschel. False by default for backward compatibility """,
        "HCTriggerBitsFromADCs":
        """ If True, compute the Herschel L0 trigger bit sum based on the values of the raw ADCs in each counter with respect to a threshold. """,
        "L0HCAlgThresholdsB0":
        """ Herschel B0 thresholds if computing L0 HC trigger bits """,
        "L0HCAlgThresholdsB1":
        """ Herschel B1 thresholds if computing L0 HC trigger bits """,
        "L0HCAlgThresholdsB2":
        """ Herschel B2 thresholds if computing L0 HC trigger bits """,
        "L0HCAlgThresholdsF1":
        """ Herschel F1 thresholds if computing L0 HC trigger bits """,
        "L0HCAlgThresholdsF2":
        """ Herschel F2 thresholds if computing L0 HC trigger bits """,
        "FakeHCL0Digits":
        """ If True, set all Herschel trigger bits to 1 ie all counters over-threshold. """
    }
    ...
    def checkOptions(self):
        ...
        if self.getProp("SimulateL0") and self.getProp("EmulateL0"):
            raise L0ConfError(
                "SimulateL0", "EmulateL0",
                "Simulation and Emulation can not run in the same job.")
        if self.getProp("ReplaceL0BanksWithEmulated") and self.getProp(
                "SimulateL0"):
        ...
        for l0prop in ("DecodeL0", "DecodeL0DU", "ReplaceL0BanksWithEmulated",
                       "SimulateL0", "EmulateL0"):
        ...
        if self.getProp("ForceSingleL0Configuration"
                        ) and not self.isPropertySet("TCK"):
        ...
        if self.getProp("HCTriggerBitsFromADCs") and self.getProp(
                "FakeHCL0Digits"):
            raise L0ConfError(
                "HCTriggerBitsFromADCs", "FakeHCL0Digits",
                "Does not make sense to request computed and faked Herschel L0 trigger bits simultaneously"
            )
    ...
    def l0monitoringSeq(self, name="L0MoniSeq"):
        ...
        # Build the sequence
        l0monitoringSeq.Members += [
            monitorL0Calo(), monitorL0Muon(),
            monitorL0DU()
        ]
    ...
    def _setAlgTCK(self, alg):
        ...
        if self.isPropertySet("TCK"):  # Use L0Conf.TCK if set
            ...
            if alg.isPropertySet("TCK"):
                log.warning(
                    "%s.TCK and L0Conf().TCK both set, using L0Conf().TCK = %s"
                    % (alg.getName(), tck))
            alg.TCK = tck
        else:
            if alg.isPropertySet("TCK"):  # Use already defined TCK
                log.info(
                    "%s.TCK was set independently of L0Conf" % (alg.getName()))
                ...
                if self.getProp("DataType").upper() in ["MC09"]:
                    ...
                    alg.TCK = "0x0032"
        log.info(
            "%s will use TCK=%s" % (alg.getFullName(), alg.getProp('TCK')))
    ...
    def _defineETC(self):
        ...
        if not MyWriter.isPropertySet("Output"):
            MyWriter.Output = "Collection='EVTTAGS/TagCreator/1' ADDRESS='/Event' DATAFILE='" + self.getProp(
                "ETCOutput") + "' TYP='POOL_ROOTTREE' OPT='RECREATE'"

        writeTagSeq.Members += [tagCreator, MyWriter]

    def _enableTAEOnDemand(self):
        list_of_taeintes = ['Prev%d/' % (itae) for itae in range(1, 8)] + [
            ''
        ] + ['Next%d/' % (itae) for itae in range(1, 8)]
        for taeintes in list_of_taeintes:
            self._setSpecificDecodingOptions(taeintes)
            self._dataOnDemand(taeintes)
            decodeL0Calo(taeintes).RawEventLocations = [
                taeintes + 'DAQ/RawEvent'
            ]
            decodeL0Muon(taeintes).RawEventLocations = [
                taeintes + 'DAQ/RawEvent'
            ]
            decodeL0DU(taeintes).RawEventLocations = [
                taeintes + 'DAQ/RawEvent'
            ]
    ...
    def _setSpecificDecodingOptions(self, rootintes=''):
        ...
        if self.getProp("EnsureKnownTCK"):
            decodeL0DU(rootintes).EnsureKnownTCK = self.getProp(
                "EnsureKnownTCK")
    ...
    def _setSpecificEmulationOptions(self):
        ...
        if self.isPropertySet("IgnoreL0MuonCondDB"):
            log.info(
                "L0Muon emulator will use the event TCK to get the FOI values : %s"
                % (self.getProp("IgnoreL0MuonCondDB")))
            l0muon = emulateL0Muon()
        ...
        if self.isPropertySet("L0MuonUseTCKFromData"):
            log.info(
                "L0Muon emulator will use the event TCK to get the FOI values : %s"
                % (self.getProp("L0MuonUseTCKFromData")))
            l0muon = emulateL0Muon()
        ...
        if self.isPropertySet("L0EmulatorContext"):
            ...
            log.info(
                "The results of the L0 emulation will be written at location+%s"
                % (l0context))
    ...
    def _dataOnDemand(self, rootintes):
        ...
        DataOnDemandSvc().AlgMap[
            rootintes +
            "Trig/L0/L0DUReport"] = "L0DUDecoder/" + L0DUFromRawAlgName + rootintes
