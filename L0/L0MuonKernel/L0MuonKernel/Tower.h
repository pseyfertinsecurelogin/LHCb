...
namespace L0Muon {
  ...
  /** @class Tower Tower.h L0MuonKernel/Tower.h
    ...
    - Representation of data :
      \n\n
      The logical pads which have been fired are stored in an array of @link Tower::StationMap StationMap @endlink
    (#m_bittable). The tower contains 5 StationMaps (1 per station). They are filled in the CoreUnit::makePads method
    which make use of Tower::setBit. \n\n The tower dimensions are determined by the logical pads layout and by the
    fields of interest used by the muon candidate finding algorithm. \n\n The granularity is different from a muon
    station to an other. Thus, the number of logical pads in a trigger sector depends on the station. In the tower, all
    stations are bring to the finest granularity (M3). Therefore, 1 hit in M1 will light 2 bits in the tower's
    StationMap. \n\n In addition, the tower is extended w.r.t. a trigger sector to allow to search for hits which would
    be inside the field of interest but outside the trigger sector boundaries. In other words, the tower takes into
    account the data received by a PU from it's neighbours. Therefore, the number of rows and columns in each station
    depends on the field of interest. \n\n To size up the tower, the maximum allowed values of the field of interest are
    used. They are stored in the #m_maxXFoI and #m_maxYFoI attributes of the tower which are initialized in the @link
    Tower::Tower  default constructor @endlink (with values given in the granularity of M3).\n\n In conclusion, the
    tower (StationMaps) dimensions are :
  ...
  */
  ...
  public:
    ...
    */
    void setProcVersion( int version ) { m_procVersion = version; }
    ...
    */
    void setIgnoreM2( bool ignoreM2 ) { m_ignoreM2 = ignoreM2; }

    /** set pointer to LUT
        @param m_lut : pointer to the L0MPtLUT object holding the pt LUT
    */
    void setLUTPointer( L0MPtLUT* lut ) { m_lut = lut; }
    ...
    void setDebugMode( bool debug = true ) {
      if ( debug ) std::cout << "Tower:setDebugMode\n";
      m_debug = debug;
    }
