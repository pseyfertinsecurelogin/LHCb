...
namespace L0Muon {
  ...
  */
  ...
  public:
    ...
    int colM3() const { return m_colM3; }

    /// Get the address in M3 (row)
    int rowM3() const { return m_rowM3; }

    /// Get the offset in M2
    int offM2() const { return m_offM2; }

    /// Get the offset in M1
    int offM1() const { return m_offM1; }

    /// Get the quarter
    int quarter() const { return m_quarter; }

    /// Get the pu number
    int pu() const { return m_pu; }
    ...
    void setColM3( int colM3 ) { m_colM3 = colM3; }

    /// Set the address in M3 (row)
    void setRowM3( int rowM3 ) { m_rowM3 = rowM3; }

    /// Set the offset in M2
    void setOffM2( int offM2 ) { m_offM2 = offM2; }

    /// Set the offset in M1
    void setOffM1( int offM1 ) { m_offM1 = offM1; }

    /// Set the pu number
    void setQuarter( int quarter ) { m_quarter = quarter; }

    /// Set the pu number
    void setPu( int pu ) { m_pu = pu; }
    ...
    /// Set the charge
    void setCharge( int charge ) { m_charge = charge > 0 ? +1 : 0; }

    /// Set the charge
    void setCharge( double pT ) { m_charge = pT > 0. ? +1 : 0; }
...
inline bool L0Muon::MuonCandidate::operator<( const L0Muon::MuonCandidate& cand ) const { return pT() < cand.pT(); }

inline bool L0Muon::MuonCandidate::operator==( const L0Muon::MuonCandidate& cand ) const {
  bool eq = ( pT() == cand.pT() ) && ( charge() == cand.charge() ) && ( offM2() == cand.offM2() ) &&
            ( offM1() == cand.offM1() ) && ( colM3() == cand.colM3() ) && ( rowM3() == cand.rowM3() ) &&
            ( pu() == cand.pu() ) && ( board() == cand.board() ) && ( quarter() == cand.quarter() );
  return eq;
}

inline bool L0Muon::MuonCandidate::operator!=( const L0Muon::MuonCandidate& cand ) const { return !( cand == *this ); }
