...
namespace L0Muon {
  ...
  */
    ...
    */
    ...
    void dump( std::string tab = "" ) {
      for ( int i = 0; i < 2; ++i ) {
        ...
        for ( int ib = 0; ib < 12; ++ib ) { m_candRegHandlerBCSU[i][ib].dump( -1, tab ); }
      }
    }
  ...
  private:
    ...
    CandRegisterHandler m_candRegHandler[2]; ///< Final candidates registers (candidates selected by the controller
                                             ///< board).

    CandRegisterHandler m_candRegHandlerBCSU[2][12]; ///< BCSU candidates registers (candidates received by the
                                                     ///< controller board).
