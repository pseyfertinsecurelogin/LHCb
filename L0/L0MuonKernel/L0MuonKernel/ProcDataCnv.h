...
namespace L0Muon {
  ...
  */
  ...
  private:
    static const unsigned int PB_OpticalLinks_size =
        24; ///< Number of words in the opt. link field of a processing board
    static const unsigned int PB_Neighbours_size =
        34; ///<  Number of words in the neighbour field of a processing board
  ...
  private:
    ...
    */
    int compressedPBWords_to_bitset( const std::vector<unsigned int>& raw, const int size, unsigned int& word_index,
                                     int& bit_index, boost::dynamic_bitset<>& rawbitset );
    ...
    /** Consider an uncompressed slice of raw data in L0MuonProcData bank corresponding to the optical link field
        ...
        @param bit_index : index of the first bit of the slice in the firt word
    */
    int notcompressedPBWords_to_bitset( const std::vector<unsigned int>& raw, const int size, unsigned int& word_index,
    ...
    std::map<LHCb::MuonTileID, TileRegister*> m_olsMap;    ///< Map of pointers to TileRegister containing the optical
                                                           ///< links data; the key is the MuonTileID of the PU
    std::map<LHCb::MuonTileID, TileRegister*> m_neighsMap; ///< Map of pointers to TileRegister containing the neighbour
                                                           ///< data: the key is the MuonTileID of the PU
