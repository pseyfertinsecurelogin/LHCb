  ...
  for ( i1 = strips.begin(); i1 < strips.end(); ++i1 ) {
    if ( i1->station() == 0 ) {
      ...
      for ( i2 = i1 + 1; i2 < strips.end(); ++i2 ) {
        ...
        if ( pad.isValid() ) { pads.push_back( pad ); }
      }
    }
  }
}
...
std::string L0Muon::MonUtilities::hname_channels_hist( int qua, Channel_type type, int sta, int reg,
                                                       std::string toolname ) {
  return toolname + "Hits_" + quarterName( qua ) + "_" + stationName( sta ) + "_" + regionName( reg ) + "_" +
         channelTypeName( type );
}

std::string L0Muon::MonUtilities::hname_channels_hist2D( int qua, Channel_type type, int sta, int reg,
                                                         std::string toolname ) {
  return hname_channels_hist( qua, type, sta, reg, toolname ) + "2D";
}

std::string L0Muon::MonUtilities::hname_channels_histBX( int qua, Channel_type type, int sta, int reg,
                                                         std::string toolname ) {
  return hname_channels_hist( qua, type, sta, reg, toolname ) + "-VS-BX";
}

std::string L0Muon::MonUtilities::hname_channels_histDT( int qua, Channel_type type, int sta, int reg,
                                                         std::string toolname ) {
  return hname_channels_hist( qua, type, sta, reg, toolname ) + "-wrt-Muon";
}

std::string L0Muon::MonUtilities::hname_optlinks_evt( std::string toolname ) { return toolname + "evt_in_error"; }
...
std::string L0Muon::MonUtilities::hname_cand_pt( std::string toolname ) { return toolname + "PT"; }

std::string L0Muon::MonUtilities::hname_cand_encodedpt( std::string toolname ) { return toolname + "encodedPT"; }
...
std::string L0Muon::MonUtilities::hname_cand_sum( std::string toolname ) { return toolname + "number_of_candidates"; }
