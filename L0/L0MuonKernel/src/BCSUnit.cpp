...
void L0Muon::BCSUnit::initialize() {
  ...
  if ( itoutputs == m_outputs.end() )
    std::cout << "L0Muon::BCSUnit::initialize key " << buf << " not found in output registers\n";
...
void L0Muon::BCSUnit::execute() {
  if ( m_debug )
    std::cout << "L0Muon::BCSUnit::execute mid: "
  ...
  for ( itHandlerMap = m_candRegHandlerIn.begin(); itHandlerMap != m_candRegHandlerIn.end(); itHandlerMap++ ) {
    ...
    for ( int icand = 0; icand < ncand; icand++ ) {
      ...
      if ( m_debug )
        std::cout << "L0Muon::BCSUnit::execute setting PU # " << ipu << " for candidate # " << icand << std::endl;
