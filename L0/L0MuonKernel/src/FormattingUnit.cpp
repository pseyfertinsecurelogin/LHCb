...
void L0Muon::FormattingUnit::preexecute() {
  ...
  for ( ir = m_inputs.begin(); ir != m_inputs.end(); ir++ ) {
    ...
    if ( m_debug ) {
      std::cout << "*!* Formatting::preexecute:    "
                << " register is " << ir->first << " size: " << itr->size() << std::endl;
  ...
  if ( m_debug )
    std::cout << "*!* Formatting::preexecute: firedPads contains " << firedPads.size() << " tiles" << std::endl;
  ...
  if ( !m_outputs.empty() ) {
    ...
    for ( out = m_outputs.begin(); out != m_outputs.end(); out++ ) {
        ...
        for ( outit = outPads.begin(); outit != outPads.end(); outit++ ) {
          ...
          for ( init = firedPads.begin(); init != firedPads.end(); init++ ) {
            ...
            if ( outit->intercept( *init ).isValid() ) {
              ...
              if ( m_debug )
                std::cout << "*!* Formatting::preexecute: tiles IN: " << init->toString()
          ...
          if ( outit->station() == 1 ) {
            ...
            for ( init = firedTiles.begin(); init != firedTiles.end(); init++ ) {
              if ( outit->intercept( *init ) == ( *init ) ) {
                if ( m_debug )
                  std::cout << "*!* Formatting::preexecute: is contained tiles IN: " << init->toString()
                            << " - OUT: " << outit->toString() << std::endl;
      ...
      if ( m_debug )
        std::cout << "*!* Formatting::preexecute:   " << out->first << " " << outPads.size() << " pads"
                  << " " << outtr->firedTiles().size() << " fired tiles" << std::endl;
    }
  }
}
