...
int L0Muon::CtrlCandErrors::decodingError() const { return int( decoding.inError() ); }
...
int L0Muon::CtrlCandErrors::statusError() const { return best_status.value(); }

int L0Muon::CtrlCandErrors::statusError( int ib ) const { return bcsu_status[ib].value(); }
...
int L0Muon::CtrlCandErrors::sumCounters() const {
  ...
  n += l0_B_Id.counter();
  for ( int i = 0; i < 2; ++i ) { n += ctrl_bcid[i].counter(); }
  ...
  for ( int i = 0; i < 12; ++i ) {
    ...
    for ( int ich = 0; ich < 2; ++ich ) { n += link[i][ich].counter(); }
  }
  n += errJ.counter();
  n += errK.counter();
