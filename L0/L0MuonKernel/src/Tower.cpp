...
L0Muon::Tower::Tower() {
  ...
  // Init the idmap
  for ( int i = 0; i < 5; i++ ) {
    if ( m_idmap[i].size() > 0 ) { m_idmap[i].clear(); }
  }
  ...
  for ( sta = 0; sta < 5; sta++ ) {
    ...
    for ( row = 0; row < 4 + 2 * m_maxYFoI[sta]; row++ ) { m_bittable[sta].push_back( tmprow ); }
  }
...
void L0Muon::Tower::reset() {
  ...
  int row;
  for ( sta = 0; sta < 5; sta++ ) {
    for ( row = 0; row < 4 + 2 * ( m_maxYFoI[sta] ); row++ ) { m_bittable[sta][row].reset(); }
...
void L0Muon::Tower::setBit( int sta, int row, int col ) {
  ...
  if ( col < 0 || col >= ( 24 + 2 * xFoI ) ) {
    ...
    if ( m_debug )
      std::cout << "--- Tower::setBit -> Station: " << sta << " Col: " << col << " min=0 - max=" << 24 + 2 * xFoI
                << std::endl;
    return;
  }
  if ( row < 0 || row >= ( 4 + 2 * yFoI ) ) {
    ...
    if ( m_debug )
      std::cout << "--- Tower::setBit -> Station: " << sta << " Row: " << row << " min=0 - max=" << 4 + 2 * yFoI
                << std::endl;
    return;
  }
...
void L0Muon::Tower::setPadIdMap( int sta, std::pair<int, int> YX, LHCb::MuonTileID mid ) { m_idmap[sta][YX] = mid; }
...
void L0Muon::Tower::draw() {
  ...
  int sta;
  for ( sta = 4; sta >= 0; sta-- ) { drawStation( sta ); }
  std::cout << std::endl;
}
...
std::vector<L0Muon::PMuonCandidate> L0Muon::Tower::processTower( LHCb::MuonTileID& puID ) {
  ...
  if ( m_debug )
    std::cout << "--- Tower::processTower: Process Tower for PU "
              << "Q" << puID.quarter() + 1 << "R" << puID.region() + 1 << puID.nX() << puID.nY() << " foi:" << m_xfoi[0]
              << " ," << m_yfoi[0] << std::endl;
  ...
  for ( rowM3 = m_bittable[2].begin(); rowM3 != m_bittable[2].end(); rowM3++ ) {
    ...
    for ( boost::dynamic_bitset<>::size_type colM3 = 0; colM3 < ( *rowM3 ).size(); colM3++ ) {
      ...
      if ( ( *rowM3 ).test( colM3 ) ) {
        ...
        if ( m_debug )
          std::cout << "--- Tower::processTower: Seed found col,row= " << colseed << " " << rowseed << std::endl;
        ...
        for ( sta = 3; sta < 5; sta++ ) {
          ...
          if ( m_debug )
            std::cout << "--- Tower::processTower: Max foi X,Y= " << m_maxXFoI[sta] << " , " << m_maxYFoI[sta]
                      << std::endl;
          if ( m_debug )
          ...
          if ( m_debug ) {
            ...
            for ( rowInd = rowCentral - m_yfoi[sta]; rowInd <= rowCentral + m_yfoi[sta]; rowInd++ ) {
          ...
          for ( rowInd = rowCentral - m_yfoi[sta]; rowInd <= rowCentral + m_yfoi[sta]; rowInd++ ) {
            candFlag = true;
            if ( m_debug )
              std::cout << "--- Tower::processTower:   full bitset size is " << ( *rowInd ).size() << std::endl;
            if ( m_debug ) std::cout << "--- Tower::processTower:   full bitset is " << ( *rowInd ) << std::endl;
            field = ( *rowInd ) >> ( colseed + m_maxXFoI[sta] - m_xfoi[sta] );
            if ( m_debug )
              std::cout << "--- Tower::processTower:   shift is " << colseed + m_maxXFoI[sta] - m_xfoi[sta] << " ( "
                        << colseed << " + " << m_maxXFoI[sta] << " - " << m_xfoi[sta] << " )" << std::endl;
        ...
        // If M2 is used
          ...
          rowCentral = m_bittable[sta].begin() + rowseed + m_maxYFoI[sta];
          if ( m_debug )
            std::cout << "--- Tower::processTower:   full bitset size is " << ( *rowCentral ).size() << std::endl;
          ...
          if ( field.none() == false ) {
            ...
            for ( int icol = 0; icol < 2 * m_xfoi[sta] + 1; icol++ ) {
              ...
              if ( m_debug )
                std::cout << "--- Tower::processTower:     inside loop over columns ipendulum= " << ipendulum
                          << std::endl;
              ...
              if ( m_debug )
                std::cout << "--- Tower::processTower:     inside loop over columns test= " << field.test( colInd )
                          << std::endl;
              if ( field.test( colInd ) == true ) {
                ...
                candFlag = true;
                if ( m_debug )
                  std::cout << "--- Tower::processTower:     Bit ON found yxM2 is " << yxM2.first << "," << yxM2.second
                            << std::endl;
                if ( m_debug )
                  std::cout << "--- Tower::processTower:     Bit ON found mid is " << padM2.toString() << std::endl;
                break;
              }
        ...
        if ( m_ignoreM1 == true ) {
          ...
          if ( m_debug )
            std::cout << "--- Tower::processTower:     forced Bit is NOT valid yxM1 is " << yxM1.first << ","
                      << yxM1.second << std::endl;
          if ( !padM1.isValid() ) { // If at the edge of a quarter, the extrapolated point might not be valid
            for ( int icol = 0; icol < 2 * m_xfoi[sta] + 1; icol++ ) {
              ...
              if ( m_debug )
                std::cout << "--- Tower::processTower:       forced Bit for ipendulum = " << ipendulum << " yxM1 is "
                          << yxM1_notcentral.first << "," << yxM1_notcentral.second << std::endl;
              padM1 = getPadIdMap( sta, yxM1_notcentral );
              if ( padM1.isValid() ) {
                if ( m_debug )
                  std::cout << "--- Tower::processTower:     forced Bit for ipendulum IS VALID " << std::endl;
                offM1 = ipendulum;
                break;
              }
            }
          ...
          if ( padM1.isValid() ) {
            if ( m_debug ) {
              std::cout << "--- Tower::processTower:     forced Bit ON found yxM1 is " << yxM1.first << ","
                        << yxM1.second << std::endl;
              std::cout << "--- Tower::processTower:     forced Bit ON found mid is " << padM1.toString() << std::endl;
            }
        ...
        // If M1 is used
          ...
          if ( m_debug )
            std::cout << "--- Tower::processTower:   shift is " << colseed + m_maxXFoI[sta] + extrapM1 - m_xfoi[sta]
                      << " ( " << colseed << " + " << m_maxXFoI[sta] << " - " << extrapM1 << " - " << m_xfoi[sta]
                      << " )" << std::endl;
          if ( m_debug ) std::cout << "--- Tower::processTower:   shifted >> field is " << field << std::endl;
          field.resize( 2 * m_xfoi[sta] + 1 );
          if ( m_debug ) std::cout << "--- Tower::processTower:   new size is " << 2 * m_xfoi[sta] + 1 << std::endl;
          ...
          if ( field.none() == false ) {
            ...
            for ( int icol = 0; icol < 2 * m_xfoi[sta] + 1; icol++ ) {
              //               int ipendulum = (icol==0) ? 0 : int(pow(-1,icol+1)*int((icol+1)/2));
              //               int ipendulum = (icol==0) ? 0 : int(pow(-1,icol)*int((icol+1)/2)); // start searching
              //               towards beam
              int ipendulum = L0Muon::pendulumM1( icol, m_procVersion );
              colInd        = colCentral + ipendulum;
              if ( m_debug ) {
              ...
              if ( field.test( colInd ) == true ) {
                ...
                candFlag = true;
                if ( m_debug ) {
                  std::cout << "--- Tower::processTower:     Bit ON found yxM1 is " << yxM1.first << "," << yxM1.second
                            << std::endl;
                  std::cout << "--- Tower::processTower:     Bit ON found mid is " << padM1.toString() << std::endl;
                }
        ...
        if ( m_lut == 0 ) {
          ...
          if ( m_debug )
            std::cout << "--- Tower::processTower: (from kine) pt= " << pt << " encoded= " << ipt << std::endl;
        } else {
          ...
          if ( !m2pad.isValid() ) {
            if ( m2pad.nX() >= 2 * m2pad.layout().xGrid() ) {
              ...
              // if (lpads.size()!=1) {
              //  std::cout <<"--- Tower::processTower: ERROR M2 PAD DOES NOT COVER EXACTLY ONE PAD IN UPPER
              //  REGION"<<std::endl;
              //}
              m2pad = lpads[0];
            } // else {
          ...
          if ( !m1pad.isValid() ) {
            if ( m1pad.nX() >= 2 * m1pad.layout().xGrid() ) {
              ...
              // if (lpads.size()!=1) {
              //  std::cout <<"--- Tower::processTower: ERROR M1 PAD DOES NOT COVER EXACTLY ONE PAD IN UPPER
              //  REGION"<<std::endl;
              //}
              m1pad = lpads[0];
            } // else {
...
void L0Muon::Tower::cleanSeed( L0Muon::Tower::StationMap& map ) {
  ...
  for ( irow = map.begin(); irow != map.end(); irow++ ) {
    ...
    for ( boost::dynamic_bitset<>::size_type icol = ( *irow ).size(); icol >= 2; icol-- ) {
      if ( ( *irow ).test( icol - 2 ) ) { ( *irow ).reset( icol - 1 ); }
    }
  }
}
