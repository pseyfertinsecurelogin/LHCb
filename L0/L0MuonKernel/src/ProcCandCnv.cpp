...
int L0Muon::ProcCandCnv::decodeBank_v2( const std::vector<unsigned int>& raw ) {
  ...
  std::cout << "\t=> L0Muon::ProcCandCnv::decodeBank v2-- dump raw bank of size: " << raw.size() << std::hex
            << std::endl;
  ...
  for ( int ib = 0; ib < 12; ib++ ) { // Loop over processing boards
    ...
    for ( int ipu = 0; ipu < 4; ++ipu ) {
      ...
      if ( statusPU[ipu] > 2 )
        ...
        m_errors[ib].pus_status[ipu].set( 0 );
...
int L0Muon::ProcCandCnv::rawBank_v1( std::vector<unsigned int>& raw, int ievt ) {
  ...
  for ( int ib = 0; ib < 12; ib++ ) {
    ...
    for ( int ipu = 0; ipu < 4; ++ipu ) { word |= ( m_candRegHandlerPU[ib][ipu].getStatus() << ( 4 + ipu * 4 ) ); }
    raw.push_back( word );
...
int L0Muon::ProcCandCnv::rawBank_v2( std::vector<unsigned int>& raw, int ievt, bool compression ) {
  ...
  for ( int ib = 0; ib < 12; ib++ ) {
    ...
    // 2nd line : BCIDs
    word = bcid;
    for ( int ipu = 0; ipu < 4; ++ipu ) { word |= ( bcid << ( 4 + ipu * 4 ) ); }
    raw.push_back( word );
