...
void L0Muon::CtrlRawCnv::release() {
  ...
  for ( int iq = 0; iq < 2; ++iq ) {
    ...
    for ( int ib = 0; ib < 12; ++ib ) { m_candRegHandlerBCSU[iq][ib].clear(); }
  }
  m_activ = false;
}
...
std::vector<L0Muon::PMuonCandidate> L0Muon::CtrlRawCnv::muonCandidates() {
  ...
  for ( int iq = 0; iq < 2; ++iq ) {
    ...
    for ( int icand = 0; icand < ncand; icand++ ) { cands.push_back( m_candRegHandler[iq].getMuonCandidate( icand ) ); }
  }
...
void L0Muon::CtrlRawCnv::dump( const std::vector<unsigned int>& raw ) {
  ...
  std::cout << "  ";
  for ( int iq = 0; iq < 2; ++iq ) { std::cout << "    Q" << std::setw( 1 ) << m_side * 2 + iq + 1 << "    "; }
  std::cout << std::endl;
  ...
  for ( unsigned int iline = 0; iline < L0Muon::CtrlRawCnv::board_full_data_size; ++iline ) {
    ...
    for ( int iq = 0; iq < 2; ++iq ) {
      std::cout << " 0x" << std::setw( 8 ) << std::setfill( '0' )
                << raw[L0Muon::CtrlRawCnv::board_full_data_size * iq + iline];
    }
    std::cout << std::dec;
    std::cout << std::endl;
  }
...
void L0Muon::CtrlRawCnv::decodeBank( const std::vector<unsigned int>& raw, int bankVersion, int& Refl0EventNumber,
                                     int& Refl0_B_Id ) {
...
void L0Muon::CtrlRawCnv::decodeBank_v1( const std::vector<unsigned int>& raw, int& Refl0EventNumber, int& Refl0_B_Id ) {
  ...
  int decodingError[2];
  for ( int iq = 0; iq < 2; ++iq ) { decodingError[iq] = 0; }
  ...
  for ( int iq = 0; iq < 2; ++iq ) {
    ...
    for ( int ib = 4; ib < 12; ++ib ) {
      ...
      if ( empty != 0 ) {
        ++decodingError[iq]; // std::cout<<"L0Muon::CtrlRawCnv::decodeBank decodingError Q"<<m_side*2+iq<<"
                             // F"<<std::endl;
      }
    }
    ++isuwd;
    ...
    int ncand = m_candRegHandler[iq].numberOfCandidates();
    for ( int icand = 0; icand < ncand; ++icand ) { m_candRegHandler[iq].setCandQuarter( m_side * 2 + iq, icand ); }
...
void L0Muon::CtrlRawCnv::rawBank( std::vector<unsigned int>& raw, int bankVersion ) {
  ...
  for ( int iq = 0; iq < 2; ++iq ) {
    ...
    word = ( ( ( m_errors[iq].err_bcid.value() << 8 ) & 0x1 ) + ( ( m_errors[iq].cu_bcid.value() << 4 ) & 0xF0 ) +
    ...
    for ( int ib = 0; ib < 12; ++ib ) {
      ...
      if ( ib % 2 == 0 ) {
        ...
        raw.push_back( word );
        word = ( ( ( c1 << 28 ) & 0x10000000 ) + ( ( c2 << 24 ) & 0x1000000 ) + ( ( status << 20 ) & 0xF00000 ) +
                 ( ( bcid << 16 ) & 0xF0000 ) + ( cand[1] & 0x03FF ) );
        raw.push_back( word );
        word = 0;
      }
    ...
    for ( int icand = 0; icand < 2; ++icand ) {
      ...
      cand[icand]    = ( ( ( cand_board << 12 ) & 0xF000 ) + ( ( cand_pu << 8 ) & 0x300 ) +
                      ( ( cand_offM2 << 4 ) & 0xF0 ) + ( (cand_offM1)&0xF ) );
    }
