...
/**
   Constructor.
*/
L0Muon::CtrlCandCnv::CtrlCandCnv() : m_side( 0 ), m_ref_l0_B_Id( 0 ), m_ref_l0EventNumber( 0 ) {}
/**
   Constructor.
*/
L0Muon::CtrlCandCnv::CtrlCandCnv( int side ) : m_side( side ), m_ref_l0_B_Id( 0 ), m_ref_l0EventNumber( 0 ) {
  char buf[4096];
...
int L0Muon::CtrlCandCnv::decodeBank( const std::vector<unsigned int>& raw, int bankVersion, int mode ) {
  ...
  /// - 2nd part : input candidates received by the controller boards of 1 side (candidates sent by the processing
  /// boards)
  ///
    ...
    for ( int i = 0; i < 2; ++i ) { // Loop over the 2 words with the status
      ...
      for ( int ii = 0; ii < 6; ++ii ) { // Loop over the 4 boards in the current word
        ...
        if ( status > 2 )
          ...
          m_errors[iq].bcsu_status[iboard].set( 0 );
        ++iboard;
      } // End of loop over the 4 boards in the current word
...
int L0Muon::CtrlCandCnv::rawBank( std::vector<unsigned int>& raw, int ievt, int bankVersion, int mode,
                                  bool compression ) {
  raw.clear();
  if ( bankVersion < 2 ) return 0;
  if ( ievt < 0 ) return 0;
