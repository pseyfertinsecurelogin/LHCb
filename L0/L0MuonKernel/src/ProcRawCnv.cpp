...
void L0Muon::ProcRawCnv::release() {
  for ( int ib = 0; ib < 12; ib++ ) {
    ...
    for ( int ipu = 0; ipu < 4; ipu++ ) { m_candRegHandlerPU[ib][ipu].clear(); }
  }
  for ( int iboard = 0; iboard < 12; iboard++ ) {
    for ( int ipu = 0; ipu < 4; ipu++ ) {
...
void L0Muon::ProcRawCnv::dump( const std::vector<unsigned int>& raw ) {
  ...
  std::cout << "__ "
            << " - Q" << m_quarter + 1 << " ; bank size: " << raw.size() << std::endl;

  std::cout << "__ ";
  std::cout << "  ";
  for ( int ib = 0; ib < 12; ++ib ) { std::cout << "     " << std::setw( 2 ) << ib << "    "; }
  std::cout << std::endl;
  ...
  for ( unsigned int iline = 0; iline < L0Muon::ProcRawCnv::board_full_data_size; ++iline ) {
    ...
    for ( int ib = 0; ib < 12; ++ib ) {
      std::cout << " 0x" << std::setw( 8 ) << std::setfill( '0' )
                << raw[L0Muon::ProcRawCnv::board_full_data_size * ib + iline];
    }
    std::cout << std::dec;
    std::cout << std::endl;
  }
...
void L0Muon::ProcRawCnv::dumpErrorCounters( std::string& os ) {
  ...
  for ( int ib = 0; ib < 12; ++ib ) m_errors[ib].printCounters( os, ( boost::format( "\tPB%2d " ) % ib ).str() );
}

void L0Muon::ProcRawCnv::decodeBank( const std::vector<unsigned int>& original, int bankVersion, int& Refl0EventNumber,
                                     int& Refl0_B_Id ) {
...
void L0Muon::ProcRawCnv::decodeBank_v1( const std::vector<unsigned int>& original, int& Refl0EventNumber,
                                        int& Refl0_B_Id ) {
  ...
  //     std::cout
  //     <<std::setw(3)<<icount<<std::hex<<"\t0x"<<std::setw(8)<<std::setfill('0')<<(*itraw)<<std::dec<<std::endl;
  //     ++icount;
  //   }
  //   std::cout<<std::dec;
  //   std::cout.unsetf(std::ios::uppercase);
  ...
  for ( int ib = 0; ib < 12; ++ib ) { decodingError[ib] = 0; }
  ...
  for ( int pp = 0; pp < 4; ++pp ) {   // Loop over PPs
    for ( int bb = 0; bb < 3; ++bb ) { // 1st loop over boards in PP : candidates
      ...
      for ( int ich = 0; ich < 2; ++ich ) {
        ...
        std::cout << " L0Muon::ProcRawCnv::decodeBank l0EventNumber= " << l0EventNumber << " Ref=" << Refl0EventNumber
                  << std::endl;
#endif
...
#if _DEBUG_RAWDATA_2_ == 1
        std::cout << " L0Muon::ProcRawCnv::decodeBank ib= " << ib << " ipu= " << ipu << " error word= 0x" << std::hex
      ...
      for ( int ipu = 0; ipu < 4; ++ipu ) { // Loop over PUs in board : ol data
        ...
        if ( m_ols[ib][ipu] != 0 ) {
          ...
          std::cout << "L0Muon::ProcRawCnv::decodeBank OLs   bits found in"
                    << " buffer= " << olbitset.count() << " register= " << pads.size() << " model= " << nexpected;
    ...
    for ( int bb = 0; bb < 3; ++bb ) { // 3rd loop over boards in PP : neigh data
      ...
      for ( int ipu = 0; ipu < 4; ++ipu ) { // Loop over PUs in board : neigh data
#if _DEBUG_RAWDATA_2_ == 1
        std::cout << " L0Muon::ProcRawCnv::decodeBank neigh ib= " << ib << " ipu= " << ipu << " iwd= " << iwd
                  << std::endl;
#endif
        ...
        if ( m_neighs[ib][ipu] != 0 ) {
          ...
          //                 std::cout << "L0Muon::ProcRawCnv::decodeBank Neigh   !!! en moins :
          ...
          //             if (m_neighs[ib][ipu]->test(ii*16+iii)){
          //               if (!mids[ii*16+iii].isValid()) {
          //                 std::cout << "L0Muon::ProcRawCnv::decodeBank Neigh   !!! en plus  :
          //                 "<<mids[ii*16+iii].toString()
          //                           <<" @ pos"<<ii*16+iii
          //                           <<" word "<<ii<<" bit "<<iii<<std::endl;
          //               }
...
#  endif
        ...
        if ( m_neighs[ib][ipu] != 0 ) {
          ...
          std::cout << "L0Muon::ProcRawCnv::decodeBank Neigh   bits found in"
                    << " buffer= " << neighbitset.count() << " register= " << pads.size() << " model= " << nexpected;
...
void L0Muon::ProcRawCnv::rawBank( std::vector<unsigned int>& data, int bankVersion ) {
  ...
  for ( int ib = 0; ib < 12; ++ib ) {
    ...
    for ( int icand = 0; icand < 2; ++icand ) {
      ...
      add[icand] =
          ( ( rowM3 << 13 ) & 0x6000 ) + ( ( colM3 << 8 ) & 0x1F00 ) + ( ( offM2 << 4 ) & 0xF0 ) + ( offM1 & 0xF );
    ...
    word = ( ( add[1] << 16 ) & 0x7FFF0000 ) + ( ( pu[0] << 10 ) & 0xC00 ) + ( ( pu[1] << 8 ) & 0x200 ) +
           ( ( status << 4 ) & 0xF0 ) + ( bcid & 0xF );
    raw.push_back( word );
    ...
    for ( int ipu = 0; ipu < 4; ++ipu ) {
      ...
      if ( ipu % 2 == 0 ) {
        ...
        word    = ( ( ( add << 24 ) & 0xFF000000 ) + ( ( opt << 16 ) & 0xFF0000 ) + ( ( ser << 8 ) & 0xFF00 ) +
                 ( ( par << 0 ) & 0xFF ) );
...
void L0Muon::ProcRawCnv::reorderOLChannels( const std::vector<unsigned int>& raw,
