...
/**
   Constructor.
*/
L0Muon::ProcDataCnv::ProcDataCnv() : m_quarter( 0 ) {}
...
std::vector<LHCb::MuonTileID> L0Muon::ProcDataCnv::ols( LHCb::MuonTileID puid ) {
  ...
  if ( it != m_olsMap.end() ) {
    if ( ( *it ).second != 0 ) { pads = ( ( *it ).second )->firedTiles(); }
  }
  //   if (it==m_olsMap.end()) {
...
std::vector<LHCb::MuonTileID> L0Muon::ProcDataCnv::pus() {
  ...
  for ( itmap = m_olsMap.begin(); itmap != m_olsMap.end(); ++itmap ) { lpuids.push_back( itmap->first ); }
...
int L0Muon::ProcDataCnv::decodeBank_v1( const std::vector<unsigned int>& raw ) {
  ...
  for ( int iboard = 0; iboard < 12; iboard++ ) {
    ...
    for ( int ipu = 0; ipu < 4; ipu++ ) {
      ...
      bitset.resize( 17 * 16 );
      if ( m_neighs[iboard][ipu] != 0 ) m_neighs[iboard][ipu]->set( bitset );
    }
...
int L0Muon::ProcDataCnv::decodeBank_v2( const std::vector<unsigned int>& raw ) {
  ...
  std::cout << "L0Muon::ProcDataCnv::decodeBank (v2) "
            << " nheader= " << nheader << " nOLbytes= " << nOLbytes << " nOLwords= " << nOLwords
  ...
  for ( int ib = 0; ib < 12; ++ib ) { // Loop over processing boards
...
#if _DEBUG_PROCDATA_ > 0
    std::cout << "L0Muon::ProcDataCnv::decodeBank OLs board# " << ib << " BEFORE DECODING"
              << " bitset size= " << bitset.size() << " first_word =" << iwd << " first_bit =" << ibit << std::endl;
#endif
    if ( compressed )
    ...
    if ( ok < 0 ) {
      // ERROR
#if _DEBUG_PROCDATA_ > 0
      std::cout << "L0Muon::ProcDataCnv::decodeBank ERROR PBWords_to_bitset returned " << ok << " OLs board# " << ib
                << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
      for ( int jb = 0; jb < 12; ++jb ) m_errors[jb].decoding.set( decoding_error[jb] );
      return -1;
    }
#if _DEBUG_PROCDATA_ > 0
    std::cout << "L0Muon::ProcDataCnv::decodeBank OLs board# " << ib << " AFTER  DECODING"
              << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
    ...
    if ( ( ( ib + 1 ) % 3 ) == 0 ) { // take care of padding bits at the end of each PP
      ibit = 16 * ( ( ibit + 16 ) / 16 ) - 1;
#if _DEBUG_PROCDATA_ > 0
      std::cout << "L0Muon::ProcDataCnv::decodeBank OLs board# " << ib << " AFTER   PADDING"
                << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
  ...
  std::cout << "L0Muon::ProcDataCnv::decodeBank "
            << " OL decoding done ..."
            << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
  ...
  if ( iwd != ( nOLwords + nheader ) ) {
    ...
    std::cout << "L0Muon::ProcDataCnv::decodeBank ERROR  end of 1st part current word (" << iwd
              << ") doesn't match 1st part size (" << ( nOLwords + nheader ) << ")" << std::endl;
#endif
    for ( int jb = 0; jb < 12; ++jb ) m_errors[jb].decoding.set( decoding_error[jb] );
    return -1;
  }
  ...
  for ( int ib = 0; ib < 12; ++ib ) { // Loop over processing boards
    ...
    boost::dynamic_bitset<> bitset;
#if _DEBUG_PROCDATA_ > 0
    std::cout << "L0Muon::ProcDataCnv::decodeBank Neigh board# " << ib << " BEFORE DECODING"
              << " bitset size= " << bitset.size() << " first_word =" << iwd << " first_bit =" << ibit << std::endl;
#endif
    if ( compressed )
    ...
    if ( ok < 0 ) {
      // ERROR
#if _DEBUG_PROCDATA_ > 0
      std::cout << "L0Muon::ProcDataCnv::decodeBank ERROR PBWords_to_bitset returned " << ok << " Neigh board# " << ib
                << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
      for ( int jb = 0; jb < 12; ++jb ) m_errors[jb].decoding.set( decoding_error[jb] );
      return -2;
...
#if _DEBUG_PROCDATA_ > 0
    std::cout << "L0Muon::ProcDataCnv::decodeBank Neigh board# " << ib << " AFTER  DECODING"
              << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
    ...
    if ( ( ( ib + 1 ) % 3 ) == 0 ) { // take care of padding bits at the end of each PP
      ...
      ibit = 31;
#if _DEBUG_PROCDATA_ > 0
      std::cout << "L0Muon::ProcDataCnv::decodeBank Neigh board# " << ib << " AFTER   PADDING"
                << " bitset size= " << bitset.size() << " last_word  =" << iwd << " last_bit  =" << ibit << std::endl;
#endif
    } // end padding
  ...
  if ( iwd != raw.size() ) {
    ...
    std::cout << "L0Muon::ProcDataCnv::decodeBank ERROR  end of 2nd part current word (" << iwd
              << ") doesn't match raw bank size (" << raw.size() << ")" << std::endl;
#endif
    for ( int jb = 0; jb < 12; ++jb ) m_errors[jb].decoding.set( decoding_error[jb] );
    return -2;
  }
...
int L0Muon::ProcDataCnv::rawBank_v2( std::vector<unsigned int>& raw, int mode, bool compression ) {
      ...
      for ( int ich = 0; ich < 3; ++ich ) { // Loop over processing boards in PP FPGA
        ...
        for ( int ipu = 0; ipu < 4; ++ipu ) { // Loop over PUs OL data
          ...
          for ( unsigned int iwd = 0; iwd < PU_OpticalLinks_size_16 / 2; ++iwd ) { // Loop over PU OL words
            ...
            unsigned int word = m_ols[ib][ipu]->getulong( 32, ( PU_OpticalLinks_size_16 / 2 - iwd - 1 ) * 32 );
            // std::cout<<"L0Muon::ProcDataCnv::rawBank_v2 iwd: "<<iwd<<" word =
  ...
  if ( compression ) { header |= 0x80000000; }

  //   std::cout<<"L0Muon::ProcDataCnv::rawBank_v2 Q"<<(m_quarter+1)<<" 1st part size : "<<raw.size()<<"
  //   words"<<std::endl;

  raw.insert( raw.begin(), header );

  //   std::cout<<"L0Muon::ProcDataCnv::rawBank_v2 Q"<<(m_quarter+1)<<" including header : "<<raw.size()<<"
  //   words"<<std::endl;
...
boost::dynamic_bitset<> L0Muon::ProcDataCnv::applyCompression( boost::dynamic_bitset<> bitset_to_compress ) {
  ...
  leftover = nchar % maximumLength;
  size     = n0;
  if ( leftover > 0 ) { // if the file ends with a number of 0s which is a multiple of maximumLength, do not write the
                        // last sequence
    size += nbitsSequence;
    sequence         = ( leftover << 1 ) + 1;
    leftoverSequence = boost::dynamic_bitset<>( size, sequence );
...
boost::dynamic_bitset<> L0Muon::ProcDataCnv::unapplyCompression( boost::dynamic_bitset<>& compressed,
  ...
  if ( compressed.size() == 0 ) { return uncompressed; }
...
int L0Muon::ProcDataCnv::notcompressedPBWords_to_bitset( const std::vector<unsigned int>& raw, const int size_in_words,
                                                         unsigned int& iwd, boost::dynamic_bitset<>& bitset ) {
...
int L0Muon::ProcDataCnv::compressedPBWords_to_bitset( const std::vector<unsigned int>& raw, const int size_in_word,
                                                      unsigned int& iwd, int& ibit, boost::dynamic_bitset<>& bitset ) {
  ...
  boost::dynamic_bitset<> bitset_of_32_ones( 32 );
  bitset_of_32_ones.flip();
  boost::dynamic_bitset<> bitset_of_16_ones( 16 );
  bitset_of_16_ones.flip();

  for ( int iw = 0; iw < size_in_word; ++iw ) { // Loop over the number of original words
#if _DEBUG_PROCDATA_ > 1
    std::cout << "L0Muon::ProcDataCnv::compressedPBWords_to_bitset iwd= " << iwd << " word: 0x" << std::hex << raw[iwd]
              << std::dec << " ibit= " << ibit << std::endl;
#endif
...
#if _DEBUG_PROCDATA_ > 1
        std::cout << "L0Muon::ProcDataCnv::compressedPBWords_to_bitset     0x" << std::hex << debug_word << std::dec
                  << std::endl;
#endif
#if _DEBUG_PROCDATA_ > 1
        std::cout << "L0Muon::ProcDataCnv::compressedPBWords_to_bitset bitset size= " << bitset.size()
    ...
    std::cout << "L0Muon::ProcDataCnv::compressedPBWords_to_bitset decompressed word= 0x" << std::hex << debug_word
              << std::dec << std::endl;
#endif
...
void L0Muon::ProcDataCnv::word_to_compressedBitset( unsigned int word, boost::dynamic_bitset<>& bitset ) {
  if ( word == 0 ) {
    ...
    bitset.push_back( 1 );
    bitset.push_back( 0 );
    for ( int i = 0; i < 16; ++i ) { bitset.push_back( ( word >> ( 16 - 1 - i ) ) & 1 ); }
  } else {
    int  nones = 0;
    int  npos[5];
    ...
    if ( more_than_five ) {
      bitset.push_back( 1 );
      bitset.push_back( 1 );
      for ( int i = 0; i < 32; ++i ) { bitset.push_back( ( word >> ( 32 - 1 - i ) ) & 1 ); }
    } else {
      // std::cout <<"L0Muon::ProcDataCnv::word_to_compressedBitset word="<<word<<" = 0x"<<std::hex<<word<<std::dec
      //          <<" ; nones "<<nones<<std::endl;
      bitset.push_back( 1 );
      bitset.push_back( 0 );
      for ( int i = 0; i < 3; ++i ) { bitset.push_back( ( nones >> ( 3 - 1 - i ) ) & 1 ); }
      for ( int iones = 0; iones < nones; ++iones ) {
        for ( int i = 0; i < 5; ++i ) { bitset.push_back( ( npos[iones] >> ( 5 - 1 - i ) ) & 1 ); }
      }
    }
  }
  // std::cout <<"L0Muon::ProcDataCnv::word_to_compressedBitset word=0x"<<std::hex<<word<<std::dec<<" bitset
  // "<<bitset<<std::endl;
}
...
int L0Muon::ProcDataCnv::setRegisters_for_PB_OpticalLinks( int iboard, boost::dynamic_bitset<>& bitset ) {
  ...
  for ( int ipu = 0; ipu < 4; ipu++ ) { // Loop over PUs errors
    ...
    unsigned int word = bitset.to_ulong();
#if _DEBUG_PROCDATA_ > 2
    std::cout << "L0Muon::ProcDataCnv::decodeBank error word ipu= " << ipu << " 0x" << std::hex << word << std::dec
              << std::endl;
#endif
  ...
  for ( int ipu = 0; ipu < 4; ipu++ ) { // Loop over PUs
    ...
    if ( m_ols[iboard][ipu] != 0 ) m_ols[iboard][ipu]->set( pubitset );
    ...
    if ( m_ols[iboard][ipu] != 0 ) {
      ...
      std::cout << "L0Muon::ProcDataCnv::decodeBank OLs   bits found in"
                << " buffer= " << pubitset.count() << " register= " << pads.size() << " model= " << nexpected;
...
int L0Muon::ProcDataCnv::setRegisters_for_PB_Neighbours( int iboard, boost::dynamic_bitset<>& neighbitset ) {
  ...
  for ( int ipu = 0; ipu < 4; ipu++ ) { // Loop over PUs
    ...
    if ( m_neighs[iboard][ipu] != 0 ) m_neighs[iboard][ipu]->set( pubitset );
    ...
    if ( m_neighs[iboard][ipu] != 0 ) {
      ...
      //                 std::cout << "L0Muon::ProcDataCnv::decodeBank Neigh   !!! en moins :
      //                 "<<mids[ii*16+iii].toString()
      //                           <<" @ pos"<<ii*16+iii
      ...
      //             if (m_neighs[iboard][ipu]->test(ii*16+iii)){
      //               if (!mids[ii*16+iii].isValid()) {
      //                 std::cout << "L0Muon::ProcDataCnv::decodeBank Neigh   !!! en plus  :
      //                 "<<mids[ii*16+iii].toString()
      //                           <<" @ pos"<<ii*16+iii
      //                           <<" word "<<ii<<" bit "<<iii<<std::endl;
      //               }
